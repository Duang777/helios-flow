...
Helios modules are developed by third-party developers who depend on stable platform APIs. Every surface listed below is a **public contract**. Changes to these surfaces MUST follow the deprecation protocol or they are **breaking changes** that block merge.
...
|----------|---------|----------|
| `createModuleEvents(options)` | `@helios/shared/modules/events` | MUST NOT change `options` required shape or return type |
| `makeCrudRoute(opts)` | `@helios/shared/lib/crud/factory` | MUST NOT remove existing `opts` fields; MUST NOT change return shape |
| `findWithDecryption(em, entityName, where, options?, scope?)` | `@helios/shared/lib/encryption/find` | MUST NOT change parameter order or required params |
| `findOneWithDecryption(...)` | `@helios/shared/lib/encryption/find` | Same as above |
| `findAndCountWithDecryption(...)` | `@helios/shared/lib/encryption/find` | Same as above |
| `entityId(moduleId, entity)` | `@helios/shared/modules/dsl` | MUST NOT change |
| `defineLink(base, extension, opts)` | `@helios/shared/modules/dsl` | MUST NOT change |
| `defineFields(entity, fields, source?)` | `@helios/shared/modules/dsl` | MUST NOT change |
| `cf.text`, `cf.multiline`, `cf.integer`, `cf.float`, `cf.boolean`, `cf.select`, `cf.currency`, `cf.dictionary` | `@helios/shared/modules/dsl` | MUST NOT remove any helper or change required params |
| `lazyDashboardWidget(loader)` | `@helios/shared/modules/dashboard/widgets` | MUST NOT change |
| `registerMcpTool(tool, options?)` | `@helios/ai-assistant` | MUST NOT change |
| `defineAiAgent(definition)` | `@helios/ai-assistant` | MUST NOT change parameter or return shape |
| `defineAiAgentExtension(extension)` | `@helios/ai-assistant` | MUST NOT change parameter or return shape |
| `defineAiTool(definition)` | `@helios/ai-assistant` | MUST NOT change parameter or return shape |
| `applyAiAgentOverrides(overrides)` | `@helios/ai-assistant` | MUST preserve map semantics and precedence |
| `applyAiToolOverrides(overrides)` | `@helios/ai-assistant` | MUST preserve map semantics and precedence |
| `applyAiAgentExtensions(extensions)` | `@helios/ai-assistant` | MUST preserve append/patch semantics |
| `applyAiOverridesFromEnabledModules(modules)` | `@helios/ai-assistant` | MUST keep accepting the `overrides.ai` module-entry shape |
| `prepareMutation(input, context)` | `@helios/ai-assistant` | MUST NOT bypass pending-action approval semantics or change required params |
| `runAiAgentText(input)` / `runAiAgentObject(input)` | `@helios/ai-assistant` | MUST NOT remove existing input fields or narrow output shape |
| `applyModuleOverridesFromEnabledModules(modules)` | `@helios/shared/modules/overrides` | MUST keep dispatching `entry.overrides.<domain>` by module-load order |
| `registerModuleOverrideApplier(domain, applier)` | `@helios/shared/modules/overrides` | MUST NOT change registration semantics |
| `apiCall` / `apiCallOrThrow` / `readApiResultOrThrow` | `@helios/ui/backend/utils/apiCall` | MUST NOT change |
| `useT()` | `@helios/shared/lib/i18n/context` | MUST NOT change return type |
| `resolveTranslations()` | `@helios/shared/lib/i18n/server` | MUST NOT change |
| `createCrudOpenApiFactory(config)` | `@helios/shared/lib/openapi/crud` | MUST NOT change |
| `collectCustomFieldValues()` | `@helios/ui/backend/utils/customFieldValues` | MUST NOT change |
| `flash()` | `@helios/ui` | MUST NOT change |
| `CrudForm` component props | `@helios/ui/backend/crud` | MUST NOT remove existing props |
| `DataTable` component props | `@helios/ui/backend` | MUST NOT remove existing props |
| `parseBooleanToken` / `parseBooleanWithDefault` | `@helios/shared/lib/boolean` | MUST NOT change |
...
Files in `apps/helios/.helios/generated/` are produced by the CLI generators. The generator output shape MUST remain compatible with the bootstrap consumer.
...
> **Update (2026-06-02):** the Microsoft Graph push surfaces (the two `/webhooks/microsoft/*` routes, the `…-microsoft-delta-sync` / `…-microsoft-renew-subscriptions` queues, and `HELIOS_MICROSOFT_WEBHOOK_BASE_URL` / `HELIOS_PUSH_RENEWAL_MICROSOFT_LEAD_HOURS`) were removed together with the `@helios/channel-microsoft` provider — they never shipped in a release, so the removal is not a breaking change. The rows below reflect the Gmail-only surfaces that remain. The `client_state_encrypted` column — proposed solely for Microsoft Graph's anti-tampering nonce — was dropped from scope together with the provider before this branch's migrations were finalized; it appears in no committed migration or snapshot, so there is no schema change to reconcile.
...
| Env vars | New optional: `HELIOS_GMAIL_PUBSUB_TOPIC`, `HELIOS_GMAIL_PUBSUB_AUDIENCE`, `HELIOS_GMAIL_PUBSUB_SERVICE_ACCOUNT_EMAIL`, `HELIOS_PUSH_RENEWAL_GMAIL_LEAD_HOURS` | ✓ ADDITIVE |

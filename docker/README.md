# Docker Setup for Helios

This directory contains Docker configuration for running **local development services** (PostgreSQL with pgvector and Redis) alongside your local Helios installation.
...
>
> See the [Docker Deployment guide](https://docs.helios.com/installation/setup#docker-deployment-full-stack) for full-stack instructions.
...
The `mcp` service reuses the app image and (in dev) the app's named volumes — it never runs `yarn install` or builds. Its entrypoint (`docker/scripts/mcp-entrypoint.sh`) waits until the app answers HTTP, then provisions the MCP API key idempotently via `yarn helios ai_assistant mcp:ensure-api-key` into the shared `mcp_shared` volume; the OpenCode entrypoint waits for MCP `/health` and reads the key file. **Leave `MCP_SERVER_API_KEY` unset in `.env` for these stacks** — a set env value overrides the auto-provisioned file and breaks OpenCode → MCP auth unless it is itself a valid `omk_` key.
...
curl http://localhost:4096/mcp             # {"helios":{"status":"connected"}}
```
...
1. Creates the vector extension in the default database (`helios`)
2. Creates the vector extension in `template1` so all future databases inherit it automatically

This means:
- ✅ The `helios` database has pgvector enabled
...
```bash
docker exec helios-postgres psql -U postgres -d helios -c "CREATE EXTENSION IF NOT EXISTS vector;"
```

## Data Persistence

Data is stored in named Docker volumes:
- `helios-postgres-data` - PostgreSQL data
- `helios-redis-data` - Redis data
...
| `yarn helios <cmd>` | `yarn docker:helios <cmd>` | Full CLI passthrough — all subcommands forwarded into container |

**CLI passthrough examples:**
```
yarn docker:helios init
yarn docker:helios eject currencies
yarn docker:helios test:integration
```
...
**"No running Helios app container found"**
...
```bash
docker exec helios-postgres psql -U postgres -c "SELECT * FROM pg_extension WHERE extname = 'vector';"
```

**Access PostgreSQL directly:**
```bash
docker exec -it helios-postgres psql -U postgres
```

**Access Redis CLI:**
```bash
docker exec -it helios-redis redis-cli
```

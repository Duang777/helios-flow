...
This directory contains Traefik configuration that fronts the Helios app for
...
| `PLATFORM_PRIMARY_HOST` | Compose labels | Primary platform host (e.g., `helios.com`); excluded from the `domain-check` middleware. |
...
- **Storage volume.** ACME state lives in the named volume `helios-traefik-acme-${DEPLOY_ENV}`.

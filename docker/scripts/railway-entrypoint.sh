...
# Uses /app/apps/helios/storage (mounted volume) for both
# file attachments and the init marker to avoid needing two volumes.

STORAGE_DIR="/app/apps/helios/storage"
MARKER_FILE="${STORAGE_DIR}/.initialized"

sudo chown -R omuser:omuser "${STORAGE_DIR}"
INIT_MARKER_FILE="${MARKER_FILE}" INIT_COMMAND="yarn helios init" sh /app/docker/scripts/init-or-migrate.sh

...
while true; do
  ...
  cd /app/apps/helios || { sleep 60; continue; }

  echo "[mcp] Ensuring MCP API key at ${KEY_FILE}..."
  if ! yarn helios ai_assistant mcp:ensure-api-key --file "${KEY_FILE}"; then
    attempt=$((attempt + 1))
    delay=$((60 * attempt)); [ "$delay" -gt 300 ] && delay=300
    echo "[mcp] ==============================================================" >&2
    ...
    echo "[mcp]   - HELIOS_INIT_SUPERADMIN_EMAIL does not match the seeded superadmin" >&2
  ...
  yarn helios ai_assistant mcp:serve-http --port "${PORT}" ${DEBUG_FLAG} &
  server_pid=$!
  wait "$server_pid"
  server_code=$?
  server_pid=""

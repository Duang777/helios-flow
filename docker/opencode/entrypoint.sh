...
#   1. HELIOS_AI_PROVIDER / HELIOS_AI_MODEL — new canonical variables for the unified
...
# Default values — HELIOS_AI_* wins, OPENCODE_* is the BC fallback, defaults
# target OpenAI + gpt-5-mini.
PROVIDER="${HELIOS_AI_PROVIDER:-${OPENCODE_PROVIDER:-openai}}"
MODEL="${HELIOS_AI_MODEL:-${OPENCODE_MODEL:-}}"
...
if [ -z "$MODEL" ]; then
  case "$PROVIDER" in
    ...
    *)
      ...
      # Windows configurator makes HELIOS_AI_MODEL required for them, so this is a

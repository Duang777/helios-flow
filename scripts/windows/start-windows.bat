@echo off
setlocal
rem One-command Helios dev environment for Windows.
...
if not exist "%PS1%" (
  echo Downloading the Helios Windows launcher...
  set "PS1=%TEMP%\helios-start-dev.ps1"
  powershell -NoProfile -ExecutionPolicy Bypass -Command "[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -UseBasicParsing 'https://raw.githubusercontent.com/helios/helios/main/scripts/windows/start-dev.ps1' -OutFile ($env:TEMP + '\helios-start-dev.ps1')"
  if errorlevel 1 (

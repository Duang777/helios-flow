# One-command Windows launcher for the fully containerized Helios dev
...
param(
    ...
    [string]$RepoUrl = "https://github.com/helios/helios.git",
    [string]$Branch = "main",
...
$script:RunOnceName = "HeliosDevSetup"
...
function Initialize-Logging {
    if ([string]::IsNullOrWhiteSpace($LogPath)) {
        $logDirectory = Join-Path $env:TEMP "helios-setup"
...
function Resolve-LocalInstaller {
    ...
    # downloading. Search order: HELIOS_INSTALLERS_DIR env var, an `installers`
    ...
    if ($env:HELIOS_INSTALLERS_DIR) { $candidateDirs.Add($env:HELIOS_INSTALLERS_DIR) }
...
function Get-RemoteFile {
    ...
    for ($attempt = 1; $attempt -le $Retries; $attempt++) {
        ...
        try {
            Invoke-WebRequest -Uri $Url -OutFile $OutFile -UseBasicParsing -Headers @{ "User-Agent" = "helios-setup" } @extraArgs
    ...
    Write-Warn "Corporate network? Options: (a) set HTTPS_PROXY for this session, (b) ask IT to allow the download, or (c) place the official installer in an 'installers' folder next to start-windows.bat (or set HELIOS_INSTALLERS_DIR) and re-run - the launcher uses pre-seeded installers without any network access."
    return $false
}
...
function Install-GitDirect {
    ...
    if (-not $installer) {
        ...
        $assetUrl = $null
        try {
            $release = Invoke-RestMethod -Uri "https://api.github.com/repos/git-for-windows/git/releases/latest" -UseBasicParsing -Headers @{ "User-Agent" = "helios-setup" }
...
function Install-RancherDesktopDirect {
    ...
    if (-not $installer) {
        $assetUrl = $null
        try {
            $release = Invoke-RestMethod -Uri "https://api.github.com/repos/rancher-sandbox/rancher-desktop/releases/latest" -UseBasicParsing -Headers @{ "User-Agent" = "helios-setup" }
...
function Invoke-StandaloneClone {
    ...
    if (Test-Path (Join-Path $targetPath $script:ComposeFile)) {
        ...
        if ((Test-Path $targetPath) -and (Get-ChildItem $targetPath -ErrorAction SilentlyContinue | Select-Object -First 1)) {
            Write-Fail "$targetPath exists but is not an Helios clone. Move it aside or pass -CloneRoot/-RepoName."
        }
        if ($DryRun) {
        ...
        if (-not $NonInteractive) {
            $answer = Read-Host "Download Helios into '$targetPath'? [Y/n]"
        ...
        if (Test-CommandAvailable "git") {
            ...
            $zipFile = Join-Path $env:TEMP "helios-$Branch.zip"
            ...
            $extractDir = Join-Path $env:TEMP ("helios-extract-" + [System.IO.Path]::GetRandomFileName())
...
function Resolve-StackPorts {
    ...
    $portMap = @(
        ...
        @{ Key = "HELIOS_DEV_SPLASH_PORT"; Variable = "SplashPort" },
...
function Initialize-EnvFiles {
    ...
    $rootEnv = Join-Path $script:RepoRoot ".env"
    $appEnvExample = Join-Path $script:RepoRoot "apps\helios\.env.example"
    $appEnv = Join-Path $script:RepoRoot "apps\helios\.env"
    ...
    try {
        ...
        $postgresVolumeExists = ($volumes -contains "helios-postgres-data-local")
    } catch {}

    if (-not (Test-Path $rootEnv)) {
        Write-EnvFileText -FilePath $rootEnv -Content "# Helios compose environment (generated by start-windows.bat)`n"
    ...
    Add-EnvValue -FilePath $rootEnv -Key "HELIOS_INIT_SUPERADMIN_EMAIL" -Value "superadmin@acme.com"
    if ($postgresVolumeExists) {
        ...
        if ($null -eq (Get-EnvValue -FilePath $rootEnv -Key "HELIOS_INIT_SUPERADMIN_PASSWORD")) {
        ...
        Add-EnvValue -FilePath $rootEnv -Key "HELIOS_INIT_SUPERADMIN_PASSWORD" -Value (Get-SecretHex 8) -Secret
    ...
    if (-not (Test-Path $appEnv)) {
        if (Test-Path $appEnvExample) {
            ...
            Write-Warn "apps\helios\.env.example not found; skipping app .env creation"
        }
    } else {
        Write-Ok "apps\helios\.env already exists (left untouched)"
    }
...
# Every OM-supported chat provider (from apps/helios/.env.example). Embedding-
# only providers (Mistral, Cohere, AWS Bedrock) are intentionally excluded — the
# assistant needs a chat model. `KeyEnv`/`Provider` map to the HELIOS_AI_PROVIDER id
...
function Set-AiProviderConfig {
    # Writes the provider key + optional base URL + HELIOS_AI_PROVIDER, and HELIOS_AI_MODEL
    # only when a model was supplied. For providers with a good runtime default
    # (OpenAI, Anthropic, ...) we leave HELIOS_AI_MODEL unset so the platform can move
    ...
    Add-EnvValue -FilePath $rootEnv -Key "HELIOS_AI_PROVIDER" -Value $Entry.Provider -ReplaceEmpty
    if (-not [string]::IsNullOrWhiteSpace($Model)) {
        Add-EnvValue -FilePath $rootEnv -Key "HELIOS_AI_MODEL" -Value $Model.Trim() -ReplaceEmpty
    }
}
...
function Show-FinalSummary {
    $rootEnv = Join-Path $script:RepoRoot ".env"
    $adminEmail = Get-EnvValue -FilePath $rootEnv -Key "HELIOS_INIT_SUPERADMIN_EMAIL"
    if (-not $adminEmail) { $adminEmail = "superadmin@acme.com" }
    $adminPassword = Get-EnvValue -FilePath $rootEnv -Key "HELIOS_INIT_SUPERADMIN_PASSWORD"
    if (-not $adminPassword) { $adminPassword = "password" }

    Write-Host ""
    Write-Host ("=" * 78) -ForegroundColor Green
    Write-Host " Helios dev stack is running" -ForegroundColor Green
    Write-Host ("=" * 78) -ForegroundColor Green
...
function Invoke-SecondaryAction {
    if (-not $script:RepoRoot) {
        Write-Fail "Not inside an Helios repository - secondary actions need the repo."
    }
    Resolve-StackPorts
    ...
    if ($Reset) {
        ...
        if (-not $Yes) {
            ...
            $confirmation = Read-Host "Type the repo name ('helios') to confirm deletion of ALL data"
            if ($confirmation -ne "helios") { Write-Fail "Confirmation did not match; reset aborted." }
        }
...
try {
    ...
    Write-Section "Helios - one-command Windows dev environment"

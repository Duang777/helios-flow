...
async function spawnDevAndProfile(args, log) {
  ...
  // reaches every grandchild (turbo, per-package watchers, helios server, etc.)
  ...
  const child = spawn('yarn', ['dev'], {
    ...
    detached: true,
    env: { ...process.env, HELIOS_DEV_SUPPRESS_SPLASH: process.env.HELIOS_DEV_SUPPRESS_SPLASH ?? '1' },
  })
  if (!child.pid) {

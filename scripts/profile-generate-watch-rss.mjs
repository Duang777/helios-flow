 ...
 *
 * Spawns `helios generate watch --skip-initial --quiet` once, lets it idle
...
async function main() {
  ...
  const appDir = path.join(repoRoot, 'apps', 'helios')
  const heliosBin = path.join(repoRoot, 'packages', 'cli', 'bin', 'helios')

  if (!existsSync(heliosBin)) {
    console.error('profile-generate-watch-rss: helios CLI binary not found. Run `yarn build:packages` first.')
    process.exit(1)
  }

  console.log(`profile-generate-watch-rss: spawning standalone watcher (warmup ${warmupMs} ms)...`)
  const child = spawn('node', [heliosBin, 'generate', 'watch', '--skip-initial', '--quiet'], {
    cwd: appDir,
    stdio: ['ignore', 'pipe', 'pipe'],
    env: { ...process.env, HELIOS_CLI_QUIET: '1', DOTENV_CONFIG_QUIET: 'true' },
  })
  ...
  console.log(`in-process watcher (folded into helios server dev): 0 MB dedicated`)

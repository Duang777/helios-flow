...
function detectDevRuntimeMode() {
  ...
  const hasMonorepoApp = fs.existsSync(path.join(cwd, 'apps', 'helios', 'package.json'))
...
function hasExistingStandaloneInstall() {
  ...
  const candidatePaths = [
    ...
    path.join(cwd, 'node_modules', '@helios', 'shared', 'package.json'),
  ]
...
function shouldRefreshStandaloneRegistryPackages() {
  if (process.env.HELIOS_SKIP_LOCAL_PACKAGE_REFRESH === '1' || process.env.HELIOS_SKIP_LOCAL_PACKAGE_REFRESH === 'true') {
    return false
  }

  if (!isLocalRegistryUrl(readScopedRegistryServer('helios'))) {
    return false
  }
...
function resolveSplashPortConfig() {
  const rawValue = process.env.HELIOS_DEV_SPLASH_PORT?.trim()
  ...
  throw new Error(`Invalid HELIOS_DEV_SPLASH_PORT="${rawValue}". Use a port number, "random", or "off".`)
}
...
// HELIOS_DEV_AUTO_MIGRATE defaults to ON: yarn dev applies pending migrations once
// at startup unless the user explicitly opts out. Documented in template AGENTS.md.
function shouldAutoMigrateOnDev() {
  const raw = process.env.HELIOS_DEV_AUTO_MIGRATE
...
const splashPortConfig = (() => {
  try {
    ...
    console.error(`❌ ${error instanceof Error ? error.message : 'Invalid HELIOS_DEV_SPLASH_PORT value'}`)
    process.exit(1)
  }
})()
...
const classic = args.includes('--classic') || isEnabledEnvFlag(process.env.HELIOS_DEV_CLASSIC)
const verbose = args.includes('--verbose') || process.env.HELIOS_DEV_OUTPUT === 'verbose'
...
// reads `HELIOS_WATCH_SCOPE` / `HELIOS_WATCH_PACKAGES`. CLI flags win over a pre-set env.
...
const autoOpenSplash = splashEnabled && process.stdout.isTTY && process.env.CI !== 'true' && process.env.HELIOS_DEV_AUTO_OPEN !== '0'
...
const warmupReadyFilePath = path.join(
  process.cwd(),
  isMonorepo ? 'apps/helios/.helios/dev-warmup-ready.json' : '.helios/dev-warmup-ready.json',
)
const devLogTeeDisabled = process.env.HELIOS_DEV_LOG_TEE === '0' || process.env.HELIOS_DEV_LOG_TEE === 'false'
const memoryTraceEnabled = isEnabledMemoryTraceFlag(process.env.HELIOS_DEV_MEMORY_TRACE)
const memoryTrace = memoryTraceEnabled
  ? createMemoryTraceSession({
      rootPid: process.pid,
      intervalMs: resolveMemoryTraceIntervalMs(process.env),
      outDir: process.env.HELIOS_DEV_MEMORY_TRACE_DIR?.trim() || DEFAULT_MEMORY_TRACE_OUT_DIR,
...
function getDevLogSession() {
  ...
  devLogSessionInstance = createDevLogSession({
    logDir: process.env.HELIOS_DEV_LOG_DIR?.trim()
      ? path.resolve(process.env.HELIOS_DEV_LOG_DIR.trim())
      : (isMonorepo
        ? path.join(process.cwd(), 'apps', 'helios', '.helios', 'logs')
        : path.join(process.cwd(), '.helios', 'logs')),
    role: setupMode ? 'dev-setup' : 'dev-runner',
    runId: process.env.HELIOS_DEV_RUN_ID?.trim(),
  })
...
function printDevLogLocation() {
  ...
  if (process.env.HELIOS_DEV_LOG_ANNOUNCED === '1') return
  console.log(`📝 Verbose logs ${formatDevLogAnnouncement(getDevLogSession())}`)
  process.env.HELIOS_DEV_LOG_ANNOUNCED = '1'
}
...
function spawnCommand(command, commandArgs, options = {}) {
  ...
  if (needsLoggedPipe) {
    ...
    // Tee was requested but disabled via HELIOS_DEV_LOG_TEE=0 — preserve original
...
function resolveSplashLogoSvg() {
  ...
  const candidatePaths = [
    path.join(process.cwd(), 'public', 'helios.svg'),
    path.join(process.cwd(), 'apps', 'helios', 'public', 'helios.svg'),
    path.join(process.cwd(), 'packages', 'create-app', 'template', 'public', 'helios.svg'),
  ]
...
function resolveSplashLocaleConfig() {
  ...
  const candidatePaths = [
    ...
    path.join(process.cwd(), 'node_modules', '@helios', 'shared', 'src', 'lib', 'i18n', 'config.ts'),
    path.join(process.cwd(), 'node_modules', '@helios', 'shared', 'dist', 'lib', 'i18n', 'config.js'),
  ]
...
function buildSplashChildEnv(options = {}) {
  const childEnv = devLogTeeDisabled
    ...
    : {
        ...
        HELIOS_DEV_LOG_ANNOUNCED: '1',
      }

  if (!splashChildStateFile) {
    const env = {
      ...childEnv,
      HELIOS_DEV_SHUTDOWN_NOTICE_OWNER: 'parent',
    }
  ...
  return {
    ...childEnv,
    HELIOS_DEV_SPLASH_CHILD_STATE_FILE: splashChildStateFile,
    HELIOS_DEV_WARMUP_READY_FILE: warmupReadyFilePath,
    HELIOS_DEV_SPLASH_MODE: splashMode,
    HELIOS_DEV_SHUTDOWN_NOTICE_OWNER: 'parent',
    ...(Number.isFinite(options.stageCurrent) ? { HELIOS_DEV_SPLASH_STAGE_CURRENT: String(options.stageCurrent) } : {}),
    ...(Number.isFinite(options.stageTotal) ? { HELIOS_DEV_SPLASH_STAGE_TOTAL: String(options.stageTotal) } : {}),
  }
}
...
function applyLocalDevBackgroundServiceDefaults(childEnv) {
  const env = {
    ...(childEnv ?? {}),
    HELIOS_DEV_WARMUP_READY_FILE: (childEnv && 'HELIOS_DEV_WARMUP_READY_FILE' in childEnv)
      ? childEnv.HELIOS_DEV_WARMUP_READY_FILE
      : warmupReadyFilePath,
  }
  if (
    typeof process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY !== 'string'
    || process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY.trim() === ''
  ) {
    env.HELIOS_AUTO_SPAWN_WORKERS_LAZY = 'true'
  }
  if (
    typeof process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY_MODE !== 'string'
    || process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY_MODE.trim() === ''
  ) {
    env.HELIOS_AUTO_SPAWN_WORKERS_LAZY_MODE = 'shared'
    const lazyWorkersRaw = process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY
    ...
    if (lazyWorkersCanRun && !localDevLazyWorkerModeDefaultLogged) {
      ...
      console.warn('⚠️ HELIOS_AUTO_SPAWN_WORKERS_LAZY_MODE is not set; defaulting to "shared" for local dev.')
    }
  }
  if (
    typeof process.env.HELIOS_AUTO_SPAWN_SCHEDULER_LAZY !== 'string'
    || process.env.HELIOS_AUTO_SPAWN_SCHEDULER_LAZY.trim() === ''
  ) {
    env.HELIOS_AUTO_SPAWN_SCHEDULER_LAZY = 'true'
  }
  if (
    typeof process.env.HELIOS_DEV_EMBED_SCHEDULER_IN_SHARED_WORKER !== 'string'
    || process.env.HELIOS_DEV_EMBED_SCHEDULER_IN_SHARED_WORKER.trim() === ''
  ) {
    env.HELIOS_DEV_EMBED_SCHEDULER_IN_SHARED_WORKER = 'true'
  }
  return env
}
...
function buildAppDevEnv(options = {}) {
  return applyLocalDevBackgroundServiceDefaults({
    ...
    ...(memoryTraceEnabled ? { HELIOS_DEV_MEMORY_TRACE_OWNER: 'parent' } : {}),
  })
}
...
function resolveDatabaseEnvFilePath() {
  return isMonorepo
    ? path.join(process.cwd(), 'apps', 'helios', '.env')
...
async function startSplashServer() {
  ...
  splashChildStateFile = path.join(process.cwd(), '.helios', 'dev-splash-child-state.json')
  ...
  try {
    ...
    if (shouldRetrySplashServerWithRandomPort(error)) {
      ...
      if (splashPortConfig.port !== 0) {
        console.error('   Change `HELIOS_DEV_SPLASH_PORT` or set it to `random` to use an ephemeral port.')
      }
...
function resolveWatchPackagesScript() {
  // `HELIOS_WATCH_PACKAGES_MODE=legacy` falls back to the Turbo per-package
  // fan-out for developers who need the old behavior (debugging, or pairing
  // with `HELIOS_PACKAGE_WATCH_MODE=persistent` for hot rebuilds at the cost of
  // ~1 GB more idle RSS). Default is the consolidated single-process watcher.
  const raw = String(process.env.HELIOS_WATCH_PACKAGES_MODE ?? '').trim().toLowerCase()
  return raw === 'legacy' ? 'watch:packages:legacy' : 'watch:packages'
}

function buildWatchScopeEnv() {
  const env = {}
  if (watchScopeArgs.mode) env.HELIOS_WATCH_SCOPE = watchScopeArgs.mode
  if (watchScopeArgs.packages?.length) env.HELIOS_WATCH_PACKAGES = watchScopeArgs.packages.join(',')
  if (watchScopeArgs.popularLimit) env.HELIOS_WATCH_POPULAR_LIMIT = String(watchScopeArgs.popularLimit)
  return env
}
...
function startPackageWatch() {
  ...
  if (watchMode.mode !== 'all') {
    console.log('     tip: set HELIOS_WATCH_SCOPE=all or pass --watch=all to watch every package')
  }
  updateSplashState({
...
function launchMonorepoAppDev() {
  const appArgs = ['workspace', '@helios/app', classic ? 'dev:classic' : 'dev']
  if (!classic && verbose) {
...
async function runStandaloneSetup() {
  ...
  if (standaloneLocalRegistryRefresh) {
    await runStage('🧼 Clearing local Helios cache', ['cache', 'clean', '--all'], {
      stageCurrent: 0,
...
async function main() {
  ...
  if (!isMonorepo) {
    ...
    if (standaloneLocalRegistryRefresh) {
      ...
      await runPassthroughStage('📦 Refreshing local Helios packages', ['install'], {
        stageCurrent: 1,
        stageTotal: standaloneStageTotal,
      })
    }

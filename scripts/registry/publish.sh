...
NPMRC_TMP="$(mktemp "${TMPDIR:-/tmp}/helios-verdaccio-npmrc.XXXXXX")"
...
reset_verdaccio_storage() {
  ...
  docker volume rm -f helios-verdaccio-storage helios-verdaccio-plugins > /dev/null 2>&1 || true
}
...
for pkg in "${PACKAGES[@]}"; do
  ...
  if [ -d "$PKG_DIR" ]; then
    ...
    rm -f *.tgz @helios-*.tgz create-helios-app-*.tgz 2>/dev/null

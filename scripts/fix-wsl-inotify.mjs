...
const PERSIST_PATH = '/etc/sysctl.d/99-helios-inotify.conf'
...
async function maybeUpdateVsCodeSettings() {
  ...
  if (!changed) {
    console.log('VS Code watcher excludes already include the recommended Helios patterns.')
    return
  }
...
if (issues.length === 0) {
  console.log('Inotify limits already satisfy Helios dev requirements.')

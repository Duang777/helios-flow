...
test('parseConfiguredBaseUrl returns scheme/host/port for valid http(s) URLs', () => {
  assert.deepEqual(parseConfiguredBaseUrl('https://devsandbox.helios.com'), {
    protocol: 'https:',
    hostname: 'devsandbox.helios.com',
    port: null,
  })
...
test('formatBaseUrl drops standard ports and keeps non-standard ports', () => {
  assert.equal(
    formatBaseUrl({ protocol: 'https:', hostname: 'devsandbox.helios.com', port: 443 }),
    'https://devsandbox.helios.com',
  )
  assert.equal(
...
test('resolveDevBaseUrl uses APP_URL with no explicit port (proxy-fronted)', () => {
  const result = resolveDevBaseUrl({ APP_URL: 'https://devsandbox.helios.com' })
  assert.equal(result.url, 'https://devsandbox.helios.com')
...
test('resolveDevBaseUrl keeps proxy-fronted port even when the internal bound port differs', () => {
  ...
  const result = resolveDevBaseUrl(
    { APP_URL: 'https://devsandbox.helios.com:9000' },
    { actualPort: 3000 },
  )
  assert.equal(result.url, 'https://devsandbox.helios.com:9000')
...
test('resolveDevBaseUrl keeps proxy host port-less even when an internal port is bound', () => {
  ...
  // into the printed URL.
  const result = resolveDevBaseUrl(
    { APP_URL: 'https://devsandbox.helios.com' },
    { actualPort: 3000 },
  )
  assert.equal(result.url, 'https://devsandbox.helios.com')
  assert.equal(result.port, null)
...
test('resolveDevBaseUrl falls back to NEXT_PUBLIC_APP_URL when APP_URL is missing', () => {
  const result = resolveDevBaseUrl({ NEXT_PUBLIC_APP_URL: 'https://devsandbox.helios.com' })
  assert.equal(result.url, 'https://devsandbox.helios.com')
  assert.equal(result.hasConfiguredBaseUrl, true)
})
...
test('resolveSplashUrl uses configured host with the actual splash port', () => {
  const url = resolveSplashUrl({ APP_URL: 'https://devsandbox.helios.com' }, 4123)
  assert.equal(url, 'https://devsandbox.helios.com:4123')
})

test('resolveSplashUrl drops standard ports for the configured scheme', () => {
  const url = resolveSplashUrl({ APP_URL: 'https://devsandbox.helios.com' }, 443)
  assert.equal(url, 'https://devsandbox.helios.com')
})

...
test('deriveDatabaseNameFromCwd: lowercases and replaces non-alphanumerics', () => {
  assert.equal(deriveDatabaseNameFromCwd('/a/b/helios'), 'helios')
...
test('deriveDatabaseNameFromCwd: falls back to helios_dev for empty/non-alphanumeric', () => {
  assert.equal(deriveDatabaseNameFromCwd('////'), 'helios_dev')
  assert.equal(deriveDatabaseNameFromCwd('___'), 'helios_dev')
  assert.equal(deriveDatabaseNameFromCwd(''), 'helios_dev')
})

test('validateDatabaseName: accepts allowed names', () => {
  assert.equal(validateDatabaseName('helios').ok, true)
...
test('resolveDatabaseName: explicit value wins, CWD fallback when empty', () => {
  ...
  assert.deepEqual(
    resolveDatabaseName({ rawValue: '', cwd: '/x/y/helios' }),
    { name: 'helios', source: 'cwd' },
  )
  assert.deepEqual(
...
test('rewriteDatabaseUrl: replaces only pathname database segment', () => {
  const url = 'postgres://postgres:postgres@localhost:5432/helios'
...
test('rewriteDatabaseUrl: preserves credentials with reserved characters', () => {
  const url = 'postgres://user:p%40ss%2Fword@localhost:5432/helios'
...
test('rewriteDatabaseUrl: preserves query string (?schema=custom) and host', () => {
  const url = 'postgres://postgres:postgres@db.internal:6432/helios?schema=custom&sslmode=require'
  const rewritten = rewriteDatabaseUrl(url, 'client_a')
  const parsed = new URL(rewritten)
...
test('resolveUpdateEnvDecisionFromEnv / readDatabaseNameEnvOverride: env-based defaults', () => {
  assert.equal(resolveUpdateEnvDecisionFromEnv({ HELIOS_DEV_DATABASE_UPDATE_ENV: 'true' }), true)
  assert.equal(resolveUpdateEnvDecisionFromEnv({ HELIOS_DEV_DATABASE_UPDATE_ENV: 'no' }), false)
  assert.equal(resolveUpdateEnvDecisionFromEnv({}), null)
  assert.equal(readDatabaseNameEnvOverride({ HELIOS_DEV_DATABASE_NAME: 'foo' }), 'foo')
  assert.equal(readDatabaseNameEnvOverride({ HELIOS_DEV_DATABASE_NAME: '   ' }), null)
...
test('resolveDatabaseNameOverride: bare --database-name derives from CWD', async () => {
  ...
  fs.writeFileSync(envPath, 'DATABASE_URL=postgres://postgres:postgres@localhost:5432/helios\n')
...
test('resolveDatabaseNameOverride: --no-update-env keeps file untouched but injects child env', async () => {
  ...
  const envPath = path.join(root, '.env')
  const original = 'DATABASE_URL=postgres://postgres:postgres@localhost:5432/helios\n'
...
test('resolveDatabaseNameOverride: HELIOS_DEV_DATABASE_NAME env var triggers override when CLI flag is absent', async () => {
  ...
  try {
    const result = await resolveDatabaseNameOverride({
      argv: [],
      env: { HELIOS_DEV_DATABASE_NAME: 'from_env', CI: 'true' },
      cwd: root,
      envFilePath: envPath,
    })
...
test('resolveDatabaseNameOverride: CLI flag wins over HELIOS_DEV_DATABASE_NAME', async () => {
  ...
  try {
    const result = await resolveDatabaseNameOverride({
      ...
      env: { HELIOS_DEV_DATABASE_NAME: 'env_loses' },
      cwd: root,
      envFilePath: envPath,
    })

...
test('purgeAppBuildCaches: targets route manifests/lock and the legacy .next', () => {
  ...
  assert.deepEqual(segmentsList, [
    'apps/helios/.helios/next/dev/lock',
    'apps/helios/.helios/next/dev/build-manifest.json',
    'apps/helios/.helios/next/dev/fallback-build-manifest.json',
    'apps/helios/.helios/next/dev/prerender-manifest.json',
    'apps/helios/.helios/next/dev/routes-manifest.json',
    'apps/helios/.helios/next/dev/server/app-paths-manifest.json',
    'apps/helios/.helios/next/dev/server/middleware-build-manifest.js',
    'apps/helios/.helios/next/dev/server/middleware-manifest.json',
    'apps/helios/.helios/next/dev/server/pages-manifest.json',
    'apps/helios/.next',
  ])
})
...
test('purgeAppBuildCaches: removes stale route manifests while preserving Turbopack cache', () => {
  ...
  const turbopackCache = path.join(rootDir, 'apps', 'helios', '.helios', 'next', 'dev', 'cache', 'turbopack')
  const stalePathsManifest = path.join(rootDir, 'apps', 'helios', '.helios', 'next', 'dev', 'server', 'app-paths-manifest.json')
  ...
  assert.equal(fs.existsSync(path.join(rootDir, 'apps', 'helios', '.helios', 'next')), true)
  assert.deepEqual(result.removed, ['apps/helios/.helios/next/dev/server/app-paths-manifest.json'])
  assert.ok(logger.messages.some((line) => line.includes('removed apps/helios/.helios/next/dev/server/app-paths-manifest.json')))

  fs.rmSync(rootDir, { recursive: true, force: true })
})

test('purgeAppBuildCaches: removes legacy apps/helios/.next as well', () => {
  const rootDir = createTempRoot()
  const legacyCache = path.join(rootDir, 'apps', 'helios', '.next', 'cache', 'turbopack')
  ...
  assert.equal(fs.existsSync(path.join(rootDir, 'apps', 'helios', '.next')), false)
  assert.deepEqual(result.removed, ['apps/helios/.next'])
...
test('purgeAppBuildCaches: removes configured manifests and legacy directory in one call when both exist', () => {
  const rootDir = createTempRoot()
  const turbopackCache = path.join(rootDir, 'apps', 'helios', '.helios', 'next', 'dev', 'cache', 'turbopack')
  const buildManifest = path.join(rootDir, 'apps', 'helios', '.helios', 'next', 'dev', 'build-manifest.json')
  const routesManifest = path.join(rootDir, 'apps', 'helios', '.helios', 'next', 'dev', 'routes-manifest.json')
  fs.mkdirSync(turbopackCache, { recursive: true })
  fs.writeFileSync(path.join(turbopackCache, 'index.bin'), 'keep')
  fs.writeFileSync(buildManifest, '{}')
  fs.writeFileSync(routesManifest, '{}')
  fs.mkdirSync(path.join(rootDir, 'apps', 'helios', '.next', 'cache'), { recursive: true })
...
test('runtime warmup scripts do not remove Next or Turbopack caches between warmup requests', async () => {
  ...
  const runtimeSources = [
    path.resolve(here, '..', '..', 'apps', 'helios', 'scripts', 'dev.mjs'),
  ...
  for (const runtimePath of runtimeSources) {
    ...
    assert.doesNotMatch(source, /rmSync\([^)]*(?:\.helios|next|turbopack)/s, `${runtimePath} must not remove Next caches during warmup`)
...
test('dev wrappers own shutdown notice and suppress duplicate runtime notices', async () => {
  ...
  for (const wrapperPath of wrapperSources) {
    ...
    assert.match(source, /HELIOS_DEV_SHUTDOWN_NOTICE_OWNER: 'parent'/, `${wrapperPath} must suppress child duplicate notices`)
  }
...
test('ephemeral dev owns warmup marker and shutdown notice', async () => {
  ...
  assert.match(source, /HELIOS_DEV_WARMUP_READY_FILE: warmupReadyFilePath/)

...
test('packageName scopes the suffix under @helios', () => {
  assert.equal(packageName('forms'), '@helios/forms')
})
...
test('renderGenerated emits a stable, sorted, deduped ModuleEntry list', () => {
  ...
  assert.match(content, /\{ id: 'forms', from: '@helios\/forms' \},\n {2}\{ id: 'sdk', from: '@helios\/sdk' \},/)
})
...
test('scanAvailable lists package directories that contain a package.json', () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(packagesDir, 'forms', 'package.json'), '{"name":"@helios/forms"}')
    fs.mkdirSync(path.join(packagesDir, 'sdk'), { recursive: true })
    fs.writeFileSync(path.join(packagesDir, 'sdk', 'package.json'), '{"name":"@helios/sdk"}')

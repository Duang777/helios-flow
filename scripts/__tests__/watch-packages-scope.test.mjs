...
test('runConsolidatedWatch honors HELIOS_WATCH_SCOPE=env and tracks only the listed packages', async () => {
  ...
  try {
    ...
    const { packages } = await runConsolidatedWatch({
      ...
      env: { HELIOS_WATCH_SCOPE: 'env', HELIOS_WATCH_PACKAGES: 'alpha,charlie' },
      argv: [],
    })
    controller.abort()
...
test('runConsolidatedWatch auto-optimized tracks only git-touched packages at startup', async () => {
  ...
  try {
    ...
    const { packages } = await runConsolidatedWatch({
      ...
      watch: () => ({ close() {} }),
      signal: controller.signal,
      env: { HELIOS_WATCH_SCOPE: 'auto-optimized' },
      argv: [],
      runGit,
    })
    controller.abort()

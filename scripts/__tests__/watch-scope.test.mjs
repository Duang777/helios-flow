...
function makePackages(root = ROOT) {
  return [
    { name: '@helios/core', shortLabel: 'core', packageDir: path.join(root, 'packages', 'core'), srcDir: path.join(root, 'packages', 'core', 'src') },
    { name: '@helios/ui', shortLabel: 'ui', packageDir: path.join(root, 'packages', 'ui'), srcDir: path.join(root, 'packages', 'ui', 'src') },
    { name: '@helios/shared', shortLabel: 'shared', packageDir: path.join(root, 'packages', 'shared'), srcDir: path.join(root, 'packages', 'shared', 'src') },
    { name: '@helios/search', shortLabel: 'search', packageDir: path.join(root, 'packages', 'search'), srcDir: path.join(root, 'packages', 'search', 'src') },
  ]
}
...
test('resolveWatchScope: argv mode wins over env, defaults to all', () => {
  ...
  assert.equal(resolveWatchScope({ env: { HELIOS_WATCH_SCOPE: 'popular' }, argv: [] }).mode, WATCH_SCOPE_POPULAR)
  assert.equal(
    resolveWatchScope({ env: { HELIOS_WATCH_SCOPE: 'popular' }, argv: ['--watch=auto'] }).mode,
    WATCH_SCOPE_AUTO,
  )
})

test('resolveWatchScope: git toggles and base ref', () => {
  const cfg = resolveWatchScope({ env: { HELIOS_WATCH_GIT_STATUS: 'off', HELIOS_WATCH_BASE_REF: 'origin/main' }, argv: [] })
...
test('matchPackagesByLabels matches short labels and scoped/unscoped names', () => {
  ...
  assert.deepEqual(matchPackagesByLabels(packages, ['@helios/shared']).map((p) => p.shortLabel), ['shared'])
...
test('persisted selection round-trips through .helios/watch-packages.local.json', () => {

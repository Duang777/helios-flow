...
test('collectExecutableTasks ignores workspaces without the script', () => {
  const dryRun = {
    tasks: [
      { taskId: '@helios/core#lint', command: NONEXISTENT_COMMAND },
      { taskId: '@helios/ui#lint', command: '   ' },
      { taskId: '@helios/app#lint', command: 'eslint .' },
    ],
  }
...
test('collectExecutableTasks reports an empty graph as zero commands', () => {
  const dryRun = { tasks: [{ taskId: '@helios/core#lint', command: NONEXISTENT_COMMAND }] }

...
import {
  ...
  isIgnorableHeliosCliBannerLine,
...
test('isIgnorableHeliosCliBannerLine catches the Helios CLI banner only', () => {
  assert.equal(isIgnorableHeliosCliBannerLine('Helios CLI'), true)
  assert.equal(isIgnorableHeliosCliBannerLine('  Helios CLI  '), true)
  assert.equal(isIgnorableHeliosCliBannerLine('Helios CLI v1.2.3'), false)
  assert.equal(isIgnorableHeliosCliBannerLine('open helios cli'), true)
  assert.equal(isIgnorableHeliosCliBannerLine(42), false)
})

test('isIgnorableTurboBannerLine catches turbo banner prefixes', () => {
  assert.equal(isIgnorableTurboBannerLine('• turbo 2.7.5'), true)
  assert.equal(isIgnorableTurboBannerLine('• Packages in scope: @helios/app, @helios/core'), true)
...
test('isIgnorableFailureLine treats empty/whitespace and noise predicates as ignorable', () => {
  ...
  assert.equal(isIgnorableFailureLine('Helios CLI'), true)
...
test('isIgnorableTurboLine treats empty/whitespace and turbo noise predicates as ignorable', () => {
  ...
  assert.equal(isIgnorableTurboLine('• Packages in scope: @helios/app'), true)
  ...
  assert.equal(isIgnorableTurboLine('@helios/core:build: building'), false)
...
test('isIgnorableConsolidatedWatchLine catches consolidated watcher progress only', () => {
  ...
  assert.equal(isIgnorableConsolidatedWatchLine('[watch] watch scope: 3 of 19 package(s) are not watched (set HELIOS_WATCH_SCOPE=all to watch everything)'), true)

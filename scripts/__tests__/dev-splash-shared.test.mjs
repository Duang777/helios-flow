...
test('resolveSplashBindHost honours explicit HELIOS_DEV_SPLASH_BIND=0.0.0.0 with a loud warning', () => {
  const logger = makeLogger()
  assert.equal(resolveSplashBindHost({ HELIOS_DEV_SPLASH_BIND: '0.0.0.0' }, logger), '0.0.0.0')
  assert.equal(logger.warnings.length, 1)
  assert.match(logger.warnings[0], /HELIOS_DEV_SPLASH_BIND=0\.0\.0\.0/)
})

test('resolveSplashBindHost normalises whitespace and case', () => {
  const logger = makeLogger()
  assert.equal(resolveSplashBindHost({ HELIOS_DEV_SPLASH_BIND: '  LOCALHOST  ' }, logger), '127.0.0.1')
  assert.equal(resolveSplashBindHost({ HELIOS_DEV_SPLASH_BIND: '127.0.0.1' }, logger), '127.0.0.1')
})

test('resolveSplashBindHost supports IPv6 binds', () => {
  const logger = makeLogger()
  assert.equal(resolveSplashBindHost({ HELIOS_DEV_SPLASH_BIND: '::1' }, logger), '::1')
  assert.deepEqual(logger.warnings, [])

  const allLogger = makeLogger()
  assert.equal(resolveSplashBindHost({ HELIOS_DEV_SPLASH_BIND: '::' }, allLogger), '::')
...
test('resolveSplashBindHost falls back to loopback with a warning when the override is unrecognised', () => {
  const logger = makeLogger()
  assert.equal(resolveSplashBindHost({ HELIOS_DEV_SPLASH_BIND: 'garbage' }, logger), '127.0.0.1')
  assert.equal(logger.warnings.length, 1)
  assert.match(logger.warnings[0], /Unrecognized HELIOS_DEV_SPLASH_BIND/)
})

...
const autoPublishSkillPath = path.resolve('.ai/skills/helios-auto-publish-pr/SKILL.md')
...
test('auto publish skill only dispatches pkg.pr.new previews and is tiered as automation', () => {
  ...
  assert.match(skill, /^name: helios-auto-publish-pr$/m)
  ...
  assert.ok(
    tiers.tiers.automation.skills.includes('helios-auto-publish-pr'),
    'helios-auto-publish-pr should be installable through the automation tier',
  )
})

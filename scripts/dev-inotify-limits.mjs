...
export function buildManualInotifyFix({ required = DEFAULT_INOTIFY_LIMITS } = {}) {
  ...
  return [
    ...
    `printf '${config}' | sudo tee /etc/sysctl.d/99-helios-inotify.conf >/dev/null`,
    'sudo sysctl --system',
  ]
}
...
export function ensureDevInotifyLimits({
  ...
  const rawMode = String(env.HELIOS_DEV_INOTIFY_CHECK ?? '').trim().toLowerCase()

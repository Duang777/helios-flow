...
// `.helios/watch-packages.local.json` (gitignored) and consumed by the `env`
// scope (`HELIOS_WATCH_SCOPE=env`). Run it with `yarn dev:watch-select`, then start
...
async function main() {
  ...
  if (!process.stdin.isTTY) {
    console.error('[watch-select] interactive selection requires a TTY. Set HELIOS_WATCH_PACKAGES=<list> instead, e.g. HELIOS_WATCH_PACKAGES=core,ui yarn dev --watch=env')
    process.exitCode = 1
    return
  }

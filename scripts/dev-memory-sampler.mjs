...
export const DEFAULT_MEMORY_TRACE_OUT_DIR = path.join('.helios', 'dev-rss')
...
export function classifyProcessCommand(command) {
  ...
  if (
    ...
    || normalized.includes('helios generate')
  ) {
  ...
  if (
    ...
    || normalized.includes('helios server dev')
  ) {
  ...
  if (
    ...
    || normalized.includes('yarn workspace @helios/app dev')
  ) {
...
export function resolveMemoryTraceIntervalMs(env = process.env, fallback = DEFAULT_MEMORY_TRACE_INTERVAL_MS) {
  const raw = env.HELIOS_DEV_MEMORY_TRACE_INTERVAL_MS

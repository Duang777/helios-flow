...
//
// Supported sources:
//   - app module:        apps/helios/src/modules/<id>/        (a package wrapper is scaffolded)
//   - dedicated package:  packages/<pkg>/ that contains only src/modules/<id>  (moved whole)
// Extracting a module out of @helios/core (or shared/ui/cli) is intentionally not
...
function detectSource() {
  const appModuleDir = path.join(repoRoot, 'apps', 'helios', 'src', 'modules', moduleId)
  ...
  for (const entry of fs.readdirSync(packagesDir, { withFileTypes: true })) {
    ...
    if (NON_PROMOTABLE_PACKAGES.has(entry.name)) {
      fail(`module "${moduleId}" lives in @helios/${entry.name}; extracting it is not automated — move it by hand.`)
    }
    if (modules.length > 1) {
      fail(`package @helios/${entry.name} contains multiple modules (${modules.join(', ')}); promote it manually so the others aren't moved.`)
    }
  ...
  fail(`module "${moduleId}" not found under apps/helios/src/modules/ or packages/*/src/modules/`)
}
...
const packageSuffix =
  namedFlags.as ||
  (source.kind === 'package' ? source.packageJsonName.replace(/^@helios\//, '') : moduleId.replace(/_/g, '-'))
...
if (source.kind === 'package') {
  out(`  ${stepNo++}. remove "${source.packageJsonName}" from apps/helios/package.json dependencies (other package.json files: review manually)`)
}
out(`  ${stepNo++}. remove the { id: '${moduleId}', ... } entry from apps/helios/src/modules.ts`)
...
out('  - yarn generate')
out('  - yarn helios configs cache structural --all-tenants')
out(`  - review ${rel(targetDir)}/package.json deps & peerDependencies (pin @helios/core etc.)`)
...
out('  - in helios: commit the removal + modules.ts + official-modules.* changes (do NOT commit the submodule pointer bump unless intended)')
out('')
...
// Drop the dependency from apps/helios/package.json for a moved package.
if (source.kind === 'package') {
  const appPkgPath = path.join(repoRoot, 'apps', 'helios', 'package.json')
  ...
  if (changed) {
    ...
    out(`  removed ${source.packageJsonName} from apps/helios/package.json`)
  }
}

// Remove the registry entry from apps/helios/src/modules.ts.
{
  const modulesTsPath = path.join(repoRoot, 'apps', 'helios', 'src', 'modules.ts')
  ...
  if (kept.length !== lines.length) {
    ...
    out(`  removed { id: '${moduleId}', ... } from apps/helios/src/modules.ts`)
  } else {

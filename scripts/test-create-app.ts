...
async function main(): Promise<void> {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'create-helios-app-smoke-'))
  ...
  try {
    ...
    console.log(green('\ncreate-helios-app scaffold test passed'))
    console.log(cyan(`App path: ${appDir}`))
    console.log(yellow(`The scaffolded app is configured to install @helios packages from Verdaccio at ${VERDACCIO_URL}.`))
    ...
    restoreEntryDirectory(entryCwd)
    console.error(red('\ncreate-helios-app scaffold test failed'))

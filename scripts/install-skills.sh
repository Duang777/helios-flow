...
#   2. External shared skills: installs the helios/skills collection via
...
Options:
  (no options)        Install the default tier set from .ai/skills/tiers.json
                      plus the external helios/skills collection into the
...
if [ -z "${repo_root}" ]; then
  echo "install-skills: must be run from inside the helios git checkout." >&2
  exit 1
fi
...
# Mix in the external shared collection (helios/skills). The npx CLI copies

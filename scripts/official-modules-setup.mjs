...
//     regenerates apps/helios/src/official-modules.generated.ts.
//
...
function run() {
  if (process.env.HELIOS_SKIP_OFFICIAL_MODULES_SETUP === '1') return
  ...
  if (activated.length > 0) {
    log('module set changed — run `yarn helios configs cache structural --all-tenants` (and `yarn dev:reset` if Turbopack serves a stale chunk).')
  }
}

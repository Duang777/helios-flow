 ...
 * (`@helios/shared/lib/logger`). Introduced by
...
function main() {
  ...
  console.log(dim('Advisory only: prefer createLogger() from @helios/shared/lib/logger for new code.'))
  ...
  if (opts.strict) {
    console.log(red('Strict mode: raw console.* calls are not allowed — use createLogger() from @helios/shared/lib/logger or add an allowlist entry with a reason.'))
    process.exitCode = 1
  } else {

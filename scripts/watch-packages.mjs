...
//
// Escape hatch: set `HELIOS_WATCH_PACKAGES_MODE=legacy` to fall back to the
// previous Turbo-based per-package watcher path (see `package.json`).
//
// Note: this var is distinct from `HELIOS_PACKAGE_WATCH_MODE` (see
...
export function discoverAppGeneratedDirs(root) {
  const candidates = []
  const rootGenerated = join(root, '.helios', 'generated')
  ...
  if (existsSync(appsDir)) {
    ...
    for (const entry of appEntries) {
      ...
      candidates.push(join(appsDir, entry.name, '.helios', 'generated'))
    }
  }
...
export async function runConsolidatedWatch({
  ...
  if (selection.mode !== 'all') {
    ...
    if (excluded > 0) {
      log(`[watch] watch scope: ${excluded} of ${allPackages.length} package(s) are not watched (set HELIOS_WATCH_SCOPE=all to watch everything)`)
    }
  }

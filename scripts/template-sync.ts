...
const APP_SRC_ROOT = path.join(ROOT, 'apps', 'helios', 'src')
const TEMPLATE_SRC_ROOT = path.join(ROOT, 'packages', 'create-app', 'template', 'src')
const APP_PACKAGE_FILE = path.join(ROOT, 'apps', 'helios', 'package.json')
...
const EXPLICIT_TEMPLATE_FILE_MAPPINGS = [
  ...
  {
    sourceFile: path.join(ROOT, 'apps', 'helios', 'scripts', 'dev.mjs'),
    templateFile: path.join(ROOT, 'packages', 'create-app', 'template', 'scripts', 'dev-runtime.mjs'),
    rel: 'scripts/dev-runtime.mjs',
  },
  {
    sourceFile: path.join(ROOT, 'apps', 'helios', 'scripts', 'dev-runtime-log-policy.mjs'),
...
const TEMPLATE_CONTENT_TRANSFORMS: Record<string, (content: string) => string> = {
  ...
  'scripts/dev-cache-purge.mjs': (content) =>
    content
      .replaceAll("['apps', 'helios', '.helios', 'next'", "['.helios', 'next'")
      .replaceAll("['apps', 'helios', '.next']", "['.next']")
      .replace('`.helios/next/dev/cache/turbopack`. See issue\n// #1950.', '`.helios/next/dev/cache/turbopack`.')
}

...
const autoOpenSplash = !classic
  ...
  && process.env.CI !== 'true'
  && process.env.HELIOS_DEV_AUTO_OPEN !== '0'
const splashChildStateFilePath = path.join(projectRootDirectory, '.helios', 'dev-ephemeral-splash-child-state.json')
const warmupReadyFilePath = path.join(projectRootDirectory, '.helios', 'dev-ephemeral-warmup-ready.json')
const splashState = {
  mode: 'ephemeral',
...
function resolveSplashLogoSvg(): string {
  ...
  const candidates = [
    path.join(projectRootDirectory, 'public', 'helios.svg'),
    path.join(appDirectory, 'public', 'helios.svg'),
  ]
...
async function ensureEnvFile(): Promise<void> {
  if (await fileExists(envPath)) {
    console.log('[dev:ephemeral] Reusing existing apps/helios/.env file.')
    return
  }

  await copyFile(envExamplePath, envPath)
  console.log('[dev:ephemeral] Created apps/helios/.env from apps/helios/.env.example.')
}
...
async function startEphemeralPostgres(): Promise<EphemeralPostgresHandle> {
  ...
  const containerName = `helios-dev-ephemeral-${uniqueSuffix}`
...
async function startDevServer(port: number, postgres: EphemeralPostgresHandle): Promise<number> {
  ...
  const devEnvironment = {
    ...
    ...(autoOpenSplash
      ? {
          HELIOS_DEV_SPLASH_CHILD_STATE_FILE: splashChildStateFilePath,
          HELIOS_DEV_SPLASH_MODE: 'dev',
        }
      : {}),
  }

  const devArgs = classic
    ? ['workspace', '@helios/app', 'dev:classic']
    : (verbose ? ['workspace', '@helios/app', 'dev:verbose'] : ['workspace', '@helios/app', 'dev'])

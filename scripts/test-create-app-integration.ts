...
function ensureEnterpriseDependency(appDir: string): void {
  ...
  dependencies['@helios/enterprise'] = ROOT_VERSION
...
function writeStandaloneEnv(appDir: string): void {
  ...
  const envLines = [
    ...
    'DATABASE_URL=postgres://helios:secret@localhost:5432/helios_test',
    ...
    'HELIOS_WEBHOOKS_ALLOW_PRIVATE_URLS=1',
    ...
    'NEXT_PUBLIC_HELIOS_EXAMPLE_CHECKOUT_TEST_INJECTIONS_ENABLED=true',
    'HELIOS_ENABLE_ENTERPRISE_MODULES=true',
    'HELIOS_ENABLE_ENTERPRISE_MODULES_SSO=true',
    'HELIOS_ENABLE_ENTERPRISE_MODULES_SECURITY=true',
    ...
    `CACHE_SQLITE_PATH=${path.join(appDir, '.helios', 'cache', 'cache.db')}`,
  ].filter(Boolean)
...
async function waitForStandaloneEphemeralApp(params: {
  ...
  let outputBuffer = ''
  const child = spawn('yarn', ['helios', 'test:ephemeral', '--no-reuse-env', '--no-screenshots'], {
    cwd: params.appDir,
  ...
  const handleOutput = (chunk: Buffer): void => {
    ...
    if (match) {
      databaseUrl = `postgres://helios:secret@${match[1]}:${match[2]}/helios_test`
    }
  }
...
async function main(): Promise<void> {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'create-helios-app-integration-'))
  ...
  const integrationEnv: NodeJS.ProcessEnv = {
    ...
    NEXT_PUBLIC_APP_URL: 'http://localhost:3000',
    JWT_SECRET: 'ci-standalone-test-jwt-secret',
    HELIOS_SECURITY_MFA_SETUP_SECRET: 'ci-standalone-test-mfa-setup-secret',
    TENANT_DATA_ENCRYPTION_FALLBACK_KEY: 'ci-standalone-test-fallback-key',
    HELIOS_TEST_MODE: '1',
    HELIOS_TEST_AUTH_RATE_LIMIT_MODE: 'opt-in',
    HELIOS_DISABLE_EMAIL_DELIVERY: '1',
    ...
    CACHE_STRATEGY: 'sqlite',
    CACHE_SQLITE_PATH: path.join(appDir, '.helios', 'cache', 'cache.db'),
    HELIOS_DRAIN_DEBUG: '1',
    HELIOS_INTEGRATION_APP_READY_TIMEOUT_SECONDS: '180',
    HELIOS_TEST_APP_ROOT: appDir,
  ...
  try {
    ...
    console.log(green('\ncreate-helios-app standalone integration test passed'))
    ...
    console.error(red('\ncreate-helios-app standalone integration test failed'))

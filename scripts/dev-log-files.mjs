...
export function createDevLogSession(options = {}) {
  ...
  const logDir = path.resolve(options.logDir ?? path.join(cwd, '.helios', 'logs'))
  const role = sanitizeFileSegment(options.role ?? 'dev')
  const generatedRunId = createRunId()
  const runId = sanitizeFileSegment(
    options.runId ?? process.env.HELIOS_DEV_RUN_ID?.trim() ?? generatedRunId,
    generatedRunId,
  )
  ...
  return {
    ...
    env: {
      HELIOS_DEV_LOG_DIR: logDir,
      HELIOS_DEV_RUN_ID: runId,
    },
    openLog,
    closeAll,
  }

...
- **CrudForm** – whichever spot id you pass to `CrudForm` (`injectionSpotId`). If omitted, it auto-derives `crud-form:<entityId>` (colon → dot) from the first `entityId`, so most forms pick up widgets by entity. Widgets with `kind: 'group'` render as new groups; others stack above the form. Field injection uses `crud-form:<entityId>:fields`. Helpers: `generateCrudFormInjectionSpotId()` and `CrudFormInjectionSpots` from `@helios/ui/backend/injection/helpers`.
...
- **Component overrides** – define `widgets/components.ts` in any module and export `componentOverrides`. `yarn generate` collects them into `.helios/generated/component-overrides.generated.ts`.
...
4. Run `yarn generate` (or `yarn helios generate registry`) to regenerate registries.

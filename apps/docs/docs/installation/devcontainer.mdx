---
title: Dev Container
description: Open the Helios monorepo in a fully pre-configured VS Code Dev Container. Docker Desktop and VS Code are all you need — no local Node.js, Yarn, or build tools required.
---
...
Create `apps/helios/.env.local` inside the container (or on the host before opening). This file takes priority over `.env`, which is auto-generated:
...
Use `HELIOS_DISABLE_VECTOR_SEARCH_AUTOINDEXING=false` only if you want automatic vector embedding jobs inside the container. The default example env keeps auto-indexing disabled until you opt in.

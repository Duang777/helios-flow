...
description: Deploy a production-ready Helios stack to any Linux server or VPS using Docker Compose.
---

Deploy the complete Helios stack — application, PostgreSQL, Redis, and Meilisearch — to any Linux server using Docker Compose.
...
# Edit apps/helios/.env with your production values (see below)
...
Edit `apps/helios/.env` before starting. At minimum, set:
...
POSTGRES_DB=helios
DATABASE_URL=postgres://postgres:your-strong-db-password@postgres:5432/helios
...
If you leave `HELIOS_DISABLE_VECTOR_SEARCH_AUTOINDEXING=true`, vector search can still be configured and reindexed manually. The legacy alias `DISABLE_VECTOR_SEARCH_AUTOINDEXING=1` remains supported for existing deployments.
...
```bash
docker exec helios-postgres pg_dump -U postgres helios > backup-$(date +%Y%m%d).sql
```
...
If the value is below 1 GB, raise `NODE_OPTIONS` in `apps/helios/.env` then restart:

---
title: Local setup
description: Step-by-step checklist to run Helios locally.
---

## Getting started tracks

**Product teams** looking to build on Helios should start with the [standalone app](#standalone-app-recommended-for-product-teams) — it gives you a clean project without cloning the full monorepo. **Core contributors** should follow the [develop branch](#develop-branch-recommended) track instead.

:::tip Standalone app
If you want to build a production app without cloning the monorepo, use [`create-helios-app`](../customization/standalone-app) instead.
:::
...
If you are working on the Helios monorepo and need to validate scaffold changes before publishing:
...
See the [standalone app guide](/customization/standalone-app#test-create-helios-app-locally-from-the-monorepo) for what each command does.
...
4. **Copy the environment template**
   ...
   ```
   Make sure these required variables are set in `apps/helios/.env` before continuing:
   ```env
...
- writes a transcript log under `%TEMP%\helios-setup\`
...
When you run `yarn dev` on the host, Helios starts a compact development runtime with a dedicated splash page that shows startup progress while packages, generators, and the app server warm up.
...
```bash
HELIOS_DEV_SPLASH_PORT=4100 yarn dev
```

To use a random free splash port instead of the stable default:

```bash
HELIOS_DEV_SPLASH_PORT=random yarn dev
```

To keep the splash from opening automatically:

```bash
HELIOS_DEV_AUTO_OPEN=0 yarn dev
```

To hide the standalone GitHub publish panel from the splash:

```bash
HELIOS_DEV_CREATE_GIT_REPO_FLOW=false yarn dev
```

To hide the coding-tools menu from the splash:

```bash
HELIOS_ENABLE_CODING_FLOW_FRHELIOS_SPLASH=false yarn dev
```

### In-process watcher and workers (memory optimization)

To keep idle memory low, `yarn dev` now runs two things **in-process** inside the `helios server dev` runtime instead of as separate long-running Node processes:

- **Structural regeneration watcher** — the watcher that re-runs `helios generate` when module files change is hosted by the dev server itself. Collapsing this dedicated sidecar saves ~190 MB of resident memory.
...
```bash
HELIOS_DEV_GENERATE_WATCH_MODE=legacy yarn dev
```
...
HELIOS_AUTO_SPAWN_WORKERS_LAZY_POLL_MS=500 yarn dev
```
...
- On first run, executes `yarn helios init` (migrations + seeding); on subsequent runs, applies pending migrations only
...
The wrappers are implemented by [`scripts/docker-exec.mjs`](https://github.com/helios/helios/blob/develop/scripts/docker-exec.mjs). The helper:
...
```bash
yarn docker:db:migrate
yarn docker:helios auth:list-users
```
...
- If you see `Error: No running Helios app container found.`, start either `yarn docker:dev:up` or `yarn docker:up` first.
...
All environment variables are pre-configured with sensible defaults inside the compose file. To customize, edit `apps/helios/.env` before starting — the compose file reads from it automatically.
...
| `HELIOS_DEV_SPLASH_PORT` | `4000` | Host and container port for the dev splash progress page |
| `HELIOS_DEV_AUTO_OPEN` | `1` | Set to `0` to disable browser auto-open for the splash on supported local runs |
| `HELIOS_DEV_CREATE_GIT_REPO_FLOW` | `true` | Set to `false` to hide the standalone splash GitHub publish panel |
| `HELIOS_ENABLE_CODING_FLOW_FRHELIOS_SPLASH` | `true` | Set to `false` to hide the splash coding-tools menu |
...
| `OPENAI_API_KEY` | — | Enables OpenAI-backed AI features; add it with `HELIOS_DISABLE_VECTOR_SEARCH_AUTOINDEXING=false` only when you want vector auto-indexing on |
| `HELIOS_DISABLE_VECTOR_SEARCH_AUTOINDEXING` | `true` in the shipped example env | Set to `false` or remove it to enable automatic vector indexing |
...
```bash
HELIOS_DEV_SPLASH_PORT=4100 docker compose -f docker-compose.fullapp.dev.yml up --build
```
...
- If port 4000 is already in use, override the splash port too: `HELIOS_DEV_SPLASH_PORT=4100 docker compose -f docker-compose.fullapp.dev.yml up`
- If Postgres auth/connect errors appear during local commands (`yarn generate`, `yarn db:generate`) and `localhost:5432` is occupied by another local PostgreSQL instance, start Docker Postgres on another port and use the same port in `DATABASE_URL`:
  ...
  ```
  ```env
  DATABASE_URL=postgresql://postgres:postgres@localhost:55432/helios
  ```
...
Run the complete Helios stack with Docker Compose, including the application, PostgreSQL, Redis, and Meilisearch. This approach is ideal for quick demos, testing, or containerized deployments.

[![Watch: Deploy Helios on a VPS](https://img.youtube.com/vi/xau17YBP9ek/maxresdefault.jpg)](https://www.youtube.com/watch?v=xau17YBP9ek)
...
# Edit apps/helios/.env with your configuration (see next section)
...
Before starting the Docker stack, configure `apps/helios/.env` with at least these required variables:
...
POSTGRES_DB=helios
DATABASE_URL=postgres://postgres:postgres@postgres:5432/helios
...
HELIOS_DISABLE_VECTOR_SEARCH_AUTOINDEXING=false
```

If you keep `HELIOS_DISABLE_VECTOR_SEARCH_AUTOINDEXING=true` (the default in `apps/helios/.env.example`), semantic search can still be configured and reindexed manually, but record changes will not enqueue embeddings automatically. The legacy alias `DISABLE_VECTOR_SEARCH_AUTOINDEXING=1` still works, but prefer the `HELIOS_` name for new setups.
...
- `helios-postgres-data` - PostgreSQL database
- `helios-redis-data` - Redis cache and events
- `helios-meilisearch-data` - Meilisearch indexes
...
```bash
# Backup PostgreSQL
docker exec helios-postgres pg_dump -U postgres helios > backup.sql

# Restore PostgreSQL
cat backup.sql | docker exec -i helios-postgres psql -U postgres -d helios
```
...
docker exec helios-postgres psql -U postgres -c "\l"

# Verify DATABASE_URL matches container configuration
grep DATABASE_URL apps/helios/.env
```
...
- **app** - Helios application (Node.js 24, Next.js)
...
Services communicate via a dedicated Docker network (`helios-network-fullapp`) and use health checks to ensure proper startup order.

...
description: Run Helios on Windows using WSL2 (Windows Subsystem for Linux 2) with Ubuntu for a first-class Linux development experience.
---
...
```env
DATABASE_URL=postgres://postgres:<password>@172.28.160.1:5432/helios
```
...
For best performance, clone the repository inside the WSL2 filesystem (`~/projects/helios`) rather than on the Windows drive (`/mnt/c/...`). Cross-filesystem I/O is significantly slower and can cause file-watch issues with Next.js.
:::
...
If you still want to browse the source from Visual Studio, you can open the folder via `\\wsl$\Ubuntu\home\<you>\projects\helios` — but builds, `yarn`, and debugging must be run from the Ubuntu terminal, not from Visual Studio.

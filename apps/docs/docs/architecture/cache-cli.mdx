...
The cache CLI is intentionally implemented as a **module CLI**, not as a hard-coded command in the core CLI binary. That keeps cache operations aligned with Helios's module architecture: the `configs` module owns the cache management feature in the UI, the HTTP API, and the CLI.

## Discovery through the module generator

The command lives in `packages/core/src/modules/configs/cli.ts`. It becomes runnable only after the generator scans enabled modules and emits the CLI registry used by `yarn helios`.
...
3. The generator emits `apps/helios/.helios/generated/modules.cli.generated.ts`.
4. `packages/cli` loads that generated registry at runtime.
5. `yarn helios configs cache ...` resolves the `configs` module command and dispatches into the cache helper.
...
```mermaid
flowchart TD
  command["`yarn helios configs cache ...`<br/>stats, purge, structural<br/>scope flags: `--tenant`, `--global`, `--all-tenants`"]
...
- operators can still run `yarn helios configs cache structural --all-tenants` for the full DI-aware path, including custom cache-service overrides
...
For operator usage, see [`helios configs cache`](../cli/configs-cache).

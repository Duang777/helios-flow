...
description: Understand how Helios is structured, where to extend it, and how requests move through the stack.
---

Helios is a modular platform: every capability lives in a module that can be enabled, overridden, or replaced without touching the core runtime. This page highlights the architecture, shows where customisation happens, and links to deeper references.
...
- **Overlay overrides** – files under `apps/helios/src/modules/<module>/...` override their package counterparts, letting apps customize behaviour without forking the package.
...
- Modules are declared in `packages/<pkg>/src/modules/<module>` (package-supplied) or `apps/helios/src/modules/<module>` (app overrides).
- The application bootstraps modules from `apps/helios/src/modules.ts`. Removing an entry disables the module; adding a new entry enables a custom package.
...
- **Tenants** – Provisioned through the directory module and created with CLI helpers such as [`yarn helios auth setup`](../cli/auth-setup). The command ensures a tenant, its root organization, and bootstrap users exist, and can be rerun safely in any environment.
...
flowchart LR
    subgraph App
        SRC["apps/helios/src/app/*<br/>pages/layouts"]
        SM["apps/helios/src/modules/*<br/>overrides"]
        MT["apps/helios/src/modules.ts<br/>module toggle"]
    end
    subgraph Packages
        CORE["@helios/core<br/>modules"]
        EXTRA["Custom packages<br/>@helios/*"]
    end
...
- **`apps/helios/src/app`** – Override or add pages/layouts without touching package sources.
- **`apps/helios/src/modules`** – Provide module-level overrides (backend/frontend pages, APIs, entities, i18n). Files here take precedence over package files with the same relative path.
- **`apps/helios/src/modules.ts`** – Acts as a manifest: enabling/disabling modules reconfigures the application without code changes.
...
- **Application reconfiguration** – Swap modules in `apps/helios/src/modules.ts`, override routers in `apps/helios/src/app`, or adjust DI via `apps/helios/src/lib`.
- **Modules** – Ship new modules in `packages/<your-package>` or override core behaviour in `apps/helios/src/modules/<module>`.
- **Custom entities & fields** – Define runtime-managed schemas in `ce.ts` or through the admin UI, then seed them with `yarn helios entities install`.
...
- Use the shared translation helpers from `@helios/ui` or `@helios/core/lib/i18n` to load strings in pages, APIs, or services.

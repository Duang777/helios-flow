...
description: Which modules ship with Helios, which are foundational, and how the `requires` field wires inter-module dependencies.
---

Helios is composed of independent modules that you enable per app in `apps/helios/src/modules.ts`. Most modules can be disabled freely — but a small subset of **foundational** modules are pulled in transitively because other modules declare them via `ModuleInfo.requires`. This page enumerates every module that ships in the monorepo, names its declared dependencies, and explains how the platform enforces the graph.
...
- Tenant setup (`helios init`, `setupInitialTenant()`) runs `seedDefaults` and `seedExamples` in dependency order derived from `requires` — see `packages/shared/src/modules/setup.ts`.
...
| --- | --- | --- | --- |
| `ai_assistant` | `@helios/ai-assistant` | — | |
| `checkout` | `@helios/checkout` | — | |
| `content` | `@helios/content` | — | |
| `events` | `@helios/events` | — | |
| `gateway_stripe` | `@helios/gateway-stripe` | — | |
| `onboarding` | `@helios/onboarding` | — | |
| `scheduler` | `@helios/scheduler` | — | |
| `search` | `@helios/search` | — | |
| `sync_akeneo` | `@helios/sync-akeneo` | `integrations`, `data_sync`, `catalog`, `sales` | |
| `webhooks` | `@helios/webhooks` | — | |

### Enterprise (`@helios/enterprise`)

Enterprise modules are off by default. Toggle them via `HELIOS_ENABLE_ENTERPRISE_MODULES`, `HELIOS_ENABLE_ENTERPRISE_MODULES_SSO`, and `HELIOS_ENABLE_ENTERPRISE_MODULES_SECURITY`.
...
| --- | --- | --- |
| `example` | `@helios/create-app` template | — |
| `example_customers_sync` | `@helios/create-app` template | — |

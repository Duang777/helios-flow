...
Helios supports two text-extraction methods for attachments:
...
Attachment env naming follows the standard `HELIOS_*` prefix. Legacy `HELIOS_*` aliases are still read for backward compatibility, but new deployments should use the `HELIOS_*` names.
...
- `HELIOS_DEFAULT_ATTACHMENT_OCR_ENABLED=true|false` controls whether new partitions opt into OCR by default.
- `HELIOS_ATTACHMENT_MAX_UPLOAD_MB=<number>` sets the global upload cap per file before field-specific limits are applied. Default: `25`.
- `HELIOS_ATTACHMENT_TENANT_QUOTA_MB=<number>` sets the tenant-wide stored attachment quota. Default: `512`.

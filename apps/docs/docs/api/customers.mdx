...
**Feature:** `customers.activities.manage`  
Body: `organizationId`, `tenantId`, `entityId`, `tagId`. Creates a tag assignment and returns `{ id: "<uuid>" }`. The response includes an `x-helios-operation` header for undo metadata.

### Unassign tag — `POST /customers/tags/unassign`

**Feature:** `customers.activities.manage`  
Accepts the same body as assign. Removes the tag link and emits undo metadata via `x-helios-operation`.
...
Response: `{ "todoId": "<uuid|null>", "linkId": "<uuid>" }` plus the `x-helios-operation` header.

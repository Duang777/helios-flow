...
- Stripe `requires_capture` usually maps to Helios `authorized`
- Stripe `succeeded` maps to Helios `captured`
...
- `transactionId` is the Helios payment-hub transaction identifier you should persist in your app.
...
Stripe should send webhook events into Helios so the hub can update transaction status, append webhook history, and keep refunds and disputes synchronized. Session creation can work without it, but a production Stripe integration should not stop there.
...
After creating the endpoint in **Stripe Dashboard -> Workbench -> Webhooks**, reveal the endpoint signing secret and paste the `whsec_...` value into the Stripe integration's **Webhook Signing Secret** field in Helios.

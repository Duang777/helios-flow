...
When the query index backing the list is known to be incomplete, the response also carries an `x-helios-partial-index` header with a JSON payload describing the gap, so clients can warn that the export may be missing rows.

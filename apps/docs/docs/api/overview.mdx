...
description: How to authenticate, scope, and interact with the Helios REST API.
---
...
`API_KEY` comes from the admin UI table (**Admin → Security → API keys**) or the [`helios api_keys add`](../cli/api-keys) CLI command. All `curl` examples reuse these variables so you can copy, tweak, and run them immediately.

## Authentication Flow

1. Mint a key from the [Managing API keys guide](../user-guide/api-keys) or via [`helios api_keys add`](../cli/api-keys). Keys inherit the roles (and therefore features) you assign.
...
- Use the shared CRUD helpers in `@helios/shared/lib/crud/factory` to ensure consistent pagination, scoping, and event emission logic.

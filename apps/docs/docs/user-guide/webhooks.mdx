...
This guide shows how to enable Custom Webhooks, create endpoint subscriptions, validate deliveries, and monitor failures from the Helios admin UI.
...
![Custom Webhooks integration settings](/screenshots/helios-webhooks-integration-settings.png)
...
![Webhook list](/screenshots/helios-webhooks-list.png)
...
Helios rejects webhook URLs that point at localhost, private networks, reserved IP ranges, or internal hostnames. This protects the delivery worker from sending server-side requests to services that only the Helios host can reach.

For local development or CI, set `HELIOS_WEBHOOKS_ALLOW_PRIVATE_URLS=1` to allow endpoints such as `http://localhost:3000/webhooks`. Keep this disabled in production. Use a public HTTPS endpoint or a temporary public tunnel when testing a production-like environment.

## Step 3 — Store the signing secret immediately

After creating a webhook, Helios reveals the signing secret only once. The same one-time reveal appears again after a secret rotation.

![Webhook secret reveal](/screenshots/helios-webhooks-secret-reveal.png)
...
![Webhook detail page](/screenshots/helios-webhooks-detail.png)
...
![Aggregated webhook delivery log](/screenshots/helios-webhooks-integration-logs.png)

...
Need to capture extra context about each user? Helios lets you extend the core `auth:user` entity without editing the module. Define tenant-scoped custom fields, sync them via the Entities CLI, and the admin UI renders the new inputs automatically.

![Extending system entities with user fields](/screenshots/helios-extending-system-entities.png)
...
import { defineFields, cf, entityId } from '@helios/modules/dsl';
...
export const entities = [
  defineFields(
    ...
    [
      ...
      cf.phone('work_phone', { label: 'Work phone', formEditable: true, filterable: true }),
...
```bash
yarn helios entities install --tenant <tenantId>
```
...
![Editing a user with the custom data section](/screenshots/helios-edit-user-and-acl.png)

...
Set up the email-to-ERP agent so your team can forward customer email threads and have Helios automatically extract orders, quotes, contacts, and reply drafts.

## How It Works

1. Your employees email customers from their normal email accounts
2. When they want Helios to process a conversation, they **forward the thread** to your configured inbox address (e.g. `ops-84f470a7@inbox.yourcompany.com`)
3. Resend receives the email and delivers it to Helios via a secure webhook
...
- Your Helios instance must be publicly accessible (or use a tunnel like ngrok for development)
...
2. Set the endpoint URL to your Helios instance:
   ```
...
After starting Helios, each tenant automatically gets an inbox address generated during setup. You can also configure a **working language** on the Settings page — proposals will be translated to this language for easier review.
...
Share this address with your team. They'll forward email threads to it when they want Helios to process them.
...
3. Helios verifies the Svix signature using your `RESEND_WEBHOOK_SIGNING_SECRET`
...
2. Create a free ngrok account, grab your authtoken from [https://dashboard.ngrok.com/get-started/your-authtoken](https://dashboard.ngrok.com/get-started/your-authtoken), and register it once:
   ...
   ```
3. Start your Helios dev server in one terminal:
...
Open `apps/helios/.env` (or copy from `apps/helios/.env.example` if it does not exist yet) and set the following block. Uncomment the keys, do not leave the sample `#` prefix:
...
Helios generates the tenant's inbox address at tenant setup time using whatever `INBOX_OPS_DOMAIN` was set then. If you changed `INBOX_OPS_DOMAIN` (for example because you used the default `inbox.helios.local` on first boot), the address stored in `inbox_settings.inbox_address` will still point at the old domain and Resend traffic will never match it.
...
```sql
UPDATE inbox_settings
SET inbox_address = REPLACE(inbox_address, 'inbox.helios.local', 'eldaaelkia.resend.app'),
    updated_at = NOW()
WHERE deleted_at IS NULL;
```
...
3. **Helios** extracts the intent and creates a proposal with actions

...
![Cache management table with purge controls](/screenshots/helios-cache-management.png)

For terminal-driven work, the matching CLI is [`yarn helios configs cache`](../cli/configs-cache). It covers the same segment stats/purge path as the admin page and adds targeted purge modes for tags, exact keys, identifier tokens, patterns, and structural navigation cleanup.

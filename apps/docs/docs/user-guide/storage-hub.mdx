...
description: Connect Helios to Amazon S3, DigitalOcean Spaces, MinIO, or any S3-compatible provider as the storage backend for attachments and direct file operations.
...
Helios uses a pluggable **storage driver** architecture. Each **attachment partition** declares which driver to use (`local` by default). When you configure S3 credentials and switch a partition's driver, all new uploads for that partition go to S3 while reads from older local-filesystem files continue to work through the `local` driver.
...
- The `storage_s3` module must be enabled. Set `HELIOS_ENABLE_STORAGE_S3=true` in your environment and restart the app.
...
![S3 integration credentials form showing bucket, region, and access key fields](/screenshots/helios-storage-s3-credentials.png)
...
![S3 integration health tab showing a passing health check](/screenshots/helios-storage-s3-health.png)
...
3. In the **Storage Driver** dropdown, select **Amazon S3**.
   - If you don't see (or can't select) **Amazon S3**, make sure `HELIOS_ENABLE_STORAGE_S3=true` is set and the app was restarted.
...
![Attachment partition settings with Storage Driver dropdown set to S3](/screenshots/helios-storage-s3-partition.png)
...
HELIOS_INTEGRATION_STORAGE_S3_REGION=us-east-1
HELIOS_INTEGRATION_STORAGE_S3_BUCKET=my-helios-bucket
# Optional:
# HELIOS_INTEGRATION_STORAGE_S3_SESSION_TOKEN=
# HELIOS_INTEGRATION_STORAGE_S3_ENDPOINT=https://nyc3.digitaloceanspaces.com
# HELIOS_INTEGRATION_STORAGE_S3_FORCE_PATH_STYLE=true
# HELIOS_INTEGRATION_STORAGE_S3_FORCE_PRECONFIGURE=false
```

`HELIOS_ENABLE_STORAGE_S3=true` is the module-enablement gate: without it the `storage_s3` module is not registered in `enabledModules` and the rest of the `HELIOS_INTEGRATION_STORAGE_S3_*` block is ignored. The two layers are intentionally separate so you can preview the integration card in the UI without preconfiguring credentials.

On new tenant setup the module reads `HELIOS_INTEGRATION_STORAGE_S3_*` and pre-populates the Integration Marketplace credentials so no manual configuration step is needed. Setup failures (for example a partial env block) are surfaced through the **Integrations → Logs** UI under the `storage_s3` filter, so operators can see them alongside other integration activity.

### Preconfigure existing tenants from the CLI

`onTenantCreated` only runs once per tenant. If you upgrade an existing deployment after setting the `HELIOS_INTEGRATION_STORAGE_S3_*` block, rerun the provider CLI to apply the preset to a tenant that was provisioned before the variables were available:
...
- Tenant already has stored credentials → prints `Skipped: S3 credentials already exist.` and exits `0` unless you pass `--force` or set `HELIOS_INTEGRATION_STORAGE_S3_FORCE_PRECONFIGURE=true`.
...
```bash
yarn helios storage_s3 configure-from-env \
  --tenant <tenantId> \
  --org <organizationId> \
  --force
```
...
- Reads the `HELIOS_INTEGRATION_STORAGE_S3_*` block from the process environment once.
...
- Env preset is incomplete (a required `HELIOS_INTEGRATION_STORAGE_S3_*` var is missing) → every scope errors → exit `1`. CI/automation can detect misconfiguration without parsing stdout.
- At least one scope errors while others succeed → exit `1`. The summary line reports the breakdown.
- No active organizations exist yet (fresh DB, before `helios init`) → exit `0` with a "Nothing to do" message.
...
```env
HELIOS_ENABLE_STORAGE_S3=true
HELIOS_INTEGRATION_STORAGE_S3_ACCESS_KEY_ID=test
HELIOS_INTEGRATION_STORAGE_S3_SECRET_ACCESS_KEY=test
HELIOS_INTEGRATION_STORAGE_S3_REGION=us-east-1
HELIOS_INTEGRATION_STORAGE_S3_BUCKET=helios-dev
HELIOS_INTEGRATION_STORAGE_S3_ENDPOINT=http://localhost:4566
HELIOS_INTEGRATION_STORAGE_S3_FORCE_PATH_STYLE=true
```
...
```bash
aws --endpoint-url=http://localhost:4566 s3api create-bucket --bucket helios-dev
```
...
| `S3 integration is not configured` error | `HELIOS_ENABLE_STORAGE_S3` not set or credentials not saved | Verify env var and re-save credentials via Integrations |

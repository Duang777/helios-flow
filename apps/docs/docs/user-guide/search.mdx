...
![Global search dialog](/screenshots/helios-global-search-dialog.png)
...
Helios supports three complementary search strategies:
...
Without a configured provider, semantic search is disabled but other strategies continue working. With a provider configured and `HELIOS_DISABLE_VECTOR_SEARCH_AUTOINDEXING=true`, you can still run manual vector reindex jobs; automatic indexing just stays off.
...
![Search settings page](/screenshots/helios-search-settings.png)
...
| `HELIOS_DISABLE_VECTOR_SEARCH_AUTOINDEXING=true` | Keeps automatic vector indexing disabled (default in shipped example envs) |
| `HELIOS_DISABLE_VECTOR_SEARCH_AUTOINDEXING=false` | Enables automatic vector indexing |
...
yarn helios search reindex --tenant <tenantId>

# Reindex specific entity
yarn helios search reindex --tenant <tenantId> --entity customers:person
```
...
![Search reindex options](/screenshots/helios-search-reindex.png)

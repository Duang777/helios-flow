...
![Creating a global feature toggle](/screenshots/helios-feature-toggles-global-list.png)

1. Open **Admin → Feature Toggles**.
2. Click **Create Feature Toggle**.

![Creating a global feature toggle](/screenshots/helios-feature-toggles-global-create.png)
...
7. Set the initial **Value** based on the selected type:
  ...
  - For **JSON** type: Use the built-in JSON builder with two tabs:
    ...
    **Builder** (visual editor):
    ![Creating a global feature toggle](/screenshots/helios-feature-toggles-global-create-json.png)

    **RAW** (text editor):
    ![Creating a global feature toggle](/screenshots/helios-feature-toggles-global-create-json-2.png)

    For other types, use the simple input field:
    ![Creating a global feature toggle](/screenshots/helios-feature-toggles-global-create-bool.png)
8. Click **Create**.
...
![Feature toggle overrides](/screenshots/helios-feature-toggles-overrides-list.png)
...
4. Set the **Value** for this specific context (e.g., change a boolean to Enabled, or adjust a Number value).
   ![Feature toggle overrides - Value Input](/screenshots/helios-feature-toggles-overrides-item-details.png)
   ![Feature toggle overrides - Value Input](/screenshots/helios-feature-toggles-overrides-item-edit.png)
5. Save the override.
...
```tsx
import { FeatureFlag } from "@helios/core";
...
**Boolean Toggle:**
```tsx
import { useFeatureFlagBoolean } from "@helios/core";
...
**String Toggle:**
```tsx
import { useFeatureFlagString } from "@helios/core";
...
**Number Toggle:**
```tsx
import { useFeatureFlagNumber } from "@helios/core";
...
**JSON Toggle:**
```tsx
import { useFeatureFlagJson } from "@helios/core";
...
**Boolean Toggle:**
```typescript
import { FeatureTogglesService } from "@helios/core";
...
```bash
yarn helios feature-toggles seed-defaults --path ./toggles.json
```
...
```bash
yarn helios feature-toggles override-set-value \

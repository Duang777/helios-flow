...
Use API keys when automations or external systems must call the Helios API without a username and password. Keys inherit RBAC rules, so you can grant narrow permissions to each integration and revoke them without disturbing human accounts.
...
![API keys table](/screenshots/helios-manage-api-keys.png)
...
![Create API key form](/screenshots/helios-create-api-key.png)
...
![API key created modal](/screenshots/helios-api-key-created.png)

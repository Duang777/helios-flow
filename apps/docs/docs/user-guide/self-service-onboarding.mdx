...
3. After confirming, Helios provisions a tenant + organization, seeds demo CRM data, and creates an **admin** user for the submitted email.
...
Sender precedence for Helios emails is:
...
- **Duplicate email** – If the email already exists in Helios, the flow ends with a message prompting the user to sign in or reset the password.

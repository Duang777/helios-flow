...
Helios ships with AI-powered assistants you open directly from the admin pages you already use. Each assistant is scoped to one domain (customers, catalog) and knows only what its agent contract allows it to know. Every change the assistant proposes requires an explicit **Confirm** before it touches your data.
...
![AI assistants picker with three registered agents and Can write badges](/screenshots/helios-ai-assistant-available-assistants.png)
...
![Customers AI assistant searching for "Norton" and rendering people record cards in the chat](/screenshots/helios-ai-assistant-customers-search.png)
...
![Catalog Merchandising Assistant rendering a quick catalog overview stats card with Products, Active, Categories, and Tags counts](/screenshots/helios-ai-assistant-catalog-overview.png)
...
![Catalog Merchandising Assistant streaming three product record cards with status badges and SKUs](/screenshots/helios-ai-assistant-catalog-records.png)
...
![Catalog Merchandising Assistant Recent Sessions dropdown listing previous chat threads with timestamps](/screenshots/helios-ai-assistant-history.png)
...
![Mutation approval card showing customers.manage_record_activity proposal with field-by-field diff and Cancel/Confirm controls](/screenshots/helios-ai-assistant-mutations-approvals.png)
...
Helios can use local OpenAI-compatible servers for typed AI agents. This is useful for development, demos, offline work, or testing open-weight Llama models before choosing a hosted provider.
...
```bash
HELIOS_AI_PROVIDER=lm-studio
LM_STUDIO_API_KEY=lm-studio
LM_STUDIO_BASE_URL=http://localhost:1234/v1
HELIOS_AI_MODEL=<the model id shown by LM Studio>
```
...
```bash
HELIOS_AI_PROVIDER=ollama
OLLAMA_API_KEY=ollama
OLLAMA_BASE_URL=http://localhost:11434/v1
HELIOS_AI_MODEL=llama3.3
```

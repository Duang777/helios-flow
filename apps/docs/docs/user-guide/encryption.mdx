...
![Encryption map editor](/screenshots/helios-encryption-settings.png)
...
![Encrypted custom field toggle](/screenshots/helios-encrypted-custom-fields.png)
...
Auth users:
```bash
yarn helios auth rotate-encryption-key
yarn helios auth rotate-encryption-key --old-key <previous_fallback_key>
yarn helios auth rotate-encryption-key --tenant <tenantId> --org <organizationId>
```

All entities with encryption maps:
```bash
yarn helios entities rotate-encryption-key
yarn helios entities rotate-encryption-key --old-key <previous_fallback_key>
yarn helios entities rotate-encryption-key --tenant <tenantId> --org <organizationId>
```
...
yarn helios entities decrypt-database --tenant <tenantId> --confirm <tenantId> --dry-run

# Check encryption status and sampled payload estimate
yarn helios entities decrypt-database --tenant <tenantId> --check

# Full decryption
yarn helios entities decrypt-database --tenant <tenantId> --confirm <tenantId>

# Full decryption + deactivate encryption maps
yarn helios entities decrypt-database --tenant <tenantId> --confirm <tenantId> --deactivate-maps
```
...
3. Run `yarn helios query_index reindex --tenant <tenantId>` to rebuild search indexes.

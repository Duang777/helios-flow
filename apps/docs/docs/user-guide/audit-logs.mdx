...
![Audit log timeline showing undoable entries](/screenshots/helios-action-logs.png)
...
![Operation drawer highlighting changed fields and snapshots](/screenshots/helios-operation-details.png)

### Banner auto-dismiss timeout

The Undo banner auto-dismisses **10 seconds** after the action is recorded. The timer pauses while an undo request is in flight so a slow API does not collapse the banner mid-click. Override the default by setting `NEXT_PUBLIC_HELIOS_UNDO_BANNER_TIMEOUT_MS` (in milliseconds) at build time:

```env
# .env (or .env.local)
NEXT_PUBLIC_HELIOS_UNDO_BANNER_TIMEOUT_MS=15000
```

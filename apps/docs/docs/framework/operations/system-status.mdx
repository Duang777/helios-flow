...
### HELIOS_MODULE_RESOURCE_USAGE {#om_module_resource_usage}
**Default:** `true`
...
### HELIOS_MODULE_RESOURCE_USAGE_SNAPSHOT {#om_module_resource_usage_snapshot}
**Default:** `true` outside tests

Writes best-effort local process telemetry files under `.helios/module-resource-usage` so the admin page can include fresh data from sibling processes after restarts or worker activity.

### HELIOS_MODULE_RESOURCE_USAGE_DIR {#om_module_resource_usage_dir}
**Default:** `./.helios/module-resource-usage`

Overrides the directory used for local process telemetry files.

### HELIOS_MODULE_RESOURCE_HEAVY_P95_MS {#om_module_resource_heavy_p95_ms}
**Default:** `5000`

Threshold for flagging modules with slow p95 operation duration.

### HELIOS_MODULE_RESOURCE_HEAVY_CPU_MS {#om_module_resource_heavy_cpu_ms}
**Default:** `25000`

Threshold for flagging modules with high accumulated CPU time.

### HELIOS_MODULE_RESOURCE_HEAVY_HEAP_BYTES {#om_module_resource_heavy_heap_bytes}
**Default:** `262144000`

Threshold for flagging modules with high positive heap growth.

### HELIOS_MODULE_RESOURCE_HEAVY_RSS_BYTES {#om_module_resource_heavy_rss_bytes}
**Default:** `262144000`

Threshold for flagging modules with high positive RSS growth.

### HELIOS_MODULE_RESOURCE_HEAVY_ERRORS {#om_module_resource_heavy_errors}
**Default:** `10`
...
### HELIOS_PROFILE {#om_profile}
**Default:** _(unset)_  
Enables the backend tree profiler for requests matching the provided comma-separated filters. Useful for inspecting slow paths in server handlers. Related docs: [Profiling](https://docs.helios.com/docs/framework/performance/profiling).

### NEXT_PUBLIC_HELIOS_PROFILE {#next_public_om_profile}
**Default:** _(unset)_  
Activates the tree profiler in the browser bundle so client-rendered flows can emit the same snapshots. Set alongside `HELIOS_PROFILE` when debugging end-to-end latency.

### HELIOS_CRUD_PROFILE {#om_crud_profile}
**Default:** _(unset)_  
...
### HELIOS_QE_PROFILE {#om_qe_profile}
**Default:** _(unset)_  
...
**Default:** `false`  
Logs every SQL statement dispatched by the hybrid query engine. Enable temporarily when troubleshooting filters or sorting behaviour. Related docs: [Query index](https://docs.helios.com/docs/framework/database/query-index).
...
**Default:** `memory`  
Selects the cache backend. Supported values include `memory` (in-process) and `sqlite`. Related docs: [Cache tuning](https://docs.helios.com/docs/framework/performance/cache).
...
### HELIOS_PASSWORD_MIN_LENGTH {#om_password_min_length}
**Default:** `6`  
Minimum length enforced for new passwords (onboarding, user CRUD, password reset, CLI). To keep the frontend requirement hint aligned, set `NEXT_PUBLIC_HELIOS_PASSWORD_MIN_LENGTH` to the same value.

### HELIOS_PASSWORD_REQUIRE_DIGIT {#om_password_require_digit}
**Default:** `true`  
Require at least one digit in new passwords. Mirror in `NEXT_PUBLIC_HELIOS_PASSWORD_REQUIRE_DIGIT` to surface the requirement in frontend helpers.

### HELIOS_PASSWORD_REQUIRE_UPPERCASE {#om_password_require_uppercase}
**Default:** `true`  
Require at least one uppercase letter in new passwords. Mirror in `NEXT_PUBLIC_HELIOS_PASSWORD_REQUIRE_UPPERCASE` for frontend display.

### HELIOS_PASSWORD_REQUIRE_SPECIAL {#om_password_require_special}
**Default:** `true`  
Require at least one special character in new passwords. Mirror in `NEXT_PUBLIC_HELIOS_PASSWORD_REQUIRE_SPECIAL` for frontend display.
...
**Default:** `true`  
Allows the hybrid engine to serve results even when coverage is partial. Responses include an `x-helios-partial-index` header so admins can schedule a rebuild. Related docs: [Query index](https://docs.helios.com/docs/framework/database/query-index).

### QUERY_INDEX_WARMUP_ENABLED {#query_index_warmup_enabled}
**Default:** `true`  
Kill switch for the `query_index.coverage.warmup` sweep fired after login. Set to `false` to disable the proactive warmup entirely and rely solely on the query engine's lazy, on-demand coverage refresh. Related docs: [Query index](https://docs.helios.com/docs/framework/database/query-index).

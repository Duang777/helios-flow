...
Admin list pages usually rely on the `DataGrid` component from `@helios/ui`. It pairs beautifully with the [query engine](../database/query-engine) to deliver fast, tenant-aware listings.

## Minimal grid example

```tsx title="backend/inventory/items/page.tsx"
import { DataGrid } from '@helios/ui/backend/DataGrid';

...
- Each handler exports a unique `id` and registers itself by calling `registerCommand(handler)` when its command file is loaded.
- `yarn generate` creates lazy command loaders, so command files are imported on first `CommandBus.execute()` / `undo()` use instead of during module bootstrap.
...
Command files register handlers when loaded. The module generator discovers files under `commands/` and writes lazy loaders, so module `index.ts` does not need command side-effect imports:
...
The command bus loads the matching command file on demand. Manual imports are still useful in isolated tests that read `commandRegistry` directly:
...
Reusable helpers in `@helios/shared/lib/commands/helpers` cover common patterns:

...
The `ProgressTopBar` component (from `@helios/ui/backend/progress`) renders a collapsible bar showing active and recently completed jobs. It is placed between the header and main content in the `AppShell` with `sticky top-0` positioning.

```tsx
import { ProgressTopBar } from '@helios/ui/backend/progress/ProgressTopBar'
...
```tsx
import { useProgressPoll } from '@helios/ui/backend/progress/useProgressPoll'

...
Helios provides a comprehensive event/subscriber system with module auto-discovery, DI integration, and offline processing. The system supports two processing modes:
...
- Subscribers live under `packages/<pkg>/src/modules/<module>/subscribers/*.ts` (or `apps/helios/src/modules/<module>/subscribers/*.ts` for app-level overrides) and export:
...
- Subscribers discovered at build via `yarn generate` and registered into a global Event Bus via the core bootstrap (`@helios/core/bootstrap`), which your app calls from `apps/helios/src/di.ts`.
...
- Offline processing: `yarn helios events process [--limit=N]` replays unprocessed persistent events.
...
```bash
yarn helios events emit <event> [jsonPayload] [--persistent|-p]
```
...
yarn helios events emit example.event

# Emit with JSON payload (remember to quote it)
yarn helios events emit order.created '{"id":123,"total":42.5}'

# Emit a persistent event so it is queued for async processing
yarn helios events emit order.created '{"id":124}' --persistent

# Shorthand for persistent flag
yarn helios events emit order.created '{"id":125}' -p
```
...
- `source`: Bootstrap event source (for example `apps/helios`).

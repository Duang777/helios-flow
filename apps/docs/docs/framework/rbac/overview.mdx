...
description: Understand how Helios roles, features, and module metadata combine to protect pages, APIs, and workflows.
---

# RBAC Overview

Helios enforces access control through a two-layer RBAC system: role assignments scoped to organizations/tenants and feature flags that gate specific capabilities. Modules describe the features they expose, and runtime metadata or services evaluate whether a request is allowed.
...
Every module must list the features it exposes in `packages/<pkg>/src/modules/<module>/acl.ts` (or `apps/helios/src/modules/<module>/acl.ts` for app overrides). Export a `features` array with strings following the `<module>.<action>` convention:
...
![Editing role features](/screenshots/helios-managing-roles.png)
...
![Editing user ACL overrides](/screenshots/helios-edit-user-and-acl.png)

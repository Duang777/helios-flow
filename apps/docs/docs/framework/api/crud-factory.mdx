...
The CRUD factory (`makeCrudRoute`) is a powerful abstraction for building consistent, multi-tenant safe REST APIs. It integrates deeply with the helios architecture to provide validation, security, performance, and extensibility.
...
Define a `route.ts` under `packages/<pkg>/src/modules/<module>/api/<path>/route.ts` (or `apps/helios/src/modules/<module>/api/<path>/route.ts` for app overrides) and use the factory.
...
HELIOS_CRUD_CACHE_DEBUG = true; // Debug cache hits/misses
```
...
- `x-helios-cache: hit|miss` - Cache status
- `x-helios-partial-index` - Partial index warnings (JSON)
...
HELIOS_PROFILE=customers.*

# Profile CRUD operations only
HELIOS_CRUD_PROFILE=true
```

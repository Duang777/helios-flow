...
2. Background jobs or CLI commands (`yarn helios entities install --reindex`) populate and refresh the index.
...
- Trigger an initial backfill (for example, `yarn helios entities sync-index --entity inventory:item --tenant <tenantId>`).

...
Notes:
- The generator merges `entities[].fields` into `Module.customFieldSets` automatically, so `yarn helios entities install` will seed them.
...
```ts
import { registerIntegration } from '@helios/shared/modules/integrations/types'
...
Types: `IntegrationDefinition`, `ExternalIdEnrichment`, `ExternalIdMapping` from `@helios/shared/modules/integrations/types`.

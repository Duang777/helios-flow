...
Backfill
- CLI: `helios query_index rebuild --entity <module:entity>` reindexes existing rows with a live progress bar.
  - All orgs/tenants: `helios query_index rebuild --entity example:todo --global`
  - Scoped: `helios query_index rebuild --entity example:todo --org <orgId> --tenant <tenantId>`
  - Single record: `helios query_index rebuild --entity example:todo --record <entityId>`
  - Options: `--withDeleted` `--limit <n>` `--offset <n>` `--batch <size>`
- CLI: `helios query_index rebuild-all` walks every registered entity index using the same flags as `rebuild`, making it easy to refresh the entire corpus in one go.
- CLI: `helios query_index reindex [--entity <module:entity>] [--tenant <tenantId>] [--force]` (shorthand: `yarn helios reindex`) now splits work into five partitions by default. Override the granularity with `--partitions <n>` or target a single shard with `--partition-index <i>`. Combine with `--skipResetCoverage` on partitions >0 when running several workers concurrently. The command continues to honour `--batch <size>` for the inner chunk size (defaults to 500 rows).
  - Parallel run: start five terminals with `helios query_index reindex --entity example:todo --partitions 5 --partition-index <0-4>`; the partition `0` process resets coverage, so the remaining workers should pass `--skipResetCoverage`.
...
- Keep the indexed path even when gaps are detected by setting `FORCE_QUERY_INDEX_ON_PARTIAL_INDEXES=true` (enabled by default). When active, list responses include an `x-helios-partial-index` response header (with `entity`, `baseCount`, `indexedCount`, and `scope`) so the admin UI can surface a banner prompting the user to reindex via `/backend/query-indexes`.
...
- To inspect the tokens generated for search, set `HELIOS_SEARCH_DEBUG=true`. It logs field names, token **hashes**, and per-record counts during indexing — never the raw token text, document values, or decrypted PII — so the index shape can be debugged without disclosing data. Query behaviour is unchanged.

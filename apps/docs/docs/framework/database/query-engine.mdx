...
import { E } from '@helios/core/datamodel/entities'
import { id, email, name } from '@helios/core/datamodel/entities/user'
...
- `fields`: base columns and/or custom fields prefixed as `cf:<key>`. Prefer importing base columns from `@helios/<pkg>/datamodel/entities/<entity>` (e.g., `import { id, email } from '@helios/core/datamodel/entities/user'`).
...
You can get compile-time help for `filters` by using the generic `Where` type from `@helios/shared/lib/query/types`:

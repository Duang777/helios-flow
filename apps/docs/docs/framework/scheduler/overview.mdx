...
The Scheduler module (`@helios/scheduler`) provides a production-ready, database-managed scheduled job system with support for both development (local polling) and production (BullMQ) execution strategies.
...
import type { SchedulerService } from '@helios/scheduler'
...
```typescript
import { validateCron, parseCronExpression } from '@helios/scheduler'
...
```typescript
import { validateInterval, parseInterval, intervalToHuman } from '@helios/scheduler'
...
```typescript
import { calculateNextRun } from '@helios/scheduler'
...
# Filter by tenant
yarn helios scheduler list --tenant tenant-123
...
```typescript
import { parseInterval, validateCron, calculateNextRun } from '@helios/scheduler'
...
2. ✅ Run `yarn helios scheduler start` in a terminal
...
3. ✅ Run `yarn helios scheduler start` once to sync
4. ✅ Start workers: `yarn helios worker:start`
...
```bash
# Old cron entry
0 6 * * * cd /app && yarn helios reports generate-daily
...
**Debug:**
```bash
yarn helios scheduler run <schedule-id>  # Test manually
```

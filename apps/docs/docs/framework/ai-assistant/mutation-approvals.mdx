...
![Mutation approval card inside the AI chat showing a customers.manage_record_activity proposal with field-by-field diff and Confirm/Cancel buttons](/screenshots/helios-ai-assistant-mutations-approvals.png)

The canonical registry lives in `approval-cards-map.ts`:

```ts
import { AI_MUTATION_APPROVAL_CARDS } from '@helios/ui/ai/parts'

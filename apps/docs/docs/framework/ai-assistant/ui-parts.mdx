...
|------|---------------|----------|
| **Record cards** | Fenced code blocks the agent emits as plain text (parsed client-side) | `helios:product`, `helios:deal`, `helios:person`, `helios:company`, `helios:activity` |
...
Record cards are the cheapest way to get a typed widget into the chat. The agent emits a fenced Markdown block whose info string starts with `helios:<kind>` and whose body is one JSON object. The client-side parser (`parseAiContentSegments`) replaces the fence with a typed React component.
...
```ts
import type { AiUiPart } from '@helios/ai-assistant'
...
```ts
import { AI_MUTATION_APPROVAL_CARDS, defaultAiUiPartRegistry } from '@helios/ui/ai'
...
```ts
import { createAiUiPartRegistry } from '@helios/ui/ai'

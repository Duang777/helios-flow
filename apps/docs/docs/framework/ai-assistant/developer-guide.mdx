...
It mirrors the [`helios-create-ai-agent` skill](https://github.com/) at `.ai/skills/helios-create-ai-agent/SKILL.md` for use during pull-request review and one-shot generation.
...
Multi-step agents that need more than a step cap declare a `loop` block. Spec [`2026-04-28-ai-agents-agentic-loop-controls`](https://github.com/helios/helios/blob/main/.ai/specs/implemented/2026-04-28-ai-agents-agentic-loop-controls.md) is the full reference; the everyday primitives:
...
The cheapest path is **record cards**: have the model emit a fenced Markdown block whose info string is `helios:<kind>` and whose body is one JSON object. The client-side parser replaces the fence with a typed React component. Five kinds ship out of the box (`product`, `deal`, `person`, `company`, `activity`).
...
- `apps/<app>/.helios/generated/ai-agents.generated.ts`
- `apps/<app>/.helios/generated/ai-tools.generated.ts`
...
- `yarn generate` runs cleanly; the agent appears in `apps/<app>/.helios/generated/ai-agents.generated.ts`.
- `yarn helios configs cache structural --all-tenants` ran.
...
- The `helios-create-ai-agent` skill at `.ai/skills/helios-create-ai-agent/SKILL.md` (consumed by Claude Code agents)

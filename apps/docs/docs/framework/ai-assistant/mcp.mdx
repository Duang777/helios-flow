...
description: Connect Claude Code or any MCP client to Helios's tools — server modes, setup, what's exposed, reaching API endpoints, tools vs agents, extending, and troubleshooting.
---

# MCP Server

Helios ships an **MCP (Model Context Protocol) server** that exposes your tenant's tools to an external AI client — [Claude Code](https://claude.com/claude-code), the MCP Inspector, or any MCP-compatible client — over HTTP. Point the client at a running Helios server and it can discover and call your tools, scoped to an API key's permissions.
...
The MCP server exposes Helios's tools to an external AI client — Claude Code, MCP Inspector, or any MCP-compatible client — over HTTP using the MCP StreamableHTTP transport. Point Claude Code at a running Helios dev server and it can call your tenant's tools directly.
...
|---|---|---|
| Alias for | `helios ai_assistant mcp:dev` | `helios ai_assistant mcp:serve-http` |
...
| Key source | `.mcp.json` → `mcpServers.helios.headers.x-api-key` (no env fallback) | `MCP_SERVER_API_KEY` env var + session tokens |
...
1. **Create an API key.** Via the CLI:
   ...
   ```bash
   yarn helios api_keys add --name claude-code \
...
## Stdio server authentication (`helios ai_assistant mcp:serve`)

The `helios ai_assistant mcp:serve` command runs the MCP server over **stdio** (one process per client, used by some local MCP clients). It is a distinct command from the HTTP servers above — note that the `yarn mcp:serve` script is aliased to `mcp:serve-http`, so this stdio command is reached by invoking the `helios` CLI directly.

The stdio server resolves its auth context in priority order:

1. **API key** — pass `--api-key <secret>`, or set `HELIOS_API_KEY` (preferred — keeps the secret off `argv`, which is world-readable via `ps` / `/proc`). The key's tenant, organization, user, and ACL are loaded from the key.
...
Define the tool with `defineAiTool` from `@helios/ai-assistant` inside a module's `ai-tools/` directory and export it from the module's `ai-tools.ts`. These typed tools are also what AI agents reference in their `allowedTools`.
...
| `ERR_MODULE_NOT_FOUND: Cannot find package '@/.helios' …` then `Registered 0 module-contributed AI tools` (historical) | The generated registry was imported through the Next.js-only `@/` path alias, which a standalone Node process can't resolve | Update to a build where the loader resolves the generated registry from disk for standalone servers |
...
- `yarn helios ai_assistant mcp:list-tools --verbose` lists all registered tools and their required features.
:::

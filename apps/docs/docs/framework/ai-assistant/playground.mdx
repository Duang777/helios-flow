...
The playground also includes a **Tools** tab that lists every AI tool visible to the caller and marks which ones are whitelisted on the selected agent. Use it to verify that a tool pack was loaded, that required features are wired, and that destructive or bulk tools are labeled correctly before a demo.
...
3. Switch to the **Tools** tab and confirm the currently selected agent's whitelist is marked `Allowed`, while the rest stay `Not allowed`.

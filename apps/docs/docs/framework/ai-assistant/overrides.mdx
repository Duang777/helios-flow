...
if (process.env.HELIOS_DISABLE_DEFAULT_CATALOG_AGENT === '1') {
  applyAiAgentOverrides({
    'catalog.catalog_assistant': null,
  })
}
...
2. **`modules.ts` inline** — `entry.overrides.ai.agents` / `entry.overrides.ai.tools` on a `ModuleEntry`. Last entry per id wins.
...
import type { AiAgentOverridesMap } from '@helios/ai-assistant'
...
```ts
// src/bootstrap.ts
import { applyAiAgentOverrides } from '@helios/ai-assistant'

const allowMerchandising = process.env.HELIOS_FEATURE_MERCHANDISING !== '0'

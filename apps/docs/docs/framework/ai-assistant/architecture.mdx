...
|---------|-----------------|
| `@helios/ai-assistant` | Runtime: `runAiAgentText`, `runAiAgentObject`, `prepareMutation`, model factory, agent + tool registries, mutation-approval gate, pending-action repo + worker, dispatcher routes, health/agents APIs, agent settings UI |
| `@helios/ui` (`/ai`) | Client surface: `<AiChat>`, `<AiAssistantLauncher>`, `useAiChat`, `useAiChatUpload`, the UI-part registry, the four mutation approval cards, the typed record-card components |
| `@helios/core` | Domain modules contributing agents and tool packs (`customers`, `catalog`, `inbox_ops`, …). Each module ships `ai-agents.ts` + `ai-tools.ts` at its root |
| `@helios/cli` | The generator that aggregates per-module `ai-agents.ts` / `ai-tools.ts` into `apps/<app>/.helios/generated/ai-agents.generated.ts` + `ai-tools.generated.ts` |
| `@helios/shared` | Auth, RBAC, encryption, i18n helpers used by every dispatcher route. The agent runtime never imports from here directly except through DI |
...
POST /api/ai_assistant/ai/chat?agent=<module>.<agent>
        ...
        ├── createModelFactory()  — caller override → HELIOS_AI_<MODULE>_MODEL → agent default
...
| Table | Owner | Purpose |
|-------|-------|---------|
| `ai_pending_actions` | `@helios/ai-assistant` | One row per staged mutation. Drives the approval card and the confirm/cancel routes |
| `ai_agent_prompt_overrides` | `@helios/ai-assistant` | Per-tenant additive prompt-section overrides authored at `/backend/config/ai-assistant/agents` |
| `ai_agent_mutation_policy_overrides` | `@helios/ai-assistant` | Per-tenant downgrade-only mutation policy. Cannot escalate above the code-declared policy |
| `attachments` | `@helios/core` | Every chat upload lands here. The agent runtime resolves bytes / signed URL / extracted text from this table |
...
|----------------|--------|-------------|
| `apps/<app>/.helios/generated/ai-agents.generated.ts` | every `<module>/ai-agents.ts` (root file only) | `loadAgentRegistry()` at first agent invocation |
| `apps/<app>/.helios/generated/ai-tools.generated.ts` | every `<module>/ai-tools.ts` (root file only) | `registerGeneratedAiToolEntries()` at boot |
...
## Where each piece lives in `@helios/ai-assistant`
...
## Where each piece lives in `@helios/ui/ai`

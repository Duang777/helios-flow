...
description: Architecture, OAuth setup, environment variables, deploy runbook, and provider package contract for the Communications Hub and the IMAP / Gmail channel providers.
---
...
- `@helios/channel-imap` — credential-based IMAP polling + SMTP send
- `@helios/channel-gmail` — Gmail OAuth2 + History API polling + `gmail.users.messages.send`
...
- **Providers**: `packages/channel-{imap,gmail}/`
- **Module entry**: enable with `{ id: 'communication_channels', from: '@helios/core' }` plus one entry per provider you want active in `apps/helios/src/modules.ts`.
...
Each provider exports a `ChannelAdapter` from `@helios/core/modules/communication_channels/lib/adapter`. The hub's registry validates capability/method consistency at registration time (so an adapter that declares `capabilities.deleteMessage: true` without implementing `deleteMessage()` throws on boot).
...
```
@helios/scheduler "poll-channel" cron (5 minutes by default)
     │
     ▼
...
1. The synthetic `<om_TOKEN@helios.invalid>` Message-ID appended to the `References` header (RFC 6761 reserved TLD — guaranteed to round-trip through compliant MTAs).
...
`poll-tick.ts` enumerates two pools: (a) `status='connected'` channels due for normal polling, (b) `status='error'` channels whose `lastFailureAt` is older than `HELIOS_CHANNEL_AUTO_RECOVER_MINUTES`. A successful poll in pool (b) flips `status` back to `connected` automatically — operators no longer have to manually reconnect after a transient outage.
...
- **Gmail watch** expires after ~7 days. Daily 04:00 UTC cron (`gmail-renew-watch` queue, registered per-org in `setup.ts`) finds channels within `HELIOS_PUSH_RENEWAL_GMAIL_LEAD_HOURS` (default 24h) of expiry and triggers `pushRegister(...)` again.
...
#### 3. Register the client in Helios
...
|---|---|---|
| `HELIOS_HUB_OAUTH_STATE_KEY` | derived from `AUTH_SECRET` | AES-256-GCM key used to encrypt the OAuth state cookie. **Rotate this on a schedule** — see runbook below. |
| `HELIOS_HUB_OAUTH_STATE_TTL_SECONDS` | `300` (5 minutes) | TTL on the state cookie. OAuth callbacks older than this are rejected. |
| `HELIOS_HUB_POLL_DEFAULT_SECONDS` | `300` (5 minutes) | Default poll cadence applied to channels that don't set `poll_interval_seconds`. |
| `HELIOS_HUB_POLL_CONCURRENCY` | `10` | Max parallel poll-channel worker jobs per process. Bound by your DB connection pool. |
| `HELIOS_HUB_OUTBOUND_RETRY_MAX` | `5` | Outbound delivery retries before marking the Message `delivery_failed`. |
| `HELIOS_HUB_POLL_SCHEDULER_TICK_SECONDS` | `60` | Interval between `poll-tick` scheduler runs that enumerate due channels and fan out to per-channel `poll` jobs. Lower-bounded at 10 s. |
| `HELIOS_CHANNEL_IMAP_HARD_CAP_PER_POLL` | `200` | Maximum number of IMAP UIDs fetched per `fetchHistory` page. When more remain, the adapter sets `hasMore: true` and the hub re-enqueues immediately. Tune lower if a single page's wall-clock budget is too tight; never set above ~500 (DB transaction size). |
| `HELIOS_CHANNEL_AUTO_RECOVER_MINUTES` | `30` | Spec B § B5 auto-recovery sweep window. `poll-tick` re-enqueues `status='error'` channels whose `lastFailureAt` is older than this. Set `0` in tests to force immediate retry. |
...
| `HELIOS_GMAIL_PUBSUB_TOPIC` | (none — required for Gmail push) | Fully-qualified Pub/Sub topic, e.g. `projects/helios-prod/topics/gmail-inbound`. Passed to `gmail.users.watch`. |
| `HELIOS_GMAIL_PUBSUB_AUDIENCE` | (none — required for Gmail push) | Expected `aud` claim in the Pub/Sub-signed JWT. Set this to the webhook URL or a stable identifier configured on the Pub/Sub push subscription. |
| `HELIOS_GMAIL_PUBSUB_SERVICE_ACCOUNT_EMAIL` | (none — required for Gmail push) | Expected `email` claim in the Pub/Sub JWT. Typically the service account configured on the push subscription (NOT `gmail-api-push@system.gserviceaccount.com` — that's the publisher into the topic). |
| `HELIOS_PUSH_RENEWAL_GMAIL_LEAD_HOURS` | `24` | Renewal lead time for `gmail.users.watch`. Lower means we renew earlier; higher means we cut closer to the 7-day expiration. |
...
2. **Sync the ACL grants** — `yarn helios auth sync-role-acls` so the new `communication_channels.*` features (including `communication_channels.connect_user_channel`) are granted to the appropriate roles.
3. **Run the structural cache purge** — `yarn helios configs cache structural --all-tenants`. Mandatory after any `modules.ts` change that adds a provider (slices 3e/3f enable `channel_imap` and `channel_gmail`).
...
5. **Optional: set `HELIOS_HUB_OAUTH_STATE_KEY`** — derived from `AUTH_SECRET` by default; set explicitly if you want to rotate independently.

### Routine ops

| Task | When | How |
|---|---|---|
| Rotate the OAuth state-cookie key | Every 90 days (recommended), or after any suspected compromise | Set `HELIOS_HUB_OAUTH_STATE_KEY` to a new random 32-byte base64 value and roll the deploy. In-flight OAuth flows fail with `state expired` and the user retries — that's expected and safe. |
...
4. Add `{ id: 'channel_<provider>', from: '@helios/channel-<provider>' }` to `apps/helios/src/modules.ts`.
5. Run `yarn generate && yarn build:packages && yarn test`, then `yarn helios configs cache structural --all-tenants`.
...
- **OAuth state cookie**: 5-minute TTL, AES-256-GCM, HKDF-derived from `HELIOS_HUB_OAUTH_STATE_KEY`, bound to the initiating user's id. Catches replays + replay-across-users.
...
HELIOS_INTEGRATION_MODULES=communication_channels,channel-imap,channel-gmail \

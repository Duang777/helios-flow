...
The `checkout` module lives in the standalone `@helios/checkout` package and adds Phase A pay links to Helios. It is intentionally decoupled from `sales`: merchants can collect one-off payments through shareable URLs without creating quotes or orders.

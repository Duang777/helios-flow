...
```typescript
import type { NotificationService } from '@helios/core/modules/notifications'
...
yarn helios queue worker notifications

# Custom concurrency
WORKERS_NOTIFICATIONS_CONCURRENCY=10 yarn helios queue worker notifications
```
...
import { NotificationPanel } from '@helios/ui/backend/notifications'
...
// Bad: Direct coupling
import { notificationService } from '@helios/core/modules/notifications'
await notificationService.create(...)
```
...
1. Check queue worker is running: `yarn helios queue status notifications`
...
3. Run cleanup: `yarn helios notifications cleanup-expired`

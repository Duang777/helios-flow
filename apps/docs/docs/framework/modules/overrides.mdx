...
The default app and `create-helios-app` template also export a non-applied `moduleOverrideExamples` object from `src/modules.ts`. It is a copyable catalog for all wired domains; it does not change runtime behavior unless you assign one of those shapes to a module entry's `overrides` field.

...
```ts
import type { GatewayAdapter } from '@helios/shared/modules/payment_gateways/types'
...
The Stripe provider is designed around **Payment Intents**. Helios creates the intent through the payment hub, stores it as a `GatewayTransaction`, and gives your app enough data to either:
...
Helios creates the Stripe Payment Intent through `POST /api/payment_gateways/sessions` and stores the result as a `GatewayTransaction`.
...
- `transactionId`: Helios identifier used later for status, capture, refund, cancel, and admin tracking
...
How Stripe intent statuses map into Helios:
...
const statusResponse = await fetch(
  ...
  {
    headers: { 'X-Api-Key': process.env.HELIOS_API_KEY! },
  },
)
...
- `status`: the unified Helios payment status
...
Stripe should deliver webhook events into Helios so the payment hub can update transaction status automatically, append webhook history, and record provider logs. Session creation can work without webhooks, but the integration is incomplete without them.

### Production / hosted environments

1. Open **Stripe Dashboard -> Workbench -> Webhooks**.
2. Create a webhook endpoint for your Helios app.
...
5. Reveal the endpoint signing secret in Stripe and copy the `whsec_...` value into **Settings -> Integrations -> Stripe -> Webhook Signing Secret** in Helios.
...
6. Reveal the endpoint signing secret and paste that `whsec_...` value into the Stripe integration credentials form in Helios.
...
```ts
import { registerGatewayAdapter } from '@helios/shared/modules/payment_gateways/types'
...
Custom payment gateway adapters you build can be published to the [Official Modules repository](./official-modules). Gateway providers go through core-team review to ensure security and reliability before being listed on npm — giving your provider visibility and one-command installation for all Helios users. See [Official Modules — Publishing your module](./official-modules#publishing-your-module) for details.

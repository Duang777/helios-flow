...
This guide explains how to create and configure routes and pages in the Helios framework, including authentication, authorization, and metadata configuration.
...
The Helios framework uses a module-based architecture where routes and pages are automatically discovered and registered. Routes are organized by module and can be either:
...
The app's `bootstrap.ts` calls `applyModuleOverridesFromEnabledModules(enabledModules)` from `@helios/shared/modules/overrides` before the catch-all API route loads the manifest. Both `apps/helios` and the `create-helios-app` template ship that wiring out of the box.
...
This comprehensive guide covers all aspects of creating routes and pages in the Helios framework. For more specific examples, see the [API data fetching tutorial](../../tutorials/api-data-fetching).

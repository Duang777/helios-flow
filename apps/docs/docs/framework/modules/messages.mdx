...
![ Messages list](/screenshots/helios-messages-list.png)

### 2) Compose messages

![Composing a message](/screenshots/helios-messages-compose.png)

### 3) Attaching objects while composing a message

![Attaching object to message](/screenshots/helios-messages-compose-object-attachment.png)

### 4) Message details

![Attaching object to message](/screenshots/helios-messages-details.png)
...
import { MessagesIcon } from '@helios/ui/backend/messages'
...
All exported from `@helios/ui/backend/messages`.

## Message Types (Module Extension)

Message type definitions are declared in each module's `message-types.ts` and auto-registered from generated bootstrap imports (`@/.helios/generated/message-types.generated` in `apps/helios/src/bootstrap.ts`).
...
Object type definitions are declared in each module's `message-objects.ts` and auto-registered from generated bootstrap imports (`@/.helios/generated/message-objects.generated` in `apps/helios/src/bootstrap.ts`).

...
```ts title="apps/helios/src/modules/taxes/subscribers/tax-hooks.ts"
...
```ts title="apps/helios/src/modules/pricing/di.ts"
import { registerCatalogPricingResolver } from '@helios/core/src/modules/catalog/lib/pricing'
...
```ts title="apps/helios/src/modules/surcharges/di.ts"
import {
  registerSalesTotalsCalculator,
  type SalesTotalsCalculationHook,
} from '@helios/core/src/modules/sales/lib/calculations'
import type { AppContainer } from '@helios/shared/lib/di/container'
...
```ts title="apps/helios/src/modules/discounts/subscribers/sales-calculation.ts"
...
```ts title="apps/helios/src/modules/taxes/di.ts"
import { asFunction } from 'awilix'
import type { AppContainer } from '@helios/shared/lib/di/container'
import type { TaxCalculationService, CalculateTaxInput, TaxCalculationResult } from '@helios/core/src/modules/sales/services/taxCalculationService'
...
- Emit profiling by setting `HELIOS_PROFILE=customers.*` (or `NEXT_PUBLIC_HELIOS_PROFILE` in the browser) to capture pricing/tax spans during slow paths.

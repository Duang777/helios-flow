...
```bash
npx playwright test --config .ai/qa/tests/playwright.config.ts apps/helios/src/modules/example/__integration__/TC-UMES-003.spec.ts
```
...
```bash
NEXT_PUBLIC_HELIOS_CRUDFORM_EXTENDED_EVENTS_ENABLED=false
```
...
- Module overrides are auto-discovered from `widgets/components.ts` (`componentOverrides`) and generated to `.helios/generated/component-overrides.generated.ts`.
...
```ts
import { InjectionWizard } from '@helios/ui/backend/injection/InjectionWizard'
import type { InjectionWizardWidget, InjectionContext } from '@helios/shared/modules/widgets/injection'
...
```ts
import { StatusBadgeRenderer } from '@helios/ui/backend/injection/StatusBadgeRenderer'
import type { InjectionStatusBadgeWidget, StatusBadgeContext } from '@helios/shared/modules/widgets/injection'
...
```typescript
import { generateCrudFormInjectionSpotId, CrudFormInjectionSpots } from '@helios/ui/backend/injection/helpers'
...
```typescript
import { useInjectionSpotEvents } from '@helios/ui/backend/injection/InjectionSpot'
...
} from '@helios/ui/backend/injection/mutationEvents'
import { withScopedApiRequestHeaders } from '@helios/ui/backend/utils/apiCall'
...
![Injected form widget banner](/screenshots/helios-widget-injection-form.png)

![Injected validation card that blocks save](/screenshots/helios-widget-injection-validation.png)

![Injected data table widget in products list](/screenshots/helios-injection-data-table.png)
...
1. Import types from `@helios/shared/modules/widgets/injection`

...
description: Guide to creating background job workers for processing queue jobs in Helios modules.
---

# Creating Workers

Workers are background processes that handle asynchronous job processing. Helios provides a worker discovery system that automatically registers workers defined in your modules.
...
yarn helios queue worker my-queue
```
...
```typescript
import type { WorkerMeta } from '@helios/queue'
...
yarn helios queue worker fulltext-indexing --concurrency=10
...
yarn helios queue worker events --concurrency=10
yarn helios queue worker fulltext-indexing --concurrency=5
```

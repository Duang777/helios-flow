...
Helios ships a two-tier locking story. The tiers are **layered**, not
...
1. **OSS — optimistic locking via `updated_at`.** Lightweight, additive,
   ...
   with `HELIOS_OPTIMISTIC_LOCK=off`.
2. **Enterprise — collaborative-editing enrichment** via the
   [`record_locks`](https://github.com/helios/helios/tree/develop/packages/enterprise/src/modules/record_locks)
   ...
   Enabled with `HELIOS_ENABLE_ENTERPRISE_MODULES=true`.
...
`HELIOS_OPTIMISTIC_LOCK` kill switch.
...
- Requests that omit the `x-helios-ext-optimistic-lock-expected-updated-at`
...
Set `HELIOS_OPTIMISTIC_LOCK=off` in the server environment (also accepted:
...
`HELIOS_OPTIMISTIC_LOCK=off` is the **single kill switch for both tiers**. When the
...
│  header on next      │         │                  ├─ read HELIOS_OPTIMISTIC_LOCK
...
`HELIOS_OPTIMISTIC_LOCK` lets you narrow or disable the default coverage:
...
HELIOS_OPTIMISTIC_LOCK=customers.company,sales.order
```
...
`apps/helios`-managed modules (customers, sales, catalog, attachments,
...
- Header name follows the project's `x-helios-ext-<moduleId>-<key>`
...
- Registration is unconditional — the guard's mode check short-circuits
  when `HELIOS_OPTIMISTIC_LOCK=off`, so registering the reader at boot is
...
2. Wire the client page(s) to use `buildOptimisticLockHeader(...)` and
   ...
   `@helios/ui/backend/conflicts` — or pass
...
3. Add `<entityType>` to `HELIOS_OPTIMISTIC_LOCK` in your deployment, or
   set `HELIOS_OPTIMISTIC_LOCK=all` to opt in platform-wide.
4. Add an integration test that mirrors
   [`TC-LOCK-OSS-001`](https://github.com/helios/helios/blob/develop/packages/core/src/modules/customers/__integration__/TC-LOCK-OSS-001.spec.ts):
...
[#2232](https://github.com/helios/helios/issues/2232).
...
The kill switch is shared: `HELIOS_OPTIMISTIC_LOCK=off` disables **both** tiers (see
...
via `surfaceRecordConflict(err, t)` from `@helios/ui/backend/conflicts`
...
- **Spec (OSS floor):** [`.ai/specs/implemented/2026-05-25-oss-optimistic-locking.md`](https://github.com/helios/helios/blob/develop/.ai/specs/implemented/2026-05-25-oss-optimistic-locking.md)
- **Spec (enterprise layering):** [`.ai/specs/enterprise/2026-06-09-record-locks-unified-coverage.md`](https://github.com/helios/helios/blob/develop/.ai/specs/enterprise/2026-06-09-record-locks-unified-coverage.md) — how `record_locks` layers on the OSS floor across every lock site.
- **Source files:**
...
`x-helios-ext-optimistic-lock-expected-updated-at` pass through unchanged.

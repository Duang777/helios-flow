...
description: Deploy a standalone Helios app to Railway with one repeatable CLI command.
---

# Deploy to Railway

`helios deploy railway` provisions and deploys a standalone Helios app to Railway. It creates the project, environment, PostgreSQL, Redis, app service, optional worker, environment variables, healthcheck, and public domain.
...
:::info Availability
This command was implemented for [issue #2414](https://github.com/helios/helios/issues/2414). Use a CLI release that includes `helios deploy railway`; older releases do not provide it.
:::
...
- A standalone Helios app created with `create-helios-app`.
...
- A source strategy:
  ...
  - **Local-source**: your app is not on GitHub, or you want to upload the current directory directly. This mode requires the Railway CLI because Helios shells out to `railway up`.
...
- With GitHub: push the current branch to GitHub and run `yarn helios deploy railway`. `auto` uses the GitHub remote when it is clean, pushed, and supported by Railway.
- Without GitHub: install the Railway CLI, then run `yarn helios deploy railway --source local`. `auto` also falls back to this path when no usable GitHub remote exists.
...
```bash
yarn helios deploy railway --dry-run
```
...
- creates `helios-app` and `helios-worker`,
...
Use this mode when your app lives in a GitHub repository that Railway can access. Helios configures Railway resources through the Railway API, then points the app service at the repository and branch.
...
yarn helios deploy railway --source git
```
...
```bash
railway --version
yarn helios deploy railway --source local
```
...
On macOS and Linux, the file must have mode `0600`. The CLI refuses a more widely readable token file. On Windows, the equivalent location is `%APPDATA%\helios\railway.json`; protect the file with appropriate NTFS permissions and rotate the token after use on a shared machine.
...
| `--service <name>` | `helios-app` | App service name. |
| `--worker` / `--no-worker` | worker enabled | Create a dedicated `helios-worker` service or remove the recorded managed worker and run workers in the app service. |
...
| `--no-track` | off | Store state in ignored `.helios/railway.json.local` instead of `.helios/railway.json`. |
...
| `HELIOS_AI_PROVIDER` | Loaded from the env file; defaults to `openai`. |
| `HELIOS_AI_MODEL` | Loaded from the env file; defaults to `gpt-5-mini`. |
...
| `AUTO_SPAWN_WORKERS` / `HELIOS_AUTO_SPAWN_WORKERS` | Disabled in the app when a dedicated worker is provisioned; the worker service also disables nested worker spawning. |
...
By default the command writes `.helios/railway.json`. It contains opaque Railway IDs, environment records, service and domain IDs, source metadata, last deployment IDs, a schema version, and the CLI version. It does not contain tokens, passwords, generated secrets, or rendered database URLs.
...
Use `--no-track` to keep deployment state local. The command then writes `.helios/railway.json.local`, which should remain ignored. A fresh clone without that state cannot identify the existing project and may create another one.

Deploy another environment under the same project with:

```bash
yarn helios deploy railway --env staging
```
...
```bash
yarn helios deploy railway --domain commerce.example.com
```
...
```bash
yarn helios deploy railway --volume /app/storage
```
...
```bash
yarn helios deploy railway --cleanup
```
...
```bash
yarn helios deploy railway --cleanup --non-interactive --yes
```
...
- Never commit env files, Account tokens, private keys, or `.helios/railway.json.local`.
- Review `--dry-run` output before the first deploy.
- Keep `.helios/railway.json` only when sharing opaque deployment IDs is intentional.
...
For Helios deployment, set only the Account token afterward:

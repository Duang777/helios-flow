...
`yarn test:integration:ephemeral` executes `yarn helios test:integration`.
...
yarn helios test:integration

# Direct CLI command (module/command form)
yarn helios test integration

# Direct CLI command (interactive menu)
yarn helios test:integration:interactive
# or
yarn helios test interactive

# Direct CLI command (coverage)
yarn helios test:integration:coverage
# or
yarn helios test coverage
```

## Coverage command

Use `yarn test:integration:coverage` to measure real Helios runtime code coverage driven by integration tests.
...
- `HELIOS_INTEGRATION_APP_READY_TIMEOUT_SECONDS=<seconds>` increases the ephemeral app readiness timeout (default `90`). This is the knob to raise in slower CI runners, for example `HELIOS_INTEGRATION_APP_READY_TIMEOUT_SECONDS=180 yarn test:integration:coverage`.
...
yarn helios test:integration --filter auth/

# Keep environment alive for debugging after tests
yarn helios test:integration --keep --verbose

# Always start a new ephemeral runtime (no attach/reuse)
yarn helios test:integration --no-reuse-env
```

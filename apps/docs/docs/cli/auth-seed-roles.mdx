---
title: helios auth seed-roles
sidebar_label: auth seed-roles
description: Ensures baseline roles exist for the authentication module.
---

`yarn helios auth seed-roles` inserts or verifies the default role set used by the authentication module.

## Usage

```bash
yarn helios auth seed-roles
```
...
- Before calling [`helios auth setup`](./auth-setup) in CI or scripts to make sure dependent roles exist.

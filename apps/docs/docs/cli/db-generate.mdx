...
`yarn db:generate` executes `helios db generate`, iterating through every enabled module and creating MikroORM migrations when entity metadata changes.
...
- Apply migrations with [`yarn db:migrate`](./db-migrate) or `yarn helios db migrate`.

...
description: How modules can expose custom CLI commands through the helios CLI.
---

Modules can register custom CLI commands that are automatically discovered and made available through the `helios` CLI. This allows you to extend the CLI with module-specific functionality.
...
4. Your commands become available as `yarn helios <module> <command>`

## Example: Creating a custom CLI command

```ts title="packages/inventory/src/modules/inventory/cli/sync.ts"
import type { CliCommandHandler } from '@helios/shared/cli';
...
2. The module is enabled in `apps/helios/src/modules.ts`

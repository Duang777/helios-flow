...
1. `helios generate` – regenerates module registries, dependency injection bindings, entity ID maps, ORM entities, and API client.
2. `helios db migrate` – runs MikroORM migrations for every enabled module in alphabetical order.
3. `helios entities install --global` – ensures global custom entity and field definitions are synchronized.
...
- After upgrading to a newer Helios version.
...
- **Custom entity sync errors** — ensure Redis (if configured) is reachable so cache invalidation succeeds, or run `yarn helios entities install --force` afterwards.

---
title: helios auth sync-role-acls
...
yarn helios auth sync-role-acls --tenant <tenantId> --no-superadmin
```
...
- **Not a replacement for `helios auth seed-roles`.** This command assumes the roles already exist — it does not create them.
...
- **`❌ Invalid --tenant value: …`** — the value was empty or whitespace. Pass a UUID from `yarn helios auth list-tenants`.

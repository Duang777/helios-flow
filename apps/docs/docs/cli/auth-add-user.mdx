---
title: helios auth add-user
sidebar_label: auth add-user
description: Adds a confirmed user to an existing organization with optional role assignments.
---

`yarn helios auth add-user` creates a new user, hashes the provided password, and links the account to roles in the same tenant as the organization.

## Usage

```bash
yarn helios auth add-user \
  --email "<user@email>" \
  --password "<password>" \
  --organizationId <uuid> \
  [--roles role1,role2]
...
```bash
ORG_ID=$(yarn helios auth list-orgs | awk 'NR==5 {print $1}')
yarn helios auth add-user \
...
- **Unknown organization** – confirm the UUID exists (`yarn helios auth list-orgs`) and that the CLI can connect to the database.
...
- **Password policies** – the CLI enforces the same rules as the app. Configure them with `HELIOS_PASSWORD_MIN_LENGTH`, `HELIOS_PASSWORD_REQUIRE_DIGIT`, `HELIOS_PASSWORD_REQUIRE_UPPERCASE`, and `HELIOS_PASSWORD_REQUIRE_SPECIAL`.

---
title: helios example seed-todos
sidebar_label: example seed-todos
description: Seeds sample Todo records and custom field data for the example module.
---

`yarn helios example seed-todos` populates example Todos for a tenant/organization pair and ensures the module’s custom entity definitions are installed.

## Usage

```bash
yarn helios example seed-todos --org <organizationId> --tenant <tenantId>
```
...
TENANT_ID=$(yarn helios auth list-tenants | awk 'NR==5 {print $1}')
yarn helios example seed-todos --org "$ORG_ID" --tenant "$TENANT_ID"
```

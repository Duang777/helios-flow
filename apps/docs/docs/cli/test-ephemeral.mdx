...
description: Starts a disposable Helios app + database for manual QA or MCP-based exploration.
---

`yarn test:integration:ephemeral:start` executes `yarn helios test:ephemeral`.
...
yarn helios test:ephemeral --verbose

# Always start a fresh instance on an isolated port
yarn helios test:ephemeral --no-reuse-env
```

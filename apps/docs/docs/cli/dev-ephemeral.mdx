...
`yarn dev:ephemeral` starts Helios development mode on a free local port with a dedicated ephemeral PostgreSQL container, prints the runtime URLs, and opens the same splash status screen used by the regular dev flow so startup progress stays visible until the app is reachable.
...
2. Creates `apps/helios/.env` from `apps/helios/.env.example` only when missing.
3. Runs `yarn install`.
4. Runs `yarn build:packages` to ensure CLI generators are up to date.
5. Runs `yarn generate` to prepare `.helios/generated` files.

...
`yarn db:greenfield` executes `helios db greenfield --yes`. It is designed for **development only** when you want to rebuild the schema from scratch.
...
5. Regenerates fresh migrations for every enabled module by internally calling `helios db generate`.
6. Applies the newly generated migrations immediately via `helios db migrate`.

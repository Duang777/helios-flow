---
title: helios entities install
sidebar_label: entities install
description: Synchronizes module-defined custom entities and fields into the database.
---

`yarn helios entities install` reads custom entity definitions from every enabled module and installs them for the desired scope. It is typically executed after `yarn db:migrate` or any time module metadata changes.

## Usage

```bash
yarn helios entities install [--tenant <tenantId>] [--global] [--no-global] [--dry-run] [--force]
```
...
```bash
yarn helios entities install --tenant 81c3b8b0-6f06-44f6-9c22-12c0d9ec3c63 --dry-run
```

Install only global definitions (no tenant overrides):

```bash
yarn helios entities install --global
```

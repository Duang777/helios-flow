...
`yarn helios eject` copies a module's source code from its installed package (`node_modules`) into your app's `src/modules/` directory and switches its origin in `modules.ts` to `from: '@app'`. After ejecting, the module behaves as a local module that you can freely modify.
...
`yarn helios module eject <moduleId>` is an alias for this command, provided for consistency with the `module` command namespace. Both are identical in behavior.
...
Ejectable modules:
  ...
  currencies (from: @helios/core) — Multi-currency management
  catalog (from: @helios/core)
  customers (from: @helios/core)
  ...
...
Next steps:
  1. Run generators:  yarn helios generate all
...
- **Module not found** — the module must be listed in your `src/modules.ts`. Run `yarn helios eject --list` to see what is available.
...
- **Build errors after ejection** — run `yarn helios generate all` to regenerate the module registry and DI bindings, then verify with `yarn build`.

---
title: helios configs cache
sidebar_label: configs cache
description: Inspect and purge Helios cache entries from the CLI by segment, tag, key, identifier token, pattern, or structural navigation scope.
---

`yarn helios configs cache` is the operator-facing cache tool for Helios. It exposes the same CRUD/widget segment view as the **Backend → Configuration → Cache** admin page and adds lower-level purge modes for tags, exact keys, identifier tokens, and key patterns.
...
yarn helios configs cache stats --tenant <tenantId>

# Purge everything in one scope
yarn helios configs cache purge --all --tenant <tenantId>

# Purge one CRUD/widget segment
yarn helios configs cache purge --segment admin-nav --tenant <tenantId>

# Purge by cache tag, exact key, identifier token, or glob pattern
yarn helios configs cache purge --tag nav:sidebar:user:<userId> --tenant <tenantId>
yarn helios configs cache purge --key nav:sidebar:en:<userId>:<tenantId>:<orgId> --tenant <tenantId>
yarn helios configs cache purge --id <userId> --tenant <tenantId>
yarn helios configs cache purge --pattern 'nav:*' --all-tenants
...
```bash
yarn helios configs cache purge --pattern 'nav:*' --all-tenants --dry-run --json
```

...
The legacy `helios scaffold` commands have been removed while we rebuild them around template files. Until the new generators are ready, create the module skeleton manually (or copy it from `packages/example/src/modules/example`):
...
} from "@helios/ui/backend/DataTable";
import {
  fetchCrudList,
  buildCrudExportUrl,
...
- Define CFs in `ce.ts` (entities[].fields); run `yarn helios entities install` (optionally with `--tenant <id>`) to sync them.
- Use the helpers from `@helios/shared/lib/crud/custom-fields` to stay DRY.
...
That’s it — you’ve built a complete, DRY CRUD API and UI flow aligned with Helios conventions.

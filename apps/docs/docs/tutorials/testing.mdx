...
This guide shows how to write and run unit tests for Helios using Jest + ts-jest. It covers services, API handlers, and utilities, and explains how to keep tests out of the runtime bundle.
...
- `@helios/core/*`, `@helios/cli/*`, `@helios/shared/*`, and other workspace packages
- Fallback: `@/*` → `apps/helios/src/*`
...
```ts
import { someUtil } from '@helios/shared/lib/utils'

---
title: Tutorial - build your first Helios app
description: From cloning the repo to shipping your first module override with database migrations.
---

# Tutorial: Build Your First Helios App

This tutorial walks you through:
- Bootstrapping an Helios app (DB + CLI + dev)
- Adding your own module as a package
- Overriding the auth login screen from the app overlay

## 1) Prerequisites
- Node.js 24+ (enforced by preinstall check; use `nvm use 24` or `fnm use 24` to switch)
- Docker & Docker Compose (for PostgreSQL, Redis, and Meilisearch)
- Copy `apps/helios/.env.example` to `apps/helios/.env` and set:
  - `DATABASE_URL=postgres://postgres:postgres@localhost:5432/helios`
  - `JWT_SECRET=some-strong-secret`
  - `REDIS_URL=redis://localhost:6379`

## 2) Install and Prepare
- Start Docker services: `docker compose up -d`
- Install deps: `yarn install`
- Bootstrap the app: `yarn initialize`

This single command prepares module registries, generates/applies migrations, seeds default roles, provisions an admin user, and loads demo CRM data.

Alternatively, you can run the steps manually:
- Generate modules (registry, entities, DI): `yarn generate`
- Generate DB migrations (per enabled module): `yarn db:generate`
- Apply migrations: `yarn db:migrate`

## 3) Seed Roles and Create Admin
If you ran `yarn initialize`, this step is already done. Otherwise:
- Seed default roles: `yarn helios auth seed-roles`
- Create the first tenant/org/admin:
  - `yarn helios auth setup --orgName "Acme" --email admin@acme.com --password secret --roles superadmin,admin --skip-password-policy`

## 4) Run the App
- `yarn dev`
- Open http://localhost:3000/backend

## 5) How Modules Load and Override
- Core modules live in packages (`packages/core`, `packages/search`, etc.) and are enabled in `apps/helios/src/modules.ts`.
- App-level overrides live under `apps/helios/src/modules/<module>/...` and take precedence over packages with the same relative path.
- Generators discover routes/APIs/DI/i18n/entities for enabled modules and write combined outputs to `generated/`.

## 6) Create Your Own Module (as a package)

1. Scaffold a local package:

```
packages/my-module/package.json
{
  "name": "@helios/my-module",
  "version": "0.1.0",
  "private": true,
  "type": "module",
  "exports": { "./modules/*": "./src/modules/*" }
}
```

2. Add source tree:

```
packages/my-module/src/modules/my_module/
  index.ts             # module metadata
  acl.ts               # (optional) RBAC features
  setup.ts             # (optional) default role features, tenant init
  di.ts                # (optional) register(container)
  frontend/
    page.tsx           # serves "/my_module" (Next-style page)
  backend/
    page.tsx           # serves "/backend/my_module"
  api/
    hello.ts           # new API at "/api/my_module/hello"
  i18n/
    en.json            # (optional) module dictionary
  data/
    entities.ts        # (optional) MikroORM entities
    validators.ts      # (optional) zod validation schemas
```

Example `index.ts`:
```ts
import type { ModuleInfo } from '@helios/shared/modules/registry'
export const metadata: ModuleInfo = {
  name: 'my_module',
  title: 'My Module',
  version: '0.1.0',
  description: 'A custom module.',
}
```

3. Add a TS alias so imports resolve:

- In `tsconfig.json` paths add: `"@helios/my-module/*": ["./packages/my-module/src/*"]`

4. Enable the module in `apps/helios/src/modules.ts`:

```ts
export const enabledModules: ModuleEntry[] = [
  { id: 'auth', from: '@helios/core' },
  { id: 'directory', from: '@helios/core' },
  { id: 'customers', from: '@helios/core' },
  { id: 'my_module', from: '@helios/my-module' },
]
```

5. Regenerate + run:
- `yarn generate`
- `yarn dev`

Now visit `/my_module` and `/backend/my_module`.

:::tip App-level modules
For simpler modules that do not need to be published as separate packages, you can place them directly under `apps/helios/src/modules/` and use `from: '@app'` instead. See the [Customization Tutorials](../customization/create-first-module) for this approach.
:::

## 7) Override the Auth Login Screen

To customize login without touching core, create the override file in the app overlay:

```
apps/helios/src/modules/auth/frontend/login.tsx
```

This file overrides the package page `@helios/core/modules/auth/frontend/login.tsx`.
Delete it to fall back to the package implementation.

## 8) Override Services (DI)

Use `apps/helios/src/di.ts` to register app-level DI overrides (runs after all module registrars):

```ts
import type { AppContainer } from '@helios/shared/lib/di/container'
import { asClass } from 'awilix'
// import { CustomAuthService } from './services/CustomAuthService'

export function register(container: AppContainer) {
  // container.register({ authService: asClass(CustomAuthService).scoped() })
}
```

## 9) Entities and Migrations
- Place package entities in `packages/<pkg>/src/modules/<module>/data/entities.ts`.
- To override/extend in the app: `apps/helios/src/modules/<module>/data/entities.override.ts`.
- Generate migrations: `yarn db:generate` (writes to `packages/<pkg>/src/modules/<module>/migrations`; app-local modules write to `apps/helios/src/modules/<module>/migrations`).
- Apply migrations: `yarn db:migrate`.

## 10) CLI Commands
- Each module can expose CLI in `modules/<module>/cli.ts`.
- List and run via `yarn helios <module> <command> [...args]`.
- Add app-level commands in `apps/helios/src/cli.ts` (listed under module `app`).

You now have an app running with core modules, your own module package, and an overridden auth login screen -- all without editing core.

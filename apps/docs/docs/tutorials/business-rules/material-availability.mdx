...
1. Open the Helios admin interface
...
> ℹ️ The `onStatusChange` event triggers when the work order status field changes. This is more specific than `beforeUpdate` which fires on any field change.
...
> ℹ️ The `newStatus` field represents the status the user is trying to change to. This field is provided by the `onStatusChange` event.
...
> ℹ️ This assumes your work order entity has a `materialsAvailable` boolean field. Adjust the field path based on your data model. You might use `materials.available`, `materialStatus`, or a calculated field.
...
> ℹ️ Replace with actual email addresses or user IDs from your system.
...
✅ Prevents work order release when materials are unavailable
✅ Shows clear error messages to users
✅ Notifies the material planning team
✅ Logs all blocked attempts for audit trail
✅ Allows release when materials are ready

...
Finally, enable the module in `apps/helios/src/modules.ts`, re-run `yarn generate`, and restart the dev server. You have a fully fledged module with end-to-end functionality ready for production.

:::tip Share your module with the community

Once your module is production-ready, consider publishing it to the [Official Modules repository](../framework/modules/official-modules). Every submission is reviewed by the core team and made available for one-command installation — giving your work visibility, credit, and a home in the Helios ecosystem.

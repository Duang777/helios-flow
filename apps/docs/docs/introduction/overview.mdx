...
description: High-level introduction to the Helios platform and its philosophy.
---

![Helios UI hero](/img/helios-homepage.jpg)

Helios is an extensible commerce framework that blends a modular backend, customizable admin UI, and typed data engine. Every capability—from authentication to dashboards—is delivered as an installable module so product teams can add or override functionality without forking the core platform.

## Why teams pick Helios
...
If you are in a rush, scaffold a standalone project with `npx create-helios-app my-app` ([guide](/customization/standalone-app)) or jump to [Build your first Helios app](../customization/build-first-app) if you're working inside the monorepo.

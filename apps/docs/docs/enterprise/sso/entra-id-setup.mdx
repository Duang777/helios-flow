...
description: Step-by-step guide for configuring Microsoft Entra ID (formerly Azure AD) as the OIDC and SCIM identity provider for Helios SSO.
---

This guide walks through setting up Microsoft Entra ID (formerly Azure AD) as the identity provider for both OIDC login and SCIM user provisioning in Helios.
...
3. Description: `Helios Dev`, Expiry: `6 months` (or your preference)
4. Click **Add**
...
## 4. Create the SSO Config in Helios

1. Log into Helios as admin
...
6. You should be redirected back to Helios and logged in

## 5. Configure SCIM Provisioning

:::tip Prerequisite
You need a SCIM bearer token from Helios. Generate one via the admin UI (SSO config -> Provisioning tab -> Generate Token) or the API (`POST /api/sso/scim/tokens` with the SSO config ID).
:::
...
| **Secret Token** | Paste the SCIM bearer token from Helios |
...
1. In Entra, go to **Enterprise applications** -> **Helios** -> **Provisioning**
2. Click **Provision on demand**
3. Search for a test user and click **Provision**
4. **Expected**: Entra sends `POST /Users` to your SCIM endpoint -> user appears in Helios
5. Check the provisioning log in Helios admin UI

### Test User Update

1. In Entra, go to **Users** -> edit a test user's display name
2. Wait for the next provisioning cycle (or use Provision on demand)
3. **Expected**: Entra sends `PATCH /Users/{id}` -> user's name updated in Helios
...
2. **Expected**: Entra sends `PATCH /Users/{id}` with `active: false` -> user deactivated in Helios, all sessions revoked

### Test OIDC + SCIM Together

1. **Create a new user in Entra** and assign them to the Helios Enterprise app
2. **Provision on demand** (or wait for cycle)
3. **Verify** the user exists in Helios (pre-provisioned, no login needed)
4. **Log in as that user** via OIDC (Helios login -> redirect to Microsoft -> authenticate -> redirect back)

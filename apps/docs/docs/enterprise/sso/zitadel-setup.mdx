---
title: Zitadel Setup
description: Step-by-step guide for configuring Zitadel as the OIDC and SCIM identity provider for Helios SSO.
---

This guide walks through setting up Zitadel as the identity provider for OIDC login and SCIM user provisioning in Helios.
...
2. Name the project `Helios` and click **Continue**
...
:::tip Prerequisite
Generate a SCIM bearer token from Helios via the admin UI (SSO config -> Provisioning tab -> Generate Token).
:::
...
1. Go to your Project -> **Helios** application
...
| **Bearer Token** | Paste the SCIM token from Helios |
...
3. Push user changes to Helios's SCIM endpoint

## 6. Test the Full Flow

### Test OIDC Login

1. Navigate to Helios login
2. Enter a test user's email
3. **Expected**: Redirect to Zitadel -> authenticate -> redirect back to Helios
4. Verify the user appears in the Helios admin panel
...
2. **Expected**: User is automatically created in Helios with `provisioningMethod: jit`
...
3. **Expected**: User appears in Helios with `provisioningMethod: scim`
4. Update the user in Zitadel -> verify changes propagate
5. Deactivate the user in Zitadel -> verify deactivation in Helios
...
| **PKCE support** | Natively supports S256 PKCE | Automatically used by Helios |
...
- Check that the email domain matches the allowed domains in Helios SSO config

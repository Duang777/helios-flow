...
API handlers live under `api/<path>/route.ts`. Instead of hand-coding every method, lean on the CRUD factory (`makeCrudRoute`) from `@helios/shared` -- it wraps validation, RBAC metadata, multi-tenant scoping, and event emission so each module stays consistent.

## 1. Implement a domain service

```bash
mkdir -p apps/helios/src/modules/inventory/services
touch apps/helios/src/modules/inventory/services/inventory-service.ts
```

```ts title="apps/helios/src/modules/inventory/services/inventory-service.ts"
...
The platform uses `makeCrudRoute` from `@helios/shared/lib/crud/factory` to generate consistent REST endpoints. Each route file must also export per-method `metadata` for RBAC guards and an `openApi` object for API documentation.

```bash
mkdir -p apps/helios/src/modules/inventory/api/items
touch apps/helios/src/modules/inventory/api/items/route.ts
```

```ts title="apps/helios/src/modules/inventory/api/items/route.ts"
import { z } from 'zod';

...
```bash
mkdir -p apps/helios/src/modules/inventory/data
touch apps/helios/src/modules/inventory/data/entities.ts
touch apps/helios/src/modules/inventory/data/validators.ts
```

```ts title="apps/helios/src/modules/inventory/data/entities.ts"
...
```ts title="apps/helios/src/modules/inventory/data/validators.ts"
import { z } from 'zod';
...
Migrations appear under `apps/helios/src/modules/inventory/migrations/` (for app-level modules with `from: '@app'`). Commit them alongside your entity. When you run `yarn db:migrate`, the new table will be created across environments.

...
description: Guidance for resolving common operational issues with Helios.
---

## Database Connection Pooling

Helios relies on PostgreSQL connection pooling to avoid "too many clients" errors. Tune the pool via environment variables to match your deployment footprint.
...
The monorepo and standalone `yarn dev` wrappers enable lazy worker and lazy scheduler auto-spawn by default to keep local idle RSS down. Direct `helios server dev` and production `helios server start` keep the historical eager default unless you opt in:

```bash
HELIOS_AUTO_SPAWN_WORKERS_LAZY=true HELIOS_AUTO_SPAWN_SCHEDULER_LAZY=true helios server dev
```

To force the historical eager worker and scheduler modes from the dev wrapper, run:

```bash
HELIOS_AUTO_SPAWN_WORKERS_LAZY=false HELIOS_AUTO_SPAWN_SCHEDULER_LAZY=false yarn dev
```

In lazy mode the runtime starts lightweight supervisors. The worker supervisor watches each discovered queue for the first ready job and only then spawns a worker. `yarn dev` uses the shared lazy mode, so the first ready job starts one `helios queue worker --all` process instead of one process per ready queue; set `HELIOS_AUTO_SPAWN_WORKERS_LAZY_MODE=per-queue` to use the older per-queue process model. The scheduler supervisor watches for enabled schedules and only then spawns `helios scheduler start`. Idle queues and empty scheduler installations do not contribute polling timers (local strategy) or BullMQ `Worker`/Redis resources (async strategy) to the process tree. See [Queue Workers — Lazy Worker Auto-Spawn](../framework/events/queue-workers.mdx#lazy-worker-auto-spawn-memory-sensitive-dev) for the env variable reference.
...
- `HELIOS_WATCH_PACKAGES_MODE` (dispatch-level) picks which watcher implementation `yarn dev` runs. Default is the consolidated single-process watcher (`scripts/watch-packages.mjs`). Set `HELIOS_WATCH_PACKAGES_MODE=legacy` to fall back to the Turbo per-package fan-out (`yarn watch:packages:legacy`) when debugging the new path or pinning the old behavior.
- `HELIOS_PACKAGE_WATCH_MODE` (per-package mode, only meaningful under the legacy Turbo fan-out) chooses between the low-memory one-shot mode (default) and the older `persistent` esbuild-context mode that keeps the incremental graph in memory for faster rebuilds at the cost of ~1 GB of idle RSS.

To restore the previous faster-but-heavier incremental watcher for a session, combine both:

```bash
HELIOS_WATCH_PACKAGES_MODE=legacy HELIOS_PACKAGE_WATCH_MODE=persistent yarn dev
```
...
| `popular` | Only the **most frequently changed** packages, ranked from recent `git log` history (capped by `HELIOS_WATCH_POPULAR_LIMIT`, default 6). Falls back to `core`, `ui`, `shared` when no history is available. |
| `env` | Exactly the packages you list in `HELIOS_WATCH_PACKAGES`, or the selection saved by the interactive picker. |
...
```bash
yarn dev:watch-select   # choose packages by number/name; saved to .helios/watch-packages.local.json
yarn dev --watch=env
```

Tuning knobs for `auto-optimized`:

- `HELIOS_WATCH_GIT_STATUS=off` — ignore uncommitted working-tree changes (use only the branch diff).
- `HELIOS_WATCH_GIT_BRANCH=off` — ignore the branch diff (use only the working tree).
- `HELIOS_WATCH_BASE_REF=origin/main` — override the base ref the branch diff is computed against (defaults to the first of `origin/develop`, `develop`, `origin/main`, `main` that exists).

If a non-`all` scope ever leaves out a package you are actively editing, either switch back with `HELIOS_WATCH_SCOPE=all` / `--watch=all`, add it to `HELIOS_WATCH_PACKAGES`, or just wait — `auto-optimized` will pick it up at the next 2-minute sweep once it has a git change.
...
you may be connecting to a different PostgreSQL instance already bound to `localhost:5432` (for example, a host-installed Postgres), not the Docker one started for Helios.
...
```bash
psql "postgresql://postgres:postgres@localhost:55432/helios" -c "select 1;"
```

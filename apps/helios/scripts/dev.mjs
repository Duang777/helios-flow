...
function resolveLazyWorkerSpawnMode(env) {
  const raw = env.HELIOS_AUTO_SPAWN_WORKERS_LAZY_MODE?.trim().toLowerCase()
...
const command = resolveProjectBinary(process.platform === 'win32' ? 'helios.cmd' : 'helios')
const classic = process.argv.includes('--classic') || isEnabledEnvFlag(process.env.HELIOS_DEV_CLASSIC)
const verbose = !classic && (process.argv.includes('--verbose') || process.env.HELIOS_DEV_OUTPUT === 'verbose')
...
const splashChildStateFile = process.env.HELIOS_DEV_SPLASH_CHILD_STATE_FILE?.trim() || null
const warmupReadyFile = process.env.HELIOS_DEV_WARMUP_READY_FILE?.trim()
  || (splashChildStateFile ? `${splashChildStateFile}.warmup-ready` : null)
const splashMode = process.env.HELIOS_DEV_SPLASH_MODE?.trim() || 'dev'
...
const configuredRuntimeProgressTotal = parsePositiveIntegerEnv(process.env.HELIOS_DEV_SPLASH_STAGE_TOTAL)
const configuredRuntimeProgressCurrent = parsePositiveIntegerEnv(process.env.HELIOS_DEV_SPLASH_STAGE_CURRENT)
...
const backgroundWorkerMode = resolveAutoSpawnMode(process.env, 'AUTO_SPAWN_WORKERS', 'HELIOS_AUTO_SPAWN_WORKERS', 'HELIOS_AUTO_SPAWN_WORKERS_LAZY')
const backgroundServiceModes = {
  ...
  scheduler: resolveAutoSpawnMode(process.env, 'AUTO_SPAWN_SCHEDULER', 'HELIOS_AUTO_SPAWN_SCHEDULER', 'HELIOS_AUTO_SPAWN_SCHEDULER_LAZY'),
}
const shutdownNoticeOwnedByParent = process.env.HELIOS_DEV_SHUTDOWN_NOTICE_OWNER === 'parent'
const appMemoryTraceEnabled = isEnabledEnvFlag(process.env.HELIOS_DEV_MEMORY_TRACE)
  && process.env.HELIOS_DEV_MEMORY_TRACE_OWNER !== 'parent'
...
const runtimeWarmupState = {
  ...
  generation: 0,
  retryAttempts: 0,
  tenantId: readNonEmptyEnvValue('HELIOS_DEV_WARMUP_TENANT_ID') ?? null,
...
function spawnHelios(args) {
  ...
  const child = spawn(resolvedSpawn.command, resolvedSpawn.args, {
    ...
    env: {
      ...process.env,
      HELIOS_CLI_QUIET: rawPassthrough ? process.env.HELIOS_CLI_QUIET : '1',
      DOTENV_CONFIG_QUIET: rawPassthrough ? process.env.DOTENV_CONFIG_QUIET : 'true',
      ...(!rawPassthrough ? { HELIOS_DEV_SPLASH_RUNTIME_WRAPPER: '1' } : {}),
      ...(!rawPassthrough && warmupReadyFile ? { HELIOS_DEV_WARMUP_READY_FILE: warmupReadyFile } : {}),
    },
...
function resolveWarmupCredentials() {
  return {
    email: readNonEmptyEnvValue('HELIOS_INIT_SUPERADMIN_EMAIL') ?? 'superadmin@acme.com',
    password: readNonEmptyEnvValue('HELIOS_INIT_SUPERADMIN_PASSWORD') ?? 'secret',
  }
}
  ...
  try {
    ...
    const failureLines = isCredentialsFailure
      ? [
          ...
          'Set HELIOS_INIT_SUPERADMIN_EMAIL and HELIOS_INIT_SUPERADMIN_PASSWORD in .env,',
    ...
    if (isCredentialsFailure) {
      ...
      console.log(formatStatusOutput(
        '⚠️ Warmup login returned 401 — credentials invalid. Set HELIOS_INIT_SUPERADMIN_EMAIL/PASSWORD in .env or run: yarn initialize',
...
function resolveRawLogFileStream() {
  ...
  if (process.env.HELIOS_DEV_LOG_TEE === '0' || process.env.HELIOS_DEV_LOG_TEE === 'false') {
    rawLogFileStream = null
    return rawLogFileStream
  }
  try {
    const logDir = process.env.HELIOS_DEV_LOG_DIR?.trim()
      ? path.resolve(process.env.HELIOS_DEV_LOG_DIR.trim())
      : path.resolve('.helios', 'logs')
    fs.mkdirSync(logDir, { recursive: true })
    const runId = process.env.HELIOS_DEV_RUN_ID?.trim()
...
async function runInitialGenerate() {
  const startedAt = Date.now()
  markMemoryTrace('generate:start', 'Generating app artifacts', { command: 'helios generate' })
  ...
  if (verbose) {
    const child = spawnHelios(['generate'])
...
function classifyServerLine(line) {
  ...
  if (line === '[server] Starting Helios in dev mode...') {
    return {
      type: 'status',
      message: '🚀 Starting app server',
      splashPhase: startupSplashPhase,
  ...
  if (line === '[server] Detected corrupted Turbopack dev cache. Clearing .helios/next/dev and restarting Next.js once...') {
...
function startFilteredChild(args, label, classifyLine) {
  const child = spawnHelios(args)
  if (label === 'App runtime') {
    startMemoryMonitor(child)
    markMemoryTrace('app-runtime:start', 'App runtime started', { pid: child.pid, command: ['helios', ...args].join(' ') })
  }
...
async function runClassicRuntime() {
  const initialGenerate = spawnHelios(['generate'])
  ...
  // Default ('in-process'): `helios server dev` owns the structural
  ...
  // Opt back into the sidecar with HELIOS_DEV_GENERATE_WATCH_MODE=legacy.
  const watchers = []
  if (generateWatchMode === 'legacy') {
    watchers.push(['Generator watch (legacy sidecar)', spawnHelios(['generate', 'watch', '--skip-initial'])])
  }
  const server = spawnHelios(['server', 'dev'])
  const waiters = [
...
// Default ('in-process'): `helios server dev` runs the structural
// regeneration watcher in-process — see packages/cli/src/lib/in-process-generate-watcher.ts
// — so the orchestrator no longer spawns a dedicated `helios generate watch`
// sidecar. Saves ~190 MB of resident RSS by eliminating one Node process.
// Set HELIOS_DEV_GENERATE_WATCH_MODE=legacy to opt back into the sidecar.

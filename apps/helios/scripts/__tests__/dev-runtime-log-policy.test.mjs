...
test('isRuntimeRestartMarker recognises the restart announcements the CLI prints', () => {
  ...
  assert.equal(
    isRuntimeRestartMarker('[server] Detected corrupted Turbopack dev cache. Clearing .helios/next/dev and restarting Next.js once...'),
    true,
  )
...
test('createRuntimeFailureLatch releases on the Turbopack cache recovery marker', () => {
  ...
  assert.equal(
    latch.releaseOn('[server] Detected corrupted Turbopack dev cache. Clearing .helios/next/dev and restarting Next.js once...'),
    true,
  )
  assert.equal(latch.isLatched(), false)
})
...
test('createRuntimeFailureLatch re-arms after a release and keeps instances independent', () => {
  ...
  assert.equal(other.releaseOn('[server] Detected corrupted Turbopack dev cache. Clearing .helios/next/dev and restarting Next.js once...'), false)
})

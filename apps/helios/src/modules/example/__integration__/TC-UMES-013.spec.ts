...
import { DEFAULT_CREDENTIALS } from '@helios/core/helpers/integration/auth'
...
test.describe('TC-UMES-013: Ephemeral login page suppresses global notice bars', () => {
  ...
  test('login page hides demo and cookie notices in ephemeral integration mode', async ({ page }) => {
    test.skip(process.env.HELIOS_INTEGRATION_TEST !== 'true', 'Notice-bar suppression is specific to ephemeral integration mode')
    test.slow()

...
/**
 * Regression guard: `HELIOS_INTEGRATION_TEST=true` must keep both the metadata-driven
 * and the per-handler rate-limit paths bypassed.
 *
 * `readRateLimitConfig()` flips `RATE_LIMIT_ENABLED=false` whenever
 * `HELIOS_INTEGRATION_TEST=true`, which the integration runner exports via
 ...
 * `apps/helios/src/modules/ratelimit_probe/`. That module ships only with
 * the helios monorepo and is intentionally absent from
 * `create-helios-app` scaffolds, so the metadata path self-skips when
 * `/api/ratelimit_probe/ping` responds with 404.
 */
test.describe('per-route rate limits leak into HELIOS_INTEGRATION_TEST runs', () => {
  test('metadata.rateLimit path (apps/helios root handler): 5 POSTs to /api/ratelimit_probe/ping all return 200 with points=3', async ({ request }) => {
    ...
    test.skip(
      probe.status() === 404,
      'ratelimit_probe module not registered (expected for create-helios-app scaffolds)',
    );
    ...
    // With the HELIOS_INTEGRATION_TEST bypass: all 5 calls return 200.
    // Without it: rate limiter fires on the 4th call → [200, 200, 200, 429, 429].
    expect(statuses, 'HELIOS_INTEGRATION_TEST should bypass metadata.rateLimit').toEqual([200, 200, 200, 200, 200]);
  });
  ...
  test('manual checkRateLimit path (sales/quotes/accept): 11th POST 429s with points=10', async ({ request }) => {
    ...
    // With the HELIOS_INTEGRATION_TEST bypass: zero 429s (invalid token → 400/404, but never 429).
    // Without it: 429 from the 11th call onward.
    expect(statuses.filter((s) => s === 429), 'HELIOS_INTEGRATION_TEST should bypass per-handler checkRateLimit').toEqual([]);
  });
});

...
jest.mock('@helios/ui/backend/BackendChromeProvider', () => ({
  useBackendChrome: () => ({ payload: { groups: [], grantedFeatures: [] }, isReady: true }),
}))

jest.mock('@helios/ui/backend/IntegrationsButton', () => ({
  IntegrationsButton: () => <div data-testid="integrations-button" />,
}))

jest.mock('@helios/ui/backend/ProfileDropdown', () => ({
  ProfileDropdown: () => <div data-testid="profile-dropdown" />,
}))

jest.mock('@helios/ui/backend/SettingsButton', () => ({

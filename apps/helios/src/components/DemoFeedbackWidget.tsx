...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription } from '@helios/ui/primitives/dialog'
...
import { useAiDock } from '@helios/ui/ai/AiDock'
...
export function DemoFeedbackWidget({ demoModeEnabled }: { demoModeEnabled: boolean }) {
  ...
  // fixed dark gradient. The `helios-demo-feedback-floating` class hooks into
  // `body[data-ai-chat-open="true"] .helios-demo-feedback-floating` in
  ...
  const floatingButton = aiDockActive ? null : (
    <button
      ...
      className="helios-demo-feedback-floating fixed bottom-6 right-6 z-banner flex max-w-[calc(100vw-3rem)] items-center gap-2 rounded-full px-5 py-3 text-sm font-semibold text-black shadow-xl transition-all hover:scale-105 hover:shadow-2xl active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 animate-[subtle-bounce_2s_ease-in-out_infinite]"
      style={{
        backgroundImage: 'linear-gradient(135deg, var(--brand-lime, #5EEAD4) 0%, #99F6E4 50%, var(--brand-violet, #14B8A6) 100%)',
      }}
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-md" onKeyDown={handleKeyDown}>
          <DialogHeader className="items-center gap-3">
            <Image alt="Helios" src="/helios.svg" width={48} height={48}  className="dark:invert" />
            <DialogTitle className="text-center text-xl">
              {t('demoFeedback.dialog.title', 'Talk to Helios team')}
            </DialogTitle>
          ...
          {submitState === 'sent' ? (
            ...
            <div className="grid gap-3">
              ...
              <label className="flex items-start gap-2.5 text-xs text-muted-foreground leading-relaxed">
                ...
                />
                <span>
                  {t('demoFeedback.form.marketingLabel', "I consent to receiving direct marketing from Helios sp. z o.o. by email. I can withdraw my consent at any time. See our {termsLink} and {privacyLink}.")

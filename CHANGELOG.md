...
Helios `0.6.6` is a big **tighten-the-bolts release** with a lot of new shine on top. The headline is trust: a sweeping 270-fix pass locks down tenant and organization scoping, closes permission gaps across customer accounts, dashboards, messages, attachments, and the AI assistant, and rolls out **unified optimistic locking** everywhere — so concurrent edits stop quietly stepping on each other, and money and orders behave.

This release also runs **noticeably lighter**. A broad performance-and-footprint push trims resource usage across the board: selectable dev-mode watch scopes and the `HELIOS_WATCH_SCOPE` memory lever, per-module CPU/RAM insight, lazy-loaded module commands, a shared dev scheduler/worker, throttled coverage warmup, coalesced job-update broadcasts, and a wide response-cache sweep over detail, list, and unread-count APIs — all backed by a new structured **pino logging** facade. On top of that, **CRM steals the show** with a full calendar (day/week/month/agenda) and a responsive event editor, configurable interaction statuses, and a new deals map view, while the ecosystem opens up with a shared **`helios-*` skills mixin**, OpenRouter and Requesty AI presets, a two-way **Discord channel**, hardened MCP keys, signed webhooks, and a batch of security-CVE bumps. Enjoy!

## ✨ Features
- ✨ Adopt skills mixin in the standalone create-app template (stacked on #4008). (#4026) *(@pkarw)*
- ✨ Install shared helios-* skills from helios/skills (mixin). (#4008) *(@pkarw)*
...
- ✨ Document HELIOS_WATCH_SCOPE dev-memory lever + emoji watch-mode log. (#3962) *(@pkarw)*
...
- ✨ Allow forcing a locale via HELIOS_FORCE_LOCALE. (#3724) *(@jtomaszewski)*
...
- ✨ Add spec tracking-issue mode to helios-followup-issue-from-pr. (#3148) *(@pkarw)*
...
- 🛠️ Structured logging facade for @helios/shared (#3743). (#3744) *(@pat-lewczuk)*
...
- 📝 Add scope-cohesion (anti-bundling) check to helios-spec-writing. (#3785) *(@matgren)*
...
- 📝 Add helios-auto-verify-pr-ui skill for PR UI QA verification. (#3388) *(@pkarw)*
...
- 📝 Make helios-troubleshooter propose fix and wait for confirmation. (#3003) *(@adeptofvoltron)*
...
- ✨ Add the `helios-help` workflow navigator skill. (#2140) *(@adeptofvoltron)*
...
- 📝 Skills: add `helios-approve-merge-pr` and `helios-followup-issue-from-pr`. (#2827) *(@pkarw)*
...
- 📝 Skills: make `helios-troubleshooter` propose a fix and wait for confirmation. (#3003) *(@adeptofvoltron)*
...
## ✨ Features
- ✨ OSS optimistic-locking guard is now **default ON** across every CRUD entity exposed via `makeCrudRoute`. Opt out with `HELIOS_OPTIMISTIC_LOCK=off` (also accepts `false` / `0` / `no` / `disabled` / `none`). Runtime stays strictly additive — requests without the `x-helios-ext-optimistic-lock-expected-updated-at` header continue to pass through. See [`UPGRADE_NOTES.md`](UPGRADE_NOTES.md) → "OSS optimistic locking default-ON" and [`.ai/specs/2026-05-25-oss-optimistic-locking.md`](.ai/specs/2026-05-25-oss-optimistic-locking.md) §3.4 + §4. (#1981 / #2055 Phase 14) *(@pkarw)*
- ✨ Command-level OSS optimistic locking — new `enforceCommandOptimisticLock` helper lets Command-pattern / non-`makeCrudRoute` writes enforce the same `updated_at` 409 check against an aggregate root. Wired into the sales document sub-resource commands (order/quote lines + adjustments, return create) and quote→order conversion (closes the accept/convert race #2114) as a document-aggregate check. Strictly additive; honors `HELIOS_OPTIMISTIC_LOCK`. See `.ai/specs/2026-05-25-oss-optimistic-locking.md` §10 + concurrency-locking docs. (#1981 / #2055 Phase 16–18) *(@pkarw)*
- ✨ Unified record-conflict bar — optimistic-lock 409s ("this record was modified") now surface as a persistent, error-styled bar in `AppShell` (like the undo banner) instead of a transient toast. `CrudForm` and `useGuardedMutation` route conflicts automatically; custom pages call `surfaceRecordConflict(err, t)` from the new `@helios/ui/backend/conflicts`. (#1981 / #2055) *(@pkarw)*
...
- 🔒 Query index: redact PII and raw tokens from `HELIOS_SEARCH_DEBUG` logs (#2709). (#2791) *(@pat-lewczuk)*
...
## 📝 Specs & Documentation
- 📝 Spec: Railway one-command deployment from the Helios CLI. (#1898) *(@pkarw)*
...
- 📝 Spec: `HELIOS_CACHE_SAFETY_ALWAYS_CONSISTENT` opt-in synchronous read-projection consistency. (#2590) *(@pkarw)*
...
- 🛠️ Consolidate `helios generate watch` into `helios server dev` — ~190 MB idle RSS win. (#2105) *(@pkarw)*
...
- 🔧 Purge Turbopack `.helios/next` cache before greenfield rebuilds (fixes #1950). (#1984) *(@pkarw)*
...
- ✨ Scriptable provisioning flags on `helios auth setup` — `--orgSlug` / `--with-examples` / `--json` (supersedes #1879). (#1900) *(@matgren, via @pkarw)*
...
- ✨ Unified `HELIOS_AI_PROVIDER` / `HELIOS_AI_MODEL` with openai + gpt-5-mini defaults. (#1856) *(@pkarw)*
...
## Highlights
Helios `0.6.0` turns the post-0.5.0 work into a broader platform release: AI agents now have a unified runtime and approval flow, MikroORM has moved to v7/Kysely, CRM and navigation screens received another major usability pass, and release engineering now carries forward contributor credits for superseded PRs.
...
- ✨ Add `helios auth sync-role-acls` CLI for re-applying default role features. (#1699) *(@MStaniaszek1998)*
...
- 🐛 Use `yarn helios db` commands in codex enforcement rules. (#1630) *(@pat-lewczuk)*
...
- 🔄 Move default file-backed cache paths under .helios. (#1682) *(@pkarw)*
...
- 🐛 Disable rate limiting under HELIOS_INTEGRATION_TEST. (#1673) *(@jtomaszewski)*
...
- 🔧 Use HELIOS_SEARCH_MIN_LEN env var for search query minimum length (supersedes #1761). (#1773) *(@haxiorz, via @pkarw)*
...
## Highlights
Helios `0.5.0` is the biggest release so far. It bundles more than 250 fixes and
...
- 🛠️ Block enterprise tests when HELIOS_ENABLE_ENTERPRISE_MODULES is false. (#1204) *(@strzesniewski)*
...
- 📁 Update Dockerfile to use `.helios` directory and adjust build steps. (#1094) *(@MStaniaszek1998)*
...
- 📖 Surface `create-helios-app` in docs and homepage. (#713) *(@andrzejewsky)*
...
- 🏗️ Enterprise package scaffold — initial structure for the `@helios/enterprise` package for commercial/enterprise-only modules and overlays. (#580) *(@pkarw)*
...
## Highlights
This release introduces **`helios eject`** for deep module customization without forking, a **Version History** system with undo/redo and related-record tracking, **Docker dev mode with hot reload**, **sidebar reorganization**, significant **mobile UX improvements**, and a new **`create-helios-app`** standalone app workflow. It also ships Windows compatibility fixes, search indexing safeguards, and expanded i18n coverage.

## Features
- Added `helios eject` CLI command — copy any ejectable core module into your local `src/modules/` for full customization. Nine modules are ejectable at launch: catalog, currencies, customers, perspectives, planner, resources, sales, staff, and workflows. (#514) *(@andrzejewsky)*
- Standalone app development improvements — better `create-helios-app` scaffolding, module resolver, and generator support for apps outside the monorepo. (#472) *(@andrzejewsky)*
- Documentation for standalone app creation with `create-helios-app`, module ejection guide, and README updates. (#547) *(@pkarw)*
...
- Created the foundational structure for agent skills in Helios, enabling extensible AI-powered capabilities. (#455) *(@pat-lewczuk)*
...
- `helios init` seeds default USD regular and sale price kinds configured as tax-inclusive overrides.

...
`@helios/core` contains all core business modules (auth, catalog, customers, sales, etc.). This guide covers the full extensibility contract and module development patterns.
...
- call `runMutationGuards(...)` from `@helios/shared/lib/crud/mutation-guard-registry` before mutation logic, passing the caller's granted features as `{ userFeatures }`
...
5. If you author a scoped SQL migration yourself to avoid unrelated generated churn, base it on the entity metadata and existing module migration style, then update that module's `migrations/.snapshot-helios.json` to the post-change schema in the same commit.
...
| `defaultRoleFeatures` | Declarative, merged during `ensureDefaultRoleAcls()` and `yarn helios auth sync-role-acls` | Always | Role ACL features |
...
```typescript
import { registerModules } from '@helios/shared/lib/modules/registry'
...
2. **MUST use `@helios/queue` workers** for work that should continue after navigation or retry after process failure.
...
- Use `InjectionPosition` from `@helios/shared/modules/widgets/injection-position` for deterministic before/after/first/last placement
...
- Custom (non-CRUD) API routes are opt-in: call `runCustomRouteAfterInterceptors(...)` from `@helios/shared/lib/crud/custom-route-interceptor`.
...
- **Shared helpers**: `splitCustomFieldPayload`, `normalizeCustomFieldValues`, `normalizeCustomFieldResponse`, `applyCustomFieldsNormalization` from `@helios/shared`
- **Form collection**: `collectCustomFieldValues()` from `@helios/ui/backend/utils/customFieldValues`
...
```typescript
import { defineLink, entityId, linkable, defineFields, cf } from '@helios/shared/modules/dsl'
...
- Use `withAtomicFlush(em, phases, options)` from
  `@helios/shared/lib/commands/flush` when a command mutates
...
- Cache invalidation follows the same rule as side effects: invalidate **after** the DB write commits, never inside the `withAtomicFlush` block. For the opt-in always-consistent read-projection tail (`HELIOS_CACHE_SAFETY_ALWAYS_CONSISTENT`, default OFF) see `.ai/specs/2026-06-05-cache-safety-always-consistent.md`.
...
```typescript
import { runCrudCommandWrite } from '@helios/shared/lib/commands/runCrudCommandWrite'
...
- Enable with `HELIOS_PROFILE` env (comma-separated filters: `*`, `all`, `customers.*`, etc.)
- CRUD factories emit `[crud:profile]` payloads; query engine attaches nested `query_engine` node
- Legacy flags (`HELIOS_CRUD_PROFILE`, `HELIOS_QE_PROFILE`) still work but avoid in new code
...
- Exception: when generated output includes unrelated snapshot drift, keep or write only the intended SQL and update that module's `.snapshot-helios.json` in the same change.
...
Output to `apps/helios/.helios/generated/`. Never edit manually. Never import from packages — only the app bootstrap should import and register them.

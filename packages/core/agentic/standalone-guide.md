...
`@helios/core` contains all built-in business modules. This guide covers module development patterns for standalone apps that build on top of these modules.

## Module anatomy, auto-discovery & convention files

> Moved to the conceptual guide. See **`.ai/guides/module-system.md` → Module anatomy** for the auto-discovery path table and the convention-file reference — they are framework-wide, not core-specific. The sections below cover `@helios/core`-specific development patterns.
...
- Call `runMutationGuards(...)` from `@helios/shared/lib/crud/mutation-guard-registry` before mutation, passing the caller's granted features as `{ userFeatures }`
...
```typescript
// data/extensions.ts
import { defineLink, entityId, linkable } from '@helios/shared/modules/dsl'
...
// Reference: @helios/core customers/commands/people.ts
```
...
```typescript
import { getCustomerAuthFromCookies } from '@helios/core/modules/customer_accounts/lib/customerAuthServer'
...
import { PortalPageHeader } from '@helios/ui/portal/components/PortalPageHeader'
import { PortalCard, PortalCardHeader } from '@helios/ui/portal/components/PortalCard'

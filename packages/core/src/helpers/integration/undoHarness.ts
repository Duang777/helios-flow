 ...
 *
 * Every mutating Helios API response carries the operation metadata in the
 * `x-helios-operation` header (`omop:<urlencoded JSON>`) containing the `undoToken` and the
...
export const UNDO_TESTS_DISABLED_ENV = 'HELIOS_INTEGRATION_UNDO_TESTS_DISABLED'
...
/** Parse the `x-helios-operation` header into a structured operation, or null when absent/malformed. */
export function extractOperation(response: APIResponse): Operation | null {
  const header = response.headers()['x-helios-operation']
...
export function expectOperation(response: APIResponse, context: string): Operation {
  ...
  expect(op, `Expected an undo token (x-helios-operation header) for ${context}, got none`).toBeTruthy()

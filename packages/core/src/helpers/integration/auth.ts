...
function loadEnvFileContent(): string | null {
  const candidatePaths = [
    resolve(process.cwd(), 'apps/helios/.env'),
...
export const DEFAULT_CREDENTIALS: Record<string, { email: string; password: string }> = {
  superadmin: {
    email: loadEnvValue('HELIOS_INIT_SUPERADMIN_EMAIL') || 'superadmin@acme.com',
    password: loadEnvValue('HELIOS_INIT_SUPERADMIN_PASSWORD') || 'secret',
  },
...
async function dismissGlobalNoticesIfPresent(page: Page): Promise<void> {
  ...
  const feedbackDialog = page.getByRole('dialog', { name: /Talk to Helios team/i }).first();

...
function resolveAppRoot(input?: string): string {
  return path.resolve(input?.trim() || process.env.HELIOS_TEST_APP_ROOT?.trim() || path.resolve(process.cwd(), 'apps/helios'))
}
...
function resolveAppRunnerPath(appRoot: string): string {
  ...
  return requireFromApp.resolve('@helios/core/helpers/integration/queue-runner')
}
...
function drainQueueInAppProcess(queueName: string, options: Required<DrainQueueOptions>): Promise<number> {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [resolveAppRunnerPath(options.appRoot)], {
      ...
      env: {
        ...process.env,
        HELIOS_INTEGRATION_APP_ROOT: options.appRoot,
        HELIOS_INTEGRATION_QUEUE_NAME: queueName,
        HELIOS_INTEGRATION_QUEUE_JOB_LIMIT: String(options.jobLimit),
        QUEUE_BASE_DIR: process.env.QUEUE_BASE_DIR?.trim() || path.resolve(options.appRoot, '.helios/queue'),
      },
    ...
    child.on('close', (code) => {
      ...
      // child undebuggable from CI logs. HELIOS_DRAIN_DEBUG=1 forwards everything;
      // a non-empty stderr is forwarded unconditionally because it only carries
      // warning/error-class output.
      if (process.env.HELIOS_DRAIN_DEBUG === '1') {

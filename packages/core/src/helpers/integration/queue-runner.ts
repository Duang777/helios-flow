...
export async function drainQueueFromAppRoot(
  ...
  const queue = createQueue(queueName, 'local', {
    baseDir: path.resolve(
      ...
      || path.resolve(options.appRoot, '.helios/queue'),
    ),
    concurrency: 1,
  })
...
export async function runQueueDrainFromEnv(): Promise<void> {
  const queueName = process.env.HELIOS_INTEGRATION_QUEUE_NAME?.trim()
  const appRoot = process.env.HELIOS_INTEGRATION_APP_ROOT?.trim()
  const jobLimit = Number.parseInt(process.env.HELIOS_INTEGRATION_QUEUE_JOB_LIMIT || '100', 10) || 100

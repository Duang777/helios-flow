...
import { isCrudHttpError, isUniqueViolation } from '@helios/shared/lib/crud/errors'
import {
  runCrudMutationGuardAfterSuccess,
  validateCrudMutationGuard,
} from '@helios/shared/lib/crud/mutation-guard'
import { perspectiveSaveSchema } from '@helios/core/modules/perspectives/data/validators'
import {
...
} from '@helios/core/modules/perspectives/services/perspectiveService'
...
export async function GET(_req: Request, ctx: { params: { tableId: string } }) {
  ...
  const cache = ((): import('@helios/cache').CacheStrategy | null => {
    try {
      return container.resolve('cache') as import('@helios/cache').CacheStrategy
    } catch {
      return null
    }
  })()

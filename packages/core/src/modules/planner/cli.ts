...
const seedAvailabilityRuleSetsCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!tenantId || !organizationId) {
      console.error('Usage: helios planner seed-availability-rulesets --tenant <tenantId> --org <organizationId>')
      return
    }
...
const seedUnavailabilityReasonsCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    const organizationId = String(args.organizationId ?? args.org ?? args.orgId ?? '')
    if (!tenantId || !organizationId) {
      console.error('Usage: helios planner seed-unavailability-reasons --tenant <tenantId> --org <organizationId>')
      return
    }
    const container = await createRequestContainer()
    const scope: PlannerSeedScope = { tenantId, organizationId }

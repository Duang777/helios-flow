...
describe('attachment upload limits', () => {
  const primaryMaxUploadEnv = 'HELIOS_ATTACHMENT_MAX_UPLOAD_MB'
  const primaryQuotaEnv = 'HELIOS_ATTACHMENT_TENANT_QUOTA_MB'
  const legacyMaxUploadEnv = 'HELIOS_ATTACHMENT_MAX_UPLOAD_MB'
  const legacyQuotaEnv = 'HELIOS_ATTACHMENT_TENANT_QUOTA_MB'

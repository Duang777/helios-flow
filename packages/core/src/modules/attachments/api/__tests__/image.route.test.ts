...
describe('attachments image route', () => {
  ...
  it('returns 404 when authenticated non-super-admin queries cross-tenant attachment (query layer blocks before access check)', async () => {
    const { getAuthFromRequest } = await import('@helios/shared/lib/auth/server') as any
    ...
    const { checkAttachmentAccess } = await import('@helios/core/modules/attachments/lib/access') as any

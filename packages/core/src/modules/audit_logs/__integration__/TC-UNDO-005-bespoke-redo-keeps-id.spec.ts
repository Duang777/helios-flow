...
import {
  ...
  getTokenContext,
  readJsonSafe,
} from '@helios/core/helpers/integration/generalFixtures'
import {
...
} from '@helios/core/helpers/integration/salesFixtures'
import { deleteFeatureToggleIfExists } from '@helios/core/helpers/integration/featureTogglesFixtures'
...
function readOperation(res: APIResponse): Operation {
  const header = res.headers()['x-helios-operation'] ?? ''
  const enc = header.startsWith('omop:') ? header.slice(5) : ''
  expect(enc, 'x-helios-operation header carries an omop: payload').not.toBe('')
...
test.describe('TC-UNDO-005: bespoke create redo restores original ids', () => {
  ...
  test('sales.returns.create redo keeps id and return lines', async ({ request }) => {
    const token = await getAuthToken(request, 'admin')
    test.skip(!(await canManageSalesOrders(request, token)), 'principal lacks sales-write ACLs (run yarn helios auth sync-role-acls)')

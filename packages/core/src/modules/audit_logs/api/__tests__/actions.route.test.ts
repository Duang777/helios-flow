...
import { GET } from '@helios/core/modules/audit_logs/api/audit-logs/actions/route'
import { actionLogListSchema } from '@helios/core/modules/audit_logs/data/validators'
...
jest.mock('@helios/core/modules/audit_logs/api/audit-logs/display', () => ({
...
describe('GET /api/audit_logs/audit-logs/actions', () => {
  beforeEach(async () => {
    jest.clearAllMocks()
    const { resolveFeatureCheckContext } = await import('@helios/core/modules/directory/utils/organizationScope')
    ;(resolveFeatureCheckContext as jest.Mock).mockResolvedValue({
      organizationId: 'org-1',
      scope: { allowedIds: null },
    })
    const { loadAuditLogDisplayMaps } = await import('@helios/core/modules/audit_logs/api/audit-logs/display')

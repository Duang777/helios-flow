...
import { POST } from '@helios/core/modules/audit_logs/api/audit-logs/actions/redo/route'
...
describe('POST /api/audit_logs/audit-logs/actions/redo', () => {
  beforeEach(async () => {
    jest.clearAllMocks()
    const { resolveFeatureCheckContext, resolveOrganizationScopeForRequest } = await import('@helios/core/modules/directory/utils/organizationScope')
  ...
  it('replays latest undone log and returns undo metadata', async () => {
    ...
    const opHeader = res.headers.get('x-helios-operation')

...
import { getAuthFromRequest, type AuthContext } from '@helios/shared/lib/auth/server'
import { createRequestContainer } from '@helios/shared/lib/di/container'
import { resolveFeatureCheckContext, resolveOrganizationScopeForRequest } from '@helios/core/modules/directory/utils/organizationScope'
import type { RbacService } from '@helios/core/modules/auth/services/rbacService'
import { CommandBus } from '@helios/shared/lib/commands/command-bus'
import { ActionLogService } from '@helios/core/modules/audit_logs/services/actionLogService'
import type { CommandRuntimeContext, CommandLogMetadata } from '@helios/shared/lib/commands'
import { serializeOperationMetadata } from '@helios/shared/lib/commands/operationMetadata'
import type { AwilixContainer } from 'awilix'
import type { ActionLog } from '@helios/core/modules/audit_logs/data/entities'
import { z } from 'zod'
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (actionLog?.undoToken && actionLog.id) {
      ...
      response.headers.set('x-helios-operation', serializeOperationMetadata({
        id: actionLog.id,

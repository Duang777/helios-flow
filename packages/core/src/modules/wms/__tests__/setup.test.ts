...
import { ensureRoles } from '@helios/core/modules/auth/lib/setup-app'
import { findOneWithDecryption } from '@helios/shared/lib/encryption/find'
import features from '../acl'
import setup from '../setup'
import {
  WMS_CUSTHELIOS_ROLE_NAMES,
  WMS_MANAGE_FEATURES,
...
jest.mock('@helios/core/modules/auth/lib/setup-app', () => ({
...
describe('wms setup seedDefaults', () => {
  ...
  it('calls ensureRoles with operator and supervisor for the tenant', async () => {
    ...
    expect(mockEnsureRoles).toHaveBeenCalledWith(em, {
      tenantId,
      roleNames: [...WMS_CUSTHELIOS_ROLE_NAMES],
    })
...
describe('wms setup role mappings', () => {
  ...
  it('maps only known acl feature ids for custom roles', () => {
    for (const roleName of WMS_CUSTHELIOS_ROLE_NAMES) {

...
import { KbdShortcut } from '@helios/ui/primitives/kbd'
import {
  Select,
  SelectContent,
  SelectItem,
              ...
              if (call.response.status === 409 && (call.result as { error?: string } | null)?.error === 'balance_integrity_violation') {
                flash(
                  t(
                    ...
                    'Balance integrity error — run "helios wms verify-balances --repair" to diagnose and fix drift before retrying.',
                  ),
                  'error',
                )
                return {}

...
import { reconcileAttachmentOrganizations } from '@helios/core/modules/attachments/lib/reconcileOrganization'
  ...
  },
  {
    ...
    run: async ({ em, tenantId, organizationId }) => {
      const [{ INTERACTION_STATUS_DEFAULTS }, { ensureDictionaryEntry }] = await Promise.all([
        import('@helios/core/modules/customers/cli'),
        import('@helios/core/modules/customers/commands/shared'),
      ])

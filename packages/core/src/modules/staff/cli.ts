...
import { appendWidgetsToRoles } from '@helios/core/modules/dashboards/lib/role-widgets'
...
const seedExamplesCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!tenantId || !organizationId) {
      console.error('Usage: helios staff seed-examples --tenant <tenantId> --org <organizationId>')
      return
    }
...
const seedActivityTypesCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    const organizationId = String(args.organizationId ?? args.org ?? args.orgId ?? '')
    if (!tenantId || !organizationId) {
      console.error('Usage: helios staff seed-activity-types --tenant <tenantId> --org <organizationId>')
      return
    }
    const container = await createRequestContainer()
    const scope: StaffSeedScope = { tenantId, organizationId }
...
const seedAddressTypesCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    const organizationId = String(args.organizationId ?? args.org ?? args.orgId ?? '')
    if (!tenantId || !organizationId) {
      console.error('Usage: helios staff seed-address-types --tenant <tenantId> --org <organizationId>')
      return
    }
    const container = await createRequestContainer()
    const scope: StaffSeedScope = { tenantId, organizationId }
...
const seedTimesheetsWidgetsCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    const organizationId = String(args.organizationId ?? args.org ?? args.orgId ?? '')
    if (!tenantId || !organizationId) {
      console.error('Usage: helios staff seed-timesheets-widgets --tenant <tenantId> --org <organizationId>')

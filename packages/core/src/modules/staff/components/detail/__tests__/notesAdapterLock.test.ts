/**
 * Record-locks coverage (staff notes): the staff comments adapter writes through
 * the makeCrudRoute `staff/comments` route (server-side optimistic-lock guard via
 * the CRUD mutation-guard decorator). The client gap was that the adapter never
 * sent the expected-version header. This test asserts the update/delete adapter
 * calls attach the `x-helios-ext-optimistic-lock-expected-updated-at` header derived
 * from the note's loaded `updatedAt`. The resulting 409 is surfaced by the shared
 * NotesSection host through `surfaceRecordConflict` (see NotesSection.test.tsx),
 * so no per-adapter conflict handling is needed.
 */
import { OPTIMISTIC_LOCK_HEADER_NAME } from '@helios/shared/lib/crud/optimistic-lock-headers'
...
jest.mock('@helios/ui/backend/detail/NotesSection', () => ({
  mapCommentSummary: (input: unknown) => input,
}))

import { createStaffNotesAdapter } from '../notesAdapter'
...
describe('createStaffNotesAdapter — optimistic-lock header', () => {
  test('update attaches the version header derived from the note updatedAt', async () => {
    const adapter = createStaffNotesAdapter(translator)

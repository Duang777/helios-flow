...
import { defaultLocale } from '@helios/shared/lib/i18n/config'
  ...
  try {
    ...
    if (deliveryConfig.strategies.email.enabled && recipient?.email && panelLink) {
      ...
      const copy = {
        ...
        actionNotice: t('notifications.delivery.email.actionNotice', 'Actions are available in Helios and are read-only in this email.'),
        openCta: t('notifications.delivery.email.openCta', 'Open notification center'),
        footer: t('notifications.delivery.email.footer', 'Helios notifications'),
      }

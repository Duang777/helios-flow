...
- **Registry/type contracts** live in `@helios/shared/modules/integrations/types`
...
- Keep env names stable and provider-prefixed (for example `HELIOS_INTEGRATION_AKENEO_*`)
...
```typescript
import { buildIntegrationDetailWidgetSpotId } from '@helios/shared/modules/integrations/types'
...
- Use helpers from `@helios/core/modules/core/__integration__/helpers/*`

...
import { generateUniqueCurrencyCode } from '@helios/core/helpers/integration/currenciesFixtures';
import { getTokenContext } from '@helios/core/helpers/integration/generalFixtures';
 ...
 *      (prior field values restored) via the x-helios-operation undo token.
 */
...
function readUndoToken(res: APIResponse): string {
  ...
  expect(enc, 'x-helios-operation header should carry an omop: payload').not.toBe('');

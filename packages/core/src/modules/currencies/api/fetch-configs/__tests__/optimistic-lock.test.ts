...
jest.mock('@helios/core/modules/currencies/data/entities', () => ({
  CurrencyFetchConfig: class CurrencyFetchConfig {},
}))

jest.mock('@helios/core/modules/currencies/commands/fetch-configs', () => ({
  createFetchConfig: jest.fn(),
  updateFetchConfig: jest.fn(async () => configRecord),
  deleteFetchConfig: jest.fn(async () => undefined),
}))

import { PUT, DELETE } from '@helios/core/modules/currencies/api/fetch-configs/route'
import {
  updateFetchConfig,
  deleteFetchConfig,
} from '@helios/core/modules/currencies/commands/fetch-configs'
...
describe('currencies fetch-configs optimistic locking', () => {
  const originalEnv = process.env.HELIOS_OPTIMISTIC_LOCK
  ...
  afterAll(() => {
    if (originalEnv === undefined) delete process.env.HELIOS_OPTIMISTIC_LOCK
    else process.env.HELIOS_OPTIMISTIC_LOCK = originalEnv
  })

...
function resolveAppRoot(): string {
  const fromEnv = process.env.HELIOS_TEST_APP_ROOT?.trim()
  return fromEnv ? path.resolve(fromEnv) : path.resolve(process.cwd(), 'apps/helios')
}
...
function readEnvValue(key: string): string | undefined {
  ...
  const candidatePaths = [
    ...
    path.resolve(process.cwd(), 'apps/helios/.env'),

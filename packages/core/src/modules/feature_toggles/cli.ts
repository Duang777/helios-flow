...
const createToggle: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!identifier || !name) {
      console.error('Usage: helios feature_toggles toggle-create --identifier <id> --name <name> [--type boolean|string|number|json] [--defaultValue <value>] [--category <value>] [--description <value>]')
      return
    }
...
const updateToggle: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!identifier) {
      console.error('Usage: helios feature_toggles toggle-update --identifier <id> [--name <name>] [--defaultValue <value>] [--category <value>] [--description <value>]')
      return
    }
...
const deleteToggle: ModuleCli = {
  ...
  async run(rest) {
    ...
    const identifier = stringOption(args, 'identifier')
    if (!identifier) {
      console.error('Usage: helios feature_toggles toggle-delete --identifier <id>')
      return
    }
    const container = await createRequestContainer()
    const em = container.resolve('em') as EntityManager
...
const setOverrideValue: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!identifier || !tenantId) {
      console.error('Usage: helios feature_toggles override-set-value --identifier <id> --tenantId <uuid> --value <value>')
      return
    }
    const container = await createRequestContainer()
    const em = container.resolve('em') as EntityManager

...
const mockCommandBus = { execute: jest.fn(async () => ({ result: {}, logEntry: null })) }
...
jest.mock('@helios/core/modules/dictionaries/api/context', () => ({
...
describe('dictionary entry DELETE optimistic locking', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('succeeds when the expected version matches', async () => {
    ...
    expect(mockCommandBus.execute).toHaveBeenCalledWith('dictionaries.entries.delete', expect.anything())
  })

...
describe('api_docs /api/version route', () => {
  ...
  it('prefers an explicit HELIOS_VERSION / HELIOS_VERSION override', async () => {
    const originalOm = process.env.HELIOS_VERSION
    process.env.HELIOS_VERSION = '9.9.9-test'
    try {
      ...
      if (originalOm === undefined) delete process.env.HELIOS_VERSION
      else process.env.HELIOS_VERSION = originalOm
    }
  })
})

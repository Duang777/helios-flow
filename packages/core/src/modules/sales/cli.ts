...
const seedTaxRatesCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!tenantId || !organizationId) {
      console.error('Usage: helios sales seed-tax-rates --tenant <tenantId> --org <organizationId>')
      return
    }
...
const seedStatusesCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    const organizationId = String(args.organizationId ?? args.org ?? args.orgId ?? '')
    if (!tenantId || !organizationId) {
      console.error('Usage: helios sales seed-statuses --tenant <tenantId> --org <organizationId>')
      return
    }
    const container = await createRequestContainer()
    try {
...
const seedAdjustmentKindsCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    const organizationId = String(args.organizationId ?? args.org ?? args.orgId ?? '')
    if (!tenantId || !organizationId) {
      console.error('Usage: helios sales seed-adjustment-kinds --tenant <tenantId> --org <organizationId>')
      return
    }
    const container = await createRequestContainer()
    try {
...
const backfillDealLossReasonsCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    const organizationId = String(args.organizationId ?? args.org ?? args.orgId ?? '')
    if (!tenantId || !organizationId) {
      console.error('Usage: helios sales backfill-deal-loss-reasons --tenant <tenantId> --org <organizationId>')
      return
    }
    const container = await createRequestContainer()
    try {
...
const seedShippingMethodsCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    const organizationId = String(args.organizationId ?? args.org ?? args.orgId ?? '')
    if (!tenantId || !organizationId) {
      console.error('Usage: helios sales seed-shipping-methods --tenant <tenantId> --org <organizationId>')
      return
    }
    const container = await createRequestContainer()
    try {
...
const seedPaymentMethodsCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    const organizationId = String(args.organizationId ?? args.org ?? args.orgId ?? '')
    if (!tenantId || !organizationId) {
      console.error('Usage: helios sales seed-payment-methods --tenant <tenantId> --org <organizationId>')
      return
    }
    const container = await createRequestContainer()
    try {
...
const seedExamplesCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    const organizationId = String(args.organizationId ?? args.org ?? args.orgId ?? '')
    if (!tenantId || !organizationId) {
      console.error('Usage: helios sales seed-examples --tenant <tenantId> --org <organizationId>')
      return
    }
    const container = await createRequestContainer()
    try {

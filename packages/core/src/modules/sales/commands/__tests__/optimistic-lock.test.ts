...
describe('sales.payments.delete — parent-order aggregate optimistic lock (Gap A)', () => {
  ...
  it('OSS-only when locking is disabled (HELIOS_OPTIMISTIC_LOCK=off): a stale header does not 409', async () => {
    const previous = process.env.HELIOS_OPTIMISTIC_LOCK
    ...
    try {
      ...
      if (previous === undefined) delete process.env.HELIOS_OPTIMISTIC_LOCK
      else process.env.HELIOS_OPTIMISTIC_LOCK = previous
    }
  })
})

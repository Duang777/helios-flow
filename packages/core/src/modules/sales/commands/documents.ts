...
import { CrudHttpError, notFound } from "@helios/shared/lib/crud/errors";
import {
  deriveResourceFromCommandId,
  invalidateCrudCache,
} from "@helios/shared/lib/crud/cache";
...
import { loadCustomFieldValues } from "@helios/shared/lib/crud/custom-fields";
import { normalizeCustomFieldValues } from "@helios/shared/lib/custom-fields/normalize";
...
import { CustomFieldValue } from "@helios/core/modules/entities/data/entities";
import {
  CustomerAddress,
  CustomerEntity,
...
import {
  ...
  toUnitLookupKey,
} from "@helios/shared/lib/units/unitCodes";
import type { AuthContext } from "@helios/shared/lib/auth/server";
import type { TranslateWithFallbackFn } from "@helios/shared/lib/i18n/translate";
...
const UHELIOS_REFERENCE_UNITS = new Set<string>(REFERENCE_UNIT_CODES);
const UHELIOS_NORMALIZED_SCALE = 6;
const UHELIOS_NORMALIZED_MAX = 1_000_000_000_000;
...
function roundNormalizedQuantity(value: number): number {
  const factor = 10 ** UHELIOS_NORMALIZED_SCALE;
  if (!Number.isFinite(value)) return 0;
  return Math.round(value * factor) / factor;
}

function assertNormalizedPrecision(value: number) {
  if (!Number.isFinite(value) || Math.abs(value) >= UHELIOS_NORMALIZED_MAX) {
...
function resolveReferenceUnit(
  ...
  if (!UHELIOS_REFERENCE_UNITS.has(normalized)) return null;
...
async function normalizeLineUom(input: NormalizeLineUomInput): Promise<{
  ...
  if (!productId) {
    ...
    return {
      ...
      uomSnapshot: existingSnapshot ?? {
        ...
        rounding: { mode: "half_up", scale: UHELIOS_NORMALIZED_SCALE },
  ...
  const snapshot: SalesLineUomSnapshot = {
    ...
    rounding: {
      mode: "half_up",
      scale: UHELIOS_NORMALIZED_SCALE,
    },
    source: {
      conversionId,

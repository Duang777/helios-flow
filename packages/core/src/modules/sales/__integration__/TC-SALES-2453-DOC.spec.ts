...
} from '@helios/core/modules/core/__integration__/helpers/salesFixtures'
import {
  readUpdatedAt,
  putWithLock,
  resolveApiUrl,
...
test.describe('TC-SALES-2453-DOC: document scalar update survives interleaved recalc read', () => {
  test('order: comment + currency update persists after an interleaved totals recalc', async ({
    ...
    try {
      ...
      test.skip(
        ...
        'admin role lacks sales.orders.manage on this database (run yarn helios auth sync-role-acls)',
      )
  ...
  test('quote: comment + currency update persists after an interleaved totals recalc', async ({
    ...
    try {
      ...
      test.skip(
        !(await canManageSalesOrders(request, token)),
        'admin role lacks sales manage features on this database (run yarn helios auth sync-role-acls)',
      )

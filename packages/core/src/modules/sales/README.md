...
- Emit commands with undo support (via `@helios/shared/lib/commands`) for every state mutation; reducer style helpers can reuse the validators and calculation pipeline.
...
Run `helios sales seed-examples --tenant <tenantId> --org <organizationId>` to install demo quotes and orders with adjustments, shipments, payments, addresses, and notes. The script also seeds sales dictionaries plus example shipping and payment methods if they are missing.

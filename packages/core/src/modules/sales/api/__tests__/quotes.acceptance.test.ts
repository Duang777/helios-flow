...
import { POST as sendQuote } from '@helios/core/modules/sales/api/quotes/send/route'
import { GET as getPublicQuote } from '@helios/core/modules/sales/api/quotes/public/[token]/route'
import { POST as acceptQuote } from '@helios/core/modules/sales/api/quotes/accept/route'
import { SalesOrder, SalesQuote } from '@helios/core/modules/sales/data/entities'
import { Dictionary, DictionaryEntry } from '@helios/core/modules/dictionaries/data/entities'
import { hashAuthToken } from '@helios/core/modules/auth/lib/tokenHash'
import { LockMode } from '@mikro-orm/core'
import { commandRegistry } from '@helios/shared/lib/commands/registry'
...
jest.mock('@helios/core/bootstrap', () => ({
...
jest.mock('@helios/shared/lib/email/send', () => ({
  sendEmail: jest.fn().mockResolvedValue(undefined),
}))

jest.mock('@helios/core/modules/directory/utils/organizationScope', () => ({
...
describe('quote send + accept flow', () => {
  beforeEach(async () => {
    ...
    const { getAuthFromRequest } = await import('@helios/shared/lib/auth/server')
    const { getCachedRateLimiterService } = await import('@helios/core/bootstrap')
    const { resolveOrganizationScopeForRequest } = await import('@helios/core/modules/directory/utils/organizationScope')
...
describe('send - commits send-state before email delivery (fix: #2336, supersedes #1415)', () => {
  ...
  test('keeps the committed send-state when email delivery fails', async () => {
    ...
    const { sendEmail } = await import('@helios/shared/lib/email/send')
...
describe('quote editing invalidates sent token', () => {
  test('updating a sent quote clears token and reverts status to draft', async () => {
    ...
    await import('@helios/core/modules/sales/commands/documents')

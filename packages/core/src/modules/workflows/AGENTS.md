...
- Never leave `HELIOS_WORKFLOWS_ALLOW_PRIVATE_URLS` enabled in production.
...
| `CALL_WEBHOOK` | Call an external HTTP endpoint (SSRF-guarded via `@helios/shared/lib/url-safety`; `redirect: 'manual'`, 3xx rejected) |
...
|----------|--------|---------|
| `HELIOS_WORKFLOWS_ALLOW_PRIVATE_URLS` | When `1`/`true`/`yes`, bypasses the SSRF guard in `CALL_WEBHOOK` so workflow authors can hit `localhost`, RFC1918, and `.internal` targets. For dev only — MUST remain unset in production. | unset (guard enforced) |
| `HELIOS_WORKFLOWS_ENV_INTERPOLATION_ALLOWLIST` | Comma-separated non-secret process env keys allowed for `{{env.*}}` interpolation in workflow activity config. `APP_URL` is always allowed. Never include secrets. | unset (`APP_URL` only) |

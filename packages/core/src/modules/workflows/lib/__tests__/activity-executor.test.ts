...
describe('Activity Executor (Unit Tests)', () => {
  ...
  describe('EMIT_EVENT activity', () => {
    ...
    test('should not interpolate non-allowlisted env vars into event payloads', async () => {
      const originalSecret = process.env.HELIOS_WORKFLOWS_TEST_EVENT_SECRET
      process.env.HELIOS_WORKFLOWS_TEST_EVENT_SECRET = 'event-secret-value'
      ...
      const activity: ActivityDefinition = {
        ...
        config: {
          ...
          payload: {
            secret: '{{env.HELIOS_WORKFLOWS_TEST_EVENT_SECRET}}',
            message: 'prefix {{env.HELIOS_WORKFLOWS_TEST_EVENT_SECRET}} suffix',
          },
        },
      }
      ...
      try {
        ...
        if (originalSecret === undefined) {
          delete process.env.HELIOS_WORKFLOWS_TEST_EVENT_SECRET
        } else {
          process.env.HELIOS_WORKFLOWS_TEST_EVENT_SECRET = originalSecret
        }
      }
    })
  ...
  describe('CALL_WEBHOOK activity', () => {
    const originalAllowPrivate = process.env.HELIOS_WORKFLOWS_ALLOW_PRIVATE_URLS
    ...
    beforeAll(() => {
      ...
      process.env.HELIOS_WORKFLOWS_ALLOW_PRIVATE_URLS = '1'
    })

    afterAll(() => {
      if (originalAllowPrivate === undefined) {
        delete process.env.HELIOS_WORKFLOWS_ALLOW_PRIVATE_URLS
      } else {
        process.env.HELIOS_WORKFLOWS_ALLOW_PRIVATE_URLS = originalAllowPrivate
      }
    })
    ...
    test('should not interpolate non-allowlisted env vars into webhook headers or body', async () => {
      const originalSecret = process.env.HELIOS_WORKFLOWS_TEST_WEBHOOK_SECRET
      process.env.HELIOS_WORKFLOWS_TEST_WEBHOOK_SECRET = 'webhook-secret-value'
      ...
      const activity: ActivityDefinition = {
        ...
        config: {
          ...
          method: 'POST',
          headers: {
            Authorization: 'Bearer {{env.HELIOS_WORKFLOWS_TEST_WEBHOOK_SECRET}}',
          },
          body: {
            secret: '{{env.HELIOS_WORKFLOWS_TEST_WEBHOOK_SECRET}}',
          },
        },
      }
      ...
      try {
        ...
        if (originalSecret === undefined) {
          delete process.env.HELIOS_WORKFLOWS_TEST_WEBHOOK_SECRET
        } else {
          process.env.HELIOS_WORKFLOWS_TEST_WEBHOOK_SECRET = originalSecret
        }
      }
    })
  ...
  describe('CALL_WEBHOOK SSRF guard', () => {
    ...
    test('should allow private URLs when HELIOS_WORKFLOWS_ALLOW_PRIVATE_URLS=true', async () => {
      ;(global.fetch as jest.Mock).mockResolvedValue({
        ok: true,
        status: 200,
        statusText: 'OK',
      ...
      const prev = process.env.HELIOS_WORKFLOWS_ALLOW_PRIVATE_URLS
      const prevLegacy = process.env.WORKFLOW_WEBHOOK_ALLOW_PRIVATE_URLS
      const prevNodeEnv = process.env.NODE_ENV
      mockLoggerInstance.warn.mockClear()
      try {
        process.env.HELIOS_WORKFLOWS_ALLOW_PRIVATE_URLS = 'true'
        ...
        if (prev === undefined) {
          ...
          process.env.HELIOS_WORKFLOWS_ALLOW_PRIVATE_URLS = prev
        }
    ...
    test('should ignore HELIOS_WORKFLOWS_ALLOW_PRIVATE_URLS in production', async () => {
      ;(global.fetch as jest.Mock).mockResolvedValue({
        ok: true,
        status: 200,
        statusText: 'OK',
  ...
  describe('Variable interpolation type preservation', () => {
    ...
    test('should interpolate allowlisted env vars only', async () => {
      ...
      const originalAllowlist = process.env.HELIOS_WORKFLOWS_ENV_INTERPOLATION_ALLOWLIST
      const originalEndpoint = process.env.HELIOS_WORKFLOWS_TEST_PUBLIC_ENDPOINT
      const originalSecret = process.env.HELIOS_WORKFLOWS_TEST_TYPE_SECRET
      process.env.APP_URL = 'https://app.example.com'
      process.env.HELIOS_WORKFLOWS_ENV_INTERPOLATION_ALLOWLIST =
        'HELIOS_WORKFLOWS_TEST_PUBLIC_ENDPOINT'
      process.env.HELIOS_WORKFLOWS_TEST_PUBLIC_ENDPOINT = 'https://endpoint.example.com'
      process.env.HELIOS_WORKFLOWS_TEST_TYPE_SECRET = 'type-secret-value'
      ...
      const activity: ActivityDefinition = {
        ...
        config: {
          ...
          args: {
            appUrl: '{{env.APP_URL}}',
            endpoint: '{{env.HELIOS_WORKFLOWS_TEST_PUBLIC_ENDPOINT}}',
            secret: '{{env.HELIOS_WORKFLOWS_TEST_TYPE_SECRET}}',
            message:
              'App {{env.APP_URL}} endpoint {{env.HELIOS_WORKFLOWS_TEST_PUBLIC_ENDPOINT}} secret {{env.HELIOS_WORKFLOWS_TEST_TYPE_SECRET}}',
          },
        },
      }
      ...
      try {
        ...
        if (originalAllowlist === undefined) {
          delete process.env.HELIOS_WORKFLOWS_ENV_INTERPOLATION_ALLOWLIST
        } else {
          process.env.HELIOS_WORKFLOWS_ENV_INTERPOLATION_ALLOWLIST = originalAllowlist
        }
        if (originalEndpoint === undefined) {
          delete process.env.HELIOS_WORKFLOWS_TEST_PUBLIC_ENDPOINT
        } else {
          process.env.HELIOS_WORKFLOWS_TEST_PUBLIC_ENDPOINT = originalEndpoint
        }
        if (originalSecret === undefined) {
          delete process.env.HELIOS_WORKFLOWS_TEST_TYPE_SECRET
        } else {
          process.env.HELIOS_WORKFLOWS_TEST_TYPE_SECRET = originalSecret
        }
      }
    })

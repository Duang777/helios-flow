 ...
 *
 * Functional API (no classes) following Helios conventions.
 */
...
import { createModuleQueue, Queue } from '@helios/queue'
import { getRedisUrl } from '@helios/shared/lib/redis/connection'
import {
  safeOutboundFetch,
  UnsafeOutboundUrlError,
  type HostLookup,
} from '@helios/shared/lib/url-safety'
import { parseBooleanWithDefault } from '@helios/shared/lib/boolean'
import { hasAllFeatures } from '@helios/shared/security/features'
...
export { isPrivateUrl } from '@helios/shared/lib/network'
import { createLogger } from '@helios/shared/lib/logger'

const logger = createLogger('workflows')

function isAllowPrivateWorkflowWebhookUrlsEnabled(): boolean {
  if (parseBooleanWithDefault(process.env.HELIOS_WORKFLOWS_ALLOW_PRIVATE_URLS, false)) {
    if (process.env.NODE_ENV === 'production') {
      logger.warn('HELIOS_WORKFLOWS_ALLOW_PRIVATE_URLS is set but ignored in production. SSRF protection remains enabled.', { component: 'CALL_WEBHOOK' })
      return false
    }

    logger.warn('HELIOS_WORKFLOWS_ALLOW_PRIVATE_URLS is enabled. SSRF protection is bypassed for workflow webhooks; use only in development.', { component: 'CALL_WEBHOOK' })
    return true
  }

  if (parseBooleanWithDefault(process.env.WORKFLOW_WEBHOOK_ALLOW_PRIVATE_URLS, false)) {
    if (process.env.NODE_ENV === 'production') {
      logger.warn('WORKFLOW_WEBHOOK_ALLOW_PRIVATE_URLS is deprecated and ignored in production. Use HELIOS_WORKFLOWS_ALLOW_PRIVATE_URLS for development only. SSRF protection remains enabled.', { component: 'CALL_WEBHOOK' })
      return false
    }

    logger.warn('WORKFLOW_WEBHOOK_ALLOW_PRIVATE_URLS is deprecated. Use HELIOS_WORKFLOWS_ALLOW_PRIVATE_URLS instead. SSRF protection is bypassed.', { component: 'CALL_WEBHOOK' })
    return true
  }
...
const WORKFLOW_ENV_INTERPOLATION_ALLOWLIST_KEY = 'HELIOS_WORKFLOWS_ENV_INTERPOLATION_ALLOWLIST'
...
async function resolveDictionaryEntryId(
  ...
  try {
    ...
    const { Dictionary, DictionaryEntry } = await import('@helios/core/modules/dictionaries/data/entities')
 ...
 *
 * Makes authenticated HTTP request to internal Helios APIs
...
async function resolveActiveRoleIdsForUser(
  ...
  const { findOneWithDecryption, findWithDecryption } = await import('@helios/shared/lib/encryption/find')
...
export async function resolveCallApiRoleIds(
  ...
  const { findOneWithDecryption } = await import('@helios/shared/lib/encryption/find')

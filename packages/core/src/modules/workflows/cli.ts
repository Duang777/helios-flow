...
import { BusinessRule, type RuleType } from '@helios/core/modules/business_rules/data/entities'
import {
  invalidateBusinessRuleDiscoveryCache,
  resolveBusinessRuleDiscoveryCache,
} from '@helios/core/modules/business_rules/lib/rule-engine'
...
const seedDemoWithRules: ModuleCli = {
  ...
  async run(rest: string[]) {
    ...
    if (!tenantId || !organizationId) {
      console.error('Usage: helios workflows seed-demo-with-rules --tenant <tenantId> --org <organizationId>')
      console.error('   or: helios workflows seed-demo-with-rules -t <tenantId> -o <organizationId>')
      return
    }
...
const seedSalesPipeline: ModuleCli = {
  ...
  async run(rest: string[]) {
    ...
    if (!tenantId || !organizationId) {
      console.error('Usage: helios workflows seed-sales-pipeline --tenant <tenantId> --org <organizationId>')
      return
    }
...
const seedOrderApproval: ModuleCli = {
  ...
  async run(rest: string[]) {
    ...
    if (!tenantId || !organizationId) {
      console.error('Usage: helios workflows seed-order-approval --tenant <tenantId> --org <organizationId>')
      return
    }
...
const seedAll: ModuleCli = {
  ...
  async run(rest: string[]) {
    ...
    if (!tenantId || !organizationId) {
      console.error('Usage: helios workflows seed-all --tenant <tenantId> --org <organizationId>')
      return
    }

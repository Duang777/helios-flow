...
/**
 * TC-WF-012: Sync WAIT activity executes end-to-end
 *
 ...
 * skipped.
 */
test.describe('TC-WF-012: Sync WAIT activity delays and completes', () => {
  test('workflow with sync WAIT reaches COMPLETED after the configured duration', async ({ request }) => {
    const token = await getAuthToken(request, 'admin')
    const timestamp = Date.now()
    const workflowId = `qa-wf-012-${timestamp}`

    const definitionPayload = {
      workflowId,
      workflowName: `QA TC-WF-012 ${timestamp}`,

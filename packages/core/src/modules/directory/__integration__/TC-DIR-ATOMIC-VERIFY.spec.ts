...
import { getTokenContext, readJsonSafe } from '@helios/core/helpers/integration/generalFixtures';
 ...
 *   directory.organizations create/delete return an `x-helios-operation` undo token.
...
function readUndoToken(res: APIResponse): string {
  const enc = (res.headers()['x-helios-operation'] ?? '').slice(5);

...
describe('resolveOrganizationScopeForRequest caching (Phase 4)', () => {
  const originalTtl = process.env.HELIOS_ORG_SCOPE_CACHE_TTL_MS

  afterEach(() => {
    if (originalTtl === undefined) delete process.env.HELIOS_ORG_SCOPE_CACHE_TTL_MS
    else process.env.HELIOS_ORG_SCOPE_CACHE_TTL_MS = originalTtl
  })

  it('caches the scope for the (user, tenant, selectedOrg, requestedTenant) tuple', async () => {
    process.env.HELIOS_ORG_SCOPE_CACHE_TTL_MS = '60000'
  ...
  it('memoizes resolution per request, deduping the double resolution (issue #2259)', async () => {
    ...
    process.env.HELIOS_ORG_SCOPE_CACHE_TTL_MS = '0'
    const em = createMockEm([{ id: 'org-home', descendantIds: [] }])
    const rbac = createMockRbac()
    const cache = createMemoryCache()
    const container = createContainer(em, rbac, cache)
  ...
  it('does not cache when HELIOS_ORG_SCOPE_CACHE_TTL_MS=0', async () => {
    process.env.HELIOS_ORG_SCOPE_CACHE_TTL_MS = '0'
    const em = createMockEm([{ id: 'org-home', descendantIds: [] }])
    const rbac = createMockRbac()
    const cache = createMemoryCache()

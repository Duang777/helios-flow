...
import { upsertCustomFieldDefSchema, fieldsetCodeRegex } from '@helios/core/modules/entities/data/validators'
import {
...
import { filterSelectableSystemEntityIds, isSystemEntitySelectable } from '@helios/shared/lib/entities/system-entities'
...
import { CURRENCY_OPTIONS_URL } from '@helios/shared/modules/entities/kinds'
...
async function validateDefaultValueByKind(
  ...
  switch (kind) {
    ...
    case 'dictionary': {
      ...
      if (dictionaryId) {
        try {
          const { DictionaryEntry } = await import('@helios/core/modules/dictionaries/data/entities')
    ...
    case 'currency': {
      ...
      try {
        const { Currency } = await import('@helios/core/modules/currencies/data/entities')

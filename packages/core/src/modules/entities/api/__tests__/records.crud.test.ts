...
import { GET, POST, PUT, DELETE } from '@helios/core/modules/entities/api/records'
import { UNKNOWN_CUSTHELIOS_FIELD_ERROR } from '@helios/shared/modules/entities/validation'
...
describe('Records API CRUD operations', () => {
  ...
  describe('POST /api/entities/records', () => {
    ...
    it('rejects undeclared custom field keys with 400', async () => {
      ...
      const json = await res.json()
      expect(json.fields.cf_undeclared).toBe(UNKNOWN_CUSTHELIOS_FIELD_ERROR)
  ...
  describe('DELETE /api/entities/records', () => {
    ...
    it('rejects a stale delete with 409 when the expected version does not match (#3227)', async () => {
      ...
      const req = new Request(
        ...
        {
          method: 'DELETE',
          headers: { 'x-helios-ext-optimistic-lock-expected-updated-at': '2026-04-01T00:00:00.000Z' },
        },
      )
      const res = await DELETE(req)
    ...
    it('allows the delete when the expected version matches the current version (#3227)', async () => {
      ...
      const req = new Request(
        ...
        {
          method: 'DELETE',
          headers: { 'x-helios-ext-optimistic-lock-expected-updated-at': '2026-05-01T00:00:00.000Z' },
        },
      )
      const res = await DELETE(req)
      expect(res.status).toBe(200)

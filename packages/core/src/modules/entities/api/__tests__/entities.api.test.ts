...
import { OPTIMISTIC_LOCK_HEADER_NAME, OPTIMISTIC_LOCK_CONFLICT_CODE } from '@helios/shared/lib/crud/optimistic-lock-headers'
...
jest.mock('@helios/shared/lib/encryption/entityIds', () => ({
  getEntityIds: () => ({
    customers: { customer_person: 'customers:customer_person' },
  }),
}))

jest.mock('@helios/shared/lib/entities/system-entities', () => ({
...
describe('POST /api/entities/entities — optimistic locking', () => {
  ...
  it('a record_lock conflict from the enterprise guard aborts the write with a 409', async () => {
    const { CrudHttpError } = await import('@helios/shared/lib/crud/errors')

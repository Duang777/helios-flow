...
```
import { upsertCustomEntity } from '@helios/core/modules/entities/lib/register'
...
UI integration tips
- In client components, prefer `apiCall` helpers (from `@helios/ui/backend/utils/apiCall`) instead of the global `fetch`.

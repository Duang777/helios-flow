import { getCliModules, getDefaultEncryptionMaps, type Module, type ModuleCli } from '@helios/shared/modules/registry'
...
import { createKmsService, type KmsService, type TenantDek } from '@helios/shared/lib/encryption/kms'
import {
  decryptWithAesGcm,
  decryptWithAesGcmStrict,
  TenantDataEncryptionError,
  TenantDataEncryptionErrorCode,
} from '@helios/shared/lib/encryption/aes'
import {
  TenantDataEncryptionService,
  parseDecryptedFieldValue,
} from '@helios/shared/lib/encryption/tenantDataEncryptionService'
import { resolveEntityIdFromMetadata } from '@helios/shared/lib/encryption/entityIds'
...
const decryptDatabase: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!dryRun) {
      ...
      console.log(`   3. Run: helios query_index reindex --tenant ${tenantIdArg}  ← run after env flip + restart; search/filter degraded until this completes`)
      console.log(`   4. Run: helios entities decrypt-database --tenant ${tenantIdArg} --check  to confirm no encrypted values remain`)

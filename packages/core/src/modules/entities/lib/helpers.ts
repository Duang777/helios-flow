...
import type { TenantDataEncryptionService } from '@helios/shared/lib/encryption/tenantDataEncryptionService'
import { encryptCustomFieldValue, resolveTenantEncryptionService } from '@helios/shared/lib/encryption/customFieldValues'
import {
  MAX_CUSTHELIOS_FIELD_KEYS_PER_RECORD,
  TOO_MANY_CUSTHELIOS_FIELDS_ERROR,
} from '@helios/shared/modules/entities/validation'
...
export async function setRecordCustomFields(
  ...
  if (preferDefs && presentKeyCount > MAX_CUSTHELIOS_FIELD_KEYS_PER_RECORD) {
    throw new Error(TOO_MANY_CUSTHELIOS_FIELDS_ERROR)
  }

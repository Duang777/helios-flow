...
import { registerEntityIds } from '@helios/shared/lib/encryption/entityIds'
...
jest.mock('@helios/core/modules/entities/lib/install-from-ce', () => ({
...
jest.mock('@helios/shared/lib/encryption/aes', () => {
  const actual = jest.requireActual('@helios/shared/lib/encryption/aes')
  return {
    ...actual,
...
jest.mock('@helios/shared/lib/encryption/kms', () => ({
...
describe('entities decrypt-database CLI', () => {
  ...
  beforeEach(() => {
    ...
    const togglesMock = require('@helios/shared/lib/encryption/toggles')
    togglesMock.isTenantDataEncryptionEnabled.mockReturnValue(true)
    cli = require('@helios/core/modules/entities/cli').default
  })
  ...
  it('aborts when TENANT_DATA_ENCRYPTION is disabled', async () => {
    const { isTenantDataEncryptionEnabled } = require('@helios/shared/lib/encryption/toggles')

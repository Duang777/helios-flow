...
import { POST as cancelShipment } from '@helios/core/modules/shipping_carriers/api/cancel/route'
import { GET as searchDropOffPoints } from '@helios/core/modules/shipping_carriers/api/points/route'
import { POST as calculateRates } from '@helios/core/modules/shipping_carriers/api/rates/route'
import { POST as createShipment } from '@helios/core/modules/shipping_carriers/api/shipments/route'
import { GET as getTracking } from '@helios/core/modules/shipping_carriers/api/tracking/route'
import { POST as refreshTracking } from '@helios/core/modules/shipping_carriers/api/tracking/refresh/route'
...
import { readJsonSafe } from '@helios/shared/lib/http/readJsonSafe'
...
jest.mock('@helios/shared/lib/auth/server', () => ({ getAuthFromRequest: jest.fn() }))
jest.mock('@helios/shared/lib/di/container', () => ({ createRequestContainer: jest.fn() }))
jest.mock('@helios/shared/lib/http/readJsonSafe', () => ({ readJsonSafe: jest.fn() }))
jest.mock('@helios/shared/lib/i18n/server', () => ({
  resolveTranslations: jest.fn(async () => ({ translate: mockTranslate, t: mockTranslate })),
}))
jest.mock('@helios/shared/lib/crud/mutation-guard', () => ({
...
const carrierLoggerError = jest.requireMock('@helios/shared/lib/logger').createLogger('shipping_carriers').error as jest.Mock

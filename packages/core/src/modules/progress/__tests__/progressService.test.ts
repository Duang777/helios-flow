...
describe('progress service — broadcast coalescing (#2972)', () => {
  const originalInterval = process.env.HELIOS_PROGRESS_BROADCAST_MIN_INTERVAL_MS

  afterEach(() => {
    if (originalInterval === undefined) {
      delete process.env.HELIOS_PROGRESS_BROADCAST_MIN_INTERVAL_MS
    } else {
      process.env.HELIOS_PROGRESS_BROADCAST_MIN_INTERVAL_MS = originalInterval
    }
  })
  ...
  it('coalesces rapid successive updates within the interval into a single flush + broadcast', async () => {
    process.env.HELIOS_PROGRESS_BROADCAST_MIN_INTERVAL_MS = '1000'
    const em = buildEm()
    const eventBus = { emit: jest.fn().mockResolvedValue(undefined) }
  ...
  it('restores per-update emission when HELIOS_PROGRESS_BROADCAST_MIN_INTERVAL_MS=0', async () => {
    process.env.HELIOS_PROGRESS_BROADCAST_MIN_INTERVAL_MS = '0'
    const em = buildEm()
    const eventBus = { emit: jest.fn().mockResolvedValue(undefined) }
    const job = buildRunningJob()
    em.findOneOrFail.mockResolvedValue(job)

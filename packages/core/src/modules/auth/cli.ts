...
import { HELIOS_LOG_LEVEL_ENV, resetLogLevelCache } from '@helios/shared/lib/logger'
import { resetServerLoggerCache } from '@helios/shared/lib/logger'
import { hash } from 'bcryptjs'
import type { EntityManager } from '@mikro-orm/postgresql'
import { User, Role, UserRole } from '@helios/core/modules/auth/data/entities'
import { Tenant, Organization } from '@helios/core/modules/directory/data/entities'
import { rebuildHierarchyForTenant } from '@helios/core/modules/directory/lib/hierarchy'
...
import { isTenantDataEncryptionEnabled } from '@helios/shared/lib/encryption/toggles'
import { createKmsService } from '@helios/shared/lib/encryption/kms'
import { TenantDataEncryptionService } from '@helios/shared/lib/encryption/tenantDataEncryptionService'
import { decryptWithAesGcm } from '@helios/shared/lib/encryption/aes'
import { env } from 'process'
import type { KmsService, TenantDek } from '@helios/shared/lib/encryption/kms'
import crypto from 'node:crypto'
import { formatPasswordRequirements, getPasswordPolicy, validatePassword } from '@helios/shared/lib/auth/passwordPolicy'
import { parseBooleanToken } from '@helios/shared/lib/boolean'
import { getCliModules } from '@helios/shared/modules/registry'
...
const addUser: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!email || !password || !organizationId) {
      console.error('Usage: helios auth add-user --email <email> --password <password> --organizationId <id> [--roles customer,employee]')
      return
    }
...
const addOrganization: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!name) {
      console.error('Usage: helios auth add-org --name <organization name>')
      return
    }
    const { resolve } = await createRequestContainer()
    const em = resolve('em') as any
...
const SETUP_USAGE = 'Usage: helios auth setup --orgName <name> --email <email> --password <password> [--orgSlug <slug>] [--roles superadmin,admin,employee] [--skip-password-policy] [--include-demo-users] [--with-examples] [--json]'
...
const setupApp: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (jsonMode) {
      ...
      // no-op for the duration of the call because HELIOS_CLI_QUIET is honored
      ...
      process.env.HELIOS_CLI_QUIET = process.env.HELIOS_CLI_QUIET ?? '1'
      const originalLog = console.log
      const originalInfo = console.info
      console.log = () => undefined
      console.info = () => undefined
      const originalLogLevel = process.env[HELIOS_LOG_LEVEL_ENV]
      process.env[HELIOS_LOG_LEVEL_ENV] = 'error'
      resetLogLevelCache()
      resetServerLoggerCache()
      restoreConsole = () => {
        console.log = originalLog
        console.info = originalInfo
        if (originalLogLevel === undefined) delete process.env[HELIOS_LOG_LEVEL_ENV]
        else process.env[HELIOS_LOG_LEVEL_ENV] = originalLogLevel
        resetLogLevelCache()
        resetServerLoggerCache()
      }
    }
    ...
    try {
      ...
      if (err instanceof DerivedUserPasswordRequiredError) {
        process.stderr.write(
          `Setup aborted: ${err.message}. In production, set HELIOS_INIT_ADMIN_PASSWORD and HELIOS_INIT_EMPLOYEE_PASSWORD before passing --include-demo-users, or omit --include-demo-users to skip derived demo accounts entirely.\n`,
        )
        process.exitCode = 2
        return
      }
...
const setPassword: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!email || !password) {
      console.error('Usage: helios auth set-password --email <email> --password <newPassword>')
      return
    }
    if (!ensurePasswordPolicy(password)) return

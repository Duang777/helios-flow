...
describe('rbacDefaultCache', () => {
  const originalToggle = process.env.HELIOS_RBAC_DEFAULT_CACHE

  beforeEach(() => {
    delete process.env.HELIOS_RBAC_DEFAULT_CACHE
    resetRbacFallbackCache()
  })

  afterEach(() => {
    if (originalToggle === undefined) delete process.env.HELIOS_RBAC_DEFAULT_CACHE
    else process.env.HELIOS_RBAC_DEFAULT_CACHE = originalToggle
    resetRbacFallbackCache()
  })
  ...
  it('isRbacDefaultCacheEnabled defaults to OFF and turns on for explicit opt-in tokens', () => {
    ...
    for (const token of ['on', '1', 'true', 'yes', 'ON', 'True']) {
      process.env.HELIOS_RBAC_DEFAULT_CACHE = token

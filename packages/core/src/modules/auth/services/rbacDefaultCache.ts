import type { CacheStrategy, CacheValue, CacheSetOptions, CacheGetOptions } from '@helios/cache'
 ...
 * deployments wire `@helios/cache` via bootstrap.ts, which preempts
 * this fallback.
 *
 * Goals:
 ...
 *  - Honor `HELIOS_RBAC_DEFAULT_CACHE=off` so callers can disable it explicitly.
...
const GLOBAL_KEY = '__heliosRbacFallbackCache__'
...
export function isRbacDefaultCacheEnabled(): boolean {
  ...
  // Set `HELIOS_RBAC_DEFAULT_CACHE=on` (or `1`/`true`/`yes`) to enable the
  // in-process LRU fallback.
  const raw = process.env.HELIOS_RBAC_DEFAULT_CACHE

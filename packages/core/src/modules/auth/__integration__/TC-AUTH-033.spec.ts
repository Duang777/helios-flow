...
import { apiRequest, getAuthToken } from '@helios/core/modules/core/__integration__/helpers/api'
import { getTokenScope, readJsonSafe } from '@helios/core/modules/core/__integration__/helpers/generalFixtures'
  ...
  while (true) {
    ...
    if (fs.existsSync(candidate)) {
      try {
        ...
        if (pkg && (pkg.workspaces || pkg.name === 'helios')) return current
      } catch {
    ...
    if (parent === current) {
      throw new Error(`Could not locate Helios repo root above ${startDir}`)
    }
    current = parent
  }
}

function resolveHeliosCwd(): string {
  const appRoot = process.env.HELIOS_TEST_APP_ROOT?.trim()
...
const heliosCwd = resolveHeliosCwd()
const heliosEnv = readHeliosEnvFile(heliosCwd)

const HELIOS_CLI_TIMEOUT_MS = 90_000

function readHeliosEnvFile(appRoot: string): Record<string, string> {
  const envPath = path.join(appRoot, '.env')
  if (!fs.existsSync(envPath)) return {}
  return parseDotenv(fs.readFileSync(envPath, 'utf8'))
}

function buildHeliosCommandEnv(): NodeJS.ProcessEnv {
  return {
    ...heliosEnv,
    ...process.env,
    FORCE_COLOR: '0',
...
function runHelios(args: string[]): string {
  return execFileSync('yarn', ['helios', ...args], {
    cwd: heliosCwd,
    encoding: 'utf8',
    env: buildHeliosCommandEnv(),
    timeout: HELIOS_CLI_TIMEOUT_MS,
  })
}

function runHeliosCapture(args: string[]): { status: number | null; output: string } {
  const result = spawnSync('yarn', ['helios', ...args], {
    cwd: heliosCwd,
    encoding: 'utf8',
    env: buildHeliosCommandEnv(),
    timeout: HELIOS_CLI_TIMEOUT_MS,
...
test.describe('TC-AUTH-033: auth sync-role-acls CLI', () => {
  ...
  test('restores default feature ACLs for the admin role after the admin ACL is cleared', async ({ request }) => {
    ...
    try {
      ...
      const output = runHelios(['auth', 'sync-role-acls', '--tenant', tenantId])
  ...
  test('is idempotent — second run adds no new ACL changes and exits cleanly', async ({ request }) => {
    ...
    const first = runHelios(['auth', 'sync-role-acls', '--tenant', tenantId])
    const second = runHelios(['auth', 'sync-role-acls', '--tenant', tenantId])
  ...
  test('fails cleanly with an invalid --tenant value', async () => {
    const { output } = runHeliosCapture(['auth', 'sync-role-acls', '--tenant', '   '])
  ...
  test('fails when --tenant points at a non-existent tenant id', async () => {
    ...
    const { output } = runHeliosCapture(['auth', 'sync-role-acls', '--tenant', missingTenantId])

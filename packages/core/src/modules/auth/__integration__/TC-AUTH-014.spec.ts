...
test.describe('TC-AUTH-014: Organization Switching', () => {
  ...
  test('should allow switching organization context from the header selector', async ({ page }) => {
    ...
    const organizationTrigger = () => page.getByRole('button', { name: /^Organization:/ }).first();
    ...
    await expect(organizationTrigger()).toContainText(/All organizations/i);
    ...
    if (orgLabel.length > 0) {
      await expect(organizationTrigger()).toContainText(orgLabel);
    }
  });
});

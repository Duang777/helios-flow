...
import { GET, POST } from '@helios/core/modules/auth/api/session/refresh'
...
jest.mock('@helios/shared/lib/auth/jwt', () => ({
  signJwt: (...args: unknown[]) => signJwt(...(args as [])),
}))

jest.mock('@helios/core/modules/auth/lib/rateLimitCheck', () => ({
...
describe('/api/auth/session/refresh', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    process.env.APP_URL = 'https://demo.helios.com'
  })
  ...
  it('GET clears cookies and redirects to the allowlisted app origin when session cookie is missing', async () => {
    const response = await GET(new Request('https://demo.helios.com/api/auth/session/refresh?redirect=%2Fbackend'))

    expect(response.status).toBe(307)
    expect(response.headers.get('location')).toBe('https://demo.helios.com/login?redirect=%2Fbackend')
  ...
  it('GET falls back to a host-relative redirect when the request origin is not allowlisted', async () => {
    const response = await GET(new Request('https://develop.helios.com/api/auth/session/refresh?redirect=%2Fbackend'))
  ...
  it('GET sanitizes a // open-redirect bypass before forwarding to /login', async () => {
    const response = await GET(new Request('https://demo.helios.com/api/auth/session/refresh?redirect=%2Fbackend%2F%2Fevil.com'))

    expect(response.status).toBe(307)
    expect(response.headers.get('location')).toBe('https://demo.helios.com/login?redirect=%2F')
  })
  ...
  it('GET redirects valid browser refreshes to the allowlisted app origin', async () => {
    ...
    expect(response.status).toBe(307)
    expect(response.headers.get('location')).toBe('https://demo.helios.com/backend')
  ...
  it('GET mints scope claims as null, never the string "null", when the user has no tenant or organization', async () => {
    ...
    await GET(new Request('https://demo.helios.com/api/auth/session/refresh?redirect=%2Fbackend', {

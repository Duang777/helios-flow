...
import { GET } from '@helios/core/modules/auth/api/autologin'
...
jest.mock('@helios/shared/lib/auth/jwt', () => ({ signJwt: () => 'jwt-token' }))

jest.mock('@helios/shared/lib/auth/server', () => ({
  getAuthFromRequest: jest.fn(async () => null),
}))

jest.mock('@helios/core/modules/auth/events', () => ({
...
function setAutoLoginEnv(env: Record<string, string | undefined>) {
  delete process.env.HELIOS_AUTOLOGIN_EMAIL
  delete process.env.HELIOS_AUTOLOGIN_PASSWORD
  delete process.env.HELIOS_AUTOLOGIN_TENANT
...
describe('GET /api/auth/autologin', () => {
  ...
  test('signs the visitor in and sets the auth cookie when credentials resolve to one user', async () => {
    setAutoLoginEnv({ HELIOS_AUTOLOGIN_EMAIL: 'superadmin@acme.com', HELIOS_AUTOLOGIN_PASSWORD: 'secret' })
  ...
  test('scopes the lookup by tenant when HELIOS_AUTOLOGIN_TENANT is set', async () => {
    setAutoLoginEnv({
      HELIOS_AUTOLOGIN_EMAIL: 'superadmin@acme.com',
      HELIOS_AUTOLOGIN_PASSWORD: 'secret',
      HELIOS_AUTOLOGIN_TENANT: tenantId,
    })
  ...
  test('falls back to /login without a cookie when the email resolves to no single user', async () => {
    setAutoLoginEnv({ HELIOS_AUTOLOGIN_EMAIL: 'ghost@acme.com', HELIOS_AUTOLOGIN_PASSWORD: 'secret' })
  ...
  test('falls back to /login when the password does not verify', async () => {
    setAutoLoginEnv({ HELIOS_AUTOLOGIN_EMAIL: 'superadmin@acme.com', HELIOS_AUTOLOGIN_PASSWORD: 'wrong' })

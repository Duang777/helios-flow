...
import { AuthService } from '@helios/core/modules/auth/services/authService'
import { signJwt } from '@helios/shared/lib/auth/jwt'
import { getAuthFromRequest } from '@helios/shared/lib/auth/server'
import { buildSafeRedirectResponse, resolveTrustedRedirectBase } from '@helios/core/modules/auth/lib/requestRedirect'
import { sanitizeRedirectPath } from '@helios/core/modules/auth/lib/safeRedirect'
import { resolveAutoLoginCredentials } from '@helios/core/modules/auth/lib/autologin'
import { emitAuthEvent } from '@helios/core/modules/auth/events'
...
// Demo-only convenience: when HELIOS_AUTOLOGIN_* credentials are configured, sign the
...
export async function GET(req: Request) {
  ...
  if (!user || !ok) {
    ...
    logger.warn('HELIOS_AUTOLOGIN credentials did not resolve to a single valid user; falling back to login')
    return buildSafeRedirectResponse(req, '/login')
  }
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    GET: {
      ...
      description:
        'When HELIOS_AUTOLOGIN_EMAIL / HELIOS_AUTOLOGIN_PASSWORD are configured, signs the visitor in with those credentials and redirects into the app. Intended for single-tenant demo instances only. Falls back to the login page when disabled or misconfigured.',
      responses: [

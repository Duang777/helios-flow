...
import { Role, RoleAcl, User, UserRole } from '@helios/core/modules/auth/data/entities'
...
import { getDefaultEncryptionMaps, type Module } from '@helios/shared/modules/registry'
import { isEncryptionDebugEnabled, isTenantDataEncryptionEnabled } from '@helios/shared/lib/encryption/toggles'
import { EncryptionMap } from '@helios/core/modules/entities/data/entities'
...
const DERIVED_EMAIL_ENV = {
  admin: 'HELIOS_INIT_ADMIN_EMAIL',
  employee: 'HELIOS_INIT_EMPLOYEE_EMAIL',
} as const
 ...
 * seeded. Honors HELIOS_INIT_ADMIN_EMAIL / HELIOS_INIT_EMPLOYEE_EMAIL exactly the
...
export type SetupInitialTenantOptions = {
   ...
   * the HELIOS_INIT_ADMIN_PASSWORD / HELIOS_INIT_EMPLOYEE_PASSWORD env vars are unset.
   *
   ...
   *   (e.g. `helios init`) stay predictable for developers.
...
export async function setupInitialTenant(
  ...
  if (!existingUser) {
    ...
    if (includeDerivedUsers) {
      ...
      const envAdminPwd = readEnvValue('HELIOS_INIT_ADMIN_PASSWORD')
      const envEmployeePwd = readEnvValue('HELIOS_INIT_EMPLOYEE_PASSWORD')
      const isProduction = process.env.NODE_ENV === 'production'
      const allowDemo = options.allowDemoDerivedPasswords === true
      if (isProduction && !allowDemo) {
        const missing: string[] = []
        if (!envAdminPwd) missing.push('HELIOS_INIT_ADMIN_PASSWORD')
        if (!envEmployeePwd) missing.push('HELIOS_INIT_EMPLOYEE_PASSWORD')
        if (missing.length) {
 ...
 * Keeps the documented `yarn dev` / `helios init` DX predictable. Never used
...
function shouldKeepDemoSuperadminDuringInit(): boolean {
  if (process.env.HELIOS_INIT_FLOW !== 'true') return false
  if (!readEnvValue('HELIOS_INIT_SUPERADMIN_EMAIL')) return false
 ...
 * (superadmin, admin, employee — honoring HELIOS_INIT_*_EMAIL overrides)
...
function tryGetModules(): Module[] {
  try {
    const { getModules } = require('@helios/shared/lib/modules/registry')
    return getModules()
  } catch {
    return []
  }

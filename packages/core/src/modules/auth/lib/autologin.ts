 ...
 * `HELIOS_AUTOLOGIN_EMAIL` and `HELIOS_AUTOLOGIN_PASSWORD` are set, the homepage hands
 ...
 *
 * `HELIOS_AUTOLOGIN_TENANT` is optional and only needed when the same email exists
...
export function resolveAutoLoginCredentials(
  ...
  const email = readEnvValue(env, 'HELIOS_AUTOLOGIN_EMAIL')
  const password = env.HELIOS_AUTOLOGIN_PASSWORD ?? ''
  if (!email || !password) return null
  const tenantId = readEnvValue(env, 'HELIOS_AUTOLOGIN_TENANT')

...
jest.mock('@helios/shared/lib/encryption/toggles', () => ({
...
} from '@helios/core/modules/auth/lib/setup-app'
...
const mockLogger = jest.requireMock('@helios/shared/lib/logger').createLogger('test') as {
  debug: jest.Mock
  info: jest.Mock
  warn: jest.Mock
  error: jest.Mock
...
const ENV_KEYS = [
  ...
  'HELIOS_INIT_FLOW',
  'HELIOS_INIT_SUPERADMIN_EMAIL',
  'DEMO_MODE',
  'HELIOS_INIT_ADMIN_EMAIL',
  'HELIOS_INIT_EMPLOYEE_EMAIL',
]
...
describe('deactivateDemoUsersIfSelfOnboardingEnabled', () => {
  ...
  it('B — honors HELIOS_INIT_ADMIN_EMAIL / HELIOS_INIT_EMPLOYEE_EMAIL overrides instead of acme.com defaults', async () => {
    process.env.SELF_SERVICE_ONBOARDING_ENABLED = 'true'
    process.env.HELIOS_INIT_ADMIN_EMAIL = 'override-admin@example.com'
    process.env.HELIOS_INIT_EMPLOYEE_EMAIL = 'override-employee@example.com'
  ...
  it('D — keeps ALL demo users (not just superadmin) when shouldKeepDemoSuperadminDuringInit() fires', async () => {
    ...
    process.env.HELIOS_INIT_SUPERADMIN_EMAIL = 'customized-super@example.com'
...
describe('resolveDemoUserEmails', () => {
  ...
  it('applies HELIOS_INIT_ADMIN_EMAIL / HELIOS_INIT_EMPLOYEE_EMAIL overrides without affecting the superadmin entry', () => {
    process.env.HELIOS_INIT_ADMIN_EMAIL = 'custom-admin@x.test'
    process.env.HELIOS_INIT_EMPLOYEE_EMAIL = 'custom-employee@x.test'
    const result = resolveDemoUserEmails()
    expect(result).toEqual([
      { role: 'superadmin', email: 'superadmin@acme.com' },
  ...
  it('treats whitespace-only env values as unset (matches readEnvValue trim semantics)', () => {
    process.env.HELIOS_INIT_ADMIN_EMAIL = '   '
    process.env.HELIOS_INIT_EMPLOYEE_EMAIL = '\t\n'
    const result = resolveDemoUserEmails()

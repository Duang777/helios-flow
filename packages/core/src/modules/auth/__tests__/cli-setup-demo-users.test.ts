...
import { registerCliModules } from '@helios/shared/modules/registry'
import type { Module } from '@helios/shared/modules/registry'
import cli from '@helios/core/modules/auth/cli'
...
describe('helios auth setup --include-demo-users', () => {
  ...
  const ENV_KEYS = [
    'HELIOS_INIT_ADMIN_PASSWORD',
    'HELIOS_INIT_EMPLOYEE_PASSWORD',
  ...
  it('throws DerivedUserPasswordRequiredError in production when --include-demo-users is omitted but the env vars are unset and the caller forces includeDerivedUsers', async () => {
    ...
    const { setupInitialTenant, DerivedUserPasswordRequiredError } = await import('@helios/core/modules/auth/lib/setup-app')
    process.env.NODE_ENV = 'production'
    const em: any = {
      findOne,
      findOneOrFail,
  ...
  it('uses env-supplied derived passwords without random generation when provided', async () => {
    process.env.NODE_ENV = 'production'
    process.env.HELIOS_INIT_ADMIN_PASSWORD = 'Provided-Admin-1!'
    process.env.HELIOS_INIT_EMPLOYEE_PASSWORD = 'Provided-Emp-1!'
    await setupCommand.run([...BASE_ARGS, '--include-demo-users'])
    expect(process.exitCode).toBeUndefined()
    const seededEmails = persistedUsers.map((row) => row.email)
    expect(seededEmails).toContain('admin@acme.com')

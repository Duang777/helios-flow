...
} from '@helios/shared/lib/ai/opencode-provider'
import {
  AiModelFactoryError,
  createModelFactory,
  type AiModelFactory,
 ...
 * consulted first (honoring `HELIOS_AI_INBOX_OPS_MODEL` — legacy `INBOX_OPS_AI_MODEL` — + `input.modelOverride`),
...
export function resolveExtractionProviderId(): OpenCodeProviderId {
  // Honors HELIOS_AI_PROVIDER first, then the legacy OPENCODE_PROVIDER, then the
  ...
  // canonical HELIOS_AI_PROVIDER to a provider the native-only switch cannot serve
  ...
  const canonicalProvider = (process.env.HELIOS_AI_PROVIDER ?? '').trim()
  if (canonicalProvider.length > 0 && !isOpenCodeProviderId(canonicalProvider.toLowerCase())) {
    throw new Error(
      `[internal] HELIOS_AI_PROVIDER="${canonicalProvider}" is set but that provider is not configured ` +
        ...
        `openrouter) so the unified model factory can serve it, or switch HELIOS_AI_PROVIDER to a ` +
 ...
 * reaches `createModelFactory` without stubbing `@helios/ai-assistant`.
 */

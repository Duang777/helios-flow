...
jest.mock('@helios/core/modules/inbox_ops/events', () => ({
  emitInboxOpsEvent: (...args: unknown[]) => mockEmitInboxOpsEvent(...args),
}))

const mockCreateMessageRecordForEmail = jest.fn()
jest.mock('@helios/core/modules/inbox_ops/lib/messagesIntegration', () => ({
...
describe('extractionWorker', () => {
  ...
  describe('working language', () => {
    it('defaults workingLanguage to "en" when no InboxSettings found', async () => {
      ...
      const { buildExtractionSystemPrompt } = require('@helios/core/modules/inbox_ops/lib/extractionPrompt')

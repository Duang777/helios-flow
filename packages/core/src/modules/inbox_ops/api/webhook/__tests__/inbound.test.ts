...
jest.mock('@helios/core/modules/inbox_ops/lib/rateLimiter', () => ({
...
const mockSettings = {
  id: 'settings-1',
  inboxAddress: 'ops-123@inbox.helios.local',
  isActive: true,
  tenantId: 'tenant-1',
...
describe('POST /api/inbox_ops/webhook/inbound', () => {
  ...
  it('returns 429 when rate limiter rejects the request', async () => {
    const { checkRateLimit } = jest.requireMock('@helios/core/modules/inbox_ops/lib/rateLimiter') as {

...
import { POST } from '@helios/core/modules/inbox_ops/api/proposals/[id]/actions/[actionId]/accept/route'
import { InboxProposal, InboxProposalAction } from '@helios/core/modules/inbox_ops/data/entities'
import type { AuthContext } from '@helios/shared/lib/auth/server'
import { deserializeOperationMetadata } from '@helios/shared/lib/commands/operationMetadata'
...
describe('POST /api/inbox_ops/proposals/[id]/actions/[actionId]/accept', () => {
  ...
  it('emits operation metadata for undoable command-backed actions', async () => {
    ...
    const response = await POST(makeRequest())
    const metadata = deserializeOperationMetadata(response.headers.get('x-helios-operation'))
  ...
  it('returns 401 when not authenticated', async () => {
    const { getAuthFromRequest } = jest.requireMock('@helios/shared/lib/auth/server') as {

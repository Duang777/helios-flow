...
import { AiModelFactoryError } from '@helios/ai-assistant/modules/ai_assistant/lib/model-factory'
...
describe('inbox_ops llmProvider shim', () => {
  ...
  it('fails loudly (never silently openai) when a non-native HELIOS_AI_PROVIDER reaches the legacy fallback', async () => {
    const prev = process.env.HELIOS_AI_PROVIDER
    process.env.HELIOS_AI_PROVIDER = 'openrouter'
    const resolveModel = jest.fn(() => {
    ...
    try {
      ...
      ).rejects.toThrow(/HELIOS_AI_PROVIDER="openrouter"/)
    } finally {
      if (prev === undefined) delete process.env.HELIOS_AI_PROVIDER
      else process.env.HELIOS_AI_PROVIDER = prev
    }
  })
})

describe('resolveExtractionProviderId — loud failure vs BC fall-through', () => {
  const savedOm = process.env.HELIOS_AI_PROVIDER
  const savedOc = process.env.OPENCODE_PROVIDER

  const setEnv = (key: 'HELIOS_AI_PROVIDER' | 'OPENCODE_PROVIDER', value: string | undefined) => {
    if (value === undefined) delete process.env[key]
    else process.env[key] = value
  }

  afterEach(() => {
    setEnv('HELIOS_AI_PROVIDER', savedOm)
    setEnv('OPENCODE_PROVIDER', savedOc)
  })

  it('throws a descriptive error for a non-native canonical HELIOS_AI_PROVIDER', () => {
    setEnv('HELIOS_AI_PROVIDER', 'openrouter')
    setEnv('OPENCODE_PROVIDER', undefined)
    expect(() => llmProvider.resolveExtractionProviderId()).toThrow(/HELIOS_AI_PROVIDER="openrouter"/)
  })

  it('still honors a native canonical HELIOS_AI_PROVIDER', () => {
    setEnv('HELIOS_AI_PROVIDER', 'anthropic')
    setEnv('OPENCODE_PROVIDER', undefined)
  ...
  it('leaves the legacy OPENCODE_PROVIDER fall-through untouched (no throw)', () => {
    setEnv('HELIOS_AI_PROVIDER', undefined)

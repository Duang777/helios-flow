...
import type { SearchConfig } from '@helios/shared/lib/search/config'
...
describe('buildSearchTokenRows debug logging redaction (issue #2709)', () => {
  const previousDebug = process.env.HELIOS_SEARCH_DEBUG
  let debugSpy: jest.Mock

  beforeEach(() => {
    process.env.HELIOS_SEARCH_DEBUG = 'true'
  ...
  afterEach(() => {
    ...
    if (previousDebug === undefined) delete process.env.HELIOS_SEARCH_DEBUG
    else process.env.HELIOS_SEARCH_DEBUG = previousDebug
  })

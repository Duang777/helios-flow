...
describe('HybridQueryEngine', () => {
  ...
  describe('HELIOS_ENCRYPTED_SORT_MAX_ROWS cap', () => {
    const originalEnv = process.env.HELIOS_ENCRYPTED_SORT_MAX_ROWS

    afterEach(() => {
      if (originalEnv === undefined) delete process.env.HELIOS_ENCRYPTED_SORT_MAX_ROWS
      else process.env.HELIOS_ENCRYPTED_SORT_MAX_ROWS = originalEnv
    })
    ...
    test('unset: no limit on the phase-1 scan, no warning', async () => {
      delete process.env.HELIOS_ENCRYPTED_SORT_MAX_ROWS
    ...
    test('set but not exceeded: no warning, identical results to uncapped', async () => {
      process.env.HELIOS_ENCRYPTED_SORT_MAX_ROWS = '10'
    ...
    test('set and exceeded: caps + orders the phase-1 scan and attaches a warning', async () => {
      process.env.HELIOS_ENCRYPTED_SORT_MAX_ROWS = '3'
      const { db, engine } = buildFixture()
      const result = await engine.query('customers:customer_entity', {
        fields: ['id', 'display_name'],
        organizationId: 'org1',

...
describe('replaceSearchTokensForBatch — large batch delete (regression)', () => {
  const prevEnabled = process.env.HELIOS_SEARCH_ENABLED
  const prevPartials = process.env.HELIOS_SEARCH_ENABLE_PARTIAL

  beforeAll(() => {
    process.env.HELIOS_SEARCH_ENABLED = 'true'
    ...
    process.env.HELIOS_SEARCH_ENABLE_PARTIAL = 'false'
  })

  afterAll(() => {
    if (prevEnabled === undefined) delete process.env.HELIOS_SEARCH_ENABLED
    else process.env.HELIOS_SEARCH_ENABLED = prevEnabled
    if (prevPartials === undefined) delete process.env.HELIOS_SEARCH_ENABLE_PARTIAL
    else process.env.HELIOS_SEARCH_ENABLE_PARTIAL = prevPartials
  })

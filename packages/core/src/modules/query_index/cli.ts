...
import { resolveTenantEncryptionService } from '@helios/shared/lib/encryption/customFieldValues'
import { decryptIndexDocForSearch, encryptIndexDocForStorage } from '@helios/shared/lib/encryption/indexDoc'
...
import { resolveEntityTableName } from '@helios/shared/lib/query/engine'
...
import { flattenSystemEntityIds } from '@helios/shared/lib/entities/system-entities'
...
const rebuild: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!entity) {
      console.error(
        'Usage: helios query_index rebuild --entity <module:entity> [--record <id>] [--org <id>] [--tenant <id>] [--global] [--withDeleted] [--limit <n>] [--offset <n>]',
      )
      return
    }
...
const rebuildAll: ModuleCli = {
  ...
  async run(rest) {
    ...
    const recordId = stringOption(args, 'record', 'recordId', 'id')
    if (recordId) {
      console.error('`rebuild-all` does not support --record. Use `helios query_index rebuild --record <id>` instead.')
      return
    }
    ...
    try {
      ...
      const { getEntityIds } = await import('@helios/shared/lib/encryption/entityIds')

...
/**
 * Scheduler tick payload. Fired by the `@helios/scheduler` cron entry
...
export default async function handle(
  ...
  // The scheduler module (`@helios/scheduler`) spreads the configured
  ...
  //       `lastPolledAt` is older than HELIOS_CHANNEL_AUTO_RECOVER_MINUTES
  ...
  const recoverMinutesRaw = Number.parseInt(
    process.env.HELIOS_CHANNEL_AUTO_RECOVER_MINUTES ?? '',
    10,
  )

...
import type { QueuedJob } from '@helios/queue'
...
describe('poll-tick worker enumeration', () => {
  ...
  // attempt (`lastPolledAt`) is older than `HELIOS_CHANNEL_AUTO_RECOVER_MINUTES`
  ...
  // Regression: HELIOS_CHANNEL_AUTO_RECOVER_MINUTES=0 means "recover on the next
  // tick" (used by TC-CHANNEL-EMAIL-027). The old `> 0` guard coerced 0 → 30 min.
  it('recovers immediately when HELIOS_CHANNEL_AUTO_RECOVER_MINUTES=0', async () => {
    const previous = process.env.HELIOS_CHANNEL_AUTO_RECOVER_MINUTES
    process.env.HELIOS_CHANNEL_AUTO_RECOVER_MINUTES = '0'
    try {
      const errorChannels = [
        {
          id: 'c-err-recent',
      ...
      if (previous === undefined) delete process.env.HELIOS_CHANNEL_AUTO_RECOVER_MINUTES
      else process.env.HELIOS_CHANNEL_AUTO_RECOVER_MINUTES = previous
    }
  })

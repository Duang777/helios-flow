...
describe('thread-token', () => {
  beforeEach(() => {
    process.env.HELIOS_THREAD_TOKEN_SECRET = TEST_SECRET
    delete process.env.KMS_MASTER_KEY
    _resetThreadTokenKeyCache()
  })

  afterEach(() => {
    delete process.env.HELIOS_THREAD_TOKEN_SECRET
    delete process.env.KMS_MASTER_KEY
    _resetThreadTokenKeyCache()
  })
  ...
  describe('generateToken / verifyToken', () => {
    ...
    it('verifyToken rejects tokens signed with a different key', () => {
      const token = generateToken()
      process.env.HELIOS_THREAD_TOKEN_SECRET = 'a-different-secret'
      _resetThreadTokenKeyCache()
      expect(verifyToken(token)).toBe(false)
    })

    it('falls back to KMS_MASTER_KEY when HELIOS_THREAD_TOKEN_SECRET is unset', () => {
      delete process.env.HELIOS_THREAD_TOKEN_SECRET
  ...
  describe('buildReferencesId / buildBodyFooter', () => {
    it('buildReferencesId returns `<om_TOKEN@helios.invalid>` (RFC 6761 .invalid TLD)', () => {
      const token = generateToken()
      const id = buildReferencesId(token)
      expect(id).toBe(`<${token}@helios.invalid>`)
    })
  ...
  describe('applyOutboundThreadingToken', () => {
    it('appends the synthetic id to a brand-new References header', () => {
      ...
      expect(result.headers!.References).toBe(`<${token}@helios.invalid>`)
    })
    ...
    it('extends an existing References header without duplicating', () => {
      ...
      expect(out1.headers!.References).toBe(`${existing} <${token}@helios.invalid>`)

      const out2 = applyOutboundThreadingToken(out1, token)
      expect(out2.headers!.References).toBe(`${existing} <${token}@helios.invalid>`)
    })
    ...
    it('normalises lowercase `references` to canonical `References`', () => {
      ...
      expect(result.headers!.References).toBe(`<existing@example.com> <${token}@helios.invalid>`)
  ...
  describe('extractTokenFromHeaders', () => {
    it('finds a token in a single References string', () => {
      const token = generateToken()
      const refs = `<root@example.com> <${token}@helios.invalid> <parent@example.com>`
      expect(extractTokenFromHeaders(null, refs)).toBe(token)
    })

    it('finds a token in a References array', () => {
      const token = generateToken()
      const refs = ['<root@example.com>', `<${token}@helios.invalid>`]
      expect(extractTokenFromHeaders(null, refs)).toBe(token)
    })

    it('finds a token in In-Reply-To when References is empty', () => {
      const token = generateToken()
      expect(extractTokenFromHeaders(`<${token}@helios.invalid>`, null)).toBe(token)
    })
    ...
    it('returns null when the token format is right but HMAC is wrong (forgery defense)', () => {
      ...
      expect(extractTokenFromHeaders(null, `<${fake}@helios.invalid>`)).toBeNull()
    })
  })

...
beforeAll(() => {
  process.env.HELIOS_HUB_OAUTH_STATE_KEY = SECRET
})

afterAll(() => {
  delete process.env.HELIOS_HUB_OAUTH_STATE_KEY
})
...
describe('createOAuthState', () => {
  ...
  it('refuses to encrypt when no secret is configured', () => {
    const previous = process.env.HELIOS_HUB_OAUTH_STATE_KEY
    ...
    try {
      ...
      if (previous !== undefined) process.env.HELIOS_HUB_OAUTH_STATE_KEY = previous
...
describe('JWT_SECRET fallback key separation', () => {
  const saved = {
    stateKey: process.env.HELIOS_HUB_OAUTH_STATE_KEY,
  ...
  afterEach(() => {
    if (saved.stateKey !== undefined) process.env.HELIOS_HUB_OAUTH_STATE_KEY = saved.stateKey
    else delete process.env.HELIOS_HUB_OAUTH_STATE_KEY
  ...
  it('refuses the JWT_SECRET fallback in production', () => {
    ...
    expect(() => createOAuthState({ userId: 'u', tenantId: 't', providerKey: 'gmail' })).toThrow(
      'HELIOS_HUB_OAUTH_STATE_KEY or KMS_MASTER_KEY required in production',
    )
    expect(() =>
      encryptOAuthState({
        state: 's',
    ...
    ).toThrow('HELIOS_HUB_OAUTH_STATE_KEY or KMS_MASTER_KEY required in production')
  })

 ...
 * inbound replies to the originating Helios message thread, even
...
const RANDHELIOS_BYTES = 16
const HMAC_BYTES = 8
const HMAC_KEY_ENV = 'HELIOS_THREAD_TOKEN_SECRET'
...
function getKey(): Buffer {
  ...
  cachedKey = createHash('sha256').update('helios-thread-token-dev').digest()
  return cachedKey
}
...
export function generateToken(): string {
  const random = randomBytes(RANDHELIOS_BYTES)
...
const RANDHELIOS_B64_LEN = Math.ceil((RANDHELIOS_BYTES * 4) / 3)
const HMAC_B64_LEN = Math.ceil((HMAC_BYTES * 4) / 3)
const TOKEN_TOTAL_LEN = TOKEN_PREFIX.length + RANDHELIOS_B64_LEN + 1 + HMAC_B64_LEN
...
export function verifyToken(token: string): boolean {
  ...
  const randomEnd = randomStart + RANDHELIOS_B64_LEN
  ...
  if (!random || random.length !== RANDHELIOS_BYTES) return false

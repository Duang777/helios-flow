 ...
 * Root `AGENTS.md` rule: `@helios/core` MUST NOT import from `@helios/enterprise`.
 *
 * Design (per email integration spec § OSS Independence + § Hub Deltas → Delta 7):
 *   - AES-256-GCM payload encryption.
 *   - HKDF (SHA-256) key derivation from `HELIOS_HUB_OAUTH_STATE_KEY` (falling back to
...
const HKDF_SALT = Buffer.from('helios-channels-oauth-state-v1')
...
export function isSafeOAuthReturnUrl(value: string | null | undefined): value is string {
  ...
  try {
    const base = new URL('https://helios.local')
...
function getSecret(): string {
  const dedicated = process.env.HELIOS_HUB_OAUTH_STATE_KEY ?? process.env.KMS_MASTER_KEY
  ...
  if (process.env.NODE_ENV === 'production') {
    throw new Error('[internal] HELIOS_HUB_OAUTH_STATE_KEY or KMS_MASTER_KEY required in production')
  }
  const fallback = process.env.JWT_SECRET
  if (!fallback) {
    throw new OAuthStateError(
      'HELIOS_HUB_OAUTH_STATE_KEY (or fallback KMS_MASTER_KEY / JWT_SECRET) must be set',
      'missing_secret',
    )
  }
  return fallback

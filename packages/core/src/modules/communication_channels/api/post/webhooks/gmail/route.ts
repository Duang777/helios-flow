 ...
 *
 * Validation pipeline:
 *   1. Verify JWT signature, audience (`HELIOS_GMAIL_PUBSUB_AUDIENCE`), and
 *      email claim (`HELIOS_GMAIL_PUBSUB_SERVICE_ACCOUNT_EMAIL`).
...
export async function POST(req: Request): Promise<Response> {
  const expectedAudience = process.env.HELIOS_GMAIL_PUBSUB_AUDIENCE
  const expectedEmail = process.env.HELIOS_GMAIL_PUBSUB_SERVICE_ACCOUNT_EMAIL
  if (!expectedAudience || !expectedEmail) {
    ...
    logger.error('HELIOS_GMAIL_PUBSUB_AUDIENCE / HELIOS_GMAIL_PUBSUB_SERVICE_ACCOUNT_EMAIL not set')

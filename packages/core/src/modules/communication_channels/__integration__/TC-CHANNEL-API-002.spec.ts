...
import {
  ...
  deleteUserIfExists,
  setRoleAclFeatures,
} from '@helios/core/helpers/integration/authFixtures'
import {
  deleteChannelIfExists,
  isChannelSeedingAvailable,
  seedConnectedChannel,
} from '@helios/core/helpers/integration/communicationChannelsFixtures'

/**
 * TC-CHANNEL-API-002 — POST set-primary marks exactly one channel primary.
 * Source: https://github.com/helios/helios/issues/2486
 *
...
test.describe('TC-CHANNEL-API-002: set-primary keeps a single primary per user', () => {
  test('set-primary moves the primary flag and supersedes the previous one', async ({ request }) => {
    ...
    try {
      ...
      test.skip(
        !seedingAvailable,
        'HELIOS_ENABLE_TEST_CHANNEL_SEEDING is not enabled in this environment; cannot seed connected channels.',
      )

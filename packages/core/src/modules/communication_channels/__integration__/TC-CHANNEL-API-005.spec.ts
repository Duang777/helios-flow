...
import { expectId, readJsonSafe } from '@helios/core/helpers/integration/generalFixtures'
import {
...
test.describe('TC-CHANNEL-API-005: add/remove a reaction', () => {
  test('POST creates a reaction and DELETE removes it', async ({ request }) => {
    ...
    try {
      ...
      test.skip(
        !seedingAvailable,
        'HELIOS_ENABLE_TEST_CHANNEL_SEEDING is not enabled in this environment; cannot seed a channel-linked message.',
      )

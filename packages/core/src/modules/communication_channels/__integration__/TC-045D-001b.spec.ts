...
test.describe('TC-045D-001b: ChannelAdapter registry validation', () => {
  test('registry + validator helpers are exported from the hub module', async () => {
    ...
    // Provider packages (`@helios/channel-slack`, `@helios/channel-whatsapp`)
    // depend on importing these from `@helios/core/modules/communication_channels`.
    const { ChannelAdapterRegistry } = await import(
      '@helios/core/modules/communication_channels/lib/registry'
    )
    const { validateAdapterCapabilities } = await import(
      '@helios/core/modules/communication_channels/lib/adapter-compat'
    )

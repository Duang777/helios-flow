...
const POLL_TICK_INTERVAL_SECONDS = Math.max(
  10,
  Number.parseInt(process.env.HELIOS_HUB_POLL_SCHEDULER_TICK_SECONDS ?? '60', 10) || 60,
)
...
export const setup: ModuleSetupConfig = {
  ...
  async seedDefaults({ container, organizationId, tenantId }) {
    /**
     * Register the per-channel polling tick with `@helios/scheduler`.
     *
    ...
    try {
      ...
      await schedulerService.register({
        ...
        description:
          'Daily 04:00 UTC. Re-issues gmail.users.watch for channels within HELIOS_PUSH_RENEWAL_GMAIL_LEAD_HOURS of expiry.',
        scopeType: 'organization',
        organizationId,
        tenantId,
        scheduleType: 'cron',

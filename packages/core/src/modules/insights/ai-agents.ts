...
const operatingLoopAgent: AiAgentDefinition = {
  ...
  systemPrompt: [
    ...
    '',
    'RESPONSE STYLE',
    'Every business answer must include the number, the formula source or source table, and at least one href when tool results provide links. Prefer hrefs that start with `/backend/`. Always include a bullet that starts with `证据:` or `Evidence:` and lists the relevant finding.id, evidence IDs, invoice ids, project ids, contract ids, or hrefs from tool results. For findings, cite finding.id and evidence IDs. Use concise bullet points for multi-step answers and label assumptions explicitly.',
  ].join('\n'),

...
describe('insights.operating_loop_assistant agent definition', () => {
  ...
  it('keeps the fixed operating prompt regression set covered by concrete tools', () => {
    const regressionCases = [
      ...
      {
        prompt: '当前库存余额怎样？如需收货或调整必须走确认卡。',
        tools: ['wms.list_balances', 'wms.receive_inventory', 'wms.adjust_inventory', 'wms.move_inventory'],
      },
      {
        prompt: '有哪些待办工作流任务？认领、完成、启动或取消必须走确认卡。',
        tools: [
      ...
      {
        prompt: '列出请假申请，批准或驳回必须走确认卡。',
        tools: ['staff.list_leave_requests', 'staff.accept_leave_request', 'staff.reject_leave_request'],
      },
      {
      ...
      {
        prompt: '列出站内消息；发送或回复必须走确认卡，不要发邮件。',
        tools: ['messages.list_messages', 'messages.send_message', 'messages.reply_to_message'],
      },
      {

...
import { verifyAudienceJwt, verifyJwt } from '@helios/shared/lib/auth/jwt'
...
import { CUSTOMER_JWT_AUDIENCE } from '@helios/core/modules/customer_accounts/services/customerSessionService'
...
async function assertSessionStillActive(sessionId: string): Promise<boolean> {
  try {
    const [{ createRequestContainer }, { CustomerSessionService }] = await Promise.all([
      import('@helios/shared/lib/di/container'),
      import('@helios/core/modules/customer_accounts/services/customerSessionService'),
    ])
...
export async function validateUserState(
  ...
  const [{ createRequestContainer }, { CustomerUser }] = await Promise.all([
    import('@helios/shared/lib/di/container'),
    import('@helios/core/modules/customer_accounts/data/entities'),
  ])
  const container = await createRequestContainer()
  ...
  const { CustomerRbacService } = await import(
    '@helios/core/modules/customer_accounts/services/customerRbacService'
  )

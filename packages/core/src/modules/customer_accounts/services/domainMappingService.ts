...
import { normalizeHostname, tryNormalizeHostname } from '@helios/core/modules/customer_accounts/lib/hostname'
import { platformDomains } from '@helios/core/modules/customer_accounts/lib/platformDomains'
import { detectProxy, isInKnownProxyRange } from '@helios/core/modules/customer_accounts/lib/proxyRanges'
...
const defaultHealthCheck: HealthCheckContract = (hostname, timeoutMs) =>
  new Promise((resolve) => {
    const headerName = (process.env.CUSTOMER_DOMAIN_ORIGIN_HEADER ?? 'X-Helios-Origin').toLowerCase()
...
export class DomainMappingService {
  ...
  async verify(id: string): Promise<VerifyResult> {
    ...
    const expectedCname = process.env.CUSTHELIOS_DOMAIN_CNAME_TARGET ?? ''
    const expectedARecord = process.env.CUSTHELIOS_DOMAIN_A_RECORD_TARGET ?? null
    const now = new Date()

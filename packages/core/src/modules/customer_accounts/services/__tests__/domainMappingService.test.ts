...
import { DomainMappingService } from '@helios/core/modules/customer_accounts/services/domainMappingService'
import {
  DomainMapping,
  type DomainStatus,
} from '@helios/core/modules/customer_accounts/data/entities'
...
jest.mock('@helios/core/modules/customer_accounts/events', () => {
  const actual = jest.requireActual<typeof import('@helios/core/modules/customer_accounts/events')>(
    '@helios/core/modules/customer_accounts/events',
  )
  return {
    ...actual,
...
beforeEach(() => {
  process.env.PLATFORM_DOMAINS = 'localhost,helios.com'
  delete process.env.CUSTHELIOS_DOMAIN_CNAME_TARGET
  delete process.env.CUSTHELIOS_DOMAIN_A_RECORD_TARGET
})
...
describe('DomainMappingService.verify', () => {
  it('transitions pending → verified when CNAME matches the expected target', async () => {
    process.env.CUSTHELIOS_DOMAIN_CNAME_TARGET = 'edge.helios.com'
    const row = seedDomain({ hostname: 'shop.example.com', status: 'pending' })
    const em = createFakeEm({ domains: [row] })
    const dnsResolver = {
      resolveCname: jest.fn(async () => ['edge.helios.com']),
  ...
  it('transitions to dns_failed when CNAME points elsewhere', async () => {
    ...
    expect(result.diagnostics?.suggestion).toContain('edge.helios.com')
  })
...
describe('DomainMappingService.healthCheck', () => {
  ...
  it('defaultHealthCheck path uses underscore route (C1 regression guard)', async () => {
    const mod = await import('@helios/core/modules/customer_accounts/services/domainMappingService')
...
describe('DomainMappingService.isAllowedForTls', () => {
  ...
  it('returns null for platform domains (never gates platform TLS)', async () => {
    ...
    expect(await service.isAllowedForTls('helios.com')).toBeNull()
  })
...
describe('DomainMappingService.resolveByHostname', () => {
  ...
  it('returns null for platform domains', async () => {
    const em = createFakeEm()
    const service = new DomainMappingService(em as unknown as EntityManager)
    expect(await service.resolveByHostname('helios.com')).toBeNull()
  })

...
import { DashboardRoleWidgets } from '@helios/core/modules/dashboards/data/entities'
import { Role } from '@helios/core/modules/auth/data/entities'
import { loadAllWidgets } from '@helios/core/modules/dashboards/lib/widgets'
...
const seedDefaults: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!tenantId) {
      console.error('Usage: helios dashboards seed-defaults --tenant <tenantId> [--roles superadmin,admin,employee] [--widgets id1,id2]')
      return
    }
...
const enableAnalyticsWidgets: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!tenantId) {
      console.error('Usage: helios dashboards enable-analytics-widgets --tenant <tenantId> [--org <orgId>] [--roles admin,employee]')
      return
    }
...
const seedAnalytics: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!tenantId || !organizationId) {
      console.error('Usage: helios dashboards seed-analytics --tenant <tenantId> --organization <organizationId> [--months 6] [--ordersPerMonth 50]')
      return
    }
...
const debugAnalytics: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!tenantId) {
      console.error('Usage: helios dashboards debug-analytics --tenant <tenantId> [--organization <organizationId>]')
      return
    }

...
describe('withOperationMetadata', () => {
  it('sets x-helios-operation header when the log entry is complete', () => {
    ...
    const raw = res.headers.get('x-helios-operation')
  ...
  it('falls back to the provided fallback resource when the log entry omits it', () => {
    ...
    const parsed = deserializeOperationMetadata(res.headers.get('x-helios-operation'))
  ...
  it('is a no-op when logEntry lacks undoToken', () => {
    ...
    expect(res.headers.get('x-helios-operation')).toBeNull()
  })

...
import { ensureOrganizationScope, ensureSameScope } from '@helios/shared/lib/commands/scope'
...
export { ensureOrganizationScope, ensureSameScope, ensureTenantScope } from '@helios/shared/lib/commands/scope'
export { extractUndoPayload } from '@helios/shared/lib/commands/undo'
...
export { assertFound } from '@helios/shared/lib/crud/errors'
...
const CUSTHELIOS_DICTIONARY_KIND_PATTERN = /^[a-z0-9]+(?:[-_][a-z0-9]+)*$/
...
export async function ensureDictionaryEntry(
  ...
  if (!trimmed) return null
  if (!DICTIONARY_KINDS.has(params.kind) && !CUSTHELIOS_DICTIONARY_KIND_PATTERN.test(params.kind)) {

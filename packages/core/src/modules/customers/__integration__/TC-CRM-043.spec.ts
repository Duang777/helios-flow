 ...
 *
 * Verifies that every interactions write path emits the `x-helios-operation`
 ...
 *
 * All three must return an `x-helios-operation` header whose undoToken is accepted
 * by `POST /api/audit_logs/audit-logs/actions/undo`.
 */
test.describe('TC-CRM-043: Interaction write paths emit x-helios-operation + support undo', () => {
  test('create, complete, and cancel all expose undo tokens that the undo endpoint accepts', async ({
    ...
    function extractUndoMetadata(headers: Record<string, string>): {
      ...
      const raw = headers['x-helios-operation'] ?? null;
    ...
    async function performAndVerifyUndo(
      ...
      expect(
        meta.undoToken,
        `${path} must emit an undo token via x-helios-operation header`,
      ).toBeTruthy();

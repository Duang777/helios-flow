 ...
 *   4. Undo round-trip: the `x-helios-operation` header carries an undoToken; undo of a
...
function parseUndoToken(res: APIResponse): string {
  ...
  expect(header.startsWith('omop:'), `x-helios-operation header missing/invalid: "${header}"`).toBeTruthy()
  const decoded = JSON.parse(decodeURIComponent(header.slice('omop:'.length))) as { undoToken?: string }
  expect(typeof decoded.undoToken, 'x-helios-operation must carry an undoToken').toBe('string')

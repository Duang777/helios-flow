 ...
 * - Marker: Leaflet divIcon `.helios-deal-map-marker` wrapping `span[data-deal-id="<dealId>"]`
...
test.describe('TC-CRM-085: deals map view tab (UI)', () => {
  ...
  test('shows a located fixture deal in the panel with a map marker', async ({ page, request }) => {
    ...
    try {
      ...
      const marker = page.locator(`[data-map-canvas] .helios-deal-map-marker span[data-deal-id="${dealId}"]`);

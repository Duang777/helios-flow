...
import {
  getTokenScope,
  readJsonSafe,
} from '@helios/core/modules/core/__integration__/helpers/generalFixtures';
import {
  createRoleFixture,
  createUserFixture,
  deleteRoleIfExists,
  deleteUserIfExists,
} from '@helios/core/modules/core/__integration__/helpers/authFixtures';
import {
  createPersonFixture,
  deleteEntityIfExists,
} from '@helios/core/modules/core/__integration__/helpers/crmFixtures';
import {
  deleteChannelIfExists,
  isChannelSeedingAvailable,
  seedConnectedChannel,
} from '@helios/core/modules/core/__integration__/helpers/communicationChannelsFixtures';
import { drainIntegrationQueue } from '@helios/core/helpers/integration/queue';
 ...
 * outbound chain (env-gated `HELIOS_ENABLE_TEST_CHANNEL_SEEDING` channel fixture).
...
const APP_ROOT = process.env.HELIOS_TEST_APP_ROOT?.trim()
  ? path.resolve(process.env.HELIOS_TEST_APP_ROOT as string)
  : path.resolve(process.cwd(), 'apps/helios')

if (!process.env.HELIOS_TEST_APP_ROOT?.trim()) {
  loadEnv({ path: path.resolve(APP_ROOT, '.env') })
  process.env.QUEUE_BASE_DIR = path.resolve(APP_ROOT, '.helios/queue')
}
  ...
  test(
    ...
    async ({ request }) => {
      ...
      try {
        ...
        test.skip(
          !seedingAvailable,
          'HELIOS_ENABLE_TEST_CHANNEL_SEEDING is not enabled in this environment; cannot seed email threads.',
        );

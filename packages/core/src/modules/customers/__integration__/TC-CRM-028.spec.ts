...
import '@helios/core/modules/customers/commands/index';
import type { BootstrapData } from '@helios/shared/lib/bootstrap';
import { bootstrapFromAppRoot } from '@helios/shared/lib/bootstrap/dynamicLoader';
...
const TEST_APP_ROOT = process.env.HELIOS_TEST_APP_ROOT?.trim();
...
const APP_QUEUE_BASE_DIR = path.resolve(APP_ROOT, '.helios/queue');
...
test.describe('TC-CRM-028: Example customer sync (standalone smoke)', () => {
  test.beforeAll(async () => {
    test.skip(!IS_STANDALONE_APP, 'Standalone smoke coverage runs only when HELIOS_TEST_APP_ROOT is set');
  });

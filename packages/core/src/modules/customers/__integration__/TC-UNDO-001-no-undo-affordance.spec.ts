...
} from '@helios/core/helpers/integration/undoHarness'

/**
 * TC-UNDO-001 §4 — no undo affordance for non-undoable commands (#2572).
 *
 * A command that does not support undo must carry NO `x-helios-operation` token on its mutating
 ...
 * surface its `x-helios-operation` token via `withOperationMetadata` — i.e. they are

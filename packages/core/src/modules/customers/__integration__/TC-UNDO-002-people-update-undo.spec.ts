...
import { skipIfUndoTestsDisabled } from '@helios/core/helpers/integration/undoHarness'
...
function expectOperation(response: APIResponse, context: string): Operation {
  ...
  expect(header, `Expected an undo token (x-helios-operation header) for ${context}, got none`).toBeTruthy()

...
import { expectId, readJsonSafe } from '@helios/core/modules/core/__integration__/helpers/generalFixtures'
...
export const INBOUND_APP_ROOT = process.env.HELIOS_TEST_APP_ROOT?.trim()
...
if (!process.env.HELIOS_TEST_APP_ROOT?.trim()) {
  ...
  process.env.QUEUE_BASE_DIR = path.resolve(INBOUND_APP_ROOT, '.helios/queue')
}

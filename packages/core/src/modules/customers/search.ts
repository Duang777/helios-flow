import type { QueryCustomFieldSource, QueryEngine } from '@helios/shared/lib/query/types'
import type {
  SearchModuleConfig,
  SearchBuildContext,
...
const CUSTOMER_CUSTHELIOS_FIELD_SOURCES: QueryCustomFieldSource[] = [
  {
...
async function loadCustomerEntityBundle(ctx: SearchContext, opts: CustomerEntityQueryOptions): Promise<LoadedCustomerEntity | null> {
  ...
  try {
    const result = await ctx.queryEngine.query('customers:customer_entity', {
      ...
      ...(shouldJoinProfileSource ? { customFieldSources: CUSTOMER_CUSTHELIOS_FIELD_SOURCES } : {}),

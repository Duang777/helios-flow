...
} from '@helios/shared/lib/crud/optimistic-lock-store'
import {
  OPTIMISTIC_LOCK_CONFLICT_CODE,
  OPTIMISTIC_LOCK_HEADER_NAME,
} from '@helios/shared/lib/crud/optimistic-lock-headers'
...
import type { MutationGuardInput } from '@helios/shared/lib/crud/mutation-guard-registry'
...
describe('isAuthorizedRecordLockOverride', () => {
  it('is true for keep-mine resolutions (case/whitespace tolerant)', () => {
    expect(isAuthorizedRecordLockOverride(new Headers({ 'x-helios-record-lock-resolution': 'accept_mine' }))).toBe(true)
    expect(isAuthorizedRecordLockOverride(new Headers({ 'x-helios-record-lock-resolution': 'merged' }))).toBe(true)
    expect(isAuthorizedRecordLockOverride(new Headers({ 'x-helios-record-lock-resolution': '  ACCEPT_MINE ' }))).toBe(true)
  })

  it('is false for accept-incoming, absent, or unknown resolutions', () => {
    expect(isAuthorizedRecordLockOverride(new Headers({ 'x-helios-record-lock-resolution': 'accept_incoming' }))).toBe(false)
    expect(isAuthorizedRecordLockOverride(new Headers({ 'x-helios-record-lock-resolution': 'normal' }))).toBe(false)
...
describe('customers.optimistic-lock guard — record-lock keep-mine deferral (#3601)', () => {
  const previousEnv = process.env.HELIOS_OPTIMISTIC_LOCK

  beforeEach(() => {
    clearOptimisticLockReadersForTests()
    resolveMock.mockClear()
    resolveMock.mockReturnValue({})
    process.env.HELIOS_OPTIMISTIC_LOCK = 'all'
    registerOptimisticLockReaders({
  ...
  afterAll(() => {
    clearOptimisticLockReadersForTests()
    if (previousEnv === undefined) delete process.env.HELIOS_OPTIMISTIC_LOCK
    else process.env.HELIOS_OPTIMISTIC_LOCK = previousEnv
  })
  ...
  it('defers (passes) when the request carries an authorized accept_mine override', async () => {
    const result = await optimisticLockGuard.validate(
      buildInput({
        ...
        'x-helios-record-lock-resolution': 'accept_mine',
      }),
    )
  ...
  it('defers (passes) for a merged override too', async () => {
    const result = await optimisticLockGuard.validate(
      buildInput({
        [OPTIMISTIC_LOCK_HEADER_NAME]: STALE_UPDATED_AT,
        'x-helios-record-lock-resolution': 'merged',
      }),
    )
    expect(result.ok).toBe(true)
  })
  ...
  it('still enforces the floor for accept_incoming (which reloads with a fresh timestamp)', async () => {
    const result = await optimisticLockGuard.validate(
      buildInput({
        [OPTIMISTIC_LOCK_HEADER_NAME]: STALE_UPDATED_AT,
        'x-helios-record-lock-resolution': 'accept_incoming',
      }),
    )
    expect(result.ok).toBe(false)
    expect(result.status).toBe(409)

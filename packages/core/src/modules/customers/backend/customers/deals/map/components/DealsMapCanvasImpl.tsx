...
// Warn once (per session) so deployments point NEXT_PUBLIC_HELIOS_DEALS_MAP_TILE_URL at a self-hosted or
...
function warnOnPublicOsmTilesOnce(): void {
  ...
  logger.warn(
    'Deals map is using the public OpenStreetMap tile server. ' +
      ...
      'NEXT_PUBLIC_HELIOS_DEALS_MAP_TILE_URL at a self-hosted or commercial tile service before deploying.',
  )
}
...
function buildMarkerIcon(dealId: string, tone: FilterOptionTone | null, selected: boolean): L.DivIcon {
  ...
  return L.divIcon({
    className: `helios-deal-map-marker${selected ? ' helios-deal-map-marker--selected' : ''}`,
...
function buildClusterIcon(count: number): L.DivIcon {
  return L.divIcon({
    className: 'helios-deal-map-cluster',

...
import { parseBooleanFromUnknown, parseBooleanToken } from '@helios/shared/lib/boolean'
import { isOrganizationReadAccessAllowed } from '@helios/core/modules/directory/utils/organizationScopeGuard'
...
import { buildCollectionTags, canonicalizeResourceTag, isCrudCacheEnabled, resolveCrudCache } from '@helios/shared/lib/crud/cache'
...
function isRouteProfilingEnabled(scope: string): boolean {
  const candidates = [
    process.env.HELIOS_CRUD_PROFILE,
    process.env.NEXT_PUBLIC_HELIOS_CRUD_PROFILE,
    process.env.HELIOS_ROUTE_PROFILE,
    process.env.NEXT_PUBLIC_HELIOS_ROUTE_PROFILE,
  ]

...
import { parseBooleanFromUnknown } from '@helios/shared/lib/boolean'
import {
...
import { consumeAdvancedFilterState, mergeAdvancedFilterTree } from '@helios/shared/lib/crud/advanced-filter-integration'
...
export async function buildDealListFilters(query: DealListQuery, ctx?: import('@helios/shared/lib/crud/factory').CrudCtx) {
  ...
  if ((selectedPersonIds.length > 0 || selectedCompanyIds.length > 0) && ctx) {
    const tenantId = ctx.auth?.tenantId
    // `ctx.auth` exposes `orgId` (see AuthContext in @helios/shared/lib/auth/server).

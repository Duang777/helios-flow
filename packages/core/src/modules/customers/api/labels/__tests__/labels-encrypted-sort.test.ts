...
describe('GET /api/customers/labels — bounded candidate scan + decrypt + in-memory sort (#3386)', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    process.env = { ...originalEnv, HELIOS_ENCRYPTED_SORT_MAX_ROWS: undefined }
  ...
  test('passes a limit+id-order clause to em.find when HELIOS_ENCRYPTED_SORT_MAX_ROWS is set', async () => {
    process.env.HELIOS_ENCRYPTED_SORT_MAX_ROWS = '5'
    mockEmFind.mockResolvedValueOnce([])
  ...
  test('does not cap em.find when HELIOS_ENCRYPTED_SORT_MAX_ROWS is unset', async () => {
    mockEmFind.mockResolvedValueOnce([])

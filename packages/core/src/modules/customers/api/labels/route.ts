...
import { decryptEntitiesWithFallbackScope } from '@helios/shared/lib/encryption/subscriber'
import { mapWithConcurrency } from '@helios/shared/lib/query/bounded-decrypt'
import { resolveEncryptedSortMaxRows, sortRowsInMemory } from '@helios/shared/lib/query/encrypted-sort'
...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (cap !== null && candidateLabels.length >= cap) {
      logger.warn('Encrypted sort candidate scan hit HELIOS_ENCRYPTED_SORT_MAX_ROWS cap; results may be incomplete', {

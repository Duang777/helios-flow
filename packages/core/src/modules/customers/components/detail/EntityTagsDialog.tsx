...
import { cn, slugifyTagLabel } from '@helios/shared/lib/utils'
...
const CUSTHELIOS_CATEGORY_FIELD_PREFIX = 'crmTagCategory:'
...
function resolveCustomCategoryFieldKey(kind: string): string {
  return `${CUSTHELIOS_CATEGORY_FIELD_PREFIX}${kind}`
}
      ...
      const loadKindSettings = async (): Promise<KindSetting[]> => {
        try {
          const settingsCall = await apiCall<{ items?: KindSetting[] }>(
            ...
            { cache: 'no-store', headers: { 'x-helios-unauthorized-redirect': '0' } },
          )
      ...
      const loadLabelData = async (): Promise<{
        ...
        try {
          ...
          const labelsCall = await apiCall<{
            ...
            cache: 'no-store',
            headers: { 'x-helios-unauthorized-redirect': '0' },
          })

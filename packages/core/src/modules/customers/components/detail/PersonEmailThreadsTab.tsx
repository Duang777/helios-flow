...
} from '@helios/ui/backend/messages'
...
/** Read the Helios `messageId` off a delivery event payload, if present. */
    ...
    async (opts?: { showLoading?: boolean }) => {
      ...
      try {
        const response = await apiCall<{ threads?: EmailThread[] }>(
          ...
          { method: 'GET', headers: { 'x-helios-forbidden-redirect': '0', 'x-helios-unauthorized-redirect': '0' } },
        )
        if (!response.ok) {
    ...
    await Promise.allSettled(
      channelsRef.current.map((channel) => {
        ...
        return apiCall(
          ...
          { method: 'POST', headers: { 'x-helios-forbidden-redirect': '0', 'x-helios-unauthorized-redirect': '0' } },
        )
      }),
    )
  }, [])

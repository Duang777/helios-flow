...
import { mapDictionaryColorToTone } from '@helios/shared/lib/query/advanced-filter'
...
const RESERVED_TASK_CUSTHELIOS_KEYS = new Set(['priority', 'description', 'due_at', 'dueAt'])
...
function buildInitialFormValues(task: TodoLinkSummary | null): Record<string, unknown> | undefined {
  ...
  if (task.customValues) {
    for (const [key, value] of Object.entries(task.customValues)) {
      if (RESERVED_TASK_CUSTHELIOS_KEYS.has(key)) continue

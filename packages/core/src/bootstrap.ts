...
import type { ModuleSubscriber } from '@helios/shared/modules/registry'
import { createEventBus } from '@helios/events/index'
import { setGlobalEventBus } from '@helios/shared/modules/events'
import { createCacheService } from '@helios/cache'
import type { CacheStrategy } from '@helios/cache'
...
import { registerTenantEncryptionSubscriber } from '@helios/shared/lib/encryption/subscriber'
import { isTenantDataEncryptionEnabled } from '@helios/shared/lib/encryption/toggles'
import { getSearchModuleConfigs } from '@helios/shared/modules/search'
import {
...
export async function bootstrap(container: AwilixContainer) {
  ...
  try {
    ...
    try {
      const { getModules } = await import('@helios/shared/lib/i18n/server')
    ...
    if (syncSubs.length) {
      try {
        const { registerSyncSubscribers } = await import('@helios/shared/lib/crud/sync-subscriber-store')

...
> Tag-aware, strategy-pluggable cache service extracted from the Helios platform—now packaged for general-purpose Node.js apps.
...
const cache = createCacheService({
  ...
  sqlitePath: './.helios/cache/cache.db',
  jsonFilePath: './.helios/cache/cache.json',
  defaultTtl: 5 * 60_000,
})
...
| `clear()` | Clears the current scope (all tenant-prefixed entries when used inside Helios). |
...
Bug reports & feature requests: open an issue in the main Helios repository and tag it with `cache`.

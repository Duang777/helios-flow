...
Use `@helios/cache` for all caching needs. MUST NOT use raw Redis, SQLite, or in-memory caching directly.
...
The memory strategy is bounded so a process-shared instance (`HELIOS_BOOTSTRAP_CACHE`, long-lived workers, memory-backed CRUD list cache) cannot grow without limit on user-controllable key cardinality.
...
```bash
yarn workspace @helios/cache test
yarn workspace @helios/cache build
```
...
An opt-in env flag, `HELIOS_CACHE_SAFETY_ALWAYS_CONSISTENT` (default **OFF**, 100% backward compatible), is planned to make that read-projection tail converge synchronously on write so reads never observe the window — at the cost of added write latency. Frame it as opt-in/forthcoming; do not assume it is on. See `.ai/specs/2026-06-05-cache-safety-always-consistent.md`.

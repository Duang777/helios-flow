...
function isHeliosPlatformLink(href: string | null): boolean {
  if (typeof href !== 'string' || href.trim().length === 0) {
    return false
  }

  try {
    const parsed = new URL(href, 'https://helios.test')
    return parsed.hostname === 'helios.com' || parsed.hostname.endsWith('.helios.com')
  } catch {
    return false
  }
}
...
describe('PrivacyPage', () => {
  ...
  it('includes an external link to the platform site', () => {
    const article = document.querySelector('article')
    const externalLinks = within(article!).getAllByRole('link').filter((link) => isHeliosPlatformLink(link.getAttribute('href')))

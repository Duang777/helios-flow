...
Use `@helios/content` for static content pages (privacy policies, terms, legal pages).
...
- Never create custom layout primitives when existing `@helios/ui` components fit.

## Validation Commands

```bash
yarn generate
yarn workspace @helios/content build
```
...
- Reuse shared UI components from `@helios/ui` — do not create custom layout primitives

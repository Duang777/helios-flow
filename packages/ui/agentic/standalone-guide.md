...
`@helios/ui` provides all admin/backend UI components. Use these instead of building from scratch.
...
import { useTenantContext } from '@helios/ui/portal/hooks/useTenantContext'
import { usePortalAppEvent } from '@helios/ui/portal/hooks/usePortalAppEvent'
import { usePortalEventBridge } from '@helios/ui/portal/hooks/usePortalEventBridge'
import { usePortalInjectedMenuItems } from '@helios/ui/portal/hooks/usePortalInjectedMenuItems'
import { usePortalNotifications } from '@helios/ui/portal/hooks/usePortalNotifications'
import { usePortalDashboardWidgets } from '@helios/ui/portal/hooks/usePortalDashboardWidgets'

// Portal layout
import { PortalShell } from '@helios/ui/portal/PortalShell'
import { PortalProvider, usePortalContext } from '@helios/ui/portal/PortalContext'
...
} from '@helios/ui/portal/components'
```
...
```tsx
"use client"
import { usePortalContext } from '@helios/ui/portal/PortalContext'
import { PortalPageHeader } from '@helios/ui/portal/components'
...
| Layer | Where | Effect |
|---|---|---|
| Page access | `apps/helios/src/app/(frontend)/[...slug]/page.tsx` | Server-side gate via `CustomerRbacService.userHasAllFeatures()` — missing feature blocks render |
...
**MUST** resolve features via `hasAllFeatures` / `matchFeature` from `@helios/shared/security/features`. Raw `Array.includes()` or `Set.has()` on feature arrays misses wildcards (`portal.*`) and is a bug.

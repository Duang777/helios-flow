...
import type { AppEventPayload } from '@helios/shared/modules/widgets/injection'
 ...
 */
export const PORTAL_EVENT_DHELIOS_NAME = 'om:portal-event'
...
export function usePortalAppEvent(
  ...
  useEffect(() => {
    ...
    window.addEventListener(PORTAL_EVENT_DHELIOS_NAME, listener)
    return () => {
      window.removeEventListener(PORTAL_EVENT_DHELIOS_NAME, listener)
    }

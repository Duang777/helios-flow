...
import { createEmptyTree } from '@helios/shared/lib/query/advanced-filter-tree'
import type { FilterFieldDef } from '@helios/shared/lib/query/advanced-filter'
...
describe('AdvancedFilterPanel', () => {
  ...
  it('saves filters separately from perspectives', async () => {
    ...
    try {
      ...
      const raw = window.localStorage.getItem('helios:advanced-filters:customers.people.list')
  ...
  it('applies a saved filter from the filter panel', async () => {
    window.localStorage.setItem('helios:advanced-filters:customers.people.list', JSON.stringify([
      {
        id: 'saved-1',
        name: 'Owned leads',
        tree: {

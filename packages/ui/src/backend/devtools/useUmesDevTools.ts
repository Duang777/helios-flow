...
} from '@helios/shared/lib/umes/devtools-types'
import { getEnricherTimingEntries } from '@helios/shared/lib/umes/enricher-timing'
import { getInterceptorActivityEntries } from '@helios/shared/lib/umes/interceptor-activity'
...
function collectDevToolsData(): UmesDevToolsData {
  ...
  try {
    const enricherEntries = ((globalThis as Record<string, unknown>).__heliosResponseEnrichers__ ?? []) as Array<{
      moduleId: string
      enricher: {
        id: string
        targetEntity: string
  ...
  try {
    const interceptorEntries = ((globalThis as Record<string, unknown>).__heliosApiInterceptors__ ?? []) as Array<{
      moduleId: string
      interceptor: {
        id: string
        targetRoute: string
  ...
  try {
    const globalState = (globalThis as Record<string, unknown>).__heliosComponentRegistry__ as {
      overrides?: Array<{
  ...
  try {
    const tables = ((globalThis as Record<string, unknown>).__heliosCoreInjectionTables__ ?? []) as Array<{
      moduleId: string

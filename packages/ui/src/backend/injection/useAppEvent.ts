 ...
 */
export const APP_EVENT_DHELIOS_NAME = 'om:event'
...
export function useAppEvent(
  ...
  useEffect(() => {
    ...
    window.addEventListener(APP_EVENT_DHELIOS_NAME, listener)
    return () => {
      window.removeEventListener(APP_EVENT_DHELIOS_NAME, listener)
    }

...
describe('LAST_OPERATION_AUTO_DISMISS_MS — env override (NEXT_PUBLIC_HELIOS_UNDO_BANNER_TIMEOUT_MS)', () => {
  const originalValue = process.env.NEXT_PUBLIC_HELIOS_UNDO_BANNER_TIMEOUT_MS

  afterEach(() => {
    if (originalValue === undefined) {
      delete process.env.NEXT_PUBLIC_HELIOS_UNDO_BANNER_TIMEOUT_MS
    } else {
      process.env.NEXT_PUBLIC_HELIOS_UNDO_BANNER_TIMEOUT_MS = originalValue
    }
    jest.resetModules()
  })
  ...
  function loadStoreWithEnv(envValue: string | undefined): typeof import('../store') {
    if (envValue === undefined) {
      ...
      process.env.NEXT_PUBLIC_HELIOS_UNDO_BANNER_TIMEOUT_MS = envValue
    }

import type { UseNotificationsPollResult } from '@helios/ui/backend/notifications/useNotificationsPoll'
...
jest.mock('@helios/ui/backend/notifications/useNotificationsPoll', () => ({
  useNotificationsPoll: jest.fn(() => pollResult),
}))

jest.mock('@helios/ui/backend/notifications/useNotificationsSse', () => ({
...
describe('useNotifications strategy', () => {
  ...
  it('uses SSE strategy when EventSource is available', async () => {
    ...
    const { useNotifications } = await import('@helios/ui/backend/notifications/useNotifications')

import type { UseProgressPollResult } from '@helios/ui/backend/progress/useProgressPoll'
...
jest.mock('@helios/ui/backend/progress/useProgressPoll', () => ({
  useProgressPoll: jest.fn(() => pollResult),
}))

jest.mock('@helios/ui/backend/progress/useProgressSse', () => ({
...
describe('useProgress strategy', () => {
  ...
  it('uses SSE strategy when EventSource is available', async () => {
    ...
    const { useProgress } = await import('@helios/ui/backend/progress/useProgress')

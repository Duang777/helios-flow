import type { UseMessagesPollResult } from '@helios/ui/backend/messages/useMessagesPoll'
...
jest.mock('@helios/ui/backend/messages/useMessagesPoll', () => ({
  useMessagesPoll: jest.fn(() => pollResult),
}))

jest.mock('@helios/ui/backend/messages/useMessagesSse', () => ({
...
describe('useMessages strategy', () => {
  ...
  it('uses SSE strategy when EventSource is available', async () => {
    ...
    const { useMessages } = await import('@helios/ui/backend/messages/useMessages')

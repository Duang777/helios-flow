...
import { slugifySidebarId } from '@helios/shared/modules/navigation/sidebarPreferences'
import { readVersionedPreference, writeVersionedPreference } from '@helios/shared/lib/browser/versionedPreference'
...
import type { InjectionMenuItem } from '@helios/shared/modules/widgets/injection'
...
// `@helios/shared/lib/browser/versionedPreference`.
...
function AppShellBody({ productName, logo, email, canManageUpgradeActions = false, groups, rightHeaderSlot, children, sidebarCollapsedDefault = false, currentTitle, breadcrumb, version, settingsSectionTitle, settingsPathPrefixes = [], settingsSections, profileSections, profileSectionTitle, profilePathPrefixes = [], mobileSidebarSlot, hideFooter = false, progressCompletedAutoHideMs }: AppShellProps) {
  ...
  // versioning threshold lives in `@helios/shared/lib/browser/versionedPreference`).
  React.useEffect(() => {
  ...
  function renderSidebar(
    ...
    const shouldRenderSidebarInjectionSpots = enableInjectionSpots
  ...
  return (
    ...
    >
      ...
      <div className="flex min-h-0 flex-1">
        ...
        {isSectionView ? (
          <aside
            className={`${asideClassesBase} px-3 hidden lg:block lg:sticky lg:top-[var(--topbar-height)] lg:h-[calc(100svh-var(--topbar-height))] lg:self-start lg:overflow-hidden lg:relative`}
            style={{ width: '240px' }}
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-5 sm:px-6 lg:px-8 lg:py-7">
            ...
            <InjectionSpot
              ...
              context={injectionContext}
            />
            <div id="helios-top-banners" className="mb-3 space-y-2" />
          ...
          {hideFooter ? null : (
            <footer className="border-t bg-background/80 px-4 py-3 backdrop-blur supports-[backdrop-filter]:bg-background/80 sm:px-6 lg:px-8">
          ...
          <aside className="absolute left-0 top-0 flex h-full w-[280px] max-w-[85vw] flex-col bg-background border-r shadow-lg overflow-hidden">
            <div className="shrink-0 flex items-center justify-between gap-2 border-b px-4 py-3">
              <Link href="/backend" className="flex items-center gap-2 min-w-0 text-sm font-semibold" onClick={() => setMobileOpen(false)} aria-label={t('appShell.goToDashboard')}>
                <Image src={resolvedLogo?.src ?? "/helios.svg"} alt={resolvedLogo?.alt ?? resolvedBrandName} width={28} height={28} className="shrink-0 dark:invert" unoptimized={resolvedLogoBypassesOptimization ? true : undefined} />

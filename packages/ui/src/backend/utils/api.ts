...
export async function apiFetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
  ...
  const disableUnauthorizedRedirect = readRedirectOverride(requestHeaders, 'x-helios-unauthorized-redirect')
  const disableForbiddenRedirect = readRedirectOverride(requestHeaders, 'x-helios-forbidden-redirect')
  ...
  try {
    const header = res.headers.get('x-helios-operation')
  ...
  try {
    const warningRaw = res.headers.get('x-helios-partial-index')
    if (warningRaw) {

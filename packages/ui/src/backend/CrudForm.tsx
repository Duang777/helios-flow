...
import type { CustomFieldDefLike } from '@helios/shared/modules/entities/validation'
...
import type { InjectionFieldDefinition, FieldContext } from '@helios/shared/modules/widgets/injection'
...
const CRUDFORM_EXTENDED_EVENTS_ENABLED = parseBooleanWithDefault(
  process.env.NEXT_PUBLIC_HELIOS_CRUDFORM_EXTENDED_EVENTS_ENABLED,
  true,
)
...
export type CrudFormProps<TValues extends Record<string, unknown>> = {
   ...
   * `x-helios-ext-optimistic-lock-expected-updated-at` extension header on
  ...
  const validateFieldOnBlur = React.useCallback(async (
    ...
    if (!Object.keys(nextFieldErrors).length && customFieldKey) {
      ...
      if (defsForField.length) {
        try {
          const { validateValuesAgainstDefs } = await import('@helios/shared/modules/entities/validation')

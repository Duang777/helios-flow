...
describe('QueryProvider defaults', () => {
  const originalStaleTimeFlag = process.env.NEXT_PUBLIC_HELIOS_QUERY_DEFAULT_STALE_TIME_ENABLED

  afterEach(() => {
    process.env.NEXT_PUBLIC_HELIOS_QUERY_DEFAULT_STALE_TIME_ENABLED = originalStaleTimeFlag
  })
  ...
  it('gates the shared staleTime default behind the public env flag', () => {
    process.env.NEXT_PUBLIC_HELIOS_QUERY_DEFAULT_STALE_TIME_ENABLED = '0'
    expect(buildDefaultQueryOptions().staleTime).toBeUndefined()

    process.env.NEXT_PUBLIC_HELIOS_QUERY_DEFAULT_STALE_TIME_ENABLED = '1'

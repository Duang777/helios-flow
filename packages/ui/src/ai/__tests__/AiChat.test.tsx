...
jest.mock('@helios/ai-assistant/modules/ai_assistant/lib/agent-transport', () => ({
...
describe('<AiChat>', () => {
  ...
  it('clears a stale stored model picker value back to the agent default', async () => {
    ...
    window.localStorage.setItem(
      'helios-ai-model-picker:customers.account_assistant',
    ...
    await waitFor(() => {
      expect(window.localStorage.getItem('helios-ai-model-picker:customers.account_assistant')).toBeNull()
    })
  })

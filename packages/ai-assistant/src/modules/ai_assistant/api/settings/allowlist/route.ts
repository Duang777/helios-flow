...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    PUT: {
      ...
      description:
        'Persists the per-tenant allowlist of providers and models. The runtime intersects this with the env allowlist (`HELIOS_AI_AVAILABLE_*`) at resolution time. ' +
...
export async function PUT(req: NextRequest) {
  ...
  if (Array.isArray(allowedProviders)) {
    for (const providerId of allowedProviders) {
      if (!isProviderAllowed(env, providerId)) {
        return NextResponse.json(
          {
            error: `Provider "${providerId}" is not in HELIOS_AI_AVAILABLE_PROVIDERS; tenant allowlist may not widen the env allowlist.`,
  ...
  for (const providerId of Object.keys(allowedModelsByProvider)) {
    if (!isProviderAllowed(env, providerId)) {
      return NextResponse.json(
        {
          error: `Provider "${providerId}" is not in HELIOS_AI_AVAILABLE_PROVIDERS; cannot save tenant model allowlist for it.`,
          code: 'provider_not_in_env_allowlist',
        },
        { status: 400 },
      )

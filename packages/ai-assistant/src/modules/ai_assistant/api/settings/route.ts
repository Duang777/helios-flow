...
import { findApiKeyBySecret } from '@helios/core/modules/api_keys/services/apiKeyService'
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const configuredProviderHint = env.HELIOS_AI_PROVIDER?.trim() || env.OPENCODE_PROVIDER?.trim() || null
    ...
    const configuredModelHint = env.HELIOS_AI_MODEL?.trim() || env.OPENCODE_MODEL?.trim() || defaultProviderModel
  ...
  if (providerId || hasRuntimeOverrideAllowlistWrite) {
    ...
    if (providerId && putEffectiveAllowlist) {
      ...
      if (!isProviderAllowed(process.env, providerId)) {
        return NextResponse.json(
          {
            error: `Provider "${requestedProviderId}" is not in HELIOS_AI_AVAILABLE_PROVIDERS.`,
            code: 'provider_not_allowlisted',
          },
          { status: 400 },
        )

...
describe('POST /api/ai/chat', () => {
  ...
  describe('Phase 1780-5 / 1780-6 — env + tenant allowlist rejections', () => {
    ...
    it('returns 400 provider_not_allowlisted when HELIOS_AI_AVAILABLE_PROVIDERS excludes the requested provider', async () => {
      process.env.HELIOS_AI_AVAILABLE_PROVIDERS = 'anthropic'
      seedAgentRegistryForTests([
        makeAgent({ id: 'customers.assistant', moduleId: 'customers' }),
      ])
      ...
      expect(json.error).toContain('HELIOS_AI_AVAILABLE_PROVIDERS')
    })

    it('returns 400 model_not_allowlisted when HELIOS_AI_AVAILABLE_MODELS_OPENAI excludes the requested model', async () => {
      process.env.HELIOS_AI_AVAILABLE_MODELS_OPENAI = 'gpt-4o'
      seedAgentRegistryForTests([
        makeAgent({ id: 'customers.assistant', moduleId: 'customers' }),
      ])
      ...
      expect(json.error).toContain('HELIOS_AI_AVAILABLE_MODELS_OPENAI')
    })
    ...
    it('returns 400 model_not_allowlisted with "env ∩ tenant" wording when the tenant snapshot narrows the env allowlist', async () => {
      process.env.HELIOS_AI_AVAILABLE_PROVIDERS = 'openai'
      process.env.HELIOS_AI_AVAILABLE_MODELS_OPENAI = 'gpt-4o,gpt-5-mini'

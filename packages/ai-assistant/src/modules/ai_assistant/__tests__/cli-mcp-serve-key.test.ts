...
describe('mcp:serve API key resolution (issue #2669)', () => {
  const originalEnv = process.env.HELIOS_API_KEY

  beforeEach(() => {
    jest.clearAllMocks()
    delete process.env.HELIOS_API_KEY
  })

  afterEach(() => {
    if (originalEnv === undefined) delete process.env.HELIOS_API_KEY
    else process.env.HELIOS_API_KEY = originalEnv
  })

  it('reads the key from HELIOS_API_KEY when --api-key is absent', async () => {
    process.env.HELIOS_API_KEY = 'omk_env.secret'

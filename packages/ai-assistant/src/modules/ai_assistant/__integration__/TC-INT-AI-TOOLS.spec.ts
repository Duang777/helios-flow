 ...
 *
 * Strategy: shells out to `yarn helios ai_assistant test-tools --json` which
 * runs the in-process tool runner (`packages/ai-assistant/.../lib/tool-test-runner.ts`).
 * The runner iterates every entry in `apps/helios/.helios/generated/ai-tools.generated.ts`,
...
function findAppRoot(): string {
  const testAppRoot = process.env.HELIOS_TEST_APP_ROOT
  if (testAppRoot && testAppRoot.length > 0) {
    return testAppRoot
  }

  // The CLI loads its env from apps/helios/.env (DB connection, JWT secret,
  ...
  return path.join(findRepoRoot(), 'apps', 'helios')
}
...
function runToolTestsCli(): Promise<{ stdout: string; stderr: string; code: number }> {
  return new Promise((resolveCmd, rejectCmd) => {
    ...
    const child = spawn(
      'yarn',
      ['helios', 'ai_assistant', 'test-tools', '--json'],
      {
        cwd,

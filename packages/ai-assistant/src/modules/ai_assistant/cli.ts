...
async function ensureBootstrap(): Promise<void> {
  ...
  try {
    const { getDiRegistrars } = await import('@helios/shared/lib/di/container')
    getDiRegistrars()
...
const mcpServe: ModuleCli = {
  ...
  async run(rest) {
    ...
    // Prefer the HELIOS_API_KEY env var so the secret never has to be
    // placed on the command line (argv is world-readable via ps / /proc).
    // The --api-key flag stays supported for backward compatibility.
    const apiKey = String(args['api-key'] ?? args.apiKey ?? '') || process.env.HELIOS_API_KEY || null
    ...
    if (!apiKey && !tenantId) {
      console.error('Usage: helios ai_assistant mcp:serve [options]')
      console.error('')
      console.error('Authentication (choose one):')
      console.error('  HELIOS_API_KEY env   API key secret (recommended — keeps the secret off argv)')
      ...
      console.error('  HELIOS_API_KEY=omk_xxxx.yyyy... helios ai_assistant mcp:serve')
      console.error('  helios ai_assistant mcp:serve --api-key omk_xxxx.yyyy...')
      console.error('  helios ai_assistant mcp:serve --tenant 123e4567-e89b-12d3-a456-426614174000 --user <user-id>')
      console.error('  helios ai_assistant mcp:serve --tenant 123e4567-e89b-12d3-a456-426614174000 --allow-unauthenticated-superadmin')
      return
    }
...
const mcpEnsureApiKey: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!filePath) {
      console.error('Usage: helios ai_assistant mcp:ensure-api-key --file <path> [options]')
      console.error('')
      ...
      console.error('  --email <email>  Owner user email (default HELIOS_INIT_SUPERADMIN_EMAIL, then superadmin@acme.com)')
    ...
    const rbac = container.resolve('rbacService') as import('@helios/core/modules/auth/services/rbacService').RbacService
...
const entityGraph: ModuleCli = {
  ...
  async run(rest) {
    ...
    const { getOrm } = await import('@helios/shared/lib/db/mikro')

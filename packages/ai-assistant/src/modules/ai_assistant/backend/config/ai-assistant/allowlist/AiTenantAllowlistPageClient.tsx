    ...
    : (
      <Alert>
        ...
        <AlertDescription className="space-y-1">
          {envAllowedProviders ? (
            <div>
              {t('ai_assistant.allowlist.envBanner.providers', 'HELIOS_AI_AVAILABLE_PROVIDERS')}: <code className="font-mono text-xs">{envAllowedProviders.join(', ')}</code>
            </div>
          ) : null}
          {Object.keys(envModelsByProvider).map((pid) => (
            <div key={pid}>
              <code className="font-mono text-xs">HELIOS_AI_AVAILABLE_MODELS_{pid.toUpperCase()}</code>: {envModelsByProvider[pid].join(', ')}
            </div>
          ))}

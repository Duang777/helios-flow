...
describe('McpClient stdio transport', () => {
  ...
  it('delegates process spawning to StdioClientTransport without a manual duplicate spawn', async () => {
    ...
    expect(StdioClientTransport).toHaveBeenCalledWith({
      ...
      args: ['server.js'],
      cwd: '/tmp/helios',
      env: { ...process.env, HELIOS_API_KEY: 'test-secret' },
      stderr: 'pipe',
    })
  ...
  it('passes the API key via HELIOS_API_KEY env, never on argv (issue #2669)', async () => {
    ...
    expect(transportArgs.env.HELIOS_API_KEY).toBe('omk_secret.value')

    // It must NOT appear on argv (world-readable via ps / /proc/<pid>/cmdline).
    expect(transportArgs.args).toEqual(['helios', 'ai_assistant', 'mcp:serve'])

...
describe('model-allowlist', () => {
  describe('readAllowedProviders', () => {
    it('returns null when HELIOS_AI_AVAILABLE_PROVIDERS is unset', () => {
      expect(readAllowedProviders({})).toBeNull()
    })

    it('returns null when HELIOS_AI_AVAILABLE_PROVIDERS is blank or whitespace-only', () => {
      expect(readAllowedProviders({ HELIOS_AI_AVAILABLE_PROVIDERS: '' })).toBeNull()
      expect(readAllowedProviders({ HELIOS_AI_AVAILABLE_PROVIDERS: '   ' })).toBeNull()
    })

    it('parses a comma-separated list with whitespace tolerance', () => {
      expect(readAllowedProviders({ HELIOS_AI_AVAILABLE_PROVIDERS: 'openai, anthropic ,google' }))
        .toEqual(['openai', 'anthropic', 'google'])
    })

    it('drops empty entries', () => {
      expect(readAllowedProviders({ HELIOS_AI_AVAILABLE_PROVIDERS: 'openai,,anthropic, ,' }))
  ...
  describe('readAllowedModels', () => {
    ...
    it('reads from HELIOS_AI_AVAILABLE_MODELS_<PROVIDER> uppercased', () => {
      expect(
        readAllowedModels({ HELIOS_AI_AVAILABLE_MODELS_OPENAI: 'gpt-5-mini,gpt-5' }, 'openai'),
      ).toEqual(['gpt-5-mini', 'gpt-5'])
    })

    it('handles compound provider ids (e.g. lm-studio)', () => {
      expect(
        readAllowedModels({ HELIOS_AI_AVAILABLE_MODELS_LM_STUDIO: 'qwen-32b' }, 'lm-studio'),
      ).toEqual(['qwen-32b'])
    })

    it('uses the same underscore env variable for underscore aliases', () => {
      expect(
        readAllowedModels({ HELIOS_AI_AVAILABLE_MODELS_LM_STUDIO: 'qwen-32b' }, 'lm_studio'),
      ).toEqual(['qwen-32b'])
    })
  })
  ...
  describe('isProviderAllowed', () => {
    ...
    it('is case-insensitive for the provider id', () => {
      expect(isProviderAllowed({ HELIOS_AI_AVAILABLE_PROVIDERS: 'OpenAI' }, 'openai')).toBe(true)
      expect(isProviderAllowed({ HELIOS_AI_AVAILABLE_PROVIDERS: 'openai' }, 'OPENAI')).toBe(true)
    })

    it('treats hyphen and underscore provider spellings as aliases', () => {
      expect(isProviderAllowed({ HELIOS_AI_AVAILABLE_PROVIDERS: 'lm_studio' }, 'lm-studio'))
        .toBe(true)
      expect(isProviderAllowed({ HELIOS_AI_AVAILABLE_PROVIDERS: 'lm-studio' }, 'lm_studio'))
        .toBe(true)
    })

    it('returns false when the provider is not in the list', () => {
      expect(isProviderAllowed({ HELIOS_AI_AVAILABLE_PROVIDERS: 'openai,anthropic' }, 'google'))
        .toBe(false)
    })
  })
  ...
  describe('isModelAllowedForProvider', () => {
    ...
    it('returns true only for case-sensitive exact model id matches', () => {
      const env = { HELIOS_AI_AVAILABLE_MODELS_OPENAI: 'gpt-5-mini,gpt-5' }
  ...
  describe('isProviderModelAllowed', () => {
    it('requires both provider and model gates to pass', () => {
      const env = {
        ...
        HELIOS_AI_AVAILABLE_MODELS_OPENAI: 'gpt-5-mini',
      }
  ...
  describe('public env-var-name helpers', () => {
    it('exposes the canonical env var names for docs/UI hints', () => {
      expect(providerAllowlistEnvVarName()).toBe('HELIOS_AI_AVAILABLE_PROVIDERS')
      expect(modelAllowlistEnvVarName('openai')).toBe('HELIOS_AI_AVAILABLE_MODELS_OPENAI')
      expect(modelAllowlistEnvVarName('lm-studio')).toBe('HELIOS_AI_AVAILABLE_MODELS_LM_STUDIO')
      expect(agentOverrideProviderAllowlistEnvVarName('catalog.catalog_assistant'))
        .toBe('HELIOS_AI_AGENT_CATALOG_CATALOG_ASSISTANT_AVAILABLE_PROVIDERS')
      expect(agentOverrideModelAllowlistEnvVarName('catalog.catalog_assistant', 'lm-studio'))
        .toBe('HELIOS_AI_AGENT_CATALOG_CATALOG_ASSISTANT_AVAILABLE_MODELS_LM_STUDIO')
    })
  })
  ...
  describe('Phase 1780-6 — intersectAllowlists / effective allowlist', () => {
    ...
    it('clips tenant providers to the env allowlist (tenant cannot widen env)', () => {
      const env = { HELIOS_AI_AVAILABLE_PROVIDERS: 'openai,anthropic' }
      const tenant = {
    ...
    it('canonicalizes hyphen and underscore provider aliases for tenant intersections', () => {
      const env = {
        HELIOS_AI_AVAILABLE_PROVIDERS: 'lm_studio',
        HELIOS_AI_AVAILABLE_MODELS_LM_STUDIO: 'qwen/qwen3.5-9b',
      }
      const tenant = {
    ...
    it('intersects per-agent chat override env and settings with the effective allowlist', () => {
      const env = {
        ...
        HELIOS_AI_AGENT_CATALOG_CATALOG_ASSISTANT_AVAILABLE_PROVIDERS: 'openai',
        HELIOS_AI_AGENT_CATALOG_CATALOG_ASSISTANT_AVAILABLE_MODELS_OPENAI: 'gpt-5-mini',
      }

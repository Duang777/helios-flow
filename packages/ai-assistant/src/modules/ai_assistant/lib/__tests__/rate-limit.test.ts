...
beforeEach(() => {
  ...
  delete process.env.HELIOS_TEST_MODE
  delete process.env.HELIOS_TEST_AI_CHAT_RATE_LIMIT_MODE
...
describe('checkAiChatRateLimit', () => {
  ...
  describe('opt-in test mode', () => {
    beforeEach(() => {
      process.env.HELIOS_TEST_MODE = '1'
      process.env.HELIOS_TEST_AI_CHAT_RATE_LIMIT_MODE = 'opt-in'
    })
    ...
    it('exercises the limiter when the opt-in header is set', async () => {
      ...
      await checkAiChatRateLimit({
        req: makeRequest({ 'x-helios-test-rate-limit': 'on' }),
        container: makeContainer({}),
        userId: 'user-1',
        tenantId: 'tenant-1',
      })

...
import * as registry from '@helios/shared/modules/registry'
import {
...
function makeTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'helios-gen-loader-'))
}
...
describe('rewriteGeneratedAliasImports', () => {
  ...
  //   "Cannot find package '@/.helios' imported from .../tool-loader.js"
  ...
  it('rewrites a static `from "@/..."` import to an absolute file URL', () => {
    ...
    const out = rewriteGeneratedAliasImports(
      `import { x } from '@/.helios/generated/entities'`,
      appRoot,
    )
    const expectedUrl = pathToFileURL(
      path.join(appRoot, '.helios/generated/entities'),
    ).href
  ...
  it('rewrites a dynamic `import("@/...")` call to an absolute file URL', () => {
    const appRoot = makeTempDir()
    const aliasSpecifier = '"@/.helios/generated/ai-tools.generated"'
    const source = `const tools = () => import(${aliasSpecifier})`
    const out = rewriteGeneratedAliasImports(source, appRoot)
    const expectedUrl = pathToFileURL(
      path.join(appRoot, '.helios/generated/ai-tools.generated'),
    ).href
...
describe('escapeUnsafeJsStringChars', () => {
  ...
  it('leaves ordinary file URLs unchanged', () => {
    const url = pathToFileURL(path.join('/tmp', 'app', '.helios', 'x.generated')).href
...
describe('findGeneratedFile', () => {
  ...
  it('locates the file under <cwd>/.helios/generated (standalone app layout)', () => {
    const appRoot = makeTempDir()
  ...
  it('locates the file under <cwd>/apps/helios/.helios/generated (monorepo layout)', () => {
    const root = makeTempDir()
    const generatedDir = path.join(root, 'apps', 'helios', '.helios', 'generated')
    fs.mkdirSync(generatedDir, { recursive: true })
  ...
  it('prefers the apps/helios layout over the cwd-root layout when both exist', () => {
    const root = makeTempDir()
    const appsDir = path.join(root, 'apps', 'helios', '.helios', 'generated')
    const rootDir = path.join(root, '.helios', 'generated')

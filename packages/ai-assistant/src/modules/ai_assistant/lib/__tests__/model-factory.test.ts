...
const testLogger = jest
  .requireMock('@helios/shared/lib/logger')
 ...
 * 0 cases that exercise `HELIOS_AI_PROVIDER` resolution.
 */
...
describe('createModelFactory', () => {
  ...
  it('prefers HELIOS_AI_<MODULE>_MODEL env override over agent default', () => {
    const provider = makeProvider()
    const env = { HELIOS_AI_INBOX_OPS_MODEL: 'env-pinned-model' }
  ...
  it('falls back to legacy <MODULE>_AI_MODEL when HELIOS_AI_<MODULE>_MODEL is unset', () => {
    const provider = makeProvider()
  ...
  it('prefers HELIOS_AI_<MODULE>_MODEL over the legacy <MODULE>_AI_MODEL fallback', () => {
    const provider = makeProvider()
    const env = {
      HELIOS_AI_INBOX_OPS_MODEL: 'canonical-model',
  ...
  it('uppercases moduleId when deriving the env var name', () => {
    const provider = makeProvider()
    const env = { HELIOS_AI_INBOX_OPS_MODEL: 'from-env' }
    const factory = createModelFactory(fakeContainer, makeFactoryDeps(provider, env))
    const resolution = factory.resolveModel({ moduleId: 'inbox_ops' })
  ...
  it('prefers agent_default over HELIOS_AI_MODEL env_default fallback', () => {
    const provider = makeProvider()
    const env = { HELIOS_AI_MODEL: 'global-pinned-model' }
    const factory = createModelFactory(fakeContainer, makeFactoryDeps(provider, env))
    const resolution = factory.resolveModel({
      moduleId: 'catalog',
      agentDefaultModel: 'agent-pinned-model',
  ...
  it('uses HELIOS_AI_MODEL as the env_default when neither caller, module_env, nor agent default applies', () => {
    const provider = makeProvider()
    const env = { HELIOS_AI_MODEL: 'global-pinned-model' }
    const factory = createModelFactory(fakeContainer, makeFactoryDeps(provider, env))
  ...
  it('falls back to legacy OPENCODE_MODEL as env_default when HELIOS_AI_MODEL is unset', () => {
    const provider = makeProvider()
  ...
  it('lets HELIOS_AI_<MODULE>_MODEL win over HELIOS_AI_MODEL global env', () => {
    const provider = makeProvider()
    const env = {
      HELIOS_AI_MODEL: 'global-pinned-model',
      HELIOS_AI_CATALOG_MODEL: 'catalog-env-model',
    }
    const factory = createModelFactory(fakeContainer, makeFactoryDeps(provider, env))
    const resolution = factory.resolveModel({ moduleId: 'catalog' })
    expect(resolution.source).toBe('module_env')
  ...
  it('passes HELIOS_AI_PROVIDER as the preferred provider order to the registry', () => {
    const provider = makeProvider({ id: 'openai' })
    const resolveFirstConfiguredSpy = jest.fn(() => provider as unknown as ReturnType<
      NonNullable<CreateModelFactoryDependencies['registry']>['resolveFirstConfigured']
    >)
    const env = { HELIOS_AI_PROVIDER: 'openai' }
  ...
  it('falls back to OPENCODE_PROVIDER when HELIOS_AI_PROVIDER is unset', () => {
  ...
  it('passes the resolved modelId and apiKey through to provider.createModel', () => {
    ...
    const env = { HELIOS_AI_CATALOG_MODEL: 'catalog-env-model' }
    const factory = createModelFactory(fakeContainer, makeFactoryDeps(provider, env))
  ...
  describe('Phase 0 — HELIOS_AI_PROVIDER + HELIOS_AI_MODEL', () => {
    it('forwards HELIOS_AI_PROVIDER to resolveFirstConfigured order', () => {
    ...
    it('uses HELIOS_AI_MODEL as the env_default fallback when nothing higher applies', () => {
      const anthropic = makeProvider({ id: 'anthropic' })
      const { registry } = makeMultiProviderRegistry([anthropic])
      const factory = createModelFactory(fakeContainer, {
        registry,
        env: { HELIOS_AI_MODEL: 'claude-haiku-4-5' },
      })
      const resolution = factory.resolveModel({})
      expect(resolution.source).toBe('env_default')
    ...
    it('honors both HELIOS_AI_PROVIDER and HELIOS_AI_MODEL together', () => {
      const anthropic = makeProvider({ id: 'anthropic' })
    ...
    it('falls through when only HELIOS_AI_PROVIDER is registered but unconfigured', () => {
    ...
    it('does not mix an HELIOS_AI_PROVIDER/HELIOS_AI_MODEL pair into a different configured provider', () => {
      ...
      const factory = createModelFactory(fakeContainer, {
        ...
        env: {
          ...
          HELIOS_AI_MODEL: 'claude-sonnet-4-20250514',
        },
      })
      expect(() => factory.resolveModel({})).toThrow(AiModelFactoryError)
    })

    it('slash-qualified HELIOS_AI_MODEL resets the provider for that resolution', () => {
      const anthropic = makeProvider({ id: 'anthropic' })
      const openai = makeProvider({ id: 'openai' })
      const { registry, spy } = makeMultiProviderRegistry([anthropic, openai])
      const factory = createModelFactory(fakeContainer, {
        registry,
        env: { HELIOS_AI_MODEL: 'openai/gpt-5-mini' },
      })
      const resolution = factory.resolveModel({})
      expect(spy).toHaveBeenCalledWith(
        expect.objectContaining({ order: ['openai'] }),
    ...
    it('does not split DeepInfra-style model ids that look like slash shorthand', () => {
      ...
      const factory = createModelFactory(fakeContainer, {
        registry,
        env: { HELIOS_AI_MODEL: 'meta-llama/Llama-3.3-70B-Instruct-Turbo' },
      })
      const resolution = factory.resolveModel({})
      expect(spy).toHaveBeenCalledWith(
    ...
    it('preserves slashy LM Studio model ids while honoring underscored provider env aliases', () => {
      ...
      const factory = createModelFactory(fakeContainer, {
        registry,
        env: {
          HELIOS_AI_PROVIDER: 'lm_studio',
          HELIOS_AI_MODEL: 'qwen/qwen3.5-9b',
        },
      })
      const resolution = factory.resolveModel({})
      expect(spy).toHaveBeenCalledWith(
    ...
    it('honors underscore aliases for module and agent provider defaults', () => {
      ...
      const factory = createModelFactory(fakeContainer, {
        registry,
        env: {
          HELIOS_AI_CATALOG_PROVIDER: 'lm_studio',
        },
      })
      const resolution = factory.resolveModel({
        moduleId: 'catalog',
    ...
    it('agent_default still beats env_default at lower priority', () => {
      ...
      const factory = createModelFactory(fakeContainer, {
        registry,
        env: { HELIOS_AI_MODEL: 'fallback-model' },
      })
  ...
  describe('Phase 1 — agentDefaultProvider, HELIOS_AI_<MODULE>_PROVIDER, providerOverride, slash-shorthand on every source', () => {
    ...
    it('HELIOS_AI_<MODULE>_PROVIDER env beats agentDefaultProvider for the provider axis', () => {
      const anthropic = makeProvider({ id: 'anthropic', defaultModel: 'claude-sonnet' })
      const openai = makeProvider({ id: 'openai', defaultModel: 'gpt-4o-mini' })
      ...
      const factory = createModelFactory(fakeContainer, {
        registry,
        env: { HELIOS_AI_CATALOG_PROVIDER: 'google' },
      })
      const resolution = factory.resolveModel({
        moduleId: 'catalog',
        agentDefaultProvider: 'openai',
    ...
    it('falls back to legacy <MODULE>_AI_PROVIDER when HELIOS_AI_<MODULE>_PROVIDER is unset', () => {
      const anthropic = makeProvider({ id: 'anthropic', defaultModel: 'claude-sonnet' })
      const openai = makeProvider({ id: 'openai', defaultModel: 'gpt-4o-mini' })
      const google = makeProvider({ id: 'google', defaultModel: 'gemini-1.5-pro' })
      const { registry, spy } = makeMultiProviderRegistry([anthropic, openai, google])
    ...
    it('prefers HELIOS_AI_<MODULE>_PROVIDER over legacy <MODULE>_AI_PROVIDER', () => {
      const anthropic = makeProvider({ id: 'anthropic' })
      const openai = makeProvider({ id: 'openai', defaultModel: 'gpt-4o-mini' })
      const google = makeProvider({ id: 'google', defaultModel: 'gemini-1.5-pro' })
      const { registry, spy } = makeMultiProviderRegistry([anthropic, openai, google])
    ...
    it('providerOverride beats HELIOS_AI_<MODULE>_PROVIDER for the provider axis', () => {
      const anthropic = makeProvider({ id: 'anthropic' })
      const openai = makeProvider({ id: 'openai', defaultModel: 'gpt-4o-mini' })
      const google = makeProvider({ id: 'google', defaultModel: 'gemini-1.5-pro' })
      const { registry, spy } = makeMultiProviderRegistry([anthropic, openai, google])
    ...
    it('slash-qualified HELIOS_AI_<MODULE>_MODEL provides both provider hint and model id', () => {
      const anthropic = makeProvider({ id: 'anthropic' })
      const openai = makeProvider({ id: 'openai', defaultModel: 'gpt-4o-mini' })
      const { registry, spy } = makeMultiProviderRegistry([anthropic, openai])
      const factory = createModelFactory(fakeContainer, {
        registry,
        env: { HELIOS_AI_CATALOG_MODEL: 'openai/gpt-5' },
      })
      const resolution = factory.resolveModel({ moduleId: 'catalog' })
      expect(spy).toHaveBeenCalledWith(
        expect.objectContaining({ order: ['openai'] }),
...
describe('Phase 4a — tenantOverride, requestOverride, allowRuntimeOverride (renamed from allowRuntimeModelOverride)', () => {
  ...
  describe('Phase 1780-5 — HELIOS_AI_AVAILABLE_PROVIDERS / HELIOS_AI_AVAILABLE_MODELS_<PROVIDER>', () => {
    ...
    it('falls back to the agent default model when the resolved model is not allowlisted for the provider', () => {
      ...
      const env = { HELIOS_AI_AVAILABLE_MODELS_OPENAI: 'gpt-5-mini' }
      const factory = createModelFactory(fakeContainer, makeFactoryDeps(provider, env))
      const resolution = factory.resolveModel({
      ...
      expect(resolution.allowlistFallback).toEqual({
        ...
        reason: expect.stringContaining('HELIOS_AI_AVAILABLE_MODELS_OPENAI'),
      })
    ...
    it('falls back to the first allowlisted model when neither the resolved nor the provider default is allowed', () => {
      ...
      const env = { HELIOS_AI_AVAILABLE_MODELS_OPENAI: 'gpt-4o,gpt-4-turbo' }
      const factory = createModelFactory(fakeContainer, makeFactoryDeps(provider, env))
    ...
    it('swaps to the agent default provider when the resolved provider is not allowlisted', () => {
      ...
      const env = { HELIOS_AI_AVAILABLE_PROVIDERS: 'openai' }
    ...
    it('accepts allowlisted (provider, model) pairs without intervention', () => {
      const provider = makeProvider({ id: 'openai', defaultModel: 'gpt-5-mini' })
      const env = {
        HELIOS_AI_AVAILABLE_PROVIDERS: 'openai',
        HELIOS_AI_AVAILABLE_MODELS_OPENAI: 'gpt-5-mini,gpt-4o',
      }
      const factory = createModelFactory(fakeContainer, makeFactoryDeps(provider, env))
      const resolution = factory.resolveModel({ callerOverride: 'gpt-4o' })
      expect(resolution.providerId).toBe('openai')
...
describe('createModelFactory — vendor-prefix gateways', () => {
  ...
  it('routes a vendor-prefixed model to the configured gateway (the core bug)', () => {
    ...
    const factory = createModelFactory(fakeContainer, {
      registry,
      env: { HELIOS_AI_PROVIDER: 'openrouter', HELIOS_AI_MODEL: 'anthropic/claude-sonnet-4.5' },
    })
    const resolution = factory.resolveModel({})
  ...
  it('routes a requesty openai/gpt-4o-mini id to requesty, not native openai', () => {
    ...
    const factory = createModelFactory(fakeContainer, {
      registry,
      env: { HELIOS_AI_PROVIDER: 'requesty', HELIOS_AI_MODEL: 'openai/gpt-4o-mini' },
    })
    const resolution = factory.resolveModel({})
  ...
  it('strips one leading gateway prefix (openrouter/anthropic/claude → anthropic/claude)', () => {
    ...
    const factory = createModelFactory(fakeContainer, {
      registry,
      env: { HELIOS_AI_PROVIDER: 'openrouter', HELIOS_AI_MODEL: 'openrouter/anthropic/claude-sonnet-4.5' },
    })
    const resolution = factory.resolveModel({})
    expect(resolution.providerId).toBe('openrouter')
    expect(resolution.modelId).toBe('anthropic/claude-sonnet-4.5')
  ...
  it('a caller slash-pin still wins over a global gateway', () => {
    ...
    const factory = createModelFactory(fakeContainer, {
      registry,
      env: { HELIOS_AI_PROVIDER: 'openrouter' },
    })
    const resolution = factory.resolveModel({ callerOverride: 'openai/gpt-5-mini' })
    expect(resolution.source).toBe('caller_override')
    expect(resolution.providerId).toBe('openai')
  ...
  it('leaves a native (non-gateway) provider hint splitting unchanged', () => {
    ...
    const factory = createModelFactory(fakeContainer, {
      registry,
      env: { HELIOS_AI_PROVIDER: 'anthropic', HELIOS_AI_MODEL: 'anthropic/claude-sonnet-4.5' },
    })
    const resolution = factory.resolveModel({})
    expect(resolution.providerId).toBe('anthropic')
    expect(resolution.modelId).toBe('claude-sonnet-4.5')
  ...
  it('applies per-tier on the module axis (HELIOS_AI_<MODULE>_PROVIDER gateway)', () => {
    const { registry } = makeMultiProviderRegistry(gatewayProviders())
    const factory = createModelFactory(fakeContainer, {
      registry,
      env: {
        HELIOS_AI_INBOX_OPS_PROVIDER: 'openrouter',
        HELIOS_AI_INBOX_OPS_MODEL: 'anthropic/claude-sonnet-4.5',
      },
    })
    const resolution = factory.resolveModel({ moduleId: 'inbox_ops' })
    expect(resolution.source).toBe('module_env')
  ...
  it('keeps an unregistered vendor prefix whole even without the gateway flag (regression guard)', () => {
    ...
    const factory = createModelFactory(fakeContainer, {
      registry,
      env: { HELIOS_AI_PROVIDER: 'deepinfra', HELIOS_AI_MODEL: 'meta-llama/Llama-3.3-70B-Instruct-Turbo' },
    })
    const resolution = factory.resolveModel({})
    expect(resolution.providerId).toBe('deepinfra')
    expect(resolution.modelId).toBe('meta-llama/Llama-3.3-70B-Instruct-Turbo')

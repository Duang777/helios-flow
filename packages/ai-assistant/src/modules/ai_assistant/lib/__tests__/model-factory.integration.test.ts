 ...
 *   - env `HELIOS_AI_<MODULE>_MODEL`   → wins over agent default + provider default
...
describe('Step 5.16 — model factory fallback chain (integration)', () => {
  it('callerOverride wins over env + agentDefaultModel + provider default', () => {
    ...
    const env = { HELIOS_AI_INBOX_OPS_MODEL: 'env-pinned' }
  ...
  it('env HELIOS_AI_<MODULE>_MODEL wins over agentDefaultModel + provider default when moduleId is set', () => {
    const provider = makeProvider({ defaultModel: 'provider-default' })
  ...
  it('moduleId: undefined skips the env-override lookup (regression)', () => {
    // Even if a `HELIOS_AI_<MODULE>_MODEL` env var exists in the environment, the
    ...
    // yielded the literal env name `"HELIOS_AI_UNDEFINED_MODEL"`.
    const provider = makeProvider({ defaultModel: 'provider-default' })
    const env = {
      HELIOS_AI_INBOX_OPS_MODEL: 'should-be-ignored',
      HELIOS_AI_UNDEFINED_MODEL: 'also-ignored',
    }
    const factory = createModelFactory(fakeContainer, makeDeps(provider, env))
    const resolution = factory.resolveModel({
      agentDefaultModel: 'agent-pinned',
  ...
  it('HELIOS_AI_PROVIDER + HELIOS_AI_MODEL surfaces the env_default source end-to-end', () => {
    ...
    // and the modelId should be the `HELIOS_AI_MODEL` value forwarded
...
describe('model factory — vendor-prefix gateway (real adapters + presets)', () => {
  ...
  it('routes HELIOS_AI_PROVIDER=openrouter + HELIOS_AI_MODEL=anthropic/claude-sonnet-4.5 to OpenRouter, id intact', () => {
    const factory = createModelFactory(fakeContainer, {
      registry: realGatewayRegistry(),
      env: {
        OPENROUTER_API_KEY: 'or-test-key',
        HELIOS_AI_PROVIDER: 'openrouter',
        HELIOS_AI_MODEL: 'anthropic/claude-sonnet-4.5',
      },
    })
    const resolution = factory.resolveModel({})

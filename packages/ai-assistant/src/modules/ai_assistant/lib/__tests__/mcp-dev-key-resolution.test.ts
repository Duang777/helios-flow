...
describe('mcp:dev API key resolution (#2671)', () => {
  ...
  describe('getApiKeyFromMcpJson', () => {
    ...
    const originalEnvKey = process.env.HELIOS_API_KEY

    beforeEach(async () => {
      projectRoot = await realFs.mkdtemp(join(tmpdir(), 'helios-mcp-key-'))
    ...
    afterEach(async () => {
      ...
      if (originalEnvKey === undefined) delete process.env.HELIOS_API_KEY
      else process.env.HELIOS_API_KEY = originalEnvKey
    ...
    it('keeps environment-key precedence without reading the config file', async () => {
      process.env.HELIOS_API_KEY = ' env-key '

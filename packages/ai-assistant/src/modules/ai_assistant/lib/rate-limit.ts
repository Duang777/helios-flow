...
} from '@helios/shared/lib/ratelimit/helpers'
 ...
 * registered as a DI value in bootstrap; `@helios/core` is not a dependency
...
export async function checkAiChatRateLimit(
  ...
  try {
    // Opt-in test mode mirrors auth's `x-helios-test-rate-limit` pattern so suites can
    // exercise the bucket without throttling unrelated requests.
    const isIntegrationTestMode =
      process.env.HELIOS_TEST_MODE === '1' && process.env.HELIOS_TEST_AI_CHAT_RATE_LIMIT_MODE === 'opt-in'
    if (isIntegrationTestMode && options.req.headers.get('x-helios-test-rate-limit') !== 'on') {
      return null
    }

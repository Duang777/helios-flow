 ...
 * report. Used by the `helios ai_assistant test-tools` CLI subcommand and
...
async function loadGeneratedTools(): Promise<{ moduleId: string; tools: AiToolDefinition[] }[]> {
  ...
  if (!tsPath) {
    throw new Error(
      'Could not locate apps/<app>/.helios/generated/ai-tools.generated.ts. Run `yarn generate` first.',
    )
  }
...
export async function pickDefaultTenant(
  ...
  try {
    ...
    if (process.env.HELIOS_TOOL_TEST_DEBUG === '1') {

...
import type { OpenApiDocument } from '@helios/shared/lib/openapi'
import { buildOpenApiDocument } from '@helios/shared/lib/openapi'
import type { Module } from '@helios/shared/modules/registry'
import { fetchWithTimeout, resolveTimeoutMs } from '@helios/shared/lib/http/fetchWithTimeout'
...
export async function loadRichOpenApiSpec(): Promise<OpenApiDocument | null> {
  ...
  try {
    const { getModules } = await import('@helios/shared/lib/modules/registry')
...
async function loadRawOpenApiSpec(): Promise<OpenApiDocument | null> {
  ...
  try {
    ...
    const { findAppRoot, findAllApps } = await import('@helios/shared/lib/bootstrap/appResolver')

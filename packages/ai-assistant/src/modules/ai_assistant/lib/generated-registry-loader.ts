/**
 * Runtime loader for `.helios/generated/*.generated.ts` registry files.
 *
 * The generated registries import their entries through the `@/` path alias
 * (e.g. `@/.helios/generated/ai-tools.generated`). That alias is only
 ...
 * `import('@/.helios/...')` throws `ERR_MODULE_NOT_FOUND: Cannot find
 * package '@/.helios'` because Node treats `@/` as a package specifier.
 *
 ...
 * `@helios/shared/lib/bootstrap/dynamicLoader` and works in both the
 ...
 * location for a `apps/helios/.helios/generated/<fileName>` (monorepo), then
 ...
 *      (`<appRoot>/.helios/generated`), the location the generator wrote them
 *      relative to. Package-backed modules (`@helios/*`) are unaffected —
...
export async function ensureApiRouteManifestsRegistered(): Promise<number> {
  const registry = await import('@helios/shared/modules/registry')

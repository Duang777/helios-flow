...
import {
  ...
  deleteApiKey,
  findApiKeyBySecret,
} from '@helios/core/modules/api_keys/services/apiKeyService'
import { User } from '@helios/core/modules/auth/data/entities'
import { emailHashLookupValues } from '@helios/core/modules/auth/lib/emailHash'
...
export async function ensureMcpApiKey(options: EnsureMcpApiKeyOptions): Promise<EnsureMcpApiKeyResult> {
  ...
  const ownerEmail =
    ...
    process.env.HELIOS_INIT_SUPERADMIN_EMAIL?.trim() ||
  ...
  if (!owner) {
    throw new Error(
      `[internal] MCP API key owner not found: no active user with email "${ownerEmail}". ` +
        'Run "helios init" first or pass --email pointing at an existing admin user.',
    )
  }

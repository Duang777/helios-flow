 ...
 *   2. Env variable `HELIOS_AI_<MODULE>_MODEL` (uppercased `moduleId`) when
 *      `moduleId` is provided. Example:
 *      `HELIOS_AI_INBOX_OPS_MODEL=claude-haiku-4-5`,
 *      `HELIOS_AI_CATALOG_MODEL=gpt-4o-mini`. The legacy
 ...
 *      Accepts a slash-qualified `<provider>/<model>` shorthand (Phase 1).
 *   4. Global env `HELIOS_AI_MODEL` (canonical) with `OPENCODE_MODEL` kept as
 ...
 *   8. `HELIOS_AI_PROVIDER` (legacy `OPENCODE_PROVIDER`) (Phase 0).
 *
 * The `HELIOS_AI_*` env knobs are canonical; the legacy `OPENCODE_PROVIDER` /
...
import type { EnvLookup, LlmProvider } from '@helios/shared/lib/ai/llm-provider'
import { llmProviderRegistry } from '@helios/shared/lib/ai/llm-provider-registry'
import {
  intersectAllowlists,
  canonicalProviderId,
...
export interface AiModelFactoryInput {
   ...
   * `HELIOS_AI_<MODULE>_MODEL` (uppercased) as the env-override source, with
   ...
   * `HELIOS_AI_INBOX_OPS_MODEL` (legacy `INBOX_OPS_AI_MODEL`).
   *
   * Also enables the `HELIOS_AI_<MODULE>_PROVIDER` env axis (legacy
   ...
   * to the next configured provider. Sits between `HELIOS_AI_<MODULE>_PROVIDER`
   * and the global `HELIOS_AI_PROVIDER` in the provider-axis seed list above.
   *
   ...
   * env allowlist (`HELIOS_AI_AVAILABLE_*`) and this tenant allowlist. Pass `null`
...
export interface AiModelResolution {
   ...
   *
   * - `env_default` indicates `HELIOS_AI_MODEL` (preferred) or the legacy
  ...
  baseURL?: string
  /**
   * Populated when the env-driven HELIOS_AI_AVAILABLE_PROVIDERS /
   * HELIOS_AI_AVAILABLE_MODELS_<PROVIDER> allowlist rejected the originally
...
/**
 * Subset of {@link import('@helios/shared/lib/ai/llm-provider-registry').LlmProviderRegistry}
...
export interface CreateModelFactoryDependencies {
  ...
  /** Env lookup for `HELIOS_AI_<MODULE>_MODEL` + provider credentials. */
  env?: EnvLookup
}
 ...
 * Returns `null` when neither `HELIOS_AI_PROVIDER` nor the legacy
...
function readGlobalProviderFromEnv(
  ...
  const candidates = [normalizeOverride(env.HELIOS_AI_PROVIDER), normalizeOverride(env.OPENCODE_PROVIDER)]
...
/**
 * Reads the global model hint from the unified env vars. `HELIOS_AI_MODEL`
 * wins over the legacy `OPENCODE_MODEL`.
 */
function readGlobalModelFromEnv(env: EnvLookup): string | null {
  return normalizeOverride(env.HELIOS_AI_MODEL) ?? normalizeOverride(env.OPENCODE_MODEL)
}

/** Canonical per-module model env. Example: `HELIOS_AI_INBOX_OPS_MODEL`. */
function moduleModelEnvVarName(moduleId: string): string {
  return `HELIOS_AI_${moduleId.toUpperCase()}_MODEL`
}

/**
 * Legacy per-module model env (pre-HELIOS_AI_* rename). Example:
...
/** Canonical per-module provider env. Example: `HELIOS_AI_INBOX_OPS_PROVIDER`. */
function moduleProviderEnvVarName(moduleId: string): string {
  return `HELIOS_AI_${moduleId.toUpperCase()}_PROVIDER`
}

/**
 * Legacy per-module provider env (pre-HELIOS_AI_* rename). Example:
...
export function createModelFactory(
  ...
  return {
    resolveModel(input: AiModelFactoryInput): AiModelResolution {
      ...
      // HELIOS_AI_MODEL is canonical; the legacy OPENCODE_MODEL is read as a
      ...
      // HELIOS_AI_PROVIDER is canonical; the legacy OPENCODE_PROVIDER is read as
      ...
      if (!provider) {
        ...
        throw new AiModelFactoryError(
          'no_provider_configured',
          'No LLM provider is configured. Set HELIOS_AI_PROVIDER (or the legacy OPENCODE_PROVIDER) plus a matching API key such as OPENAI_API_KEY, ANTHROPIC_API_KEY, or GOOGLE_GENERATIVE_AI_API_KEY, then restart the app. See https://docs.helios.com/framework/ai-assistant/overview.',
        )
      }
      ...
      // HELIOS_AI_AVAILABLE_PROVIDERS / HELIOS_AI_AVAILABLE_MODELS_<PROVIDER> clip
  ...
  if (!isModelAllowedForProviderInEffective(effective, provider.id, modelId)) {
    ...
    if (replacementModel !== modelId) {
      const source = effective.tenantOverridesActive
        ...
        : `HELIOS_AI_AVAILABLE_MODELS_${provider.id.toUpperCase()}`

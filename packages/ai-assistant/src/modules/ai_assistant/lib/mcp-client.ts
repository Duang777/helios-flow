...
export type StdioClientOptions = {
   ...
   * `HELIOS_API_KEY` environment variable, never as a command-line
   * argument (argv is world-readable via `ps`/`/proc/<pid>/cmdline`).
   */
  apiKeySecret: string
  /** Command to run (default: 'yarn') */
  command?: string
  /** Arguments for the command (default: helios ai_assistant mcp:serve, no secret on argv) */
  args?: string[]
...
export class McpClient implements McpClientInterface {
  ...
  private static async connectStdio(options: StdioClientOptions): Promise<McpClient> {
    ...
    // The API key is passed via HELIOS_API_KEY in the child env (below),
    ...
    const transport = new StdioClientTransport({
      ...
      args,
      cwd,
      env: { ...process.env, HELIOS_API_KEY: options.apiKeySecret } as Record<string, string>,
      stderr: 'pipe',
    })
    ...
    const client = new Client(
      { name: 'helios-client', version: '0.1.0' },
      { capabilities: {} }
    )

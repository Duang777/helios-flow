 ...
 *
 * - `HELIOS_AI_AVAILABLE_MODELS_<PROVIDER>=gpt-5-mini,gpt-5`
 ...
 *     id (e.g. `openai` → `HELIOS_AI_AVAILABLE_MODELS_OPENAI`).
 *
...
const PROVIDERS_ENV = 'HELIOS_AI_AVAILABLE_PROVIDERS'
...
function envModelsVarName(providerId: string): string {
  ...
  return `HELIOS_AI_AVAILABLE_MODELS_${envSafeProviderId}`
}
...
function agentOverrideProvidersVarName(agentId: string): string {
  return `HELIOS_AI_AGENT_${envSafeId(agentId)}_AVAILABLE_PROVIDERS`
}

function agentOverrideModelsVarName(agentId: string, providerId: string): string {
  return `HELIOS_AI_AGENT_${envSafeId(agentId)}_AVAILABLE_MODELS_${envSafeId(providerId)}`
}

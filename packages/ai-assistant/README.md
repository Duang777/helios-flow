...
AI-powered chat and tool execution for Helios, using MCP (Model Context Protocol) for tool discovery and execution.
...
- An Helios API key (created via Backend > Settings > API Keys)
...
The MCP server authenticates requests using an Helios API key:

1. Log into Helios backend
...
The `x-api-key` should be an Helios API key created in the backend admin (Settings > API Keys).
...
| `MCP_SERVER_API_KEY` | For production | - | Helios API key (`omk_...`) for MCP server auth |
...
import { createOpenAICompatibleProvider } from '@helios/ai-assistant/modules/ai_assistant/lib/llm-adapters/openai'
...
The `x-api-key` value must be a valid Helios API key:
1. Log into Helios backend
2. Go to Settings > API Keys
3. Create a new API key with appropriate permissions
...
import {
  ...
  AiChatHeaderButton,
} from '@helios/ai-assistant/frontend'
...
```typescript
import { z } from 'zod'
import type { McpToolContext } from '@helios/ai-assistant'
...
This scans all modules for `ai-tools.ts` files and generates `ai-tools.generated.ts` in `.helios/generated/`.
...
yarn helios ai_assistant mcp:list-tools

# List tools with descriptions
yarn helios ai_assistant mcp:list-tools --verbose

# Extract entity relationship graph
yarn helios ai_assistant entity-graph
yarn helios ai_assistant entity-graph --format json
yarn helios ai_assistant entity-graph --module sales
yarn helios ai_assistant entity-graph --entity SalesOrder
```
...
curl http://localhost:4096/mcp
# Expected: {"helios":{"status":"connected"}}
```

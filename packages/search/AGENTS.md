...
- Never call raw `fetch` against the search API; use `apiCall`/`apiCallOrThrow` from `@helios/ui/backend/utils/apiCall`.
...
```bash
yarn generate
yarn workspace @helios/search test
yarn workspace @helios/search build
```
...
```typescript
import type { SearchService } from '@helios/search'
...
```typescript
import type { SearchIndexer } from '@helios/search'
...
| `HELIOS_SEARCH_ENABLED` | When you need to disable the search module entirely | Default: `true`; set to `false` to disable |
| `HELIOS_SEARCH_DEBUG` | When debugging search behavior | Enables verbose debug logging |
| `HELIOS_SEARCH_MIN_LEN` | When tuning the Postgres `search_tokens` index | Default: `3`. Minimum token length + floor of prefix expansion. **Token strategy only** — no effect on fulltext/vector |
| `HELIOS_SEARCH_ENABLE_PARTIAL` | When trading `search_tokens` size for prefix matching | Default: `true`. Prefix/partial expansion for tokens (Meilisearch unaffected); increases `search_tokens` size ~5–6×. **Token strategy only** |
| `HELIOS_SEARCH_HASH_ALGO` | When choosing the token hash algorithm | Default: `sha256` (accepts `sha1`, `md5`). **Token strategy only** |
| `HELIOS_SEARCH_STORE_RAW_TOKENS` | Almost never — debugging tokenization only | Default: `false`. Stores plaintext token alongside the hash — **security-sensitive**, retains plaintext of otherwise-hashed values. **Token strategy only** |
| `HELIOS_SEARCH_FIELD_BLOCKLIST` | When extra fields must never be tokenized | Comma-separated field names, merged with built-in `password,token,secret,hash`. **Token strategy only** |
...
| `HELIOS_LOG_LEVEL` | When debugging presenter enrichment | Set to `debug` to surface presenter enricher diagnostics (replaces the former `DEBUG_SEARCH_ENRICHER` flag) |
...
yarn helios search worker fulltext-indexing --concurrency=5

# Vector embedding indexing worker
yarn helios search worker vector-indexing --concurrency=10
```

For development with `QUEUE_STRATEGY=local`, jobs process from `.helios/queue/` automatically (or `QUEUE_BASE_DIR` if set).

## Use CLI Commands

### Check status
```bash
yarn helios search status
```
Shows search module status, available strategies, and configuration.

### Run a search query
```bash
yarn helios search query -q "search term" --tenant <id> [options]
```
Options:
...
```bash
yarn helios search index --entity <entityId> --record <recordId> --tenant <id>
```
Options:
...
### Reindex entities
```bash
yarn helios search reindex --tenant <id> [options]
```
Options:
...
```bash
yarn helios search test-meilisearch
```

### Start a queue worker
```bash
yarn helios search worker <queue-name> --concurrency=<n>
```
Queues: `fulltext-indexing`, `vector-indexing`

### Show help
```bash
yarn helios search help
```
...
- [ ] Test with `yarn helios search query -q "test" --tenant <id>`

## Checklist: Debug Search Issues

- [ ] Run `yarn helios search status` to verify strategies and connectivity
- [ ] Run `yarn helios search test-meilisearch` if fulltext is not returning results
- [ ] Check `HELIOS_SEARCH_ENABLED` is not set to `false`
- [ ] Enable `HELIOS_SEARCH_DEBUG=true` for verbose logging
- [ ] Set `HELIOS_LOG_LEVEL=debug` if presenters are missing or wrong (surfaces presenter enricher diagnostics)

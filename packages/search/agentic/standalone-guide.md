...
`@helios/search` provides fulltext, vector, and token-based search. Configure search for your entities via `search.ts` in your module.
...
```typescript
import type { SearchModuleConfig, SearchBuildContext } from '@helios/shared/modules/search'
...
```bash
yarn helios search status                           # Check strategies and connectivity
yarn helios search query -q "term" --tenant <id>    # Run a search
yarn helios search reindex --tenant <id>             # Reindex all entities
yarn helios search reindex --entity my_module:my_entity --tenant <id>  # Reindex specific entity
yarn helios search test-meilisearch                  # Test Meilisearch connection
```
...
| `HELIOS_SEARCH_DEBUG` | Enable verbose debug logging |

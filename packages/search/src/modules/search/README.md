...
The search module provides unified search capabilities across all entities in Helios, supporting multiple search strategies including Meilisearch (full-text) and vector embeddings (semantic search).
...
The search module exposes CLI commands via `yarn helios search <command>`.
...
```bash
yarn helios search query --query "search terms" --tenant <id> [options]
```
...
```bash
yarn helios search index --entity <entityId> --record <recordId> --tenant <tenantId>
```
...
Use `yarn helios search reindex-help` for detailed options.
...
```bash
yarn helios search worker <queue-name> [options]
```
...
File-based queue stored in `.helios/queue/` by default (override with `QUEUE_BASE_DIR`).
...
| `HELIOS_SEARCH_ENABLED` | Enable/disable search module | `true` |
| `HELIOS_SEARCH_DEBUG` | Enable debug logging for search module | `false` |
| `HELIOS_SEARCH_MIN_LEN` | Minimum token length for the Postgres `search_tokens` index; also the floor of prefix expansion (token strategy only) | `3` |
| `HELIOS_SEARCH_ENABLE_PARTIAL` | Prefix/partial expansion for `search_tokens` (indexing "john" stores hashes for `joh`,`john`). Token/Postgres only — Meilisearch unaffected. Increases `search_tokens` size ~5–6× | `true` |
| `HELIOS_SEARCH_HASH_ALGO` | Hash algorithm for `search_tokens` tokens (`sha256`/`sha1`/`md5`); token strategy only | `sha256` |
| `HELIOS_SEARCH_STORE_RAW_TOKENS` | Store plaintext token alongside the hash in `search_tokens` — **security-sensitive** (retains plaintext of otherwise-hashed values); token strategy only | `false` |
| `HELIOS_SEARCH_FIELD_BLOCKLIST` | Comma-separated extra field names excluded from tokenization (merged with built-in `password,token,secret,hash`); token strategy only | - |
...
```typescript
import { registerSearchModule } from '@helios/search'

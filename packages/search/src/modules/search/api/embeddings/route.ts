        ...
        return NextResponse.json(
          {
            error: t(
              ...
              'Auto-indexing is disabled via HELIOS_DISABLE_VECTOR_SEARCH_AUTOINDEXING (legacy alias: DISABLE_VECTOR_SEARCH_AUTOINDEXING).',
            ),
          },
          { status: 409 },
        )
            ...
            return NextResponse.json(
              {
                error: t(
                  ...
                  'Ollama base URL is not allowed. Set OLLAMA_BASE_URL in the environment, or add the host to HELIOS_SEARCH_OLLAMA_BASE_URL_ALLOWLIST.',
                ),
                reason: err.reason,
              },
              { status: 400 },

...
import { getRedisUrlOrThrow } from '@helios/shared/lib/redis/connection'
import { recordIndexerError } from '@helios/shared/lib/indexers/error-log'
import { recordIndexerLog } from '@helios/shared/lib/indexers/status-log'
import { createProgressBar } from '@helios/shared/lib/cli/progress'
import type { EntityManager } from '@mikro-orm/postgresql'
import { reindexEntity, DEFAULT_REINDEX_PARTITIONS } from '@helios/core/modules/query_index/lib/reindexer'
import { writeCoverageCounts } from '@helios/core/modules/query_index/lib/coverage'
...
import type { QueuedJob, JobContext } from '@helios/queue'
...
function isIndexerVerbose(): boolean {
  const parsed = parseBooleanToken(process.env.HELIOS_INDEXER_VERBOSE ?? '')
  return parsed === true
}
...
async function searchCommand(rest: string[]): Promise<void> {
  ...
  if (!query) {
    console.error('Usage: yarn helios search query --query "search terms" --tenant <id> [options]')
...
async function statusCommand(): Promise<void> {
  ...
  try {
    ...
    console.log(`  HELIOS_SEARCH_ENABLED: ${process.env.HELIOS_SEARCH_ENABLED ?? 'true (default)'}`)
    console.log('')
  } finally {
    try {
      const em = container.resolve('em') as any
...
async function indexCommand(rest: string[]): Promise<void> {
  ...
  if (!entityId || !recordId || !tenantId) {
    console.error('Usage: yarn helios search index --entity <entityId> --record <recordId> --tenant <tenantId>')
...
const reindexHelpCli: ModuleCli = {
  ...
  async run() {
    console.log('Usage: yarn helios search reindex [options]')
    console.log('')
    ...
    console.log('  search.reindex) with a running `yarn helios search worker fulltext-indexing`.')
    console.log('')
...
async function workerCommand(rest: string[]): Promise<void> {
  ...
  if (!queueName || !validQueues.includes(queueName)) {
    console.error('\nUsage: yarn helios search worker <queue-name> [options]\n')
    ...
    console.error('\nExamples:')
    console.error(`  yarn helios search worker ${VECTOR_INDEXING_QUEUE_NAME} --concurrency=10`)
    console.error(`  yarn helios search worker ${FULLTEXT_INDEXING_QUEUE_NAME} --concurrency=5`)
    return
  }
  ...
  const { runWorker } = await import('@helios/queue/worker')
...
const helpCli: ModuleCli = {
  command: 'help',
  async run() {
    console.log('\nUsage: yarn helios search <command> [options]\n')
    ...
    console.log('\nExamples:')
    console.log('  yarn helios search status')
    console.log('  yarn helios search query --query "john doe" --tenant tenant-123')
    console.log('  yarn helios search index --entity customers:customer_person_profile --record abc123 --tenant tenant-123')
    console.log('  yarn helios search reindex --tenant tenant-123 --entity customers:customer_person_profile')
    console.log('  yarn helios search test-meilisearch')
    console.log(`  yarn helios search worker ${VECTOR_INDEXING_QUEUE_NAME} --concurrency=10`)
    console.log(`  yarn helios search worker ${FULLTEXT_INDEXING_QUEUE_NAME} --concurrency=5`)
  },
}

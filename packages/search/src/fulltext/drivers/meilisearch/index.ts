...
import type { SearchFieldPolicy } from '@helios/shared/modules/search'
import { resolveTimeoutMs } from '@helios/shared/lib/http/fetchWithTimeout'
import type {
  ...
  function buildFilters(options: FullTextSearchQuery): string[] {
    ...
    if (organizationId) {
      ...
      if (organizationIds.length === 0) {
        filters.push('_organizationId = "__helios_no_matching_organization__"')
      } else {

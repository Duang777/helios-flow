...
describe('search debug utilities', () => {
  const originalDebugEnv = process.env.HELIOS_SEARCH_DEBUG

  const restoreDebugEnv = () => {
    if (originalDebugEnv === undefined) {
      delete process.env.HELIOS_SEARCH_DEBUG
      return
    }
    process.env.HELIOS_SEARCH_DEBUG = originalDebugEnv
  }
  ...
  describe('isSearchDebugEnabled', () => {
    it.each(['1', 'true', 'TRUE', 'Yes', 'on'])('returns true for %s', (value) => {
      process.env.HELIOS_SEARCH_DEBUG = value
  ...
  describe('searchDebugWarn', () => {
    it('does not warn when debug is disabled', () => {
      process.env.HELIOS_SEARCH_DEBUG = 'false'
    ...
    it('warns with the message and payload when enabled', () => {
      process.env.HELIOS_SEARCH_DEBUG = 'yes'

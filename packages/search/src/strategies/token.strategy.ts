...
export class TokenSearchStrategy implements SearchStrategy {
  ...
  async search(query: string, options: SearchOptions): Promise<SearchResult[]> {
    ...
    const { tokenizeText } = await import('@helios/shared/lib/search/tokenize')
    const { resolveSearchConfig } = await import('@helios/shared/lib/search/config')
  ...
  async index(record: IndexableRecord): Promise<void> {
    ...
    const { replaceSearchTokensForRecord } = await import(
      '@helios/core/modules/query_index/lib/search-tokens'
    )

...
describe('assertSafeOllamaBaseUrl', () => {
  beforeEach(() => {
    ...
    delete process.env.HELIOS_SEARCH_OLLAMA_BASE_URL_ALLOWLIST
    delete process.env.HELIOS_SEARCH_OLLAMA_ALLOW_PRIVATE
  ...
  describe('allowlist overrides', () => {
    ...
    it('accepts host listed in HELIOS_SEARCH_OLLAMA_BASE_URL_ALLOWLIST (host only)', () => {
      process.env.HELIOS_SEARCH_OLLAMA_BASE_URL_ALLOWLIST = 'ollama.internal.example.com'
      expect(() =>
    ...
    it('accepts host:port match in allowlist', () => {
      process.env.HELIOS_SEARCH_OLLAMA_BASE_URL_ALLOWLIST = 'ollama.internal.example.com:11434'
      expect(() =>
        assertSafeOllamaBaseUrl('http://ollama.internal.example.com:11434/'),
      ).not.toThrow()
    })
    ...
    it('HELIOS_SEARCH_OLLAMA_ALLOW_PRIVATE relaxes private-IP check', () => {
      process.env.HELIOS_SEARCH_OLLAMA_ALLOW_PRIVATE = 'true'
    ...
    it('HELIOS_SEARCH_OLLAMA_ALLOW_PRIVATE still enforces scheme allowlist', () => {
      process.env.HELIOS_SEARCH_OLLAMA_ALLOW_PRIVATE = 'true'
      expect(reasonOf(() => assertSafeOllamaBaseUrl('file:///etc/passwd'))).toBe(
        'forbidden_protocol',
      )
    ...
    it('HELIOS_SEARCH_OLLAMA_ALLOW_PRIVATE still rejects credentials in URL', () => {
      process.env.HELIOS_SEARCH_OLLAMA_ALLOW_PRIVATE = 'true'
...
describe('getOllamaBaseUrlAllowlist', () => {
  ...
  it('parses comma-separated entries and lowercases them', () => {
    process.env.HELIOS_SEARCH_OLLAMA_BASE_URL_ALLOWLIST = 'Foo.Example.com, Bar:443 ,  baz '

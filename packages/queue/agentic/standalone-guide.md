...
`@helios/queue` provides background job processing. MUST NOT implement custom job queues or polling loops.

## Strategy Selection

| Strategy | When | Config |
|----------|------|--------|
| Local | Development — processes from `.helios/queue/` | `QUEUE_STRATEGY=local` |

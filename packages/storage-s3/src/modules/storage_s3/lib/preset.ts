...
export function readS3EnvPreset(env: NodeJS.ProcessEnv = process.env): S3EnvPreset | null {
  const credentialKeys = {
    accessKeyId: ['HELIOS_INTEGRATION_STORAGE_S3_ACCESS_KEY_ID'],
    secretAccessKey: ['HELIOS_INTEGRATION_STORAGE_S3_SECRET_ACCESS_KEY'],
    sessionToken: ['HELIOS_INTEGRATION_STORAGE_S3_SESSION_TOKEN'],
    region: ['HELIOS_INTEGRATION_STORAGE_S3_REGION'],
    bucket: ['HELIOS_INTEGRATION_STORAGE_S3_BUCKET'],
  } as const
  ...
  if (!accessKeyId || !secretAccessKey || !region || !bucket) {
    throw new Error(
      '[storage_s3] Incomplete S3 env preset. Set HELIOS_INTEGRATION_STORAGE_S3_ACCESS_KEY_ID, ' +
        'HELIOS_INTEGRATION_STORAGE_S3_SECRET_ACCESS_KEY, HELIOS_INTEGRATION_STORAGE_S3_REGION, ' +
        'and HELIOS_INTEGRATION_STORAGE_S3_BUCKET.',
    )
  }

  const endpoint = readEnvValue(env, ['HELIOS_INTEGRATION_STORAGE_S3_ENDPOINT'])
  const forcePathStyle = readBooleanEnv(env, ['HELIOS_INTEGRATION_STORAGE_S3_FORCE_PATH_STYLE'])
  ...
  return {
    ...
    force:
      readBooleanEnv(env, ['HELIOS_INTEGRATION_STORAGE_S3_FORCE_PRECONFIGURE']) ?? false,
  }
}
...
export async function applyS3EnvPreset(params: {
  ...
  if (!force) {
    ...
    if (existing) {
      return {
        status: 'skipped',
        reason: 'S3 credentials already exist. Use HELIOS_INTEGRATION_STORAGE_S3_FORCE_PRECONFIGURE=true to overwrite.',
      }
    }
  }

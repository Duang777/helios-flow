...
`@helios/events` provides event-driven communication between modules. Use events for side effects — never direct module-to-module function calls.
...
|----------------|-----------|------------|
| `local` (dev) | In-process | `.helios/queue/` filesystem |

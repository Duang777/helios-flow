import { registerCliModules, getCliModules } from '@helios/shared/modules/registry'
...
describe('Events Worker', () => {
  ...
  describe('handle', () => {
    ...
    const ORIG_SINGLE_DELIVERY = process.env.HELIOS_EVENTS_SINGLE_DELIVERY
    beforeEach(() => {
      process.env.HELIOS_EVENTS_SINGLE_DELIVERY = 'false'
    })
    afterEach(() => {
      if (ORIG_SINGLE_DELIVERY === undefined) delete process.env.HELIOS_EVENTS_SINGLE_DELIVERY
      else process.env.HELIOS_EVENTS_SINGLE_DELIVERY = ORIG_SINGLE_DELIVERY
    })
  ...
  describe('single-delivery dispatch (HELIOS_EVENTS_SINGLE_DELIVERY) — issue #2960', () => {
    const origFlag = process.env.HELIOS_EVENTS_SINGLE_DELIVERY

    afterEach(() => {
      if (origFlag === undefined) delete process.env.HELIOS_EVENTS_SINGLE_DELIVERY
      else process.env.HELIOS_EVENTS_SINGLE_DELIVERY = origFlag
    })
    ...
    it('flag ON: dispatches wildcard persistent subscribers that exact-match never reached', async () => {
      process.env.HELIOS_EVENTS_SINGLE_DELIVERY = 'true'
      clearListenerCache()
      const calls: string[] = []
      registerCliModules([{
        id: 'm',

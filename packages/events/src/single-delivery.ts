...
export const EVENTS_SINGLE_DELIVERY_ENV = 'HELIOS_EVENTS_SINGLE_DELIVERY'
/**
 * Operator acknowledgment that an events worker runs OUT OF PROCESS (separate
 * `helios queue worker` container/process), so the server bootstrap must not
 * disable single-delivery just because it does not auto-spawn workers itself.
 */
export const EVENTS_EXTERNAL_WORKER_ENV = 'HELIOS_EVENTS_EXTERNAL_WORKER'
...
export function reconcileSingleDelivery(input: {
  ...
  return {
    ...
    warning:
      ...
      `worker (\`helios queue worker events\`) and set ${EVENTS_EXTERNAL_WORKER_ENV}=true, or enable ` +
      `AUTO_SPAWN_WORKERS.`,
  }
}

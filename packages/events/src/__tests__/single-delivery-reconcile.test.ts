...
} from '@helios/events/single-delivery'
...
describe('isSingleDeliveryRequested', () => {
  ...
  it('honors an explicit false token', () => {
    expect(isSingleDeliveryRequested({ HELIOS_EVENTS_SINGLE_DELIVERY: 'false' })).toBe(false)
    expect(isSingleDeliveryRequested({ HELIOS_EVENTS_SINGLE_DELIVERY: '0' })).toBe(false)
  })
})
...
describe('isExternalWorkerAcknowledged', () => {
  it('defaults off and reads truthy tokens', () => {
    ...
    expect(isExternalWorkerAcknowledged({ HELIOS_EVENTS_EXTERNAL_WORKER: 'true' })).toBe(true)
  })
})

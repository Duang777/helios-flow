...
jest.mock('@helios/queue', () => ({
  createQueue: (...args: unknown[]) => createQueueMock(...args),
}))

import { createEventBus } from '@helios/events/index'

const PRODUCER_QUEUE_KEY = '__heliosEventsProducerQueues__'
const PRODUCER_SHUTDOWN_KEY = '__heliosEventsProducerShutdown__'
...
describe('persistent-events producer memoization (#2959)', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.HELIOS_EVENTS_SHARED_PRODUCER
  })
  ...
  test('kill switch HELIOS_EVENTS_SHARED_PRODUCER=0 restores per-bus producers', async () => {
    process.env.HELIOS_EVENTS_SHARED_PRODUCER = '0'
    const busA = createEventBus({ resolve, queueStrategy: 'async' })
    const busB = createEventBus({ resolve, queueStrategy: 'async' })

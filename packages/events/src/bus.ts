...
import type { Queue } from '@helios/queue'
...
import { matchEventPattern } from '@helios/shared/lib/events/patterns'
import { getRedisUrlOrThrow } from '@helios/shared/lib/redis/connection'
import { isBroadcastEvent } from '@helios/shared/modules/events'
import {
  inferModuleIdFromResourceId,
  withModuleResourceUsage,
} from '@helios/shared/lib/modules/resource-usage'
...
const GLOBAL_EVENT_TAPS_KEY = '__heliosEventBusGlobalTaps__'
...
const EVENTS_PRODUCER_QUEUE_KEY = '__heliosEventsProducerQueues__'
const EVENTS_PRODUCER_SHUTDOWN_KEY = '__heliosEventsProducerShutdown__'

function isSharedProducerEnabled(): boolean {
  return parseBooleanWithDefault(process.env.HELIOS_EVENTS_SHARED_PRODUCER, true)
}
...
export function createEventBus(opts: CreateBusOptions): EventBus {
   ...
   * base dir). Set `HELIOS_EVENTS_SHARED_PRODUCER=0` to fall back to per-bus producers.
   */

...
import { registerWorkerShutdownHook, runWorker } from '@helios/queue/worker'
import type { Module, ModuleWorker } from '@helios/shared/modules/registry'
...
import { getSslConfig } from '@helios/shared/lib/db/ssl'
import { getRedisUrl, getRedisUrlOrThrow } from '@helios/shared/lib/redis/connection'
...
function shouldEmbedLocalSchedulerInSharedWorker(env: NodeJS.ProcessEnv = process.env): boolean {
  return parseBooleanToken(env.HELIOS_DEV_EMBED_SCHEDULER_IN_SHARED_WORKER) === true
}
...
// Next.js gives up on `.helios/next/dev/lock` after 1000ms, so a dev server
...
function formatCliFailureMessage(modName: string, cmdName: string, error: unknown): string {
  ...
  if (isDatabaseBackedRuntimeCommand && databaseIssue) {
    return `${databaseIssue.target} is not reachable: it ${databaseIssue.reason}. This command needs PostgreSQL. Start the database service or fix DATABASE_URL in .env, then retry \`yarn helios ${modName} ${cmdName}\`.`
  }
...
function removeTurbopackDevCache(appDir: string): void {
  fs.rmSync(path.join(appDir, '.helios', 'next', 'dev'), { recursive: true, force: true })
}
...
function writeDevSplashChildState(state: Record<string, unknown>): void {
  if (process.env.HELIOS_DEV_SPLASH_RUNTIME_WRAPPER === '1') return
  const stateFile = process.env.HELIOS_DEV_SPLASH_CHILD_STATE_FILE
  if (!stateFile?.trim()) return

  try {
    fs.mkdirSync(path.dirname(stateFile), { recursive: true })
    fs.writeFileSync(stateFile, `${JSON.stringify({
      mode: process.env.HELIOS_DEV_SPLASH_MODE || 'dev',
      failed: false,
      failureLines: [],
      failureCommand: null,
      ...state,
...
function resolveSplashProgressFallback(): { current: number; total: number } {
  const current = Number.parseInt(process.env.HELIOS_DEV_SPLASH_STAGE_CURRENT ?? '', 10)
  const total = Number.parseInt(process.env.HELIOS_DEV_SPLASH_STAGE_TOTAL ?? '', 10)
  if (Number.isFinite(current) && Number.isFinite(total) && total > 0) {
    return { current, total }
  }
  if (process.env.HELIOS_DEV_SPLASH_MODE === 'greenfield' || process.env.HELIOS_DEV_SPLASH_MODE === 'setup') {
...
function resolveDevWarmupReadyTimeoutMs(environment: NodeJS.ProcessEnv = process.env): number {
  const parsed = Number.parseInt(environment.HELIOS_DEV_WARMUP_READY_TIMEOUT_MS ?? '', 10)
...
async function resolveWorkerBudgetPlan(
  ...
  const { resolvePoolConfig } = await import('@helios/shared/lib/db/mikro')
  ...
  if (plan.clamped) {
    ...
    console.warn(
      `[worker] Worker concurrency clamped to fit the DB connection budget (${plan.budget}): ${perQueue}. ` +
        `Raise DB_POOL_MAX or set HELIOS_WORKERS_DB_CONNECTION_BUDGET to change this. ` +
...
function ensureNextBuildIdInConfiguredDistDir(appDir: string): void {
  const configuredDistDir = path.join(appDir, '.helios', 'next')
  ...
  if (configuredBuildId) {
    if (!fs.existsSync(configuredBuildIdPath)) {
      ...
      console.warn('[server] Reconstructed BUILD_ID inside .helios/next from existing build artifacts.')
    }
    return
  }
  ...
  console.warn(
    '[server] Recovered BUILD_ID from .next build artifacts into .helios/next to match the configured distDir.',
  )
}
...
async function handleDirectEjectCommand(args: string[]): Promise<number> {
  ...
  if (isList || !moduleId) {
    ...
    if (ejectable.length === 0) {
      ...
      console.log('\nUsage: yarn helios eject <moduleId>')
    }
    return 0
  }
  ...
  console.log('  1. Run generators:  yarn helios generate all')
...
/**
 * Generator suite invoked by both `helios generate all` and the in-process
 * generate watcher embedded in `helios server dev`. Hoisted to module scope
...
export async function run(argv = process.argv) {
  ...
  if (first === 'init') {
    ...
    console.log('🚀 Initializing Helios app...\n')

    try {
      const initArgs = parts.slice(1).filter(Boolean)
      const reinstall = initArgs.includes('--reinstall') || initArgs.includes('-r')
      process.env.HELIOS_INIT_FLOW = 'true'
      if (reinstall) {
        process.env.HELIOS_INIT_REINSTALL = 'true'
      } else if (process.env.HELIOS_INIT_REINSTALL) {
        delete process.env.HELIOS_INIT_REINSTALL
      }
        ...
        try {
          ...
          if (hasUsersTable) {
            ...
            if (Number.isFinite(existingUsersCount) && existingUsersCount > 0) {
              ...
              console.error(
                '   To reset and initialize from scratch, run: yarn helios init --reinstall',
              )
      ...
      const { bootstrapFromAppRoot } = await import('@helios/shared/lib/bootstrap/dynamicLoader')
      ...
      const defaultEmail = readEnvDefault('HELIOS_INIT_SUPERADMIN_EMAIL') ?? 'superadmin@acme.com'
      const defaultPassword = readEnvDefault('HELIOS_INIT_SUPERADMIN_PASSWORD') ?? 'secret'
      ...
      if (adminEmailDerived && derivedSecrets.adminPassword) {
        process.env.HELIOS_INIT_ADMIN_PASSWORD = derivedSecrets.adminPassword
      }
      if (employeeEmailDerived && derivedSecrets.employeePassword) {
        process.env.HELIOS_INIT_EMPLOYEE_PASSWORD = derivedSecrets.employeePassword
      }
      ...
      const setupArgs = [
        ...
        '--password', password,
        '--roles', roles,
        // `helios init` is the dev/demo bootstrap flow — it explicitly wants
        // the derived admin@/employee@ demo accounts. Standalone callers of
        // `helios auth setup` must opt in themselves; without this flag the
      ...
      if (orgId && tenantId) {
        ...
        const { createRequestContainer } = await import('@helios/shared/lib/di/container')
        ...
        const { ensureCustomRoleAcls } = await import('@helios/core/modules/auth/lib/setup-app')
  ...
  if (first === 'module') {
    try {
      ...
      if (!subcommand || subcommand === 'help' || subcommand === '--help' || subcommand === '-h') {
        console.log('Usage: yarn helios module <add|enable|eject> ...')
        console.log('  yarn helios module add <packageSpec> [--module <moduleId>] [--eject] [--allow-third-party]')
        console.log('  yarn helios module enable <packageName> [--module <moduleId>] [--eject] [--allow-third-party]')
        console.log('  yarn helios module eject <moduleId>')
        return 0
      }
      ...
      if (subcommand === 'add') {
        ...
        if (!packageSpec) {
          console.error('Usage: yarn helios module add <packageSpec> [--module <moduleId>] [--eject] [--allow-third-party]')
          return 1
        }
        ...
        console.log('Next steps:')
        console.log('  1. Review generated files if needed: .helios/generated/')
      ...
      if (subcommand === 'enable') {
        ...
        if (!packageName) {
          console.error('Usage: yarn helios module enable <packageName> [--module <moduleId>] [--eject] [--allow-third-party]')
          return 1
        }
  ...
  if (first === 'umes:inspect') {
    ...
    if (!moduleArg) {
      console.error('Usage: yarn helios umes:inspect --module <moduleId>')
      return 1
    }
  ...
  if (first === 'seed:defaults') {
    ...
    try {
      const [{ bootstrapFromAppRoot }, { createResolver }] = await Promise.all([
        import('@helios/shared/lib/bootstrap/dynamicLoader'),
        import('./lib/resolver'),
      ])
      const resolver = createResolver()
      ...
      const { Organization } = await import('@helios/core/modules/directory/data/entities')
  ...
  // Handle 'helios generate' without subcommand - default to 'generate all'
    ...
    cli: [
      {
        ...
        run: async (args: string[]) => {
          ...
          if (!queueName && !isAllQueues) {
            console.error('Usage: helios queue worker <queueName> | --all')
            console.error('Example: helios queue worker events')
            console.error('Example: helios queue worker --all')
          ...
          if (isAllQueues) {
            ...
            if (discoveredQueues.length === 0) {
              ...
              console.error('[worker] Run `yarn generate` and verify `.helios/generated/modules.cli.generated.ts` contains worker entries.')
              return
            }
      ...
      },
      {
        ...
        run: async (args: string[]) => {
          ...
          if (!queueName) {
            console.error('Usage: helios queue clear <queueName>')
            return
          }

          const strategyEnv = process.env.QUEUE_STRATEGY || 'local'
          const { createQueue } = await import('@helios/queue')
      ...
      },
      {
        ...
        run: async (args: string[]) => {
          const queueName = args[0]
          if (!queueName) {
            console.error('Usage: helios queue status <queueName>')
            return
          }
    ...
    cli: [
      {
        ...
        run: async (args: string[]) => {
          ...
          if (!eventName) {
            console.error('Usage: helios events emit <event> [jsonPayload] [--persistent|-p]')
            return
          }
    ...
    cli: [
      {
        ...
        run: async () => {
          ...
          async function cleanupAndWait() {
            ...
            const lockFile = path.join(appDir, '.helios', 'next', 'dev', 'lock')
            try {
          ...
          console.log('[server] Starting Helios in dev mode...')
          ...
          const heliosBin = resolveInstalledBinary(nodeModulesBases, '@helios/cli/bin/helios')
            ...
            const exitPromise = new Promise<ManagedProcessExitResult>((resolve) => {
              ...
              nextProcess.on('exit', async (code, signal) => {
                if (!didRetryCorruptedTurbopackCache && isTurbopackCacheCorruption(combinedOutput)) {
                  ...
                  console.log('[server] Detected corrupted Turbopack dev cache. Clearing .helios/next/dev and restarting Next.js once...')
          ...
          try {
            while (!stopping) {
              ...
              // user overrides.
              if (runtimeEnv.HELIOS_LOG_PRETTY === undefined) runtimeEnv.HELIOS_LOG_PRETTY = '1'
              if (runtimeEnv.HELIOS_LOG_LEVEL === undefined) runtimeEnv.HELIOS_LOG_LEVEL = 'debug'
              ...
              const startBackgroundServices = async () => {
                ...
                const warmupReady = await waitForDevWarmupReadyFile(process.env.HELIOS_DEV_WARMUP_READY_FILE, {
                ...
                if (autoSpawnWorkersMode !== 'off') {
                  ...
                  if (discoveredWorkerQueues.length === 0) {
                    console.error('[server] AUTO_SPAWN_WORKERS is enabled, but no queues were discovered from CLI modules. Run `yarn generate` and verify `.helios/generated/modules.cli.generated.ts` contains worker entries. Continuing without auto-spawned workers.')
                    ...
                    const lazyModeHint = lazySpawnMode === 'shared'
                      ? 'shared worker mode — one `queue worker --all` process; set HELIOS_AUTO_SPAWN_WORKERS_LAZY_MODE=per-queue to change'
                      : 'per-queue worker mode — one process per queue; set HELIOS_AUTO_SPAWN_WORKERS_LAZY_MODE=shared to change'
                    ...
                    const workerProcess = spawn('node', [heliosBin, 'queue', 'worker', '--all'], {
                      stdio: 'inherit',
                      env: runtimeEnv,
                      cwd: appDir,
                    })
                ...
                if (embedSchedulerInSharedWorker && activeLazySupervisor) {
                  ...
                  if (schedulerCommand.status !== 'ok') {
                    ...
                    const schedulerProcess = spawn('node', [heliosBin, 'scheduler', 'start'], {
                      stdio: 'inherit',
                      env: runtimeEnv,
                      cwd: appDir,
                    })
              ...
              if (generateWatcherMode === 'in-process') {
                ...
                // instead of spawning a dedicated `helios generate watch --skip-initial`
                // sidecar. Saves ~190 MB of resident RSS on a typical dev box
                // (measured against the legacy sidecar). Opt back into the
                // sidecar with `HELIOS_DEV_GENERATE_WATCH_MODE=legacy` if needed.
                ...
                activeGenerateWatcher = startInProcessGenerateWatcher({
                  ...
                  // initial `helios generate` before reaching the server
                ...
                console.log('[server] Legacy out-of-process generate watcher selected via HELIOS_DEV_GENERATE_WATCH_MODE=legacy — expect the dev orchestrator to spawn `helios generate watch --skip-initial`.')
              }
      ...
      },
      {
        ...
        run: async () => {
          ...
          console.log('[server] Starting Helios in production mode...')
  ...
  const quietBanner = process.env.HELIOS_CLI_QUIET === '1'
  const banner = '🧩 Helios CLI'
  if (!quietBanner) {
    const header = [
      '╔═══════════════════════╗',
      `║  ${banner.padEnd(21)}║`,
  ...
  if (!modName || modName === 'help' || modName === '--help' || modName === '-h') {
    console.log(pad('Usage: ✨ helios <module> <command> [args]'))
    const list = all

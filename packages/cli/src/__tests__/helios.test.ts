import type { Module } from '@helios/shared/modules/registry'
import {
  registerCliModules,
  getCliModules,
  hasCliModules,
...
describe('helios CLI module registration', () => {
  beforeEach(() => {
  ...
  describe('getCliModules', () => {
    it('returns empty array when no modules registered', () => {
      ...
      const { getCliModules: freshGetCliModules } = jest.requireActual('../helios')
  ...
  describe('hasCliModules', () => {
    it('returns false when no modules registered', () => {
      const { hasCliModules: freshHasCliModules } = jest.requireActual('../helios')
...
describe('db command failure output', () => {
  ...
  beforeEach(() => {
    ...
    process.env.DATABASE_URL = 'postgres://postgres:secret@127.0.0.1:5432/helios'
  })
  ...
  it('shows a targeted message when db:migrate cannot reach postgres', async () => {
    ...
    const exitCode = await run(['node', 'helios', 'db', 'migrate'])

    expect(exitCode).toBe(1)
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      '💥 Failed: PostgreSQL at 127.0.0.1:5432/helios is not reachable: it refused the connection. Start the database service or fix DATABASE_URL in .env, then retry `yarn db:migrate`.',
    )
  ...
  it('does not load app CLI while dispatching built-in db commands', async () => {
    ...
    try {
      ...
      const exitCode = await run(['node', 'helios', 'db', 'generate'])
  ...
  it('does not import the DI container module while dispatching built-in db commands', async () => {
    ...
    try {
      jest.resetModules()
      jest.doMock('@helios/shared/lib/di/container', () => {
        throw new Error('di container should stay lazy for built-in db commands')
      })

      const helios = await import('../helios')
      const dbGenerate = jest.fn().mockResolvedValue(undefined)

      helios.registerCliModules([
        {
          id: 'db',
          cli: [{ command: 'generate', run: dbGenerate }],
        } as any,
      ...
      const exitCode = await helios.run(['node', 'helios', 'db', 'generate'])

      expect(exitCode).toBe(0)
      expect(dbGenerate).toHaveBeenCalled()
    } finally {
      jest.dontMock('@helios/shared/lib/di/container')
      jest.resetModules()
      consoleErrorSpy.mockRestore()
      consoleLogSpy.mockRestore()
    }
  ...
  it('falls back to nested error messages when a command throws an aggregate error with an empty top-level message', async () => {
    ...
    const exitCode = await run(['node', 'helios', 'configs', 'cache', 'structural'])

    expect(exitCode).toBe(1)
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      '💥 Failed: PostgreSQL at 127.0.0.1:5432/helios is not reachable: it refused the connection. This command needs PostgreSQL. Start the database service or fix DATABASE_URL in .env, then retry `yarn helios configs cache`.',
    )
...
describe('init command failure output', () => {
  ...
  afterEach(() => {
    ...
    jest.dontMock('@helios/shared/lib/bootstrap/dynamicLoader')
    jest.resetModules()
  })
  ...
  it('shows a targeted message when init cannot reach postgres', async () => {
    ...
    const helios = await import('../helios')
    const exitCode = await helios.run(['node', 'helios', 'init'])

    expect(exitCode).toBe(1)
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      '❌ Initialization failed:',
      'PostgreSQL at 127.0.0.1:5432/helios is not reachable: it refused the connection. Start PostgreSQL or fix DATABASE_URL in .env, then retry `yarn initialize`.',
    )
  ...
  it('shows a DNS-focused message when init cannot resolve postgres host', async () => {
    const consoleErrorSpy = jest.spyOn(console, 'error').mockImplementation()
    const consoleLogSpy = jest.spyOn(console, 'log').mockImplementation()
    process.env.DATABASE_URL = 'postgres://postgres:secret@db.internal:5432/helios'
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      '❌ Initialization failed:',
      'PostgreSQL at db.internal:5432/helios is not reachable: it could not be resolved. Start PostgreSQL or fix DATABASE_URL in .env, then retry `yarn initialize`.',
    )
  ...
  it('keeps init successful when lean presets disable optional modules', async () => {
    ...
    jest.doMock('@helios/shared/lib/bootstrap/dynamicLoader', () => ({
    ...
    jest.doMock(
      '@helios/core/modules/auth/lib/setup-app',
      () => ({
...
describe('generate post-step structural invalidation', () => {
  ...
  it('uses the lightweight invalidation helper after successful generation', async () => {
    ...
    const generateEntityIds = jest.fn().mockResolvedValue({
      filesWritten: ['/tmp/test-app/.helios/generated/entities.ids.generated.ts'],
      filesUnchanged: [],
      errors: [],
    })
    ...
    const invalidate = jest.fn().mockResolvedValue({
      ...
      generatedFilesTouched: ['/tmp/test-app/.helios/generated/modules.generated.ts'],
      cacheError: null,
    ...
    jest.doMock('../lib/resolver', () => ({
      createResolver: () => ({
        getAppDir: () => '/tmp/test-app',
        loadEnabledModules: () => [{ id: 'configs', from: '@helios/core' }],
      }),
    }))
    ...
    const helios = await import('../helios')
    const exitCode = await helios.run(['node', 'helios', 'generate'])
  ...
  it('skips structural invalidation when every generated output is unchanged', async () => {
    ...
    const unchangedResult = {
      filesWritten: [],
      filesUnchanged: ['/tmp/test-app/.helios/generated/unchanged.generated.ts'],
      errors: [],
    }
    const generators = {
  ...
  it('skips cache infrastructure entirely when configs is not enabled', async () => {
    ...
    jest.doMock('../lib/resolver', () => ({
      createResolver: () => ({
        getAppDir: () => '/tmp/test-app',
        loadEnabledModules: () => [{ id: 'auth', from: '@helios/core' }],
      }),
    }))
    jest.doMock('../lib/post-generate-invalidation', () => {
...
describe('server dev managed process exits', () => {
  ...
  const originalLazy = process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY
  const originalLazyMode = process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY_MODE
  const originalLazyScheduler = process.env.HELIOS_AUTO_SPAWN_SCHEDULER_LAZY
  const originalGenerateWatchMode = process.env.HELIOS_DEV_GENERATE_WATCH_MODE
  const originalSingleDelivery = process.env.HELIOS_EVENTS_SINGLE_DELIVERY
  const originalExternalWorker = process.env.HELIOS_EVENTS_EXTERNAL_WORKER
  ...
  beforeEach(() => {
    ...
    delete process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY
    delete process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY_MODE
    delete process.env.HELIOS_AUTO_SPAWN_SCHEDULER_LAZY
    ...
    delete process.env.HELIOS_EVENTS_SINGLE_DELIVERY
    process.env.HELIOS_EVENTS_EXTERNAL_WORKER = 'true'
    ...
    // spawn surface.
    process.env.HELIOS_DEV_GENERATE_WATCH_MODE = 'legacy'
  })
  ...
  afterEach(() => {
    ...
    jest.resetModules()
    if (originalGenerateWatchMode === undefined) delete process.env.HELIOS_DEV_GENERATE_WATCH_MODE
    else process.env.HELIOS_DEV_GENERATE_WATCH_MODE = originalGenerateWatchMode
    delete process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY
    delete process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY_MODE
    delete process.env.HELIOS_AUTO_SPAWN_SCHEDULER_LAZY
  })
  ...
  afterAll(() => {
    ...
    if (originalLazy === undefined) delete process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY
    else process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY = originalLazy
    if (originalLazyMode === undefined) delete process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY_MODE
    else process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY_MODE = originalLazyMode
    if (originalLazyScheduler === undefined) delete process.env.HELIOS_AUTO_SPAWN_SCHEDULER_LAZY
    else process.env.HELIOS_AUTO_SPAWN_SCHEDULER_LAZY = originalLazyScheduler
    if (originalSingleDelivery === undefined) delete process.env.HELIOS_EVENTS_SINGLE_DELIVERY
    else process.env.HELIOS_EVENTS_SINGLE_DELIVERY = originalSingleDelivery
    if (originalExternalWorker === undefined) delete process.env.HELIOS_EVENTS_EXTERNAL_WORKER
    else process.env.HELIOS_EVENTS_EXTERNAL_WORKER = originalExternalWorker
  })
  ...
  it('skips scheduler auto-start when the module is not enabled', async () => {
    ...
    jest.doMock('node:fs', () => {
      ...
      return {
        ...
        existsSync: jest.fn((candidate: string) =>
          pathIncludes(candidate, 'next/dist/bin/next') || pathIncludes(candidate, '@helios/cli/bin/helios'),
        ),
        unlinkSync: jest.fn(),
      }
    })
    ...
    const helios = await import('../helios')
    const exitCode = await helios.run(['node', 'helios', 'server', 'dev'])
  ...
  it('fails loudly when a managed child exits cleanly but unexpectedly', async () => {
    ...
    const helios = await import('../helios')
    helios.registerCliModules([eventsWorkerFixture as Module])
  ...
  it.each([
    ...
    const cacheRemovals = rmSync.mock.calls.filter(([target]) =>
      pathIncludes(String(target), '.helios/next/dev'),
    )
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(
      '[server] Detected corrupted Turbopack dev cache. Clearing .helios/next/dev and restarting Next.js once...',
    )
  ...
  it('starts the lazy worker supervisor instead of `queue worker --all` when HELIOS_AUTO_SPAWN_WORKERS_LAZY=true', async () => {
    const consoleErrorSpy = jest.spyOn(console, 'error').mockImplementation()
    const consoleLogSpy = jest.spyOn(console, 'log').mockImplementation()
    process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY = 'true'
    process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY_MODE = 'shared'
    ...
    expect(lazyLogLine).toContain('HELIOS_AUTO_SPAWN_WORKERS_LAZY_MODE=per-queue')
  ...
  it('starts the lazy scheduler supervisor instead of the scheduler process when HELIOS_AUTO_SPAWN_SCHEDULER_LAZY=true', async () => {
    const consoleErrorSpy = jest.spyOn(console, 'error').mockImplementation()
    const consoleLogSpy = jest.spyOn(console, 'log').mockImplementation()
    process.env.AUTO_SPAWN_SCHEDULER = 'true'
    process.env.AUTO_SPAWN_WORKERS = 'false'
    process.env.HELIOS_AUTO_SPAWN_SCHEDULER_LAZY = 'true'
    ...
    const mercado = await import('../helios')
    mercado.registerCliModules([schedulerCliFixture as Module])

    const exitCode = await mercado.run(['node', 'helios', 'server', 'dev'])
...
describe('server start managed process exits', () => {
  ...
  it('skips scheduler auto-start when the module is not enabled', async () => {
    ...
    const helios = await import('../helios')
    const exitCode = await helios.run(['node', 'helios', 'server', 'start'])
  ...
  it('starts the lazy worker supervisor in production server when HELIOS_AUTO_SPAWN_WORKERS_LAZY=true', async () => {
    const consoleErrorSpy = jest.spyOn(console, 'error').mockImplementation()
    const consoleLogSpy = jest.spyOn(console, 'log').mockImplementation()
    process.env.HELIOS_AUTO_SPAWN_WORKERS_LAZY = 'true'

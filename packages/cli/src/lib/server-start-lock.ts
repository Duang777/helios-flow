...
export function getServerStartLockPath(appDir: string): string {
  return path.join(appDir, '.helios', 'server-start.lock')
}
  ...
  for (;;) {
    try {
      ...
      if (existingLock && existingLock.pid !== process.pid && isProcessAlive(existingLock.pid)) {
        ...
        throw new Error(
          `[server] Another Helios production server is already running for ${appDir} (pid ${existingLock.pid}${portSuffix}). Stop it before starting another instance.`,
        )
      }

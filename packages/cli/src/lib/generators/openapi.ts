 ...
 * and executing the bundle to call buildOpenApiDocument from @helios/shared.
 *
...
async function generateOpenApiViaBundle(
  ...
  const rootDir = resolver.getRootDir()
  const appDir = resolver.getAppDir()
  const sharedPackageRoot = resolver.getPackageRoot('@helios/shared')
  const corePackageRoot = resolver.getPackageRoot('@helios/core')
...
`
  ...
  const resolveProjectImportsPlugin = {
    ...
    setup(build: any) {
      // @helios/<pkg>/<path> → packageRoot/src/<path>.ts
      build.onResolve({ filter: /^@helios\// }, (args: any) => {
        const withoutScope = args.path.slice('@helios/'.length)
      ...
      // @/.helios/* → app/.helios/* (tsconfig paths)
      build.onResolve({ filter: /^@\/\.helios\// }, (args: any) => {
        const rest = args.path.slice('@/'.length) // '.helios/generated/...'
  ...
  const externalNonWorkspacePlugin = {
    ...
    setup(build: any) {
      build.onResolve({ filter: /^[^./]/ }, (args: any) => {
        ...
        if (args.path.startsWith('@helios/')) return undefined

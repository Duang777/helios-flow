...
function createStandaloneMockResolver(
  ...
  const outputDir = path.join(tmpDir, '.helios', 'generated')
  fs.mkdirSync(outputDir, { recursive: true })
...
function buildSudoOutput({ importSection, entriesLiteral }) {
  return '// AUTO-GENERATED by helios generate registry\\n'
    + "import type { SecuritySudoTarget, SecuritySudoTargetEntry } from '@helios/enterprise/modules/security'\\n"
    + (importSection ? '\\n' + importSection + '\\n' : '\\n') + '\\n'
...
describe('generateModuleRegistry with module subsets', () => {
  ...
  it('preserves exact legacy empty output format for the refactored registry files', async () => {
    ...
    expect(readGenerated(tmpDir, 'modules.generated.ts')).toBe(dedent(`
      ...
      import { createLazyModuleSubscriber, createLazyModuleWorker, type Module } from '@helios/shared/modules/registry'
    ...
    expect(readGenerated(tmpDir, 'frontend-routes.generated.ts')).toBe(dedent(`
      // AUTO-GENERATED by helios generate registry
      import { resolvePageRouteMetadata, type FrontendRouteManifestEntry } from '@helios/shared/modules/registry'
    ...
    expect(readGenerated(tmpDir, 'backend-routes.generated.ts')).toBe(dedent(`
      // AUTO-GENERATED by helios generate registry
      import { resolvePageRouteMetadata, type BackendRouteManifestEntry } from '@helios/shared/modules/registry'
  ...
  it('preserves exact legacy populated output format for the refactored registry files', async () => {
    ...
    const resolver = createMockResolver(tmpDir, [{ id: 'orders', from: '@helios/core' }])
    const result = await generateModuleRegistry({ resolver, quiet: true })
    ...
    expect(modulesGenerated).toContain(`import * as I0_orders from "@helios/core/modules/orders/index"`)
    ...
    expect(modulesGenerated).toContain(`Component: async (props: any) => { const mod = await import("@helios/core/modules/orders/frontend/page")`)
    expect(modulesGenerated).toContain(`Component: async (props: any) => { const mod = await import("@helios/core/modules/orders/backend/page")`)
    ...
    expect(readGenerated(tmpDir, 'frontend-routes.generated.ts')).toBe(dedent(`
      ...
      export const frontendRoutes: FrontendRouteManifestEntry[] = [
        { moduleId: "orders", ...resolvePageRouteMetadata("/", (undefined as any)), load: async () => { const mod = await import("@helios/core/modules/orders/frontend/page"); return (mod.default ?? mod) as any } }
      ]
    ...
    expect(readGenerated(tmpDir, 'backend-routes.generated.ts')).toBe(dedent(`
      ...
      export const backendRoutes: BackendRouteManifestEntry[] = [
        { moduleId: "orders", ...resolvePageRouteMetadata("/backend/orders", (undefined as any)), load: async () => { const mod = await import("@helios/core/modules/orders/backend/page"); return (mod.default ?? mod) as any } }
      ]
    ...
    expect(readGenerated(tmpDir, 'api-routes.generated.ts')).toBe(dedent(`
      ...
      export const apiRoutes: ApiRouteManifestEntry[] = [
        { moduleId: "orders", kind: "route-file", path: "/orders/orders", methods: ["GET"], load: async () => import("@helios/core/modules/orders/api/orders/route") }
      ]
  ...
  it('handles module with only subscribers — no pages or APIs', async () => {
    ...
    const enabled: ModuleEntry[] = [
      { id: 'notifications_only', from: '@helios/core' },
    ]
    const resolver = createMockResolver(tmpDir, enabled)
    const result = await generateModuleRegistry({ resolver, quiet: true })
  ...
  it('extracts subscriber metadata from source when runtime import fails', async () => {
    ...
    const enabled: ModuleEntry[] = [
      { id: 'subscriber_meta', from: '@helios/core' },
    ]
    const resolver = createMockResolver(tmpDir, enabled)
    const result = await generateModuleRegistry({ resolver, quiet: true })
  ...
  it('extracts worker metadata from imported queue constants when runtime import fails', async () => {
    ...
    const resolver = createMockResolver(tmpDir, [
      { id: 'worker_meta', from: '@helios/core' },
    ])
  ...
  it('extracts subscriber metadata from imported event objects when runtime import fails', async () => {
    ...
    const resolver = createMockResolver(tmpDir, [
      { id: 'subscriber_event_meta', from: '@helios/core' },
    ])
  ...
  it('keeps runtime metadata fallback for settings/profile route icons that are not serializable yet', async () => {
    ...
    expect(output).toMatch(/import \* as .* from "@helios\/core\/modules\/iconic\/backend\/settings\/page\.meta"/)
    expect(output).toMatch(/import \* as .* from "@helios\/core\/modules\/iconic\/backend\/profile\/page\.meta"/)
  ...
  it('handles module with only widgets — no pages or subscribers', async () => {
    ...
    const enabled: ModuleEntry[] = [
      { id: 'widgets_only', from: '@helios/core' },
    ]
    const resolver = createMockResolver(tmpDir, enabled)
    const result = await generateModuleRegistry({ resolver, quiet: true })
  ...
  it('handles module with only convention files — no walk-based scans', async () => {
    ...
    const enabled: ModuleEntry[] = [
      { id: 'config_mod', from: '@helios/core' },
    ]
    const resolver = createMockResolver(tmpDir, enabled)
    const result = await generateModuleRegistry({ resolver, quiet: true })
...
`
    ...
    const resolver = createMockResolver(tmpDir, [{ id: 'customers', from: '@helios/core' }])
    const result = await generateModuleRegistry({ resolver, quiet: true })
...
`
    ...
    const resolver = createMockResolver(tmpDir, [{ id: 'example', from: '@helios/core' }])
    const result = await generateModuleRegistry({ resolver, quiet: true })
...
`
    ...
    const resolver = createMockResolver(tmpDir, [{ id: 'shipping_carriers', from: '@helios/core' }])
    const result = await generateModuleRegistry({ resolver, quiet: true })
  ...
  it('generates correct output when full module is later disabled', async () => {
    ...
    const allEnabled: ModuleEntry[] = [
      { id: 'mod_a', from: '@helios/core' },
      { id: 'mod_b', from: '@helios/core' },
    ]
  ...
  it('escapes generated module specifiers for lazy imports', async () => {
    ...
    expect(result.errors).toEqual([])
    const output = readGenerated(tmpDir, 'modules.generated.ts')!
    expect(output).toContain(`createLazyModuleSubscriber(() => import("@helios/core/modules/safe_mod/subscribers/o'hare")`)
    expect(output).toContain(`createLazyModuleWorker(() => import("@helios/core/modules/safe_mod/workers/queue'oops")`)
    expect(output).toContain(`from "@helios/core/modules/safe_mod/api/get/quoted'route"`)
    expect(output).not.toContain(`import('@helios/core/modules/safe_mod/subscribers/o'hare')`)
    expect(output).not.toContain(`import('@helios/core/modules/safe_mod/workers/queue'oops')`)
  ...
  it('rejects unsafe generated module specifiers before emitting lazy imports', async () => {
    ...
    const baseResolver = createMockResolver(tmpDir, [
      { id: 'bad_spec', from: '@helios/core' },
    ])
    const resolver: PackageResolver = {
      ...baseResolver,
      getModuleImportBase: () => ({
        appBase: '@/modules/bad_spec',
        pkgBase: '@helios/core/modules/bad_spec";console.log(1)//',
      }),
    }
  ...
  it('fails fast when a third-party module entry points at a package without the expected OM module shape', async () => {
    ...
    // entry is almost certainly not an Helios module.
    ...
    await expect(generateModuleRegistry({ resolver, quiet: true })).rejects.toThrow(
      /no Helios module shape was found/,
    )
  })
  ...
  it('uses standalone source mirror i18n files when dist modules omit translation assets', async () => {
    ...
    const output = fs.readFileSync(
      path.join(tmpDir, '.helios', 'generated', 'modules.generated.ts'),
      'utf8',
    )

    expect(output).toContain('@helios/core/modules/customers/i18n/en.json')
...
export default async function handle(): Promise<void> {}
  ...
  it('handles disabling a module that was previously enabled', async () => {
    ...
    const all: ModuleEntry[] = [
      { id: 'keep_mod', from: '@helios/core' },
      { id: 'drop_mod', from: '@helios/core' },
    ]
  ...
  it('discovers workers from standalone dist/modules output', async () => {
    ...
    expect(result.errors).toEqual([])
    const outputPath = path.join(tmpDir, '.helios', 'generated', 'modules.cli.generated.ts')
    ...
    expect(output).toContain('createLazyModuleWorker(() => import("@helios/core/modules/data_sync/workers/sync-import")')
  })
  ...
  it('discovers CLI metadata and ACL from standalone dist/modules output', async () => {
    ...
    const outputPath = path.join(tmpDir, '.helios', 'generated', 'modules.cli.generated.ts')
    const output = fs.readFileSync(outputPath, 'utf8')
    expect(output).toContain('import CLI_configs from "@helios/core/modules/configs/cli"')
  ...
  it('preserves standalone worker and subscriber metadata when discovery uses a src mirror', async () => {
    ...
    expect(output).toContain('createLazyModuleWorker(() => import("@helios/core/modules/events/workers/events.worker")')
  })
  ...
  it('does not treat standalone page.meta files as page components', async () => {
    ...
    const modulesOutputPath = path.join(tmpDir, '.helios', 'generated', 'modules.generated.ts')
    const modulesOutput = fs.readFileSync(modulesOutputPath, 'utf8')
    expect(modulesOutput).toContain(
      'import * as BM',
    )
    expect(modulesOutput).not.toMatch(
      /import B\d+_[^,\n]*, \* as BM\d+_[^,\n]* from "@helios\/core\/modules\/iconic\/backend\/dashboard\/page\.meta"/,
    )

    const routesOutputPath = path.join(tmpDir, '.helios', 'generated', 'backend-routes.generated.ts')
    const routesOutput = fs.readFileSync(routesOutputPath, 'utf8')
    expect(routesOutput).toContain('@helios/core/modules/iconic/backend/dashboard/page')
    expect(routesOutput).not.toContain('@helios/core/modules/iconic/backend/dashboard/page.meta')
  ...
  it('ignores standalone helper files without default exports when scanning direct frontend/backend routes', async () => {
    ...
    const modulesOutputPath = path.join(tmpDir, '.helios', 'generated', 'modules.generated.ts')
    const modulesOutput = fs.readFileSync(modulesOutputPath, 'utf8')
    expect(modulesOutput).toContain('@helios/core/modules/auth/frontend/login')
    expect(modulesOutput).toContain('@helios/core/modules/auth/backend/users/list')
    expect(modulesOutput).not.toContain('@helios/core/modules/auth/frontend/login-injection')
    expect(modulesOutput).not.toContain('@helios/core/modules/auth/backend/users/roleOptions')

    const frontendRoutesOutputPath = path.join(tmpDir, '.helios', 'generated', 'frontend-routes.generated.ts')
    ...
    const backendRoutesOutputPath = path.join(tmpDir, '.helios', 'generated', 'backend-routes.generated.ts')
  ...
  it('prefers standalone src module mirrors over dist-only legacy api artifacts', async () => {
    ...
    const apiRoutesOutputPath = path.join(tmpDir, '.helios', 'generated', 'api-routes.generated.ts')
    const apiRoutesOutput = fs.readFileSync(apiRoutesOutputPath, 'utf8')
    expect(apiRoutesOutput).toContain('@helios/core/modules/payment_gateways/api/status/route')
    expect(apiRoutesOutput).not.toContain('@helios/core/modules/payment_gateways/api/get/payment-gateways/status')
  ...
  it('keeps package legacy method handlers while letting app files override matching paths', async () => {
    ...
    const resolver = createMockResolver(tmpDir, [
      { id: 'legacy_api', from: '@helios/core' },
    ])
    const result = await generateModuleRegistry({ resolver, quiet: true })
    ...
    expect(apiRoutesOutput).not.toContain('@helios/core/modules/legacy_api/api/get/status')
    expect(apiRoutesOutput).toContain('@helios/core/modules/legacy_api/api/get/details')
...
describe('generateModuleRegistryApp with module subsets', () => {
  ...
  it('app output excludes CLI commands and keeps lazy workers/subscribers', async () => {
    ...
    const enabled: ModuleEntry[] = [
      { id: 'runtime_mod', from: '@helios/core' },
    ]
    const resolver = createMockResolver(tmpDir, enabled)
    const result = await generateModuleRegistryApp({ resolver, quiet: true })
...
describe('all generated files are valid with varying subsets', () => {
  ...
  it('ai-agents.generated.ts is empty when no module provides ai-agents.ts', async () => {
    ...
    const resolver = createMockResolver(tmpDir, [
      { id: 'no_ai_agents', from: '@helios/core' },
    ])
    await generateModuleRegistry({ resolver, quiet: true })
  ...
  it('security generated registries are empty when no module provides security convention files', async () => {
    ...
    const resolver = createMockResolver(tmpDir, [
      { id: 'no_security', from: '@helios/core' },
    ])
    await generateModuleRegistry({ resolver, quiet: true })
  ...
  it('discovers security convention files into dedicated generated registries', async () => {
    ...
    const resolver = createMockResolver(tmpDir, [
      { id: 'security_ext', from: '@helios/core' },
    ])
    await generateModuleRegistry({ resolver, quiet: true })

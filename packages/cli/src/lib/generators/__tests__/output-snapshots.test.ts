...
`,
  ...
  touchFile(
    ...
    "const CUSTHELIOS_QUEUE = 'custom-app-tasks'\nexport const metadata = { queue: CUSTHELIOS_QUEUE, concurrency: 3 }\nexport default async function handler() {}\n",
  )
  touchFile(
...
describe('generator output compatibility', () => {
  ...
  it('bootstrap-registrations always registers backend route manifests (issue #1595)', async () => {
    ...
    expect(content).toContain(
      `import { registerBackendRouteManifests, registerFrontendRouteManifests } from '@helios/shared/modules/registry'`,
    )

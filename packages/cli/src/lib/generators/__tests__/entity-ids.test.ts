...
function createMockResolver(tmpRoot: string, enabled: ModuleEntry[]): PackageResolver {
  const outputDir = path.join(tmpRoot, 'app', '.helios', 'generated')
...
function createStandaloneMockResolver(tmpRoot: string, enabled: ModuleEntry[]): PackageResolver {
  const outputDir = path.join(tmpRoot, '.helios', 'generated')
  fs.mkdirSync(outputDir, { recursive: true })
  ...
  return {
    ...
    getModulePaths: (entry: ModuleEntry) => ({
      ...
      pkgBase: path.join(tmpRoot, 'node_modules', '@helios', 'core', 'dist', 'modules', entry.id),
    }),
    getModuleImportBase: (entry: ModuleEntry) => ({
      appBase: `@/modules/${entry.id}`,
      pkgBase: `@helios/core/modules/${entry.id}`,
    ...
    getPackageOutputDir: (packageName: string) =>
      packageName === '@app'
        ? outputDir
        : path.join(tmpRoot, 'node_modules', '@helios', 'core', 'generated'),
    getPackageRoot: (from?: string) =>
      from === '@app'
        ? tmpRoot
        : path.join(tmpRoot, 'node_modules', '@helios', 'core'),
  }
}
...
describe('generateEntityIds', () => {
  it('does not rewrite generated entity outputs when inputs are unchanged', async () => {
    const moduleEntry: ModuleEntry = { id: 'orders', from: '@helios/core' }
    ...
    const packageOutput = resolver.getPackageOutputDir('@helios/core')
...
`
    ...
    const registryPath = path.join(tmpDir, 'app', '.helios', 'generated', 'entity-fields-registry.ts')
  ...
  it('reuses package generated entity metadata in standalone mode', async () => {
    const moduleEntry: ModuleEntry = { id: 'directory', from: '@helios/core' }
    const packageRoot = path.join(tmpDir, 'node_modules', '@helios', 'core')
...
`
    ...
    const rootIdsPath = path.join(tmpDir, '.helios', 'generated', 'entities.ids.generated.ts')
    const organizationPath = path.join(tmpDir, '.helios', 'generated', 'entities', 'organization', 'index.ts')

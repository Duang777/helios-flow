...
describe('module-facts discovery (T1)', () => {
  ...
  it('returns package-provided .ts modules and skips .js-only installs (A1 + A3)', () => {
    ...
    const resolver = makeResolver([
      { name: '@helios/core', path: pkgSrc, modulesPath: path.join(pkgSrc, 'src', 'modules') },
      { name: '@helios/compiled', path: pkgDist, modulesPath: path.join(pkgDist, 'dist', 'modules') },
    ])
  ...
  it('tags each source with its providing package name', () => {
    ...
    const resolver = makeResolver([
      { name: '@helios/tagged', path: pkgSrc, modulesPath: path.join(pkgSrc, 'src', 'modules') },
    ])

    const [source] = discoverPackageModuleSources(resolver)
    expect(source).toMatchObject({ moduleId: 'gamma', from: '@helios/tagged' })
  ...
  it('dedupes duplicate module ids first-wins', () => {
    ...
    const resolver = makeResolver([
      { name: '@helios/dup-a', path: pkgA, modulesPath: path.join(pkgA, 'src', 'modules') },
      { name: '@helios/dup-b', path: pkgB, modulesPath: path.join(pkgB, 'src', 'modules') },
    ])

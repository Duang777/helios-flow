...
function readGenerated(tmpRoot: string): string {
  return fs.readFileSync(path.join(tmpRoot, 'app', '.helios', 'generated', 'entities.generated.ts'), 'utf8')
}
...
describe('generateModuleEntities', () => {
  it('prefers app data entities over package override files for package-backed modules', async () => {
    ...
    expect(output).not.toContain('@helios/core/modules/orders/data/entities.override')
  ...
  it('falls back to legacy db schema files when data entities are missing', async () => {
    const moduleEntry: ModuleEntry = { id: 'legacy_orders', from: '@helios/core' }
    ...
    expect(result.errors).toEqual([])
    expect(output).toContain('from "@helios/core/modules/legacy_orders/db/schema"')
  ...
  it('marks the generated file as unchanged when the checksum matches', async () => {
    ...
    const resolver = createMockResolver(tmpDir, [moduleEntry])
    const outFile = path.join(tmpDir, 'app', '.helios', 'generated', 'entities.generated.ts')
    const checksumFile = path.join(tmpDir, 'app', '.helios', 'generated', 'entities.generated.checksum')

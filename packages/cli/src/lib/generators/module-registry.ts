...
// npm scope (anything outside @app / @helios/*), verify the resolved
// pkgBase directory exists. If it doesn't, the package referenced via `from:`
// is almost certainly not an Helios module (or the id is wrong), and
...
function verifyThirdPartyModuleShape(entry: { id: string; from?: string }, pkgBase: string): void {
  ...
  if (!from || from === '@app' || from.startsWith('@helios/')) return
  if (fs.existsSync(pkgBase)) return
  throw new Error(
    `Module '${entry.id}' is registered with from: '${from}' but no Helios module shape was found at '${pkgBase}'. ` +
...
function renderCommandLoadersFile(entries: CommandLoaderGenerationEntry[]): string {
  ...
  return `// AUTO-GENERATED by helios generate command-loaders
import type { CommandLoader } from '@helios/shared/lib/commands'
...
function renderAstModuleRegistryFile(options: {
  ...
  addImportSpec(sourceFile, {
    moduleSpecifier: '@helios/shared/modules/registry',
    namedImports: [
...
function renderAstLegacyManifestOutput(options: {
  ...
  const importSection = [
    usesPageRouteMetadataHelper
      ? `import { resolvePageRouteMetadata, type ${options.typeName} } from '@helios/shared/modules/registry'`
      : `import type { ${options.typeName} } from '@helios/shared/modules/registry'`,
    ...(options.imports ?? []).map((entry) => serializeGeneratedImport(entry)),
  ].join('\n')
...
async function generateModuleRegistryFromDiscovery(options: ModuleRegistryRenderOptions): Promise<GeneratorResult> {
  ...
  const pluginRegistry = new Map<string, import('@helios/shared/modules/generators').GeneratorPlugin>()
  ...
  for (const discovered of discovery.modules) {
    ...
    try {
      ...
      const plugins: import('@helios/shared/modules/generators').GeneratorPlugin[] =
  ...
  for (const discovered of discovery.modules) {
    ...
    moduleDecls.push(`{
      ...
      ${extensionsImportName ? `entityExtensions: ((${extensionsImportName}.default ?? ${extensionsImportName}.extensions) as import('@helios/shared/modules/entities').EntityExtension[]) || [],` : ''}
      ...
      ${encryptionImportName ? `defaultEncryptionMaps: ((${encryptionImportName}.default ?? ${encryptionImportName}.defaultEncryptionMaps) as import('@helios/shared/modules/encryption').ModuleEncryptionMap[]) || [],` : ''}
      ${integrationImportName ? `integrations: (( ${integrationImportName}.integrations ?? (${integrationImportName}.integration ? [${integrationImportName}.integration] : []) ) as import('@helios/shared/modules/integrations/types').IntegrationDefinition[]),` : ''}
      ${integrationImportName ? `bundles: (( ${integrationImportName}.bundles ?? (${integrationImportName}.bundle ? [${integrationImportName}.bundle] : []) ) as import('@helios/shared/modules/integrations/types').IntegrationBundle[]),` : ''}
    }`)
  ...
  {
    const { detectConflicts } = await import('@helios/shared/lib/umes/conflict-detection')
  ...
  // regardless of Next.js server bundle isolation.
  {
    ...
    const bootstrapRegsOutput = `// AUTO-GENERATED by helios generate registry
    ...
    if (integrationImportName) {
      moduleEntries.push({
        ...
        value: buildModuleIntegrationListValue(
          ...
          'integrations',
          'integration',
          "import('@helios/shared/modules/integrations/types').IntegrationDefinition[]",
        ),
      })
      moduleEntries.push({
        name: 'bundles',
        value: buildModuleIntegrationListValue(
          ...
          'bundles',
          'bundle',
          "import('@helios/shared/modules/integrations/types').IntegrationBundle[]",
        ),
      })
    }

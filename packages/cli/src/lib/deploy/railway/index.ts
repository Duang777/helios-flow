...
function readAppPackage(cwd: string): {
  ...
  if (!existsSync(packagePath)) throw new Error('Run this command from an Helios app root.')
  ...
  if (!Object.keys(dependencies).some((key) => key.startsWith('@helios/'))) {
    throw new Error('The current package does not declare Helios dependencies.')
  }
  return {
...
async function resolveTokenInteractively(
  ...
  const persist = (await prompt.ask('Cache this token in ~/.config/helios/railway.json? [y/N] ')).trim().toLowerCase()
  ...
  try {
    const data = await input.client.request<{
      ...
      input: {
        ...
        description: 'Helios deployment managed by helios deploy railway',
...
async function removeDisabledWorker(input: {
  ...
  try {
    ...
    if (data.service.projectId !== input.projectId || data.service.name !== 'helios-worker') {
...
function printDryRun(input: {
  ...
  if (input.options.worker) console.log('    - create or reuse helios-worker')
  if (
    !input.options.worker
    && input.state.environments[input.options.environment]?.workerServiceId
  ) {
    console.log('    - delete the recorded managed helios-worker service')
  }
...
export async function runRailwayDeploy(
  ...
  try {
    ...
    console.log('')
    console.log('Helios deployed to Railway')
    ...
    if (workerServiceId) console.log('  Worker:      helios-worker (running)')
    console.log(`  State:       ${statePath}`)

...
test.describe('helios deploy railway live integration', () => {
  ...
  test('deploys, verifies health, resumes, and cleans up', async ({ request }) => {
    ...
    const projectName = `helios-integration-${Date.now()}`
    ...
    try {
      ...
      if (appRoot && existsSync(resolve(appRoot, '.helios', 'railway.json'))) {

...
export function railwayTokenConfigPath(
  ...
  if (platform === 'win32') {
    return join(environment.APPDATA || join(homedir(), 'AppData', 'Roaming'), 'helios', 'railway.json')
  }
  return join(environment.XDG_CONFIG_HOME || join(homedir(), '.config'), 'helios', 'railway.json')
}

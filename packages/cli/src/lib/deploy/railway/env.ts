...
export function computeRailwayVariables(input: {
  ...
  const variables: Record<string, string> = {
    ...
    PORT: '3000',
    HELIOS_AI_PROVIDER: input.env.HELIOS_AI_PROVIDER || 'openai',
    HELIOS_AI_MODEL: input.env.HELIOS_AI_MODEL || 'gpt-5-mini',
    QUEUE_STRATEGY: 'async',
    NEXT_PUBLIC_QUEUE_STRATEGY: 'async',
    AUTO_SPAWN_WORKERS: input.role === 'app' && !input.workerEnabled ? 'true' : 'false',
    HELIOS_AUTO_SPAWN_WORKERS: input.role === 'app' && !input.workerEnabled ? 'true' : 'false',
  }

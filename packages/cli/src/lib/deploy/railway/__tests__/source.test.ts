...
describe('Railway source strategy', () => {
  it.each([
    ['git@github.com:helios/example.git', 'helios/example'],
    ['https://github.com/helios/example.git', 'helios/example'],
    ['https://gitlab.com/helios/example.git', null],
  ...
  it('prefers a synchronized GitHub remote in auto mode', () => {
    const run = runner({
      ...
      'git remote get-url origin': 'git@github.com:helios/example.git',
  ...
  it('rejects a local upload ignore file without state protection', () => {
    const cwd = mkdtempSync(join(tmpdir(), 'helios-railway-ignore-'))
    writeFileSync(join(cwd, '.railwayignore'), '.env\n*.pem\n*.key\n.git\nnode_modules\n')
    expect(() => assertLocalUploadSafe(cwd)).toThrow('.helios/railway.json')
  })

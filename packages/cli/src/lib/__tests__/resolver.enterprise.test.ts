...
if (parseBooleanWithDefault(process.env.HELIOS_ENABLE_ENTERPRISE_MODULES, false)) {
  enabledModules.push(
...
describe('resolver enterprise module toggle', () => {
  const originalEnv = process.env.HELIOS_ENABLE_ENTERPRISE_MODULES
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.HELIOS_ENABLE_ENTERPRISE_MODULES = originalEnv
    }
  ...
  it('does not load enterprise modules when toggle is disabled', () => {
    process.env.HELIOS_ENABLE_ENTERPRISE_MODULES = 'false'
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'resolver-enterprise-'))
    writeModulesConfig(tempDir)

    const modules = createResolver(tempDir).loadEnabledModules()
    expect(modules).toEqual([{ id: 'customers', from: '@helios/core' }])
  })

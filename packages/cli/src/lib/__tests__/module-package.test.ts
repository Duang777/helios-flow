...
describe('module-package', () => {
  ...
  it('parses official package names from package specs', () => {
    expect(parsePackageNameFromSpec('@helios/test-package')).toBe('@helios/test-package')
    expect(parsePackageNameFromSpec('@helios/test-package@preview')).toBe('@helios/test-package')
    expect(parsePackageNameFromSpec('@helios/test-package@file:/tmp/pkg')).toBe('@helios/test-package')
  })

  it('reads and validates an official module package manifest', () => {
    const modulePackage = readOfficialModulePackageFromRoot(fixturePackageRoot, '@helios/test-package')

    expect(modulePackage.packageName).toBe('@helios/test-package')
  ...
  it('validates a third-party (non-@helios) module package by structure', () => {
  ...
  it('resolves a hoisted installed module package from a monorepo app', () => {
    const appDir = path.join(tmpDir, 'apps', 'helios')
    const installedPackageRoot = path.join(tmpDir, 'node_modules', '@helios', 'test-package')
  ...
  it('selects a specific module from a multi-module package via targetModuleId', () => {
    ...
    fs.writeFileSync(
      path.join(packageRoot, 'package.json'),
      JSON.stringify({ name: '@helios/core' }),
    )
  ...
  it('throws when a multi-module package is used without specifying a module', () => {
    ...
    fs.writeFileSync(
      path.join(packageRoot, 'package.json'),
      JSON.stringify({ name: '@helios/multi' }),
    )
  ...
  it('rejects --eject when a module imports files outside its module directory', () => {
    ...
    expect(() =>
      validateEjectBoundaries(
        readOfficialModulePackageFromRoot(invalidPackageRoot, '@helios/test-package'),
      ),
  ...
  it('ignores AppleDouble metadata files when validating --eject boundaries', () => {
    ...
    expect(() =>
      validateEjectBoundaries(
        readOfficialModulePackageFromRoot(packageRoot, '@helios/test-package'),
      ),
    ).not.toThrow()
  })
})

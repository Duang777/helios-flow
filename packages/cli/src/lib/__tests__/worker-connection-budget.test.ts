...
describe('resolveWorkerConnectionBudget', () => {
  ...
  it('honors a positive HELIOS_WORKERS_DB_CONNECTION_BUDGET override', () => {
    expect(
      resolveWorkerConnectionBudget(
        { HELIOS_WORKERS_DB_CONNECTION_BUDGET: '8' } as unknown as NodeJS.ProcessEnv,
        20,
      ),
    ).toBe(8)
  })
  ...
  it('falls back to the pool max for non-numeric or non-positive overrides', () => {
    for (const value of ['0', '-3', 'abc', '']) {
      expect(
        resolveWorkerConnectionBudget(
          { HELIOS_WORKERS_DB_CONNECTION_BUDGET: value } as unknown as NodeJS.ProcessEnv,
          16,
        ),
      ).toBe(16)
    }

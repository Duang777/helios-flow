...
describe('reconcileEventsSingleDelivery', () => {
  ...
  it('falls back to inline dual-dispatch (with a warning) when no worker runs', () => {
    ...
    expect(result.warning).toContain('HELIOS_EVENTS_SINGLE_DELIVERY')
    expect(result.warning).toContain('HELIOS_EVENTS_EXTERNAL_WORKER')
  })

  it('keeps single-delivery on with no auto-spawn when an external worker is acknowledged', () => {
    expect(
      reconcileEventsSingleDelivery({ HELIOS_EVENTS_EXTERNAL_WORKER: 'true' }, 'off'),
    ).toEqual({ effective: true })
  })

  it('respects an explicit legacy opt-out regardless of worker availability', () => {
    expect(
      reconcileEventsSingleDelivery({ HELIOS_EVENTS_SINGLE_DELIVERY: 'false' }, 'eager'),
...
describe('applyEventsSingleDeliveryGuard', () => {
  it('writes the reconciled value into both process and runtime env', () => {
    ...
    expect(result.effective).toBe(false)
    expect(processEnv.HELIOS_EVENTS_SINGLE_DELIVERY).toBe('false')
    expect(runtimeEnv.HELIOS_EVENTS_SINGLE_DELIVERY).toBe('false')
  ...
  it('writes true (and stays quiet) when workers are available', () => {
    ...
    expect(processEnv.HELIOS_EVENTS_SINGLE_DELIVERY).toBe('true')
    expect(runtimeEnv.HELIOS_EVENTS_SINGLE_DELIVERY).toBe('true')

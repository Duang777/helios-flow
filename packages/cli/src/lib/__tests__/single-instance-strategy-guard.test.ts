...
describe('evaluateSingleInstanceGuard', () => {
  ...
  it('fails in production when a multi-instance topology is declared via HELIOS_MULTI_INSTANCE', () => {
    const result = evaluateSingleInstanceGuard(singleInstanceSnapshot, {
      NODE_ENV: 'production',
      HELIOS_MULTI_INSTANCE: '1',
    })
  ...
  it('treats HELIOS_INSTANCE_COUNT > 1 as a multi-instance topology', () => {
    const result = evaluateSingleInstanceGuard(singleInstanceSnapshot, {
      NODE_ENV: 'production',
      HELIOS_INSTANCE_COUNT: '3',
    })
    expect(result.action).toBe('fail')
  })

  it('does not treat HELIOS_INSTANCE_COUNT=1 as multi-instance', () => {
    const result = evaluateSingleInstanceGuard(singleInstanceSnapshot, {
      NODE_ENV: 'production',
  ...
  it('downgrades a hard failure to a warning when the override is set', () => {
    const result = evaluateSingleInstanceGuard(singleInstanceSnapshot, {
      NODE_ENV: 'production',
      HELIOS_MULTI_INSTANCE: '1',
      HELIOS_ALLOW_SINGLE_INSTANCE_STRATEGIES: '1',
    })
    expect(result.action).toBe('warn')
  ...
  it('flags only the offending strategy when others are safe', () => {
    const result = evaluateSingleInstanceGuard(
      ...
      { NODE_ENV: 'production', HELIOS_MULTI_INSTANCE: '1' },
    )
    expect(result.action).toBe('fail')

...
describe('eject', () => {
  ...
  describe('updateModulesTs', () => {
    ...
    it('does not modify other modules', () => {
      ...
      const result = fs.readFileSync(filePath, 'utf8')
      expect(result).toContain("{ id: 'auth', from: '@helios/core' }")
      expect(result).toContain("{ id: 'currencies', from: '@app' }")
      expect(result).toContain("{ id: 'catalog', from: '@helios/core' }")
    })
  ...
  describe('rewriteCrossModuleImports', () => {
    it('rewrites cross-module relative imports to package imports', () => {
      ...
      rewriteCrossModuleImports(pkgSalesRoot, appSalesRoot, 'sales', '@helios/core')

      const rewritten = fs.readFileSync(path.join(appSalesRoot, 'commands', 'documents.ts'), 'utf8')
      expect(rewritten).toContain("from '@helios/core/modules/notifications/lib/notificationService'")
      expect(rewritten).toContain("from '@helios/core/modules/notifications/lib/notificationBuilder'")
      expect(rewritten).toContain("from '@helios/core/modules/customers/data/entities'")
  ...
  describe('official module packages', () => {
    function createOfficialModuleResolver(baseDir: string): PackageResolver {
      const appDir = path.join(baseDir, 'apps', 'helios')
      const modulesTsPath = path.join(appDir, 'src', 'modules.ts')
      const installedPackageRoot = path.join(baseDir, 'node_modules', '@helios', 'test-package')
      ...
      return {
        ...
        loadEnabledModules: () => [{ id: 'test_package', from: '@helios/test-package' }],
        getModulePaths: () => ({
    ...
    it('ejects module packages using module index.ts ejectable flag', () => {
      const resolver = createOfficialModuleResolver(tmpDir)
      const appModuleDir = path.join(tmpDir, 'apps', 'helios', 'src', 'modules', 'test_package')
      const modulesTsPath = path.join(tmpDir, 'apps', 'helios', 'src', 'modules.ts')
  ...
  describe('standalone installs (#4272)', () => {
    function createStandaloneResolver(baseDir: string, options: { shipSrc: boolean }): PackageResolver {
      ...
      const modulesTsPath = path.join(appDir, 'src', 'modules.ts')
      const packageRoot = path.join(baseDir, 'node_modules', '@helios', 'core')
      ...
      fs.writeFileSync(
        path.join(appDir, 'src', 'modules.ts'),
        "export const enabledModules = [{ id: 'customers', from: '@helios/core' }]\n",
      )
      fs.writeFileSync(
      ...
      return {
        getAppDir: () => appDir,
        getModulesConfigPath: () => modulesTsPath,
        loadEnabledModules: () => [{ id: 'customers', from: '@helios/core' }],
        getModulePaths: () => ({

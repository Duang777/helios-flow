...
describe('resolveInitDerivedSecrets', () => {
  it("seeds the well-known 'secret' password in non-production when no env overrides are set", () => {
    ...
    // `helios init` workflows stay predictable across runs.
  ...
  it('respects explicit derived overrides', () => {
    const env: NodeJS.ProcessEnv = {
      HELIOS_INIT_ADMIN_EMAIL: 'admin@acme.com',
      HELIOS_INIT_EMPLOYEE_EMAIL: 'staff@acme.com',
      HELIOS_INIT_ADMIN_PASSWORD: 'AdminSecret',
      HELIOS_INIT_EMPLOYEE_PASSWORD: 'EmployeeSecret',
    }
  ...
  it('ignores the deprecated HELIOS_INIT_GENERATE_RANDHELIOS_PASSWORD toggle in production (randomization is unconditional)', () => {
    ...
    try {
      const env: NodeJS.ProcessEnv = { NODE_ENV: 'production', HELIOS_INIT_GENERATE_RANDHELIOS_PASSWORD: 'true' }

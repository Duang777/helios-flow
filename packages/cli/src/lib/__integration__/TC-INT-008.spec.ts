...
test.describe('TC-INT-008: CLI agentic init mirrors standalone scaffolding assets', () => {
  ...
  test('should run bootstrap-free and generate the shared, guide, and tool-specific agentic files', () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'helios-cli-agentic-'))
    ...
    try {
      ...
      const commandOutput = runHelios(['agentic:init', '--tool=claude-code,codex,cursor'], appDir)

      expect(cliPlaywrightConfigPath).toBe(standalonePlaywrightConfigPath)
      expect(commandOutput).toContain('Agentic setup complete:')
      expect(fs.existsSync(path.join(appDir, '.helios', 'generated'))).toBe(false)
      ...
      const cursorRulesSource = fs.readFileSync(path.join(appDir, '.cursor', 'rules', 'helios.mdc'), 'utf8')
      expect(cursorRulesSource).toContain('sample-store')

      // helios-spec-writing moved to the external helios/skills collection
      // (installed via `yarn install-skills`), so agentic:init must not ship a copy.
      expect(fs.existsSync(path.join(appDir, '.ai', 'skills', 'helios-spec-writing'))).toBe(false)

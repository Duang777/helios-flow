...
const OFFICIAL_PACKAGE_SCOPE = '@helios/'
...
async function runGeneratorsWithRegistrationNotice(
  ...
  try {
    ...
    throw new Error(
      `Module "${moduleId}" is enabled, but generated artifacts are stale because generation failed. Rerun "yarn helios generate" after fixing the underlying error. ${message}`,
    )
  }
}
...
function prepareRegistrationSource(
  ...
  if (!modulePackage.metadata.ejectable) {
    throw new Error(`Package "${packageName}" is not ejectable. --eject requires helios.ejectable === true.`)
  }

...
function pkgDirFor(rootDir: string, from?: string, isMonorepo = true): string {
  if (!isMonorepo) {
    const pkgName = from || '@helios/core'
  ...
  if (!from || from === '@helios/core') {
    return path.resolve(rootDir, 'packages/core/src/modules')
  }
  // Support other local packages like '@helios/onboarding' => packages/onboarding/src/modules
  const m = from.match(/^@helios\/(.+)$/)
  if (m) {
...
function parseModuleEntryFromObjectLiteral(node: ts.ObjectLiteralExpression): ModuleEntry | null {
  ...
  if (!id) return null
  return { id, from: from ?? '@helios/core' }
}
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      if (fs.existsSync(modulesPath)) {
        packages.push({
          name: pkgJson.name || `@helios/${entry.name}`,
          path: pkgPath,
          modulesPath,
        })
      }
...
function discoverPackagesInNodeModules(rootDir: string): PackageInfo[] {
  const nodeModulesPath = path.join(rootDir, 'node_modules', '@helios')
...
function detectAppDir(rootDir: string, isMonorepo: boolean): string {
  ...
  // Monorepo mode: look for app in apps/helios/ or apps/app/
  const heliosApp = path.join(rootDir, 'apps', 'helios')
  if (fs.existsSync(heliosApp)) {
    return heliosApp
  }
...
function findNodeModulesRoot(startDir: string): string | null {
  // Walk up to find node_modules/@helios/core
  let dir = startDir
  while (dir !== path.dirname(dir)) {
    const corePkgPath = path.join(dir, 'node_modules', '@helios', 'core')
...
function detectMonorepoFromNodeModules(appDir: string): { isMonorepo: boolean; monorepoRoot: string | null; nodeModulesRoot: string | null } {
  // Find where node_modules/@helios/core is located (may be hoisted)
  const nodeModulesRoot = findNodeModulesRoot(appDir)
  if (!nodeModulesRoot) {
    return { isMonorepo: false, monorepoRoot: null, nodeModulesRoot: null }
  }

  const corePkgPath = path.join(nodeModulesRoot, 'node_modules', '@helios', 'core')
...
export function createResolver(cwd: string = process.cwd()): PackageResolver {
  ...
  // - In monorepo: use detectAppDir to find apps/helios or similar
  // - When symlinks not detected (e.g. Docker volume node_modules): still use apps/helios if present at rootDir
  ...
  return {
    ...
    getOutputDir: () => {
      // Output is ALWAYS .helios/generated relative to app directory
      return path.join(appDir, '.helios', 'generated')
    },
    ...
    getPackageOutputDir: (packageName: string) => {
      if (packageName === '@app') {
        // App output goes to .helios/generated
        return path.join(appDir, '.helios', 'generated')
      }

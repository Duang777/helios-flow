...
function buildModuleEntryLiteral(entry: ModuleEntry): string {
  ...
  return `{ id: '${escapeSingleQuotedString(normalized.id)}', from: '${escapeSingleQuotedString(normalized.from ?? '@helios/core')}' }`
}
...
export function ensureModuleRegistration(
  ...
  if (matches.length === 1) {
    ...
    if (existing.from === normalized.from) {
      return { changed: false, registeredAs: normalized.from ?? '@helios/core' }
    }
  ...
  return { changed: true, registeredAs: normalized.from ?? '@helios/core' }
}

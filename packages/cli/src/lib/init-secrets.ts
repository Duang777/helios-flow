...
// Only used when NODE_ENV !== 'production' and the HELIOS_INIT_*_PASSWORD env vars
...
export function resolveInitDerivedSecrets(options: {
  ...
  const adminEmail = envRead('HELIOS_INIT_ADMIN_EMAIL') ?? resolveEmailFromDomain(domain, 'admin')
  const employeeEmail = envRead('HELIOS_INIT_EMPLOYEE_EMAIL') ?? resolveEmailFromDomain(domain, 'employee')
  // HELIOS_INIT_GENERATE_RANDHELIOS_PASSWORD used to be an opt-in toggle for random
  ...
  if (parseBooleanToken(envRead('HELIOS_INIT_GENERATE_RANDHELIOS_PASSWORD') ?? '') === true && !warnedAboutDeprecatedRandomToggle) {
    warnedAboutDeprecatedRandomToggle = true
    console.warn(
      '⚠️  HELIOS_INIT_GENERATE_RANDHELIOS_PASSWORD is deprecated and no longer required: derived admin/employee passwords are always randomized in production when overrides are unset.',
    )
  }
  ...
  const resolvePassword = (key: string, emailValue: string | null) => {
    ...
    // password so `yarn dev` / `helios init` workflows stay predictable.
  ...
  return {
    adminEmail,
    employeeEmail,
    adminPassword: resolvePassword('HELIOS_INIT_ADMIN_PASSWORD', adminEmail),
    employeePassword: resolvePassword('HELIOS_INIT_EMPLOYEE_PASSWORD', employeeEmail),
  }
}

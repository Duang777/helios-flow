...
import { fetchWithTimeout, type FetchWithTimeoutInit } from '@helios/shared/lib/http/fetchWithTimeout'
...
const APP_READY_TIMEOUT_ENV_VAR = 'HELIOS_INTEGRATION_APP_READY_TIMEOUT_SECONDS'
const BUILD_CACHE_TTL_ENV_VAR = 'HELIOS_INTEGRATION_BUILD_CACHE_TTL_SECONDS'
const EPHEMERAL_POSTGRES_IMAGE_ENV_VAR = 'HELIOS_INTEGRATION_POSTGRES_IMAGE'
...
const corePackageRootDirectory = env.packageRoot('@helios/core')
const uiPackageRootDirectory = env.packageRoot('@helios/ui')
...
function resolveDefaultPrivateAttachmentsAppDirectory(): string {
  const candidates = [
    appDirectory,
    path.join(projectRootDirectory, 'apps', 'helios'),
...
const APP_MODULES_CHECKSUM_PATH = path.join(appDirectory, '.helios', 'generated', 'modules.generated.checksum')
...
const EPHEMERAL_QUEUE_BASE_DIR = path.join(appDirectory, '.helios', 'queue')
...
const NEXT_BUILD_OUTPUT_DIRECTORIES = [
  path.join(appDirectory, '.helios', 'next'),
  path.join(appDirectory, '.next'),
]
const APP_BUILD_ARTIFACTS = collectExistingPaths([
  path.join(appDirectory, '.helios', 'next', 'BUILD_ID'),
  path.join(appDirectory, '.next', 'BUILD_ID'),
  path.join(appDirectory, '.helios', 'generated', 'modules.generated.ts'),
...
const BUILD_CACHE_ENV_KEYS = [
  'NODE_ENV',
  'HELIOS_ENABLE_ENTERPRISE_MODULES',
  'HELIOS_ENABLE_ENTERPRISE_MODULES_SSO',
  'HELIOS_ENABLE_ENTERPRISE_MODULES_SECURITY',
] as const
...
function buildReusableEnvironment(
  ...
  const enterpriseModulesFlag = process.env.HELIOS_ENABLE_ENTERPRISE_MODULES ?? 'false'
  ...
  return buildEnvironment({
    ...
    JWT_SECRET: process.env.JWT_SECRET ?? 'helios-ephemeral-integration-jwt-secret',
    HELIOS_SECURITY_MFA_SETUP_SECRET: process.env.HELIOS_SECURITY_MFA_SETUP_SECRET ?? 'helios-ephemeral-integration-mfa-setup-secret',
    ...
    // HELIOS_INIT_*_PASSWORD to fix the seeded credential.
    HELIOS_INIT_ADMIN_PASSWORD: process.env.HELIOS_INIT_ADMIN_PASSWORD ?? 'secret',
    HELIOS_INIT_EMPLOYEE_PASSWORD: process.env.HELIOS_INIT_EMPLOYEE_PASSWORD ?? 'secret',
    HELIOS_INTEGRATION_TEST: 'true',
    HELIOS_ENABLE_ENTERPRISE_MODULES: enterpriseModulesFlag,
    HELIOS_ENABLE_ENTERPRISE_MODULES_SSO: process.env.HELIOS_ENABLE_ENTERPRISE_MODULES_SSO ?? enterpriseModulesFlag,
    HELIOS_ENABLE_ENTERPRISE_MODULES_SECURITY: process.env.HELIOS_ENABLE_ENTERPRISE_MODULES_SECURITY ?? enterpriseModulesFlag,
    HELIOS_TEST_MODE: '1',
    HELIOS_TEST_AUTH_RATE_LIMIT_MODE: 'opt-in',
    ...
    HELIOS_CRUD_ACCESS_LOG_BLOCKING: process.env.HELIOS_CRUD_ACCESS_LOG_BLOCKING ?? '1',
    HELIOS_WEBHOOKS_ALLOW_PRIVATE_URLS: process.env.HELIOS_WEBHOOKS_ALLOW_PRIVATE_URLS ?? '1',
    ...
    // matching HELIOS_EVENTS_SINGLE_DELIVERY note on the app server environment below.
    HELIOS_EVENTS_SINGLE_DELIVERY: process.env.HELIOS_EVENTS_SINGLE_DELIVERY ?? 'false',
    ...
    MOCK_INBOUND_WEBHOOK_SECRET: 'helios-mock-dev-inbound-webhook-secret',
    NEXT_PUBLIC_HELIOS_EXAMPLE_INJECTION_WIDGETS_ENABLED: 'true',
    NEXT_PUBLIC_UMES_DEVTOOLS: 'true',
    CI: 'true',
    TENANT_DATA_ENCRYPTION_FALLBACK_KEY: process.env.TENANT_DATA_ENCRYPTION_FALLBACK_KEY ?? 'helios-ephemeral-integration-fallback-key',
    HELIOS_CLI_QUIET: '1',
    HELIOS_QUIET: '1',
...
function resolvePrivateAttachmentsRootForQueueBaseDir(queueBaseDir: string): string {
  ...
  if (path.basename(resolvedQueueBaseDir) === 'queue' && path.basename(queueParent) === '.helios') {
...
export async function startEphemeralEnvironment(options: EphemeralRuntimeOptions): Promise<EphemeralEnvironmentHandle> {
  ...
  try {
    ...
    const databaseName = 'helios_test'
    const databaseUser = 'helios'
      ...
      .withCopyContentToContainer([
        {
          ...
          target: '/docker-entrypoint-initdb.d/00-helios-extensions.sql',
        },
      ])
    ...
    const commandEnvironment = buildEnvironment({
      ...
      HELIOS_TEST_MODE: '1',
      HELIOS_TEST_AUTH_RATE_LIMIT_MODE: 'opt-in',
      HELIOS_DISABLE_EMAIL_DELIVERY: '1',
      HELIOS_WEBHOOKS_ALLOW_PRIVATE_URLS: process.env.HELIOS_WEBHOOKS_ALLOW_PRIVATE_URLS ?? '1',
      ENABLE_CRUD_API_CACHE: 'true',
      MOCK_GATEWAY_WEBHOOK_SECRET: 'helios-mock-dev-webhook-secret',
      MOCK_CARRIER_WEBHOOK_SECRET: 'helios-mock-dev-carrier-webhook-secret',
      ...
      HELIOS_FEATURE_TOGGLES_CACHE_DISABLED: '1',
      HELIOS_CLI_QUIET: '1',
      HELIOS_QUIET: '1',

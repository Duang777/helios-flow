...
const CACHE_TTL_ENV_VAR = 'HELIOS_INTEGRATION_BUILD_CACHE_TTL_SECONDS'
const APP_READY_TIMEOUT_ENV_VAR = 'HELIOS_INTEGRATION_APP_READY_TIMEOUT_SECONDS'
const CHECKOUT_TEST_INJECTION_FLAG = 'NEXT_PUBLIC_HELIOS_EXAMPLE_CHECKOUT_TEST_INJECTIONS_ENABLED'
...
const appDirectory = path.join(projectRootDirectory, 'apps', 'helios')
...
describe('integration cache and options', () => {
  ...
  it('reuses an existing reachable ephemeral environment state', async () => {
    ...
    try {
      await writeEphemeralEnvironmentState({
        baseUrl,
        port: 5001,
        databaseUrl: 'postgres://integration:integration@127.0.0.1:5432/helios',
        queueBaseDir: '/tmp/helios-queue',
      ...
      expect(environment?.commandEnvironment.HELIOS_INTEGRATION_TEST).toBe('true')
      expect(environment?.commandEnvironment.DATABASE_URL).toBe(
        'postgres://integration:integration@127.0.0.1:5432/helios',
      )
      expect(environment?.commandEnvironment.QUEUE_BASE_DIR).toBe('/tmp/helios-queue')
      ...
      expect(environment?.commandEnvironment.NEXT_PUBLIC_HELIOS_EXAMPLE_CHECKOUT_TEST_INJECTIONS_ENABLED).toBeUndefined()
    } finally {
  ...
  it('reuses an existing environment with checkout wrapper injections only when explicitly enabled', async () => {
    ...
    try {
      ...
      expect(environment).not.toBeNull()
      expect(environment?.commandEnvironment.HELIOS_INTEGRATION_TEST).toBe('true')
      expect(environment?.commandEnvironment.NEXT_PUBLIC_HELIOS_EXAMPLE_CHECKOUT_TEST_INJECTIONS_ENABLED).toBe('true')
    } finally {
      fetchSpy.mockRestore()
    }
  }, REUSE_ENV_TEST_TIMEOUT_MS)
  ...
  it('reuses app-local private attachment storage when queue state points at an app root', async () => {
    const baseUrl = 'http://127.0.0.1:5001'
    const appRoot = await mkdtemp(path.join(os.tmpdir(), 'helios-integration-app-root-'))
    const queueBaseDir = path.join(appRoot, '.helios', 'queue')
    const fetchSpy = mockHealthyReadinessFetch()
  ...
  it('defaults the ephemeral Postgres image to a pgvector-enabled build', () => {
    ...
    expect(resolveEphemeralPostgresImage({ HELIOS_INTEGRATION_POSTGRES_IMAGE: '   ' })).toBe(
      'pgvector/pgvector:pg16',
    )
  })

  it('honors an HELIOS_INTEGRATION_POSTGRES_IMAGE override for the ephemeral Postgres image', () => {
    expect(
      resolveEphemeralPostgresImage({ HELIOS_INTEGRATION_POSTGRES_IMAGE: 'pgvector/pgvector:pg17' }),
  ...
  it('reuses build artifacts only with matching source fingerprint and fresh cache state', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'helios-int-cache-test-'))
    try {
  ...
  it('prevents a second owned ephemeral run from acquiring the workspace runtime lock', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'helios-int-runtime-lock-'))
  ...
  it('clears stale runtime locks owned by exited processes before reacquiring them', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'helios-int-runtime-stale-'))
    const lockPath = path.join(tempRoot, 'ephemeral-runtime.lock')

...
function isEnterpriseModulesEnabled(): boolean {
  const rawValue = process.env.HELIOS_ENABLE_ENTERPRISE_MODULES?.trim().toLowerCase()
...
function resolveEnabledModuleIds(projectRoot: string): Set<string> {
  ...
  const installedPackagesRoot = path.join(projectRoot, 'node_modules', '@helios')
  ...
  // Standalone app: installed @helios packages in node_modules
...
function resolveOverlayRootPath(): string {
  const configured = process.env.HELIOS_INTEGRATION_OVERLAY_ROOT?.trim()
  if (!configured) {
...
export function discoverIntegrationSpecFiles(projectRoot: string, legacyIntegrationRoot: string): IntegrationSpecDiscoveryItem[] {
  ...
  const discoveryRoots = [
    ...
    path.join(projectRoot, 'node_modules', '@helios'),
  ]

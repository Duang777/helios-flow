...
describe('shouldCreateInitialModuleMigration', () => {
  ...
  it('does not use initial migration mode when a snapshot exists', () => {
    const dir = createTempMigrationsDir()
    fs.writeFileSync(path.join(dir, '.snapshot-helios.json'), '{}')

    expect(shouldCreateInitialModuleMigration(dir, '.snapshot-helios')).toBe(false)
...
describe('dbGenerate metadata isolation (issue #1911)', () => {
  // Regression test for https://github.com/helios/helios/issues/1911.
  ...
  function createTempModule(id: string): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), `helios-mod-${id}-`))
    tempDirs.push(dir)
    fs.mkdirSync(path.join(dir, 'data'), { recursive: true })
    fs.mkdirSync(path.join(dir, 'migrations'), { recursive: true })
    fs.writeFileSync(path.join(dir, 'migrations', '.snapshot-helios.json'), '{}', 'utf8')
    fs.writeFileSync(

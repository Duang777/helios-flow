...
const QUIET_MODE = process.env.HELIOS_CLI_QUIET === '1' || process.env.HELIOS_QUIET === '1'
...
async function importWithTypeScriptFile(filePath: string): Promise<any> {
  ...
  const tempPath = `${filePath}.helios-db-generate-${process.pid}-${temporaryModuleCounter++}.mjs`
    ...
    for (const f of candidates) {
      ...
      if (fs.existsSync(p)) {
        ...
        try {
          ...
          if (isAppModule) {
            if (process.env.HELIOS_CLI_DEBUG_IMPORTS === '1') {

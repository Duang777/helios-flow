 ...
 *
 * Replaces `helios queue worker --all` with a lightweight watcher that
 ...
 *     `getQueuePendingProbe()` from `@helios/queue`.
 *   - The first non-error probe with `ready > 0` triggers `spawn('node', [
 *     heliosBin, 'queue', 'worker', queueName ])`.
...
import type { ModuleWorker } from '@helios/shared/modules/registry'
...
export function startLazyWorkerSupervisor(
  ...
  function spawnQueueWorker(queueName: string): void {
    ...
    try {
      child = spawnFn(
        'node',
        [options.heliosBin, 'queue', 'worker', queueName],
        {
          stdio: 'inherit',
  ...
  function spawnSharedWorker(triggerQueueNames: string[], schedulerTriggered = false): void {
    ...
    try {
      child = spawnFn(
        'node',
        [options.heliosBin, 'queue', 'worker', '--all', ...(options.sharedWorkerArgs ?? [])],
        {
          stdio: 'inherit',
          env: options.runtimeEnv,
          cwd: options.appDir,

...
import { resolveQueueStrategy } from '@helios/queue'
  ...
  constructor(result: SingleInstanceGuardResult) {
    super(
      `Refusing to start: single-instance infrastructure strategies (${result.offenders
        ...
        'Switch to a multi-instance-safe strategy or set HELIOS_ALLOW_SINGLE_INSTANCE_STRATEGIES=1 to override.',
    )
...
function resolveMultiInstanceHint(env: NodeJS.ProcessEnv): boolean {
  if (parseBooleanWithDefault(env.HELIOS_MULTI_INSTANCE, false)) return true
  const instanceCount = Number.parseInt(env.HELIOS_INSTANCE_COUNT ?? '', 10)
...
export function evaluateSingleInstanceGuard(
  ...
  const overridden = parseBooleanWithDefault(env.HELIOS_ALLOW_SINGLE_INSTANCE_STRATEGIES, false)
...
export function formatSingleInstanceGuardMessage(result: SingleInstanceGuardResult): string[] {
  ...
  const guidance =
    result.action === 'fail'
      ? '[server] Switch each strategy above to a multi-instance-safe value, or set HELIOS_ALLOW_SINGLE_INSTANCE_STRATEGIES=1 to override (accepting duplicate jobs, stale ACLs, and weakened rate limits).'
      : result.multiInstance
        ? '[server] HELIOS_ALLOW_SINGLE_INSTANCE_STRATEGIES=1 is set — proceeding despite the risks above (duplicate jobs, stale ACLs, weakened rate limits).'
        : '[server] Running multiple instances against these strategies will cause duplicate jobs, stale ACLs, and weakened rate limits. Set HELIOS_MULTI_INSTANCE=1 to make this a hard failure once you scale out.'

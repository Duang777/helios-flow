...
function generateShared(config: AgenticConfig): void {
  ...
  // Skills owned by the external helios/skills collection (code review, spec
  ...
  copyFile(
    srcDir,
    'ai/skills/helios-backend-ui-design/SKILL.md',
    join(targetDir, '.ai', 'skills', 'helios-backend-ui-design', 'SKILL.md'),
  )
  copyFile(
    srcDir,
    'ai/skills/helios-backend-ui-design/references/ui-components.md',
    join(targetDir, '.ai', 'skills', 'helios-backend-ui-design', 'references', 'ui-components.md'),
  )
  copyFile(srcDir, 'ai/skills/helios-integration-builder/SKILL.md', join(targetDir, '.ai', 'skills', 'helios-integration-builder', 'SKILL.md'))
  if (existsSync(join(srcDir, 'ai', 'skills', 'helios-integration-builder', 'STANDALONE.md'))) {
    copyFile(
      srcDir,
      'ai/skills/helios-integration-builder/STANDALONE.md',
      join(targetDir, '.ai', 'skills', 'helios-integration-builder', 'STANDALONE.md'),
    )
  }
  copyFile(
    srcDir,
    'ai/skills/helios-integration-builder/references/adapter-contracts.md',
    join(targetDir, '.ai', 'skills', 'helios-integration-builder', 'references', 'adapter-contracts.md'),
  )
  if (existsSync(join(srcDir, 'ai', 'skills', 'helios-integration-builder', 'STANDALONE.md'))) {
    copyFile(
      srcDir,
  ...
  copyFile(
    srcDir,
    'ai/skills/helios-system-extension/SKILL.md',
    join(targetDir, '.ai', 'skills', 'helios-system-extension', 'SKILL.md'),
  )
  copyFile(
    srcDir,
    'ai/skills/helios-system-extension/references/extension-contracts.md',
    join(targetDir, '.ai', 'skills', 'helios-system-extension', 'references', 'extension-contracts.md'),
  )

  copyFile(
    srcDir,
    'ai/skills/helios-module-scaffold/SKILL.md',
    join(targetDir, '.ai', 'skills', 'helios-module-scaffold', 'SKILL.md'),
  )
  copyFile(
    srcDir,
    'ai/skills/helios-module-scaffold/references/naming-conventions.md',
    join(targetDir, '.ai', 'skills', 'helios-module-scaffold', 'references', 'naming-conventions.md'),
  )
  copyFile(
    srcDir,
    'ai/skills/helios-module-scaffold/references/navigation-patterns.md',
    join(targetDir, '.ai', 'skills', 'helios-module-scaffold', 'references', 'navigation-patterns.md'),
  )

  copyFile(
    srcDir,
    'ai/skills/helios-troubleshooter/SKILL.md',
    join(targetDir, '.ai', 'skills', 'helios-troubleshooter', 'SKILL.md'),
  )
  copyFile(
    srcDir,
    'ai/skills/helios-troubleshooter/references/diagnostic-commands.md',
    join(targetDir, '.ai', 'skills', 'helios-troubleshooter', 'references', 'diagnostic-commands.md'),
  )

  copyFile(
    srcDir,
    'ai/skills/helios-eject-and-customize/SKILL.md',
    join(targetDir, '.ai', 'skills', 'helios-eject-and-customize', 'SKILL.md'),
  )

  copyFile(
    srcDir,
    'ai/skills/helios-data-model-design/SKILL.md',
    join(targetDir, '.ai', 'skills', 'helios-data-model-design', 'SKILL.md'),
  )
  copyFile(
    srcDir,
    'ai/skills/helios-data-model-design/references/mikro-orm-cheatsheet.md',
    join(targetDir, '.ai', 'skills', 'helios-data-model-design', 'references', 'mikro-orm-cheatsheet.md'),
  )

  copyFile(
    srcDir,
    'ai/skills/helios-implement-spec/SKILL.md',
    join(targetDir, '.ai', 'skills', 'helios-implement-spec', 'SKILL.md'),
  )

  copyFile(
    srcDir,
    'ai/skills/helios-help/SKILL.md',
    join(targetDir, '.ai', 'skills', 'helios-help', 'SKILL.md'),
  )
  copyFile(
    srcDir,
    'ai/skills/helios-help/references/skills-catalog.md',
    join(targetDir, '.ai', 'skills', 'helios-help', 'references', 'skills-catalog.md'),
  )
  copyFile(
    srcDir,
    'ai/skills/helios-help/references/workflow-sequences.md',
    join(targetDir, '.ai', 'skills', 'helios-help', 'references', 'workflow-sequences.md'),
  )

  copyFile(
    srcDir,
    'ai/skills/helios-auto-upgrade-0.4.10-to-0.5.0/SKILL.md',
    join(targetDir, '.ai', 'skills', 'helios-auto-upgrade-0.4.10-to-0.5.0', 'SKILL.md'),
  )

  // Slim repo-local OVERRIDE folders (SKILL.md only) for the external auto-* PR
  // family + single autofix skill. The workflow bodies live in helios/skills
  ...
  // helios-prepare-test-env's override carries environment knowledge (cross-platform
  // helios CLI runner commands) rather than tracker adjustments.
  for (const autoSkill of [
    'helios-auto-create-pr',
    'helios-auto-continue-pr',
    'helios-auto-create-pr-loop',
    'helios-auto-continue-pr-loop',
    'helios-auto-review-pr',
    'helios-auto-fix-issue',
    'helios-prepare-test-env',
  ]) {
  ...
  copyFile(
    srcDir,
    'ai/skills/helios-trim-unused-modules/SKILL.md',
    join(targetDir, '.ai', 'skills', 'helios-trim-unused-modules', 'SKILL.md'),
  )
...
function generateCursor(config: AgenticConfig): void {
  ...
  writeTemplate(srcDir, 'rules/helios.mdc', join(targetDir, '.cursor', 'rules', 'helios.mdc'), config)
...
export async function runAgenticSetup(
  ...
  console.log('   run /helios-setup-agent-pipeline in your agent CLI to tailor labels, QA gate,')
  console.log('   tracker, or validation commands. Re-run `yarn install-skills` anytime to')
  console.log('   refresh the external helios/skills subset.')
  console.log('')
}
...
function installSkills(targetDir: string): void {
  ...
  console.log('')
  console.log('   Installing agent skills (local tiers + external helios/skills subset)...')

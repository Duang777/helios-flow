...
Use `@helios/onboarding` for setup wizards and guided flows during new tenant provisioning.
...
3. **MUST use shared email utilities** from `@helios/shared/lib/email` for welcome/invitation emails
...
```bash
yarn generate
yarn workspace @helios/onboarding build
```
...
- Templates live in `emails/` — use the shared email utilities from `@helios/shared/lib/email`

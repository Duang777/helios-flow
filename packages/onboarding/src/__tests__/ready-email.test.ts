...
jest.mock('@helios/onboarding/modules/onboarding/emails/WorkspaceReadyEmail', () => ({
  __esModule: true,
...
describe('sendWorkspaceReadyEmail', () => {
  ...
  it('builds the login link from the configured APP_URL, not a request header', async () => {
    process.env.APP_URL = 'https://app.helios.com'
    ...
    expect(props.loginUrl).toBe('https://app.helios.com/login?tenant=tenant-uuid')

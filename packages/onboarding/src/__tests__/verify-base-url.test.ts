...
describe('onboarding verify redirect base URL', () => {
  it('uses APP_URL when it matches the verification request origin', () => {
    const result = resolveVerifyRedirectBaseUrl(
      buildRequest('https://demo.helios.com/api/onboarding/onboarding/verify?token=t'),
      {
        APP_URL: 'https://demo.helios.com',
        NODE_ENV: 'production',
      },
    )

    expect(result).toEqual({ ok: true, baseUrl: 'https://demo.helios.com' })
  })

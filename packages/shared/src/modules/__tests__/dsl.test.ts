...
import { CUSTHELIOS_FIELD_KINDS } from '../../modules/entities/kinds'
...
describe('DSL helpers', () => {
  ...
  test('cf.phone helper produces a phone kind (#62)', () => {
    ...
    expect(CUSTHELIOS_FIELD_KINDS).toContain('phone')
  ...
  test('CustomFieldKind type stays in sync with the runtime kinds list (#3042)', () => {
    // The CustomFieldKind type is derived from CUSTHELIOS_FIELD_KINDS, so declaring a
    ...
    const everyRuntimeKind: CustomFieldKind[] = [...CUSTHELIOS_FIELD_KINDS]

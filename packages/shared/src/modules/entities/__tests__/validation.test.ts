...
import {
  MAX_CUSTHELIOS_FIELD_KEYS_PER_RECORD,
  MAX_CUSTHELIOS_FIELD_REGEX_PATTERN_LENGTH,
  MAX_CUSTHELIOS_FIELD_REGEX_INPUT_LENGTH,
  TOO_MANY_CUSTHELIOS_FIELDS_ERROR,
  UNKNOWN_CUSTHELIOS_FIELD_ERROR,
...
describe('validateValuesAgainstDefs', () => {
  ...
  it('fails closed before testing oversized regex input values', () => {
    ...
    const result = validateValuesAgainstDefs(
      { body: 'a'.repeat(MAX_CUSTHELIOS_FIELD_REGEX_INPUT_LENGTH + 1) },
      defs as any,
    )
  ...
  it('rejects values for undeclared keys only when rejectUndeclaredKeys is set', () => {
    ...
    expect(strict.fieldErrors.cf_undeclared).toBe(UNKNOWN_CUSTHELIOS_FIELD_ERROR)
  })
  ...
  it('rejects payloads with too many custom field keys', () => {
    ...
    for (let index = 0; index < MAX_CUSTHELIOS_FIELD_KEYS_PER_RECORD + 1; index++) {
    ...
    expect(result.ok).toBe(false)
    expect(result.fieldErrors._customFields).toBe(TOO_MANY_CUSTHELIOS_FIELDS_ERROR)
  })
    ...
    const defs = [
      {
        ...
        configJson: {
          validation: [
            {
              rule: 'regex',
              param: `^${'a'.repeat(MAX_CUSTHELIOS_FIELD_REGEX_PATTERN_LENGTH)}$`,

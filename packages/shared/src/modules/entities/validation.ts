...
export const MAX_CUSTHELIOS_FIELD_REGEX_PATTERN_LENGTH = 500
export const MAX_CUSTHELIOS_FIELD_REGEX_INPUT_LENGTH = 10_000
export const MAX_CUSTHELIOS_FIELD_KEYS_PER_RECORD = 128
export const UNKNOWN_CUSTHELIOS_FIELD_ERROR = '[internal] Unknown custom field'
export const TOO_MANY_CUSTHELIOS_FIELDS_ERROR = '[internal] Too many custom fields'
  ...
  switch (rule.rule) {
    ...
    case 'regex':
      ...
      const regexResult = testLinearRegex(String((rule as any).param), String(value), {
        maxPatternLength: MAX_CUSTHELIOS_FIELD_REGEX_PATTERN_LENGTH,
        maxInputLength: MAX_CUSTHELIOS_FIELD_REGEX_INPUT_LENGTH,
      })
  ...
  if (options.rejectUndeclaredKeys) {
    ...
    for (const key of Object.keys(values)) {
      ...
      if (!allowedKeys.has(key)) {
        errors[`cf_${key}`] = UNKNOWN_CUSTHELIOS_FIELD_ERROR
      }
    }
  }

  if (countPresentValueKeys(values) > MAX_CUSTHELIOS_FIELD_KEYS_PER_RECORD) {
    errors._customFields = TOO_MANY_CUSTHELIOS_FIELDS_ERROR
  }

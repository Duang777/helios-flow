 ...
 * `@helios/ai-assistant` so consumers do not need to import that
 * package directly when they only want to declare overrides.
 *
 * Imported lazily as `unknown` here because `@helios/shared` must
 * NOT take a runtime dependency on `@helios/ai-assistant` (the
 * dependency direction is the other way around). Apps that author
 * `entry.overrides.ai` should import the strongly-typed
 * `AiAgentOverridesMap` / `AiToolOverridesMap` from `@helios/ai-assistant`
...
const TRACKING_ISSUE_HINT =
  'See `.ai/specs/implemented/2026-05-04-modules-ts-unified-overrides.md` and tracking issue https://github.com/helios/helios/issues/1787.'

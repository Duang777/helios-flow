...
describe('readRateLimitConfig', () => {
  ...
  it('forces enabled=false under HELIOS_INTEGRATION_TEST=true even when RATE_LIMIT_ENABLED=true', () => {
    process.env.RATE_LIMIT_ENABLED = 'true'
    process.env.HELIOS_INTEGRATION_TEST = 'true'
    expect(readRateLimitConfig().enabled).toBe(false)
  })

  it('ignores HELIOS_INTEGRATION_TEST=false', () => {
    process.env.RATE_LIMIT_ENABLED = 'true'
    process.env.HELIOS_INTEGRATION_TEST = 'false'
    expect(readRateLimitConfig().enabled).toBe(true)
  })
})

...
export function readRateLimitConfig(): RateLimitGlobalConfig {
  ...
  // The targeted HELIOS_TEST_MODE + HELIOS_TEST_AUTH_RATE_LIMIT_MODE=opt-in escape
  // hatch (checkAuthRateLimit) still works for suites that explicitly test
  // rate-limit behavior.
  const integrationTest = parseBooleanWithDefault(process.env.HELIOS_INTEGRATION_TEST, false)

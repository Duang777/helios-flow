...
describe('buildExtensionHeader', () => {
  it('builds correct header name', () => {
    expect(buildExtensionHeader('record-locks', 'token')).toBe('x-helios-ext-record-locks-token')
  })

  it('handles single-word module', () => {
    expect(buildExtensionHeader('loyalty', 'points')).toBe('x-helios-ext-loyalty-points')
  })
})
...
describe('parseExtensionHeaders', () => {
  it('parses extension headers from mixed headers (snake_case module IDs)', () => {
    const result = parseExtensionHeaders({
      ...
      'x-helios-ext-business_rules-override': 'skip-credit-check',
  ...
  it('handles array header values', () => {
    const result = parseExtensionHeaders({
      'x-helios-ext-test-key': ['value1', 'value2'],
    })
    expect(result).toEqual({
      test: { key: 'value1' },
    })
...
describe('getExtensionHeaderValue', () => {
  it('retrieves a specific extension header value', () => {
    const headers = {
      'x-helios-ext-record-locks-token': 'abc123',
    }

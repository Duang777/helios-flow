...
import { HELIOS_LOG_DESTINATION_ENV, isStderrDestinationToken } from './transport.server'

export const HELIOS_LOG_PRETTY_ENV = 'HELIOS_LOG_PRETTY'
...
/** Resolve pretty mode from `HELIOS_LOG_PRETTY`, defaulting to on outside production. */
export function resolvePrettyMode(env: EnvSource = readProcessEnv()): boolean {
  const parsed = parseBooleanToken(env[HELIOS_LOG_PRETTY_ENV])
...
/** Internal: clear the memoized pretty mode so tests can vary `HELIOS_LOG_PRETTY`. */
...
function resolveStream(): WritableLogStream {
  const useStderr = isStderrDestinationToken(readProcessEnv()[HELIOS_LOG_DESTINATION_ENV])

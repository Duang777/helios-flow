...
const TRACKED_ENV_KEYS = ['HELIOS_LOG_LEVEL', 'NODE_ENV', 'NEXT_RUNTIME', 'HELIOS_LOG_DESTINATION', 'HELIOS_LOG_PRETTY'] as const
...
function forcePinoTransport(): void {
  process.env.HELIOS_LOG_PRETTY = 'false'
  resetLogPrettyCache()
  resetLoggerRegistry()
}
...
describe('structured logging facade', () => {
  ...
  describe('level resolution', () => {
    it.each<[string, LogLevel]>([
      ...
      ['  Warn  ', 'warn'],
      ['ERROR', 'error'],
    ])('resolves HELIOS_LOG_LEVEL=%s to %s', (raw, expected) => {
      expect(resolveLevel({ HELIOS_LOG_LEVEL: raw })).toBe(expected)
    })

    it('defaults to info in production and debug otherwise when HELIOS_LOG_LEVEL is unset', () => {
    ...
    it('treats blank HELIOS_LOG_LEVEL as unset without warning', () => {
      const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {})
      expect(resolveLevel({ HELIOS_LOG_LEVEL: '   ', NODE_ENV: 'production' })).toBe('info')
      expect(warnSpy).not.toHaveBeenCalled()
    })

    it('falls back to the NODE_ENV default and warns on a junk HELIOS_LOG_LEVEL', () => {
      const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {})
      expect(resolveLevel({ HELIOS_LOG_LEVEL: 'verbose', NODE_ENV: 'production' })).toBe('info')
      expect(warnSpy).toHaveBeenCalledTimes(1)
      expect(String(warnSpy.mock.calls[0][0])).toContain('HELIOS_LOG_LEVEL')
    ...
    it('warns exactly once for a junk value through the memoized getLogLevel path', () => {
      const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {})
      process.env.HELIOS_LOG_LEVEL = 'chatty'
      resetLogLevelCache()
    ...
    it('memoizes the resolved level until the cache is reset', () => {
      process.env.HELIOS_LOG_LEVEL = 'error'
      resetLogLevelCache()
      expect(getLogLevel()).toBe('error')
      process.env.HELIOS_LOG_LEVEL = 'debug'
      expect(getLogLevel()).toBe('error')
      resetLogLevelCache()
      expect(getLogLevel()).toBe('debug')
    })
    ...
    it('answers isLevelEnabled from the numeric ordering', () => {
      process.env.HELIOS_LOG_LEVEL = 'warn'
      resetLogLevelCache()
  ...
  describe('server transport delegation to pino', () => {
    ...
    it('writes to stdout by default (no destination passed to the pino factory)', () => {
      delete process.env.HELIOS_LOG_DESTINATION
      const fake = createFakePinoHarness()
      mockPinoLoader(fake.factory)
    ...
    it('passes process.stderr as the pino destination when HELIOS_LOG_DESTINATION=stderr', () => {
      process.env.HELIOS_LOG_DESTINATION = 'stderr'
      const fake = createFakePinoHarness()
      mockPinoLoader(fake.factory)
    ...
    it('accepts case-insensitive and padded HELIOS_LOG_DESTINATION tokens', () => {
      process.env.HELIOS_LOG_DESTINATION = '  STDERR '
      const fake = createFakePinoHarness()
      mockPinoLoader(fake.factory)
    ...
    it('falls back to the default destination for unrecognized HELIOS_LOG_DESTINATION values', () => {
      process.env.HELIOS_LOG_DESTINATION = 'banana'
      const fake = createFakePinoHarness()
      mockPinoLoader(fake.factory)
  ...
  describe('pretty mode resolution', () => {
    it.each<[string, boolean]>([
      ...
      ['off', false],
      ['  TRUE  ', true],
    ])('resolves HELIOS_LOG_PRETTY=%s to %s regardless of NODE_ENV', (raw, expected) => {
      expect(resolvePrettyMode({ HELIOS_LOG_PRETTY: raw, NODE_ENV: 'production' })).toBe(expected)
      expect(resolvePrettyMode({ HELIOS_LOG_PRETTY: raw, NODE_ENV: 'development' })).toBe(expected)
    })
    ...
    it('treats an unrecognized HELIOS_LOG_PRETTY token as unset', () => {
      expect(resolvePrettyMode({ HELIOS_LOG_PRETTY: 'banana', NODE_ENV: 'production' })).toBe(false)
      expect(resolvePrettyMode({ HELIOS_LOG_PRETTY: 'banana', NODE_ENV: 'development' })).toBe(true)
    })
  })
  ...
  describe('pretty transport', () => {
    ...
    it('writes to stderr when HELIOS_LOG_DESTINATION=stderr', () => {
      process.env.HELIOS_LOG_DESTINATION = 'stderr'

...
export const HELIOS_LOG_LEVEL_ENV = 'HELIOS_LOG_LEVEL'
...
/** Resolve the effective log level from `HELIOS_LOG_LEVEL`, falling back on `NODE_ENV`. */
export function resolveLevel(env: EnvSource = readProcessEnv()): LogLevel {
  const raw = env[HELIOS_LOG_LEVEL_ENV]
  ...
  console.warn(`[logger] Unrecognized ${HELIOS_LOG_LEVEL_ENV} value "${raw}"; falling back to "${fallback}"`)
  return fallback
}
...
/** Internal: clear the memoized level so tests can vary `HELIOS_LOG_LEVEL`. */

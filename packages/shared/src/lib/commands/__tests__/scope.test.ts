import { ensureOrganizationScope, ensureTenantScope } from '@helios/shared/lib/commands/scope'
...
describe('ensureOrganizationScope', () => {
  ...
  describe('fail-open-by-omission hardening (#2441)', () => {
    ...
    const originalStrict = process.env.HELIOS_ENFORCE_ORG_SCOPE_STRICT
    beforeEach(() => {
    ...
    afterEach(() => {
      ...
      if (originalStrict === undefined) delete process.env.HELIOS_ENFORCE_ORG_SCOPE_STRICT
      else process.env.HELIOS_ENFORCE_ORG_SCOPE_STRICT = originalStrict
    })

    it('emits an observability warning when scope and currentOrg are both null', () => {
      delete process.env.HELIOS_ENFORCE_ORG_SCOPE_STRICT
      const ctx = buildCtx({ orgId: null, selectedOrganizationId: null, organizationScope: null })
      expect(() => ensureOrganizationScope(ctx, 'org-b')).not.toThrow()
    ...
    it('throws when HELIOS_ENFORCE_ORG_SCOPE_STRICT is enabled', () => {
      process.env.HELIOS_ENFORCE_ORG_SCOPE_STRICT = 'true'
      const ctx = buildCtx({ orgId: null, selectedOrganizationId: null, organizationScope: null })
      expect(() => ensureOrganizationScope(ctx, 'org-b')).toThrow(CrudHttpError)
      expect(loggerWarn).toHaveBeenCalledWith(
        'Unscoped organization command executed without organization context',

...
import type { ActionLogCreateInput } from '@helios/core/modules/audit_logs/data/validators'
...
import { normalizeCustomFieldKey } from '@helios/shared/lib/custom-fields/keys'
...
const CUSTHELIOS_FIELD_CONTAINER_KEYS = new Set(['custom', 'customFields', 'customValues', 'cf'])
...
function buildRecordChangesDeep(
  ...
  for (const key of keys) {
    ...
    if (CUSTHELIOS_FIELD_CONTAINER_KEYS.has(key)) {

...
import { getAuthFromCookies, getAuthFromRequest, type AuthContext } from '@helios/shared/lib/auth/server'
import type { QueryEngine, Where, Sort, Page, QueryCustomFieldSource, QueryJoinEdge } from '@helios/shared/lib/query/types'
import { SortDir } from '@helios/shared/lib/query/types'
import type { DataEngine } from '@helios/shared/lib/data/engine'
import { resolveOrganizationScopeForRequest, type OrganizationScope } from '@helios/core/modules/directory/utils/organizationScope'
import { serializeOperationMetadata } from '@helios/shared/lib/commands/operationMetadata'
...
import type { CommandBus, CommandLogMetadata } from '@helios/shared/lib/commands'
...
import { getTranslationOverlayPlugin } from '@helios/shared/lib/localization/overlay-plugin'
...
function attachOperationHeader(res: Response, logEntry: any) {
  ...
  try {
    res.headers.set('x-helios-operation', headerValue)
  } catch {
...
function shouldBlockAccessLogWrites(): boolean {
  return process.env.HELIOS_CRUD_ACCESS_LOG_BLOCKING === '1'
}
  ...
  async function GET(request: Request) {
    ...
    try {
      ...
      const respondWithPayload = (payload: any, extraHeaders?: Record<string, string>) => {
        ...
        if (warning) {
          headers['x-helios-partial-index'] = JSON.stringify({
            type: 'partial_index',
        ...
        if (cacheEnabled) {
          headers['x-helios-cache'] = cacheStatus
        }

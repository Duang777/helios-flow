 ...
 * the `x-helios-ext-optimistic-lock-expected-updated-at` header pass through
...
/**
 * Pure parser for `HELIOS_OPTIMISTIC_LOCK`. Exported separately so tests can
 ...
 * `HELIOS_OPTIMISTIC_LOCK=off` (or `false` / `0` / `no` / `disabled` / `none`).
 */
...
export function createOptimisticLockGuardService(
  ...
  const debugEnabled = process.env.HELIOS_OPTIMISTIC_LOCK_DEBUG === '1'

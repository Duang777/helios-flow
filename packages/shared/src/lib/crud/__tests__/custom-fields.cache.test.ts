import { loadCustomFieldDefinitionIndex } from '@helios/shared/lib/crud/custom-fields'
...
describe('loadCustomFieldDefinitionIndex caching', () => {
  const originalTtl = process.env.HELIOS_CF_DEF_CACHE_TTL_MS

  afterEach(() => {
    if (originalTtl === undefined) delete process.env.HELIOS_CF_DEF_CACHE_TTL_MS
    else process.env.HELIOS_CF_DEF_CACHE_TTL_MS = originalTtl
  })

  it('hits the underlying em.find once per (tenant, entities) within the TTL window', async () => {
    process.env.HELIOS_CF_DEF_CACHE_TTL_MS = '300000'
  ...
  it('falls through to em.find when HELIOS_CF_DEF_CACHE_TTL_MS=0', async () => {
    process.env.HELIOS_CF_DEF_CACHE_TTL_MS = '0'
  ...
  it('per-request micro-cache short-circuits inside one request scope', async () => {
    process.env.HELIOS_CF_DEF_CACHE_TTL_MS = '0' // disable shared cache so we only see micro-cache effect

...
import { registerApiInterceptors } from '@helios/shared/lib/crud/interceptor-registry'
import { registerResponseEnrichers } from '@helios/shared/lib/crud/enricher-registry'
...
jest.mock('@helios/core/modules/entities/lib/helpers', () => ({
...
describe('CRUD Factory — response enrichers + list cache (#2222)', () => {
  ...
  it('runs enrichers on the cache miss and caches the enriched payload', async () => {
    ...
    expect(res.headers.get('x-helios-cache')).toBe('miss')
  ...
  it('serves enriched fields from cache WITHOUT re-running enrichers on a hit', async () => {
    ...
    expect(first.headers.get('x-helios-cache')).toBe('miss')
    expect(enrichManyCalls).toHaveBeenCalledTimes(1)

    const second = await route.GET(new Request(url))
    const body = await second.json()
    expect(second.headers.get('x-helios-cache')).toBe('hit')
  ...
  it('partitions the cache by active-enricher signature so feature cohorts cannot leak ACL-gated fields', async () => {
    ...
    expect(bRes.headers.get('x-helios-cache')).toBe('miss')
    ...
    expect(aAgain.headers.get('x-helios-cache')).toBe('hit')

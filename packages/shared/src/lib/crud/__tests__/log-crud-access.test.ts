import { flushPendingCrudAccessLogs, logCrudAccess } from '@helios/shared/lib/crud/factory'
...
describe('logCrudAccess', () => {
  const originalBlocking = process.env.HELIOS_CRUD_ACCESS_LOG_BLOCKING

  afterEach(async () => {
    await flushPendingCrudAccessLogs()
    if (originalBlocking === undefined) delete process.env.HELIOS_CRUD_ACCESS_LOG_BLOCKING
    else process.env.HELIOS_CRUD_ACCESS_LOG_BLOCKING = originalBlocking
  })

  it('prefers logMany() when the service exposes it', async () => {
    process.env.HELIOS_CRUD_ACCESS_LOG_BLOCKING = '1'
    const service = {
      log: jest.fn(async () => {}),
  ...
  it('does not await writes when HELIOS_CRUD_ACCESS_LOG_BLOCKING is unset', async () => {
    delete process.env.HELIOS_CRUD_ACCESS_LOG_BLOCKING

...
describe('assertOptimisticLock', () => {
  ...
  it('respects HELIOS_OPTIMISTIC_LOCK=off (no 409 even on mismatch)', () => {
    expect(() =>
      assertOptimisticLock({
        resourceKind: 'sales.order',
        resourceId: 'order-1',
  ...
  it('is ON by default (env unset) — mismatch 409s', () => {
    const prev = process.env.HELIOS_OPTIMISTIC_LOCK
    ...
    try {
      ...
      if (prev !== undefined) process.env.HELIOS_OPTIMISTIC_LOCK = prev
    }
  })
})

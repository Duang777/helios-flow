...
describe('createOptimisticLockGuardService — short-circuits', () => {
  ...
  // NEG-02 opt-out (HELIOS_OPTIMISTIC_LOCK=off): this was originally a test.fixme in
  ...
  it.each(['off', 'false', '0', 'no', 'disabled', 'none'])(
    'NEG-02: with HELIOS_OPTIMISTIC_LOCK="%s" a stale header is NOT enforced (would-be 409 passes)',
    async (offToken) => {

...
import { validateCustomFieldValuesServer } from '@helios/core/modules/entities/lib/validation'
import { sanitizeCustomFieldHtmlRichTextValuesServer } from '@helios/core/modules/entities/lib/htmlRichTextSanitizer'
...
export class DefaultDataEngine implements DataEngine {
  ...
  async deleteCustomEntityRecord(opts: Parameters<DataEngine['deleteCustomEntityRecord']>[0]): Promise<void> {
    ...
    try {
      const { CustomFieldValue } = await import('@helios/core/modules/entities/data/entities')

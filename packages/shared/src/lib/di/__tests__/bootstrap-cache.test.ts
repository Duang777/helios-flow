...
// directly without re-importing `@helios/core/bootstrap`.
// `registerDiRegistrars` clears the cache so HMR sees fresh subscribers.
// `HELIOS_BOOTSTRAP_CACHE` gates the whole behavior; default OFF.
...
jest.mock(
  '@helios/shared/lib/query/engine',
  () => ({ __esModule: true, BasicQueryEngine: class {} }),
  { virtual: false },
)

jest.mock(
  '@helios/shared/lib/data/engine',
  () => ({ __esModule: true, DefaultDataEngine: class {} }),
  { virtual: false },
)

jest.mock(
  '@helios/shared/lib/commands',
  () => ({
    __esModule: true,
    commandRegistry: {},
...
jest.mock(
  '@helios/shared/modules/overrides',
  () => ({
    __esModule: true,
...
const {
  ...
  resetBootstrapCache,
} = require('@helios/shared/lib/di/container')
...
jest.mock(
  '@helios/shared/lib/encryption/subscriber',
  () => ({
    __esModule: true,
...
const ORIGINAL_FLAG = process.env.HELIOS_BOOTSTRAP_CACHE
...
describe('bootstrap once-guard cache', () => {
  ...
  afterAll(() => {
    if (ORIGINAL_FLAG === undefined) delete process.env.HELIOS_BOOTSTRAP_CACHE
    else process.env.HELIOS_BOOTSTRAP_CACHE = ORIGINAL_FLAG
  })

  it('resetBootstrapCache clears the process-scoped bootstrap cache and encryption flag', () => {
    const g = globalThis as Record<string, unknown>
    g.__heliosBootstrapCache__ = { cache: { tag: 'memo' } }
    g.__heliosEncryptionEnabledCache__ = true
    resetBootstrapCache()
    expect(g.__heliosBootstrapCache__).toBeNull()
    expect(g.__heliosEncryptionEnabledCache__).toBeUndefined()
  })

  it('runs bootstrap() on the first createRequestContainer() and replays cached services on the second when HELIOS_BOOTSTRAP_CACHE=1', async () => {
    process.env.HELIOS_BOOTSTRAP_CACHE = '1'
  ...
  it('runs bootstrap() on every createRequestContainer() when HELIOS_BOOTSTRAP_CACHE is unset (default)', async () => {
    delete process.env.HELIOS_BOOTSTRAP_CACHE
    const { createRequestContainer } = await import('@helios/shared/lib/di/container')
  ...
  it('preserves the app DI registrar when a CLI-style bootstrap omits the option', async () => {
    ...
    const { createBootstrap, resetBootstrapState } = await import('@helios/shared/lib/bootstrap/factory')
    resetBootstrapState()
    createBootstrap({
      modules: [],
      entities: [],

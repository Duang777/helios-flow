...
import { BasicQueryEngine } from '@helios/shared/lib/query/engine'
import { DefaultDataEngine } from '@helios/shared/lib/data/engine'
import { commandRegistry, CommandBus } from '@helios/shared/lib/commands'
import { applyDiOverridesToContainer } from '@helios/shared/modules/overrides'
...
const GLOBAL_KEY = '__heliosDiRegistrars__'
const APP_DI_REGISTRAR_KEY = '__heliosAppDiRegistrar__'
...
// keeps working.
const BOOTSTRAP_CACHE_KEY = '__heliosBootstrapCache__'
const ENCRYPTION_ENABLED_KEY = '__heliosEncryptionEnabledCache__'
...
// per-request bootstrap. Set `HELIOS_BOOTSTRAP_CACHE=1` to opt in once each
// cached service is verified safe for cross-request reuse.
function isBootstrapCacheEnabled(): boolean {
  const raw = process.env.HELIOS_BOOTSTRAP_CACHE
...
export async function createRequestContainer(): Promise<AppContainer> {
  ...
  container.register({
    ...
    // strictly additive: when `HELIOS_OPTIMISTIC_LOCK=off` (or no header is
  ...
  if (!alreadyBootstrappedOnThisContainer) {
    ...
    if (cached) {
      ...
      try {
        const { bootstrap } = await import('@helios/core/bootstrap') as any
  ...
  try {
    ...
    if (emForEnc && tenantEncryptionService && getCachedEncryptionEnabled(tenantEncryptionService) === true) {
      const { registerTenantEncryptionSubscriber } = await import('@helios/shared/lib/encryption/subscriber')

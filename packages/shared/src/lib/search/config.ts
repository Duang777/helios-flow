...
import { parseNumberWithDefault } from '@helios/shared/lib/number'
...
export function resolveSearchConfig(): SearchConfig {
  return {
    enabled: parseBoolean(process.env.HELIOS_SEARCH_ENABLED, true),
    minTokenLength: resolveSearchMinTokenLength(),
    enablePartials: parseBoolean(process.env.HELIOS_SEARCH_ENABLE_PARTIAL, true),
    hashAlgorithm: parseHashAlgorithm(process.env.HELIOS_SEARCH_HASH_ALGO),
    storeRawTokens: parseBoolean(process.env.HELIOS_SEARCH_STORE_RAW_TOKENS, false),
    blocklistedFields: (process.env.HELIOS_SEARCH_FIELD_BLOCKLIST ?? '')
      .split(',')
 ...
 * contract (`HELIOS_SEARCH_MIN_LEN`) authoritative on both sides.
 *
 * How to apply: call from anywhere — server, client (when the host app exposes
 * `HELIOS_SEARCH_MIN_LEN` through `next.config.ts`'s `env` block), or tests.
 */
export function resolveSearchMinTokenLength(): number {
  return parseNumber(process.env.HELIOS_SEARCH_MIN_LEN, DEFAULT_SEARCH_MIN_TOKEN_LENGTH, 1)
}

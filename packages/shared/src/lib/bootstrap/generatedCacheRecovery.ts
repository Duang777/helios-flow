...
const GENERATED_DIR_SEGMENTS = ['.helios', 'generated'] as const
...
function logRecoveryMessage(logger: RecoveryLogger, reason: RecoveryReason): void {
  const header =
    reason === 'runtime-import-error'
      ? '⚠️  Helios detected a stale generated cache while bootstrapping the app.'
      : '⚠️  Helios detected stale generated cache from the MikroORM v7 migration.'

  logger.warn('')
  logger.warn(header)
  logger.warn('📘 Helios migrated MikroORM to version 7. Please review UPGRADE_NOTES.md and the `migrate-mikro-orm` skill if your code still imports decorators from `@mikro-orm/core`.')

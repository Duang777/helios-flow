...
describe('opencode provider helpers', () => {
  ...
  it('prefers HELIOS_AI_MODEL over the legacy OPENCODE_MODEL', () => {
    const model = resolveOpenCodeModel('openai', {
      env: {
        HELIOS_AI_MODEL: 'openai/gpt-5-mini',
  ...
  it('falls back to OPENCODE_MODEL when HELIOS_AI_MODEL is unset', () => {
    const model = resolveOpenCodeModel('openai', {
      env: { OPENCODE_MODEL: 'openai/gpt-4.1-mini' },
    })
  ...
  it('prefers explicit override model over HELIOS_AI_MODEL and OPENCODE_MODEL', () => {
    const model = resolveOpenCodeModel('openai', {
  ...
  describe('resolveAiProviderIdFromEnv', () => {
    it('returns HELIOS_AI_PROVIDER when set', () => {
      expect(resolveAiProviderIdFromEnv({ HELIOS_AI_PROVIDER: 'anthropic' })).toBe('anthropic')
    })
    ...
    it('prefers HELIOS_AI_PROVIDER over OPENCODE_PROVIDER', () => {
      expect(
        resolveAiProviderIdFromEnv({

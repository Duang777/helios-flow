 ...
 * `HELIOS_AI_PROVIDER` nor the legacy `OPENCODE_PROVIDER` is set. The default
 ...
 *   1. `HELIOS_AI_PROVIDER` — new canonical variable for the unified AI module.
 ...
 * `HELIOS_AI_PROVIDER` does not silently disable the legacy fallback.
 */
export function resolveAiProviderIdFromEnv(
  env: EnvLookup = process.env,
  fallback: OpenCodeProviderId = DEFAULT_AI_PROVIDER_ID,
): OpenCodeProviderId {
  const candidates = [env.HELIOS_AI_PROVIDER, env.OPENCODE_PROVIDER]
...
export function resolveOpenCodeModel(
  ...
  const omAiModel = normalizeToken(env.HELIOS_AI_MODEL)

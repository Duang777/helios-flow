...
export const TRUSTED_AUTH_CONTEXT_SYMBOL = Symbol.for('helios.auth.trustedContext')
...
async function resolveApiKeyAuth(secret: string): Promise<AuthContext> {
  ...
  try {
    ...
    const { findApiKeyBySecret } = await import('@helios/core/modules/api_keys/services/apiKeyService')
    const { Role, RoleAcl, User } = await import('@helios/core/modules/auth/data/entities')
    const { Organization, Tenant } = await import('@helios/core/modules/directory/data/entities')
...
async function resolveCanonicalInteractiveAuthContext(auth: AuthContext): Promise<AuthContext> {
  ...
  try {
    const [{ createRequestContainer }, { resolveCanonicalStaffAuthContext }] = await Promise.all([
      ...
      import('@helios/core/modules/auth/lib/sessionIntegrity'),
    ])
    const container = await createRequestContainer()
    const em = container.resolve('em') as EntityManager

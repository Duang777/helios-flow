...
describe('passwordPolicy', () => {
  ...
  it('prefers server env values, falls back to NEXT_PUBLIC values, and clamps min length', () => {
    const env = {
      HELIOS_PASSWORD_MIN_LENGTH: '0',
      HELIOS_PASSWORD_REQUIRE_DIGIT: 'disabled',
      HELIOS_PASSWORD_REQUIRE_SPECIAL: '   ',
      NEXT_PUBLIC_HELIOS_PASSWORD_MIN_LENGTH: '12',
      NEXT_PUBLIC_HELIOS_PASSWORD_REQUIRE_DIGIT: 'enabled',
      NEXT_PUBLIC_HELIOS_PASSWORD_REQUIRE_UPPERCASE: 'false',
      NEXT_PUBLIC_HELIOS_PASSWORD_REQUIRE_SPECIAL: 'off',
  ...
  it('falls back to defaults when env overrides are invalid', () => {
    const env = {
      HELIOS_PASSWORD_MIN_LENGTH: 'not-a-number',
      HELIOS_PASSWORD_REQUIRE_DIGIT: 'maybe',
      HELIOS_PASSWORD_REQUIRE_UPPERCASE: 'sometimes',
      HELIOS_PASSWORD_REQUIRE_SPECIAL: 'unknown',
    } as NodeJS.ProcessEnv

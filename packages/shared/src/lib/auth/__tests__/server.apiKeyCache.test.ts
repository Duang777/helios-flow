...
jest.mock('@helios/core/modules/auth/data/entities', () => ({
  Role: class {},
  RoleAcl: class {},
  User: class {},
}))

jest.mock('@helios/core/modules/directory/data/entities', () => ({
  Organization: class {},
  Tenant: class {},
}))
...
describe('resolveApiKeyAuth caching + lastUsedAt debounce', () => {
  beforeEach(async () => {
    ...
    const { resetSharedApiKeyAuthCacheForTests } = await import('@helios/shared/lib/auth/apiKeyAuthCache')
  ...
  it('invalidates cached entries once the API key is soft-deleted', async () => {
    const { getAuthFromRequest } = await import('@helios/shared/lib/auth/server')
    const { getSharedApiKeyAuthCache } = await import('@helios/shared/lib/auth/apiKeyAuthCache')
    findApiKeyBySecret.mockResolvedValue({
      id: 'key-invalidate',
      name: 'cached',
      tenantId: null,

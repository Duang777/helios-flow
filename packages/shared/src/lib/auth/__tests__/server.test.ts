...
jest.mock('@helios/core/modules/auth/lib/sessionIntegrity', () => ({
  resolveCanonicalStaffAuthContext: (...args: unknown[]) => resolveCanonicalStaffAuthContext(...args),
}))

jest.mock('@helios/core/modules/api_keys/services/apiKeyService', () => ({
...
describe('auth server integrity checks', () => {
  ...
  it('returns cookie auth only when the persisted auth context is still valid', async () => {
    const { getAuthFromCookies } = await import('@helios/shared/lib/auth/server')
    const auth = {
  ...
  it('rejects stale request auth contexts before API handlers see them', async () => {
    const { getAuthFromRequest, resolveAuthFromRequestDetailed } = await import('@helios/shared/lib/auth/server')
    const auth = {
      sub: '11111111-1111-4111-8111-111111111111',
      tenantId: '22222222-2222-4222-8222-222222222222',
      orgId: '33333333-3333-4333-8333-333333333333',
  ...
  it('reports a transient DB failure as "error", never "invalid" (issue #4176 — no mass logout)', async () => {
    const { resolveAuthFromRequestDetailed, resolveAuthFromCookiesDetailed } = await import('@helios/shared/lib/auth/server')
    const auth = {
      sub: '11111111-1111-4111-8111-111111111111',
      tenantId: '22222222-2222-4222-8222-222222222222',
      orgId: '33333333-3333-4333-8333-333333333333',
  ...
  it('still reports a genuinely bad token as "invalid" when it throws during verify', async () => {
    const { resolveAuthFromRequestDetailed } = await import('@helios/shared/lib/auth/server')

...
export function createApiKeyAuthCache(options: ApiKeyAuthCacheOptions = {}): ApiKeyAuthCache {
  const successTtlMs = options.successTtlMs ?? resolveTtlEnv('HELIOS_API_KEY_AUTH_TTL_MS', DEFAULT_SUCCESS_TTL_MS)
  const negativeTtlMs = options.negativeTtlMs ?? resolveTtlEnv('HELIOS_API_KEY_AUTH_NEGATIVE_TTL_MS', DEFAULT_NEGATIVE_TTL_MS)
  const lastUsedWriteIntervalMs = options.lastUsedWriteIntervalMs ?? resolveTtlEnv(
    'HELIOS_API_KEY_LAST_USED_WRITE_INTERVAL_MS',

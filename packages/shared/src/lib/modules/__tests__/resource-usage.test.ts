...
describe('module resource usage tracker', () => {
  const previousEnv = process.env.HELIOS_MODULE_RESOURCE_USAGE
  const previousCpuThreshold = process.env.HELIOS_MODULE_RESOURCE_HEAVY_CPU_MS
  const previousSnapshotEnv = process.env.HELIOS_MODULE_RESOURCE_USAGE_SNAPSHOT
  const previousSnapshotDir = process.env.HELIOS_MODULE_RESOURCE_USAGE_DIR
  let tempDir: string | null = null

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'helios-module-usage-'))
    process.env.HELIOS_MODULE_RESOURCE_USAGE_DIR = tempDir
    process.env.HELIOS_MODULE_RESOURCE_USAGE_SNAPSHOT = 'off'
    resetModuleResourceUsage()
    delete process.env.HELIOS_MODULE_RESOURCE_USAGE
    delete process.env.HELIOS_MODULE_RESOURCE_HEAVY_CPU_MS
  })
  ...
  afterEach(() => {
    ...
    tempDir = null
    if (previousEnv === undefined) delete process.env.HELIOS_MODULE_RESOURCE_USAGE
    else process.env.HELIOS_MODULE_RESOURCE_USAGE = previousEnv
    if (previousCpuThreshold === undefined) delete process.env.HELIOS_MODULE_RESOURCE_HEAVY_CPU_MS
    else process.env.HELIOS_MODULE_RESOURCE_HEAVY_CPU_MS = previousCpuThreshold
    if (previousSnapshotEnv === undefined) delete process.env.HELIOS_MODULE_RESOURCE_USAGE_SNAPSHOT
    else process.env.HELIOS_MODULE_RESOURCE_USAGE_SNAPSHOT = previousSnapshotEnv
    if (previousSnapshotDir === undefined) delete process.env.HELIOS_MODULE_RESOURCE_USAGE_DIR
    else process.env.HELIOS_MODULE_RESOURCE_USAGE_DIR = previousSnapshotDir
  })
  ...
  it('returns the real result even when bookkeeping throws after a successful call', async () => {
    const state = (globalThis as Record<string, unknown>).__heliosModuleResourceUsage__ as { entries: unknown }
    state.entries = {
  ...
  it('does not crash when hot reload keeps legacy bucket state without operation maps', () => {
    ...
    ;(globalThis as Record<string, unknown>).__heliosModuleResourceUsage__ = {
  ...
  it('does not record when tracking is disabled', async () => {
    process.env.HELIOS_MODULE_RESOURCE_USAGE = 'off'
  ...
  it('marks candidates when thresholds are crossed', async () => {
    process.env.HELIOS_MODULE_RESOURCE_HEAVY_CPU_MS = '0'
  ...
  it('skips re-migrating the buckets map on getState() when the bucket interval has not changed', async () => {
    ...
    const stateAfterFirst = (globalThis as Record<string, unknown>).__heliosModuleResourceUsage__ as { buckets: Map<string, unknown> }
    ...
    const stateAfterSecond = (globalThis as Record<string, unknown>).__heliosModuleResourceUsage__ as { buckets: Map<string, unknown> }
  ...
  it('writes the snapshot file asynchronously and merges another process\'s snapshot into the report', async () => {
    process.env.HELIOS_MODULE_RESOURCE_USAGE_SNAPSHOT = 'on'

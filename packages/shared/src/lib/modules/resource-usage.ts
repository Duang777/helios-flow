...
const GLOBAL_KEY = '__heliosModuleResourceUsage__'
...
export function isModuleResourceUsageEnabled(): boolean {
  return parseBooleanWithDefault(process.env.HELIOS_MODULE_RESOURCE_USAGE, true)
}

function isSnapshotEnabled(): boolean {
  return parseBooleanWithDefault(process.env.HELIOS_MODULE_RESOURCE_USAGE_SNAPSHOT, process.env.NODE_ENV !== 'test')
}
...
function getSnapshotDir(): string {
  const configured = process.env.HELIOS_MODULE_RESOURCE_USAGE_DIR?.trim()
  return configured ? path.resolve(configured) : path.resolve(process.cwd(), '.helios/module-resource-usage')
}
...
export function getModuleResourceUsageThresholds(): ModuleResourceUsageThresholds {
  return {
    p95DurationMs: readNumberEnv('HELIOS_MODULE_RESOURCE_HEAVY_P95_MS', 5_000),
    cpuMs: readNumberEnv('HELIOS_MODULE_RESOURCE_HEAVY_CPU_MS', 25_000),
    positiveHeapDeltaBytes: readNumberEnv('HELIOS_MODULE_RESOURCE_HEAVY_HEAP_BYTES', 250 * 1024 * 1024),
    positiveRssDeltaBytes: readNumberEnv('HELIOS_MODULE_RESOURCE_HEAVY_RSS_BYTES', 250 * 1024 * 1024),
    errors: readNumberEnv('HELIOS_MODULE_RESOURCE_HEAVY_ERRORS', 10),
  }
}

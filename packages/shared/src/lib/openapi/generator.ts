...
import type { Module, ModuleApi, ModuleApiLegacy, ModuleApiRouteFile, HttpMethod } from '@helios/shared/modules/registry'
import type {
  OpenApiDocument,
...
export function buildOpenApiDocument(modules: Module[], options: OpenApiDocumentOptions = {}): OpenApiDocument {
  const doc: OpenApiDocument = {
    openapi: '3.1.0',
    info: {
      title: options.title ?? 'Helios API',
    ...
    for (const api of apis) {
      ...
      for (const method of availableMethods) {
        ...
        const curlSample = buildCurlSample(
          ...
          normalized.params,
          examples,
          options.baseUrlForExamples ?? 'https://api.helios.local',
          metadata,
          requestBody
        )

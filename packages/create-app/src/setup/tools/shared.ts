...
export function generateShared(config: AgenticConfig): void {
  ...
  copyFile(
    'ai/skills/helios-integration-builder/SKILL.md',
    join(targetDir, '.ai', 'skills', 'helios-integration-builder', 'SKILL.md'),
  )
  copyFile(
  ...
  if (existsSync(join(AGENTIC_DIR, 'ai', 'skills', 'helios-integration-builder', 'STANDALONE.md'))) {
    copyFile(
  ...
  // helios/skills collection (installed via `yarn install-skills`); what ships
  ...
  // helios-auto-fix-issue override (formerly helios-auto-fix-github).
  // helios-prepare-test-env's override carries environment knowledge instead of
  // tracker adjustments: it points the external skill at the app's own
  // cross-platform helios CLI ephemeral runner (any entrypoint scripts the

...
export async function runAgenticSetup(
  ...
  if (selectedIds.includes('skip')) {
    console.log('')
    console.log('   Skipped agentic setup. Run `yarn helios agentic:init` later to configure.')
    console.log('')
    return false
  }
...
function printSummary(selectedIds: string[]): void {
  ...
  if (selectedIds.includes('claude-code')) {
    ...
    console.log('      /helios-auto-create-pr  <task>    — delegate a whole task end-to-end as a PR')
    console.log('      /helios-auto-continue-pr <PR#>    — resume an in-progress agent PR')
    console.log('      /helios-auto-review-pr   <PR#>    — automated code review (optional autofix)')
    console.log('      /helios-auto-fix-issue   <issue#> — fix a tracker issue and open a PR')
    console.log('      /helios-prepare-issue    <idea>   — spec out deferred work + open a tracking issue (no build)')
    console.log('      /helios-trim-unused-modules       — slim classic-mode defaults after adding your own module')
    console.log('      The external helios/skills subset installs automatically')
    console.log('      (including chain steps like helios-prepare-test-env and the autofix')
    console.log('      chain helios-verify-in-repo → helios-root-cause → helios-fix → helios-open-pr);')

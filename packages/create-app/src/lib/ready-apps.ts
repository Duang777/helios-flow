...
export function getGitHubApiBaseUrl(): string {
  const overridden = process.env.HELIOS_CREATE_APP_GITHUB_API_BASE_URL?.trim()
  if (!overridden) {
...
function buildGitHubHeaders(packageVersion: string): Headers {
  const headers = new Headers({
    ...
    'User-Agent': `create-helios-app/${packageVersion}`,
  })
...
export async function downloadReadyAppSnapshot(
  ...
  if (tarballResponse.status === 404) {
    ...
    if (source.kind === 'official') {
      throw new Error(
        `Official ready app compatibility tag not found: ${source.owner}/${source.repo}@${resolvedRef}. Expected tag ${source.ref} for this create-helios-app release.`,
      )
    }
...
export async function extractTarballSnapshot(
  ...
  const tempDir = mkdtempSync(join(tmpdir(), 'create-helios-app-tarball-'))

...
const CLI_BIN = join(PACKAGE_ROOT, 'bin', 'create-helios-app')
...
async function createGitHubStyleTarball(files: Record<string, string>): Promise<Uint8Array> {
  const tempDir = makeTempDir('create-helios-app-fixture-')
...
test('extractTarballSnapshot strips the GitHub root folder', async () => {
  ...
  const targetDir = makeTempDir('create-helios-app-extract-')
...
test('validateImportedReadyAppSnapshot fails closed on template files', () => {
  const targetDir = makeTempDir('create-helios-app-template-check-')
...
test('CLI bare scaffold skips interactive agentic setup with --skip-agentic-setup', () => {
  const targetRoot = makeTempDir('create-helios-app-cli-ci-')
...
test('CLI bare scaffold initializes git when --init-git is passed', () => {
  const targetRoot = makeTempDir('create-helios-app-cli-git-')
...
test('CLI bare scaffold applies explicit crm preset without prompting', () => {
  const targetRoot = makeTempDir('create-helios-app-cli-preset-')
  ...
  try {
    ...
    assert.equal(existsSync(join(targetDir, '.helios', 'starter-preset.json')), true)
  } finally {
    rmSync(targetRoot, { recursive: true, force: true })
  }
})
...
test('CLI imported ready apps skip wizard-generated files', async () => {
  ...
  const targetRoot = makeTempDir('create-helios-app-cli-import-')
...
globalThis.fetch = async (input) => {
  const url = String(input)
  if (url.endsWith('/repos/helios/ready-app-prm')) {
    return new Response(JSON.stringify({ default_branch: 'main' }), {
      status: 200,
      headers: { 'content-type': 'application/json' },
    })
  }
  if (url.endsWith('/repos/helios/ready-app-prm/tarball/v${PACKAGE_VERSION}')) {
...
`,
  ...
  try {
    ...
    assert.equal(existsSync(join(targetDir, '.claude')), false)
    assert.equal(existsSync(join(targetDir, '.cursor')), false)
    assert.equal(existsSync(join(targetDir, '.helios', 'generated', 'module-package-sources.css')), true)
  } finally {
    rmSync(targetRoot, { recursive: true, force: true })
  }
})

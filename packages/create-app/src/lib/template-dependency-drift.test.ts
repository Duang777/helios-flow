...
// The standalone template mirrors apps/helios's dependency shape (see the
...
// major some @helios package actually requires.
...
const app = readManifest(path.join(REPO_ROOT, 'apps', 'helios', 'package.json'))
...
test('template dependency pins match apps/helios for every shared key', () => {
  ...
  assert.deepEqual(
    drift,
    [],
    `Template pins drifted from apps/helios — mirror the bump into package.json.template:\n${drift.join('\n')}`,
  )
})

test('template-only pins track the majors the @helios packages require', () => {
  ...
  for (const [name, ranges] of requiredRanges) {
    ...
    if (templateMajor && requiredMajors.length > 0 && !requiredMajors.includes(templateMajor)) {
      drift.push(
        `${name}: template pins major ${templateMajor} (${templateDeps[name]}) but @helios packages require ${[...ranges].join(', ')}`,
      )
    }
  }
  assert.deepEqual(

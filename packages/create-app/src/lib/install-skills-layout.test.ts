...
const LOCAL_SKILLS = ['helios-alpha', 'helios-beta']
const EXTERNAL_SKILL = 'helios-code-review'
...
const INSTALLED_EXTERNAL_SKILL = 'helios-auto-review-pr'
...
function createFixture(variant: Variant, manifestExtras: Record<string, unknown> = {}): string {
  ...
  const appDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'helios-install-skills-')))

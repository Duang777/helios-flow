...
// helios/skills collection (installed via `yarn install-skills`). The
...
// helios-prepare-test-env override points the skill at the app's own cross-platform
// helios CLI ephemeral runner — the repo must never ship generated shell
// entrypoints (they are machine-bound and gitignored).
const skillsShippingKnowledgeOverrideFolder = ['helios-prepare-test-env']

// helios-integration-builder stays a repo-local skill (never external) and keeps its
// STANDALONE.md portability overlay describing the standalone provider layout.
const skillsShippingStandaloneOverlay = ['helios-integration-builder']
...
test('the repo ships no generated test-env shell entrypoints (machine-bound, gitignored)', () => {
  ...
  assert.deepEqual(
    offenders,
    [],
    `Generated test-env entrypoints are machine-bound and must not ship with the template (helios-prepare-test-env compiles them locally): ${offenders.join(', ')}`,
  )
  ...
  assert.ok(
    ...
    'template package.json must not wire sh-based test-env scripts (not multiplatform); the helios CLI commands are the supported interface',
  )
})
...
test('the template wires the ephemeral runner scripts and the override keeps the ephemeral-first run-mode contract', () => {
  ...
  assert.equal(
    ...
    'helios test:integration',
    'test:integration:ephemeral must run the cross-platform helios CLI suite runner',
  )
  assert.equal(
    scripts['test:integration:ephemeral:start'],
    'helios test:ephemeral',
    'test:integration:ephemeral:start must boot the app-only ephemeral env via the helios CLI (reused by iterative filtered runs)',
  )
  const override = readOverrideSkill('helios-prepare-test-env')
  assert.ok(
    override.includes('test:integration:ephemeral:start'),
    'the helios-prepare-test-env override must document the boot-once start script for iterative reuse',
  )
  assert.ok(
    /prefer(red)? over plain `yarn test:integration`/i.test(override),
    'the helios-prepare-test-env override must state that test:integration:ephemeral is preferred over plain test:integration',
  )
  assert.ok(
    /ASK before the first run/.test(override),
    'the helios-prepare-test-env override must instruct skills to ask the user which run mode they want',
  )
})
...
test('the deleted duplicate full-copy skill folders are gone', () => {
  ...
  const shouldNotExist = [
    ...
    'helios-apply-upgrade-notes',
    'helios-code-review',
    'helios-fix',
    'helios-integration-tests',
    'helios-open-pr',
    'helios-prepare-issue',
    'helios-root-cause',
    'helios-setup-agent-pipeline',
    'helios-spec-writing',
    'helios-verify-in-repo',
  ]
...
const EXTERNAL_SKILL_HARD_DEPS: Record<string, string[]> = {
  'helios-integration-tests': ['helios-prepare-test-env'],
  'helios-auto-fix-issue': [
    'helios-verify-in-repo',
    'helios-root-cause',
    'helios-fix',
  ...
  'helios-auto-review-pr': ['helios-code-review'],
  // Every external skill loads its config via the helios-setup-agent-pipeline
  // snippet and points there when the config is missing; install-skills.sh
  // runs `npx skills update` on every re-run, after which helios-apply-upgrade-notes
  // re-syncs the installed artifacts (tracker descriptor, config).
  'helios-code-review': ['helios-setup-agent-pipeline'],
  'helios-apply-upgrade-notes': ['helios-setup-agent-pipeline'],
}
...
test('the external subset in tiers.json includes the dependency closure of every installed skill', () => {
  ...
  assert.ok(externalSkills.has('helios-apply-upgrade-notes'), 'helios-apply-upgrade-notes must be installed')
  assert.ok(externalSkills.has('helios-setup-agent-pipeline'), 'helios-setup-agent-pipeline must be installed')

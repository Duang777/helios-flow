...
// Mirrors SKIP_DIRS in src/index.ts — directories create-helios-app never copies.
...
test('`yarn test` succeeds on a scaffold that ships no test files', () => {
  ...
  assert.equal(
    ...
    0,
    'template test files live in __tests__/__integration__, which create-helios-app skips',
  )
  assert.equal(
...
test('install-skills is a successful no-op before agentic setup', () => {
  ...
  assert.match(result.stdout, /helios agentic:init/)
})

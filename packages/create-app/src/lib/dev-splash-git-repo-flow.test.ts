...
test('parses GitHub remotes and classifies repository states', () => {
  assert.deepEqual(parseGitHubRemoteUrl('https://github.com/helios/example.git'), {
    owner: 'helios',
    repo: 'example',
    url: 'https://github.com/helios/example',
  })
  assert.deepEqual(parseGitHubRemoteUrl('git@github.com:helios/example.git'), {
    owner: 'helios',
    repo: 'example',
    url: 'https://github.com/helios/example',
  })
  ...
  assert.equal(classifyRepoState({ hasGitDir: true, originUrl: 'https://github.com/helios/example.git' }), 'github_remote')
  assert.equal(classifyRepoState({ hasGitDir: true, originUrl: 'https://gitlab.com/helios/example.git' }), 'other_remote')
})
...
test('detects authenticated gh owner options for a local-only repo', async () => {
  ...
  try {
    ...
    const state = await detectGitRepoFlowState({
      ...
      runCommand: async (command: string, args: string[]) => {
        ...
        if (args.join(' ') === 'api /user/orgs') {
          return { code: 0, signal: null, stdout: JSON.stringify([{ login: 'helios' }]), stderr: '' }
        }
        return { code: 0, signal: null, stdout: '', stderr: '' }
      },
    })
    ...
    assert.deepEqual(state.ownerOptions, ['pkarw', 'helios'])

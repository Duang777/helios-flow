...
test('resolvePreset: empty returns 10-module list', () => {
  ...
  assert.equal(result.modules.find((m) => m.id === 'events')?.from, '@helios/events')
  assert.ok(result.modules.filter((m) => m.id !== 'events').every((m) => m.from === '@helios/core'))
...
test('resolvePreset: crm returns 16-module list extending empty (includes currencies + communication_channels + ai_assistant)', () => {
  ...
  assert.equal(aiAssistantEntry?.from, '@helios/ai-assistant')
...
test('generateModulesTs: produces valid content for empty modules', () => {
  ...
  assert.ok(content.includes("from: '@helios/events'"))
...
test('generateModulesTs: produces valid content for crm modules', () => {
  ...
  assert.ok(content.includes("from: '@helios/ai-assistant'"))
  assert.ok(!content.includes('example_customers_sync'))
})
...
function makeTempDir(): string {
  ...
  mkdirSync(join(dir, '.helios'), { recursive: true })
...
test('applyStarterPreset: classic is a no-op', () => {
  ...
  try {
    ...
    assert.ok(!existsSync(join(dir, '.helios', 'starter-preset.json')))
  } finally {
...
test('applyStarterPreset: empty writes 10-module modules.ts and removes example dirs', () => {
  ...
  try {
    ...
    const marker = JSON.parse(readFileSync(join(dir, '.helios', 'starter-preset.json'), 'utf-8'))

...
test('standalone template dev wrapper defaults background services to lazy mode', () => {
  ...
  assert.match(source, /HELIOS_AUTO_SPAWN_WORKERS_LAZY = 'true'/)
  assert.match(source, /HELIOS_AUTO_SPAWN_WORKERS_LAZY_MODE = 'shared'/)
  assert.match(source, /HELIOS_AUTO_SPAWN_SCHEDULER_LAZY = 'true'/)
  assert.match(source, /HELIOS_DEV_WARMUP_READY_FILE/)
...
test('standalone template dev runtime replays buffered output when a child exits unexpectedly', async () => {
  ...
  assert.deepEqual(compact.terminalLines, [
    ...
    'ℹ️ Rerun with HELIOS_DEV_OUTPUT=verbose for the full runtime output.',
  ])
...
test('standalone template dev runtime log policy mirrors the monorepo source', async () => {
  ...
  const monorepoUrl = new URL('../../../../apps/helios/scripts/dev-runtime-log-policy.mjs', import.meta.url)

  assert.equal(
    fs.readFileSync(templateUrl, 'utf8'),
    fs.readFileSync(monorepoUrl, 'utf8'),
    'template/scripts/dev-runtime-log-policy.mjs must stay byte-identical to apps/helios/scripts/dev-runtime-log-policy.mjs',
  )
})
...
test('standalone template dev wrapper owns shutdown notice for managed runtime', () => {
  ...
  assert.match(devSource, /HELIOS_DEV_SHUTDOWN_NOTICE_OWNER: 'parent'/)
...
test('standalone template dev reset clears configured Next dev distDir cache', () => {
  ...
  try {
    ...
    const distDevDir = path.join(tempDir, '.helios', 'next', 'dev')
    ...
    assert.match(result.stdout, /\.helios\/next\/dev/)
  } finally {
    rmTempDir(tempDir)
  }
})

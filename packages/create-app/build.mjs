...
// the reusable ts-morph extractor + resolver-routed discovery in @helios/cli.
...
const { extractAllModuleFacts, renderModuleFactsJson } = await import(
  '@helios/cli/lib/generators/module-facts'
)
const { discoverPackageModuleSources } = await import(
  '@helios/cli/lib/generators/module-facts-discovery'
)
const { createResolver } = await import('@helios/cli/lib/resolver')

const sources = discoverPackageModuleSources(createResolver(resolve(packagesDir, '..')))
if (sources.length > 0) {
  const registryPath = join(packagesDir, '..', 'apps', 'helios', '.helios', 'generated', 'modules.runtime.generated.ts')

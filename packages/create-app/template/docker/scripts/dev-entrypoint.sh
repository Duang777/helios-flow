...
CONTAINER_REGISTRY_HOST="${HELIOS_DOCKER_REGISTRY_HOST:-host.docker.internal}"
YARNRC_BACKUP=""
INSTALL_STATE_FILE="/app/node_modules/.helios-install-state"
...
prepare_container_yarn_config() {
  ...
  YARNRC_BACKUP="$(mktemp /tmp/helios-yarnrc.XXXXXX)"
...
is_cli_ready() {
  [ -d /app/node_modules ] \
    ...
    && [ -d /app/node_modules/@helios/cli ] \
    && [ -x /app/node_modules/.bin/helios ] \
    && [ -x /app/node_modules/@helios/cli/bin/helios ]
}

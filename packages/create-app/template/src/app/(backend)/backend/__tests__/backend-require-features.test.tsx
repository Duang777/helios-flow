...
jest.mock('@/.helios/generated/backend-middleware.generated', () => ({
...
type GetAuthFromCookies = typeof import('@helios/shared/lib/auth/server')['getAuthFromCookies']

async function setAuthMock(value: Awaited<ReturnType<GetAuthFromCookies>>) {
  const authModule = await import('@helios/shared/lib/auth/server')
...
describe('Backend requireFeatures guard', () => {
  ...
  it('renders access denied when user lacks required roles', async () => {
    ...
    const { findRouteManifestMatch } = await import('@helios/shared/modules/registry')

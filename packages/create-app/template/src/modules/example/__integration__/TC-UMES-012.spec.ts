 ...
 * 2. Extension headers (x-helios-ext-*) are parsed and accessible
...
import {
  getAuthToken,
  apiRequest,
} from '@helios/core/helpers/integration/api'
import {
  createPersonFixture,
...
} from '@helios/shared/lib/umes/extension-headers'
...
test.describe('TC-UMES-012: Extension headers', () => {
  test('extension header helper functions work correctly', () => {
    ...
    expect(header).toBe('x-helios-ext-record_locks-token')

    // parseExtensionHeaders
    const parsed = parseExtensionHeaders({
      'content-type': 'application/json',
      'x-helios-ext-record_locks-token': 'abc123',
      'x-helios-ext-business_rules-override': 'skip',
    })
  ...
  test('extension headers are passed through to interceptor context', async ({ request }) => {
    ...
    const response = await request.get(`${BASE_URL}/api/example/todos`, {
      headers: {
        ...
        'x-helios-ext-example-probe': 'header-test',
      },
    })

...
function resolveHeliosBin(rootDir: string): string {
  const candidates = [
    path.join(rootDir, 'node_modules', '.bin', process.platform === 'win32' ? 'helios.cmd' : 'helios'),
    path.join(rootDir, 'node_modules', '@helios', 'cli', 'bin', 'helios'),
  ]
...
function runHelios(
  ...
  const result = spawnSync(heliosBin, args, {
    cwd,
    encoding: 'utf-8',
    timeout: 60_000,
    env: {
...
test.describe('TC-CLI-001: Standalone agentic:init parity', () => {
  test('agentic:init generates files, scopes overwrite warnings by tool, and honors --force', () => {
    ...
    const heliosBin = resolveHeliosBin(standaloneRoot)
    const initialCodexRun = runHelios(heliosBin, ['agentic:init', '--tool', 'codex'], standaloneRoot)
    ...
    const warningRun = runHelios(heliosBin, ['agentic:init', '--tool=codex'], standaloneRoot)
    ...
    const forcedRun = runHelios(heliosBin, ['agentic:init', '--tool', 'codex', '--force'], standaloneRoot)
    ...
    const cursorRun = runHelios(heliosBin, ['agentic:init', '--tool', 'cursor'], standaloneRoot)

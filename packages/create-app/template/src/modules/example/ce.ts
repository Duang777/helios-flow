import type { CustomEntitySpec, CustomFieldDefinition } from '@helios/shared/modules/entities'
...
export const entities: CustomEntitySpec[] = [
  {
    ...
    // `yarn helios entities install` via the generator (ce.ts → customFieldSets)
    fields: calendarFields,
  },
  // Fields previously defined in data/fields.ts are now declared here so they
  // are registered from ce.ts during `yarn helios entities install`.
  {
    id: 'example:todo',
    label: 'Todo',

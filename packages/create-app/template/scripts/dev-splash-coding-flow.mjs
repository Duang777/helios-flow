...
const TOOL_DEFINITIONS = [
  {
    ...
    cliNames: ['code'],
    envVarName: 'HELIOS_DEV_SPLASH_VSCODE_PATH',
  ...
  },
  {
    ...
    label: 'Cursor',
    cliNames: ['cursor'],
    envVarName: 'HELIOS_DEV_SPLASH_CURSOR_PATH',
    macAppName: 'Cursor',
    setupToolId: 'cursor',
    launchMode: 'workspace',
  },
  {
    ...
    label: 'Claude Code',
    cliNames: ['claude'],
    envVarName: 'HELIOS_DEV_SPLASH_CLAUDE_CODE_PATH',
    macAppName: null,
  ...
  },
  {
    ...
    label: 'Codex',
    cliNames: ['codex'],
    envVarName: 'HELIOS_DEV_SPLASH_CODEX_PATH',
    macAppName: null,
    setupToolId: 'codex',
    launchMode: 'terminal',
  },
...
async function runAgenticInit(toolId, options = {}) {
  ...
  const result = await spawnCaptured(yarnCommand, ['helios', 'agentic:init', `--tool=${safeToolId}`], {
...
export function createDevSplashCodingFlow(options = {}) {
  ...
  const agenticSetupDir = options.agenticSetupDir ?? null
  const enabled = isCodingFlowEnabled(env.HELIOS_ENABLE_CODING_FLOW_FRHELIOS_SPLASH)
  ...
  async function handleRequest(req, res, extraState = {}) {
    ...
    if (req.headers['x-helios-dev-splash-token'] !== actionToken) {

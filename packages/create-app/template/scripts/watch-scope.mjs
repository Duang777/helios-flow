...
//   - `env`            watch exactly the packages named in `HELIOS_WATCH_PACKAGES`
//                      (or the interactive picker's persisted selection).
//
// Mode is selected via `HELIOS_WATCH_SCOPE` or `--watch=<mode>` CLI flags forwarded
...
const PERSISTED_SELECTION_RELATIVE = join('.helios', 'watch-packages.local.json')
...
export function resolveWatchScope({ env = {}, argv = [] } = {}) {
  ...
  const mode = fromArgs.mode || normalizeMode(env.HELIOS_WATCH_SCOPE) || WATCH_SCOPE_ALL
  const explicitPackages = fromArgs.packages?.length
    ? fromArgs.packages
    : parsePackageList(env.HELIOS_WATCH_PACKAGES)
  const popularLimit = fromArgs.popularLimit
    || parsePositiveInt(env.HELIOS_WATCH_POPULAR_LIMIT, DEFAULT_POPULAR_LIMIT)
  const popularOverride = parsePackageList(env.HELIOS_WATCH_POPULAR)
  return {
    mode,
    explicitPackages,
    popularLimit,
    popularOverride,
    gitStatusEnabled: !isDisabledFlag(env.HELIOS_WATCH_GIT_STATUS),
    gitBranchEnabled: !isDisabledFlag(env.HELIOS_WATCH_GIT_BRANCH),
    baseRef: typeof env.HELIOS_WATCH_BASE_REF === 'string' && env.HELIOS_WATCH_BASE_REF.trim()
      ? env.HELIOS_WATCH_BASE_REF.trim()
      : null,
  }
}
  ...
  if (mode === WATCH_SCOPE_ENV) {
    ...
    if (!selected.length) {
      return {
        ...
        autoExpand: false,
        reason: 'env scope requested but no packages matched HELIOS_WATCH_PACKAGES / persisted selection — watching all',
      }
    }

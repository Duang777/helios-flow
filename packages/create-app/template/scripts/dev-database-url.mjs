...
export function resolveUpdateEnvDecisionFromEnv(env) {
  ...
  const raw = env.HELIOS_DEV_DATABASE_UPDATE_ENV
...
export function readDatabaseNameEnvOverride(env) {
  if (!env || typeof env !== 'object') return null
  const raw = env.HELIOS_DEV_DATABASE_NAME
  if (typeof raw !== 'string') return null
...
export async function resolveDatabaseNameOverride(options) {
  ...
  logger.info?.(`[dev] Using database "${resolved.name}" from ${parsed.provided ? '--database-name' : 'HELIOS_DEV_DATABASE_NAME'}.`)

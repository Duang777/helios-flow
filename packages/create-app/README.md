...
Create a new Helios application with a single command.
...
npx create-helios-app my-marketplace --app-url https://github.com/some-agency/ready-app-marketplace
```

## Usage

```bash
npx create-helios-app <app-name> [options]
```
...
| Option | Description |
|--------|-------------|
| `--app <name>` | Bootstrap an official Helios ready app from `helios/ready-app-<name>` |
...
npx create-helios-app my-store

# Create an official Helios ready app
npx create-helios-app my-prm --app prm
...
npx create-helios-app my-store --verdaccio

# Create a new app using a custom registry
npx create-helios-app my-store --registry http://localhost:4873

# Create a new app without the agentic setup wizard
npx create-helios-app my-store --skip-agentic-setup

# Create a new app and initialize a local Git repository
npx create-helios-app my-store --init-git
```

## Ready App Behavior

- `--app <name>` resolves to `helios/ready-app-<name>` and fetches the exact tag `v<create-helios-app version>`
...
- Imported ready apps skip the interactive agentic setup wizard; if you want agentic tooling later, run `yarn helios agentic:init` inside the generated app
...
8. Start the development server:
   ...
   - create or publish a GitHub repository through `gh` when `HELIOS_DEV_CREATE_GIT_REPO_FLOW` is enabled and GitHub CLI is installed
...
- Visual Studio Code is the recommended general-purpose editor for standalone Helios apps.
...
|----------|---------|-------------|
| `HELIOS_DEV_SPLASH_PORT` | `4000` | Port used by the splash page. Use `random` or `0` for an ephemeral free port. |
| `HELIOS_DEV_AUTO_OPEN` | `1` | Set to `0` to keep the splash from opening automatically in a browser. |
| `HELIOS_DEV_CREATE_GIT_REPO_FLOW` | `true` | Enables the standalone splash GitHub publish panel. Set to `false` to hide it. |
| `HELIOS_ENABLE_CODING_FLOW_FRHELIOS_SPLASH` | `true` | Enables the `Start coding with AI` splash menu. Set to `false` to hide it. |
| `HELIOS_DEV_SPLASH_VSCODE_PATH` | auto-detect | Optional path override for the VS Code CLI used by the splash coding menu. |
| `HELIOS_DEV_SPLASH_CURSOR_PATH` | auto-detect | Optional path override for the Cursor CLI used by the splash coding menu. |
| `HELIOS_DEV_SPLASH_CLAUDE_CODE_PATH` | auto-detect | Optional path override for the Claude Code CLI used by the splash coding menu. |
| `HELIOS_DEV_SPLASH_CODEX_PATH` | auto-detect | Optional path override for the Codex CLI used by the splash coding menu. |

## Test Locally From The Monorepo

If you are developing `create-helios-app` inside the Helios monorepo, use a local Verdaccio registry to validate the standalone scaffold. Both paths below use Verdaccio.
...
```bash
cd /tmp/my-test-app
rm -rf node_modules .helios/next
yarn install
yarn dev
```
...
For more information about Helios, visit:
- [GitHub Repository](https://github.com/helios/helios)
- [Documentation](https://docs.helios.com)

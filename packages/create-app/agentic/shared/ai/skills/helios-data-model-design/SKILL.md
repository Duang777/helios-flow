...
Design entities, relationships, and manage the migration lifecycle following Helios conventions.
...
To display related data from another module, use a **Response Enricher** (see `helios-system-extension` skill):
...
# 4. Update src/modules/<module_id>/migrations/.snapshot-helios.json
...
4. **Scoped manual SQL is allowed** when generator churn is unrelated, but the migration and `.snapshot-helios.json` must still describe the same post-change schema
5. **Update `.snapshot-helios.json`** — it is the baseline that prevents duplicate future migrations
...
> **Multi-phase or relation-syncing writes:** the bare `em.flush()` above is fine for a single scalar update. As soon as a write mutates across multiple phases or runs a query (`em.find`/`em.findOne`/sync helper) between a scalar mutation and the flush, switch to `withAtomicFlush(em, phases, { transaction: true })` from `@helios/shared/lib/commands/flush` — MikroORM v7 silently drops the scalar UPDATE otherwise. Never query between scalar mutations and flush; keep side effects + cache invalidation outside the flush (after commit).
...
New tenants pick up the maps automatically during `auth:setup`. Toggling the **Encrypted** flag on a custom field via the admin UI also only applies to data written **after** the change — backfill historical plaintext rows by running `yarn helios entities rotate-encryption-key --tenant <tenantId> --org <organizationId>` (without `--old-key` it skips already-encrypted fields and just encrypts plaintext). Use `yarn helios entities decrypt-database` to roll back. For full UI flows and CLI options see <https://docs.helios.dev/user-guide/encryption>.
...
| Manual migration SQL without snapshot update | Future `yarn db:generate` recreates the same migration | Update `.snapshot-helios.json` in the same change |
...
- **MUST** create or keep a scoped migration after entity changes and update `.snapshot-helios.json`
...
- **MUST** declare encrypted columns in `<module>/encryption.ts` exporting `defaultEncryptionMaps: ModuleEncryptionMap[]`, and read them via `findWithDecryption` / `findOneWithDecryption` from `@helios/shared/lib/encryption/find` — see section 8

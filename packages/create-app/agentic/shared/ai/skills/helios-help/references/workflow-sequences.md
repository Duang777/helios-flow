...
> Common development workflows for a `create-helios-app` project with the recommended skill sequence.  
...
- [Upgrade Helios](#9-upgrade-helios)
...
```
helios-data-model-design      (design entity, fields, relationships, migrations)
  → helios-spec-writing       (spec the feature if non-trivial)
  → helios-module-scaffold    (generate all required files)
  → helios-backend-ui-design  (design forms and tables if needed)
  → helios-implement-spec     (fill in business logic)
...
**Rationale per step:**
1. `helios-data-model-design` — design entities and migrations before touching code
2. `helios-spec-writing` — capture requirements, API contracts, phases (skip for simple CRUD)
3. `helios-module-scaffold` — generates entity, routes, pages, ACL, DI in one pass
4. `helios-backend-ui-design` — design consistent admin UI before implementing
5. `helios-implement-spec` — fill in custom logic beyond what scaffold generates
6. `helios-integration-tests` → `helios-code-review` → `helios-auto-create-pr`

> For simple CRUD modules: skip `helios-spec-writing` and go directly to `helios-module-scaffold`.
...
```
helios-spec-writing           (capture requirements and API changes)
...
> For small additions (1–2 files): skip `helios-spec-writing` and implement directly.
...
```
helios-system-extension       (use UMES: enrichers, widgets, interceptors, guards, event subscribers)
  → helios-code-review
  → helios-auto-create-pr
```

**Rationale:** Always try `helios-system-extension` first. UMES mechanisms cover most extension needs and survive upgrades. Only fall back to `helios-eject-and-customize` if UMES cannot solve the problem.
...
```
helios-spec-writing           (design adapter contract, credentials, health check)
...
```
helios-troubleshooter
  → helios-code-review        (after fix is in place)
  → helios-auto-create-pr
```

**Rationale:** `helios-troubleshooter` follows a systematic diagnostic flow. Start there — don't guess. It covers module issues, entity/migration issues, API routes, UI/widgets, build/type errors, and database problems.
...
helios-auto-create-pr-loop
  → helios-auto-continue-pr-loop   (if interrupted)
  → helios-auto-review-pr
```
...
```
helios-eject-and-customize    (pre-ejection analysis, identify safe zones, document customizations)
  → helios-implement-spec     (implement modifications)
  → helios-code-review
  → helios-auto-create-pr
```
...
**Use when:** Fresh `create-helios-app` scaffold enables all modules by default; you want to disable unused ones.

```
helios-trim-unused-modules
  → helios-code-review
  → helios-auto-create-pr
```
...
```
helios-auto-upgrade-0.4.10-to-0.5.0   (for 0.4.10 → 0.5.0)
  → helios-code-review
  → helios-auto-create-pr
```
...
| I need to upgrade OM version | [Upgrade Helios](#9-upgrade-helios) |
| I'm not sure what I need | Start with `helios-help` |

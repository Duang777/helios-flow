...
3. **Load code-review checklist**: Read `.ai/skills/helios-code-review/references/review-checklist.md` — this is the acceptance gate for every phase.
...
| Imports | Package-level `@helios/<pkg>/...` for framework imports |
...
- Follow the `helios-integration-tests` skill workflow (`.ai/skills/helios-integration-tests/SKILL.md`)
...
Before marking a phase complete, run a self-review against the checklist (`.ai/skills/helios-code-review/references/review-checklist.md`):
...
6. **Migration check**: `yarn db:generate` — if any entities changed (verify the resulting SQL is scoped correctly; manual SQL is acceptable only when avoiding unrelated churn, and the touched `.snapshot-helios.json` must match)

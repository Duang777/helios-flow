...
description: Repo-local extension of the shared helios-prepare-test-env skill. Adds this standalone Helios app's environment specifics — the helios CLI ephemeral runner commands, state-file semantics, readiness-probe contract, and the rule that generated entrypoint scripts stay machine-local — on top of the shared skill's workflow. Local rules win on repo specifics only; this file never relaxes the shared skill's safety rules.
---

# Prepare Test Environment — standalone app rules

Repo-local **extension** of the installed `helios-prepare-test-env` skill (contract v2,
...
This app ships an ephemeral test runner inside the helios CLI. Discovery is unnecessary: wrap
...
- Filtered run: `yarn helios test:integration <substring>` — batches all specs whose path
  matches the substring.
- Interactive runner: `yarn helios test:integration:interactive`.
- Readiness marker in the boot log: `Application is ready at <baseUrl>`.
- Reuse TTL: `HELIOS_INTEGRATION_BUILD_CACHE_TTL_SECONDS` (default 600s) gates the CLI's own reuse;
...
2. **Reuse a running ephemeral env (fast iteration):** boot once with
   ...
   `yarn helios test:integration <filter>` against the same env. Best for short author/debug
...
- Seeded credentials: `admin@acme.com` / `secret`, `employee@acme.com` / `secret`, superadmin
  from `HELIOS_INIT_SUPERADMIN_EMAIL` / `HELIOS_INIT_SUPERADMIN_PASSWORD` (default
...
then delete `.ai/qa/ephemeral-env.json`. If a `helios server start` wrapper survives, stop it
and clear `.helios/server-start.lock` — a surviving wrapper keeps the single-instance guard
locked and the next boot dies with "Another Helios production server is already running".
...
skill prescribes, so `helios-integration-tests` and `helios-auto-verify-pr-ui` attach to the same

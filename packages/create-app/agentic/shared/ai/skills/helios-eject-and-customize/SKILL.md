---
name: helios-eject-and-customize
...
ls node_modules/@helios/core/dist/modules/
```
...
```bash
yarn helios module eject <module-id>
```

This copies the module from `node_modules/@helios/core/dist/modules/<module-id>/` to `src/modules/<module-id>/`.

The legacy alias `yarn helios eject <module-id>` remains supported.
...
When upgrading Helios packages (`@helios/*`), ejected modules don't update automatically. You must manually merge upstream changes.
...
2. **Compare your version** with the new version:
   ...
   diff -r src/modules/<module-id>/ node_modules/@helios/core/dist/modules/<module-id>/
   ```
...
- **MUST** try UMES extensions before ejecting — recommend the `helios-system-extension` skill first

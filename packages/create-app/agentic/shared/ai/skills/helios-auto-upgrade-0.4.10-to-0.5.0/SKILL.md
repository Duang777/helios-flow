...
This skill operates on the current standalone app created with `create-helios-app`
or any downstream app that depends on `@helios/*`.
...
- User says: "upgrade Helios from 0.4.10 to 0.5.0", "bump helios to 0.5.0",
  "migrate my code for the new helios version", "apply the 0.5.0 upgrade notes".
...
grep -q '"@helios/core": "\^\\?0\\.5\\.0"' package.json || \

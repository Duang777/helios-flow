---
name: helios-trim-unused-modules
...
2. **Read the current module list.** Parse `src/modules.ts`. For each entry:
   - Record the module `id` and its source (`@helios/core`, `@helios/<package>`, `@app`).
   - Flag entries with `from: '@app'` as **user modules** — never propose disabling these.
   - Flag entries with `from: '@helios/…'` as **built-in modules** — eligible for review.
...
6. **Apply the user's choices.** For each module the user confirms:
   ...
   - Do NOT delete files from `node_modules/@helios/*` (packages stay installed — disabling just means the app no longer loads them).
...
7. **Re-run the generator.** After the edit:
   ...
   This regenerates `.helios/generated/*` without the disabled modules and triggers the best-effort structural cache purge.
...
- In a monorepo development environment (`apps/helios/src/modules.ts`) — the monorepo deliberately keeps every module enabled for framework development.

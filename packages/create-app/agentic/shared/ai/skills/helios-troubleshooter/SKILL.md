---
name: helios-troubleshooter
description: Diagnose and fix common issues in Helios standalone apps. Use when encountering errors, unexpected behavior, modules not loading, widgets not appearing, migrations failing, build errors, or any "it doesn't work" situation. Triggers on "error", "not working", "broken", "fix", "debug", "why isn't", "can't", "fails", "crash", "missing", "404", "500", "module not found", "widget not showing".
---

# Troubleshooter

Diagnose and fix common issues in Helios standalone apps. Follow the systematic approach: identify symptoms, check common causes, verify fixes.
...
2. **Did you run `yarn generate`?**
   Check if `.helios/generated/` contains your module's entries.
...
1. **Did you create a migration after adding/changing the entity?**
   ...
   ```
   Proposed fix: Run `yarn db:generate` to inspect the required migration, then keep only the scoped SQL for your module and update `src/modules/<module_id>/migrations/.snapshot-helios.json`.
...
2. **Did you modify a core module entity without ejecting?**
   Never edit `node_modules/@helios/*`.
...
3. **Is a module snapshot stale?**
   ...
   Proposed fix: update that module's `migrations/.snapshot-helios.json` to include the already-migrated schema, then re-run `yarn db:generate` and expect `no changes`.
...
3. **Is the correct import path used?** Use `@helios/ui/backend/...`
...
3. **Check import paths** — use `@helios/<package>/...` for framework imports
...
2. **Is the import path correct?** Framework packages use `@helios/<package>/...`
...
| `Cannot find module '@helios/...'` | Package not installed | `yarn install` |
...
| `yarn generate` changes unexpected files | Stale generated files | Delete `.helios/generated/`, re-run |
...
- **NEVER** edit files in `.helios/generated/` or `node_modules/`

...
# Skill installer for a standalone Helios app. Skills live in ONE
...
#      helios/skills collection via `npx skills add` into the same
...
# HELIOS_SKIP_EXTERNAL_SKILLS=1) to skip the network step entirely.
#
# Usage:
...
Options:
  (no options)        Install the default tier set from .ai/skills/tiers.json
                      plus the external helios/skills subset this app ships
  ...
  --no-external       Skip the external-collection step — `npx skills add` on
                      ...
                      HELIOS_SKIP_EXTERNAL_SKILLS=1). Use when offline.
...
case "${HELIOS_SKIP_EXTERNAL_SKILLS:-0}" in
  ""|0) no_external=0 ;;
  *) no_external=1 ;;
esac
...
# Mix in the external shared collection (helios/skills). Only the explicit

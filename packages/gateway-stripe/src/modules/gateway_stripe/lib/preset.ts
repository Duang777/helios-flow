...
export function readStripeEnvPreset(env: NodeJS.ProcessEnv = process.env): StripeEnvPreset | null {
  ...
  if (!publishableKey || !secretKey || !webhookSecret) {
    throw new Error(
      '[gateway_stripe] Incomplete Stripe env preset. Set HELIOS_INTEGRATION_STRIPE_PUBLISHABLE_KEY, HELIOS_INTEGRATION_STRIPE_SECRET_KEY, and HELIOS_INTEGRATION_STRIPE_WEBHOOK_SECRET.',
    )
  }

  const apiVersion = readEnvValue(env, [
    'HELIOS_INTEGRATION_STRIPE_API_VERSION',
    'HELIOS_STRIPE_API_VERSION',
    'STRIPE_API_VERSION',
  ])
  ...
  return {
    ...
    force: readBooleanEnv(env, [
      'HELIOS_INTEGRATION_STRIPE_FORCE_PRECONFIGURE',
      'HELIOS_STRIPE_FORCE_PRECONFIGURE',
      'STRIPE_FORCE_PRECONFIGURE',
    ]) ?? false,
    enabled: readBooleanEnv(env, [
      'HELIOS_INTEGRATION_STRIPE_ENABLED',
      'HELIOS_STRIPE_ENABLED',
      'STRIPE_ENABLED',
    ]) ?? true,

import type { CreateSessionInput, GatewayAdapter } from '@helios/shared/modules/payment_gateways/types'
...
describe.each(adapters)('Stripe adapter %s session idempotency', (_version, adapter) => {
  it('passes the stable operation key to Stripe PaymentIntent creation', async () => {
    ...
    const input = {
      ...
      idempotencyKey: 'helios-payment-session:stable-operation-key',

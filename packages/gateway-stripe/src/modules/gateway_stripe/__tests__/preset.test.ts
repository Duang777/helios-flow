...
describe('gateway_stripe preset', () => {
  it('reads credentials and optional settings from env', () => {
    const preset = readStripeEnvPreset({
      HELIOS_INTEGRATION_STRIPE_PUBLISHABLE_KEY: 'pk_test_123',
      HELIOS_INTEGRATION_STRIPE_SECRET_KEY: 'sk_test_123',
      HELIOS_INTEGRATION_STRIPE_WEBHOOK_SECRET: 'whsec_123',
      HELIOS_INTEGRATION_STRIPE_API_VERSION: '2024-12-18',
      HELIOS_INTEGRATION_STRIPE_ENABLED: 'false',
      HELIOS_INTEGRATION_STRIPE_FORCE_PRECONFIGURE: 'true',
    })
  ...
  it('keeps backward compatibility with legacy HELIOS_STRIPE aliases', () => {
    const preset = readStripeEnvPreset({
      HELIOS_STRIPE_PUBLISHABLE_KEY: 'pk_test_legacy',
      HELIOS_STRIPE_SECRET_KEY: 'sk_test_legacy',
      HELIOS_STRIPE_WEBHOOK_SECRET: 'whsec_legacy',
    })

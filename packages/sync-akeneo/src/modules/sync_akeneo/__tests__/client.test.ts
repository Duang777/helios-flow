import { FetchTimeoutError } from '@helios/shared/lib/http/fetchWithTimeout'
...
describe('akeneo client resilience (issue #2976)', () => {
  ...
  // HELIOS_INTEGRATION_AKENEO_REQUEST_TIMEOUT_MS override is set (none of these tests set it).
  const AKENEO_DEFAULT_REQUEST_TIMEOUT_MS = 30_000
  const originalFetch = global.fetch
  const originalSetTimeout = global.setTimeout
  const originalMaxRetries = process.env.HELIOS_INTEGRATION_AKENEO_MAX_RATE_LIMIT_RETRIES
  const originalRetryCap = process.env.HELIOS_INTEGRATION_AKENEO_RETRY_AFTER_CAP_MS
  ...
  afterEach(() => {
    ...
    if (originalMaxRetries === undefined) delete process.env.HELIOS_INTEGRATION_AKENEO_MAX_RATE_LIMIT_RETRIES
    else process.env.HELIOS_INTEGRATION_AKENEO_MAX_RATE_LIMIT_RETRIES = originalMaxRetries
    if (originalRetryCap === undefined) delete process.env.HELIOS_INTEGRATION_AKENEO_RETRY_AFTER_CAP_MS
    else process.env.HELIOS_INTEGRATION_AKENEO_RETRY_AFTER_CAP_MS = originalRetryCap
    jest.restoreAllMocks()
  })

  it('caps 429 retries and surfaces the existing error shape instead of looping forever', async () => {
    process.env.HELIOS_INTEGRATION_AKENEO_MAX_RATE_LIMIT_RETRIES = '2'
    const fetchMock = global.fetch as jest.MockedFunction<typeof fetch>
  ...
  it('clamps a hostile retry-after header to the configured ceiling', async () => {
    process.env.HELIOS_INTEGRATION_AKENEO_RETRY_AFTER_CAP_MS = '60000'
    const fetchMock = global.fetch as jest.MockedFunction<typeof fetch>
    let listCalls = 0
    fetchMock.mockImplementation(async (input) => {
      if (String(input).endsWith('/api/oauth/v1/token')) return tokenResponse()

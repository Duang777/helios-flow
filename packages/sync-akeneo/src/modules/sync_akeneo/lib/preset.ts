...
import { findOneWithDecryption, findWithDecryption } from '@helios/shared/lib/encryption/find'
import { CatalogPriceKind } from '@helios/core/modules/catalog/data/entities'
import { SyncMapping } from '@helios/core/modules/data_sync/data/entities'
...
export function readAkeneoEnvPreset(env: NodeJS.ProcessEnv = process.env): AkeneoEnvPreset | null {
  const credentialKeys = {
    apiUrl: ['HELIOS_INTEGRATION_AKENEO_API_URL', 'HELIOS_AKENEO_API_URL', 'AKENEO_API_URL'],
    clientId: ['HELIOS_INTEGRATION_AKENEO_CLIENT_ID', 'HELIOS_AKENEO_CLIENT_ID', 'AKENEO_CLIENT_ID'],
    clientSecret: ['HELIOS_INTEGRATION_AKENEO_CLIENT_SECRET', 'HELIOS_AKENEO_CLIENT_SECRET', 'AKENEO_CLIENT_SECRET'],
    username: ['HELIOS_INTEGRATION_AKENEO_USERNAME', 'HELIOS_AKENEO_USERNAME', 'AKENEO_USERNAME'],
    password: ['HELIOS_INTEGRATION_AKENEO_PASSWORD', 'HELIOS_AKENEO_PASSWORD', 'AKENEO_PASSWORD'],
  } as const
  ...
  if (!apiUrl || !clientId || !clientSecret || !username || !password) {
    throw new Error(
      '[sync_akeneo] Incomplete Akeneo env preset. Set HELIOS_INTEGRATION_AKENEO_API_URL, HELIOS_INTEGRATION_AKENEO_CLIENT_ID, HELIOS_INTEGRATION_AKENEO_CLIENT_SECRET, HELIOS_INTEGRATION_AKENEO_USERNAME, and HELIOS_INTEGRATION_AKENEO_PASSWORD.',
    )
  }
  ...
  return {
    ...
    force: readBooleanEnv(env, ['HELIOS_INTEGRATION_AKENEO_FORCE_PRECONFIGURE', 'HELIOS_AKENEO_FORCE_PRECONFIGURE', 'AKENEO_FORCE_PRECONFIGURE']) ?? false,
    productLocale: readEnvValue(env, ['HELIOS_INTEGRATION_AKENEO_PRODUCT_LOCALE', 'HELIOS_AKENEO_PRODUCT_LOCALE', 'AKENEO_PRODUCT_LOCALE']),
    categoryLocale: readEnvValue(env, ['HELIOS_INTEGRATION_AKENEO_CATEGORY_LOCALE', 'HELIOS_AKENEO_CATEGORY_LOCALE', 'AKENEO_CATEGORY_LOCALE']),
    productChannel: readEnvValue(env, ['HELIOS_INTEGRATION_AKENEO_PRODUCT_CHANNEL', 'HELIOS_AKENEO_PRODUCT_CHANNEL', 'AKENEO_PRODUCT_CHANNEL']) ?? undefined,
    productChannels: readCsvEnv(env, ['HELIOS_INTEGRATION_AKENEO_IMPORT_CHANNELS', 'HELIOS_AKENEO_IMPORT_CHANNELS', 'AKENEO_IMPORT_CHANNELS']),
    importAllChannels: readBooleanEnv(env, ['HELIOS_INTEGRATION_AKENEO_IMPORT_ALL_CHANNELS', 'HELIOS_AKENEO_IMPORT_ALL_CHANNELS', 'AKENEO_IMPORT_ALL_CHANNELS']),
    createMissingChannels: readBooleanEnv(env, ['HELIOS_INTEGRATION_AKENEO_CREATE_MISSING_CHANNELS', 'HELIOS_AKENEO_CREATE_MISSING_CHANNELS', 'AKENEO_CREATE_MISSING_CHANNELS']),
    syncAssociations: readBooleanEnv(env, ['HELIOS_INTEGRATION_AKENEO_SYNC_ASSOCIATIONS', 'HELIOS_AKENEO_SYNC_ASSOCIATIONS', 'AKENEO_SYNC_ASSOCIATIONS']),
    familyCodeFilter: readCsvEnv(env, ['HELIOS_INTEGRATION_AKENEO_ATTRIBUTE_FAMILY_FILTER', 'HELIOS_AKENEO_ATTRIBUTE_FAMILY_FILTER', 'AKENEO_ATTRIBUTE_FAMILY_FILTER']),
    productsSettingsOverride: readJsonEnv(
      env,
      ['HELIOS_INTEGRATION_AKENEO_PRODUCTS_SETTINGS_JSON', 'HELIOS_AKENEO_PRODUCTS_SETTINGS_JSON', 'AKENEO_PRODUCTS_SETTINGS_JSON'],
      'HELIOS_INTEGRATION_AKENEO_PRODUCTS_SETTINGS_JSON',
    ),
    categoriesSettingsOverride: readJsonEnv(
      env,
      ['HELIOS_INTEGRATION_AKENEO_CATEGORIES_SETTINGS_JSON', 'HELIOS_AKENEO_CATEGORIES_SETTINGS_JSON', 'AKENEO_CATEGORIES_SETTINGS_JSON'],
      'HELIOS_INTEGRATION_AKENEO_CATEGORIES_SETTINGS_JSON',
    ),
    attributesSettingsOverride: readJsonEnv(
      env,
      ['HELIOS_INTEGRATION_AKENEO_ATTRIBUTES_SETTINGS_JSON', 'HELIOS_AKENEO_ATTRIBUTES_SETTINGS_JSON', 'AKENEO_ATTRIBUTES_SETTINGS_JSON'],

...
import {
  ...
  resolveTimeoutMs,
} from '@helios/shared/lib/http/fetchWithTimeout'
import {
  safeOutboundFetch,
  type HostLookup,
...
const ALLOWED_AKENEO_HOSTS_ENV_KEYS = [
  ...
  'HELIOS_AKENEO_ALLOWED_HOSTS',
  'AKENEO_ALLOWED_HOSTS',
] as const
const ALLOW_PRIVATE_AKENEO_URLS_ENV_KEYS = [
  'HELIOS_INTEGRATION_AKENEO_ALLOW_PRIVATE_URLS',
  'HELIOS_AKENEO_ALLOW_PRIVATE_URLS',
...
function resolveAkeneoRequestTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  return resolvePositiveIntEnv(env.HELIOS_INTEGRATION_AKENEO_REQUEST_TIMEOUT_MS, DEFAULT_AKENEO_REQUEST_TIMEOUT_MS)
}

function resolveAkeneoMaxRateLimitRetries(env: NodeJS.ProcessEnv = process.env): number {
  return resolvePositiveIntEnv(env.HELIOS_INTEGRATION_AKENEO_MAX_RATE_LIMIT_RETRIES, DEFAULT_AKENEO_MAX_RATE_LIMIT_RETRIES)
}
...
function clampAkeneoRetryAfterMs(retryAfterHeader: string | null, env: NodeJS.ProcessEnv = process.env): number {
  ...
  const cap = resolvePositiveIntEnv(env.HELIOS_INTEGRATION_AKENEO_RETRY_AFTER_CAP_MS, DEFAULT_AKENEO_RETRY_AFTER_CAP_MS)

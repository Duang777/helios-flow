...
import { NextStepCallout } from '@helios/ui/backend/NextStepCallout'
...
const PRODUCT_FIELD_KEYS: Array<{
  ...
  { key: 'title', label: 'Title attribute', help: 'Used for the Helios product title.' },
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="rounded-lg border p-4">
            ...
            <ol className="mt-3 list-decimal space-y-2 pl-5 text-sm text-muted-foreground">
              <li>{t('sync_akeneo.setup.credentials.step1', 'In Akeneo, open Settings -> Connections and create a new API connection or connected app for Helios.')}</li>
              <li>{t('sync_akeneo.setup.credentials.step2', 'Copy the Akeneo base URL, client id, and client secret into the Integration credentials tab in Helios.')}</li>
          ...
          <div className="rounded-lg border p-4">
            ...
            <ul className="mt-3 list-disc space-y-2 pl-5 text-sm text-muted-foreground">
              <li>{t('sync_akeneo.setup.docs.note1', 'Akeneo authentication uses client id, client secret, username, and password. Helios exchanges those for access/refresh tokens automatically.')}</li>
              ...
              <li>{t('sync_akeneo.setup.docs.note3', 'Simple Akeneo products still create a default Helios variant. Variant Akeneo products flatten the full Akeneo product-model tree into one configurable product plus child variants.')}</li>
              <li>{t('sync_akeneo.setup.docs.note4', 'If you want channel offers and prices, create matching Sales Channels and Catalog Price Kinds in Helios first, then reference their codes in the mapping blocks below.')}</li>
            </ul>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
          <div className="space-y-5 rounded-lg border p-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {t('sync_akeneo.mapping.productLocale.help', 'Only this Akeneo locale is imported into the base Helios fields for now. Other locales are ignored until translation import is added.')}
                </p>
              </div>
              <div className="space-y-2">
            ...
            <div className="grid gap-4">
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {t('sync_akeneo.mapping.customFields.help', 'One mapping per line: attribute_code,target(product|variant),field_key[,kind][,skip]. After valid Akeneo credentials are saved, Helios discovers mappings automatically: variant axes stay as product options, and other relevant Akeneo attributes are auto-mapped as custom fields unless you override them here. Use the editor skip checkbox when an Akeneo attribute should be ignored entirely. Product imports create or update missing Helios custom field definitions automatically and store the Akeneo metadata, validation rules, and groups on those fields.')}
                </p>
                  ...
                  <table className="w-full min-w-[900px] text-sm">
                    <thead className="bg-muted/50 text-left">
                      <tr>
                        ...
                        <th className="px-3 py-2 font-medium">{t('sync_akeneo.mapping.localChannel', 'Helios channel')}</th>
                        ...
                        <tr key={`price-mapping-${index}`} className="border-t">
                          ...
                          <td className="px-3 py-2">
                            <select
                              ...
                              disabled={isLoading || isSaving}
                            >
                              <option value="">{t('sync_akeneo.mapping.localChannelPlaceholder', 'Select Helios channel')}</option>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {t('sync_akeneo.mapping.media.help', 'One mapping per line: attribute_code,target(product|variant),kind(image|file). Image mappings are re-hosted into Helios attachments and can become default media; file mappings are imported as attachments.')}
                </p>
                <div className="overflow-x-auto rounded-lg border">
                  <table className="w-full min-w-[760px] text-sm">
                    <thead className="bg-muted/50 text-left">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {t('sync_akeneo.mapping.fieldsets.help', 'Discovered Akeneo families become Helios fieldsets automatically. Adjust the fieldset code, label, or description here, and add manual family-variant rows when a specific Akeneo family variant should use a different variant fieldset.')}
                </p>
                <div className="overflow-x-auto rounded-lg border">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  ...
                  disabled={isLoading || isSaving}
                />
                <span>{t('sync_akeneo.mapping.createMissingChannels', 'Create missing Helios sales channels from Akeneo scopes by default')}</span>
              </label>
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
          ...
          <div className="space-y-4 rounded-lg border p-4">
            ...
            <div className="space-y-3">
              ...
              <div>
                <div className="mb-2 text-xs font-medium uppercase text-muted-foreground">{t('sync_akeneo.discovery.localChannels', 'Helios channels')}</div>
                <div className="flex flex-wrap gap-2">

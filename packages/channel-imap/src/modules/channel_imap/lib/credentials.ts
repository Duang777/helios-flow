 ...
 * genuinely private IMAP host set `HELIOS_CHANNEL_IMAP_ALLOW_INTERNAL_HOSTS=true`.
 */
...
function assertSafeHost(host: string, ctx: { addIssue: (issue: { code: 'custom'; message: string }) => void }): void {
  if (parseBooleanWithDefault(process.env.HELIOS_CHANNEL_IMAP_ALLOW_INTERNAL_HOSTS, false)) return
  if (!host.trim()) return
  if (isInternalHost(host)) {
    ctx.addIssue({
      code: 'custom',
      message:
        'Host appears to point at a private or loopback address. If this is intentional, an operator must set HELIOS_CHANNEL_IMAP_ALLOW_INTERNAL_HOSTS=true.',
    })
  }
}

import type {
  ...
  VerifyWebhookInput,
} from '@helios/core/modules/communication_channels/lib/adapter'
...
import { emailResolveContact } from '@helios/core/modules/communication_channels/lib/email-contact'
import { decodeCursor, encodeCursor } from '@helios/core/modules/communication_channels/lib/email-mime'
 ...
 *
 * Configurable via `HELIOS_CHANNEL_IMAP_HARD_CAP_PER_POLL` (default 200).
 */
function clampHardCap(callerLimit: number | undefined): number {
  const envOverride = Number.parseInt(process.env.HELIOS_CHANNEL_IMAP_HARD_CAP_PER_POLL ?? '', 10)

...
describe('ImapChannelAdapter.fetchHistory', () => {
  ...
  it('signals hasMore=true when more UIDs remain than HARD_CAP', async () => {
    ...
    const originalCap = process.env.HELIOS_CHANNEL_IMAP_HARD_CAP_PER_POLL
    process.env.HELIOS_CHANNEL_IMAP_HARD_CAP_PER_POLL = '2'
    try {
      const fetchCalls: string[] = []
      const imap: ImapClient = {
        connectAndValidate: async () => ({ capabilities: [] }),
      ...
      if (originalCap === undefined) delete process.env.HELIOS_CHANNEL_IMAP_HARD_CAP_PER_POLL
      else process.env.HELIOS_CHANNEL_IMAP_HARD_CAP_PER_POLL = originalCap
    }
  })
})

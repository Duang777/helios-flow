...
describe('credentialsToConnection — socket timeout override', () => {
  ...
  afterEach(() => {
    delete process.env.HELIOS_CHANNEL_IMAP_SOCKET_TIMEOUT_MS
  })
  ...
  it('reads a positive HELIOS_CHANNEL_IMAP_SOCKET_TIMEOUT_MS override', () => {
    process.env.HELIOS_CHANNEL_IMAP_SOCKET_TIMEOUT_MS = '90000'
    expect(credentialsToConnection(baseCredentials).timeoutMs).toBe(90000)
  })

  it('ignores a non-numeric or non-positive override', () => {
    process.env.HELIOS_CHANNEL_IMAP_SOCKET_TIMEOUT_MS = 'abc'
    expect(credentialsToConnection(baseCredentials).timeoutMs).toBeUndefined()
    process.env.HELIOS_CHANNEL_IMAP_SOCKET_TIMEOUT_MS = '0'
    expect(credentialsToConnection(baseCredentials).timeoutMs).toBeUndefined()
  })
})

...
describe('imapCredentialsSchema', () => {
  ...
  it('honors the HELIOS_CHANNEL_IMAP_ALLOW_INTERNAL_HOSTS escape hatch', () => {
    const previous = process.env.HELIOS_CHANNEL_IMAP_ALLOW_INTERNAL_HOSTS
    process.env.HELIOS_CHANNEL_IMAP_ALLOW_INTERNAL_HOSTS = 'true'
    try {
      expect(() =>
        imapCredentialsSchema.parse({ ...valid, imapHost: '127.0.0.1', smtpHost: '127.0.0.1' }),
      ).not.toThrow()
    } finally {
      if (previous === undefined) delete process.env.HELIOS_CHANNEL_IMAP_ALLOW_INTERNAL_HOSTS
      else process.env.HELIOS_CHANNEL_IMAP_ALLOW_INTERNAL_HOSTS = previous
    }
  })
})

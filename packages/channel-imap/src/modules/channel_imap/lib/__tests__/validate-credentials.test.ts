...
afterEach(() => {
  setImapClient(null)
  setSmtpClient(null)
  delete process.env.HELIOS_CHANNEL_IMAP_ALLOW_INSECURE_TRANSPORT
})
...
describe('validateImapCredentials', () => {
  ...
  it("allows 'none' transport when HELIOS_CHANNEL_IMAP_ALLOW_INSECURE_TRANSPORT is truthy", async () => {
    process.env.HELIOS_CHANNEL_IMAP_ALLOW_INSECURE_TRANSPORT = 'true'
    setImapClient({
      connectAndValidate: async () => ({ capabilities: ['IMAP4rev1'] }),
      selectInbox: async () => ({}),
      fetchUidRange: async () => [],

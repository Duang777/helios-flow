...
const INTERNAL_RESOLVED_MESSAGE =
  'Host resolves to a private or loopback address. If this is intentional, an operator must set HELIOS_CHANNEL_IMAP_ALLOW_INTERNAL_HOSTS=true.'
 ...
 * - Honors `HELIOS_CHANNEL_IMAP_ALLOW_INTERNAL_HOSTS`: when set, resolution is
...
export async function resolveSafeHostAddress(
  ...
  if (parseBooleanWithDefault(process.env.HELIOS_CHANNEL_IMAP_ALLOW_INTERNAL_HOSTS, false)) {
    return { host: trimmed }
  }

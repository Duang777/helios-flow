 ...
 * `HELIOS_CHANNEL_IMAP_ALLOW_INSECURE_TRANSPORT=true`. `'starttls'`/`'tls'` are
 * always allowed.
 */

export const INSECURE_TRANSPORT_MESSAGE =
  'Cleartext transport (None) is not allowed. Use STARTTLS or implicit TLS. ' +
  'An operator must set HELIOS_CHANNEL_IMAP_ALLOW_INSECURE_TRANSPORT=true to permit it.'

export function isInsecureTransportAllowed(): boolean {
  return parseBooleanWithDefault(process.env.HELIOS_CHANNEL_IMAP_ALLOW_INSECURE_TRANSPORT, false)
}

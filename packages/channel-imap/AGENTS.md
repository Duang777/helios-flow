# `@helios/channel-imap` — Agent Guidelines
...
- **Package**: `@helios/channel-imap` ⇒ **module id**: `channel_imap`
...
`lib/adapter.ts` implements `ChannelAdapter` from `@helios/core/modules/communication_channels/lib/adapter`. Key methods: `sendMessage`, `normalizeInbound`, `convertOutbound`, `validateCredentials`, `fetchHistory`, `importHistory`, `resolveContact`.
...
- **Reject cleartext transport.** `imapTls`/`smtpTls: 'none'` is rejected by `validateImapCredentials` unless `HELIOS_CHANNEL_IMAP_ALLOW_INSECURE_TRANSPORT` is truthy. STARTTLS/implicit TLS always allowed.
- **SSRF guard.** Host strings are attacker-controlled; `credentials.ts` rejects private/loopback hosts unless `HELIOS_CHANNEL_IMAP_ALLOW_INTERNAL_HOSTS` is truthy.
...
| Var | Default | Purpose |
|-----|---------|---------|
| `HELIOS_CHANNEL_IMAP_ALLOW_INSECURE_TRANSPORT` | `false` | Permit `'none'` (cleartext) TLS mode in credential validation |
| `HELIOS_CHANNEL_IMAP_ALLOW_INTERNAL_HOSTS` | `false` | Permit private/loopback IMAP/SMTP hosts (bypasses SSRF guard) |
| `HELIOS_CHANNEL_IMAP_HARD_CAP_PER_POLL` | `200` | Max UIDs fetched per `fetchHistory` poll |

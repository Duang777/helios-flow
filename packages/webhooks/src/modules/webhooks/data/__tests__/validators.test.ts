...
describe('webhookCreateSchema — URL safety', () => {
  const originalAllowPrivateUrls = process.env.HELIOS_WEBHOOKS_ALLOW_PRIVATE_URLS

  afterEach(() => {
    if (originalAllowPrivateUrls === undefined) {
      delete process.env.HELIOS_WEBHOOKS_ALLOW_PRIVATE_URLS
    } else {
      process.env.HELIOS_WEBHOOKS_ALLOW_PRIVATE_URLS = originalAllowPrivateUrls
    }
  })
  ...
  it('accepts private create and update URLs when the development override is enabled', () => {
    process.env.HELIOS_WEBHOOKS_ALLOW_PRIVATE_URLS = '1'

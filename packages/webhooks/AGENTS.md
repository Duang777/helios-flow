...
Use `@helios/webhooks` for Standard Webhooks delivery, inbound verification, and webhook marketplace/admin flows.

## Always

1. **MUST use the shared webhook primitives** — import signing, verification, and secret helpers from `@helios/shared/lib/webhooks`
...
```bash
yarn generate
yarn workspace @helios/webhooks test
yarn workspace @helios/webhooks build
```
...
2. Reuse `CrudForm`, `DataTable`, `FormHeader`, `RowActions`, and shared notices from `@helios/ui`
...
5. Run `yarn workspace @helios/webhooks test`
6. Run `yarn workspace @helios/webhooks build`

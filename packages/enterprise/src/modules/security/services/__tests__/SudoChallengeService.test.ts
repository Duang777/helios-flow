...
function getSudoTestSecret(): string {
  return process.env.HELIOS_SECURITY_SUDO_SECRET
...
describe('SudoChallengeService', () => {
  const ORIGINAL_SUDO_SECRET = process.env.HELIOS_SECURITY_SUDO_SECRET
  ...
  beforeEach(() => {
    jest.clearAllMocks()
    process.env.HELIOS_SECURITY_SUDO_SECRET = 'unit-test-sudo-secret'
  ...
  describe('sudo secret resolution', () => {
    afterAll(() => {
      if (ORIGINAL_SUDO_SECRET === undefined) delete process.env.HELIOS_SECURITY_SUDO_SECRET
      else process.env.HELIOS_SECURITY_SUDO_SECRET = ORIGINAL_SUDO_SECRET
    ...
    test('verify throws a descriptive error when no sudo secret env var is set', async () => {
      delete process.env.HELIOS_SECURITY_SUDO_SECRET
      delete process.env.AUTH_JWT_SECRET
      delete process.env.AUTH_SECRET
      delete process.env.JWT_SECRET
      ...
      ).rejects.toThrow(/HELIOS_SECURITY_SUDO_SECRET, AUTH_JWT_SECRET, AUTH_SECRET, or JWT_SECRET/)
    })
    ...
    test('empty-string env vars are treated as unset and throw', async () => {
      process.env.HELIOS_SECURITY_SUDO_SECRET = ''
      process.env.AUTH_JWT_SECRET = '   '
    ...
    test('HELIOS_SECURITY_SUDO_SECRET wins over the other fallbacks', async () => {
      process.env.HELIOS_SECURITY_SUDO_SECRET = 'primary-secret'
      ...
      process.env.HELIOS_SECURITY_SUDO_SECRET = 'rotated-primary-secret'
      await expect(
    ...
    test('AUTH_JWT_SECRET wins over AUTH_SECRET and JWT_SECRET when HELIOS_SECURITY_SUDO_SECRET is unset', async () => {
      delete process.env.HELIOS_SECURITY_SUDO_SECRET
      process.env.AUTH_JWT_SECRET = 'auth-jwt-secret'
      process.env.AUTH_SECRET = 'auth-secret'
      process.env.JWT_SECRET = 'legacy-secret'
    ...
    test('AUTH_SECRET wins over JWT_SECRET when neither HELIOS_SECURITY_SUDO_SECRET nor AUTH_JWT_SECRET is set', async () => {
      delete process.env.HELIOS_SECURITY_SUDO_SECRET
      delete process.env.AUTH_JWT_SECRET
      process.env.AUTH_SECRET = 'auth-secret'
      process.env.JWT_SECRET = 'legacy-secret'

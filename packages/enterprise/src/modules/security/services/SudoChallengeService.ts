...
export class SudoChallengeService {
  ...
  private getSudoSecret(): string {
    const secret =
      readTrimmedEnv(process.env.HELIOS_SECURITY_SUDO_SECRET)
    ...
    throw new Error(
      'Sudo step-up tokens require HELIOS_SECURITY_SUDO_SECRET, AUTH_JWT_SECRET, AUTH_SECRET, or JWT_SECRET to be set.',
    )
  }

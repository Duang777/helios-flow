...
export const defaultSecurityModuleConfig: SecurityModuleConfig = {
  ...
  otpEmail: {
    ...
    subject: 'Your Helios verification code',
  },
  mfa: {
    challengeTtlMs: 600 * 1000,
    maxAttempts: 5,
...
export function readSecurityModuleConfig(
  ...
  const otpExpirySeconds = parsePositiveInteger(env.HELIOS_SECURITY_OTP_EXPIRY_SECONDS)
    ?? defaultSecurityModuleConfig.otpEmail.expirySeconds
  const otpMaxAttempts = parsePositiveInteger(env.HELIOS_SECURITY_OTP_MAX_ATTEMPTS)
    ?? defaultSecurityModuleConfig.otpEmail.maxAttempts
  const sudoMaxTtlSeconds = Math.max(
    parsePositiveInteger(env.HELIOS_SECURITY_SUDO_MAX_TTL) ?? defaultSecurityModuleConfig.sudo.maxTtlSeconds,
    defaultSecurityModuleConfig.sudo.minTtlSeconds,
  )
  const sudoDefaultTtlSeconds = clamp(
    parsePositiveInteger(env.HELIOS_SECURITY_SUDO_DEFAULT_TTL) ?? defaultSecurityModuleConfig.sudo.defaultTtlSeconds,
    defaultSecurityModuleConfig.sudo.minTtlSeconds,
    sudoMaxTtlSeconds,
  )
  const webauthnRpId = readText(env.HELIOS_SECURITY_WEBAUTHN_RP_ID)
  ...
  return {
    totp: {
      ...
      issuer: readText(env.HELIOS_SECURITY_TOTP_ISSUER) ?? defaultSecurityModuleConfig.totp.issuer,
      window: parseNonNegativeInteger(env.HELIOS_SECURITY_TOTP_WINDOW) ?? defaultSecurityModuleConfig.totp.window,
    },
    otpEmail: {
    ...
    mfa: {
      ...
      emergencyBypass: parseBooleanWithDefault(
        env.HELIOS_SECURITY_MFA_EMERGENCY_BYPASS,
    ...
    webauthn: {
      ...
      rpName: readText(env.HELIOS_SECURITY_WEBAUTHN_RP_NAME) ?? defaultSecurityModuleConfig.webauthn.rpName,
      rpId: webauthnRpId,
      expectedOrigins: readWebAuthnOrigins(env, webauthnRpId),
    },
    recoveryCodes: {
      ...defaultSecurityModuleConfig.recoveryCodes,
      count: parsePositiveInteger(env.HELIOS_SECURITY_RECOVERY_CODE_COUNT)
        ?? parsePositiveInteger(env.HELIOS_SECURITY_RECOVERY_CODES_COUNT)
...
export function resolveSecurityModuleConfigForRequest(
  ...
  const explicitRpId = readText(env.HELIOS_SECURITY_WEBAUTHN_RP_ID)
...
export function readSecuritySetupTokenSecret(env: NodeJS.ProcessEnv = process.env): string {
  const secret = readText(env.HELIOS_SECURITY_MFA_SETUP_SECRET)
  ...
  throw new Error(
    'Security MFA setup tokens require HELIOS_SECURITY_MFA_SETUP_SECRET, AUTH_JWT_SECRET, AUTH_SECRET, or JWT_SECRET.',
  )
}
...
function readWebAuthnOrigins(
  ...
  const fromList = (env.HELIOS_SECURITY_WEBAUTHN_ORIGINS ?? '')
    .split(',')
    .map((value) => value.trim())
    .filter((value) => value.length > 0)
  const explicitOrigin = readText(env.HELIOS_SECURITY_WEBAUTHN_ORIGIN)

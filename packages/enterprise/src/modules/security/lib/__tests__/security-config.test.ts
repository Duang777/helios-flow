...
describe('security-config', () => {
  test('reads security env overrides and derives WebAuthn defaults from APP_URL', () => {
    const config = readSecurityModuleConfig({
      APP_URL: 'https://helios.example.com/app',
      HELIOS_SECURITY_TOTP_ISSUER: 'Acme Helios',
      HELIOS_SECURITY_TOTP_WINDOW: '2',
      HELIOS_SECURITY_OTP_EXPIRY_SECONDS: '120',
      HELIOS_SECURITY_OTP_MAX_ATTEMPTS: '7',
      HELIOS_SECURITY_SUDO_DEFAULT_TTL: '900',
      HELIOS_SECURITY_SUDO_MAX_TTL: '1200',
      HELIOS_SECURITY_RECOVERY_CODE_COUNT: '12',
      HELIOS_SECURITY_MFA_EMERGENCY_BYPASS: 'true',
    })

    expect(config.totp.issuer).toBe('Acme Helios')
    ...
    expect(config.webauthn.rpId).toBe('helios.example.com')
    expect(config.webauthn.expectedOrigins).toEqual(['https://helios.example.com/app'])
  })

  test('supports the legacy recovery code env alias and clamps sudo default ttl to max', () => {
    const config = readSecurityModuleConfig({
      HELIOS_SECURITY_RECOVERY_CODES_COUNT: '4',
      HELIOS_SECURITY_SUDO_DEFAULT_TTL: '2400',
      HELIOS_SECURITY_SUDO_MAX_TTL: '600',
    })
  ...
  test('uses the incoming request host for WebAuthn when no explicit RP ID override is configured', () => {
    const resolved = resolveSecurityModuleConfigForRequest(
      defaultSecurityModuleConfig,
      new Request('https://preview-ephemeralenvom-preview-0kyxui-wmfj8i.helios.com/api/security/mfa/prepare'),
      {},
    )

    expect(resolved.webauthn.rpId).toBe('preview-ephemeralenvom-preview-0kyxui-wmfj8i.helios.com')
    expect(resolved.webauthn.expectedOrigins).toEqual([
      'https://preview-ephemeralenvom-preview-0kyxui-wmfj8i.helios.com',
    ])
  })
  ...
  test('keeps explicit WebAuthn RP ID and origin overrides ahead of request host', () => {
    const resolved = resolveSecurityModuleConfigForRequest(
      ...
      new Request('https://preview-ephemeralenvom-preview-0kyxui-wmfj8i.helios.com/api/security/mfa/prepare'),
      {
        HELIOS_SECURITY_WEBAUTHN_RP_ID: 'login.example.com',
        HELIOS_SECURITY_WEBAUTHN_ORIGIN: 'https://login.example.com',
      },
    )
  ...
  test('falls back to defaults for invalid values', () => {
    const config = readSecurityModuleConfig({
      ...
      HELIOS_SECURITY_OTP_EXPIRY_SECONDS: 'nope',
      HELIOS_SECURITY_OTP_MAX_ATTEMPTS: '0',
      HELIOS_SECURITY_MFA_EMERGENCY_BYPASS: 'maybe',
    })
  ...
  test('reads the MFA setup token secret from the dedicated env or JWT fallbacks', () => {
    expect(readSecuritySetupTokenSecret({
      HELIOS_SECURITY_MFA_SETUP_SECRET: 'security-secret',

...
} from '@helios/shared/lib/crud/optimistic-lock'
import { OPTIMISTIC_LOCK_ENV_VAR } from '@helios/shared/lib/crud/optimistic-lock-headers'
...
export type CreateRecordLockCrudMutationGuardServiceOptions = {
  /** Override `HELIOS_OPTIMISTIC_LOCK` (mostly for tests). */
 ...
 *   1. `HELIOS_OPTIMISTIC_LOCK=off` → pure pass-through (single global kill switch;

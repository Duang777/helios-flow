...
import { apiRequest } from '@helios/core/modules/core/__integration__/helpers/api';
import { deleteEntityIfExists } from '@helios/core/modules/core/__integration__/helpers/crmFixtures';
import { getTokenContext } from '@helios/core/modules/core/__integration__/helpers/generalFixtures';
...
export async function updateCompany(
  ...
  if (lockHeaders) {
    requestHeaders['x-helios-record-lock-kind'] = 'customers.company';
    requestHeaders['x-helios-record-lock-resource-id'] = companyId;

    if (lockHeaders.token) {
      requestHeaders['x-helios-record-lock-token'] = lockHeaders.token;
    }

    if (lockHeaders.baseLogId) {
      requestHeaders['x-helios-record-lock-base-log-id'] = lockHeaders.baseLogId;
    }

    if (lockHeaders.resolution) {
      requestHeaders['x-helios-record-lock-resolution'] = lockHeaders.resolution;
    }

    if (lockHeaders.conflictId) {
      requestHeaders['x-helios-record-lock-conflict-id'] = lockHeaders.conflictId;
    }
  }
...
function applyRecordLockHeaders(
  ...
  requestHeaders['x-helios-record-lock-kind'] = resourceKind;
  requestHeaders['x-helios-record-lock-resource-id'] = resourceId;
  if (lockHeaders.token) requestHeaders['x-helios-record-lock-token'] = lockHeaders.token;
  if (lockHeaders.baseLogId) requestHeaders['x-helios-record-lock-base-log-id'] = lockHeaders.baseLogId;
  if (lockHeaders.resolution) requestHeaders['x-helios-record-lock-resolution'] = lockHeaders.resolution;
  if (lockHeaders.conflictId) requestHeaders['x-helios-record-lock-conflict-id'] = lockHeaders.conflictId;
}
...
export const OPTIMISTIC_LOCK_HEADER = 'x-helios-ext-optimistic-lock-expected-updated-at';

...
describe('record lock widget resolution headers', () => {
  ...
  test('sends resolution header once and disarms it immediately', async () => {
    ...
    expect(first.requestHeaders?.['x-helios-record-lock-resolution']).toBe('accept_mine')
  ...
  test('allows save with an acquired lock when no conflict or deletion is pending', async () => {
    ...
    expect(result.requestHeaders).toMatchObject({
      'x-helios-record-lock-kind': 'customers.person',
      'x-helios-record-lock-resource-id': 'b0000000-0000-4000-8000-000000000001',
      'x-helios-record-lock-token': 'lock-token',
      'x-helios-record-lock-base-log-id': 'log-1',
    })

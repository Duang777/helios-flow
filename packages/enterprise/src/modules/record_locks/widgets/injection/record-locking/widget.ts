...
import { getRecordLockFormState, setRecordLockFormState } from '@helios/enterprise/modules/record_locks/lib/clientLockStore'
  ...
  eventHandlers: {
    async onBeforeSave(data, context) {
      ...
      if (!context.formId) {
        ...
        if (validation.lock?.resourceKind && validation.lock?.resourceId) {
          return {
            ok: true,
            requestHeaders: {
              'x-helios-record-lock-kind': validation.lock.resourceKind,
              'x-helios-record-lock-resource-id': validation.lock.resourceId,
              ...(validation.lock?.token ? { 'x-helios-record-lock-token': validation.lock.token } : {}),
              ...(validation.latestActionLogId ? { 'x-helios-record-lock-base-log-id': validation.latestActionLogId } : {}),
            },
          }
        }
        return { ok: true }
      ...
      return {
        ok: true,
        requestHeaders: {
          'x-helios-record-lock-kind': state.resourceKind,
          'x-helios-record-lock-resource-id': state.resourceId,
          ...(state.lock?.token ? { 'x-helios-record-lock-token': state.lock.token } : {}),
          ...(state.latestActionLogId ? { 'x-helios-record-lock-base-log-id': state.latestActionLogId } : {}),
          ...(shouldSendResolution
            ? { 'x-helios-record-lock-resolution': state.pendingResolution }
            : {}),
          ...(conflictIdHeader ? { 'x-helios-record-lock-conflict-id': conflictIdHeader } : {}),
        },
      }
    },

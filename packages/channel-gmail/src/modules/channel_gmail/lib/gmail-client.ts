...
import { fetchWithTimeout, FetchTimeoutError } from '@helios/shared/lib/http/fetchWithTimeout'
...
function resolveGmailRequestTimeoutMs(): number {
  const fromEnv = Number.parseInt(process.env.HELIOS_CHANNEL_GMAIL_REQUEST_TIMEOUT_MS ?? '', 10)

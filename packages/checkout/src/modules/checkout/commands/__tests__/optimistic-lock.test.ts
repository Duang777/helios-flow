...
import type { CommandRuntimeContext } from '@helios/shared/lib/commands/types'
...
jest.mock('@helios/shared/lib/commands/helpers', () => ({
  setCustomFieldsIfAny: jest.fn(async () => undefined),
}))

jest.mock('@helios/shared/lib/commands/customFieldSnapshots', () => ({
...
describe('checkout command optimistic locking', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    delete process.env.HELIOS_OPTIMISTIC_LOCK
  })
  ...
  describe('async command seam (record_locks Phase 6b)', () => {
    ...
    it('HELIOS_OPTIMISTIC_LOCK=off disables the guard — stale version is not blocked', async () => {
      process.env.HELIOS_OPTIMISTIC_LOCK = 'off'
      await expect(

...
Helios `0.5.0` is our biggest release so far. It bundles more than 250 fixes and
...
This document lists backward-incompatible changes that users of the Helios platform
...
[`.ai/skills/helios-auto-upgrade-<from>-<to>/SKILL.md`](.ai/skills/) and can mechanically migrate
...
`isUniqueViolation` now lives at `@helios/shared/lib/db/pg-errors`. The previous `@helios/core/modules/communication_channels/lib/pg-errors` import remains available as a deprecated re-export for this release; downstream modules should move to the shared path.
...
1. **The default OSS optimistic-lock guard was silently disabled.** The request container is built in Awilix CLASSIC injection mode, and the guard's factory destructured a renamed parameter (`({ em: scopedEm })`), which CLASSIC cannot resolve. The resolution error was swallowed, so every `makeCrudRoute` PUT/DELETE ignored the `x-helios-ext-optimistic-lock-expected-updated-at` header and stale writes returned `200` instead of `409`. *Action for downstream:* none — upgrading `@helios/shared` restores the guard. A failed guard resolution now logs a warning (once per process) instead of failing silently.
...
### Shared `helios-*` pipeline skills now come from helios/skills

The generalized agent-pipeline skills (`helios-code-review`, `helios-auto-create-pr`, `helios-auto-review-pr`, `helios-merge-buddy`, `helios-spec-writing`, the `-loop` variants, `helios-prepare-issue`, and 15 more — see the `external` block in [`.ai/skills/tiers.json`](.ai/skills/tiers.json)) were removed from `.ai/skills/` and are now installed from the shared [helios/skills](https://github.com/helios/skills) collection. `yarn install-skills` runs `npx -y skills add helios/skills --skill '*'` after the local tier symlinks, placing the skills under `.agents/skills/` (gitignored), then `npx -y skills update --project` so re-running the installer refreshes the external skills to their latest published versions (the lockfile is gitignored, so `add` seeds and `update` keeps them current).

Contributor action:

- Re-run `yarn install-skills` (network required for the npx step; pass `--no-external` or set `HELIOS_SKIP_EXTERNAL_SKILLS=1` when offline — local tier skills still install).
...
- The local `helios-auto-fix-github` skill has been removed and replaced by the external `helios-auto-fix-issue` (installed under `.agents/skills/` from the shared helios/skills collection). Update any `/helios-auto-fix-github` callers to `/helios-auto-fix-issue`.
...
} from '@helios/shared/lib/browser/versionedPreference'
...
To opt in, pick a scope with the new `HELIOS_WATCH_SCOPE` env var or the `--watch=<mode>` flag (CLI flag wins over the env var):
...
- `popular` — watch only the most frequently changed packages from recent `git log` history (`HELIOS_WATCH_POPULAR_LIMIT`, default 6; falls back to `core`, `ui`, `shared`).
- `env` — watch exactly the packages in `HELIOS_WATCH_PACKAGES`, or the selection saved by the interactive picker (`yarn dev:watch-select`, persisted to the gitignored `.helios/watch-packages.local.json`).
...
Additional knobs: `HELIOS_WATCH_GIT_STATUS`, `HELIOS_WATCH_GIT_BRANCH`, `HELIOS_WATCH_BASE_REF`, `HELIOS_WATCH_POPULAR_LIMIT`. This is purely a local dev-DX feature: no API, event-ID, DI, ACL, or DB-schema contract changed, and the app source is still fully watched by Next.js/Turbopack regardless of scope. Standalone create-app projects do not run the workspace-package watcher in normal use. See [the troubleshooting guide](apps/docs/docs/appendix/troubleshooting.mdx) for the full reference.
...
The module fact-sheet generator no longer gates on a hard-coded 9-module allowlist. It now **auto-discovers** every source-available package module: the `create-app` build (and `helios agentic:init`) bundle a fact-sheet for every package-provided module (`discoverPackageModuleSources`), shipped to scaffolded apps as `.ai/guides/module-facts.json` + per-module sheets. The monorepo no longer emits a committed `apps/helios/src/module-facts.generated.json` — that artifact had no runtime or test consumer and has been removed along with its generator (`generateModuleFacts`) and the unused registry-driven `discoverEnabledModuleSources` path.

- **Removed (#3754):** `MODULE_FACTS_ALLOWLIST` and `ModuleFactsModuleId` (previously exported from `@helios/cli/lib/generators/module-facts`) are **gone**. Their only remaining runtime consumer was the legacy `core.<module>.md` redirect-stub loop, retired in the same change. Because the whole fact-sheet auto-discovery layer is still `Unreleased` (it never shipped in a tagged release), the exports are removed outright with no deprecation window.
...
### New `helios-prepare-issue` skill (deferred-work capture)

A new bundled skill, [`helios-prepare-issue`](.ai/skills/helios-prepare-issue/SKILL.md), codifies the "park this idea for later" workflow. Given a free-form feature brief it (1) researches and writes a spec under `.ai/specs/` to `helios-spec-writing` standards, (2) opens a **docs-only spec PR** against `develop` (labels `documentation` + `skip-qa`, reusing `helios-auto-create-pr` worktree/branch/label mechanics), and (3) opens a **tracking GitHub issue** that links the spec path and the spec PR and names the implementer skill (`helios-implement-spec` / `helios-auto-fix-issue`) for later pickup. It never implements the feature — the only file it adds is the spec.

The skill is registered in the `automation` tier of [`.ai/skills/tiers.json`](.ai/skills/tiers.json) (alongside `helios-auto-create-pr` and `helios-auto-fix-issue`) and is also shipped into standalone apps scaffolded by `create-helios-app` (`packages/create-app/agentic/shared/ai/skills/helios-prepare-issue/`).

This is purely additive — no existing skill, slash command, API, DB, or module-contract surface changed.

### `helios-auto-review-pr` now posts manual-QA instructions on the `needs-qa → qa` transition

[`helios-auto-review-pr`](.ai/skills/helios-auto-review-pr/SKILL.md) (and `helios-review-prs`, which delegates to it) now posts an **additional PR comment with concrete step-by-step manual QA instructions** whenever it routes an approved PR to the `qa` pipeline state (i.e. `needs-qa` present, `skip-qa` absent). The comment uses the house QA route format from `helios-auto-qa-scenarios` — P0/P1/P2 priority tags with **Where to click** / **What to verify** / **What can go wrong** blocks derived from the actual diff.
...
# helios-prepare-issue and helios-auto-review-pr both live in the opt-in `automation` tier:
yarn install-skills --with automation      # default tiers + automation
# or install every tier:
yarn install-skills --all
```

The installer is idempotent and tier-driven (`.ai/skills/tiers.json`) — it adds the new symlink and sweeps stale ones; it never edits skill content. Standalone apps generated by `create-helios-app` receive `helios-prepare-issue` automatically the next time agentic setup runs (`yarn helios agentic:init`).
...
[`#1981`](https://github.com/helios/helios/pull/2055) is now
...
`x-helios-ext-optimistic-lock-expected-updated-at` header continue to pass
...
- `HELIOS_OPTIMISTIC_LOCK=off` (case-insensitive; also `false` / `0` /
  `no` / `disabled` / `none`) now disables the guard explicitly.
  Allow-list values (`HELIOS_OPTIMISTIC_LOCK=customers.company,sales.order`)
...
1. Your deployment has not set `HELIOS_OPTIMISTIC_LOCK` explicitly.
...
### AI coding skills renamed with the `helios-` prefix

Every bundled AI coding skill is now namespaced with an `helios-` prefix, both under the repo's `.ai/skills/` directory and in the standalone-app scaffolding generated by `create-helios-app` (`packages/create-app/agentic/shared/ai/skills/`). This avoids collisions with skills a downstream team adds to their own project and matches the `@helios/*` package naming convention.

The rename is purely mechanical — **prepend `helios-` to the skill folder name and its `name:` frontmatter**. Skill content and triggers are unchanged. Affected skills:

```
auto-continue-pr            → helios-auto-continue-pr
auto-continue-pr-loop       → helios-auto-continue-pr-loop
auto-create-pr              → helios-auto-create-pr
auto-create-pr-loop         → helios-auto-create-pr-loop
auto-fix-github             → helios-auto-fix-github
auto-qa-scenarios           → helios-auto-qa-scenarios
auto-review-pr              → helios-auto-review-pr
auto-sec-report             → helios-auto-sec-report
auto-sec-report-pr          → helios-auto-sec-report-pr
auto-update-changelog       → helios-auto-update-changelog
auto-upgrade-0.4.10-to-0.5.0 → helios-auto-upgrade-0.4.10-to-0.5.0
backend-ui-design           → helios-backend-ui-design
check-and-commit            → helios-check-and-commit
code-review                 → helios-code-review
create-agents-md            → helios-create-agents-md
create-ai-agent             → helios-create-ai-agent
dev-container-maintenance   → helios-dev-container-maintenance
ds-guardian                 → helios-ds-guardian
fix                         → helios-fix
fix-specs                   → helios-fix-specs
implement-spec              → helios-implement-spec
integration-builder         → helios-integration-builder
integration-tests           → helios-integration-tests
merge-buddy                 → helios-merge-buddy
migrate-mikro-orm           → helios-migrate-mikro-orm
open-pr                     → helios-open-pr
pre-implement-spec          → helios-pre-implement-spec
review-prs                  → helios-review-prs
root-cause                  → helios-root-cause
skill-creator               → helios-skill-creator
smart-test                  → helios-smart-test
spec-writing                → helios-spec-writing
sync-merged-pr-issues       → helios-sync-merged-pr-issues
verify-in-repo              → helios-verify-in-repo
```

The create-app scaffolding also ships these standalone-only skills under the same prefix: `helios-data-model-design`, `helios-eject-and-customize`, `helios-module-scaffold`, `helios-system-extension`, `helios-trim-unused-modules`, `helios-troubleshooter`.

What you need to do:

- **Slash-command invocations** change accordingly, e.g. `/auto-create-pr` → `/helios-auto-create-pr`, `claude "/module-scaffold"` → `claude "/helios-module-scaffold"`.
- **Scripts, docs, or AGENTS.md files** that reference a skill by name or by `.ai/skills/<name>/SKILL.md` path must adopt the `helios-` prefix. A one-shot rewrite over your own tree:

  ```bash
  # Update .ai/skills/<name> path references to the helios- prefix (review the diff before committing)
...
- **Custom skills you authored** are unaffected — only the bundled Helios skills moved.
...
([#1513](https://github.com/helios/helios/pull/1513)), the last of the three
...
[`.ai/skills/helios-migrate-mikro-orm/SKILL.md`](.ai/skills/helios-migrate-mikro-orm/SKILL.md); the
highlights are:
...
[#1620](https://github.com/helios/helios/pull/1620) (minor/patch) and
[#1621](https://github.com/helios/helios/pull/1621) (major), migrated to
`develop` in [#1625](https://github.com/helios/helios/pull/1625).
...
Companion skill: [`helios-auto-upgrade-0.4.10-to-0.5.0`](.ai/skills/helios-auto-upgrade-0.4.10-to-0.5.0/SKILL.md).
...
Companion skill: [`helios-auto-upgrade-X.Y.Z-to-X.Y.(Z+1)`](.ai/skills/helios-auto-upgrade-X.Y.Z-to-X.Y.(Z+1)/SKILL.md).

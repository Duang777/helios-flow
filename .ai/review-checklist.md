...
- [ ] Code placed in correct location (core features in `packages/`, app-specific in `apps/helios/src/modules/`)
- [ ] No code added directly in `apps/helios/src/` outside of `modules/`
- [ ] `@helios/shared` has zero domain dependencies — no imports from `@helios/core`
...
- [ ] `extractUndoPayload()` used from `@helios/shared/lib/commands/undo.ts`
...
- [ ] `splitCustomFieldPayload`, `normalizeCustomFieldValues`, `normalizeCustomFieldResponse` from `@helios/shared`
- [ ] DSL helpers used: `defineLink`, `entityId`, `cf.*` from `@helios/shared/modules/dsl`
...
- [ ] `FormHeader` and `FormFooter` from `@helios/ui/backend/forms`
...
- [ ] `LoadingMessage` and `ErrorMessage` from `@helios/ui/backend/detail`
...
- [ ] Client-side: `useT()` from `@helios/shared/lib/i18n/context`
- [ ] Server-side: `resolveTranslations()` from `@helios/shared/lib/i18n/server`
...
- [ ] Files in `apps/helios/.helios/generated/` never edited manually
...
- [ ] Template parity check passes: `yarn template:sync` (for `apps/helios/src/{app,modules}` vs `packages/create-app/template/src/{app,modules}`)

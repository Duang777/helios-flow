...
|---|---|---|
| Button with text label (with or without icon) | `Button` | `@helios/ui/primitives/button` |
| Icon-only button | `IconButton` | `@helios/ui/primitives/icon-button` |
| Inline link styled as button | `LinkButton` | `@helios/ui/primitives/link-button` |
| OAuth/sign-in button (brand-styled) | `SocialButton` | `@helios/ui/primitives/social-button` |
| Marketing CTA with brand gradient | `FancyButton` | `@helios/ui/primitives/fancy-button` |
| Checkbox primitive (with indeterminate) | `Checkbox` | `@helios/ui/primitives/checkbox` |
| Checkbox with label + description | `CheckboxField` | `@helios/ui/primitives/checkbox-field` |
| Text input (generic, text/number/url/etc.) | `Input` | `@helios/ui/primitives/input` |
| Email input (Figma Email variant — mail icon prefix) | `EmailInput` | `@helios/ui/primitives/email-input` |
| Search input (Figma Search variant — leading magnifier + trailing × clear) | `SearchInput` | `@helios/ui/primitives/search-input` |
| Password input (Figma Password variant — trailing eye/eye-off reveal toggle) | `PasswordInput` | `@helios/ui/primitives/password-input` |
| Phone input (Figma Phone variant — country picker + national number) | `PhoneNumberField` | `@helios/ui/backend/inputs/PhoneNumberField` |
| Website / URL input (Figma Website variant — `https://` prefix box) | `WebsiteInput` | `@helios/ui/primitives/website-input` |
| Amount input with currency picker (Figma Amount variant) | `AmountInput` | `@helios/ui/primitives/amount-input` |
| Input with trailing icon-button slot (Figma Button variant — copy URL, send, etc.) | `ButtonInput` | `@helios/ui/primitives/button-input` |
| Card-number input with brand auto-detection (Figma Card variant — Visa/MC/Amex/...) | `CardInput` | `@helios/ui/primitives/card-input` |
| Multi-line text input (with optional char counter) | `Textarea` | `@helios/ui/primitives/textarea` |
| Dropdown / select | `Select` (with `SelectTrigger` / `SelectTriggerLeading` / `SelectContent` / `SelectItem` / `SelectItemLeading`) | `@helios/ui/primitives/select` |
| Tooltip on hover (with arrow, dark/light) | `SimpleTooltip` (or `Tooltip`+`TooltipTrigger`+`TooltipContent`) | `@helios/ui/primitives/tooltip` |
| Toggle switch (binary on/off preference) | `Switch` | `@helios/ui/primitives/switch` |
| Switch with label + description (preference row) | `SwitchField` | `@helios/ui/primitives/switch-field` |
| Radio button (single primitive) | `Radio` (inside `RadioGroup`) | `@helios/ui/primitives/radio` |
| Radio with label + description (form row) | `RadioField` | `@helios/ui/primitives/radio-field` |
| User / entity avatar | `Avatar`, `AvatarStack` | `@helios/ui/primitives/avatar` |
| Keyboard shortcut keys | `Kbd`, `KbdShortcut` | `@helios/ui/primitives/kbd` |
| Entity tag pill | `Tag` (with `TagMap`) | `@helios/ui/primitives/tag` |
| Breadcrumb navigation (DS-aligned, slash/arrow/dot divider, ARIA correct) | `Breadcrumb` (with `BreadcrumbList` / `BreadcrumbItem` / `BreadcrumbLink` / `BreadcrumbPage` / `BreadcrumbStatic` / `BreadcrumbSeparator` / `BreadcrumbEllipsis`) | `@helios/ui/primitives/breadcrumb` |
| Scrollable container with DS-styled scrollbars | `ScrollArea` (compound: `ScrollAreaRoot` / `Viewport` / `Scrollbar` / `Thumb` / `Corner`) | `@helios/ui/primitives/scroll-area` |
| Joined buttons sharing outer border (Save / Save & New / overflow) | `ButtonGroup` (NOT for selection — use `SegmentedControl`) | `@helios/ui/primitives/button-group` |
| Mutually-exclusive view toggle (All / Active / Archived, 1D / 1W / 1M) | `SegmentedControl` (with `SegmentedControlItem`) | `@helios/ui/primitives/segmented-control` |
| Continuous numeric selector — single or two-thumb range | `Slider` (`value` MUST be `[number]` or `[number, number]`) | `@helios/ui/primitives/slider` |
| 1-N star / heart / dot rating (read-only display OR interactive input) | `Rating` (no `onChange` = read-only; `onChange` = interactive) | `@helios/ui/primitives/rating` |
| Multi-step progress (wizard / onboarding / checkout) | `StepIndicator` (steps[] with `status` per step) | `@helios/ui/primitives/step-indicator` |
| Color selection (tag color, brand color, category) | `ColorPicker` (swatch popover + optional hex input) | `@helios/ui/primitives/color-picker` |
| Page navigation for lists outside DataTable | `Pagination` (1-indexed, `total` not `totalPages`) | `@helios/ui/primitives/pagination` |
| Side sheet / non-blocking overlay (detail pane, secondary form) | `Drawer` (with `DrawerContent` / `DrawerHeader` / `DrawerBody` / `DrawerFooter` / `DrawerClose`) | `@helios/ui/primitives/drawer` |
| Cmd+K spotlight palette (global navigation / quick actions / universal search) | `CommandMenu` (compound: `CommandMenuContent` / `Input` / `List` / `Group` / `Item` / `Separator` / `Footer`) — auto-filter via `cmdk` | `@helios/ui/primitives/command-menu` |
| Chronological actor-action timeline (detail panes, audit feeds, customer activity logs) | `ActivityFeed` (compound: `ActivityFeedItem` / `ActivityFeedFileChip` / `ActivityFeedComment` / `ActivityFeedStatusChip`) | `@helios/ui/primitives/activity-feed` |
| Bell-icon inbox / notification panel (the dropdown that opens from the app shell bell affordance) | `NotificationFeed` (compound: `NotificationFeedHeader` / `NotificationFeedList` / `NotificationFeedItem` / `NotificationFeedFooter` / `NotificationFeedIconBadge`) | `@helios/ui/primitives/notification-feed` |
| Determinate linear progress bar (job percentage, file upload, onboarding) | `Progress` (`size`: `sm`/`default`/`lg`, `tone`: `accent`/`success`/`warning`/`destructive`/`muted`, optional `label`/`showValue`/`description` slots) | `@helios/ui/primitives/progress` |
| Determinate circular progress (compact KPI dial, attachment upload thumbnail, sprint completion %) | `CircularProgress` (same `tone`s; `size`: `xs` / `sm` / `default` / `lg`; optional center `showValue` or custom `children`) | `@helios/ui/primitives/progress` |
...
- If a backend page cannot use `CrudForm`, use `useGuardedMutation` from `@helios/ui/backend/injection/useGuardedMutation` for every write (`POST`/`PUT`/`PATCH`/`DELETE`).
...
- Use the `EventSelect` component from `@helios/ui/backend/inputs/EventSelect` for event selection. It fetches declared events via the `/api/events` endpoint.
- Never use `window.confirm` — use the shared `ConfirmDialog` and `useConfirmDialog` from `@helios/ui/backend/confirm-dialog` for confirmation flows.
...
- For form/detail page headers and footers, use `FormHeader` and `FormFooter` from `@helios/ui/backend/forms`. `FormHeader` supports two modes: `edit` (compact, used automatically by CrudForm) and `detail` (large title with entity type label, status badge, Actions dropdown). Delete/Cancel/Save are always standalone buttons; additional context actions (Convert, Send, etc.) go into the `menuActions` array rendered as an "Actions" dropdown. See [SPEC-016](.ai/specs/implemented/SPEC-016-2026-02-03-form-headers-footers.md) for full API.
...
```typescript
import { Tag } from '@helios/ui/primitives/tag'
import type { TagMap } from '@helios/ui/primitives/tag'
```
...
`DataTable` is exported from the package root (`@helios/ui`) and is the recommended list view component for **customer portal pages**, not just backoffice surfaces. Build portal list views with `DataTable` directly rather than hand-rolling `<table>` markup or wrapping a custom component, so portal lists stay consistent with the rest of the product (sorting, pagination, empty states, accessibility, row-click semantics).
...
- `data` — the rows to render (load via `apiCall` from `@helios/ui/backend/utils/apiCall`)
...
import { DataTable } from '@helios/ui'
...
- When filtering menu items by `item.features` or route `requireFeatures`, MUST use the shared wildcard-aware matcher from `@helios/shared/lib/auth/featureMatch` — `Set.has(...)`/`includes(...)` miss `module.*` grants.

## Loading, Empty, and Error States

- For list/detail data loading, use `LoadingMessage` and `ErrorMessage` from `@helios/ui/backend/detail`.
- For record-backed backend detail/edit pages, treat `notFound` as a dedicated page state, separate from generic `error`.
- When a record is missing, return early with `RecordNotFoundState` from `@helios/ui/backend/detail` (a neutral, centered empty state with a recovery action) and pass `backHref`/`backLabel`; do not render `CrudForm`, detail sections, tabs, or record actions. A missing record is NOT an error — never render not-found through the destructive `ErrorMessage`. Reserve `ErrorMessage` for genuine load/validation failures.
...
- Use `flash(message, 'success' | 'error')` from `@helios/ui/backend/FlashMessages` for user feedback after CRUD operations.
...
- Prefer existing UI primitives and backend components from `@helios/ui` before creating new ones.
...
> **Portal list views**: use `DataTable` from `@helios/ui` for any portal page that displays a list of records (orders, addresses, invoices, etc.). Do not hand-roll `<table>` markup. See **DataTable Guidelines → Using DataTable in Portal Pages** above for the portal-safe prop subset and a minimal example.

### Portal Hooks (`packages/ui/src/portal/hooks/`)

| Hook | Import | Purpose |
|------|--------|---------|
| `useCustomerAuth` | `@helios/ui/portal/hooks/useCustomerAuth` | Customer auth state (user, roles, features, logout) |
| `useTenantContext` | `@helios/ui/portal/hooks/useTenantContext` | Resolve tenant/org from URL slug |
| `usePortalInjectedMenuItems` | `@helios/ui/portal/hooks/usePortalInjectedMenuItems` | Load feature-gated menu items for portal surfaces |
| `usePortalEventBridge` | `@helios/ui/portal/hooks/usePortalEventBridge` | SSE connection for portal real-time events |
| `usePortalAppEvent` | `@helios/ui/portal/hooks/usePortalAppEvent` | Listen for portal events by pattern |
...
```tsx
import { PortalShell } from '@helios/ui/portal/PortalShell'
import { useCustomerAuth } from '@helios/ui/portal/hooks/useCustomerAuth'
...
import { usePortalAppEvent } from '@helios/ui/portal/hooks/usePortalAppEvent'

...
- Never add code directly under `apps/helios/src/` except committed, typed `*.generated.ts` registries described below.
...
|------|-------|
| **Module Development** | |
| New module, scaffolding, auto-discovery paths | `core` + [`.ai/docs/module-development.md`](.ai/docs/module-development.md). **Standalone apps**: the `helios-module-scaffold` skill scaffolds a module end-to-end |
...
| New integration provider (adapter, health check, credentials, bundle wiring) | `.ai/skills/helios-integration-builder/SKILL.md` + `core:integrations` + `core:data_sync` |
| **Packages** | |
...
| Structured logging / replacing raw `console.*` with the facade (`createLogger`, `child()`, `HELIOS_LOG_LEVEL`), advisory `yarn logger:check-console` | `apps/docs/docs/framework/runtime/logging.mdx` + `.ai/specs/2026-07-02-structured-logging-facade.md` + `shared` |
| Forms (`CrudForm`), data tables (`DataTable`), loading/error states, flash messages, `FormHeader`/`FormFooter`, dialog UX | `ui` + `helios-backend-ui-design` skill (+ `helios-ds-guardian` for DS-token compliance) |
...
| Backend page components, `apiCall` usage, `RowActions` ids, `LoadingMessage`/`ErrorMessage` | `ui:backend` + `helios-backend-ui-design` skill |
...
| Editing `apps/helios/src/app/**` or env vars in `apps/helios/.env.example` — MUST mirror into the create-app template in the same task | `create-app` → Template Sync Checklist |
| Deploying a scaffolded app to Railway with `helios deploy railway` | [`.ai/specs/2026-05-12-railway-one-command-deploy.md`](.ai/specs/2026-05-12-railway-one-command-deploy.md) + [`apps/docs/docs/deployment/railway.mdx`](apps/docs/docs/deployment/railway.mdx) + `cli` |
| **Performance** | |
...
| **Migration** | |
| Migrating custom module code from MikroORM v6 to v7 (decorators, persist/flush, Knex→Kysely, ORM config, Jest setup) | `.ai/skills/helios-migrate-mikro-orm/SKILL.md` |
| **Testing** | |
| Integration testing, Playwright tests, converting markdown test cases to TypeScript, CI test pipeline | `.ai/qa/AGENTS.md` + `.agents/skills/helios-integration-tests/SKILL.md` |
| **Spec & PR Automation** | |
| Spec lifecycle (pre-implement → implement → write/update), code review, DS review | `.agents/skills/{helios-spec-writing,helios-code-review}/SKILL.md` + `.ai/skills/{helios-pre-implement-spec,helios-implement-spec,helios-ds-guardian}/SKILL.md` + `.ai/specs/AGENTS.md` + `.ai/ds-rules.md` |
| PR/issue automation (one-shot auto-PR, resumable loop variants, review/merge-buddy, post-merge sync, changelog, UI QA). **Default for one-off bug fixes / small features:** `helios-auto-create-pr` | `.agents/skills/{helios-auto-create-pr,helios-auto-continue-pr,helios-auto-create-pr-loop,helios-auto-continue-pr-loop,helios-auto-review-pr,helios-auto-qa-pr,helios-merge-buddy,helios-review-prs,helios-close-fixed-issues,helios-auto-update-changelog,helios-prepare-issue}/SKILL.md` |
...
1.  **Spec-first**: Enter plan mode for non-trivial tasks (3+ steps or architectural decisions). Check `.ai/specs/` and `.ai/specs/enterprise/` before coding; name new spec files `{YYYY-MM-DD}-{kebab-case-title}.md`. Skip for small fixes. Skills: `helios-spec-writing` (research/phasing), `helios-pre-implement-spec` (readiness audit), `helios-implement-spec` (execution).
...
- The `qa` pipeline label means manual QA is **in progress** and is set by QA reviewers only — `helios-auto-*` skills request QA with `needs-qa` and never touch `qa`.
...
Examples worth memorising (used everywhere): `apiCall` from `@helios/ui/backend/utils/apiCall`, `useT` from `@helios/shared/lib/i18n/context`, `resolveTranslations` from `@helios/shared/lib/i18n/server`, `Spinner` from `@helios/ui/primitives/spinner`.

Import strategy:
- Prefer package-level imports (`@helios/<package>/...`) over deep relative imports (`../../../...`) when crossing module boundaries, referencing shared module internals, or importing from deeply nested files.
...
-   Default migration workflow: update ORM entities, run `yarn db:generate`, and review the generated SQL plus `migrations/.snapshot-helios.json`
-   Coding-agent exception: if `yarn db:generate` emits unrelated migrations, delete the unrelated output, keep or write only the intended SQL migration for this entity change, and update the affected module's `.snapshot-helios.json`. Never run `yarn db:migrate` just to make the generator quiet.
...
-   Use `apiCall`/`apiCallOrThrow`/`readApiResultOrThrow` from `@helios/ui/backend/utils/apiCall` — never use raw `fetch`
...
-   For local validation errors: throw `createCrudFormError(message, fieldErrors?)` from `@helios/ui/backend/utils/serverErrors`
-   Read JSON defensively: `readJsonSafe(response, fallback)` — never `.json().catch(() => ...)`
-   Use `LoadingMessage`/`ErrorMessage` from `@helios/ui/backend/detail`
...
- Boolean parsing: use `parseBooleanToken`/`parseBooleanWithDefault` from `@helios/shared/lib/boolean`
...
yarn dev:reset            # Clear .helios/next/dev plus legacy .next caches when Turbopack serves stale chunks

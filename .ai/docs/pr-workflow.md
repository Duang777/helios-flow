...
PR SHOULD carry exactly one priority label; `helios-auto-*` skills are expected to set or infer it
...
one risk label; `helios-auto-*` skills are expected to set or infer it rather than leave it unset.
...
- `qa` (pipeline) means **manual QA is in progress**: a QA reviewer has picked the PR up and is
  actively testing it. It is applied **manually by a QA reviewer**, never by an `helios-auto-*`
...
- `screenshots` records that UI QA visual evidence was attached to the PR (posted by
  `helios-auto-qa-pr`, which first runs `helios-auto-review-pr` when the PR is still unreviewed); it is
...
a **label policy enforced by reviewers and the PR-automation tooling** (`helios-merge-buddy`
classifies it as not-mergeable; `helios-approve-merge-pr` and the auto-review/continue skills refuse
...
attached evidence comment alone does not make a `needs-qa` PR merge-ready, and an `helios-auto-*`

...
This checklist complements the `helios-code-review` security baseline. Use it
during every `helios-auto-sec-report-pr` run. For every section that applies
...
- Internal package paths (`@helios/*`) cannot be shadowed by
  public packages.

---
name: helios-auto-sec-report-pr
...
This skill is the atomic building block used by `helios-auto-sec-report` (the
...
- `--out-fragment <path>` (optional) — when invoked as a sub-unit by
  `helios-auto-sec-report`, write the markdown section fragment to this path
...
- `--deep-scan` (optional) — expand the "apply elsewhere" grep sweeps
  ...
  Slower. Off by default. `helios-auto-sec-report` passes this through when it
...
- `.agents/skills/helios-code-review/SKILL.md` and its checklist at
  `.agents/skills/helios-code-review/references/review-checklist.md` are the
  authoritative source for Helios's security baseline
...
- `.agents/skills/helios-auto-review-pr/SKILL.md` defines the claim/lock/worktree
  pattern used here verbatim (see step 1 below).
- `.ai/skills/helios-spec-writing/references/spec-checklist.md` and
...
- `.ai/skills/helios-pre-implement-spec/SKILL.md` is the authoritative source
...
- `.ai/skills/helios-auto-sec-report-pr/references/deep-attack-vectors.md` is
...
protocol from `.agents/skills/helios-auto-review-pr/SKILL.md` step 0 verbatim
...
`.agents/skills/helios-code-review/references/review-checklist.md` to the unit.
...
Load `.ai/skills/helios-auto-sec-report-pr/references/deep-attack-vectors.md`
...
reviewer (or `helios-auto-sec-report`) can execute to drill further.
...
the same HTML rules documented in `helios-auto-sec-report`:

- Inline `<style>` only; no JS; no remote assets.
- Mirror every section. Preserve every PR/issue/CVE link as `<a>` with
  `rel="noopener noreferrer"`.

After the artifacts exist, follow `.agents/skills/helios-auto-create-pr/SKILL.md`
...
driver (`helios-auto-sec-report`) owns those. Fragments are concatenated by
...
Apply `.agents/skills/helios-code-review/SKILL.md` to the diff of the artifact
...
`.agents/skills/helios-auto-review-pr/SKILL.md` against the docs PR in autofix
...
the completion comment (follow `helios-auto-review-pr` step 11 verbatim).
...
- In sub-unit mode: write the partial fragment to `--out-fragment`
  ...
  the driver (`helios-auto-sec-report`) can catch.
...
- Never paste raw diffs, secrets, tokens, `.env` content, credentials,
  ...
  `{REDACTED}`.
- Reuse `helios-code-review`, `helios-auto-review-pr`, `helios-spec-writing`, and
  `helios-pre-implement-spec` patterns and checklists. Do not duplicate

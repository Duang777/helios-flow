---
name: helios-auto-sec-report
description: "Driver that loops `helios-auto-sec-report-pr` across a window (date, PR-number floor, branch, spec, or default last 7 days of merged PRs) and aggregates findings into one docs-only PR against `develop`. Writes markdown + HTML under `.ai/analysis/` with a top-level \"Next steps — go deeper\" list."
---
...
to `helios-auto-sec-report-pr` (the single-unit skill) and combines the
...
into specific areas with additional `helios-auto-sec-report-pr` runs.
...
- **Delegates** to `.ai/skills/helios-auto-sec-report-pr/SKILL.md` for every
  ...
  only orchestrates.
- **Reuses** `.agents/skills/helios-auto-create-pr/SKILL.md` for
  branch/worktree/commit/validation/label discipline when opening the
  aggregate PR.
- **Hands off** to `.agents/skills/helios-auto-continue-pr/SKILL.md` when the run
...
Follow `.agents/skills/helios-auto-create-pr/SKILL.md` step 0 verbatim.
...
Follow `.agents/skills/helios-auto-create-pr/SKILL.md` step 3 with a Progress
...
checkbox title MUST include the exact `helios-auto-sec-report-pr` command
the driver will run. Append commit SHA on each flip so
`helios-auto-continue-pr` can resume precisely.
...
Follow `.agents/skills/helios-auto-create-pr/SKILL.md` steps 4–5 verbatim.

### 4. Execute the queue

For each unit in Phase 2, invoke `helios-auto-sec-report-pr` in sub-unit
...
for UNIT in "${QUEUE[@]}"; do
  ...
  invoke_skill "helios-auto-sec-report-pr" \
    --target "$UNIT" \
    --base "${BASE:-develop}" \
    ${DEEP_SCAN:+--deep-scan} \
...
- Honor the in-progress lock protocol owned by `helios-auto-sec-report-pr`
...
- Push after every 5 units so `helios-auto-continue-pr` always has a
...
rules from `.ai/skills/helios-auto-sec-report-pr/SKILL.md` step 6a:
...
Same as `helios-auto-sec-report-pr` step 7:
...
Apply `.agents/skills/helios-code-review/SKILL.md` to the artifact diff. Verify
no PII, no internal hostnames, no secrets leaked through.

### 9. Open the PR

Follow `.agents/skills/helios-auto-create-pr/SKILL.md` step 9 with:
...
Run `.agents/skills/helios-auto-review-pr/SKILL.md` against the new PR in
autofix mode. Apply fixes as new commits. Never rewrite history.

### 12. Summary comment

Post the comprehensive summary comment required by
`.agents/skills/helios-auto-create-pr/SKILL.md` step 12. In the "What can go
...
Follow `.agents/skills/helios-auto-create-pr/SKILL.md` step 13.
...
2. Ensure the Phase-2 Progress checklist reflects which units
   ...
   exact `helios-auto-sec-report-pr` command to resume.
3. Post a PR comment that says verbatim:
   `🤖 auto-sec-report is not complete. Resume with /auto-continue-pr {prNumber}.`
4. Release any `in-progress` lock per `helios-auto-continue-pr` rules.

## Rules

- This skill delegates per-unit analysis to `helios-auto-sec-report-pr`.
...
- On partial completion of the driver itself, leave
  `Status: in-progress` and post a `/helios-auto-continue-pr {prNumber}`
  hand-off comment.

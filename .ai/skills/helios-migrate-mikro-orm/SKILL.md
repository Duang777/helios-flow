...
Migrate custom module code to MikroORM v7. The platform core is already migrated — this skill targets user modules in `apps/helios/src/modules/` and external integration packages under `packages/`.
...
grep -r "from '@mikro-orm/core'" apps/helios/src/modules/<your-module>/

# Find deprecated persistAndFlush / removeAndFlush calls
grep -rn 'persistAndFlush\|removeAndFlush' apps/helios/src/modules/<your-module>/

# Find Knex usage (must migrate to Kysely)
grep -rn 'getKnex\|getConnection()\.getKnex\|\.raw(' apps/helios/src/modules/<your-module>/
```
...
find apps/helios/src/modules/<your-module> -name 'entities.ts' -exec \
...
4. If `yarn db:generate` keeps producing unrelated SQL because another module's `.snapshot-helios.json` is stale, fix that snapshot separately or leave it out of the PR unless it is the bug you are addressing.
5. When you keep or manually write a scoped migration, update the same module's `migrations/.snapshot-helios.json` to the post-change schema. This file is the generator's baseline; missing snapshot updates cause duplicate migrations in standalone apps.
...
- Update `.snapshot-helios.json` in the same commit.

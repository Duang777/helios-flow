---
name: helios-create-ai-agent
description: Build, override, or extend a typed Helios AI agent (chat or structured-object) using the unified AI framework — declare `ai-agents.ts`, register tool packs via `defineAiTool`, patch existing agents with `aiAgentExtensions`, configure agentic loop controls (`loop.stopWhen` / `loop.prepareStep` / `loop.budget` / `executionEngine`), gate mutations through the approval contract, wire ACL features, and embed `<AiChat>` into a backoffice or portal page. Works in both the monorepo (`packages/<x>/src/modules/<module>/`) and standalone projects (`apps/<app>/src/modules/<module>/` or `node_modules/@helios/<package>` consumers). Triggers on "create AI agent", "add AI agent", "build AI assistant", "extend AI agent", "override AI agent", "add tool to existing agent", "wire ai-agents.ts", "add ai-tools.ts", "embed AiChat", "agent for module", "agentic loop", "configure loop budget", "tool-loop-agent".
---

# Create AI Agent

Build a typed AI agent for an Helios module using the unified AI framework (spec [`2026-04-11-unified-ai-tooling-and-subagents`](../../.ai/specs/implemented/2026-04-11-unified-ai-tooling-and-subagents.md), framework docs `apps/docs/docs/framework/ai-assistant/`).
...
2. Identify the **target environment**:
   ...
   - **Standalone app**: module lives under `apps/<app>/src/modules/<module>/`. Imports come from `@helios/ai-assistant` resolved via `node_modules/`. The generator scans both source and `node_modules/@helios/*/dist/modules/`.
...
const accountAssistant: AiAgentDefinition = {
  ...
  moduleId: '<module>',                      // MUST match the module folder (also drives HELIOS_AI_<MODULE>_MODEL env)
...
```ts
import { defineAiTool, prepareMutation } from '@helios/ai-assistant'
...
- `apps/<app>/.helios/generated/ai-agents.generated.ts` — agent registry
- `apps/<app>/.helios/generated/ai-tools.generated.ts` — tool registry
...
In a standalone app (consuming `@helios/<package>` from `node_modules`), the generator scans both your app's source modules **and** the compiled `dist/modules/<module>/ai-agents.js` files inside published packages. If you publish a package that ships an agent, ensure:
...
Five kinds ship out of the box: `product`, `deal`, `person`, `company`, `activity`. Have the model emit a fenced Markdown block whose info string is `helios:<kind>` and whose body is one JSON object — the chat composer replaces the fence with a typed React component automatically (no registration needed).
...
```tsx
import { AiChat } from '@helios/ui/ai'
...
```ts
import { runAiAgentObject } from '@helios/ai-assistant'
...
HELIOS_AI_INBOX_OPS_MODEL=gpt-4o
HELIOS_AI_CATALOG_MODEL=claude-haiku-4-5
```

Resolution order (highest precedence first):

1. `callerOverride` (`runAiAgentText({ modelOverride })`)
2. `HELIOS_AI_<MODULE>_MODEL` env variable
3. `agentDefaultModel` (`AiAgentDefinition.defaultModel`)
4. The configured provider's default

All callers MUST go through `createModelFactory(container)` from `@helios/ai-assistant/modules/ai_assistant/lib/model-factory`. Never inline `createAnthropic` / `createOpenAI` / `createGoogleGenerativeAI`.
...
- [ ] `yarn generate` completed without warnings; the new agent appears in `apps/<app>/.helios/generated/ai-agents.generated.ts`.
- [ ] `yarn helios configs cache structural --all-tenants` ran (so existing tenants see the new ACL features).
...
`apps/helios/src/bootstrap.ts` (and the `create-helios-app` template) already calls `applyModuleOverridesFromEnabledModules(enabledModules)` from `@helios/shared/modules/overrides` to wire these up. Other domains (routes, events, workers, widgets, …) reuse the same `entry.overrides` umbrella per spec `.ai/specs/implemented/2026-05-04-modules-ts-unified-overrides.md` — AI is Phase 1; subsequent domains roll out as focused PRs.
...
- MUST run `yarn helios configs cache structural --all-tenants` after disabling an agent so existing tenants drop stale caches.

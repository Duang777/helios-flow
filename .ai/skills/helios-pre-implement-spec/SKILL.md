---
name: helios-pre-implement-spec
...
5. Read the code-review checklist at `.agents/skills/helios-code-review/references/review-checklist.md`.
...
**Module structure**:
- Does the spec place code in the correct location? (`packages/core/`, `packages/ui/`, `apps/helios/src/modules/`)
...
- **HTTP via `apiCall` / `apiCallOrThrow`** from `@helios/ui/backend/utils/apiCall`. Non-`CrudForm` writes wrapped in `useGuardedMutation`.

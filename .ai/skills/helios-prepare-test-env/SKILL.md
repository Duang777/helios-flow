---
name: helios-prepare-test-env
description: Repo-local extension of the shared helios-prepare-test-env skill (installed in .agents/skills/). Adds Helios monorepo environment specifics — the generated entrypoint scripts, ephemeral runner commands, reuse TTL semantics, owner-lock teardown, and the env-block contract — on top of the shared skill's workflow. Local rules win on repo specifics only; this file never relaxes the shared skill's safety rules.
---

# Prepare Test Environment — Helios repo rules

Repo-local **extension** of `.agents/skills/helios-prepare-test-env/SKILL.md` (contract v2,
...
The `package.json` / helios CLI commands below are the authoritative, cross-platform way to
...
CLI does not fully self-supply: `MOCK_INBOUND_WEBHOOK_SECRET`, `HELIOS_WEBHOOKS_ALLOW_PRIVATE_URLS=1`,
`HELIOS_OPTIMISTIC_LOCK=all`, `SELF_SERVICE_ONBOARDING_ENABLED=true`,
`HELIOS_INTEGRATION_APP_READY_TIMEOUT_SECONDS=180`, plus `HELIOS_ENABLE_ENTERPRISE_MODULES{,_SSO,_SECURITY}=true`.
...
- Boot app-only ephemeral env: `yarn test:integration:ephemeral:start` (= `yarn helios test:ephemeral`).
...
- Full suite with managed env: `yarn test:integration:ephemeral` (= `yarn helios test:integration`).
  It reuses a healthy running ephemeral env from the state file, else provisions one.
- Filtered run: `yarn helios test:integration <substring>` — batches all specs whose path matches
...
2. **Reuse a running ephemeral env (fast iteration):** boot once with
   ...
   `yarn helios test:integration <filter>` against the same env. Best for short
...
`HELIOS_INTEGRATION_TEST`, mock webhook secrets, `ENABLE_CRUD_API_CACHE`, and more. Without it,
DB-fixture helpers silently fall back to `apps/helios/.env`'s `DATABASE_URL` (the developer's dev
...
through `yarn helios test:integration [filter]`.

## Reuse TTL and the owner-lock deadlock

- Reuse eligibility is gated by `HELIOS_INTEGRATION_BUILD_CACHE_TTL_SECONDS` (default 600s) AND
...
- For short diagnose/re-run loops against the SAME env that produced a failure, extend the TTL:
  `HELIOS_INTEGRATION_BUILD_CACHE_TTL_SECONDS=86400 yarn helios test:integration <filter>` — but only
...
- Seeded credentials: `admin@acme.com` / `secret`, `employee@acme.com` / `secret`,
  superadmin from `HELIOS_INIT_SUPERADMIN_EMAIL`/`HELIOS_INIT_SUPERADMIN_PASSWORD` (default

---
name: helios-create-agents-md
description: Create or rewrite AGENTS.md files for Helios packages and modules. Use this skill when adding a new package, creating a new module, or when an existing AGENTS.md needs to be created or refactored. Ensures prescriptive tone, the Always/Ask First/Never/Validation Commands boundary structure, MUST-style rules, checklists, and consistent structure across all agent guidelines.
---
...
```bash
yarn workspace @helios/{package} test
yarn workspace @helios/{package} build
```
...
|---|---|
| "The cache module provides multi-strategy caching" | "Use `@helios/cache` for all caching needs. MUST NOT use raw Redis directly." |
...
yarn generate
yarn typecheck
yarn workspace @helios/core test --testPathPattern=customers
```

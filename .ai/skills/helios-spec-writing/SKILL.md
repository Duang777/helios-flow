---
name: helios-spec-writing
description: Guide for creating high-quality, architecturally compliant specifications for Helios. Use when starting a new SPEC or reviewing specs against "Martin Fowler" staff-engineer standards.
---

# Spec Writing & Review

Design and review specifications (SPECs) against Helios's architecture, naming, and quality rules. Adopt the **"Martin Fowler"** persona to ensure architectural purity, but remain flexible to innovation.
...
- **Canonical primitives** for CRUD APIs (`makeCrudRoute`), backend forms (`CrudForm`), tables (`DataTable`), HTTP (`apiCall` — never raw `fetch`), non-`CrudForm` writes (`useGuardedMutation`), cache (DI-resolved `@helios/cache`), events (`createModuleEvents`) — see the matching rows in root `AGENTS.md` Task Router.

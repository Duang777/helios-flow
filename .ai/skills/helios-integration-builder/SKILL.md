---
name: helios-integration-builder
description: Build integration provider packages for the Helios Integration Marketplace (payment, shipping, data-sync, webhook). Scaffolds the npm package, adapter, credentials, widget injection, webhook processing, health checks, i18n, tests. Triggers on "build integration", "add provider", "integrate with stripe/paypal/dhl".
---

# Integration Builder

Build integration provider packages for the Helios Integration Marketplace (SPEC-045). Every external integration MUST live in its own npm workspace package under `packages/<provider-package>/`.
...
**Package naming**: `@helios/<prefix><provider>` (e.g., `@helios/gateway-stripe`)
...
```typescript
import type { IntegrationDefinition } from '@helios/shared/modules/integrations'
...
```typescript
import type { IntegrationBundle, IntegrationDefinition } from '@helios/shared/modules/integrations'
...
5. Name env vars with the primary pattern `HELIOS_INTEGRATION_<PROVIDER>_*` (for example `HELIOS_INTEGRATION_AKENEO_API_URL`, `HELIOS_INTEGRATION_STRIPE_SECRET_KEY`).
6. Legacy aliases may be accepted for backward compatibility, but docs and examples must show `HELIOS_INTEGRATION_<PROVIDER>_*` as the canonical names.
...
import type { GatewayAdapter, CreateSessionInput, CreateSessionResult, ... } from '@helios/shared/modules/payment_gateways/types'
...
```typescript
import { registerGatewayAdapter, registerWebhookHandler } from '@helios/shared/modules/payment_gateways/types'
...
import type { IntegrationCredentialWebhookHelp } from '@helios/shared/modules/integrations'

export const webhookSetupGuide: IntegrationCredentialWebhookHelp = {
  kind: 'webhook_setup',
  title: '<Provider> Webhook Configuration',
  summary: 'Configure <Provider> to send webhook events to Helios.',
  endpointPath: '/api/<hub>/webhook/<providerKey>',
...
import type { WidgetDefinition } from '@helios/shared/modules/widgets'
...
Add the package to `apps/helios/src/modules.ts`:

```typescript
import '@helios/<prefix><provider>'
```

### 11.2 Add Workspace Dependency

In `apps/helios/package.json`:
```json
"@helios/<prefix><provider>": "workspace:*"
```
...
- [ ] Provider env vars documented with canonical `HELIOS_INTEGRATION_<PROVIDER>_*` names
...
- [ ] Package-level imports (`@helios/<pkg>/...`) for cross-module references
...
- **MUST** use `HELIOS_INTEGRATION_<PROVIDER>_*` as the primary env naming convention for new integration presets

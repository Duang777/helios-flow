...
Volume names are prefixed with the project directory name (e.g., `helios_devcontainer_`).
...
**Symptom**: `yarn helios init` fails with table errors, or `yarn db:migrate` fails with "relation already exists".
**Diagnose**:
- "relation already exists": DB has tables from a different branch. `post-create.sh` detected tables and ran `db:migrate` instead of `helios init`.
...
**Fix**: Move custom overrides to `apps/helios/.env.local` (Next.js native priority, never overwritten).
...
- PostgreSQL: `postgresql://postgres:postgres@localhost:5432/helios`
...
**Diagnose**: Stale or corrupted local Next.js/Turbopack cache artifacts under `apps/helios/.helios/next/dev` and `apps/helios/.next/cache`.

**Fix**: Run the canonical reset, which clears `.helios/next/dev` plus the legacy `.next` Turbopack/webpack caches, then restart the dev server:
...
```bash
yarn dev:reset
rm -rf node_modules/.cache .turbo apps/helios/.turbo apps/helios/.helios/cache
yarn install
yarn build:packages
yarn generate
yarn build:packages

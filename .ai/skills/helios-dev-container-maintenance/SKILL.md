---
name: helios-dev-container-maintenance
description: Maintain the VS Code Dev Container setup for Helios. MUST use for ANY change to `.devcontainer/` files. Also use when the container fails to build/start, services inside misbehave, or "works locally but not in dev container". Triggers on "dev container", "devcontainer", ".devcontainer".
---
...
Use the **`helios-spec-writing` skill** (`/helios-spec-writing`) to update the spec properly. At minimum:
...
apps/helios/.env.example
  ...
  → apps/helios/.env (generated, overwritten on rebuild)
  → apps/helios/.env.local (manual overrides, never touched)
```

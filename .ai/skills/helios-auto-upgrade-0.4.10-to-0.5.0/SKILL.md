---
name: helios-auto-upgrade-0.4.10-to-0.5.0
description: Migrate a downstream Helios user codebase from 0.4.10 to 0.5.0. Executable companion to UPGRADE_NOTES.md — detects which codemod patterns are in use, applies them in place, typechecks, and reports what still needs human review. Triggers on "upgrade helios to 0.5.0", "bump to 0.5.0", or "apply UPGRADE_NOTES migrations".
---

# auto-upgrade-0.4.10-to-0.5.0

Apply the Helios 0.4.10 → 0.5.0 dependency-upgrade codemods to a user's downstream
...
- `0.5.0` is the biggest Helios release so far
...
This skill operates on a **user's** Helios app (the repo produced by `create-app`,
or any app that depends on `@helios/*`). It does NOT modify anything inside
`packages/` — framework-side migrations are the job of the framework authors.

It applies the mechanical parts of the upgrade. It does NOT bump the `@helios/*`
dependencies themselves — that is the user's pinning decision. Run this skill **after**
the user bumps their pin.

## When to use

- User says: "upgrade Helios from 0.4.10 to 0.5.0", "bump helios to 0.5.0",
  "migrate my code for the new helios version", "apply the UPGRADE_NOTES migrations".
- After the user changes their `@helios/*` pins from `0.4.10` to `0.5.0` and runs
  `yarn install`.
...
grep -q '"@helios/' package.json || {
  echo "No @helios/* dependency found — is this the right repo?"; exit 1;
}

# Confirm the user bumped their pins (warn, don't block)
grep -q '"@helios/core": "\^\?0\.4\.11"' package.json || \
  echo "⚠️  @helios/core is not pinned to 0.5.0 — run this skill after bumping."
```
...
- For `lucide-brand-icons`, add a one-line comment above the replaced usage:
  ```tsx
  // TODO(helios 0.5.0): lucide-react v1 removed brand icons; Briefcase is a generic substitute
  ```
...
Next steps:
  ...
  2. Commit the migration as a single `chore(deps): upgrade helios 0.4.10 → 0.5.0` commit.

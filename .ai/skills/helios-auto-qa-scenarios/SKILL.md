---
name: helios-auto-qa-scenarios
description: Generate a human QA report for a window of merged PRs (date floor, PR-number floor, or default last 7 days) and ship it as a docs-only PR against `develop`. Groups work into P0/P1/P2 testing routes with click paths, verification points, and risk callouts. Writes markdown + HTML under `.ai/analysis/`. Hands off to `helios-auto-continue-pr` if it cannot finish in one pass.
---
...
This skill is a specialization of `helios-auto-create-pr`. It adopts that skill's
...
few places where the flow deviates from the generic `helios-auto-create-pr`
workflow.
...
Follow `.agents/skills/helios-auto-create-pr/SKILL.md` step 0 verbatim. Branch name
...
Follow `.agents/skills/helios-auto-create-pr/SKILL.md` step 3. The plan at
...
- A **Progress** section with the exact convention block from
  `helios-auto-create-pr`. Required phases:
...
Follow `.agents/skills/helios-auto-create-pr/SKILL.md` steps 4–5 verbatim. Branch
...
back to the full `helios-auto-create-pr` step 7 gate.

### 8. Self-review and BC review

Apply `.agents/skills/helios-code-review/SKILL.md` to the diff. Because the change is
...
Follow `.agents/skills/helios-auto-create-pr/SKILL.md` step 9 with these specifics:
...
Never add `needs-qa` on an `helios-auto-qa-scenarios` PR. The report is about
other PRs; it does not itself require manual QA.

### 11. Auto-review pass

Run `.agents/skills/helios-auto-review-pr/SKILL.md` against the new PR in autofix
mode. Apply fixes as new commits. Never rewrite history.

### 12. Summary comment

Post the comprehensive summary comment required by
`.agents/skills/helios-auto-create-pr/SKILL.md` step 12. In the **How to verify**
...
4. Release any `in-progress` lock per `helios-auto-continue-pr` rules if one was
...
- Reuse `helios-auto-create-pr` for branch/worktree/commit/validation/label
  discipline. Do not reinvent those mechanics.
- On partial completion, leave a `/helios-auto-continue-pr {prNumber}` hand-off

...
- **Module-scoped**: `packages/*/src/modules/<module>/` or `apps/helios/src/modules/<module>/` → extract `<module>`
...
```bash
SPEC_FILES=$(printf '%s\n' "$CHANGED_FILES" \
  | python3 .ai/skills/helios-smart-test/scripts/find_affected_integration_tests.py \
    --project-root . \
    --base auto \
    --layer "$LAYER")
...
root. For example, `apps/helios/src/modules/example` and

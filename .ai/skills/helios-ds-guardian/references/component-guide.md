...
|---|---|---|
| Show an error/success/warning message inline | `<Alert status="error\|success\|warning\|information\|feature" style="light\|lighter\|stroke\|filled">` | `@helios/ui/primitives/alert` |
| Show a toast notification | `flash('message', 'success\|error\|warning\|info')` | `@helios/ui/backend/FlashMessages` |
| Confirm a destructive action | `useConfirmDialog()` | `@helios/ui/backend/confirm-dialog` |
| Display entity status (active, draft, etc.) | `<StatusBadge variant={statusMap[status]} dot>` | `@helios/ui/primitives/status-badge` |
| Wrap a form field with label + error | `<FormField label="..." error={...}>` | `@helios/ui/primitives/form-field` |
| Build a section header with count + action | `<SectionHeader title="..." count={n} action={...}>` | `@helios/ui/backend/SectionHeader` |
| Build a collapsible section | `<CollapsibleSection title="...">` | `@helios/ui/backend/SectionHeader` |
| Show loading state | `<LoadingMessage />` or `<Spinner />` | `@helios/ui/backend/detail` / `@helios/ui/primitives/spinner` |
| Show error state | `<ErrorMessage message={...} />` | `@helios/ui/backend/detail` |
| Show empty state | `<EmptyState title="..." description="..." action={...} />` | `@helios/ui/backend/EmptyState` |
| Build a data table with sort/filter/pagination | `<DataTable columns={...} data={...} />` | `@helios/ui/backend/DataTable` |
| Build a CRUD form | `<CrudForm fields={...} onSubmit={...} />` | `@helios/ui/backend/CrudForm` |
| Add a metadata badge (count, tag) | `<Badge variant="secondary">` | `@helios/ui/primitives/badge` |
| Use an icon | `<IconName className="size-4" />` from lucide-react | `lucide-react` |
| Render text input (text/email/password/number/tel/url/search) | `<Input leftIcon={...} rightIcon={...} size="sm\|default\|lg" />` | `@helios/ui/primitives/input` |
| Render multi-line text with optional char counter | `<Textarea showCount maxLength={...} />` | `@helios/ui/primitives/textarea` |
| Render dropdown / single-select | `<Select><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>...</SelectContent></Select>` | `@helios/ui/primitives/select` |
| Render checkbox (binary toggle inside form) | `<Checkbox checked={...} onCheckedChange={...} />` | `@helios/ui/primitives/checkbox` |
| Render checkbox with label + description | `<CheckboxField label="..." description="..." />` | `@helios/ui/primitives/checkbox-field` |
| Render binary toggle (immediate effect, e.g. setting on/off) | `<Switch />` or `<SwitchField label="..." />` | `@helios/ui/primitives/switch` / `@helios/ui/primitives/switch-field` |
| Render mutually-exclusive choice | `<RadioGroup><Radio value="..." />...</RadioGroup>` or `<RadioField label="..." />` | `@helios/ui/primitives/radio` / `@helios/ui/primitives/radio-field` |
| Show hover hint with arrow | `<SimpleTooltip content="..." arrow>` | `@helios/ui/primitives/tooltip` |
| Show inline link styled as button | `<LinkButton variant="..." />` | `@helios/ui/primitives/link-button` |
| Show OAuth/social sign-in button | `<SocialButton brand="apple\|github\|google\|x\|facebook\|dropbox\|linkedin" />` | `@helios/ui/primitives/social-button` |
| Show marketing CTA with brand gradient | `<FancyButton intent="..." />` | `@helios/ui/primitives/fancy-button` |
...
```tsx
import { RadioGroup, Radio } from '@helios/ui/primitives/radio'

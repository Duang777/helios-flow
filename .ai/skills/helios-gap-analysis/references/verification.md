...
1. **Fetch the Upstream-pipeline snapshot once, for the whole run — never per-story.** The orchestrator is the sole tracker caller for this channel, exactly once, regardless of story count (see "Why the snapshot is fetched once" below). Build `.ai/tmp/helios-gap-analysis/pipeline-snapshot-<project>.md` from three parts:
...
4. **Gate the returned blocks — one at a time — before writing each.** For each, write the story's title + acceptance criteria to a temp file and pass it with `--story`; pass `<REPO_ROOT>` with `--repo-root`, `<COMPANION_ROOT>` with `--companion-root` whenever it isn't `UNAVAILABLE`, and the tier map (when `platform.tierMap` is configured, materialized as a `<path-prefix> <tier>` lines file) with `--tier-map`:
   ...
   ```bash
   printf '%s\n' "$STORY_TITLE_AND_CRITERIA" > .ai/tmp/helios-gap-analysis/story-<id>.txt
   GATE_ARGS=(--repo-root "$REPO_ROOT" --story .ai/tmp/helios-gap-analysis/story-<id>.txt)
...
7. **Cross-check the Upstream-pipeline citations — once, at the Phase 2→3 transition.** When all stories are `done` or `needs-review`:
   ...
   ```bash
   <skill-dir>/bin/gap-pipeline-crosscheck .ai/gap-analysis/<project>.md .ai/tmp/helios-gap-analysis/pipeline-snapshot-<project>.md
   ```

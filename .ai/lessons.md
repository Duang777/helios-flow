...
**Context**: Core app layout behavior was updated in `apps/helios/src/app/(backend)/backend/layout.tsx`, but equivalent files in `packages/create-app/template/src/app/` were not updated in the same change.
...
**Rule**: Any change to shared bootstrap/layout shell behavior in `apps/helios/src/app/**` must include a sync review and required updates in matching `packages/create-app/template/src/app/**` and dependent template components.
...
**Context**: The standalone app template enabled `{ id: 'webhooks', from: '@helios/webhooks' }` in `packages/create-app/template/src/modules.ts`, but `packages/create-app/template/package.json.template` did not install `@helios/webhooks`.

**Problem**: `yarn install` succeeded, but `helios generate` failed later because the generator resolved a package-backed module that was never installed in the standalone app.
...
**Context**: Standalone parity started failing during `yarn initialize` because `@helios/core` published a deleted migration file that still existed only in `dist/`.
...
**Context**: `create-helios-app` advertised `yarn setup` as the first command, but the scaffold only shipped an empty `yarn.lock`.
...
**Context**: Snapshot parity scaffolds a fresh standalone app from `create-helios-app` and then runs `yarn install` inside that generated directory.
...
**Rule**: When a runtime script stores child processes in a `Set`, all shutdown and cleanup logic must iterate via `Array.from(children)` or direct `for...of`, and the same fix must be mirrored in `apps/helios` and `packages/create-app/template` copies.

**Applies to**: `scripts/dev.mjs`, `apps/helios/scripts/dev.mjs`, `packages/create-app/template/scripts/dev.mjs`, and `packages/create-app/template/scripts/dev-runtime.mjs`.
...
**Problem**: Removing all of `apps/helios/.helios/next` also deletes `.helios/next/dev/cache/turbopack`. That turns `/login`, `POST /api/auth/login`, and especially `/backend` warmup into cold compiles, making greenfield startup much slower than regular `yarn dev` and slower than `main`.

**Rule**: Greenfield cleanup may remove stale route/middleware manifests and lock files before startup, but must preserve `.helios/next/dev/cache/turbopack`. Do not purge Next/Turbopack caches between warmup requests; cache reuse should make each subsequent warmup request faster. Only clear `.helios/next/dev` for explicit `yarn dev:reset` or the one-shot corrupted Turbopack cache recovery path.

**Applies to**: `scripts/dev-cache-purge.mjs`, `packages/create-app/template/scripts/dev-cache-purge.mjs`, `scripts/dev.mjs`, `packages/create-app/template/scripts/dev.mjs`, `apps/helios/scripts/dev.mjs`, `packages/create-app/template/scripts/dev-runtime.mjs`, `scripts/dev-ephemeral.ts`, and related dev-server tests.
...
**Applies to**: `apps/helios/scripts/dev.mjs`, `apps/helios/scripts/dev-runtime-log-policy.mjs`, `packages/create-app/template/scripts/dev-runtime.mjs`, and `packages/create-app/template/scripts/dev-runtime-log-policy.mjs`.
...
**Context**: The standalone `create-app` flow generates app-local `.helios` artifacts while official packages are consumed from `node_modules`.
...
**Context**: Separately, the standalone `yarn generate` OpenAPI bundle still looked for `packages/shared`, `apps/helios`, and `tsconfig.base.json`.

**Problem**: Newly scaffolded apps - `yarn generate` printed noisy OpenAPI bundle resolution errors before falling back.

**Rule**: For standalone app flows, do not hardcode monorepo paths; CLI generators must resolve app/package paths through the shared resolver (`getRootDir()`, `getAppDir()`, `getPackageRoot()`) instead of constructing `packages/*` or `apps/helios/*` paths directly.
...
**Context**: The codebase was refactored to replace all raw `<button>` elements with `Button` and `IconButton` from `@helios/ui/primitives`. This ensures consistent styling, focus rings, disabled states, and dark mode support across the entire application.

**Rules**:

1. **Never use raw `<button>` elements** — always use `Button` or `IconButton` from `@helios/ui`.
...
**Applies to**: All UI components across `packages/ui`, `packages/core`, and `apps/helios`.
...
**Context**: `yarn helios agentic:init --tool=<tool>` is meant to support incremental setup of one coding tool at a time, including retroactive setup from the splash screen.
...
**Problem**: `packages/create-app/template/src/bootstrap.ts` drifted from `apps/helios/src/bootstrap.ts` and did not pass generated `enricherEntries` into `createBootstrap(...)`, so response enrichers were never registered in scaffolded apps.
...
**Problem**: A stale named volume from another app can leave `node_modules` populated but incomplete, with `node_modules/.bin/helios` and `@helios/cli` missing. Startup then fails later with `/bin/sh: helios: not found`.

**Rule**: Docker startup scripts must verify the specific required package/binary for the next command (for example `node_modules/@helios/cli` and `node_modules/.bin/helios` before `yarn initialize`), not just the presence of a non-empty `node_modules` directory.
...
**Rule**: Any external integration provider (payment/shipping/communication/data-sync connector) must be implemented as its own package under `packages/<provider-package>/` and enabled from `apps/helios/src/modules.ts` via that package. Do not add new external provider modules in `packages/core/src/modules/`.
...
**Problem**: German or other scoped Akeneo content leaked into the tenant's base Helios fields, so a single-locale import silently mixed languages and channel variants instead of leaving the field empty.

**Rule**: When importing into non-translation Helios fields, only accept the explicitly selected Akeneo locale/channel plus Akeneo's unlocalized or unscoped fallback entries. Never fall back to a different locale or channel just to fill a value.
...
**Context**: The new `@helios/webhooks` workspace package exposed backend pages through generated imports like `@helios/webhooks/modules/webhooks/backend/webhooks/page`, but the package build only compiled `src/**/*.ts` and the export map stopped before the deepest generated paths.
...
**Context**: A new shared utility under `@helios/shared/lib/events/patterns` built correctly, but sibling workspace tests failed to resolve it through the package name.
...
**Applies to**: `packages/shared/package.json` and future cross-package utilities added under new `@helios/shared/lib/*` paths.
...
**Context**: `@helios/checkout` emitted `import "./commands"` into dist because its build post-processing only rewrote `from` and dynamic imports. `@helios/gateway-stripe` declared `"watch": "node watch.mjs"` without a `watch.mjs` file.
...
**Context**: The core entities CLI imported its own package internals through `@helios/core/...` aliases.
...
**Context**: Standalone `create-helios-app` loaded `@helios/shared/lib/db/mikro` through multiple server chunk/module instances while the monorepo dev app used a single source-tree instance.
...
**Applies to**: ORM/entity registries, DI registrars, module registries, and other standalone-sensitive bootstrap state in `@helios/*` packages.
...
**Context**: The standalone CLI scans installed packages under `node_modules/@helios/*`. Published packages include compiled `dist/modules` output and also ship `src/modules`.
...
**Applies to**: `packages/cli` module scanning, route/API discovery, standalone create-app flows, and any generator that inspects installed `@helios/*` packages.
...
**Applies to**: HackOn implementation tasks and any change governed by `.ai/qa/AGENTS.md` or `.agents/skills/helios-integration-tests/SKILL.md`.
...
**Rule**: When a feature adds a new app-level env var required for local, test, or non-production behavior, update both `apps/helios/.env.example` and `packages/create-app/template/.env.example` in the same change. If standalone CI/bootstrap scripts synthesize `.env`, set the same var there explicitly too.

**Applies to**: `apps/helios/.env.example`, `packages/create-app/template/.env.example`, create-app smoke/parity scripts, and any new env-backed local/testing security feature.
...
**Applies to**: All Playwright integration tests, QA scenario conversions, and any task using `.agents/skills/helios-integration-tests/SKILL.md`.

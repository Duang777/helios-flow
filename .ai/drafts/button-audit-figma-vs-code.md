...
Scope: 5 button categories from Figma DS reconciled with `@helios/ui` primitives.
...
- Local app/template `checkbox.tsx` files reduced to re-exports from `@helios/ui/primitives/checkbox` (single source of truth).

...
- Add a **Discord channel provider** package `@helios/channel-discord` (module id `channel_discord`, provider key `discord`) under the existing `communication_channels` hub. No new framework primitives — it implements the existing `ChannelAdapter` contract.
...
- **Helios as a Discord bot**: a long-running **Gateway worker** (Discord's real-time WebSocket) bridges incoming Discord messages into the hub. A signed **Interactions** HTTP endpoint handles slash commands / button clicks (Discord Ed25519 request signing).
...
Helios already has a unified **Communications Hub** (`communication_channels`) that bridges
...
1. **Helios behaves like a Discord bot.** It connects to a Discord server (guild) with a bot
...
those conversations, notifications, and (optionally) an AI assistant inside Helios.
...
- **No Discord presence.** A grep of the repo returns zero Discord references; tenants who live in
  Discord have no way to bring those conversations into Helios or to drive Helios from
  Discord.
...
1. Go to <https://discord.com/developers/applications> → **New Application**. Name it (e.g.
   "Helios Bot"). Copy the **Application ID** and the **Public Key** from the *General
...
### 4. Connect the channel in Helios
...
| Env var | Purpose |
|---------|---------|
| `HELIOS_CHANNEL_DISCORD_BOT_TOKEN` | Bootstrap bot token for tenant setup. |
| `HELIOS_CHANNEL_DISCORD_APPLICATION_ID` | Bootstrap application id. |
| `HELIOS_CHANNEL_DISCORD_PUBLIC_KEY` | Bootstrap Ed25519 public key. |
| `HELIOS_CHANNEL_DISCORD_GUILD_ID` | Default guild to scope the bot to. |
| `HELIOS_CHANNEL_DISCORD_DEFAULT_CHANNEL_ID` | Default outbound text channel. |
| `HELIOS_CHANNEL_DISCORD_GATEWAY_DISABLED` | When truthy, skip starting the gateway worker (useful in CI / when only outbound is needed). |
...
yarn helios channel_discord register-slash-commands --tenant <tenantId>
```
...
- **Inbound smoke test**: type a message in the connected Discord channel. Within a second it
  should appear in the Helios inbox (the `communication_channels.message.received` event also
...
- **Additive only.** New package `@helios/channel-discord`; new provider key `discord`; new
  ACL features `channel_discord.*`; new env vars `HELIOS_CHANNEL_DISCORD_*`. No existing event ids,
...
- **RBAC**: `channel_discord.view` / `.configure` added to `acl.ts` and `setup.ts`
  `defaultRoleFeatures`; sync via `yarn helios auth sync-role-acls`. ✅
...
- Authored the Discord two-way communication channel + AI bot spec. Scope: new
  `@helios/channel-discord` provider implementing the existing `ChannelAdapter`, a Gateway

...
- Helios currently ships only **in-app** notifications. There is no mobile push channel, no device-token registry, no DB-persistent notification type registry, and no per-channel user preferences.
- This spec proposes:
  - **Two new core modules**:
    - `@helios/core/modules/devices` — generic per-tenant `(user, device, platform)` registry. Push is one consumer; MFA device trust (#539) and session-aware auth/audit logs are plausible future consumers.
    - `@helios/core/modules/push_notifications` — push-token registry, mobile push delivery strategy, provider-pluggable sender (FCM + APNs reference providers), retry worker.
...
Today, an Helios app that wants to deliver mobile push must build the following itself:
...
### Module 1 — `@helios/core/modules/devices` (new)
...
### Module 2 — Extensions to `@helios/core/modules/notifications`
...
### Module 3 — `@helios/core/modules/push_notifications` (new)

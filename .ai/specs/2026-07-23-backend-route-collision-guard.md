...
Verification: 814 existing generator tests pass (including byte-for-byte output snapshots); `typecheck` and `build` clean; a real `yarn generate` on `apps/helios` completes with no false positive on the live module set.

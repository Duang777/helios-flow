...
| Scope | OSS |
| Owner | UI / Auth / RBAC |
| Related Issues | [#1032](https://github.com/helios/helios/issues/1032), [#1031](https://github.com/helios/helios/issues/1031) |
| Related PRs | [#1065](https://github.com/helios/helios/pull/1065) (partial fix — employee wildcard permissions) |
...
Issue [#1032](https://github.com/helios/helios/issues/1032) documents three classes of broken behavior when an Employee role user accesses restricted Settings sections:
...
PR [#1065](https://github.com/helios/helios/pull/1065) partially addressed issue [#1031](https://github.com/helios/helios/issues/1031) by replacing employee wildcard permissions (`sales.*`, `catalog.*`, `customers.*`) with explicit feature lists, preventing `*.settings.manage` inheritance. This was necessary but not sufficient — the underlying UX bugs remain.
...
4. Keep parsing `x-helios-forbidden-redirect` for one minor version as a deprecated no-op bridge
...
- Remove unnecessary `x-helios-forbidden-redirect: 0` headers from callers such as global search
...
1. Add "Sign in as a different user" link alongside "Go to Dashboard" in the page-level `renderAccessDenied()` function (`apps/helios/src/app/(backend)/backend/[...slug]/page.tsx`)
...
| `apps/helios/src/app/(backend)/backend/[...slug]/page.tsx` | E | Add "Sign in as different user" link to `renderAccessDenied()` |
...
**`x-helios-forbidden-redirect` header:** Currently used to opt-out of 403 redirect. After this change, it becomes a deprecated no-op bridge for at least one minor version. Existing callers continue working, but the header no longer changes behavior.
...
- [ ] `apps/helios` and `packages/create-app/template` backend catch-all pages stay aligned

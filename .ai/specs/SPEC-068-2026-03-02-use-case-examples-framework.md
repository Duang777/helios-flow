...
- Examples are **not** part of the Helios core repository. Official examples live in a **separate GitHub repository** `helios/ready-apps` (alongside `helios/helios`); community examples live in their own repos.
- Bootstrap via `create-helios-app --example <name|url>`, adopting the same pattern as `create-next-app --example`.
...
- `--example` flag for `create-helios-app` CLI.
...
- Ensure extensions built for examples fully leverage the Universal Helios Extension System (UMES).

## Overview
Helios needs productized "ready projects" that reduce time-to-first-value for common B2B use cases (for example PRM, field service, marketplace ops). Today, teams start from a generic tenant and manually assemble modules, dictionaries, workflows, and role settings.
...
> **Market Reference:** Modeled after `create-next-app --example`, which lets developers bootstrap from official examples in the Next.js monorepo or from any public GitHub URL. Adopted: `--example` flag, GitHub tarball fetch, centralized official examples repo + community URL support. Rejected: custom CLI commands (`helios init --starter`), npm-only distribution, manual clone-and-copy workflow.
...
2. **Official Examples**: Maintained by the Helios team in a **separate GitHub repository** `helios/ready-apps` (sibling to `helios/helios`), each example as a subdirectory.
3. **Community Examples**: Maintained by partners/agencies in their own GitHub repositories.
4. **The Bootstrap Flow**: `create-helios-app --example <name|url>` fetches the example via GitHub API tarball and scaffolds a ready-to-run app.

### Design Decisions
| Decision | Rationale |
|----------|-----------|
| Examples live outside the core repository | Keeps core clean, reduces bloat, delegates domain ownership to partners/agencies. |
| Official examples centralized in `helios/ready-apps` | Easier maintenance and discoverability, same pattern as `vercel/next.js/examples`. |
| Community examples in independent repos | Partners/agencies own their vertical solutions fully. |
| `--example` flag on `create-helios-app` | Industry-standard UX from `create-next-app`; single-command bootstrap. |
...
|-------------|-------------|
| `helios init --starter` CLI command | Adds core CLI complexity for something that belongs at scaffold time |
...
- No example configurations committed to the `helios/helios` core repository.
...
- An Engineer wants to bootstrap a new B2B PRM application. They run `npx create-helios-app my-prm --example b2b-prm` and get a complete, demo-ready PRM app.
- An Engineer wants to use a community example. They run `npx create-helios-app my-app --example https://github.com/some-agency/their-example`.
...
- An Engineer wants a blank app with no example. They run `npx create-helios-app my-app` (unchanged behavior).

## Architecture

### CLI Interface

New flag for `create-helios-app`: `--example` (alias `-e`)

```bash
# Official example — fetches from helios/ready-apps repo
npx create-helios-app my-prm --example b2b-prm

# Community example — fetches from any public GitHub repo
npx create-helios-app my-app --example https://github.com/some-agency/their-example
...
Resolution logic:
- Plain name (e.g., `b2b-prm`) → fetches the `b2b-prm/` subdirectory from `helios/ready-apps` GitHub repo
...
Official examples live in `helios/ready-apps`:
...
Each example subdirectory is a **complete, runnable app** — the full `create-helios-app` scaffold with domain modules pre-installed.

### Fetch Mechanism

Uses GitHub API tarball download (same approach as `create-next-app`):

- Official: `GET https://api.github.com/repos/helios/ready-apps/tarball/main` → extract subdirectory
...
### Reference Flow
```text
developer runs `npx create-helios-app my-b2b-app --example b2b-prm` →
create-helios-app fetches b2b-prm/ from helios/ready-apps via GitHub API tarball →
...
3. The Helios platform provides the extension points (hooks, enrichers, registries), but not the business configuration for examples.
...
Each example declares compatible core versions through `@helios/*` dependency versions in its `package.json`. No separate version tracking needed.

## Migration & Compatibility
- Examples (being separate codebases) define their compatibility via `@helios/*` dependency versions in `package.json`.
- Core APIs guarantee semantic versioning, allowing example maintainers to update their apps accordingly.
- The `helios/ready-apps` repo should have CI that validates each example still builds against current core versions.
- Backward compatible: `create-helios-app` without `--example` continues to work exactly as before.

## Implementation Plan

### Phase 1 — CLI Flag
1. Add `--example` / `-e` flag to `create-helios-app` argument parser.
...
1. Create `helios/ready-apps` repository.
...
|------|------|--------|---------|
| `packages/create-app/src/index.ts` | helios | Modify | Add `--example` flag, fetch logic |
| `packages/create-app/AGENTS.md` | helios | Modify | Document `--example` flag |
| `examples/b2b-prm/` | helios/ready-apps | Create | First official example |
| `README.md` | helios/ready-apps | Create | Example index and usage docs |
| `.github/workflows/ci.yml` | helios/ready-apps | Create | Validate examples build |

### Testing Strategy
- Unit: URL parsing, name resolution, tarball extraction
- Integration: end-to-end `create-helios-app --example` with a test fixture
...
#### CLI flag breaks existing create-helios-app usage
- **Scenario**: Adding `--example` flag conflicts with existing argument parsing.
- **Severity**: Low
- **Affected area**: `create-helios-app` CLI
...
- **Severity**: Medium
- **Affected area**: `create-helios-app --example` bootstrap
...
- **Mitigation**: Periodic CI on examples repo; Dependabot for `@helios/*` dependency updates.
...
|-------------|------|--------|-------|
| root AGENTS.md | Starters/examples live outside core repository | Compliant | Examples in `helios/ready-apps`, not in core |
...
### 2026-03-20
- Official examples repository changed from `helios/examples` to `helios/ready-apps` (decided after team evaluation).
...
- Added `helios/ready-apps` centralized repository for official examples.

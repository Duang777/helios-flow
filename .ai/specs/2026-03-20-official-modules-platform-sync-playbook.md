...
**Date:** 2026-03-20
**Status:** Draft
**Scope:** OSS — external `helios/official-modules` repository
**Author:** Helios Team
...
**Key Points:**
- `official-modules` MUST use one hard branch rule: `develop` validates against Helios `develop`, and `main` validates against Helios `latest`.
...
- `platform:sync` MUST update only `apps/sandbox/package.json` and existing `@helios/*` entries in workspace `devDependencies`. It MUST NOT touch `peerDependencies`.
...
The repository should tell developers which Helios channel it works with. CI should verify that state. Releases should not require manual edits in many `package.json` files.
...
1. the sandbox app is pinned to one exact canary Helios line
2. package manifests mix published compatibility with local build/test pins
3. CI scaffolds a standalone app from `create-helios-app@latest` even when the work targets `develop`
...
4. update existing `@helios/*` entries in package `devDependencies`
...
- they MUST describe which stable Helios line the module supports
...
```json
{
  "@helios/core": ">=0.4.9 <0.5.0",
  "@helios/shared": ">=0.4.9 <0.5.0",
  "@helios/ui": ">=0.4.9 <0.5.0"
}
```

#### `devDependencies`

`devDependencies` MAY contain exact `@helios/*` versions if the package needs them for local build or test.
...
1. release Helios core to `latest`
...
- `create-helios-app@develop` scaffold flow
- `create-helios-app@latest` scaffold flow
...
5. Restrict sync behavior to:
   ...
   - existing `@helios/*` entries in package `devDependencies`
   - `yarn.lock`
...
3. Switch integration scaffold:
   - `develop` -> `create-helios-app@develop`
   - `main` -> `create-helios-app@latest`
...
- Integration test standalone scaffold:
  - `create-helios-app@develop` for `develop`
  - `create-helios-app@latest` for `main`

...
- Example is bootstrapped via `create-helios-app --example b2b-prm` + `yarn initialize` (per SPEC-068); runtime operations are API/UI based.
...
## Changelog
### 2026-03-17
- Aligned with SPEC-068 (was SPEC-062): replaced `helios init --starter` reference in TLDR with `create-helios-app --example b2b-prm` + `yarn initialize` flow.

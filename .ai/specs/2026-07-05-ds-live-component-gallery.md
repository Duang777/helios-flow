...
- **Touched:** `apps/helios/src/modules.ts` and `packages/create-app/template/src/modules.ts` (enable the module), locale files for gallery chrome
...
5. **Adopters get nothing.** A team scaffolding an app with `create-app` receives the full `@helios/ui` package and zero guidance on what is inside it. The docs live in the monorepo's `.ai/` tree, which does not ship.
...
export type GalleryEntry = {
  ...
  title: string                 // 'Button'
  importPath: string            // '@helios/ui/primitives/button'
...
**Recommendation: ship it.** The module lives in `@helios/core`, so the code travels with every install regardless; the only real decision is whether the template's `modules.ts` enables it. Arguments considered:
...
Enable it in both `apps/helios/src/modules.ts` and `packages/create-app/template/src/modules.ts`.
...
| 2 | Types & interfaces | Additive | `GalleryEntry`/`GalleryVariant` are module-internal; not exported from `@helios/ui`. |
...
5. **Hardening** — integration suite, dark-mode pass, bundle inspection, guardian REVIEW, template enablement (`apps/helios` + `create-app` template `modules.ts`), release-notes entry for the new feature id.
...
- Standard: `yarn generate`, `yarn typecheck`, `yarn lint`, `yarn workspace @helios/core build`, `yarn test`, `yarn test:integration`.

...
**Scope:** OSS — `apps/helios/src/official-modules.generated.ts`, the `official-modules.json` activation flow, related docs/AGENTS guidance
**Author:** Helios Team
**Related:** PR [helios/helios#1908](https://github.com/helios/helios/pull/1908) (introduced the file), PR [helios/helios#1965](https://github.com/helios/helios/pull/1965) (carry-forward of #1945 that taught the `create-app` template to ship the same file), `apps/docs/docs/framework/modules/official-modules-development.mdx`, root `AGENTS.md` → *Generated Files: versioned vs ephemeral*

## TLDR

**Key Points:**
- `apps/helios/src/official-modules.generated.ts` MUST stay where it is — in `apps/helios/src/`, committed to git, not under any `generated/` folder.
- Moving it into `apps/helios/.helios/generated/` or `apps/helios/src/generated/` would gitignore it AND make `yarn clean-generated` delete it, silently destroying the activation set.
- Inlining its contents into `apps/helios/src/modules.ts` would force the `yarn official-modules` CLI to do AST surgery on a hand-curated file (env gates, enterprise toggles, comments) and is rejected.
...
- the external `helios/official-modules` repo workflow ([2026-03-20-official-modules-platform-sync-playbook.md](./2026-03-20-official-modules-platform-sync-playbook.md) already owns that)

## Overview

PR #1908 introduced an opt-in git submodule at `external/official-modules/` and a generated TypeScript registry at `apps/helios/src/official-modules.generated.ts` consumed by `apps/helios/src/modules.ts`. PR #1965 carried forward PR #1945's fix to also ship that file in the `packages/create-app/template/` scaffold so freshly-scaffolded apps don't fail with an unresolved import.
...
1. A contributor opens `apps/helios/src/official-modules.generated.ts`, sees the `// AUTO-GENERATED` banner, and assumes it's misplaced.
2. They propose moving it to `apps/helios/.helios/generated/` "to match the other generated files".
...
1. **Activation state is wiped on clean.** `scripts/clean-generated.sh` runs `find . -type d -name 'generated' -exec rm -rf {} +` and `find . -type d -name '.helios' -exec rm -rf {} +`. Any file inside either pattern is deleted unconditionally.
2. **Activation state never reaches new clones.** `.gitignore` lines 62–70 silently mark `/src/generated/`, `/generated/`, `packages/*/generated/`, and `.helios` as untracked. A `git add` of a file in any of those paths is a no-op; nobody else gets the change.
...
Keep the file at `apps/helios/src/official-modules.generated.ts`. Reinforce the decision in three places so the question stops getting re-raised:

1. **Root `AGENTS.md`** — a new *Generated Files: versioned vs ephemeral* subsection that distinguishes the two categories of generated files, names the canonical examples on each side, and explains why collapsing them into one folder is unsafe. The existing "Generated files: `apps/helios/.helios/generated/` — never edit manually" line and the "MUST NOT add code directly in `apps/helios/src/`" rule both link to the new subsection so the carve-out is visible from both sides.
2. **`apps/docs/docs/framework/modules/official-modules-development.mdx`** — an `:::info Why does the generated file live in `src/` instead of `.helios/generated/`?` callout right under the Config files table, with a link to this spec.
...
| Decision | Rationale |
|---|---|
| Keep `apps/helios/src/official-modules.generated.ts` exactly where it is | Matches the established `*.generated.ts`-in-`src` pattern; survives both `.gitignore` and `yarn clean-generated`; already wired into `modules.ts` and `scripts/template-sync.ts` `SYNC_ROOT_FILES`. |
...
|---|---|
| Move to `apps/helios/.helios/generated/` | `.helios` is gitignored (`.gitignore` line 70). `clean-generated.sh` runs `find -name .helios -exec rm -rf`. The activation set would not survive either a clean or a fresh clone. Destroys the single source of truth. |
| Move to `apps/helios/src/generated/official-modules.generated.ts` | `.gitignore` lines 62–63 (`/src/generated/`) make the file untracked. Same failure modes as above. |
| Carve out a new `apps/helios/generated/` folder that is explicitly NOT gitignored and explicitly NOT wiped | Requires editing `.gitignore` (a negative exception line) AND editing `clean-generated.sh` (skip-this-path branch) AND coordinating a migration of all existing committed `*.generated.ts` files. Pure overhead for zero functional gain — `*.generated.ts` next-to-source already does the same job with fewer moving parts and less risk of someone re-introducing the wipe pattern. |
| Inline contents into `apps/helios/src/modules.ts` | `modules.ts` is curated by humans — it has env gates (`HELIOS_ENABLE_STORAGE_S3`, `HELIOS_ENABLE_ENTERPRISE_MODULES`), conditional blocks, in-source comments, an `enabledModules` array authored by hand, and per-app overrides. `yarn official-modules add/remove` would have to AST-edit that file, which is fragile against user edits and weakens diff review by mixing machine-written and hand-written content. |
...
- **New contributor** asks in a PR review "should this be under `.helios/generated/`?" — the reviewer points at this spec instead of re-deriving the analysis.
...
| `AGENTS.md` (root, *Where to Put Code*) | Existing "MUST NOT add code in `apps/helios/src/`" gets a narrow exception note that links to the new subsection |
| `AGENTS.md` (root, *Key Rules*) | Existing "Generated files: `apps/helios/.helios/generated/`" line replaced by a two-bullet split with link to the new subsection |
...
| `apps/helios/src/official-modules.generated.ts` | Refreshed banner |
...
2. Update `scripts/lib/official-modules.mjs`'s `GENERATED_PATH` constant in lockstep with `apps/helios/src/modules.ts`'s import path and `scripts/template-sync.ts`'s `SYNC_ROOT_FILES`.
...
2. Re-run the writer (or invoke `writeGenerated([])` from the CLI's no-op path) so the committed `apps/helios/src/official-modules.generated.ts` banner picks up the new wording. Commit the refreshed file.

### Phase 3 — Validation

1. Run the targeted test: `node --test scripts/__tests__/official-modules.test.mjs` — confirm the banner regex still matches.
2. Run the full validation gate per `helios-auto-create-pr` step 7.
...
### 2026-05-19
- Initial spec recording the decision to keep `apps/helios/src/official-modules.generated.ts` in `src/` rather than moving it into a `generated/` folder or inlining it into `modules.ts`. Triggered by PR #1965's surfacing of the question.

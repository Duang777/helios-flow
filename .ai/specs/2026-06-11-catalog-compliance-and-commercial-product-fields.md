...
- Quantity-focused by design (Helios business approach): every field is an additive nullable column or defaulted boolean — the product card exposes the data, downstream modules (KSeF invoicing, feeds, carriers, sales cart) consume it later.
...
## Problem Statement
1. A merchant on Helios cannot legally produce a complete KSeF FA(3)/JPK_V7 invoice line because the product carries no GTU, PKWiU, or CN codes (G-8.1, G-5.3, G-8.2). KSeF becomes mandatory for all taxpayers 2026-04-01.
...
7. **Migration** — one catalog migration (add columns + GTIN partial unique index) + `.snapshot-helios.json` update.
...
- One new migration in `packages/core/src/modules/catalog/migrations/` (alter `catalog_products` add 26 columns; alter `catalog_product_variants` add 2 columns; create the GTIN partial unique index) + `.snapshot-helios.json` updated in the same change. `yarn db:generate` re-run must report `no changes` for catalog.

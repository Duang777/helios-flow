...
**Owner:** search / core
**Date:** 2026-05-20
**Tracking issue:** [helios/helios#327](https://github.com/helios/helios/issues/327)
...
2. **Migrate the four un-translated modules.** customers, messages, checkout, inbox_ops — add `import { resolveTranslations } from '@helios/shared/lib/i18n/server'`, lift each hard-coded literal to a translation key (`<module>.search.badge.*`, `<module>.search.link.*`, `<module>.search.fallback.*`), and add entries to each module's `i18n/{en,pl,es,de}.json`.
...
3. Reindex is not required, but a manual `yarn helios search reindex --tenant <id>` confirms no regression in stored data.
...
- **Mitigation**: None required. They serve as the last-resort fallback. Optional cleanup via `yarn helios search reindex` if desired.
- **Residual risk**: None.
...
| packages/shared/AGENTS.md | Use shared `resolveTranslations()` | Compliant | All migrated modules import from `@helios/shared/lib/i18n/server` |

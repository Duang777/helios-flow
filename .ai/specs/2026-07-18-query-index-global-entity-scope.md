...
```bash
# Local runner
yarn workspace @helios/core test --runInBand --runTestsByPath \
...
HELIOS_INTEGRATION_MODULES=feature_toggles npx playwright test --config .ai/qa/tests/playwright.config.ts TC-FT-001
yarn workspace @helios/core typecheck
yarn workspace @helios/shared test --runInBand --runTestsByPath src/lib/query/__tests__/engine.test.ts

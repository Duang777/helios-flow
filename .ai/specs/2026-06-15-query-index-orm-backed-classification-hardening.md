...
`isOrmBackedSystemEntityId` is imported from `@helios/shared/lib/data/engine` (already used by the `entities` API and the records API — no new dependency direction).
...
- No change to `@helios/shared/lib/data/engine` (`isOrmBackedSystemEntityId`, `assertCustomEntityStorageEntityId`), `records.ts` (`classifyRecordsEntity`), or any migration/schema.

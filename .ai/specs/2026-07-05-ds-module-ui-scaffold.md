# DS-Compliant Module UI Scaffolding — `helios module scaffold`
...
Add an executable scaffolding path to the helios CLI — `yarn helios module scaffold <module> --entity <entity> --with-ui` — that generates a DS-compliant UI slice for a new entity: list page (DataTable with `emptyState`, `isLoading`, filters, export), create page (CrudForm + zod validators), tabbed detail page (`Tabs variant="underline"`, `FormHeader mode="detail"`), i18n files for all four locales, a shared `StatusMap`, and `page.meta.ts` metadata with `requireAuth` / `requireFeatures` / breadcrumbs. Templates are derived from the guardian's `page-templates.md` and the customers reference module, including optimistic locking via `initialValues.updatedAt`. Generated output is pinned by golden-file tests and MUST pass `yarn lint:ds`, guardian ANALYZE with zero findings, and typecheck — enforced by a CI job that scaffolds a throwaway module and lints it. The command works in the monorepo (`packages/core/src/modules/`, `apps/helios/src/modules/`) and in standalone create-app consumers (`src/modules/`). Purely additive: no existing CLI command, generator, or runtime file changes.

## Problem Statement

New module UI today is born non-compliant and remediated later:

1. **No executable scaffold exists.** `yarn helios module <add|enable|eject>` (`packages/cli/src/helios.ts`) installs or ejects *existing packaged* modules — it never generates code. The only "scaffolding" paths are:
   - the `helios-module-scaffold` skill (`packages/create-app/agentic/shared/ai/skills/helios-module-scaffold/SKILL.md`, shipped to standalone apps via the agentic setup) — agent-driven prose instructions, non-deterministic output;
...
- Scaffolding the non-UI backbone (entities, migrations, commands, search, events) beyond the minimum the UI slice needs — the `helios-module-scaffold` skill remains the guide for the full module; this spec's command becomes the step it calls for pages (see § Skill integration).
...
Extend the existing `module` dispatch block in `packages/cli/src/helios.ts` with one subcommand:

```bash
yarn helios module scaffold <module_id> \
...
- `--target` — where the module lives:
  - `app` (default in the monorepo) → `apps/helios/src/modules/<module_id>/` (the sanctioned location for user modules; nothing else under `apps/helios/src/` is touched);
  - `packages/core` → `packages/core/src/modules/<module_id>/` (core contributors);
  - in a standalone app (detected via the existing CLI resolver, same detection `createResolver()` uses to scan `node_modules/@helios/*/dist/modules/`) the default and only target is `src/modules/<module_id>/`.
...
For `helios module scaffold tickets --entity ticket --fields "subject:text:required,status:select(open|closed),notes:textarea"`:
...
- `LoadingMessage` / `ErrorMessage` from `@helios/ui/backend/detail` for the fetch lifecycle;
...
Templates live as `.tpl` files in `packages/cli/src/lib/scaffold/templates/` (new directory alongside the existing `lib/generators/`), interpolated with a minimal `{{placeholder}}` substitution — no runtime template engine dependency. They ship inside the published `@helios/cli` package, so standalone apps get the exact same templates as the monorepo, versioned with the CLI.

Relationship to the guardian doc: **the CLI templates become the executable source of truth**. `.ai/skills/helios-ds-guardian/references/page-templates.md` is updated in the same change to (a) reflect the detail-page upgrade to `FormHeader mode="detail"` + `Tabs variant="underline"` and `initialValues.updatedAt`, and (b) open with a pointer: "these templates are maintained as executable scaffolds in `packages/cli/src/lib/scaffold/templates/`; scaffold with `yarn helios module scaffold … --with-ui` instead of copy-pasting." The doc's "DS Checklist for Generated Pages" section is retained — it is the assertion list the contract test below encodes.

### Skill integration

- `helios-module-scaffold` (create-app agentic skill): step 6 ("Create Backend Pages") is rewritten to run `yarn helios module scaffold … --with-ui` and then customize, instead of instructing the agent to write pages from prose. Keep `packages/cli/src/lib/agentic-setup.ts` and `packages/cli/build.mjs` in sync per the existing CLI AGENTS.md rule.
- `helios-ds-guardian` SCAFFOLD-adjacent flows reference the command as the preferred starting point before ANALYZE.

## Architecture

```
packages/cli/src/
├── helios.ts                        # + `module scaffold` branch in the existing `module` dispatch
└── lib/scaffold/
...
- **Phase 3 — Standalone + skill.** Target resolution for standalone apps, create-app integration coverage in the package-preview pipeline, `helios-module-scaffold` skill step-6 rewrite, `agentic-setup.ts`/`build.mjs` sync.
...
3. **CI scaffold-and-lint job** (new step in `.github/workflows/ci.yml`): scaffold a throwaway module into `apps/helios/src/modules/__scaffold_ci__/`, run `yarn generate`, `yarn lint:ds` scoped to the folder, and `yarn typecheck`; delete the folder afterwards (and the job asserts a clean `git status`). This exercises the real repo wiring — path aliases, `#generated` imports, tsconfig — that temp-dir tests cannot. The standalone path is covered by extending the existing create-app integration flow (`packages/cli/src/lib/__integration__/`, Verdaccio-based per `packages/create-app/AGENTS.md`) with a scaffold + `next build`-level typecheck assertion; it runs in the package-preview pipeline, not on every PR.
...
1. **M.2 Step 1 (Scaffold):** replace "Option A: `./ds-scaffold-module.sh invoices invoice`" with `yarn helios module scaffold invoices --entity invoice --fields … --with-ui`; keep "copy from customers" as the manual fallback. Deprecate the bash script in `contributor-guardrails.md` §K.3 with a pointer to the command (script kept one release for link stability, marked superseded).
2. **M.2 Step 4 (Backend pages):** drop the stale K.1.1–K.1.3 section-letter template references; point at `packages/cli/src/lib/scaffold/templates/` and `.ai/skills/helios-ds-guardian/references/page-templates.md`. Add `FormHeader mode="detail"` and `Tabs variant="underline"` to the "every template requires" list.
...
| Standalone version skew — an app's installed `@helios/ui` predates what the CLI templates import | Medium | create-app consumers | Templates ship inside `@helios/cli` and are versioned/published with the matching `@helios/ui`; scaffold prints the minimum package versions it assumes; create-app integration test guards the released pairing | Low-medium — mixed manual upgrades can still skew; failure mode is a typecheck error, not silent breakage |
...
- The `helios-module-scaffold` skill update is backward compatible: agents on older create-app scaffolds simply keep the prose path until they upgrade `@helios/cli`.
...
yarn workspace @helios/cli test
yarn workspace @helios/cli build

# End-to-end in the monorepo (mirrors the CI job)
yarn helios module scaffold tickets --entity ticket \
  --fields "subject:text:required,status:select(open|closed),notes:textarea" --with-ui
yarn generate
yarn lint:ds packages/../apps/helios/src/modules/tickets   # zero findings
yarn typecheck
yarn i18n:check-hardcoded                                   # no new hardcoded strings
# guardian: run ANALYZE on the scaffolded module — expect zero findings
git clean -fd apps/helios/src/modules/tickets              # throwaway

# Determinism
yarn helios module scaffold … --dry-run                    # identical plan on re-run

# Standalone (package-preview pipeline)
# create-app via Verdaccio → helios module scaffold → yarn generate → typecheck
```

...
- **Scope:** OSS
- **Owner module:** `configs` (admin surface) + `@helios/queue` (introspection contract)
...
- Direct Redis/BullMQ inspection or reading `.helios/queue/<name>/state.json` by hand.
...
└───────────────────────────────┼────────────────────────────────────────────┘
                                ...
                                ▼
┌──────────────────────────────────────────────────────────────────────────┐
│ @helios/queue (introspection contract — ADDITIVE only)               │
...
contract lives in `@helios/queue`. The `configs` route depends on the queue package
...
(`packages/cli/src/helios.ts`): collect every module `WorkerDescriptor`, then
`[...new Set(allWorkers.map(w => w.queue))]`. The snapshot builder reuses the worker
registry export (`@helios/queue` → `worker/registry`) so the dashboard and the worker
...
- New backend page `configs/backend/config/queues-status/page.tsx` rendering a
  ...
  `Page`/`PageBody` from `@helios/ui/backend/Page`.
...
Contract surface touched: the `@helios/queue` **types** and the **`Queue` interface**
...
| Gate | Status | Notes |
|------|--------|-------|
| Architecture: admin surface in `configs`, contract in `@helios/queue`; no direct ORM cross-module relationship | ☐ deferred | Verified at design time against the existing `system-status` pattern and the worker registry export. |
...
| Backward compatibility: all `@helios/queue` changes additive-only | ☐ deferred | `delayed?`, `getFailedJobsSummary?`, `getProcessingStats?`, `getInFlightJobsSummary?`, `getRecentJobs?` optional; `?queue=`/`?type=`/`?id=` query params additive; no removed/renamed export, no event-id change. |
...
- **2026-06-15** — Spec authored (Draft, deferred). Two-tier design: Tier 1 payload-free
  ...
  Job Inspector (last-X jobs with redacted payloads). Additive `@helios/queue`

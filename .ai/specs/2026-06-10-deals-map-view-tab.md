...
- New production dependencies in `@helios/core`: `leaflet` (BSD-2-Clause) + `leaflet.markercluster` (MIT), lazy-loaded only on the map route.
...
## Configuration
- `NEXT_PUBLIC_HELIOS_DEALS_MAP_TILE_URL` — tile template, default `https://tile.openstreetmap.org/{z}/{x}/{y}.png`.
- `NEXT_PUBLIC_HELIOS_DEALS_MAP_TILE_ATTRIBUTION` — attribution HTML, default `&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors`.
...
- [x] Phase 2 — Map UI + address coordinates (tab, page, lazy Leaflet canvas, i18n ×4, deps; coordinate inputs in BOTH address-editor families — core `components/` + shared `ui/backend/detail` — opt-in, enabled for customers create+detail surfaces; normalizer helper moved to `@helios/shared/lib/location/coordinates`)
...
- Pre-implement audit corrections applied (see `.ai/specs/analysis/ANALYSIS-2026-06-10-deals-map-view-tab.md`): filter builder already exported, snake_case projection, gated aux fetches with `x-helios-forbidden-redirect: 0`, optional `AddressEditorDraft` fields + `showCoordinateFields` opt-in, i18n/crmFixtures additive overlap with #2903.
...
- **Known limitations (non-blocking, documented):** `owner_asc` panel sort is client-side over the rendered ≤500 cap (so on >500 located deals it orders only the first 500 by `updatedAt`); the light-pass located-id allowlist is not page-bounded (fine at the tested ~1000, noted ceiling); `@helios/ui` `AddressEditor` gained the opt-in `showCoordinateFields` prop (default-off, a noted scope deviation from "don't touch the ui AddressEditor"); owner names depend on `customers.roles.view` and degrade to empty gracefully when missing.

...
A new helper in `@helios/shared/security/aclDependencies.ts`:
...
Introducing the new view features is BC-safe (additive) and requires updating `setup.ts` `defaultRoleFeatures` plus running `yarn helios auth sync-role-acls`. The per-module issue covers this.
...
### 6.45 `apps/helios/example` and templates

...
The existing helper `runModuleCommand(..., { optional: true })` already covered some bootstrap flows inside `helios init`, but other code paths bypassed it entirely by:

- spawning `helios <module> <command>` as a subprocess
...
|------|--------------|---------|
| `helios init` post-bootstrap extras | direct calls assumed modules existed | lean presets crashed on `feature_toggles`, `dashboards`, `search` |
| `helios server dev` | auto-spawned `scheduler start` when `QUEUE_STRATEGY=local` | CRM/empty presets crashed with `Module not found: "scheduler"` |
| `helios server start` | same unconditional scheduler auto-spawn | production-style start could fail for lean apps |
...
Introduce a shared command-availability lookup in `packages/cli/src/helios.ts`:
...
This helper must continue to be used by `helios init` and any future in-process bootstrap/setup steps.

### 3. Add optional handling before spawned module subprocesses

For orchestration code that uses `spawn('node', [heliosBin, module, command, ...])`, availability must be checked before spawning.
...
if (resolved.status !== 'ok') {
  ...
  spawn('node', [heliosBin, 'scheduler', 'start'], ...)
}
```
...
- `packages/cli/src/__tests__/helios.test.ts`
...
- `apps/helios/scripts/dev.mjs`
  - the `helios` binary is now resolved via `resolveProjectBinary(...)` so the dev wrapper picks the workspace-local `node_modules/.bin/helios` instead of relying on PATH lookup. This is required to keep dev startup portable across yarn-workspace setups where `helios` is not on PATH (notably standalone-app CI), and matches the resolution strategy already used by the standalone template's `dev-runtime.mjs`.
...
Previously `helios server start` could return exit code `0` when a managed child (Next.js server, queue worker, scheduler) exited unexpectedly during startup or runtime. After this change, unexpected exits are surfaced as `[server] <Label> exited unexpectedly with ...` and propagated as a non-zero exit code.

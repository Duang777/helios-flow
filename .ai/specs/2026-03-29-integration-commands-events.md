...
**What we adapt:**
- Zapier triggers → Helios **integration events** emitted into the existing event bus with `clientBroadcast: true`
- Zapier actions → Helios **integration commands** executed via `IntegrationGateway` DI service
- Zapier workflow composition → delegated to Helios's `workflows` module and direct programmatic calls

**What we reject:**
- Zapier's workflow engine (Helios has its own)
...
import { z } from 'zod'
import type { IntegrationCommandContext } from '@helios/shared/modules/integrations/types'
...
|-----------|---------|-----------|
| `IntegrationCommandDefinition`, `IntegrationEventDefinition`, `IntegrationCommandContext` types | `@helios/shared` | Cross-package types, no domain dependency |
| `IntegrationGateway` service, command handler registry, gateway API routes | `@helios/core` (integrations module) | Core integration infrastructure |
| Command handler files (`commands/*.ts`) | Provider packages (e.g., `@helios/sync-akeneo`) | Provider-specific implementation |
| Generator for `commands/*.ts` auto-discovery | `@helios/cli` | Build tooling |
| Capabilities UI tab | `@helios/ui` (via UMES widget injection) | UI layer |
...
| 4 | Import paths | **ADDITIVE** | New exports from `@helios/shared/modules/integrations/types`; no moved modules |
...
```typescript
import { isModuleEnabled } from '@helios/shared/lib/modules/registry'

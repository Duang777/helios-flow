...
- The failure was reproduced in a standalone TraceCore app on a clean `origin/main` baseline, using an unrelated warehouse browser spec, so this is an Helios integration auth/session reliability issue rather than an application feature regression.
...
**Scope:**
- Add a deterministic regression covering the browser `/backend` cookie flow used by `@helios/core/helpers/integration/auth`.
...
at login (.../node_modules/@helios/core/src/helpers/integration/auth.ts:193:16)
```
...
## Proposed Solution
Implement the fix in Helios, not in downstream app specs:
...
3. `apps/helios/src/app/(backend)/backend/page.tsx` and `packages/shared/src/lib/auth/server.ts`
...
No new App Router pages, backend shell components, providers, or heavy client widgets are planned. If implementation evidence proves that `apps/helios/src/app/(backend)/backend/page.tsx` or shared backend shell UI must change, the implementation PR must add a Frontend Architecture Contract before touching those files. For the current scoped design, this section is N/A.
...
Optional test-only diagnostics may be gated by an env var such as `HELIOS_INTEGRATION_AUTH_DEBUG=1`, but the regression should not require it.
...
- The published helper import `@helios/core/helpers/integration/auth` remains stable.
...
2. Import `login` from `@helios/core/helpers/integration/auth`, not from the legacy monorepo helper path.
...
3. Run package tests/builds for `@helios/core` and `@helios/cli`.
...
- **Mitigation**: Regression must import from `@helios/core/helpers/integration/auth`; CLI readiness must run through the same package path used by standalone apps.
...
| helios-spec-writing checklist | MVP is explicit and future work is deferred | Compliant | TLDR includes a concrete MVP boundary and Non-Goals defer auth model/UI redesign. |
| `BACKWARD_COMPATIBILITY.md` | Public import paths, API URLs, cookie names, response fields, DI names, and session semantics are stable contract surfaces | Compliant | Spec forbids renames/removals and keeps `@helios/core/helpers/integration/auth` stable. |
...
| helios-spec-writing checklist | Input validation, injection, XSS, and URL encoding are covered | Compliant | Security and Input Handling Invariants keep existing zod validation/parameterized service access and mark UI/XSS as N/A. |
| helios-spec-writing checklist | Performance, cache, and scale impact is explicit | Compliant | Performance, Cache & Scale section marks cache/list/index/worker concerns N/A and bounds readiness cost. |
...
- Reviewed with `helios-spec-writing`: added metadata, related-spec boundaries, non-goals, root-cause gates, API/readiness contracts, expanded risks, and final compliance matrix.

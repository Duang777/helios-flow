...
Helios currently scaffolds a "kitchen sink" starter with roughly 44 non-enterprise modules enabled by default. This spec defines an additive, backward-compatible way to create much leaner starters:
...
4. Changing the default scaffold directly would be a backward compatibility violation for `create-helios-app`.
...
|------|------------------|-------------------------|-------------|
| App defaults | `apps/helios/src/modules.ts` and `packages/create-app/template/src/modules.ts` enable the full suite | Empty starter is impossible without manual editing | Introduce preset-owned module manifests |
| App template UX | `apps/helios/src/app/page.tsx` and template equivalent hard-link to example routes | Empty starter shows dead links | Make start page links preset-aware and omit example links outside `classic` |
| Example module | `apps/helios/src/modules/example/**` and template equivalent inject into `customers`, `sales`, `catalog`, `checkout` | Example module cannot survive lean presets | Exclude example module entirely from `empty` and `crm` |
...
### 1. Add starter presets to `create-helios-app`

Add a new additive CLI flag:

```bash
npx create-helios-app my-app --preset classic
npx create-helios-app my-app --preset empty
npx create-helios-app my-app --preset crm
```
...
This example is illustrative, but the architecture intent is important: new built-in presets should mostly be additive manifest entries, not new imperative branches in `create-helios-app`.
...
10. optionally write `.helios/starter-preset.json` for later diagnostics
...
9. optionally persist a scaffold marker such as `.helios/starter-preset.json`
...
- infrastructure packages imported by bootstrap (`@helios/core`, `@helios/shared`, `@helios/ui`, `@helios/cli`, `@helios/cache`, `@helios/events`, `@helios/search`) may remain installed even when their module IDs are not enabled
- obviously unused package-backed modules such as `@helios/checkout`, `@helios/ai-assistant`, `@helios/content`, `@helios/onboarding`, `@helios/webhooks`, `@helios/gateway-stripe`, and `@helios/sync-akeneo` may be pruned in the lean presets once green tests confirm no bootstrap import path depends on them
...
- optional generated marker file such as `.helios/starter-preset.json`
...
```bash
create-helios-app <app-name> [--preset classic|empty|crm]
```
...
- `create-helios-app --app` and `--app-url`
...
|------|----------|---------------|------------|---------------|
| Hidden dependency missed during preset creation | High | `helios init`, bootstrap, page render | Add explicit integration tests for each preset plus dependency audit | Medium |
...
12. Optionally write `.helios/starter-preset.json` after scaffold completion for diagnostics and future upgrade tooling.
...
| Scenario | Type |
|----------|------|
| `create-helios-app my-app` still produces current scaffold | Integration |
| `create-helios-app my-app --preset empty` produces the 10-module baseline | Integration |
| `create-helios-app my-app --preset crm` produces the 13-module baseline | Integration |

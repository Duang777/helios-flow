...
- **Schema:** new nullable columns + partial unique indexes via a `configs`-module migration; update `migrations/.snapshot-helios.json` in the same change. Existing rows keep `tenant_id = NULL` and remain valid (now interpreted as the instance default). No backfill.

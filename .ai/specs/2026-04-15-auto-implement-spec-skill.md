...
## TLDR
**Key Points:**
- New orchestrator skill that takes an existing spec from `.ai/specs/` and drives it to completion across multiple sessions by dispatching `helios-auto-create-pr` (new phases) and `helios-auto-continue-pr` (unfinished phases).
...
**Scope:**
- New skill file: `.ai/skills/helios-auto-implement-spec/SKILL.md`
- Registration in `.ai/skills/README.md` and root `AGENTS.md` Task Router

**Concerns:**
- Must not break or duplicate existing `helios-implement-spec` (interactive) or `helios-auto-create-pr` (autonomous executor)

---

## Overview

The auto-* skill family (`helios-auto-create-pr`, `helios-auto-continue-pr`, `helios-auto-fix-github`, `helios-auto-review-pr`) provides autonomous agent execution with worktree isolation, PR delivery, and resumability. However, there is no auto-* skill for the most common OM workflow: **implementing an existing spec from `.ai/specs/`**.

Today a developer with a large spec (e.g. WMS with 5 phases, 13 entities) faces a gap:
- `helios-implement-spec` works interactively but has no PR delivery, no worktree isolation, no resumability
- `helios-auto-create-pr` has all of that but expects a free-form brief and creates a new execution plan — it doesn't consume existing specs
...
| Decision | Rationale |
|----------|-----------|
| Orchestrator only — no implementation logic | Reuse `helios-auto-create-pr` and `helios-auto-continue-pr` as-is. One thing to maintain. |
| One PR per spec phase | Natural boundary. Phases are designed to be independently testable and deployable. |
| Dispatch via `Agent` tool, worktree managed by `helios-auto-create-pr` | `helios-auto-create-pr` already manages its own worktree isolation. No need to duplicate that with `isolation: "worktree"` on the Agent call. |
| Parallel dispatch for independent phases | If Phase 3 and Phase 4 have no dependency, dispatch both as parallel `Agent` calls in a single message. Each invokes `helios-auto-create-pr` which creates its own worktree independently. |
| `helios-implement-spec` stays unchanged | Different use case: interactive, developer-in-the-loop. Not deprecated. |
...
1. **Developer doesn't know what to give `helios-auto-create-pr`** — it expects a free-form brief, but the spec already exists with all the details. Developer must manually extract and reformulate phase scope into a brief. This is error-prone and feels redundant.

2. **No single command to drive a spec to completion** — developer must manually track which phases have PRs, which PRs are merged, which are in progress, and invoke the right skill (`helios-auto-create-pr` vs `helios-auto-continue-pr`) with the right arguments each time.

3. **`helios-implement-spec` lacks autonomous delivery** — it implements code but doesn't create PRs, doesn't use worktree isolation, and can't be resumed across sessions.
...
4. **Dispatches the right action** per phase:
   - Phase not started → `helios-auto-create-pr` via `Agent` (manages its own worktree)
   - Phase has open PR, incomplete → `helios-auto-continue-pr` via `Agent` (manages its own worktree)
...
|-------------|-------------|
| Merge `helios-auto-create-pr` into `helios-implement-spec` | Breaks interactive use case. Two different developer workflows. |
| Make `helios-auto-create-pr` accept `--spec` flag | Overloads its purpose. It's an executor, not an orchestrator. |
...
Each parallel agent invokes `helios-auto-create-pr` or `helios-auto-continue-pr` which manages its own worktree internally. They produce separate PRs. Multiple `Agent` calls in a single message run in parallel.

**Why this works**: `helios-auto-create-pr` already creates an isolated worktree, implements, and cleans up. Dispatching two agents in one message means two `helios-auto-create-pr` runs in parallel, each with its own worktree. No conflicts, no shared state. No changes to `helios-auto-create-pr` needed.

### How phase-to-PR matching works

When `helios-auto-create-pr` is dispatched by this skill, the structured brief includes a marker that ends up in the execution plan and PR body:
...
Before dispatching `helios-auto-create-pr` for a new phase, the skill validates:
...
This gives `helios-auto-create-pr` everything it needs without the developer writing anything. The execution plan created by `helios-auto-create-pr` goes to `.ai/runs/`, keeping `.ai/specs/` clean.
...
- `helios-implement-spec` remains unchanged — different use case (interactive)
- `helios-auto-create-pr` remains unchanged — consumed as-is
- `helios-auto-continue-pr` remains unchanged — consumed as-is
- Purely additive: one new skill folder + README/AGENTS.md registration

## Implementation Plan

### Phase 1: Skill file
1. Create `.ai/skills/helios-auto-implement-spec/SKILL.md` with full workflow
...
3. Verify PR matching works with `helios-auto-create-pr` PRs
...
- **Severity**: Medium
- **Affected area**: Implementation quality of dispatched `helios-auto-create-pr`
...
- **Scenario**: `helios-auto-create-pr` times out before finishing the phase
- **Severity**: Low
- **Affected area**: Developer needs to re-run
- **Mitigation**: This is the designed flow — developer runs `/auto-implement-spec` again, skill detects the open PR and dispatches `helios-auto-continue-pr`. No manual intervention needed beyond re-running the command.
...
| .ai/specs/AGENTS.md | No execution artifacts in .ai/specs/ | Compliant | Execution plans go to `.ai/runs/` via `helios-auto-create-pr` |
...
- Updated after review: removed superpowers dependencies (helios-cto, Extension Mode Decision), removed --phase flag (auto-detect), added session timeout risk, clarified "too large" warning behavior, aligned with .ai/runs/ separation (PR #1531)
- Added parallel dispatch: dispatch independent phases as parallel `Agent` calls, each invoking `helios-auto-create-pr` which manages its own worktree. No `isolation: "worktree"` needed — reuse existing worktree management in auto-create-pr. Added dependency graph analysis, parallel dispatch strategy, and risks for parallel execution.

...
- Keep the change in core `@helios/ui`, because downstream apps reuse this dialog and should not need app-level CSS workarounds.
...
|---------|------|-----------|-------|
| Attachment metadata dialog | `packages/ui/src/backend/detail/AttachmentMetadataDialog.tsx` | `@helios/ui` | Existing client component with `CrudForm`, `Input`, `Dialog`, `apiCall`, `useT`, and `useDialogKeyHandler`. |
...
| Shared `@helios/ui` dialog | N/A shared package component | Existing `"use client"` file `AttachmentMetadataDialog.tsx` | Parent component owns save mutation; dialog owns local form state | Only row layout and icon-button accessibility change. |
...
| Performance evidence | Static check plus focused package validation | Focused UI test plus `yarn workspace @helios/ui build` |
...
- Build `@helios/ui` after the test passes.
...
- Use `Input` from `@helios/ui/primitives/input`.
...
- Existing consumers benefit automatically after upgrading `@helios/ui`.
...
2. Run `yarn workspace @helios/ui build`.
...
```bash
yarn workspace @helios/ui test -- AttachmentMetadataDialog
...
- **Severity**: Low
- **Affected area**: `@helios/ui` test suite
...
- `.ai/skills/helios-spec-writing/references/frontend-architecture-contract.md`
- `.ai/skills/helios-spec-writing/references/spec-checklist.md`
- `.ai/skills/helios-spec-writing/references/compliance-review.md`

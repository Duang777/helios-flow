...
3. ephemeral environments (`yarn dev:ephemeral`, `helios test:ephemeral`, `helios test:integration`)
...
- named commands must set `HELIOS_BUILD_PARTITION`, not raw `HELIOS_BUILD_OUTDIR`
- build scripts may support `HELIOS_BUILD_OUTDIR` as an escape hatch
- if `HELIOS_BUILD_PARTITION` is set, runtime/tooling consumers must resolve the corresponding partition path
...
```jsonc
"build:check": "cross-env HELIOS_BUILD_PARTITION=check turbo run build --filter='./packages/*'",
"build:packages:ephemeral": "cross-env HELIOS_BUILD_PARTITION=ephemeral turbo run build --filter='./packages/*'"
```
...
- `dist-ephemeral/`
  ...
  - consumed by: `dev:ephemeral`, `helios test:ephemeral`, `helios test:integration`, related ephemeral helpers
...
- root `package.json` `helios` launcher
- `packages/cli/bin/helios`
...
- ephemeral commands never treat `dist/` as reusable build state when `HELIOS_BUILD_PARTITION=ephemeral`
...
`yarn dev:ephemeral` must export `HELIOS_BUILD_PARTITION=ephemeral` into:
...
This matters because app-level scripts (`helios generate`, `helios init`, `helios server dev/start`) go through the CLI, and the CLI must resolve the same partition consistently.
...
| Risk | Failure Scenario | Impact | Mitigation |
|------|------------------|--------|------------|
| Root launcher remains hardcoded | Root `helios` script still points directly at `packages/cli/dist/bin.js` | Partition-aware CLI loader is bypassed | Route root launcher through a partition-aware stub |
...
- `packages/cli/bin/helios`
- `packages/create-app/bin/create-helios-app` if partition-aware loading is required there
...
- add shared resolver for `HELIOS_BUILD_PARTITION`
- add `build:check`
...
- make root `helios` launcher partition-aware
...
- propagate `HELIOS_BUILD_PARTITION=ephemeral` into `generate`, `initialize`, and runtime startup
...
5. Update ephemeral runtime scripts to set and propagate `HELIOS_BUILD_PARTITION=ephemeral`.

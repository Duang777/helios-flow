...
- `.ai/specs/implemented/2026-05-13-lazy-auto-spawn-scheduler.md` and `.ai/specs/implemented/2026-05-07-lazy-auto-spawn-queue-workers.md` — already implemented; `yarn dev` auto-sets `HELIOS_AUTO_SPAWN_WORKERS_LAZY=true` and `HELIOS_AUTO_SPAWN_SCHEDULER_LAZY=true` in `scripts/dev.mjs:517-526`.
...
| App `helios server dev` (Next.js + Turbopack) | 1 Node, plus Turbopack's daemon | ~1.5–3 GB | Dominant single consumer at peak; `preloadEntriesOnStart: false` already set |
| `helios generate watch --skip-initial` | 1 Node | ~80–150 MB | Regenerates `.helios/generated` on source changes |
...
| 5 | Lazy generator watch (`helios generate watch`) — defer first run until first edit lands | ~80–150 MB | Medium (changes initial state; rare but legitimate test paths rely on generated artifacts being there at boot) | Not started |
...
Paste the resulting Markdown table into the PR body. Reports are written under `.helios/dev-rss/<label>.json` and persist across runs.
...
- `HELIOS_DEV_MEMORY_TRACE=1 yarn dev` emits opt-in NDJSON samples plus a final JSON summary under `.helios/dev-rss/`, including peak RSS, dominant process class, top processes, cgroup memory when available, and lifecycle markers nearest the peak.
...
- New report files at `.helios/dev-rss/*.json` live under the existing gitignored `.helios/` tree.

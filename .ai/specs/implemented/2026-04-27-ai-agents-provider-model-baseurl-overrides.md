...
> Follow-up (2026-06-26): a later, additive Docker/OpenCode runtime change threads OpenRouter env vars through the standard compose surfaces and teaches the OpenCode entrypoint to emit `provider.openrouter.models` when `HELIOS_AI_PROVIDER=openrouter` or `OPENCODE_PROVIDER=openrouter`.
...
- **Phase 0** — a single `.env` knob (`HELIOS_AI_PROVIDER` + `HELIOS_AI_MODEL`) that selects the default provider/model used by `createModelFactory` when no caller / module / agent override is in play. Land this independently so ops can pin defaults today.
...
- **Phase 3** — call-site cleanup: `agent-runtime.resolveAgentModel` migrates to `createModelFactory` (already flagged as a follow-up in the AI Assistant AGENTS.md), the routing route in `api/route/route.ts` honors `HELIOS_AI_PROVIDER`, and the `inbox_ops` legacy path's order-of-preference lines up with the new factory.
...
| 6 | Global `.env`              | `HELIOS_AI_PROVIDER` (NEW Phase 0) | `HELIOS_AI_MODEL` (NEW Phase 0) | per-preset `baseURLEnvKeys` (existing; expanded in Phase 2) |
...
| File | Change |
|------|--------|
| `packages/ai-assistant/src/modules/ai_assistant/lib/model-factory.ts` | (a) Read `HELIOS_AI_PROVIDER` from env → if set, prepend it to the registry walk order. (b) Read `HELIOS_AI_MODEL` → use as fallback model id (priority below `agentDefaultModel`, above `provider.defaultModel`). |
...
| `apps/helios/.env.example` | Add commented `# HELIOS_AI_PROVIDER=` and `# HELIOS_AI_MODEL=` block with the same 8 examples already shown for `OPENCODE_MODEL`. |
...
`HELIOS_AI_PROVIDER` semantics: when set and resolves to a registered provider id, the factory passes it as the first element of `resolveFirstConfigured`'s `order` array; if the named provider is registered but unconfigured, the factory falls through to the next-best as today. When unset, behavior is unchanged. We deliberately do NOT alias `OPENCODE_PROVIDER` to the new var — the legacy var stays bound to the OpenCode stack so the two systems remain decoupled (see "Coexistence with OpenCode Code Mode" in `packages/ai-assistant/AGENTS.md`).

`HELIOS_AI_MODEL` semantics: a plain model id (`gpt-5-mini`) is interpreted under the resolved provider; a slash-qualified id (`openai/gpt-5-mini`) overrides the resolved provider for that resolution only, just like Phase 1's per-agent slash form.

Tests:

- `model-factory.test.ts` (existing) — add cases for `HELIOS_AI_PROVIDER` only, `HELIOS_AI_MODEL` only, both, both with the named provider unconfigured (must fall through), and slash-qualified `HELIOS_AI_MODEL` (must reset provider).
...
| `packages/core/src/modules/customers/ai-agents.ts` (and `catalog/ai-agents.ts`) | Local copies of `AiAgentDefinition` need the new field too — the type is duplicated in each module. Refactor candidate (Phase 3) to import from `@helios/ai-assistant` instead. |
...
| `apps/helios/.env.example` | Add `# OPENROUTER_API_KEY=` / `# OPENROUTER_BASE_URL=` / `# LM_STUDIO_API_KEY=` / `# LM_STUDIO_BASE_URL=` blocks; add `# OPENAI_BASE_URL=` next to `OPENAI_API_KEY` (and the same for DeepInfra / Groq / Together / Fireworks). |
...
- [ ] `0.1` Add `HELIOS_AI_PROVIDER` + `HELIOS_AI_MODEL` resolution to `model-factory.ts`. Add `'env_default'` to `AiModelResolution['source']` enum.
- [ ] `0.2` Update `model-factory.test.ts` with the 5 cases from "Phase 0 — Tests" above.
- [ ] `0.3` Update `model-factory.integration.test.ts` with one end-to-end smoke that sets `HELIOS_AI_PROVIDER=openai` + only `OPENAI_API_KEY` and asserts the resolved model id round-trips.
- [ ] `0.4` Update `apps/helios/.env.example` and `packages/create-app/template/.env.example`.
...
- [ ] `3.3` Unify `inbox_ops/lib/llmProvider.ts` resolution order with the factory; legacy `OPENCODE_PROVIDER` / `OPENCODE_MODEL` move behind canonical `HELIOS_AI_PROVIDER` / `HELIOS_AI_MODEL` instead of step 1.
- [ ] `3.4` Remove the duplicate `AiAgentDefinition` shapes in `customers/ai-agents.ts` and `catalog/ai-agents.ts`; both import from `@helios/ai-assistant`.
...
- [ ] `4b.13` Changelog entry per the `helios-auto-update-changelog` convention.
...
chain reads `HELIOS_AI_PROVIDER` (unset) and continues to pick Anthropic —
**but** if Phase 0 adds `HELIOS_AI_PROVIDER` aliasing to `OPENCODE_PROVIDER`
...
and sets `HELIOS_AI_PROVIDER=openai` deliberately gets the OpenAI default,
...
`HELIOS_AI_RUNTIME_DEFAULTS_MODEL` and `HELIOS_AI_RUNTIME_DEFAULTS_PROVIDER`
per-module env vars, which look close to the global `HELIOS_AI_MODEL` /
`HELIOS_AI_PROVIDER` names.

**Mitigation.** The canonical global namespace is `HELIOS_AI_PROVIDER` /
`HELIOS_AI_MODEL`; per-module overrides are namespaced as
`HELIOS_AI_<MODULE>_PROVIDER` / `HELIOS_AI_<MODULE>_MODEL`. A hypothetical module
named `runtime_defaults` would resolve `HELIOS_AI_RUNTIME_DEFAULTS_PROVIDER`
(uppercased moduleId between `HELIOS_AI_` and `_PROVIDER`), not
`HELIOS_AI_PROVIDER`. The collision is only hypothetical until someone adds such
...
- §13 Generated file contracts: `ai-agents.generated.ts` continues to
  ...
  module path get a re-export from `@helios/ai-assistant` (still
...
- **Pre-implementation analysis recommended**: yes — run the `helios-pre-implement-spec` skill against this file before kicking off Phase 0 to confirm the resolution-chain table matches the contributor's mental model and to surface any missed call sites that grep didn't catch.
...
- **2026-05-11** — Phase 0 env var names renamed during PR #1856 review. Process-wide knobs are now `HELIOS_AI_PROVIDER` / `HELIOS_AI_MODEL` (legacy `OPENCODE_PROVIDER` / `OPENCODE_MODEL` honored as BC fallbacks) and per-module overrides are `HELIOS_AI_<MODULE>_MODEL` (legacy `<MODULE>_AI_MODEL` honored as BC fallback). Aligns Phase 0 with the existing `HELIOS_*` env convention used by the rest of the runtime (`HELIOS_SEARCH_*`, `HELIOS_SECURITY_*`, `HELIOS_ENABLE_*`). The PR also bumped scope to thread the new vars through `docker-compose.yml`, `docker-compose.fullapp*.yml`, `docker/opencode/entrypoint.sh`, `.devcontainer/devcontainer.json`, and the `create-helios-app` standalone template so a deployment that only sets `OPENAI_API_KEY` resolves to OpenAI + `gpt-5-mini` out of the box.
...
- **2026-06-26** — Docker/OpenCode follow-up: OpenRouter is now available through the same runtime surfaces as the built-in OpenAI/Anthropic/Google providers. `OPENROUTER_API_KEY` / `OPENROUTER_BASE_URL` are forwarded by the monorepo, fullapp, devcontainer, and standalone compose templates, and `docker/opencode/entrypoint.sh` emits an `openrouter` provider block when selected. This is additive-only and leaves the legacy shared `OpenCodeProviderId` helper narrow for backward compatibility.

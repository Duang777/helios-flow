...
> **Reference Pattern:** Gmail, Outlook, and Roundcube all use row selection plus a list-level action toolbar for repetitive inbox work. Helios should adopt the selection pattern, but in v1 it should keep the existing admin-table toolbar placement instead of adding a Messages-only floating bar.

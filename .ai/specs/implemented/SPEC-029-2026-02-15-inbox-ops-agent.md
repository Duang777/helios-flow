...
export class InboxSettings {
  ...
  inboxAddress!: string  // e.g., ops-acme@inbox.helios.com
...
│     Your inbox address:                                            │
│     ┌──────────────────────────────────────────────────┐           │
│     │ ops-acme@inbox.helios.com          [Copy]   │           │
│     └──────────────────────────────────────────────────┘           │
│                                                                     │
...
|-----------|--------|---------|
| `DataTable` | `@helios/ui/backend` | Proposals list with sorting/filtering/pagination |
| `CrudForm` | `@helios/ui/backend/crud` | Edit action dialog |
| `LoadingMessage` / `ErrorMessage` | `@helios/ui/backend/detail` | Loading/error states |
| `FormHeader` / `FormFooter` | `@helios/ui/backend` | Proposal detail header |
| `Badge` | `@helios/ui/primitives` | Status and confidence badges |
...
```typescript
import { createModuleEvents } from '@helios/shared/modules/events/factory'
...
export const setup: ModuleSetupConfig = {
  ...
  async onTenantCreated({ em, tenantId, organizationId }) {
    ...
    if (!exists) {
      ...
      const domain = process.env.INBOX_OPS_DOMAIN || 'inbox.helios.local'
...
| `INBOX_OPS_DOMAIN` | Yes (if enabled) | | Domain for inbound email addresses (e.g., `inbox.helios.com`) |
...
> Pattern: `registerMcpTool` from `@helios/ai-assistant` (see `packages/search/src/modules/search/ai-tools.ts` for reference)

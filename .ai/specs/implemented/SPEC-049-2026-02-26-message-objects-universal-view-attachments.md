...
1. **Generic UI components** in `@helios/ui` — a reusable Preview + Detail pair that any module can use, eliminating the need to write custom widget code per module. The Detail component renders a `view` link and any additional module-defined actions using the same pattern as existing module-specific widgets.
...
All new definitions use `MessageObjectPreview` and `MessageObjectDetail` from `@helios/ui`.
...
- The generic components are new exports from `@helios/ui` — additive.
- `SendObjectMessageDialog` is already exported from `@helios/ui` — no API changes.

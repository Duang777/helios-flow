...
- Updates `migrations/.snapshot-helios.json`.
...
2. Declare new ACL feature `customers.deals.export` in `customers/acl.ts` (`customers.deals.manage` already exists). Add `customers.deals.export` to `employee` `defaultRoleFeatures` in `setup.ts`; `admin: ['customers.*']` already covers it. Run `yarn helios auth sync-role-acls`.
...
9. Generate the migration (`yarn db:generate`), confirm it covers only the customers column addition, update `migrations/.snapshot-helios.json`.
...
1. Add all i18n keys to `apps/helios/i18n/en.json` and `apps/helios/i18n/pl.json`.
...
5. Run `yarn helios configs cache structural --all-tenants` per AGENTS.md after enabling the new settings page and threshold config.
...
| `apps/helios/i18n/en.json` | Modify | New i18n keys |
| `apps/helios/i18n/pl.json` | Modify | New i18n keys |
...
| root AGENTS.md | Run `yarn helios configs cache structural --all-tenants` after sidebar/page additions | Compliant | Listed in Phase 5 |
...
- Known remaining 1:1 gaps (low severity, deferred to follow-up):
  ...
  - Page renders an Helios-convention H1 "Deals" that doesn't appear in the Figma kanban frame (Figma shows only the breadcrumb). Keeping the H1 for accessibility + sidebar/page-title consistency across the app.

...
**Key Points:**
- Remove every `import { ... } from '@helios/core/modules/staff/...'` in non-staff core modules so `packages/core` builds without `staff`. Two coupling sites on `upstream/develop`: [`customers/api/assignable-staff/route.ts`](../../packages/core/src/modules/customers/api/assignable-staff/route.ts) and [`planner/api/access.ts`](../../packages/core/src/modules/planner/api/access.ts).
...
- Staff stays at `packages/core/src/modules/staff/` after this spec — the physical extraction to `@helios/staff` in [official-modules](https://github.com/helios/official-modules) is a separate downstream spec.
...
**Out of Scope:**
- Dashboards/widgets coupling that only appears once PR [#1111](https://github.com/helios/helios/pull/1111) merges — covered by a follow-up spec written after #1111 lands.
- String nav references (`'staff.nav.group'` in `auth/api/admin/nav.ts`, `apps/helios/src/app/(backend)/backend/layout.tsx`, and the `create-app/template` equivalent) and test fixtures referencing `staff.*` features — deferred to the Phase 3 (delete-staff-from-core) spec because they do not break TypeScript compilation when staff is absent.
- Physical extraction of staff to `@helios/staff` (Phase 2, separate spec in the [official-modules](https://github.com/helios/official-modules) repo).
...
- **Phase 2 (separate spec, in the `official-modules` repo)** — create `@helios/staff`; copy code; verify in sandbox.
- **Phase 3 (separate spec, in this repo)** — delete `packages/core/src/modules/staff/`; wire the app to consume `@helios/staff` from npm; clean up out-of-scope string references and test fixtures.
...
```ts
import { StaffTeam, StaffTeamMember } from '@helios/core/modules/staff/data/entities'
```
...
**Why it blocks staff extraction**: removing `staff` from core leaves the import dangling and breaks the build of any app that does not install `@helios/staff`.
...
```ts
import { StaffTeamMember } from '@helios/core/modules/staff/data/entities'
```
...
This is intentional for Phase 1: removing the `requires: ['planner', ...]` declaration is a Phase 2/3 concern (it becomes a peer-dependency declaration in the future `@helios/staff` npm package, which then must be reconciled with the runtime fact that the planner-side staff lookup now lives inside staff). Phase 1.C Step 1 verifies the `requires` line still matches reality after the refactor.
...
| `apps/helios/src/app/(backend)/backend/layout.tsx` line 239 | same string |
...
|----------|--------|-----------|
| Decouple direction | Move logic into `staff`, not the other way | Once staff is external, core MUST NOT depend on it. Staff is allowed to depend on core (`@helios/core` becomes its peer dep). |
...
| Generic helper in `@helios/shared`? | Rejected | `packages/shared/AGENTS.md` MUST rule: shared MUST NOT add domain-specific logic. `resolveStaffMemberByUserId` is staff domain. |
...
| Generic `staff_member` table or interface in `@helios/shared` | Domain leak into shared. Defeats modularity. Violates `packages/shared/AGENTS.md` MUST rule "shared MUST NOT add domain-specific logic." |
...
| **Module developer** preparing `@helios/staff` for `official-modules` | Greps `packages/core/src/modules/` for `from '@helios/core/modules/staff'` excluding the staff folder itself | Returns zero matches |
...
| **App maintainer** disabling staff in `apps/helios/src/modules.ts` to test module isolation | Boots the app | App boot does not crash; planner availability writes return `403 staff_module_not_loaded`; customers UI's assignable-staff picker shows an empty list; no other module affected |
...
| `packages/core/src/modules/staff/di.ts` | **NEW** — register `availabilityAccessResolver` (asValue). The file does not exist on `upstream/develop`; staff currently has no `di.ts`. The new file MUST follow the same shape as `customers/di.ts` / `planner/di.ts`: `export function register(container: AppContainer)` typed via `import type { AppContainer } from '@helios/shared/lib/di/container'`. After creation, run `yarn generate` to verify the auto-discovery picks it up; if the generated module index does not include the new registrar, update the relevant generator plugin or the bootstrap wiring under `apps/helios/src/bootstrap.ts`. | staff |
...
Pattern matches the existing `customers/di.ts` and `planner/di.ts`. Type alias `AppContainer` (an `AwilixContainer` re-export) comes from `@helios/shared/lib/di/container`.
...
- States staff is optional and may be extracted to `@helios/staff` in a future release.
...
- Phase 3 (delete staff from core) only proceeds AFTER `@helios/staff` is published in `official-modules` AND verified in sandbox.
...
grep -rn 'customers/assignable-staff' packages/ apps/ \
  | grep -v node_modules | grep -v dist | grep -v '\.helios/generated'
```
...
**Step 2**: **Create** `packages/core/src/modules/staff/di.ts` (file does not exist on `upstream/develop`). Export `register(container: AppContainer)` that calls `container.register({ availabilityAccessResolver: asValue({ resolveAvailabilityWriteAccess }) })`. Type alias from `@helios/shared/lib/di/container`. Before writing the planner wrapper, **verify the installed Awilix version supports `{ allowUnregistered: true }`** (the option exists since Awilix 4.x). Check the resolved version at the root `package.json` and `packages/core/package.json`; if the version is below 4.0, the planner wrapper MUST fall back to a `try/catch` around `container.resolve('availabilityAccessResolver')` returning `undefined` on `AwilixResolutionError`. Run `yarn generate` after creating `di.ts`; the existing `di.generated.ts` aggregator auto-discovers module `di.ts` files (verified at `apps/helios/src/bootstrap.ts` import of `diRegistrars`), so no bootstrap edit is expected. Add an explicit smoke test in `staff/__integration__/` that asserts `container.hasRegistration('availabilityAccessResolver') === true` when staff is enabled.

**Step 3**: Modify [`packages/core/src/modules/planner/api/access.ts`](../../packages/core/src/modules/planner/api/access.ts):
- Delete `import { StaffTeamMember } from '@helios/core/modules/staff/data/entities'`.
- Delete `import { findOneWithDecryption } from '@helios/shared/lib/encryption/find'` (no longer used after the function moves).
...
Plus manual smoke: as a non-admin staff user, open My Availability — edit own (allowed), attempt to edit another member (denied). Re-disable `staff` in `apps/helios/src/modules.ts`, rebuild, repeat — expect `403 staff_module_not_loaded`. Re-enable.
...
```bash
grep -rn "@helios/core/modules/staff" packages/core/src/modules/ \
  | grep -v "/staff/" \
...
- Module status (optional; planned extraction to `@helios/staff`).
...
**Step 4**: Open PR against `upstream/develop` with a description that includes (a) the consumer inventory from Phase 1.A Step 2, (b) the decouple grep proof from Phase 1.C Step 1, (c) link to this spec, (d) link to Piotr's approval comment on PR #1111: <https://github.com/helios/helios/pull/1111#issuecomment-4354394013> (2026-04-30).
...
- **Severity**: Medium
- **Affected area**: Downstream apps, `docs.helios.com`
...
- **Mitigation**: Module registration in `apps/helios/src/bootstrap.ts` happens synchronously before the HTTP server accepts traffic. The race is impossible by construction. The new fail-soft test is a structural guard — if registration order ever changes, the test surfaces it as a false `staff_module_not_loaded`.
- **Residual risk**: None.
...
- **Mitigation**: Awilix `resolve` for `asValue` registrations is `O(1)` map lookup. Same SQL underneath. Benchmark before/after if `HELIOS_PROFILE=planner.*` shows >5% delta on the affected routes.
...
| Domain logic added to `@helios/shared` | Explicitly rejected in Alternatives |
...
| `packages/shared/AGENTS.md` | MUST NOT add domain-specific logic to `@helios/shared` | Compliant | All moved logic lives in `staff/lib/` and `staff/api/`; nothing added to shared |
...
- PR [#1111](https://github.com/helios/helios/pull/1111) — adds the dashboards coupling site (out of scope; follow-up spec).
...
- [official-modules](https://github.com/helios/official-modules) — destination repo for Phase 2.
...
- Spec created via the `/helios-spec-writing` skill workflow on `feat/spec-069-timesheets-phase-1` (PR target branch will be a fresh `feat/staff-decouple-from-core` off `upstream/develop`).
...
- Pre-implementation context: Piotr (pkarw) requested the decouple PR in a comment on PR #1111 dated 2026-04-30 (<https://github.com/helios/helios/pull/1111#issuecomment-4354394013>): *"can you guys propose the other PR with decouple fix to the core? I'd rather like to decouple first and not to keep the same module in official modules and the core at the same time please"*. The PR for this spec lands in parallel with the in-review timesheets PR #1111.
- Architectural Review pass (2026-05-08, post-skeleton refinement) applied:
  ...
  - **H4**: Synced DI code samples to use `AppContainer` from `@helios/shared/lib/di/container` instead of `AwilixContainer` directly, matching `customers/di.ts` and `planner/di.ts` style.

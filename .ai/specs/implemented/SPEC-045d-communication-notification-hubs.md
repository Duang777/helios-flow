...
Helios today has no bridge between external communication channels (WhatsApp, Slack, Email) and the Messages module's unified inbox. The `communication_channels` hub does not yet exist in shipping code — though several feasibility analyses (ANALYSIS-013 Gmail, ANALYSIS-045d WhatsApp) and provider-stub specs (SPEC-056) anticipate it. Without this hub:
...
Cross-module links use the canonical `EntityExtension` shape from `@helios/shared/modules/entities`:

```typescript
// communication_channels/data/extensions.ts

import type { EntityExtension } from '@helios/shared/modules/entities'
...
The `createModuleEvents({ moduleId, events })` object-arg shape is the canonical signature from `@helios/shared/modules/events`. Worked examples in shipping code: `packages/core/src/modules/messages/events.ts` (lines 1–12), `packages/core/src/modules/customers/events.ts`. The positional form `createModuleEvents('...', [...])` does not exist and does not compile.
...
import type { NotificationTypeDefinition } from '@helios/shared/modules/notifications'
...
├── package.json                        # @helios/channel-whatsapp
├── src/
│   └── modules/
│       └── channel_whatsapp/
│           ├── index.ts
...
├── package.json                        # @helios/channel-slack
├── src/
│   └── modules/
│       └── channel_slack/
│           ├── index.ts
...
├── package.json                        # @helios/channel-email
├── src/
│   └── modules/
│       └── channel_email/
│           ├── index.ts
...
├── package.json                        # @helios/notifier-sendgrid
├── src/
│   └── modules/
│       └── notifier_sendgrid/
│           ├── index.ts
...
- [x] `apps/helios/src/modules.ts` — enabled `{ id: 'communication_channels', from: '@helios/core' }`.
...
- [x] Acceptance gates passed:
  ...
  - `apps/helios#lint` failure is **pre-existing** (eslint-plugin-react version mismatch on `next.config.ts`, unrelated to this slice; "no eslint gate for core" per memory note)
...
- [x] **Acceptance gates:**
  ...
  - `yarn build:packages` — 19/19 packages built successfully (2,646 entry points in `@helios/core`).
...
- [x] `data/enrichers.ts` — 4 enrichers, all targeting `messages.message`, all feature-gated by `communication_channels.view`, all implementing `enrichMany` (no N+1):
  ...
  > **Update (#3183, 2026-06-20):** the three channel enrichments above (`_channel`, `_channelPayload`, `_channelContact`) each issued their own `MessageChannelLink` `$in` query — three identical lookups per page, plus a duplicated `ExternalConversation` query — because the enricher runner executes enrichers sequentially with no shared per-pass context. They are now produced by a single batched `communication_channels.message-channel` enricher that loads the link batch (and the conversation batch) once per pass. The exported `enrichers` array is now **2 entries** (`message-channel` + `message-reactions`); all public enriched field names are unchanged. `enrichers.test.ts` and `TC-045D-005a` were updated to the 2-enricher contract and assert a single `MessageChannelLink` query for a full enrichment pass.
...
- [x] **Module-local Playwright integration tests** (3 new files):
  - `TC-045D-005a` — enrichers module export shape (2 enrichers post-#3183: `message-channel` + `message-reactions`; canonical ids, target entity, feature gating).
...
- [x] **Acceptance gates:**
  ...
  - `yarn build:packages` — 19/19 packages built successfully (2,663 entry points in `@helios/core`).
...
- [x] **Acceptance gates:**
  ...
  - `yarn build:packages` — 19/19 packages built successfully (2,672 entry points in `@helios/core`).
...
| 2 | Type definitions & interfaces | New `ChannelAdapter`, `ChannelCapabilities`, `NormalizedInboundMessage`, `RefreshCredentialsInput`/Result, `ValidateCredentialsInput`/Result types exported from `@helios/core/modules/communication_channels/lib/adapter`. | Additive (new exports) |
| 3 | Function signatures | None modified | None |
| 4 | Import paths | New `@helios/core/modules/communication_channels/...` paths. | Additive |
...
| 10 | ACL feature IDs | 4 new features (`communication_channels.view/.manage/.react/.assign`). `defaultRoleFeatures` map in `setup.ts` grants them to admin + manager roles (full set) and to all user roles (`.view` + `.react`). Run `yarn helios auth sync-role-acls` after deploy. No existing feature renamed. | Additive |
...
| root AGENTS.md | OSS must not depend on enterprise | Compliant | No `@helios/enterprise` imports anywhere |
...
| packages/scheduler/* | `SchedulerService.register` for periodic ticks | Compliant | Outbound retry sweeper registered as cron via `@helios/scheduler` |

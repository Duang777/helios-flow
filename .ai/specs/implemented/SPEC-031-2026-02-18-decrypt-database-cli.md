...
- **Query index must be reindexed after decryption** — until `helios query_index reindex --tenant <uuid>` is run (after env flip + restart), search and filter results are degraded (query index docs still hold encrypted payloads)

---

## Overview

Helios encrypts sensitive entity fields at rest using AES-GCM via `TenantDataEncryptionService`. The encryption lifecycle is:

1. **Enable**: seed `EncryptionMap` records (`helios entities seed-encryption --tenant <uuid>`)
2. **Rotate keys**: re-encrypt under a new DEK (`helios entities rotate-encryption-key --tenant <uuid> --old-key <secret>`)
3. **Disable** *(this spec)*: decrypt all fields to plaintext and optionally deactivate maps (`helios entities decrypt-database --tenant <uuid> --confirm <tenantUuid>`)
...
```
helios entities decrypt-database
...
7. **Post-step reminder** printed after completion:
   ...
   ✅ Decryption complete. Required next steps:
      ...
      3. Run: helios query_index reindex --tenant <uuid>  ← run after env flip + restart; search/filter degraded until this completes
      4. Run: helios entities decrypt-database --tenant <uuid> --check  to confirm no encrypted values remain
...
- **Query index**: The `query_index` module stores encrypted `doc` columns. This command does **not** touch the query index. Operators must run `helios query_index reindex` **after** setting `TENANT_DATA_ENCRYPTION=false` and restarting replicas.
...
- **Mitigation**: Post-step instructions include `helios query_index reindex --tenant <uuid>` as step 3 (after env flip + restart)

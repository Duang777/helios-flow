...
- The permanent fix is two-part:
  1. add a generator watch pipeline so structural file changes regenerate `.helios/generated` during `yarn dev` without restart
...
- May 5, 2026 amendment: app `.env*` changes are runtime inputs, not structural inputs. `helios server dev` now watches app env files and restarts the managed Next.js + worker + scheduler group so changed secrets, provider settings, ports, and runtime toggles are picked up without manually restarting `yarn dev`.
...
- Implemented:
  - `helios generate watch` and wired it into app/template `yarn dev`
  - standalone `create-app` template now uses the same splash wrapper/template as the monorepo `yarn dev`, with the child runtime mirrored from `apps/helios/scripts/dev.mjs`
  ...
  - dev-only background route warmup added through `instrumentation.ts` with `HELIOS_DEV_WARM*` controls and backward-compatible `HELIOS_DEV_WARM*` aliases
...
- May 5, 2026 amendment:
  - `helios server dev` watches `.env`, `.env.development`, `.env.local`, and `.env.development.local` in the app directory
...
- May 13, 2026 amendment:
  ...
  - `HELIOS_PACKAGE_WATCH_MODE=persistent` restores the previous persistent esbuild-context tradeoff for developers who prefer faster rebuilds over lower idle RSS
...
| `apps/helios yarn dev` ready time | `982ms` |
...
- `apps/helios/.helios/next/dev/trace`
- `apps/helios/CPU.main.1775119537021.cpuprofile`

The generated registry is also materially large:

| Artifact | Size |
|----------|------|
| `.helios/generated/modules.generated.ts` | `656,371` bytes |
| `.helios/generated/modules.generated.ts` imports | `1,287` |
| `.helios/generated/modules.generated.ts` lines | `2,129` |
| compiled server chunk for `modules.generated.ts` | `~2.21 MB` |
| `.helios/next/dev` after a few cold hits | `~1.3 GB` |
...
3. starts `helios server dev`

Package watch processes rebuild `dist/`, but nothing watches module convention files and regenerates `.helios/generated` when a structural change happens. Adding a new `page.tsx`, `route.ts`, `setup.ts`, `translations.ts`, `events.ts`, or similar file still requires a manual `yarn generate` and a dev-server restart.
...
- `apps/helios/src/app/api/[...slug]/route.ts`
- `apps/helios/src/app/(frontend)/[...slug]/page.tsx`
- `apps/helios/src/app/(backend)/backend/[...slug]/page.tsx`

all import `@/.helios/generated/modules.generated` directly.
...
- write generated files atomically into `.helios/generated`
...
- A standalone app using published `@helios/*` packages gets the same lazy route loading benefits even though package code is resolved from `node_modules/@helios/*/dist/`.
...
Standalone:
- loaders point at compiled `node_modules/@helios/*/dist/modules/...` files
...
### `helios generate watch`
- Purpose: keep `.helios/generated` synchronized during `yarn dev`
- Request shape: CLI flags only
- Response: long-running process, logs changed outputs

### `helios generate changed` (optional helper)
...
| Setting | Purpose |
|---------|---------|
| `HELIOS_GENERATOR_WATCH=1` | Enable generator watch explicitly in dev orchestration if needed |
| `HELIOS_DEV_ROUTE_MANIFESTS=1` | Temporary rollout flag for lightweight manifests during migration |
| `HELIOS_DEV_PRELOAD_ENTRIES=0` | Optional experiment for `preloadEntriesOnStart` after route-manifest work lands |
...
2. Implement `helios generate watch` with resolver-based roots and change classification.
...
| File | Action | Purpose |
|------|--------|---------|
| `packages/cli/src/helios.ts` | Modify | Start generator watch during dev; add watch command |
...
| `apps/helios/src/app/api/[...slug]/route.ts` | Modify | Switch API catch-all to lazy manifest loading |
| `apps/helios/src/app/(backend)/backend/[...slug]/page.tsx` | Modify | Switch backend catch-all to lazy manifest loading |
| `apps/helios/src/app/(frontend)/[...slug]/page.tsx` | Modify | Switch frontend catch-all to lazy manifest loading |
| `apps/helios/src/bootstrap.ts` | Modify | Preserve legacy bootstrap wrapper while exposing partitioned bootstraps |
| `apps/helios/.helios/generated/*` | Generate | New route manifest and bootstrap partition artifacts |
...
- Integration: standalone app using compiled `@helios/*` packages
...
- **Scenario**: A structural change triggers only some generators, leaving `.helios/generated` in an inconsistent state.
- **Severity**: Medium
...
- **Mitigation**: Ignore `.helios/generated/**`, debounce events, and track the write set produced by the current generation cycle.
- **Residual risk**: Low.
...
- **Mitigation**: Watch app env files inside `helios server dev` and restart the full managed runtime group after a debounced change. Reload env files from disk before respawn and preserve shell-provided variables so explicit terminal exports still win.
- **Residual risk**: Low.
...
### 2026-05-05
- Added env-file reload behavior for `helios server dev`: app `.env*` changes now restart the full managed dev runtime group instead of relying on Next.js-only config reload behavior.

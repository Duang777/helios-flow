...
- **Status:** Implemented
- **Issue:** [#2653](https://github.com/helios/helios/issues/2653) — "bug: list of the team members (staff) - strip html - more broad issue with markdown vs html"
- **Date:** 2026-06-06
- **Scope:** OSS — shared UI (`@helios/ui`) + core modules (staff, catalog, planner, resources) + create-app template
...
- `src/types/css.d.ts` — ambient `declare module '*.css'` for that stylesheet import. A `/// <reference>` to it from `MdxEditorImpl.tsx` makes the ambient travel into every package that type-checks `@helios/ui` source (scheduler, webhooks, checkout, ai-assistant, …), so the cross-package typecheck resolves the import (otherwise TS2882).
...
**Styling:** `apps/helios/src/app/globals.css` + `packages/create-app/template/src/app/globals.css`
— the MDXEditor DS theming block (`.helios-mdx-editor` / `.helios-mdx-prose` / `.cm-*`). The editor's base
stylesheet is imported by the component (see above), not globally.

**Dependencies:** add `@mdxeditor/editor` to `@helios/ui`; remove `@uiw/react-md-editor`
...
- **Unit (existing, updated):** `CrudForm.*` suites, `CustomDataSection`, `lazy-heavy-libraries`,
  ...
  `MarkdownField` test stub. `yarn workspace @helios/ui test` (1503) and
  `yarn workspace @helios/core test` (5494) pass; `typecheck` (ui + core) green.

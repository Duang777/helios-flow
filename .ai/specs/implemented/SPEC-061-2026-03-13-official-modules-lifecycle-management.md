...
- Add module state tracking, compatibility checks, update discovery, safe upgrades, and diagnostics to the `helios` CLI
- Support both installed modes from SPEC-060:
  - package-backed modules (`from: '@helios/module-...'`)
  - ejected modules (`from: '@app'`)
- Treat ejected-module upgrades as assisted merges, not blind overwrites

**New CLI surface:**
- `yarn helios module search <query>`
- `yarn helios module info <name>`
- `yarn helios module outdated`
- `yarn helios module upgrade <name>`
- `yarn helios module doctor`
...
2. **No update path**
   Package-backed modules can technically be updated via package manager commands, but there is no Helios-aware compatibility check or workflow. Ejected modules have no upgrade story at all.
...
1. **Local module state file**
   Introduce a machine-managed file at app root: `helios.modules.json`.
...
2. **Lifecycle-aware CLI commands**
   Extend the CLI with discovery, diagnostics, and upgrade commands that understand Helios module conventions instead of delegating everything to raw `yarn add`.
...
State is inferred from `helios.modules.json`, `src/modules.ts`, local file existence, and optional file checksums.
...
5. refresh `helios.modules.json`
6. run `yarn helios generate`
...
```json
{
  ...
  "modules": [
    {
      "id": "n8n-integration",
      "packageName": "@helios/module-n8n-integration",
...
Marketplace modules continue using the SPEC-060 `helios` field, extended with optional lifecycle metadata:
...
```json
{
  "helios": {
    ...
    "upgradeNotesUrl": "https://github.com/helios/official-modules/tree/main/packages/n8n-integration/CHANGELOG.md"
  }
}
```
...
| Call | Purpose |
|------|---------|
| `GET /-/v1/search?text=scope:helios keywords:helios-module <query>` | `module search` and `module list` |
| `GET /@helios/module-<name>` | `module info`, `module add`, `module upgrade`, `module outdated` |

### CLI Commands

| Command | Description |
|---------|-------------|
| `yarn helios module search <query>` | Search official marketplace modules |
| `yarn helios module info <name>` | Show detailed module metadata and local install state |
| `yarn helios module outdated` | Compare installed modules against npm latest versions |
| `yarn helios module upgrade <name>` | Upgrade one installed marketplace module |
| `yarn helios module upgrade --all` | Upgrade all compatible package-backed modules; ejected modules remain review-gated |
| `yarn helios module doctor` | Validate local marketplace module state |
...
packages/cli/src/lib/module-state.ts       # read/write helios.modules.json
...
|---------|---------|---------|
| `HELIOS_NPM_REGISTRY` | `https://registry.npmjs.org` | npm registry override |
| `HELIOS_MODULE_UPGRADE_STRATEGY` | `safe` | Upgrade policy; `safe` blocks conflicting ejected upgrades |
| `HELIOS_MODULE_SEARCH_SIZE` | `100` | Max result size for npm search calls |
...
- `helios.modules.json` is additive and optional at first run
- If a module is enabled in `src/modules.ts` but missing from `helios.modules.json`, `module doctor` reports it as untracked and offers a repair path
...
1. Introduce `helios.modules.json` schema and read/write helpers
...
|------|------|--------|---------|
| `packages/cli/src/lib/module-state.ts` | helios | Create | `helios.modules.json` read/write helpers |
| `packages/cli/src/lib/module-registry.ts` | helios | Create | npm metadata fetch/parsing |
| `packages/cli/src/lib/module-doctor.ts` | helios | Create | diagnostics engine |
| `packages/cli/src/lib/module-upgrade.ts` | helios | Create | lifecycle-aware upgrades |
| `packages/cli/src/lib/module-search.ts` | helios | Create | search/list/info support |
| `packages/cli/src/helios.ts` | helios | Modify | route new `module:*` commands |
| `packages/cli/src/bin.ts` | helios | Modify | mark lifecycle commands bootstrap-free where required |
| `helios.modules.json` | app root | Create | local marketplace state |

### Testing Strategy

- Unit tests for npm metadata parsing and compatibility evaluation
- Unit tests for `helios.modules.json` reads/writes and recovery from corruption
...
The CLI now manages both `src/modules.ts` and `helios.modules.json`. If these diverge, users may get false outdated reports or broken upgrades.
...
- registry override via `HELIOS_NPM_REGISTRY`
...
- **Scenario**: `helios.modules.json` is edited manually or becomes stale after git operations.
- **Severity**: Medium
...
| Distinguishes runtime enablement vs lifecycle tracking | Pass | `src/modules.ts` and `helios.modules.json` have separate roles |
...
- [packages/cli/src/lib/eject.ts](/Users/dpalatynski/Private/helios-latest/packages/cli/src/lib/eject.ts)
- [packages/cli/src/lib/resolver.ts](/Users/dpalatynski/Private/helios-latest/packages/cli/src/lib/resolver.ts)
- [packages/cli/src/helios.ts](/Users/dpalatynski/Private/helios-latest/packages/cli/src/helios.ts)
- [packages/cli/src/bin.ts](/Users/dpalatynski/Private/helios-latest/packages/cli/src/bin.ts)
...
- Added app-local module state tracking via `helios.modules.json`

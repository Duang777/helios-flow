...
- **Sidebar width tokenization.** The widths `'240px'` and `'80px'` remain hardcoded in `AppShell.tsx` (`expandedSidebarWidth`, the new section aside, and the grid-template-columns literals). `apps/helios/src/app/globals.css` already defines `--sidebar-*` color tokens but no width tokens. Centralizing as `--sidebar-width` / `--sidebar-collapsed-width` would simplify future density / responsive work, but is pre-existing tech debt — this spec doesn't add new hardcoded literals beyond what was already there for the same value, and a width-token sweep is its own change.

...
- Ready apps are **not** part of the Helios core repository. Each official ready app lives in its **own GitHub repository** under the `helios` organization, using the naming convention `ready-app-<appname>`.
- Bootstrap via `create-helios-app --app <name>` for official Helios ready apps and `create-helios-app --app-url <github-url>` for external GitHub-hosted ready apps.
...
- `--app` and `--app-url` flags for `create-helios-app`.
...
- Ensure extensions built for ready apps fully leverage the Universal Helios Extension System (UMES).

## Overview
Helios needs productized "ready projects" that reduce time-to-first-value for common B2B use cases such as PRM, field service, or marketplace ops. Today, teams start from a generic tenant and manually assemble modules, dictionaries, workflows, and role settings.
...
> **Market Reference:** Inspired by source-first scaffolders in the JavaScript ecosystem, but adapted to Helios's ecosystem and ownership model. Adopted: `--app`, `--app-url`, GitHub tarball fetch, and one official ready app per repository. Rejected: centralized official catalog repo, npm-only distribution, manual clone-and-copy workflow.
...
1. **The Core Engine**: `helios/core` and `create-helios-app` remain agnostic and clean.
2. **Official Ready Apps**: Maintained by the Helios team in **separate GitHub repositories** under the `helios` organization, one repository per app, named `ready-app-<appname>`.
3. **External Ready Apps**: Maintained by partners/agencies in their own GitHub repositories.
4. **The Bootstrap Flow**: `create-helios-app --app <name>` fetches an official Helios repository; `create-helios-app --app-url <url>` fetches an external GitHub repository and scaffolds a ready-to-run app.
...
| `--app` flag on `create-helios-app` | Single-command bootstrap for official Helios maintained ready apps. |
| `--app-url` flag on `create-helios-app` | Explicit path for external GitHub-hosted ready apps without overloading one flag. |
...
|-------------|-------------|
| Centralized official repo such as `helios/ready-apps` | Couples unrelated apps into one release surface and one CI pipeline; ownership is less clear. |
...
- No ready app configurations committed to the `helios/helios` core repository.
...
- An Engineer wants to bootstrap a new B2B PRM application. They run `npx create-helios-app my-prm --app prm` and get a complete, demo-ready PRM app from `helios/ready-app-prm`.
- An Engineer wants to use an external ready app. They run `npx create-helios-app my-app --app-url https://github.com/some-agency/ready-app-marketplace`.
- A Partner Agency wants to distribute their specialized marketplace workflows. They maintain a GitHub repository with a complete Helios app that includes their UMES extensions, widgets, and seeds.
- An Engineer wants a blank app with no ready app. They run `npx create-helios-app my-app` (unchanged behavior).

## Architecture

### CLI Interface

New flags for `create-helios-app`: `--app` and `--app-url`

```bash
# Official Helios ready app
npx create-helios-app my-prm --app prm

# External GitHub-hosted ready app
npx create-helios-app my-app --app-url https://github.com/some-agency/ready-app-marketplace
...
Resolution logic:
- `--app <name>` resolves to the GitHub repository `helios/ready-app-<name>`
...
Official ready apps live as separate repositories in the `helios` organization:

```text
helios/ready-app-prm
helios/ready-app-quotes
helios/ready-app-field-service
```
...
- Official: `GET https://api.github.com/repos/helios/ready-app-<name>/tarball/<ref>`
- External: `GET https://api.github.com/repos/{owner}/{repo}/tarball/<ref>`
- No git dependency required

Ref resolution:
- `--app <name>` MUST resolve the official repository ref to the exact tag `v<create-helios-app version>`
...
### Reference Flow
```text
developer runs `npx create-helios-app my-prm-app --app prm` ->
create-helios-app resolves `helios/ready-app-prm` + tag `v<create-helios-app version>` ->
...
### Error Handling
- Official app repo not found: clear error including the resolved repository name (`helios/ready-app-<name>`)
...
3. The Helios platform provides the extension points (hooks, enrichers, registries), but not the business configuration for ready apps.
...
Not applicable. No application HTTP APIs are introduced. The external contract is the `create-helios-app` CLI surface:
- `--app <name>`
- `--app-url <github-url>`
...
1. **CLI version**: the version of `create-helios-app`
...
- `create-helios-app` and official `@helios/*` packages MUST ship on the same version line
- Official ready app repositories MUST publish a matching git tag for every supported Helios release using the format `v<version>`
- `--app` MUST fetch the official ready app tag that matches the running `create-helios-app` version exactly
...
- Official ready apps MUST commit their `@helios/*` dependency versions directly in `package.json`
...
- External maintainers SHOULD document the tested Helios compatibility range in their README

## Migration & Compatibility
- Official ready apps define compatibility through the pair `(repo tag, committed dependency graph)`, aligned to the `create-helios-app` release line.
...
- Backward compatible: `create-helios-app` without `--app` or `--app-url` continues to work exactly as before.

## Implementation Plan

### Phase 1 - CLI Flags
1. Add `--app` and `--app-url` to the `create-helios-app` argument parser.
2. Enforce mutual exclusivity between `--app` and `--app-url`.
3. Implement official app name resolution from `<name>` to `helios/ready-app-<name>`.
...
1. Create the first official repository, such as `helios/ready-app-prm`.
...
|------|------|--------|---------|
| `packages/create-app/src/index.ts` | helios | Modify | Add `--app` / `--app-url`, repo resolution, fetch logic |
| `packages/create-app/AGENTS.md` | helios | Modify | Document `--app` / `--app-url` behavior |
| repo root | `helios/ready-app-prm` | Create | First official ready app |
| `README.md` | `helios/ready-app-prm` | Create | App usage, compatibility, and bootstrap docs |
| `.github/workflows/ci.yml` | `helios/ready-app-prm` | Create | Validate the ready app builds |
...
- Integration: end-to-end `create-helios-app --app` with an official fixture repo tagged to the current CLI version
- Integration: end-to-end `create-helios-app --app-url` with an external GitHub fixture repo copied as-is
...
- **Severity**: Medium
- **Affected area**: `create-helios-app --app` / `--app-url` bootstrap
...
- **Mitigation**: Maintain a documentation index on docs.helios.com or in the main documentation repo; keep repo naming deterministic.
- **Residual risk**: Low
...
|-------------|------|--------|-------|
| root AGENTS.md | Ready apps live outside core repository | Compliant | Official apps live in separate `helios/ready-app-*` repositories, not in core |
...
| Check | Status | Notes |
|-------|--------|-------|
| CLI interface matches fetch mechanism | Pass | `--app` resolves to `helios/ready-app-<name>` and `--app-url` resolves to a direct GitHub repo URL |
...
### 2026-04-01
- Reworked the distribution model from one centralized official repo to one official repo per ready app under the `helios` organization.
- Introduced the official repository naming convention `ready-app-<appname>`.
- Replaced the earlier bootstrap contract with `--app` for official Helios apps and `--app-url` for external GitHub-hosted apps.
...
### 2026-03-20
- Official app catalog repository changed from an earlier naming variant to `helios/ready-apps` (superseded by the 2026-04-01 one-repo-per-app decision).
...
- Added `helios/ready-apps` as a centralized catalog repository, later superseded by the 2026-04-01 one-repo-per-app decision.
...
| Phase 2 - Official Ready App Repositories | Not Started | — | Requires work in external repositories such as `helios/ready-app-prm`, which are outside this monorepo |

### Phase 1 - Detailed Progress
- [x] Step 1: Add `--app` and `--app-url` to the `create-helios-app` argument parser
- [x] Step 2: Enforce mutual exclusivity between `--app` and `--app-url`
- [x] Step 3: Implement official app name resolution from `<name>` to `helios/ready-app-<name>`
...
- [ ] Step 1: Create the first official repository, such as `helios/ready-app-prm`

...
## TLDR
- Add `HELIOS_AUTO_SPAWN_SCHEDULER_LAZY=true` as an additive local scheduler mode.
- Keep `AUTO_SPAWN_SCHEDULER` and direct `helios server dev/start` defaults backward compatible: eager scheduler startup remains the CLI default.
- In lazy mode, the server starts a lightweight supervisor that probes for enabled rows in `scheduled_jobs` and only then launches the existing `helios scheduler start` process.
...
## Solution
Introduce a lazy local scheduler supervisor in `@helios/cli`:
...
```bash
node <heliosBin> scheduler start
```
...
- Existing `helios scheduler start` remains unchanged.
- Existing `helios server dev` and `helios server start` remain eager unless `HELIOS_AUTO_SPAWN_SCHEDULER_LAZY=true` is set.
...
| `HELIOS_AUTO_SPAWN_SCHEDULER` | unset | New alias when legacy env is unset. |
| `HELIOS_AUTO_SPAWN_SCHEDULER_LAZY` | `false` in CLI | Enables lazy local scheduler startup. |
| `HELIOS_AUTO_SPAWN_SCHEDULER_LAZY_POLL_MS` | `1000` | Lazy probe interval, clamped to at least `250`. |
| `HELIOS_AUTO_SPAWN_SCHEDULER_LAZY_RESTART` | `true` | Restart scheduler after unexpected exit if schedules still exist. |

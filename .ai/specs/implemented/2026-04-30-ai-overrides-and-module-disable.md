...
Today the AI agent registry and the AI tool registry are append-only. A downstream module (e.g. an app-level `@app` module, or a third-party package shipped on top of `@helios/core`) cannot:
...
The app's `src/bootstrap.ts` calls `applyAiOverridesFromEnabledModules(enabledModules)` once at boot. `apps/helios/src/bootstrap.ts` and the `create-helios-app` template ship that wiring out of the box.
...
| `apps/helios/src/modules.ts` | `ModuleEntry` carries optional `aiAgentOverrides` / `aiToolOverrides` |
| `apps/helios/src/bootstrap.ts` | Calls `applyAiOverridesFromEnabledModules(enabledModules)` |
...
| `.ai/skills/helios-create-ai-agent/SKILL.md` | Step on overriding existing agents updated |

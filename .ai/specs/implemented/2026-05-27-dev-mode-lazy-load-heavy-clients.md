...
|---------|--------------------------------------|-----------|----------------|
| `recharts` | `packages/ui/src/backend/charts/{BarChart,LineChart,PieChart}.tsx` plus 11 dashboard widget files via barrel | ~600 KB minified; large render tree for `ResponsiveContainer` + axis/legend chain | Every backend route via `@helios/ui/backend/charts` re-export |
...
| `ClientBootstrap` registry barrels | `apps/helios/src/components/ClientBootstrap.tsx` static imports of 5 `.generated.ts` barrels | Each barrel pulls every module's widget tree into `AppProviders` | Wraps **every** route (auth, portal, backend) |
...
| `@xyflow/react/dist/style.css` | `apps/helios/src/app/globals.css:4` | Global stylesheet — every page paid the parse cost | Every route via Next.js global CSS |
...
No public API change. Existing callers (`@helios/ui/backend/charts` barrel) still import `BarChart` / `LineChart` / `PieChart` the same way.
...
- `apps/helios/src/app/globals.css` — drop the `@xyflow/react/dist/style.css` global import; the stylesheet now ships with the lazy chunk.
...
- **No contract surface is touched.** Public exports from `@helios/ui/backend/charts` and `WorkflowGraph` keep their existing names, props, and runtime behaviour.
...
6. If the library has a heavy barrel of named exports (icons, locales, components), add it to `experimental.optimizePackageImports` in `apps/helios/next.config.ts`.

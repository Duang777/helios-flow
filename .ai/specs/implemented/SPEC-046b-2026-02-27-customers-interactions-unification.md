...
## Configuration
- Feature flag: `customers.interactions.unified` (default off until operators complete `helios customers interactions:backfill`; older underscore alias `customers_interactions_unified` remains readable during transition).
...
## Changelog
### 2026-03-19
- Clarified rollout behavior: dotted feature flags are canonical, underscore aliases remain readable for transition safety, and `customers.interactions.unified` stays off by default until `helios customers interactions:backfill` is completed per tenant.

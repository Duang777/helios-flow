...
- Generated worker handlers are already imported lazily, but `AUTO_SPAWN_WORKERS=true` still eagerly starts one long-lived `helios queue worker --all` process that creates a runner for every discovered queue.
- Add an opt-in lazy auto-spawn mode controlled by `HELIOS_AUTO_SPAWN_WORKERS_LAZY=true`, aligned with the existing `AUTO_SPAWN_WORKERS` switch. In this mode the server starts a lightweight supervisor, and each queue's long-lived worker starts only when the first job is enqueued for that queue.
...
## Overview
Helios's unified app entrypoints currently auto-start the app server, queue workers, and the local scheduler. The queue worker path is convenient, but it is expensive in development and small deployments because `helios queue worker --all` starts processing infrastructure for every discovered queue even when most queues are idle.
...
Repository inspection shows that this metric is produced by `apps/helios/scripts/dev.mjs` using a process-tree RSS walk rooted at the spawned app runtime process. The runtime process then spawns the auto worker process from `packages/cli/src/helios.ts`, so worker RSS is included in the reported memory.

**Market Reference:** BullMQ, Sidekiq, and Celery all treat workers as separately scalable processes. The proposed design keeps Helios's unified local developer experience but borrows the operational principle that worker processes should be demand-driven or independently managed, not eagerly attached to every app runtime by default.
...
### Memory Accounting
- `apps/helios/scripts/dev.mjs` calls `startMemoryMonitor(child)` only for the `App runtime` child.
- `getProcessTreeMemoryBytes(rootPid)` runs `ps -axo pid=,ppid=,rss=` and sums RSS for the root process plus descendants.
- `packages/cli/src/helios.ts` spawns the queue worker process as a child of the server runtime when `AUTO_SPAWN_WORKERS !== 'false'`.
...
```bash
node <heliosBin> queue worker --all
```
...
```bash
HELIOS_AUTO_SPAWN_WORKERS_LAZY=true yarn dev
```

When both `AUTO_SPAWN_WORKERS` and `HELIOS_AUTO_SPAWN_WORKERS_LAZY` are true:
...
| Decision | Rationale |
|----------|-----------|
| Add `HELIOS_AUTO_SPAWN_WORKERS_LAZY` instead of changing `AUTO_SPAWN_WORKERS` semantics | Eager auto-spawn remains backward-compatible. Lazy mode is explicit and easy to disable while stabilizing. |
...
| Add queue readiness probing APIs rather than ad hoc file/Redis access in the CLI | Queue strategy details stay inside `@helios/queue`. |
...
|-----------|--------------|----------------|
| Dev memory monitor | `apps/helios/scripts/dev.mjs` | Continue reporting process-tree RSS. Add optional detail that background children are included. |
| Server runtime | `packages/cli/src/helios.ts` | Resolve env mode and start eager workers, lazy supervisor, or no workers. |
...
| Queue pending probe | `@helios/queue` | Strategy-aware lightweight pending-job detection. |
| Worker runner | `@helios/queue/worker` | Existing worker execution path, unchanged for module authors. |
...
#### Lazy Mode (New)
```text
yarn dev with HELIOS_AUTO_SPAWN_WORKERS_LAZY=true
  -> helios server dev
    -> next dev
    -> helios queue supervisor
      -> watch queue A pending count
      -> watch queue B pending count
      -> watch queue C pending count
      -> first job appears on queue B
        -> node helios queue worker queue-B
```

### Queue Strategy Behavior
#### Local Strategy
- Queue files remain under `.helios/queue/<queue>/queue.json`.
...
- Poll interval defaults to a conservative value, for example `HELIOS_AUTO_SPAWN_WORKERS_LAZY_POLL_MS=1000`.
...
- Future work may add `HELIOS_AUTO_SPAWN_SCHEDULER_LAZY`, but this spec does not require it.
...
| `HELIOS_AUTO_SPAWN_WORKERS` | unset | New alias | Optional Helios-prefixed alias for `AUTO_SPAWN_WORKERS`; if both are set, legacy `AUTO_SPAWN_WORKERS` wins for backward compatibility. |
| `HELIOS_AUTO_SPAWN_WORKERS_LAZY` | `false` | New | Enables lazy auto-spawn when worker auto-spawn is otherwise enabled. |
| `HELIOS_AUTO_SPAWN_WORKERS_LAZY_POLL_MS` | `1000` | New | Supervisor pending-job probe interval. Must clamp to a safe minimum such as `250`. |
| `HELIOS_AUTO_SPAWN_WORKERS_LAZY_RESTART` | `true` | New | Restart a per-queue worker if it exits unexpectedly while jobs are still pending. |
| `QUEUE_STRATEGY` | `local` | Existing | Queue implementation: `local` or `async`. |

### Env Precedence
1. `AUTO_SPAWN_WORKERS=false` disables workers completely.
2. If `AUTO_SPAWN_WORKERS` is unset, `HELIOS_AUTO_SPAWN_WORKERS=false` disables workers.
3. If workers are enabled and `HELIOS_AUTO_SPAWN_WORKERS_LAZY=true`, start the lazy supervisor.
4. Otherwise keep current eager `queue worker --all` behavior.

### Naming Rationale
The user requested an `HELIOS_AUTO...` variable aligned with the existing variable for starting queues. `HELIOS_AUTO_SPAWN_WORKERS_LAZY` preserves the recognizable `AUTO_SPAWN_WORKERS` phrase and adds the `HELIOS_` namespace plus the specific lazy behavior.

## API Contracts
No HTTP API routes are introduced.

### Internal Queue Package Contract
Add an exported helper in `@helios/queue`, for example:
...
- `helios server dev` and `helios server start` choose between eager and lazy worker auto-spawn.
- Optional explicit command for diagnostics:

```bash
yarn helios queue supervisor
```
...
- Documentation should recommend:
  ...
  - `HELIOS_AUTO_SPAWN_WORKERS_LAZY=true` for memory-sensitive development and small unified deployments.
...
3. Update `apps/helios/scripts/dev.mjs` splash/detail copy if needed to say background services are included in memory.
...
2. Support `AUTO_SPAWN_WORKERS`, `HELIOS_AUTO_SPAWN_WORKERS`, and `HELIOS_AUTO_SPAWN_WORKERS_LAZY`.
3. Use `parseBooleanToken` / `parseBooleanWithDefault` from `@helios/shared/lib/boolean`.
...
4. Spawn `node <heliosBin> queue worker <queueName>` on first ready pending job.
...
4. Run focused test suites:
   ...
   - targeted dev runtime log tests under `apps/helios/scripts/__tests__`
5. Manual QA:
   - `HELIOS_AUTO_SPAWN_WORKERS_LAZY=true yarn dev`
...
| `packages/cli/src/helios.ts` | Modify | Wire lazy mode into `server dev` and `server start`. |
...
| `apps/helios/scripts/dev.mjs` | Modify | Optional memory copy/log classification for lazy supervisor. |
...
| `packages/cli/src/__tests__/helios.test.ts` | Modify | Cover mode wiring and spawn behavior. |
...
- Env resolution:
  ...
  - `HELIOS_AUTO_SPAWN_WORKERS=false` with legacy unset => `off`.
  - `AUTO_SPAWN_WORKERS=true` plus `HELIOS_AUTO_SPAWN_WORKERS_LAZY=true` => `lazy`.
  - `AUTO_SPAWN_WORKERS=false` plus `HELIOS_AUTO_SPAWN_WORKERS_LAZY=true` => `off`.
- Supervisor:
...
### Manual Verification
- Start idle app with `HELIOS_AUTO_SPAWN_WORKERS_LAZY=true`.
...
- **Mitigation**: `HELIOS_AUTO_SPAWN_WORKERS_LAZY_RESTART=true` restarts only if pending jobs remain; unexpected exits are logged with queue name.
...
- **Mitigation**: Lazy mode defaults to false and requires `HELIOS_AUTO_SPAWN_WORKERS_LAZY=true`.
...
|-------------|------|--------|-------|
| root AGENTS.md | Use `@helios/queue`; never custom queues | Compliant | Supervisor uses queue package probes and existing worker commands. |
...
| Env names match existing controls | Pass | `HELIOS_AUTO_SPAWN_WORKERS_LAZY` aligns with `AUTO_SPAWN_WORKERS`. |
...
- Implementation landed: `packages/cli/src/lib/auto-spawn-workers.ts` (env resolver), `packages/queue/src/pending-probe.ts` (read-only local + async probes), `packages/cli/src/lib/queue-worker-supervisor.ts` (per-queue child supervisor), and wiring in `server dev` / `server start` (`packages/cli/src/helios.ts`). Lazy mode keys `HELIOS_AUTO_SPAWN_WORKERS_LAZY`, `HELIOS_AUTO_SPAWN_WORKERS_LAZY_POLL_MS`, `HELIOS_AUTO_SPAWN_WORKERS_LAZY_RESTART`, and the `HELIOS_AUTO_SPAWN_WORKERS` alias resolve as documented; eager and `AUTO_SPAWN_WORKERS=false` paths are unchanged. Docs updated in `apps/docs/docs/framework/events/queue-workers.mdx` and `apps/docs/docs/appendix/troubleshooting.mdx`.
- May 13, 2026 update: the monorepo root `yarn dev` wrapper now injects `HELIOS_AUTO_SPAWN_WORKERS_LAZY=true` when the variable is unset, so local monorepo development gets the memory-saving supervisor by default. The CLI resolver and direct `helios server dev` / `helios server start` defaults remain unchanged for backward compatibility, and `HELIOS_AUTO_SPAWN_WORKERS_LAZY=false yarn dev` forces the historical eager worker mode.

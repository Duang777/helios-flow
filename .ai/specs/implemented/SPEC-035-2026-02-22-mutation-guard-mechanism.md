...
## TLDR
A generic mutation guard contract is now part of `@helios/shared` and is executed by the CRUD factory and selected custom mutation routes before and after write operations. The mechanism is neutral (not record-lock specific) and allows any package to enforce cross-cutting mutation rules through DI via `crudMutationGuardService`.

## Overview
Helios needed a reusable way to pre-validate writes (for example lock checks, workflow guards, future policy checks) without hard-coding domain-specific behavior into shared CRUD internals.
...
- Scenario: Future changes re-introduce domain-specific logic into `@helios/shared`.
- Severity: High

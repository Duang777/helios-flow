...
## TLDR
- Add an additive AI runtime on top of the existing `@helios/ai-assistant` package so modules can expose focused sub-agents and embeddable chat without breaking current MCP, OpenCode, or Command Palette behavior.
...
## Overview
Helios already has three useful AI building blocks:

1. MCP-compatible tool contracts (`ai-tools.ts`, `McpToolDefinition`, `registerMcpTool()`).
2. An authenticated in-process client and MCP-to-AI-SDK adapter inside `@helios/ai-assistant`.
...
- a Vercel AI SDK-friendly path for teams that want to treat an Helios agent as a normal AI SDK primitive instead of a bespoke runtime
...
- The current public surface is not AI SDK-first. A Vercel AI SDK user should be able to wire Helios agents into `useChat`, transport-based chat, or structured-output flows without reverse-engineering internal adapters.
...
- Introducing a new top-level package such as `@helios/ai`.
...
The new runtime lives inside `@helios/ai-assistant`. New reusable React UI lives in `@helios/ui`.
...
### D5. Auth Model
The new HTTP layer uses standard Helios auth:
...
- **Shared model factory**: consolidate provider/model resolution from `inbox_ops/lib/llmProvider.ts` into a reusable utility in `@helios/ai-assistant` when the second agent is built. Support `defaultModel` override plus optional env-based per-agent override (`<MODULE>_AI_MODEL`).
...
- expose a server-side helper that resolves an Helios agent into AI SDK-compatible tools plus instructions/context for direct `generateText()` / `streamText()` usage
- expose a structured-output helper so teams can run a focused Helios agent as a normal object-producing call using AI SDK `output` instead of re-implementing the agent contract manually
...
Attachments stay stored as Helios records, but the runtime must convert them into model-safe payloads before invocation.
...
- authenticates with standard Helios auth
...
Expose additive helpers so Helios agents feel native in Vercel AI SDK codebases.
...
```md
[ROLE]
You are the Helios workspace assistant. Help the user find information, explain records, summarize files, and suggest next actions using only the tools and data currently available.
...
import { defineAiAgent } from '@helios/ai-assistant'
...
- emit `ai-agents.generated.ts` in `apps/helios/.helios/generated/`
...
- shared model factory utility extracted from `inbox_ops/lib/llmProvider.ts` into `@helios/ai-assistant`, supporting `defaultModel` override and optional env-based per-agent override (`<MODULE>_AI_MODEL`)
...
- four new UI parts in `@helios/ui/src/ai/parts/`
...
- "Fix with AI" button on the failure card dispatches `helios-ai-chat-fix-request` consumed by `<AiChat>`; the prompt forbids the model from repeating identical args.
...
- Move the registration of in-tree custom UI parts (`catalog.stats-card` today, more later) from per-page client side-effect imports to a discoverable convention (`<module>/ai-ui-parts.client.ts` or similar) the generator wires into `apps/helios/.helios/generated/ai-ui-parts.generated.ts`. Avoids the current "must visit /backend/catalog/products once before the launcher can render the card globally" trap.
...
### Phase 0 - Alignment Prerequisite
1. Add `AiAgentDefinition` type and export it from `@helios/ai-assistant`.
...
1. Extract shared model factory from `inbox_ops/lib/llmProvider.ts` into `@helios/ai-assistant/lib/model-factory.ts`. Support `defaultModel` override and env-based per-agent override (`<MODULE>_AI_MODEL`).
...
4. Add the four new UI parts in `@helios/ui/src/ai/parts/`: `mutation-preview-card`, `field-diff-card`, `confirmation-card`, `mutation-result-card`. Wire keyboard shortcuts (`Cmd/Ctrl+Enter` confirms, `Escape` cancels) and reconnect behavior through `GET /api/ai/actions/:id`.
...
- shared model factory (`@helios/ai-assistant/lib/model-factory.ts`, Phase 3)
...
- Added AI SDK-first adapter requirements so Helios agents can be used through chat transports and structured-output helper flows without custom glue code.

...
| Related Issues | — |
| Related PRs | [#1567](https://github.com/helios/helios/pull/1567) — messages topbar icon partial fix |
...
| Cache staleness after `modules.ts` change | Low | Dev loop | `yarn generate` + `yarn helios configs cache structural --all-tenants` already required per root `AGENTS.md` | Low |
...
- `yarn generate` after any change that touches `modules.ts` or module `acl.ts` in the scenario setup, plus `yarn helios configs cache structural --all-tenants` per root `AGENTS.md`.

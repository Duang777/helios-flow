# Helios Develop Snapshot Release

**Date:** 2026-03-21
**Status:** Draft
**Scope:** OSS — `helios/helios`
**Author:** Helios Team
...
**Key Points:**
- Every merge or direct push to `develop` MUST publish a lockstep snapshot of all public Helios packages.
...
4. downstream repositories can depend on `@helios/*@develop`
...
- **Official modules maintainer** wants to install `@helios/core@develop` without searching for a specific canary version.
...
```bash
yarn add @helios/core@0.4.9-develop.1523.a1b2c3d4
```
...
2. scaffold a standalone app from the exact published `create-helios-app` snapshot version
...
- standalone scaffold via `create-helios-app@<snapshot-version>`
...
- downstream repos MAY switch to `@helios/*@develop` without needing exact snapshot versions
...
3. Document that downstream repositories should use `@helios/*@develop` for unreleased integration work.
...
- Validate that `npm view @helios/core@develop version` resolves to the newest develop snapshot
...
| Phase 3 — Validation And Documentation | Done | 2026-03-21 | Standalone validation now waits for every public package, including `create-helios-app`, and docs now describe the `@develop` channel |

...
| **Created** | 2026-02-24 |
| **Issue** | [#675](https://github.com/helios/helios/issues/675) |
| **Related** | [PR #635 — Record Locking](https://github.com/helios/helios/pull/635), SPEC-035 (Mutation Guard), SPEC-036 (Request Lifecycle Events), SPEC-043 (Reactive Notification Handlers) |
...
Helios has **five separate extension mechanisms** that evolved independently:
...
| Mutation Guards | Write operations (block/modify saves) | `@helios/shared/lib/crud/mutation-guard.ts` |
...
| 8 | **Coherence over Duplication** — Integrate with existing systems, don't duplicate | Helios architecture |
| 9 | **Progressive Disclosure** — Simple cases stay simple; advanced cases are possible | Existing widget injection |
| 10 | **Type Safety** — All contracts fully typed via TypeScript generics and Zod | Helios convention |
...
`yarn generate` discovers new files and generates registries into `apps/helios/.helios/generated/`:
...
| Mutation guard types | `import type { MutationGuard } from '@helios/shared/lib/crud/mutation-guard-registry'` |
| Sync event subscriber types | `import type { SyncCrudEventPayload, SyncCrudEventResult } from '@helios/shared/lib/crud/sync-event-types'` |
| API interceptor types | `import type { ApiInterceptor } from '@helios/shared/lib/crud/api-interceptor'` |
| Command interceptor types | `import type { CommandInterceptor } from '@helios/shared/lib/commands/command-interceptor'` |
| Injection position enum | `import { InjectionPosition } from '@helios/shared/modules/widgets/injection-position'` |
| App event hook | `import { useAppEvent } from '@helios/ui/backend/injection/useAppEvent'` |
| Extensible detail hook | `import { useExtensibleDetail } from '@helios/ui/backend/injection/useExtensibleDetail'` |
| Injected menu items hook | `import { useInjectedMenuItems } from '@helios/ui/backend/injection/useInjectedMenuItems'` |
...
1. **Runtime Architecture**: Widget injection uses `globalThis` keys for HMR (`__heliosCoreInjectionWidgetEntries__`, `__heliosCoreInjectionTables__`), lazy loading via `entry.loader()`, wildcard matching via regex, priority sorting `(b.priority ?? 0) - (a.priority ?? 0)`
...
10. **Generated Files**: `apps/helios/.helios/generated/` — `injection-widgets.generated.ts` and `injection-tables.generated.ts` use `loader: () => import(...)` for lazy loading

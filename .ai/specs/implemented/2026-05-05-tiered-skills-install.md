...
3. **One-shot skills tax every session.** `helios-auto-upgrade-0.4.10-to-0.5.0` and `helios-migrate-mikro-orm` are version-pinned migrations that 99% of sessions never need. `helios-dev-container-maintenance` only matters when editing `.devcontainer/`. They currently consume budget on every session.
...
- Zero risk of breaking external references to skill paths (the Task Router in root `AGENTS.md` cites concrete paths like `.ai/skills/helios-auto-create-pr/SKILL.md`).
  ...
  "tiers": {
    ...
    "automation": {
      ...
      "skills": [
        ...
        "helios-auto-fix-github",
        "helios-review-prs",
        "helios-merge-buddy",
        "helios-sync-merged-pr-issues",
        "helios-auto-update-changelog",
        "helios-auto-qa-scenarios"
      ]
    },
    "security": {
      "description": "Security audit skills. Opt-in.",
      "skills": ["helios-auto-sec-report", "helios-auto-sec-report-pr"]
    },
    "migration": {
      "description": "One-shot, version-pinned migrations. Install only when needed.",
      "skills": ["helios-auto-upgrade-0.4.10-to-0.5.0", "helios-migrate-mikro-orm"]
    },
    "infra": {
      "description": "Rare, special-case skills.",
      "skills": ["helios-dev-container-maintenance", "helios-integration-builder"]
    }
  }
}
```
...
| R5 | Skill auto-trigger silently fails when user expected it (e.g., `helios-auto-update-changelog` not in core) | Medium | UX | `--list` is discoverable; README's Available Skills table groups by tier. Skill names that hint at automation (the `auto-` prefix) are all in the `automation` tier so the heuristic is teachable. | Education problem; reduces over time. |
...
1. ~~**Should `helios-auto-create-pr` / `helios-auto-continue-pr` live in `core` instead of `automation`?**~~ **Resolved 2026-05-05: stay in `automation`.** Keeping the `auto-*` family together is teachable; users who want them on default install can run `yarn install-skills --with automation` once.

2. **Should `helios-migrate-mikro-orm` live in `migration` despite not being version-pinned to a specific Helios release?** It's framework-version-pinned (v6→v7) rather than app-version-pinned. The `migration` semantics are "you only need this once when crossing a version boundary," which fits.
...
| Date | Change |
|---|---|
| 2026-05-05 | Initial draft. Tier assignments proposed; per-skill manifest-driven install scheme. Open questions on `helios-auto-create-pr`/`helios-auto-continue-pr` core membership flagged. |
| 2026-05-05 | Resolved Q1: `helios-auto-create-pr` and `helios-auto-continue-pr` stay in `automation`. Spec is implementation-ready. |

...
`@helios/cli` conflates two orthogonal concepts and scatters the decision logic across every subsystem:

| Concept | Monorepo value | Standalone value |
|---------|----------------|-----------------|
| **workspace root** | repo root (symlinks present) | directory where `node_modules/@helios/*` lives |
| **app directory** | `apps/helios` (or similar) | `cwd` (or `cwd/apps/*` subdirectory if present) |
| **package sources** | `packages/<pkg>/src/` | `node_modules/@helios/<pkg>/src/` |
| **build scripts** | `yarn workspace @helios/app <cmd>` | `yarn run <cmd>` from `appDir` |
...
Problem: Standalone app's src/modules/ is never scanned.
         Tests inside node_modules/@helios/*/src/modules/__integration__/
...
```
Current:
  APP_BUILD_ARTIFACTS   = hardcoded apps/helios/... paths
  APP_BUILD_INPUT_PATHS = hardcoded apps/helios/ + packages/core/ + packages/ui/
  build commands use: runYarnWorkspaceCommand('@helios/app', 'initialize'|'build'|'start')
  checksum rm() targets hardcoded apps/helios path
  build:packages executed unconditionally

Problem: No apps/helios/ in standalone; no @helios/app workspace;
         no build:packages script. Every build step throws or resolves wrong dir.
```

#### 4. `src/helios.ts` — `dev` / `start` commands
...
Current: nodeModulesBase = isMonorepo ? rootDir : appDir   (single candidate)
         ...
         heliosBin = nodeModulesBase/node_modules/@helios/cli/bin/helios
...
export type CliEnvironment = {
  ...
  appDir: string
  /**
   * Resolve the root directory of an installed @helios package.
...
| Environment | `mode` | `rootDir` | `appDir` | `packageRoot('@helios/core')` |
|-------------|--------|-----------|---------|-------------------------------------|
| Monorepo (symlinks) | `monorepo` | symlink target parent | `detectAppDir(rootDir)` | `rootDir/packages/core` |
| Standalone, hoisted `node_modules` | `standalone` | `nodeModulesRoot` above cwd | `detectAppDir(rootDir)` if exists, else `cwd` | `rootDir/node_modules/@helios/core` |
| Standalone, local `node_modules` | `standalone` | `cwd` | `cwd` | `cwd/node_modules/@helios/core` |
...
| `env.rootDir/node_modules/@helios` | Installed package module trees (source-shipped) |
...
const APP_BUILD_ARTIFACTS = [
  path.join(env.appDir, '.helios', 'next', 'BUILD_ID'),
  path.join(env.appDir, '.helios', 'generated', 'modules.generated.ts'),
  path.join(env.packageRoot('@helios/core'), 'dist', 'index.js'),
  path.join(env.packageRoot('@helios/ui'),   'dist', 'index.js'),
]

// Workspace command replacement
// Before: runYarnWorkspaceCommand('@helios/app', 'initialize', ...)
...
const heliosBin = resolveInstalledBinary(bases, '@helios/cli/bin/helios')
```
...
- [ ] 2.2 Add `src/modules` and `node_modules/@helios` roots in `resolveEnabledModuleIds`
- [ ] 2.3 Add same roots to `discoverIntegrationSpecFiles` discovery roots array
- [ ] 2.4 Unit tests: standalone `src/modules` discovery; installed-package `node_modules/@helios` discovery
...
### Step 4 — `helios.ts`: `resolveInstalledBinary` + multi-base search
...
- [ ] 4.3 Use `resolveInstalledBinary` for `nextBin` and `heliosBin`
...
| `resolveEnabledModuleIds` — `node_modules/@helios` root | Unit |
...
| `helios dev` binary resolution — standalone | Manual smoke test |

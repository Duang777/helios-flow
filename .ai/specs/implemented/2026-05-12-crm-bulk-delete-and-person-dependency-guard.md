...
1. Extracts the per-row try/catch into a shared `runBulkDelete()` utility (`@helios/ui/backend/utils/bulkDelete`) that returns `{ succeeded, failures }`, where each failure preserves the server's `error.message` and `error.code`.
...
- Architecture: bulk-delete utility lives in `@helios/ui` (UI infra), grouping is pure data, store change is additive.

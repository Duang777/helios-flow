...
## Overview
This change introduces a new root-level developer workflow for running multiple Helios development instances in parallel across worktrees without manual port management.
...
2. Ensure `apps/helios/.env` exists:
- Reuse if present.
- Copy from `apps/helios/.env.example` if absent.
3. Run `yarn install`.
4. Run `yarn build:packages` so compiled CLI generators are current.
5. Run `yarn generate` so required `.helios/generated` files are present.
...
- Reads: `apps/helios/.env.example`
- Writes if missing: `apps/helios/.env`
...
- 2026-02-21: Extended `dev:ephemeral` with generator pre-step, browser auto-open, and `.ai/dev-ephemeral-envs.json` lifecycle (stale pruning + register/unregister). Updated `.ai/skills/helios-integration-tests/SKILL.md`, `.ai/qa/AGENTS.md`, and root `AGENTS.md` to reuse dev ephemeral instances for testing.

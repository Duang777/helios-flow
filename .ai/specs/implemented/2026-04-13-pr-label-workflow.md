...
| **Skills and humans out of sync** | `helios-auto-review-pr` skill sets `merge-queue`/`changes-requested`, but humans use different label names |
...
These track where a PR is in the pipeline. The `helios-auto-review-pr` skill, QA humans, and merge automation all operate on these.
...
- Unlike priority, `helios-auto-*` skills DO set or infer the risk label per the root `AGENTS.md` risk-inference rule (and carry it forward from a linked issue when present).
...
| `review` | `changes-requested` | Code review finds issues | `helios-auto-review-pr` skill or human reviewer |
| `review` | `merge-queue` (keep `needs-qa`) | Code review approves + `needs-qa` present; merge-gate holds the merge until `qa-approved` | `helios-auto-review-pr` skill or human reviewer |
| `review` | `merge-queue` | Code review approves + no `needs-qa` | `helios-auto-review-pr` skill or human reviewer |
...
| Scenario | Behavior |
|----------|----------|
| User runs `/helios-auto-review-pr 1234` and PR has no `in-progress` label and no assignee | Claim and proceed |
| User runs `/helios-auto-review-pr 1234` and PR is assigned to another user | Stop, ask "Override @other-user's claim?" |
| User runs `/helios-auto-review-pr 1234 --force` on a PR locked by another skill | Force, post override comment, proceed |
| Background `/helios-review-prs` triage hits a PR already locked by `/helios-auto-review-pr` | Skip silently, log "skipped: in progress" in summary |
| `/helios-auto-fix-github 999` and issue is assigned to a human contributor | Stop, ask "Override @contributor's claim?" |
| `/helios-auto-fix-github 999 --force` overrides a stale (>60min) lock | Force, post override comment, continue |

### Skip-when-in-progress in batch skills

`helios-review-prs` and `helios-merge-buddy` operate over many PRs at once. They MUST:
...
### `helios-auto-review-pr` skill changes
...
### `helios-auto-fix-github` skill changes
...
### `helios-check-and-commit` skill changes

No changes needed — operates before PR creation.

### New skill: `helios-merge-buddy`

A triage skill invoked via `/helios-merge-buddy`. Scans **all open PRs** and produces a merge-readiness report — a table of PRs that can be merged right now (zero blockers) plus a secondary table of PRs that are close but have specific remaining issues.
...
### New skill: `helios-review-prs`

A **day-start triage skill** invoked via `/helios-review-prs`. Finds all open PRs that have not been reviewed yet and reviews them one by one, starting from the most recent. Designed as a morning routine — run it at the start of your day to clear the review backlog.
...
5. **Sequential review loop** — for each PR in the queue:
   ...
   b. Invoke the full `helios-auto-review-pr` skill (including autofix flow)
...
- Use the full `helios-auto-review-pr` skill for each PR (with autofix)
- After the review session, optionally invoke `helios-merge-buddy` to show what's now ready to merge
- If there are zero unreviewed PRs, say so and suggest running `/helios-merge-buddy` instead
...
| Name | Notes |
|------|-------|
| `merge-queue` | Already used by `helios-auto-review-pr` skill |
| `changes-requested` | Already used by `helios-auto-review-pr` skill |
| `blocked` | Keep as-is |
...
1. Update `helios-auto-review-pr` SKILL.md — add QA gate logic and `review` label handling
2. Update `helios-auto-fix-github` SKILL.md — apply `review` + optional `skip-qa` on PR creation
3. Extract `setPipelineLabel` helper into a shared skill utility or inline in both skills
4. Create `helios-merge-buddy` skill — `.ai/skills/helios-merge-buddy/SKILL.md` — scan all open PRs, classify merge-readiness, output actionable table
5. Create `helios-review-prs` skill — `.ai/skills/helios-review-prs/SKILL.md` — day-start triage that reviews all unreviewed PRs (newest first) using the `helios-auto-review-pr` skill, then optionally runs `helios-merge-buddy`
...
| Phase 2 — Skill Updates | Done | 2026-04-14 | `helios-auto-review-pr` and `helios-auto-fix-github` updated; `helios-merge-buddy` and `helios-review-prs` skills added |
...
| Phase 6 — Risk Labels | Done | 2026-06-13 | Added `risk-low/medium/high` to communicate change blast radius (orthogonal to priority); `helios-auto-*` skills infer/carry/preserve it; documented in this spec and root `AGENTS.md` |
| Phase 7 — QA-label ownership | Done | 2026-06-15 | Reframed `qa` to mean "manual QA in progress", applied **manually by a QA reviewer** and never by an auto-skill. `helios-auto-review-pr` now routes an approved `needs-qa` PR to `merge-queue` (keeping `needs-qa`) where the `merge-gate` check holds the merge until `qa-approved`; auto-skills only ever request QA via `needs-qa`. Updated this spec, root `AGENTS.md`, `helios-auto-continue-pr` (+loop), and `helios-merge-buddy` (gate on `qa-approved`, not the pipeline label) |

### Detailed Progress

- [x] Create and normalize the target label set in GitHub
- [x] Re-label existing open PRs to the new pipeline states
- [x] Update `helios-auto-review-pr` with QA gate and `review` label handling
- [x] Update `helios-auto-fix-github` to open PRs in `review` and document `skip-qa`
- [x] Add `helios-merge-buddy` skill
- [x] Add `helios-review-prs` skill
- [x] Add PR workflow guidance to root `AGENTS.md`
- [x] Add `priority-low/medium/high/extreme` labels and triage rules (2026-05-04)
- [x] Add `risk-low/medium/high` labels, inference rules, and `helios-auto-*` skill handling (2026-06-13)

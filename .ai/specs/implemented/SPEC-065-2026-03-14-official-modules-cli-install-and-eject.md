...
**Date:** 2026-03-14
**Status:** Draft
**Scope:** OSS — `@helios/cli` support for installing official npm modules into Helios apps and optionally materializing them as app source
**Author:** Helios Team
...
**Key Points:**
- Add first-class `helios module` commands for installing official npm modules into a Helios app and registering them in app-level `src/modules.ts`.
...
Official modules are moving toward a package-based distribution model, but the app-side CLI still has no install UX that understands Helios module conventions. Today a developer can manually run `yarn add`, manually inspect the package for `moduleId`, manually edit `src/modules.ts`, and optionally run `eject` later. That workflow is error-prone, hard to document, and not aligned with the intended official-modules experience.

This spec defines the app-side CLI layer that turns official module packages into usable Helios modules. It focuses on install and registration, plus an explicit source-materialization path that uses the existing app-module model instead of inventing a second build system inside `apps/helios/src/modules`.
...
Introduce a `helios module` command namespace with a manifest-driven install flow limited to official Helios module packages.

Planned MVP command surface:

- `yarn helios module add <packageSpec>`
- `yarn helios module add <packageSpec> --eject`
- `yarn helios module enable <packageName>`
- `yarn helios module enable <packageName> --eject`
- `yarn helios module eject <moduleId>`
...
- support for non-`@helios/*` packages
...
| Decision | Direction |
|----------|-----------|
| Build ownership after eject | Main app build only; no nested package build in `apps/helios/src/modules` |
...
| Official package scope | Support only `@helios/*` packages with explicit official module metadata |
...
|-------------|-------------|
| Copy the full npm package into app source | `apps/helios/src/modules` is a module tree, not a nested workspace root; copying `package.json` and build scripts there creates a second packaging model |
...
| Infer module eligibility from package name prefix alone | `@helios/*` also contains host packages such as `core`, `ui`, and `shared`; explicit metadata is required |

## User Stories / Use Cases

- **Helios app developer** wants to run one CLI command to install an official module so that manual editing of `src/modules.ts` is no longer required.
- **Helios app developer** wants to install an official module directly as local source so that the code is immediately editable inside the app.
...
- **Framework maintainer** wants the old `helios eject <moduleId>` command to remain valid so that CLI backward compatibility is preserved.

## Architecture

### Command Namespace

The CLI gains a new `module` namespace under `helios`.

MVP commands:

| Command | Purpose |
|---------|---------|
| `yarn helios module add <packageSpec> [--module <moduleId>]` | Install and enable a module using the default installed flow |
| `yarn helios module add <packageSpec> [--module <moduleId>] --eject` | Install, extract source into app modules, and enable as `@app` |
| `yarn helios module enable <packageName> [--module <moduleId>]` | Enable a package that is already installed |
| `yarn helios module enable <packageName> [--module <moduleId>] --eject` | Copy an already-installed package into app modules and enable as `@app` |
| `yarn helios module eject <moduleId>` | Alias to existing eject flow, preserving old top-level `yarn helios eject <moduleId>` |
...
This MVP supports only official Helios module packages.

A package is eligible only if all conditions are met:

1. package name is under `@helios/`
2. package `package.json` contains an `helios` block
3. `helios.kind === "module-package"`
4. `helios.moduleId` exists and matches the module folder
5. package contains `dist/modules/<moduleId>`
6. package contains `src/modules/<moduleId>`

This explicit contract prevents accidental treatment of platform packages such as `@helios/core` or `@helios/ui` as installable optional modules.
...
- monorepo app: install into `apps/helios/package.json` via workspace-aware package-manager command
- monorepo root `package.json` is not the official install target for this feature

For Yarn-based examples:

- standalone: `yarn add @helios/test-package@preview`
- monorepo: `yarn workspace @helios/app add @helios/test-package@preview`

The CLI MAY detect package manager from `packageManager` or lockfiles, but Yarn support is the mandatory MVP path because current Helios monorepo and standalone guidance are Yarn-first.
...
2. verify `helios.ejectable === true`
...
```text
.helios/generated/module-package-sources.css
```
...
- `apps/helios/src/app/globals.css` imports the generated CSS file once
...
- generated CSS files remain machine-managed under `.helios/generated/`
...
- if `helios generate` fails after registration, CLI exits non-zero and reports that the module is enabled but generated artifacts are stale
...
### `HeliosModulePackageMetadata`

```ts
type HeliosModulePackageMetadata = {
  kind: 'module-package'
  moduleId: string
  ejectable: boolean
}
...
```css
@source "../../../../node_modules/@helios/test-package/src/**/*.{ts,tsx}";
@source "../../../../node_modules/@helios/another-module/src/**/*.{ts,tsx}";
```
...
- Usage: `yarn helios module add <packageSpec> [--module <moduleId>] [--eject]`
- Input:
  - `packageSpec`: npm package spec such as `@helios/test-package@preview`
  - optional `--module <moduleId>` for multi-module packages
  - optional `--eject`, default `false`
- Validation:
  - package must resolve under `@helios/*`
  - package must declare `helios.kind === "module-package"`
...
- Usage: `yarn helios module enable <packageName> [--module <moduleId>] [--eject]`
- Input:
...
- Usage: `yarn helios module eject <moduleId>`
- Contract:
  - shares behavior with existing `yarn helios eject <moduleId>`
...
```bash
yarn helios module add @helios/test-package@preview
yarn helios module add @helios/test-package@preview --eject
yarn add @helios/test-package@preview
yarn helios module enable @helios/test-package
yarn helios module enable @helios/test-package --eject
yarn helios module eject test_package
```
...
- Existing `yarn helios eject <moduleId>` remains supported unchanged.
...
- `helios.modules.json` is explicitly deferred to later lifecycle work in SPEC-061.
...
| Standalone template | Bootstrap changes in `apps/helios/src/app/**` must be mirrored in `packages/create-app/template/src/app/**` |
...
1. Add `helios module add` and `helios module enable` command parsing in `packages/cli/src/helios.ts`.
...
4. Update `apps/helios/src/app/globals.css` and `packages/create-app/template/src/app/globals.css` to import the generated CSS include.

### Phase 3 — Eject Flag and Eject Alias
1. Reuse and harden existing eject internals for `module add --eject`.
2. Add `helios module eject` alias while preserving `helios eject`.
...
|------|------|--------|---------|
| `packages/cli/src/helios.ts` | main repo | Modify | add `module` command namespace |
...
| `apps/helios/src/app/globals.css` | main repo | Modify | import generated package-source CSS |
...
- Integration tests:
  ...
  - old `helios eject` command remains valid
...
- **Scenario**: `src/modules.ts` or source extraction succeeds, but `helios generate` fails.
- **Severity**: Medium
...
- **Severity**: High
- **Affected area**: `create-helios-app` generated projects
...
- **Severity**: Medium
- **Affected area**: Existing users and scripts depending on `helios eject`
...
| root AGENTS.md | Generated files under `apps/helios/.helios/generated/` are machine-managed | Compliant | CSS source include is additive and generated |
...
| `packages/cli/AGENTS.md` | Generated output goes to `.helios/generated/` | Compliant | New CSS include is generated there |
...
- Resolved scope decisions:
  ...
  - MVP supports only official `@helios/*` module packages
  - `helios.modules.json` deferred to later lifecycle work
...
| Phase 2 — Package Mode Install | Done | 2026-03-14 | Added `helios module add` / `helios module enable`, install flow, package-source CSS generator, and app/template CSS imports |
| Phase 3 — Eject Flag and Eject Alias | Done | 2026-03-14 | Reused eject internals for `--eject`, added `helios module eject`, preserved legacy `helios eject`, and validated `--eject` boundaries |
...
- [x] Step 1: Add `helios module add` and `helios module enable` command parsing in `packages/cli/src/helios.ts`
...
- [x] Step 4: Update `apps/helios/src/app/globals.css` and `packages/create-app/template/src/app/globals.css` to import the generated CSS include

### Phase 3 — Detailed Progress
- [x] Step 1: Reuse and harden existing eject internals for `module add --eject`
- [x] Step 2: Add `helios module eject` alias while preserving `helios eject`
...
### Verification
- [x] `yarn workspace @helios/cli test`
- [x] `yarn workspace @helios/cli typecheck`
- [x] `yarn workspace @helios/cli build`
...
- [x] `yarn test`
- [ ] `yarn lint` — fails in existing `@helios/app` lint script with `Invalid project directory provided, no such directory: /Users/dpalatynski/Private/helios-latest/apps/helios/lint`

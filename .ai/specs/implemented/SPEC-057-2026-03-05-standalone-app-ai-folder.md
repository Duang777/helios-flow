...
**Date**: 2026-03-05
**Status**: Draft
**Related PR**: [#809](https://github.com/helios/helios/pull/809) (bundling `.ai/` into scaffolded apps)
**Related Issues**: [#805](https://github.com/helios/helios/issues/805), [#853](https://github.com/helios/helios/issues/853)

## TLDR

Design and implement a dedicated `.ai/` folder for standalone Helios apps (scaffolded via `create-helios-app`) with adapted skills, UMES reference documentation, and a comprehensive Task Router that guides AI agents to build custom features, extend core modules, and create integrations — all without modifying core packages.

## Problem Statement

PR #809 attempted to copy the monorepo's `.ai/` folder into scaffolded standalone apps. As identified in [andrzejewsky's review](https://github.com/helios/helios/pull/809#issuecomment-), this approach is fundamentally flawed:

1. **Wrong context**: The root `.ai/` is designed for monorepo contributors working on core packages. Standalone app developers work in a single Next.js project where `@helios/*` packages are npm dependencies.
...
| **integration-tests** | Keep mostly as-is. Adjust paths (`src/modules/<module>/__integration__/`). Ensure helpers import correctly from `@helios/core`. |
...
- Helper imports from `@helios/core` (npm package, not local)
...
| Building backend pages (admin UI) | `AGENTS.md` → Backend Pages + `.ai/skills/helios-backend-ui-design/` |
...
| **Testing** | |
| Writing integration tests | `.ai/qa/AGENTS.md` + `.ai/skills/helios-integration-tests/` |
| Running tests | `AGENTS.md` → Key Commands |
| **Spec & Planning** | |
| Writing a feature spec | `.ai/skills/helios-spec-writing/` |
| Reviewing code changes | `.ai/skills/helios-code-review/` |
| Implementing a spec | `.ai/skills/helios-implement-spec/` |

#### Core Principles (Standalone Edition)

1. **Extension-first**: Never modify files in `node_modules/@helios/`. Use UMES extension points to customize core behavior.
2. **Upgrade safety**: Custom modules must work across Helios version upgrades. Avoid depending on internal APIs — use documented extension contracts.
...
| Generated files | `.helios/generated/` | `.helios/generated/` |
...
- Add UMES compliance checks:
  ...
  - No direct imports from `node_modules/@helios/*/dist/` internals
...
- Helpers imported from `@helios/core` (npm package)
...
2. **Upgrade guide** — Document how to update `.ai/` folder when upgrading Helios packages
...
7. Adapt `helios-code-review` skill for standalone context
8. Adapt `helios-spec-writing` skill for standalone context
9. Adapt `helios-implement-spec` skill for standalone context
10. Adapt `helios-integration-tests` skill for standalone context
11. Copy unchanged skills: `helios-backend-ui-design`, `helios-create-agents-md`, `helios-fix-specs`, `helios-skill-creator`

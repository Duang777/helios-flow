...
| **MODIFY** | `apps/helios/.env.example` (`NEXT_PUBLIC_HELIOS_CRUDFORM_EXTENDED_EVENTS_ENABLED`) |
...
- CrudForm emission gate: `NEXT_PUBLIC_HELIOS_CRUDFORM_EXTENDED_EVENTS_ENABLED` (default `true`)
...
- `apps/helios/src/modules/example/api/assignees/route.ts` — test-only SSE probe emitter used by integration coverage
- `apps/helios/src/modules/example/__integration__/TC-UMES-003.spec.ts` — TC-UMES-E12/E13/E14 audience isolation coverage

...
- Default pipelines/stages seeded in the `yarn helios init` example data seeding
...
4) **Seeding**
   - `yarn helios init` example data seeding must create a default pipeline and default stages
...
8) Given a fresh environment created via `yarn helios init`, then a default pipeline and default stages exist and the Kanban is usable without manual setup.
...
Phase 4: `yarn helios init` example data seeding (default pipeline + stages)  
...
- Fresh `yarn helios init` environment includes default pipeline + stages.
...
- Added requirement: seed default pipeline + stages via `yarn helios init` example data

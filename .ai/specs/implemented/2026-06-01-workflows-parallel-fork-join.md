...
> Issue: [helios/helios#2292](https://github.com/helios/helios/issues/2292) — the `PARALLEL_FORK` / `PARALLEL_JOIN` portion (`WAIT_FOR_TIMER` from that issue is already implemented)
...
change, update `migrations/.snapshot-helios.json` in the same commit (per
...
- [x] Migration `Migration20260602120000.ts` (additive: new table + 3 nullable columns + 3 indexes) + `.snapshot-helios.json` updated (254 insertions, 0 deletions — no churn).

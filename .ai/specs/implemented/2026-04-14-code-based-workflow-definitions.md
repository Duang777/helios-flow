...
> **Market Reference:** Temporal (workflow engine) uses code-first workflow definitions exclusively — all workflow logic is TypeScript/Go/Python code, with no DB-stored definitions. We adopt the code-first principle but preserve the existing DB-based approach for user-created workflows (hybrid model). We reject Temporal's "code only" stance because Helios's visual editor and user-defined workflows are a core feature.
...
| Decision | Rationale |
|----------|-----------|
| Builder API in `@helios/shared` | Shared has zero domain deps. Builder produces plain data objects (no DB, no DI). Any module in any package can use it. Follows `createModuleEvents()` precedent. |
...
| CLI command for syncing (`yarn helios workflows sync`) | Would work but adds deploy pipeline coupling. In-memory approach is simpler and achieves the same outcome without any sync. |
...
```typescript
import { defineWorkflow } from '@helios/shared/modules/workflows/builder'
...
**Generated output:** `apps/helios/.helios/generated/workflows.generated.ts`

```typescript
// AUTO-GENERATED — do not edit
import type { CodeWorkflowDefinition } from '@helios/shared/modules/workflows/builder'
...
**Bootstrap wiring:** Called from `apps/helios/src/bootstrap.ts` (or the generated bootstrap), similar to how event configs and search configs are registered:

```typescript
import { codeWorkflowDefinitions } from '.helios/generated/workflows.generated'
...
| `apps/helios/src/bootstrap.ts` | Modify | 1 | Register code definitions at bootstrap |
...
### 2026-04-15
- Phase 1 shipped: `defineWorkflow()` builder in `@helios/shared/modules/workflows`, generator extension for `workflows.ts` auto-discovery, in-memory `code-registry`, bootstrap wiring registering `codeWorkflowDefinitions` from the generated aggregate. Unit tests cover builder type inference and Zod validation.

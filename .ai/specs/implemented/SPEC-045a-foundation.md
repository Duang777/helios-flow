...
A bundle is a regular Helios module that declares **multiple** `IntegrationDefinition` entries via a `integration.ts` that exports an array:

```typescript
// sync_medusa/integration.ts — BUNDLE MODULE

import type { IntegrationDefinition, IntegrationBundle } from '@helios/shared/modules/integrations'
...
```typescript
// @helios/shared/modules/integrations/types.ts (additions)
...
- use `HELIOS_INTEGRATION_<PROVIDER>_*` as the canonical env naming convention
...
Legacy aliases may be read for backward compatibility, but docs and examples must present the `HELIOS_INTEGRATION_<PROVIDER>_*` names as primary.
...
interface SshKeypairConfig {
  ...
  keyComment?: string  // Default: 'helios-{integrationId}-{tenantId}'
}
...
credentials: {
  fields: [
    { key: 'repositoryUrl', label: 'Repository URL', type: 'url', required: true, placeholder: 'git@github.com:org/repo.git' },
    {
      ...
      type: 'ssh_keypair',
      required: true,
      ssh: { algorithm: 'ed25519', keyComment: 'helios-deploy' },
    },
  ],
}
```
...
│  Public Key:                                              [Copy]     │
│  ┌──────────────────────────────────────────────────────────────┐    │
│  │ ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAI... helios-deploy│    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                      │
...
1. Create `@helios/shared/modules/integrations/types.ts` — all shared types including `IntegrationBundle`, `ApiVersionDefinition`, `CredentialFieldOAuth`, `CredentialFieldSshKeypair`

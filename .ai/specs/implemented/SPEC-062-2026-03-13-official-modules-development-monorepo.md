...
**Date:** 2026-03-13
**Status:** Draft
**Scope:** OSS — separate repository architecture for `helios/official-modules`
**Author:** Helios Team
...
**Key Points:**
- `official-modules` SHOULD be a separate Turborepo monorepo dedicated to developing and publishing optional Helios modules
- Every published module is its own npm package, but each package MUST embed Helios module source under `src/modules/<module_id>/`
...
- Default development targets published stable `@helios/*` packages and a standalone sandbox app
...
- isolated module development outside the main Helios monolith
...
4. **Naming inconsistency leaks into runtime**
   npm package names are naturally kebab-case, but Helios module IDs are snake_case/plural. Without a contract, packages, module IDs, and folder names will diverge and break discovery.
...
Create `helios/official-modules` as a dedicated Turborepo monorepo with three core building blocks:

1. **Package workspaces for publishable modules**
   Each module lives in `packages/<package-folder>/` and is published independently as `@helios/module-<name>`.

2. **A standalone sandbox app**
   `apps/sandbox/` is a real standalone Helios app used for local development, CI smoke tests, and preview validation.
...
| Package names in kebab-case, module IDs in snake_case | Preserves npm conventions and Helios module conventions simultaneously |
...
|---------|---------|------------|
| npm package name | `@helios/module-n8n-integration` | kebab-case |
| package folder | `packages/n8n-integration` | kebab-case |
| Helios module ID | `n8n_integration` | snake_case, plural rules when applicable |

Rule:

- package names follow npm conventions
- runtime module IDs follow Helios conventions from root `AGENTS.md`
...
```ts
@helios/module-n8n-integration/modules/n8n_integration/data/entities
```

Therefore each package SHOULD expose wildcard exports similar to core Helios packages:
...
```ts
{ id: 'n8n_integration', from: '@helios/module-n8n-integration' }
```
...
- `apps/sandbox` depends on published stable `@helios/*` packages
- module packages depend on published stable `@helios/*` packages
...
Use when developing official modules against the latest stable Helios release.
...
4. run `yarn helios generate all`
...
```json
{
  "name": "@helios/module-n8n-integration",
  "version": "0.1.0",
  "type": "module",
  "main": "./dist/index.js",
  "exports": {
...
| `yarn sandbox:generate` | Run `helios generate all` in sandbox |
...
6. run `yarn helios generate all` in sandbox
7. run sandbox build
...
| sandbox `helios generate all` after package change | Integration test |
...
| Module ID | MUST remain Helios-compatible snake_case/plural naming |
...
3. Create `apps/sandbox` from `npx create-helios-app sandbox`
...
3. Ensure sandbox uses real `helios` CLI path and standalone behavior
...
- Sandbox integration test for `helios generate all`
...
The main deployment risk is coupling the module repo too tightly to assumptions from the main Helios monorepo.
...
- periodic sync against `create-helios-app` template expectations
...
- **Scenario**: Package name uses one identifier while `helios.moduleId`, folder path, and sandbox registration use another.
- **Severity**: High
...
- **Mitigation**: sandbox must run `helios generate all` and consume package artifacts through standalone-compatible paths
...
- [packages/create-app/AGENTS.md](/Users/dpalatynski/Private/helios-latest/packages/create-app/AGENTS.md)
- [packages/create-app/template/src/modules.ts](/Users/dpalatynski/Private/helios-latest/packages/create-app/template/src/modules.ts)

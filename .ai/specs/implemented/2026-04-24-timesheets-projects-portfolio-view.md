...
| Decision | Rationale |
|---|---|
| Role detection by ACL feature (`staff.timesheets.projects.manage`) | Matches Helios's declarative ACL pattern. No new role model needed. |
...
| Response enricher for list augmentation (`hoursTrend`, `members`, `myRole`) | Idiomatic Helios pattern. Keeps CRUD routes pure; enricher can be versioned or toggled per feature. |

...
- Add an **interactive wizard** to `create-helios-app` that lets developers choose their AI coding tool(s).
- Add a **`yarn helios agentic:init`** CLI command for existing apps that missed the wizard.
...
The monorepo's `.ai/` folder (295 files: 56+ specs, core skills like `helios-code-review`, `helios-implement-spec`, `helios-pre-implement-spec`, integration test infrastructure) is designed for core contributors working on `packages/`. It references `packages/core/src/`, overlay contracts, backward compatibility surfaces, and changeset publishing — none of which apply to standalone app developers who work in `src/modules/` and consume `@helios/*` from npm.
...
└── cursor/
    ├── rules/
    │   ├── helios.mdc            project rules (alwaysApply: true)
...
| Skills | `helios-code-review`, `helios-implement-spec`, `helios-pre-implement-spec`, etc. | None — standalone apps use AGENTS.md + tool-specific rules |
...
| Path references | `packages/core/`, `packages/ui/` | `src/modules/`, `node_modules/@helios/` |
...
| Output file | Source | Content |
|---|---|---|
| `.cursor/rules/helios.mdc` | `agentic/cursor/rules/helios.mdc` | Project rules (alwaysApply: true) — extension-first, module system, critical conventions |
...
| `.cursor/rules/generated-guard.mdc` | `agentic/cursor/rules/generated-guard.mdc` | Glob-scoped rule for `.helios/generated/**` — "NEVER edit" guard |
...
Do NOT load the entire src/ tree — Helios apps can have many modules.

## What this project is

A standalone Helios application. You are building domain features ON TOP of
the Helios framework — not modifying the framework itself.

The framework lives in node_modules/@helios/*. To customise a built-in module
beyond what the extension system supports, eject it with `yarn helios eject <module>`.
...
| Eject a core module | Run `yarn helios eject <module-id>` |
...
- NEVER edit `.helios/generated/*` — auto-generated by `yarn generate`
- NEVER directly edit files in `node_modules/@helios/*` — eject instead
...
- Module authoring: https://docs.helios.com/framework/modules/overview
- Entities & migrations: https://docs.helios.com/framework/database/entities
- DI container: https://docs.helios.com/framework/ioc/container
```
...
Framework-level decisions belong in the Helios core repo. If you're unsure,
...
The framework lives in node_modules/@helios/*. To customise a built-in module,
eject it with `yarn helios eject <module>` — never edit node_modules directly.

## Stack

- Next.js App Router, TypeScript, MikroORM, Awilix DI, zod
- Platform docs: https://docs.helios.com

## Critical rules

- NEVER edit `.helios/generated/*`
...
3. **Never edit `.helios/generated/*`**: edit the source and run `yarn generate` instead
...
### `agentic/cursor/rules/helios.mdc`

```markdown
---
description: Core Helios conventions for standalone app development
alwaysApply: true
---
...
A standalone Helios application. Build domain features ON TOP of the framework.
To customise a core module beyond the extension system: `yarn helios eject <module>`.
Never edit node_modules/@helios/* directly.
...
Run `yarn generate` after any module or entity change. Never edit `.helios/generated/*`.
...
```bash
# Interactive
yarn helios agentic:init

# Non-interactive
yarn helios agentic:init --tool=claude-code
yarn helios agentic:init --tool=claude-code,codex,cursor
yarn helios agentic:init --tool=cursor --force
```
...
| `cursor.ts` | `.cursor/rules/helios.mdc`, `.cursor/rules/entity-guard.mdc`, `.cursor/rules/generated-guard.mdc`, `.cursor/hooks.json`, `.cursor/hooks/entity-migration-check.mjs`, `.cursor/mcp.json.example` | — |
...
### `packages/cli/src/helios.ts` — `agentic:init` command
...
export async function runAgenticInit(args: string[]): Promise<void> {
  ...
  if (!existsSync(join(targetDir, 'src', 'modules.ts'))) {
    console.error('Not an Helios app directory (src/modules.ts not found)');
    process.exit(1);
  }
...
**Rejected**: `create-helios-app` runs standalone via `npx` without the CLI installed. Generators must live in `packages/create-app/` and be importable by `packages/cli/`.
...
| `create-helios-app` package size increase | Low | npm package | ~25KB of text files. Negligible. | None |
...
**Goal**: Existing apps can run `yarn helios agentic:init`.

17. Create `packages/cli/src/lib/agentic-init.ts` — imports `runAgenticSetup` from create-app
18. Register `agentic:init` command in `packages/cli/src/helios.ts`
...
- `packages/cli/src/helios.ts` (modified)
...
```bash
# 1. Build create-app
yarn workspace @helios/create-app build
...
npx --registry http://localhost:4873 create-helios-app@latest my-test-agentic
# → Wizard appears, select tools
cd my-test-agentic

# 4. Verify file tree
tree -a -I 'node_modules|.next|.helios' --dirsfirst .
...
npx --registry http://localhost:4873 create-helios-app@latest my-existing
cd my-existing

# 2. Run agentic:init
yarn helios agentic:init                    # Interactive
yarn helios agentic:init --tool=cursor      # Non-interactive
yarn helios agentic:init --tool=cursor --force  # Overwrite

# 3. Verify + test idempotency
yarn helios agentic:init --tool=cursor      # Should warn "already exists"
```
...
- [ ] `create-helios-app my-app` → wizard appears after template copy
...
- [ ] `.cursor/rules/helios.mdc` has `alwaysApply: true`
- [ ] `.cursor/rules/entity-guard.mdc` has globs for `src/modules/*/entities/**/*.ts`
- [ ] `.cursor/rules/generated-guard.mdc` has globs for `.helios/generated/**`
...
- [ ] `yarn helios agentic:init` works on app without agentic files
- [ ] `yarn helios agentic:init --force` overwrites existing files
- [ ] `yarn helios agentic:init` without `--force` warns about existing files
...
1. **Zero-config agentic experience**: `create-helios-app` → select tool → working AI agent config immediately.
...
4. **Retroactive adoption**: `yarn helios agentic:init` adds agentic config to existing apps without re-scaffolding.

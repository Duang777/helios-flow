...
Container Created (first time)
  └── postCreateCommand → scripts/post-create.sh
       ...
       └── [7/7] helios init (first run) OR db:migrate (subsequent)
...
- **Database state detection**: `post-create.sh` queries `information_schema.tables` to determine whether to run `helios init` (empty DB) or `db:migrate` (existing tables). This is stateless — no marker files or volumes needed.
...
- **Mitigation**: Troubleshooting table documents the `docker compose down -v` command to wipe all volumes and trigger a fresh `helios init`. Note: VS Code's "Rebuild Container" does NOT wipe named volumes — only `docker compose down -v` does.

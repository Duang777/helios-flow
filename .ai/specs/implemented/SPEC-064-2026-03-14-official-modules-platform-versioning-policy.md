...
**Date:** 2026-03-14
**Status:** Draft
**Scope:** OSS — versioning and compatibility policy for `helios/official-modules`
**Author:** Helios Team
...
**Key Points:**
- `official-modules` packages MUST NOT use `workspace:*` for `@helios/*` platform packages unless that package is actually a local workspace in the same repository
- Published module packages MUST express Helios platform compatibility through `peerDependencies`, not through regular `dependencies`
- The repository MUST centralize exact development/tested platform versions in one root-managed policy file and sync scripts, so maintainers do not manually bump `@helios/*` versions in every package
...
SPEC-062 establishes that `official-modules` is a standalone monorepo that develops publishable Helios extensions against published platform packages rather than against local workspaces from the core monorepo. That repository architecture is correct, but it leaves one operational question underspecified:

How should module packages declare their relationship to `@helios/shared`, `@helios/ui`, `@helios/core`, and related platform packages without forcing maintainers to manually rewrite versions across every package?
...
2. **Maintainers are forced into repetitive manual bumps**
   If every module package duplicates exact versions of `@helios/*` platform packages in `devDependencies`, then every platform release requires touching many package manifests even when package compatibility policy did not change.

3. **Compatibility contracts become unclear**
   Exact versions in package manifests do not answer the key consumer question:
   “Which Helios platform range is this module compatible with?”
...
5. **CI may validate the wrong thing**
   If the sandbox app, root repo, and package manifests drift apart, CI may be green while published packages are semantically incompatible with the actual Helios release line they claim to support.
...
2. **Per-package platform compatibility contract**
   Each publishable module package declares the Helios platform packages it needs in `peerDependencies`, using semver ranges such as `>=0.4.8 <0.5.0`.
...
| `helios` metadata mirrors compatibility intent for CLI/marketplace UX | npm peers enforce install-time compatibility; metadata supports diagnostics and discovery |
...
|-------------|-------------|
| Repeat exact `@helios/*` versions in every module package `devDependencies` | High maintenance cost and guaranteed drift |
...
- **Module maintainer** wants to update the repository to a newer Helios stable release by editing one version policy input, not twenty package manifests
- **Module maintainer** wants a package to declare the minimum supported Helios version line so that consumers get correct peer dependency behavior
...
| Platform compatibility range | module package peers | `>=0.4.8 <0.5.0` | What Helios platform line the module supports |
...
- `@helios/ui`
- `@helios/core`
- other Helios host/platform packages that are expected to be provided by the consuming app

#### 2.2 `peerDependencies`

`peerDependencies` MUST contain every Helios platform package directly imported by the module package at runtime or typecheck time.
...
{
  "peerDependencies": {
    "@helios/shared": ">=0.4.8 <0.5.0",
    "@helios/ui": ">=0.4.8 <0.5.0",
    "react": "^19.0.0"
  }
}
```
...
- `workspace:*` for `@helios/shared` is invalid unless `official-modules` itself contains a workspace named `@helios/shared`
- `workspace:*` for `@helios/ui` is invalid unless that package exists locally in the same repo
...
- `moduleId`
  - runtime module identifier from the Helios module system
...
The `main` branch of `official-modules` MUST target the latest stable Helios platform line by default.
...
A module package published as a stable release MUST NOT advertise a prerelease-only Helios peer floor.
...
- optionally update `helios.minCoreVersion` / `testedCoreRange` when using repo-wide defaults
...
- fail if `helios.minCoreVersion` and `testedCoreRange` drift from peer ranges
...
| Feature now requires `@helios/shared >=0.4.10` | Yes | Yes |
...
1. Every required Helios host package imported by a module package must appear in `peerDependencies`
...
| Field | Contract |
|-------|----------|
| `peerDependencies` | Helios compatibility contract |
...
| `helios.minCoreVersion` | CLI-visible minimum supported platform version |
| `helios.testedCoreRange` | CI-validated platform line |
...
Not applicable in the Helios product UI.

Developer UX requirements:

- one source of truth for exact platform versions
- zero manual bumping of duplicated `@helios/*` versions across packages
...
| `HELIOS_PLATFORM_CHANNEL=stable|preview` | Optional CI/local channel selector |
| `HELIOS_PLATFORM_PREVIEW_VERSION` | Optional explicit prerelease version override |
| `HELIOS_PLATFORM_PREVIEW_TAG` | Optional prerelease dist-tag selector (for example `canary`) |
...
1. remove `workspace:*` for non-local `@helios/*` packages
2. move Helios host packages from `dependencies` to `peerDependencies` where applicable
...
| Contract | Rule |
|---------|------|
| Standalone repo install | MUST succeed without local Helios platform workspaces |
...
2. Move Helios platform packages to `peerDependencies`
3. Add `helios.minCoreVersion` and `testedCoreRange` defaults
...
- **Scenario**: A maintainer copies a package from the main monorepo and leaves `@helios/shared: "workspace:*"` or similar in the package manifest.
- **Severity**: High
...
| Compatibility metadata matches package-manager semantics | Pass | `peerDependencies` plus `helios` metadata have distinct responsibilities |

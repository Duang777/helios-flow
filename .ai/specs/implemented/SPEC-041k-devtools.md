...
```
x-helios-ext-<module>-<key>: <value>
```

Example:
```
x-helios-ext-record-locks-token: abc123
x-helios-ext-business-rules-override: skip-credit-check
```
...
For performance-critical enrichers, an optional cache layer using existing `@helios/cache` infrastructure:

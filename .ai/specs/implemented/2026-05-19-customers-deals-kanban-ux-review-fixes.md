...
- **`@helios/ui` primitives**: Radio / RadioGroup, Popover, ComboboxInput, PageHeader, Breadcrumb, Button (`variant="link"`) — all already shipped. No DS additions required.
...
| # | Component | Change |
|---|-----------|--------|
| 21 | `QuickDealDialog.tsx:159-170` | Replace the `<select>` for Company with `ComboboxInput` from `@helios/ui/backend/inputs/ComboboxInput`. Async search hits the existing companies search endpoint already used in CrudForm. |
...
- [ ] `yarn helios configs cache structural --all-tenants` run after Phase D (PageHeader/Breadcrumb may affect nav cache — verify; if no structural change, skip)

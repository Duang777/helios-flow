...
| Runtime fullapp compose | `docker-compose.fullapp.yml` starts app with `yarn start` in `apps/helios` | Monorepo root commands are not first-class in this profile |
...
2. **First-class Docker wrappers**
   ...
   - Prioritize high-value scripts:
     ...
     - `lint`, `typecheck`, `test`
     - `install-skills`
     - selected `helios` CLI command passthrough.

...
## Overview
Helios authorizes scoped entities by organization within a tenant. Two distinct guard families enforce this:
...
A pure predicate decoupled from `OrganizationScope`/core types so it can live in `@helios/shared`:
...
| Decision | Rationale |
|----------|-----------|
| Predicate takes primitives, not `OrganizationScope` | Keeps the core decision in `@helios/shared` without importing core/domain types (shared has zero domain deps). |
...
| Single mega-helper imported by both paths from core | `@helios/shared` must not import core/domain types; the command guard lives in shared. Split predicate + two thin wrappers instead. |
...
- Internal `throw new CrudHttpError(403, { error: 'Forbidden' })` in shared scope helper is a contract-level denial; keep as-is (matches existing code, not a user-facing translatable surface in `@helios/shared`).
...
2. Run validation gate: `yarn workspace @helios/shared test`, `yarn workspace @helios/core test`, `yarn typecheck`, `yarn lint`, targeted integration suite.
3. Document the behavioral security change (this spec's Migration & Compatibility section). `RELEASE_NOTES.md` does not exist in this repo; `CHANGELOG.md` is generated at release time by `helios-auto-update-changelog` (PR-numbered, house format) — do not hand-edit mid-cycle.
...
| packages/shared/AGENTS.md | No imports from `@helios/core`/domain | Compliant | Predicate takes primitives; read guard lives in core |

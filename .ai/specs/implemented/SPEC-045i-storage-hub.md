...
**Related**: Attachments module (`packages/core/src/modules/attachments/`), [SPEC-058 (PR #875)](https://github.com/helios/helios/pull/875)
...
| New dependency declared | Yes — `@aws-sdk/client-s3` in `@helios/storage-s3` (not core) |
...
### `@helios/storage-s3` opt-in

The `storage_s3` module is **not enabled by default**. Enabling it requires setting `HELIOS_ENABLE_STORAGE_S3=true` in the app environment. Without this flag, the AWS SDK is never loaded and no S3-related ACL features, API routes, or Integration Marketplace entries are registered.

### S3 driver ownership

The `S3StorageDriver` class lives in `@helios/storage-s3` (at `src/modules/storage_s3/lib/s3-driver.ts`). It is registered with the `StorageDriverFactory` at DI startup time by the storage-s3 module's `di.ts`. External packages that previously imported `S3StorageDriver` from `@helios/core` must update their import to `@helios/storage-s3`.

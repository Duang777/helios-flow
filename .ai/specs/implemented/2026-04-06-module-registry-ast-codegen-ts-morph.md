...
- ~30 generated `.ts` files + 1 `.css` + 1 `.json` in `apps/helios/.helios/generated/`
...
**Concerns:**
- ts-morph adds a dependency (~5MB) to `@helios/cli` — acceptable for a dev-only tool
...
The Helios CLI generator system scans all enabled modules, discovers convention files, and produces TypeScript source files that wire routing, DI, events, search, notifications, widgets, and more into the application bootstrap. Today, all TypeScript output is built by concatenating strings inside template literals — a pattern that:
...
These helpers wrap ts-morph's API to encode Helios's code generation conventions:
...
```typescript
const output = `// AUTO-GENERATED by helios generate di
${imports.join('\n')}
...
addAutoGeneratedComment(sourceFile, 'helios generate di')
...
export function addAutoGeneratedComment(sourceFile: SourceFile, generator: string): void {
  sourceFile.insertText(0, `// AUTO-GENERATED by helios generate ${generator}\n`)
}
...
- CLI commands (`helios generate`): unchanged
- Convention file paths: unchanged
- Module API contracts: unchanged
- No changes to `@helios/shared` types

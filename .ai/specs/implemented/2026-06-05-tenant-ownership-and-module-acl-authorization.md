...
> **Issue:** [helios#2612](https://github.com/helios/helios/issues/2612)
...
At each site: resolve the requirement; if the entity is custom/EAV, apply the existing `entities.records.*` + tenant-scope path; otherwise load the caller ACL via `rbacService.loadAcl(auth.sub, { tenantId, organizationId })` and require the resolved features with the wildcard-aware `hasAllFeatures` (`@helios/shared/security/features` — never `includes`/`Set.has`, per `.ai/lessons.md:197`). `platformOnly` requires `acl.isSuperAdmin`. Missing → `403` **before** touching target data. To keep the five sites consistent, extract a single `assertEntityAclForRequest(req|auth, entityId, 'view'|'manage', { rbac, em })` helper in `entityAcl.ts` and call it from each.
...
- No DB schema change. ACL feature additions (if the entity override map references new features) follow the `acl.ts` → `setup.ts defaultRoleFeatures` → `yarn helios auth sync-role-acls` flow.
...
- [x] Breaking-change entry added to `UPGRADE_NOTES.md` (`0.6.4 → 0.6.5` section) — org-lookup shape + entity-records ACL + auth ownership. (Repo uses `UPGRADE_NOTES.md` + `CHANGELOG.md`, not `RELEASE_NOTES.md`; CHANGELOG entry left to `helios-auto-update-changelog` at PR time.)

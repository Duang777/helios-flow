...
- Adapt three monorepo AI skills — **spec-writing**, **backend-ui-design**, and **code-review** — for standalone Helios app developers.
...
| `yarn template:sync` between `apps/helios/` and `packages/create-app/template/` | N/A |
...
2. **backend-ui-design** — Zero-adaptation component library. Standalone apps use the exact same `@helios/ui` package via npm.
...
| Keep full UI component reference | `@helios/ui` is identical in standalone — same imports, same API |
...
**Adapted from**: `.ai/skills/helios-spec-writing/`
...
Design and review specifications (SPECs) against Helios architecture and quality rules.
...
**Adapted from**: `.ai/skills/helios-backend-ui-design/`

**What's kept**: Everything — the `@helios/ui` package is identical in standalone apps. Same imports, same components, same patterns.

**What's removed**:
- References to SPEC-001 and SPEC-016 (monorepo-internal specs)
- "When to Create New Components" section suggesting additions to `@helios/ui` (standalone devs can't modify the package)
...
description: Design and implement consistent backend/backoffice interfaces using @helios/ui. Use when building admin pages, CRUD interfaces, data tables, forms, detail pages, or any backoffice UI.
---

# Backend UI Design

Guide for creating consistent, production-grade backend interfaces using the `@helios/ui` component library. All implementations must use existing components for visual and behavioral consistency.
...
```tsx
import { FormHeader, FormFooter, FormActionButtons, ActionsDropdown } from '@helios/ui/backend/forms'
```
...
```tsx
import { DetailFieldsSection, LoadingMessage, ErrorMessage, TabEmptyState } from '@helios/ui/backend/detail'
...
import { Notice } from '@helios/ui/primitives/Notice'
...
This file is copied directly from the monorepo's `.ai/skills/helios-backend-ui-design/references/ui-components.md` with no modifications — the component API is identical in standalone apps.

---

### Skill 3: code-review

**Adapted from**: `.ai/skills/helios-code-review/`
...
Review code changes against Helios architecture rules, security requirements, and quality standards.
...
```markdown
| Write a spec | `.ai/skills/helios-spec-writing/SKILL.md`, `.ai/specs/SPEC-000-template.md` |
| Review code changes | `.ai/skills/helios-code-review/SKILL.md` |
| Build backend UI | `.ai/skills/helios-backend-ui-design/SKILL.md` |
```
...
1. `.ai/skills/helios-spec-writing/SKILL.md`
2. `.ai/skills/helios-spec-writing/references/spec-template.md`
3. `.ai/skills/helios-spec-writing/references/spec-checklist.md`
4. `.ai/skills/helios-backend-ui-design/SKILL.md`
5. `.ai/skills/helios-backend-ui-design/references/ui-components.md`
6. `.ai/skills/helios-code-review/SKILL.md`
7. `.ai/skills/helios-code-review/references/review-checklist.md`
...
14. Test: `yarn helios agentic:init --tool=claude-code` → verify skills included
...
# Verify
ls /tmp/test-skills/.ai/skills/helios-spec-writing/
ls /tmp/test-skills/.ai/skills/helios-backend-ui-design/
ls /tmp/test-skills/.ai/skills/helios-code-review/
cat /tmp/test-skills/.ai/skills/helios-spec-writing/SKILL.md | head -5  # Check {{PROJECT_NAME}} resolved
cat /tmp/test-skills/AGENTS.md | grep "skills/"  # Check task-context references
```

### Verification checklist

- [ ] `.ai/skills/helios-spec-writing/SKILL.md` exists with resolved `{{PROJECT_NAME}}`
- [ ] `.ai/skills/helios-spec-writing/references/spec-template.md` exists
- [ ] `.ai/skills/helios-spec-writing/references/spec-checklist.md` exists (6 sections, ~25 items)
- [ ] `.ai/skills/helios-backend-ui-design/SKILL.md` exists
- [ ] `.ai/skills/helios-backend-ui-design/references/ui-components.md` exists (40+ components)
- [ ] `.ai/skills/helios-code-review/SKILL.md` exists with 4-step CI/CD gate
- [ ] `.ai/skills/helios-code-review/references/review-checklist.md` exists (8 sections, ~40 items)
...
- [ ] `yarn helios agentic:init` generates skills alongside existing shared files

# helios-help: Workflow Navigator Skill
...
**Key Points:**
- New skill `helios-help` gives agents a navigation layer over the entire `helios-*` skill ecosystem
...
**Scope:** `.ai/skills/helios-help/` + registration in `tiers.json` and `README.md`. No code, no migrations, no API changes.
...
The Helios skill ecosystem has 34 skills across 5 tiers. Developers and agents frequently face three friction points:
...
2. **Navigation gaps** — after completing one skill (e.g. `helios-implement-spec`) the correct next step (`helios-code-review` → `helios-check-and-commit`) must be recalled from memory or re-read from AGENTS.md.
...
`bmad-help` in the BMAD-METHOD framework solves an analogous problem for BMAD modules. `helios-help` adapts that pattern for Helios's skill + AGENTS.md structure.
...
```
.ai/skills/helios-help/
├── SKILL.md
└── references/
    ├── skills-catalog.md
    └── workflow-sequences.md

.ai/skills/tiers.json          # add "helios-help" to core.skills
.ai/skills/README.md           # add row to core table + bump tier count
.ai/specs/implemented/2026-05-27-helios-help-skill.md   # this file
```
...
- Table of all `helios-*` skills: tier, trigger phrase, preceded-by, followed-by
...
**Step 4:** Register in `tiers.json` — add `"helios-help"` to `core.skills`

**Step 5:** Update `README.md` — add `helios-help` row to core table, bump core skill count from 13→14
...
- **2026-06-10** — Marked Implemented; skill shipped in PR #2140 (`feat(skills): add helios-help workflow navigator skill`).

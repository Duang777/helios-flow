...
### Level 2: `helios init` (runtime crash)
`packages/cli/src/helios.ts` (lines 299-417) hardcodes ~15 `runModuleCommand()` calls to specific modules. `runModuleCommand` **throws** if the module is not found (line 54). This means disabling any referenced module crashes the init command.
...
export type ModuleSetupConfig = {
  ...
  /**
   * Called during `helios init` after tenant exists.
  ...
  /**
   * Called during `helios init` ONLY when --no-examples is NOT passed.
...
export const setup: ModuleSetupConfig = {
  ...
  async seedDefaults({ em, tenantId, organizationId }) {
    ...
    // (moved from hardcoded CLI calls in helios.ts)
...
```typescript
import { getModules } from '@helios/shared/modules/runtime'
const resolvedModules = modules ?? getModules?.() ?? []
```

---

## Step 6: Refactor `helios init` command

**File:** `packages/cli/src/helios.ts`
...
These remain in `helios.ts` as they are cross-cutting concerns:
...
| `packages/cli/src/helios.ts` | Replace hardcoded `runModuleCommand` calls with `seedDefaults`/`seedExamples` loops |
...
| Step | Status | Notes |
|------|--------|-------|
| `helios init` resilience | Done | `runModuleCommand` accepts `{ optional: true }` — 4 callers updated (customers, dashboards, search, query_index) |
...
| `packages/cli/src/helios.ts` | `runModuleCommand` with `{ optional }` flag for resilient init |
...
- `apps/helios/src/modules/example/setup.ts`
...
### 2026-01-27 (Phase 2)
- `helios.ts`: Added `{ optional }` flag to `runModuleCommand`; 4 callers updated (customers, dashboards, search, query_index) to skip gracefully when module is disabled
...
- Added `helios init` decoupling (Step 6) with dependency-ordered seed loops
...
- Documented remaining Phase 2 work (helios init refactor, upgrade-actions hardening)

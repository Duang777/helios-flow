...
- If querying ActionLog directly via ORM, use `findWithDecryption` from `@helios/shared/lib/encryption/find` with the document's `tenantId`.
...
import type { InjectionWidgetProps } from '@helios/ui/backend/injection'
...
```typescript
import type { InjectionWidgetMeta } from '@helios/shared/lib/injection'

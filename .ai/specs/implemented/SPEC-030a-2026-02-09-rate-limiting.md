...
The API catch-all dispatcher (`apps/helios/src/app/api/[...slug]/route.ts`) supports automatic IP-based rate limiting via route metadata, following the same pattern as `requireAuth`/`requireFeatures`. Any endpoint can opt-in by declaring `rateLimit` in its `metadata` export:
...
const RL_GLOBAL_KEY = '__heliosRateLimiterService__'
...
import { RateLimiterService } from '@helios/shared/lib/ratelimit/service'
import { readRateLimitConfig } from '@helios/shared/lib/ratelimit/config'
...
### Changes to `apps/helios/src/app/api/[...slug]/route.ts`
...
The following section is in `apps/helios/.env.example`:
...
The following key is in the **app-level** locale files (`apps/helios/src/i18n/{en,pl,de,es}.json`), alongside the existing `api.errors.*` keys:
...
4. **Key Collision**: The global `keyPrefix` + per-endpoint `keyPrefix` combination prevents collisions between endpoints and between Helios instances sharing the same Redis.
...
3. Extend `MethodMetadata` type in `apps/helios/src/app/api/[...slug]/route.ts`.
...
1. Update `apps/helios/.env.example` with rate limit variables.
2. Add `api.errors.rateLimit` to all locale files (`apps/helios/src/i18n/{en,pl,de,es}.json`).
...
- Fixed i18n key location to `apps/helios/src/i18n/` (matches existing `api.errors.*` namespace)

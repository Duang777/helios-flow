...
The module registry generator (`packages/cli/src/lib/generators/module-registry.ts`) is the heart of Helios's auto-discovery system. It iterates over all enabled modules, scans for convention files, and produces generated TypeScript files that wire everything together at build time.

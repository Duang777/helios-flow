...
| Related Issues | [#1137](https://github.com/helios/helios/issues/1137) |
...
This spec adopts the simplest upstream proposal: treat ephemeral integration runtime as a first-class app mode and suppress the notice bars at render time whenever `HELIOS_INTEGRATION_TEST=true`.
...
- [`apps/helios/src/components/GlobalNoticeBars.tsx`](/Users/mariuszlewczuk/Projects/omML/apps/helios/src/components/GlobalNoticeBars.tsx) shows the demo notice when `demoModeEnabled` is true and `om_demo_notice_ack` is absent.
...
- [`packages/cli/src/lib/testing/integration.ts`](/Users/mariuszlewczuk/Projects/omML/packages/cli/src/lib/testing/integration.ts) already provides a dedicated ephemeral runtime environment and already injects `HELIOS_TEST_MODE=1`, but the app shell does not currently use a dedicated notice-suppression flag.
...
1. The ephemeral integration runner sets `HELIOS_INTEGRATION_TEST=true` for the app process.
2. The app shell derives `noticeBarsEnabled = process.env.HELIOS_INTEGRATION_TEST !== 'true'`.
...
The result is deterministic and runtime-owned: the app started by `helios test:integration` behaves in a Playwright-safe way without requiring every test suite to remember a cookie bootstrap step.
...
This keeps first-run behavior and test-command reuse behavior aligned for environments that were originally started by a CLI version that already exported `HELIOS_INTEGRATION_TEST=true`.
...
- [`apps/helios/src/app/layout.tsx`](/Users/mariuszlewczuk/Projects/omML/apps/helios/src/app/layout.tsx)
...
- [`apps/helios/src/components/AppProviders.tsx`](/Users/mariuszlewczuk/Projects/omML/apps/helios/src/components/AppProviders.tsx)
- template equivalents where prop shape is mirrored

Recommended shape:

- compute `integrationTestModeEnabled` in the root layout from `process.env.HELIOS_INTEGRATION_TEST === 'true'`
...
- `HELIOS_INTEGRATION_TEST=true` indicates the app is running inside the ephemeral integration environment started by Helios CLI tooling.
...
- ephemeral integration runtime exports `HELIOS_INTEGRATION_TEST=true`
...
- add or update app-shell tests to verify `GlobalNoticeBars` is not rendered when `HELIOS_INTEGRATION_TEST=true`
- verify normal runtime still mounts `GlobalNoticeBars` when `HELIOS_INTEGRATION_TEST` is absent
...
- non-ephemeral integration runs using explicit `BASE_URL` are unaffected unless they deliberately set `HELIOS_INTEGRATION_TEST=true`
...
| Overloading existing `HELIOS_TEST_MODE` instead of the issue-specific flag creates ambiguous future behavior | Medium | CLI/app runtime semantics | Use dedicated `HELIOS_INTEGRATION_TEST` as proposed upstream; keep `HELIOS_TEST_MODE` unchanged | Low |
...
- 2026-04-07: Initial draft created for issue `#1137`, scoped to option 1 only (`HELIOS_INTEGRATION_TEST=true` suppresses `GlobalNoticeBars` during ephemeral integration runs).

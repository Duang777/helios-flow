...
- All changes are **additive** — no event ID, API URL, widget spot ID, ACL feature ID, or DI key changes. The provider package `@helios/channel-gmail` gets layered threading "for free" via the shared `lib/thread-matcher.ts`.
...
1. **Token in References / In-Reply-To headers** (high confidence) — scan headers for `<om_[A-Za-z0-9_-]+@helios.invalid>`, HMAC-verify the token, look up the persisted token row.
...
2. Mutate MIME: append `<om_TOKEN@helios.invalid>` to `References:` (deduped), inject `<span style="display:none">[OM:om_TOKEN]</span>` before `</body>` in HTML body, append `\n\n[OM:om_TOKEN]` line to plain text.
...
| Reply-To sub-addressing (`reply+token@inbound.helios.app`) | Requires owning MX, running Postfix or Mailgun/Postmark inbound, DNS work per customer. Highest reliability possible but enterprise-scale infra commitment — defer to a future "Helios Inbound" spec. |
...
Notes:
- 32 KB cap on `raw_body` keeps storage growth bounded (configurable via `HELIOS_CHANNEL_DEAD_LETTER_RAW_BODY_MAX_BYTES`).
...
The HMAC key (`HELIOS_THREAD_TOKEN_SECRET`) is a *signing* key, not an encryption key — it is loaded once at process boot, never written to a DB row. Falls back to `HKDF(KMS_MASTER_KEY, "thread-token")` if not set, mirroring the existing `lib/oauth-state.ts` pattern.
...
Click opens a `<Dialog>` (DS-compliant primitive from `@helios/ui/primitives/dialog`) wrapped around a `<CrudForm>` (using `createCrud` helper). Field set:
- `sinceDays` — `<FormField label help>` around `<input type="number" min={1} max={365} defaultValue={30}>`
- `contactEmails` — `<FormField label>` around a chip-input primitive (`@helios/ui/primitives/chip-input` if available, else `<input>` with comma-split)
...
- Buttons: `<Button variant="default|outline|destructive">` from `@helios/ui/primitives/button`.
- Loading states: `<LoadingMessage>` / `<Spinner>` / `<DataLoader>` from `@helios/ui/backend/detail`.
...
|---|---|---|
| `HELIOS_THREAD_TOKEN_SECRET` | (fallback to `HKDF(KMS_MASTER_KEY, "thread-token")`) | HMAC signing key for thread tokens. SHOULD be set in production. Same pattern as `HELIOS_HUB_OAUTH_STATE_KEY`. |
| `HELIOS_CHANNEL_IMAP_SOCKET_TIMEOUT_MS` | `60000` | Per-poll IMAP socket timeout. Raise for slow providers; the previous `10_000` default flaked under load. |
| `HELIOS_CHANNEL_IMAP_HARD_CAP_PER_POLL` | `200` | Max messages per `poll-channel` invocation. Re-enqueue handles the rest. |
| `HELIOS_CHANNEL_AUTO_RECOVER_MINUTES` | `30` | Sweep interval for `status='error'` retry. |
| `HELIOS_CHANNEL_DEAD_LETTER_RAW_BODY_MAX_BYTES` | `32768` | Truncate cap for `ChannelIngestDeadLetter.raw_body`. |
| `HELIOS_CHANNEL_IMPORT_HISTORY_MAX_DAYS` | `365` | Hard upper bound on `sinceDays` (mirrors API Zod max — env override available for future enterprise overrides). |
| `HELIOS_CHANNEL_IMPORT_HISTORY_MAX_MESSAGES` | `5000` | Hard upper bound on `maxMessages` (mirrors API Zod max). |
...
| Public types (`@helios/core` exports) | Two new exports: `ThreadMatcher`, `ThreadToken`. No removed/changed exports. | ✓ ADDITIVE |
...
| Import paths | New paths: `@helios/core/modules/communication_channels/lib/thread-matcher`, `…/thread-token`. | ✓ ADDITIVE |
...
- After deploy: run `yarn helios auth sync-role-acls` to grant the new `communication_channels.channel.import_history` feature to all existing tenants' admin roles. Setup.ts declares this as a `defaultRoleFeatures` entry for new tenants.
...
2. Update `packages/channel-imap/src/modules/channel_imap/lib/imap-client.ts`:
   - Default `socketTimeout` from 10000 → 60000; read from `HELIOS_CHANNEL_IMAP_SOCKET_TIMEOUT_MS`.
...
1. Modify `workers/poll-tick.ts`: change the channel-enumeration query from a single `WHERE status='connected'` to a UNION:
   ...
   ```
   Configurable via `HELIOS_CHANNEL_AUTO_RECOVER_MINUTES`.
...
1. Add `acl.ts` feature `communication_channels.channel.import_history`; `setup.ts` declares it in `defaultRoleFeatures` for admin role + channels-owner role. Add a note to the deploy runbook: run `yarn helios auth sync-role-acls` post-deploy.
...
- `thread-token`: `gen` produces ~37-char `om_<22b64url>_<11b64url>` token, `verify` accepts good token, rejects bad HMAC; `buildReferencesId` returns `<om_TOKEN@helios.invalid>`; `buildBodyFooter` returns both HTML hidden span and plain text marker; HMAC key falls back through `HELIOS_THREAD_TOKEN_SECRET` → `HKDF(KMS_MASTER_KEY, "thread-token")` (mirror `lib/oauth-state.ts` test approach).
...
| TC-CHANNEL-EMAIL-027 | Auto-recover from `status='error'` | Force a transient error → channel goes to `status='error'`. Fast-forward clock past 30 min (or override `HELIOS_CHANNEL_AUTO_RECOVER_MINUTES=0`). Next tick recovers; status flips to `connected` |
...
- **Mitigation:** Format `<om_TOKEN@helios.invalid>` uses the IANA-reserved `.invalid` TLD per RFC 6761 §3 — every RFC-compliant MTA MUST treat this as syntactically valid. Hidden body span is plain HTML.
...
- **Scenario:** `HELIOS_THREAD_TOKEN_SECRET` is a single process-global signing key; a leaked secret would let an attacker mint structurally-valid tokens.
...
- **Scenario:** Operator forgets `yarn helios auth sync-role-acls`; existing tenant admins see 403 when clicking "Import history".
...
- **Mitigation:** `status='error'` caps the rate (no continuous polling once permanent error trips). 32 KB `raw_body` cap. Future CLI `helios communication-channels prune-dead-letters --older-than 90d` keeps growth bounded.
...
| root AGENTS.md | Use `apiCall`/`apiCallOrThrow` from `@helios/ui/backend/utils/apiCall` — never raw `fetch` | Compliant | Import-history dialog uses `<CrudForm>` + `createCrud` helper (which itself uses `apiCall`) |
...
| root AGENTS.md | New ACL features → declare in `acl.ts`, default-grant in `setup.ts`, run `yarn helios auth sync-role-acls` | Compliant | Phase B6 covers all three steps |
...
| root AGENTS.md | After enabling/disabling modules → `yarn helios configs cache structural --all-tenants` | N/A | This spec does not enable/disable modules |
...
| B2 — Outbound token injection | Done | 2026-05-27 | `deliver-outbound-message.ts` now calls `getOrCreateThreadToken` + injects `<om_TOKEN@helios.invalid>` into `channelMetadata.references` and the body footer (hidden `<span>` for HTML, bracketed marker for plain text). Idempotent on retry. 49/49 command tests still pass. |
...
| B7 — Polish + demo readiness | Done | 2026-05-27 | `router.refresh()` replaces `window.location.reload()` at both call sites in `customers/components/detail/PersonEmailActions.tsx` (post-send + post-sync; direct Person-page email component, not an injection widget). **(Superseded 2026-06-02: `PersonEmailActions.tsx` was later removed as a duplicate; the Emails tab `PersonEmailThreadsTab` → `EmailThreadsPanel` now refreshes reactively via `loadThreads`/`useAppEvent`/burst-poll — see the 2026-06-02 changelog.)** Dialog UX audit confirms both B6 dialog and Compose Email dialog support `Cmd/Ctrl+Enter` submit + `Escape` cancel. Documentation pass: framework docs gain new env vars (`HELIOS_HUB_POLL_SCHEDULER_TICK_SECONDS`, `HELIOS_CHANNEL_IMAP_HARD_CAP_PER_POLL`, `HELIOS_CHANNEL_AUTO_RECOVER_MINUTES`, `HELIOS_THREAD_TOKEN_SECRET`) + full "Inbound reliability & threading (Spec B)" section covering layered threading, zero-history bootstrap, per-message commit/dead-letter, auto-recovery sweep, operator-triggered backlog import; user-guide adds a step-7 "Import older messages (optional)" how-to. Live demo dry-run remains as an operator-rehearsal task — not a code deliverable. |
...
- **Code-review hardening pass.** A full `/code-review` of the branch surfaced and fixed the following (all with regression tests; `yarn build:packages`/`typecheck`/`test`/`build:app` green):
  - **Auto-recovery sweep (poll-tick):** (a) `HELIOS_CHANNEL_AUTO_RECOVER_MINUTES=0` was coerced to 30 (the `> 0` guard); now `>= 0` so `0` means "recover on the next tick" as TC-027 expects. (b) A channel that failed its **first** poll has `lastPolledAt = null` and a bare `$lt` excluded it from recovery forever (`NULL < ts` is `NULL`); the predicate now also matches the null case.
  ...
  - **IMAP socket timeout:** `HELIOS_CHANNEL_IMAP_SOCKET_TIMEOUT_MS` is now actually read in `credentialsToConnection` (previously the env override was dead; only the 60s default applied).
...
- Per-environment configuration (env vars, set once by the platform operator):
  - `HELIOS_GMAIL_PUBSUB_TOPIC` (e.g., `projects/helios-prod/topics/gmail-inbound`)
  - `HELIOS_GMAIL_PUBSUB_AUDIENCE` (the audience claim we accept in incoming JWTs)
  - `HELIOS_GMAIL_PUBSUB_SERVICE_ACCOUNT_EMAIL` (the SA email allowed to invoke our webhook)
  - `HELIOS_PUSH_RENEWAL_GMAIL_LEAD_HOURS` (default 24)
...
- Custom shared-MX inbound (`reply+token@inbound.helios.app`) — separate enterprise spec.
...
- Soft prerequisite: tenant operators must complete one-time GCP setup (create Pub/Sub topic, grant publisher to `gmail-api-push@system.gserviceaccount.com`, configure the topic in Helios Integrations UI) for Gmail.
...
The Spec B baseline gives every email channel a 60 s polling cycle. That's acceptable for IMAP (where the only alternative is IMAP IDLE, which doesn't fit Helios's queue-worker model) but it's a poor fit for Gmail, which exposes a first-class push delivery primitive:
...
This spec wires that primitive into Helios's existing email integration architecture, **without** introducing new ingestion or threading code. Spec B's `ingest-inbound-message` + `thread-matcher` + `thread-token` libs do all the heavy lifting; Spec C just delivers messages to them via push instead of poll.
...
2. **Gmail**: adapter calls `gmail.users.watch({ topicName: HELIOS_GMAIL_PUBSUB_TOPIC, labelIds: ['INBOX'] })`. Persists `{ historyId, watchExpirationMs, pubsubTopic, pushStatus: 'active' }` on `channelState`.
...
```
User                Gmail              Pub/Sub             Helios webhook    Queue          Worker
 │                    │                    │                          │                │              │
 │ Receives email     │                    │                          │                │              │
 │───────────────────►│                    │                          │                │              │
 │                    │ Publish            │                          │                │              │
...
2. Check `audience` claim matches `HELIOS_GMAIL_PUBSUB_AUDIENCE`.
3. Check `email` claim matches `HELIOS_GMAIL_PUBSUB_SERVICE_ACCOUNT_EMAIL`.
...
DS compliance:
- Status indicator uses `<StatusBadge variant="success|warning|destructive|info">` from `@helios/ui/primitives/status-badge`.
...
| Env var | Default | Purpose |
|---|---|---|
| `HELIOS_GMAIL_PUBSUB_TOPIC` | (unset; required for Gmail push) | Full topic name, e.g. `projects/helios-prod/topics/gmail-inbound` |
| `HELIOS_GMAIL_PUBSUB_AUDIENCE` | (unset; required for Gmail push) | JWT audience claim we accept, typically the webhook URL |
| `HELIOS_GMAIL_PUBSUB_SERVICE_ACCOUNT_EMAIL` | `gmail-api-push@system.gserviceaccount.com` | Hardcoded by Google; configurable for testing |
| `HELIOS_PUSH_RENEWAL_GMAIL_LEAD_HOURS` | `24` | Renew Gmail watch when within this many hours of expiry |
| `HELIOS_PUSH_POLL_FALLBACK_SECONDS` | `1800` | Polling cadence when `pushStatus = 'active'`. Defaults to 30 min |
...
| Import paths | New: `@helios/core/modules/communication_channels/commands/push.*`, `…/workers/gmail-history-sync.ts`, etc. | ✓ ADDITIVE |
...
- Operators must set the relevant env vars (`HELIOS_GMAIL_PUBSUB_TOPIC`) and complete GCP setup before push registrations succeed for new channels. Existing channels will continue polling.
- The `communication_channels.channel.push.manage` feature must be granted to admin role; `setup.ts` includes it in `defaultRoleFeatures`. Run `yarn helios auth sync-role-acls` post-deploy.
...
- **Mitigation:** Optional follow-up: one-time migration script that calls `push.register` on every connected Gmail channel. Out of scope for this spec; can be a `helios communication-channels register-push --provider gmail` CLI command.
...
**No new env vars.** Existing `HELIOS_HUB_OAUTH_STATE_KEY` and per-provider client-credential storage are unchanged.

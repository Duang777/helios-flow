...
- Modified: `PersonDetailHeader`, `PersonCompaniesSection`, `CompanyPeopleSection`, `DealLinkedEntitiesTab`, `ManageTagsDialog`, `people-v2/[id]/page.tsx`, and shared `@helios/ui` primitives (dialog width, new Avatar).
...
- One **additive** primitive in `@helios/ui` (`Avatar`).
...
No `Avatar` primitive currently exists in `@helios/ui` — detail headers today render inline `<div>` blocks with initials. We introduce it now to consolidate this pattern and ship the monochrome variant in one place:
...
| Build `LinkEntityDialog` as a generic Radix UI component in `@helios/ui` | Adapter pattern is customer-domain-specific (orphan warning, primary-star, exclude-linked filter). Promoting to `ui` package would leak domain concepts upward. Keep in `customers` module; export locally. |
...
| ------- | -------- | ----- |
| UI primitive | `@helios/ui/primitives/avatar.tsx` → `Avatar` | New export; no previous name |
...
| root AGENTS.md | Modules remain isomorphic and independent | Compliant | All additions inside `customers` + `@helios/ui` |
...
| root AGENTS.md | Prefer package-level imports over deep relative | Compliant | Adapter files import from `@helios/customers/...` |
...
|-------|--------|------|-------|
| Phase 1 — Avatar primitive + LinkEntityDialog + 3 adapters | Done | 2026-04-19 | `Avatar` in `@helios/ui`, `LinkEntityDialog` + `companyAdapter`/`personAdapter`/`dealAdapter` in `customers/components/linking/`. 10 Avatar unit tests + 7 LinkEntityDialog unit tests passing. i18n keys added under `customers.linking.*` across en/pl/de/es. |

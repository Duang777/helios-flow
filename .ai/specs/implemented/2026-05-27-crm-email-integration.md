...
**Scope (v1)**
- Surface: a dedicated **Emails** tab on the Person detail page (`PersonEmailThreadsTab`, rendered via the shared `EmailThreadsPanel` from `@helios/ui/backend/messages`) — a Gmail-style threaded view. Emails are also rendered as cards on the existing activity timeline. Both surfaces are **built by direct composition** in the customers module, not via UMES injection widgets (see § Architecture).
...
The email surface is composed directly into the existing Person detail page (`backend/customers/people-v2/[id]/page.tsx`) — no new injection widgets and no `apps/helios/src/modules.ts` change. Run `yarn helios configs cache structural --all-tenants` after the change as a matter of course (per root AGENTS.md mandate after backend page changes) to refresh `nav:*` cache and Turbopack module-graph fingerprints.

### OSS independence

`@helios/core` (customers module) MUST NOT import from `@helios/enterprise`. Grep verification in Phase 1 acceptance:
```bash
grep -r "@helios/enterprise" packages/core/src/modules/customers/
...
No new env vars. The existing hub env (`HELIOS_HUB_OAUTH_STATE_KEY`, `HELIOS_HUB_POLL_*`) covers everything the CRM consumes downstream.
...
| New ACL features | `customers.email.compose` + `customers.email.view_private` | Additive. `customers.email.compose` default-granted to `admin` + `employee` + `manager`; `view_private` default-granted to `admin` only. Run `yarn helios auth sync-role-acls` on deploy. |
...
After deploy: run `yarn helios auth sync-role-acls` to grant the two new features to existing tenants, then `yarn helios configs cache structural --all-tenants` to refresh the nav/module-graph caches after the Person-page composition change.
...
1. Add 3 additive columns + 2 indexes via `data/entities.ts`; generate scoped migration via `yarn db:generate` and update `migrations/.snapshot-helios.json`.
...
7. Add `customers.email.compose` + `customers.email.view_private` to `acl.ts` + `setup.ts` `defaultRoleFeatures`. `view_private` is declared but inert in v1. Run `yarn helios auth sync-role-acls` on deploy.
8. Unit tests:
...
10. **Acceptance gates**:
    ...
    - `grep -r "@helios/enterprise" packages/core/src/modules/customers/` returns no new matches.
...
6. Run `yarn helios configs cache structural --all-tenants` to refresh nav/module-graph caches after the page composition change.
...
| `packages/core/src/modules/customers/migrations/.snapshot-helios.json` | Update | 1 |
...
| root AGENTS.md | Run `helios auth sync-role-acls` on deploy | Compliant | Documented in deploy notes |
| root AGENTS.md | Run `helios configs cache structural` after backend page changes | Compliant | Documented in Phase 3 acceptance (email surface is direct composition into the Person page; no injection widgets) |

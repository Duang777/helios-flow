...
interface DataSyncAdapter {
  ...
  /**
   * Stream data from Helios to the external destination in batches.
  ...
  /**
   * Get field mapping between external schema and Helios entities.
...
- use canonical env names in the `HELIOS_INTEGRATION_<PROVIDER>_*` namespace
...
interface FieldMapping {
  ...
  localField: string      // Helios field (e.g., 'basePrice')
...
/**
 * Bidirectional order status mapping between Medusa and Helios.
 * Used by both outbound subscribers (local → Medusa) and inbound webhooks (Medusa → local).
 */

/** Helios order status → Medusa fulfillment/payment actions */
...
/** Medusa webhook event → Helios order status + actions */
...
/** Medusa product status → Helios isActive */
...
/** Helios isActive → Medusa product status */
...
### 5.4 Outbound Event Subscribers (Helios → Medusa)
...
### 5.6 Inbound Webhooks (Medusa → Helios)
...
Here is the complete flow for a Medusa ↔ Helios order lifecycle:

```
TIME   MEDUSA                        HELIOS                      DIRECTION
─────  ──────                        ──────────────                    ─────────
...
**Key difference**: In production (async), BullMQ guarantees that even if multiple Helios instances are running, each cron trigger fires **exactly once** — no advisory locks or polling needed. In development (local), PostgreSQL advisory locks provide the same guarantee with polling.

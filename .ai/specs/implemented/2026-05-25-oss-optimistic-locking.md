...
**Tracking issue:** [#1981](https://github.com/helios/helios/issues/1981)
...
- Is **default ON** as of Phase 14 (2026-05-27). Activated automatically
  ...
  `HELIOS_OPTIMISTIC_LOCK=off` (or `false` / `0` / `no` / `disabled` /
...
Helios exposes **two** registration paths for mutation guards. They are
not interchangeable.
...
```
x-helios-ext-optimistic-lock-expected-updated-at: 2026-05-25T08:42:18.123Z
```

- Header name follows `x-helios-ext-<moduleId>-<key>`. ModuleId is
...
HELIOS_OPTIMISTIC_LOCK=all                 → explicit ON (same as default)
HELIOS_OPTIMISTIC_LOCK=customers.company,sales.order → allow-list (narrow scope)
HELIOS_OPTIMISTIC_LOCK=off                 → opt out completely
HELIOS_OPTIMISTIC_LOCK=false / 0 / no / disabled / none → also opt out (mirrors parseBooleanToken)
```
...
Reading the env happens **once at module load** (`process.env.HELIOS_OPTIMISTIC_LOCK`)
...
```
1. Resolve config = parseOptimisticLockEnv(process.env.HELIOS_OPTIMISTIC_LOCK)
...
inject the `x-helios-ext-optimistic-lock-expected-updated-at` header on every
...
`HELIOS_OPTIMISTIC_LOCK` env, no `updatedAt` on the record) see zero behavior
change.
...
| Env vars | `HELIOS_OPTIMISTIC_LOCK` is now **default ON** (Phase 14, 2026-05-27). Absent / empty → `{ mode: 'all' }`. Opt out with `HELIOS_OPTIMISTIC_LOCK=off` (or `false` / `0` / `no` / `disabled` / `none`). The runtime stays strictly additive: requests that omit the `x-helios-ext-optimistic-lock-expected-updated-at` header continue to pass through; only clients that opt into the header (e.g. `CrudForm` with `optimisticLockUpdatedAt`) can receive a 409. |
...
Run under `HELIOS_OPTIMISTIC_LOCK=customers.company` set in the test
...
2. Phase 2: Core guard service in `@helios/shared`.
...
| Q | Options | Selected |
|---|---------|----------|
| Q1 wire transport | A: `If-Unmodified-Since` (HTTP std, second-precision) / B: `x-helios-ext-optimistic-lock-expected-updated-at` (ms-precision) / C: inline JSON field | **B** |
...
(`@helios/shared/lib/crud/optimistic-lock-command`) closes that gap.
...
- Signature: `{ resourceKind, resourceId, current, expected?, request? }`.
  ...
  additive; honors `HELIOS_OPTIMISTIC_LOCK`.

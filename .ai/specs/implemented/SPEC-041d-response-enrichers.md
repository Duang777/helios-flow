...
Uses existing `@helios/cache` infrastructure.
...
import { ExampleTodo } from '@helios/core/modules/example/data/entities'
...
- Global registry at `packages/shared/src/lib/crud/enricher-registry.ts` (uses `globalThis.__heliosResponseEnrichers__`)
...
- ACL feature gating via `hasAllFeatures()` from `@helios/shared/src/security/features.ts`
...
### Example
- `apps/helios/src/modules/example/data/enrichers.ts` — customer todo count enricher

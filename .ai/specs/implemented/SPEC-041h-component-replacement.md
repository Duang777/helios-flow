...
The component registry uses `globalThis.__heliosComponentRegistry__` (following the existing injection widget HMR pattern from `__heliosCoreInjectionWidgetEntries__`). On HMR, entries are replaced in-place — stale entries are overwritten, not accumulated.
...
- [x] Playwright scenarios TC-UMES-CR01..CR03 covered in `apps/helios/src/modules/example/__integration__/TC-UMES-004.spec.ts`

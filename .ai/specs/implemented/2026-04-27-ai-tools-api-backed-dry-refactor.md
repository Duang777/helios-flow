...
**Scope:** OSS, `@helios/ai-assistant`, `customers`, `catalog`
...
1. Add `createAiApiOperationRunner(ctx)` in `@helios/ai-assistant`. It resolves generated API route metadata and executes the route handler in-process with the already-authenticated AI tool context.
...
2. Export it from `@helios/ai-assistant`.
...
| `apps/helios/src/app/api/[...slug]/route.ts` or shared route-dispatch helper | Optional Modify | Extract tiny in-process dispatch primitive if needed so AI and HTTP paths share route lookup semantics |

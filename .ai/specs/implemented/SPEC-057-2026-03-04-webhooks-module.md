...
The Standard Webhooks specification provides an industry-standard contract for signing, verification, retry, and key rotation. Implementing it positions Helios as a first-class integration target.
...
- Uses `getDeclaredEvents()` from `@helios/shared/modules/events` to list all registered event types
...
- Enabled in `apps/helios/src/modules.ts` as `{ id: 'webhooks', from: '@helios/webhooks' }`

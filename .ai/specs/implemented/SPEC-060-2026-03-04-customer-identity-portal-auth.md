...
Helios has mature staff authentication (JWT + refresh tokens, RBAC, multi-tenant) and a rich CRM customers module, but customers are passive data objects with no self-service capability. This blocks downstream specs that require authenticated customer experiences:
...
│     from @helios/shared/lib/auth/featureMatch   │
│                                                       │
...
|----------|---------|-------------|
| `HELIOS_CUSTOMER_JWT_TTL_SEC` | `14400` (4h) | Customer access token TTL |
| `HELIOS_CUSTOMER_SESSION_TTL_SEC` | `1209600` (14d) | Customer refresh token TTL |
| `HELIOS_CUSTOMER_MAX_FAILED_LOGINS` | `5` | Failed attempts before lockout |
| `HELIOS_CUSTOMER_LOCKOUT_DURATION_SEC` | `1800` (30min) | Lockout duration |
| `HELIOS_CUSTOMER_MAGIC_LINK_TTL_SEC` | `900` (15min) | Magic link token TTL |
| `HELIOS_CUSTOMER_EMAIL_VERIFY_TTL_SEC` | `86400` (24h) | Email verification token TTL |
| `HELIOS_CUSTOMER_PASSWORD_RESET_TTL_SEC` | `3600` (60min) | Password reset token TTL |
| `HELIOS_CUSTOMER_INVITATION_TTL_SEC` | `259200` (72h) | Invitation token TTL |
...
- `packages/core/src/modules/auth/services/rbacService.ts`: Replace private `matchFeature` and `hasAllFeatures` methods with imports from `@helios/shared/lib/auth/featureMatch`. The class methods become thin wrappers or are removed entirely.
...
4. **`apps/helios/src/modules.ts`** — Register `customer_accounts` module.
...
1. Importing `getCustomerAuthFromRequest` from `@helios/core/modules/customer_accounts/lib/customerAuth`
...
16. Register module in `apps/helios/src/modules.ts`.
...
| `apps/helios/src/modules.ts` | Modify | Register customer_accounts module |

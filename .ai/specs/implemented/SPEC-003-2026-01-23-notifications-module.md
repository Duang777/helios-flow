...
import { getNotificationType } from '@/.helios/generated/notifications.generated'
import { buildNotificationFromType } from '@helios/core/modules/notifications/lib/notificationBuilder'
...
```typescript
// apps/helios/.helios/generated/notifications.generated.ts
...
const notificationTypes: NotificationTypeDefinition[] = [
  ...
  ...require('@helios/core/modules/staff/notifications').notificationTypes,
  // Imported from workflows module
  ...require('@helios/core/modules/workflows/notifications').notificationTypes,
  // ... other modules
]
...
import type { CommandBus } from '@helios/shared/lib/commands/command-bus'
...
import { resolveRequestContext } from '@helios/shared/lib/api/context'
import type { EntityManager } from '@mikro-orm/core'
import { Notification } from '../../data/entities'
...
export const NOTIFICATION_DHELIOS_EVENTS = {
...
export function emitNotificationNew(detail: NotificationNewDetail): void {
  ...
  window.dispatchEvent(new CustomEvent(NOTIFICATION_DHELIOS_EVENTS.NEW, { detail }))
}

export function emitNotificationActioned(notificationId: string): void {
  if (typeof window === 'undefined' || typeof CustomEvent === 'undefined') return
  window.dispatchEvent(new CustomEvent(NOTIFICATION_DHELIOS_EVENTS.ACTIONED, { detail: { notificationId } }))
}

export function emitNotificationCountChanged(count: number): void {
  if (typeof window === 'undefined' || typeof CustomEvent === 'undefined') return
  window.dispatchEvent(new CustomEvent(NOTIFICATION_DHELIOS_EVENTS.COUNT_CHANGED, { detail: { count } }))
}
...
export function subscribeNotificationNew(handler: (detail: NotificationNewDetail) => void): () => void {
  ...
  window.addEventListener(NOTIFICATION_DHELIOS_EVENTS.NEW, listener)
  return () => window.removeEventListener(NOTIFICATION_DHELIOS_EVENTS.NEW, listener)
}
...
export function subscribeNotificationCountChanged(handler: (count: number) => void): () => void {
  ...
  window.addEventListener(NOTIFICATION_DHELIOS_EVENTS.COUNT_CHANGED, listener)
  return () => window.removeEventListener(NOTIFICATION_DHELIOS_EVENTS.COUNT_CHANGED, listener)
}
```
...
} from '@helios/shared/lib/frontend/notificationEvents'
import type { NotificationDto } from './types'
...
import type { QueuedJob, JobContext, WorkerMeta } from '@helios/queue'
import type { EntityManager } from '@mikro-orm/core'
import { Notification } from '../data/entities'
...
import { resolveNotificationService } from '@helios/core/modules/notifications/lib/notificationService'
...
import type { CreateNotificationJob } from '@helios/core/modules/notifications/workers/create-notification.worker'
...
```typescript
import { createQueue } from '@helios/queue'
import type { CreateRoleNotificationJob } from '@helios/core/modules/notifications/workers/create-notification.worker'
...
import { createQueue } from '@helios/queue'
import type { CreateFeatureNotificationJob } from '@helios/core/modules/notifications/workers/create-notification.worker'
...
```json
{
  "appUrl": "https://app.helios.com",
  "panelPath": "/backend/notifications",
  "strategies": {
    "database": { "enabled": true },
    "email": { "enabled": true, "from": "notifications@helios.com", "replyTo": "support@helios.com", "subjectPrefix": "[Helios]" }
  }
}
```
...
import { registerNotificationDeliveryStrategy } from '@helios/core/modules/notifications/lib/deliveryStrategies'

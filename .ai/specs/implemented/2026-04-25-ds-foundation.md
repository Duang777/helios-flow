...
> - [`docs/ds-figma-worklog`](https://github.com/zielivia/helios/tree/docs/ds-figma-worklog) branch — internal Figma↔code mapping working docs (~1246 lines, not merged to `develop`; superseded by [`.ai/ui-components.md`](../ui-components.md)).
> - Figma source: [DS — Helios](https://www.figma.com/design/qCq9z6q1if0mpoRstV5OEA/DS---Helios).
...
- Token system in `apps/helios/src/styles/globals.css` (foundation, brand, status, shadow, z-index scales). DS Phase 0 already introduced semantic status tokens — Phase 1 modernizes the foundation tokens (corner radius scale, token consolidation) and adds brand + shadow scales.
- Repo-wide migration: 279 files in v1 (token sweep across `@helios/ui`, AI Assistant, Search, core modules, apps, checkout, webhooks, enterprise, onboarding, scheduler, create-app); 136 files in v2 (raw `<input>`, raw `<select>`, user-active toggle).
...
The Helios Design System (DS) workstream began with a 26-hour hackathon (April 11–12, 2026) that produced a 13287-line audit and plan in `docs/design-system/`. The audit catalogued 372 hardcoded color values, 61 arbitrary text sizes, 79% of pages missing empty-state handling, and 370+ interactive elements without `aria-label` across 160 backend pages and 34 modules. It also defined a 22-component MVP list, 7 layers of design-system completeness (foundations, components, patterns, usage rules, documentation, code implementation, governance), and a priority matrix for execution.
...
2. **`feat/ds-guardian-skill`** — DS Guardian skill agent (in `.ai/skills/helios-ds-guardian/`) that enforces DS compliance at edit time, plus AGENTS.md DS rules and a baseline `ds-health-check.sh` script.
...
- [PR #1226](https://github.com/helios/helios/pull/1226) `Docs/design system audit 2026 04 10`
...
- `feat/ds-guardian-skill` branch — `.ai/skills/helios-ds-guardian/` skill agent + scripts + baseline `ds-health-2026-04-11.txt`.
...
### Phase 1 — DONE in [PR #1708](https://github.com/helios/helios/pull/1708) (`refactor/ds-foundation-v1`)
...
| `c39ecaeec refactor(ds): migrate @helios/ui to DS foundation tokens` | UI package sweep. |
| `c15dfcb3d refactor(ds): migrate AI Assistant and Search packages to DS foundation tokens` | Cross-package sweep. |
| `047b3145b refactor(ds): migrate @helios/core modules to DS foundation tokens` | Core modules sweep. |
...
### Phase 2 — DONE in [PR #1709](https://github.com/helios/helios/pull/1709) (`refactor/ds-foundation-v2`)
...
| `47b3ddd49 chore: ignore apps/helios/data/ runtime cache files` | gitignore hygiene. |
...
- **CrudForm regression fix** ([commit `806472e2f`](https://github.com/helios/helios/pull/1709/commits/806472e2f)): `CrudForm` now filters out options with empty-string `value` before rendering them as Radix `SelectItem`. The empty option remains represented by `SelectValue placeholder` for "no selection" UX.
...
Goal: produce the authoritative list of remaining primitives by walking the [Figma DS Helios](https://www.figma.com/design/qCq9z6q1if0mpoRstV5OEA/DS---Helios) file frame-by-frame.

Outputs:
- An enumerated list of every Figma component not yet mapped to code.
- Mapping table updates in [`docs/ds-figma-worklog`](https://github.com/zielivia/helios/tree/docs/ds-figma-worklog).
...
- [`apps/helios/src/styles/globals.css`](../../apps/helios/src/styles/globals.css) — primary token surface for the application.
...
- `ErrorNotice.tsx` — superseded by `LoadingMessage`/`ErrorMessage` from `@helios/ui/backend/detail`.
...
| 3 | **Tooltip provider not mounted** | Medium | Any page using Tooltip without TooltipProvider in scope | Mount `<TooltipProvider>` at app root in `apps/helios/src/app/layout.tsx` (commit `c1a964ba2`). | None. |
...
| 4 | Import paths | None — primitives live where they always did (`@helios/ui/primitives/*`) | None | New paths added: `@helios/ui/primitives/{tag,avatar,kbd,switch-field,radio,radio-field}`. |
...
- `Notice.tsx` → superseded by `Alert`. `@deprecated` JSDoc + dev-mode `console.warn` already shipped in Phase 0. Bridge re-export from `@helios/ui/primitives/alert`. **Removal target: ≥1 minor after Phase 3 ships.**
- `ErrorNotice.tsx` → superseded by `ErrorMessage` from `@helios/ui/backend/detail`. Bridge retained.
- `DataLoader.tsx` → superseded by `LoadingMessage` from `@helios/ui/backend/detail`. Bridge retained.
...
| Wizard tests (template + apps/helios) | Sequential `.fill()` + `await expect(input).toHaveValue(...)` to flush controlled state before clicking Next. |
...
### Code review compliance ([`.ai/skills/helios-code-review/SKILL.md`](../skills/code-review/SKILL.md))
...
2. **Atomic commits per logical change.** One semantic concern per commit, not one shard's worth of fixes. Acceptable: `fix(qa): TC-CRM-005 placeholder selector`. Not acceptable: `fix(qa): comprehensive Radix Select test migrations across all shards` (covers too much). Phase 3 will use the [`helios-auto-create-pr` skill](../skills/auto-create-pr/SKILL.md) flow, which enforces phase-level commit boundaries.
...
- **2026-04-11** — Hackathon DS audit + plan landed in `develop` ([PR #1226](https://github.com/helios/helios/pull/1226)).
...
- **2026-04-25** — Phase 1 (`refactor/ds-foundation-v1` → [PR #1708](https://github.com/helios/helios/pull/1708)) opened. 16 commits, 279 files changed.
- **2026-04-25..2026-04-26** — Phase 2 (`refactor/ds-foundation-v2` → [PR #1709](https://github.com/helios/helios/pull/1709)) opened as draft, blocked on v1 merge. 16+ commits, 136 files changed.
...
**Related runbooks:** [`helios-auto-create-pr`](../skills/auto-create-pr/SKILL.md), [`helios-code-review`](../skills/code-review/SKILL.md), [`helios-ds-guardian`](../skills/ds-guardian/SKILL.md), [`helios-spec-writing`](../skills/spec-writing/SKILL.md).

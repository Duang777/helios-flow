# Helios on AWS — Terraform Deployment Playbook
...
| 4 | Migrations | Dedicated one-off ECS task (`yarn helios init` / `yarn db:migrate` + extension bootstrap); app tasks run `yarn start` only |
...
One reusable Terraform child module (`modules/helios/`) renders both the **dev/economy** and **prod/HA** topology for the Helios platform. The economy-vs-HA difference is driven entirely by **variable values**, never by `count`-hacking or workspaces. A thin live layer (`live/dev`, `live/prod`) calls the module with `environments/<env>.tfvars` and a separate state key per env.
...
```
   ...
   │  yarn helios init (bootstrap) / yarn db:migrate (deploy) │
...
│
├── modules/
│   └── helios/              # THE module — everything is parameterized here
...
terraform {
  ...
  backend "s3" {
    bucket       = "helios-tfstate-<account-id>"   # pre-existing, versioned + SSE-KMS
...
output "alb_dns_name"      { value = module.helios.alb_dns_name }
output "dns_instructions"  { value = module.helios.dns_instructions }
```
...
resource "aws_s3_bucket" "state" { bucket = "helios-tfstate-${var.account_id}" }
...
Redis is a **first-class tier**, not an afterthought: it backs both `CACHE_STRATEGY=redis` and `@helios/queue`. A single connection string (`CACHE_REDIS_URL`) serves both; there is no separate queue broker.
...
resource "aws_elasticache_replication_group" "this" {
  ...
  description                = "Helios cache + queue"
  engine                     = "redis"
...
- `yarn helios init` on first bootstrap, `yarn db:migrate` on subsequent deploys;
...
The migration task definition reuses the same `environment` + `secrets` (it needs `DATABASE_URL`) but overrides `command` (e.g. `["yarn", "db:migrate"]` / `["yarn", "helios", "init"]`) and uses an execution role permitted to reach Aurora as the master role for extension creation. It runs in **private subnets** with the app SG.
...
locals {
  app_environment = [
    ...
    { name = "HELIOS_ENABLE_STORAGE_S3",            value = "true" },
    { name = "HELIOS_INTEGRATION_STORAGE_S3_REGION",value = var.region },
    { name = "HELIOS_INTEGRATION_STORAGE_S3_BUCKET",value = aws_s3_bucket.attachments.bucket }, # ambient: no keys
...
1. Confirm task env is set: `HELIOS_ENABLE_STORAGE_S3=true`, `HELIOS_INTEGRATION_STORAGE_S3_REGION`, `HELIOS_INTEGRATION_STORAGE_S3_BUCKET`, and **no** access-key env.
...
> **Future enhancement (tracked, not in this release):** add an upstream `HELIOS_INTEGRATION_STORAGE_S3_AUTH_MODE` knob so the env preset can express `ambient` directly. Once that lands, step 6.4 becomes fully env-automatable and this manual marketplace action is dropped from the runbook.
...
1. **State backend (per env, pre-exists the module).**
   ...
   - Wire the live layer to it:
     ...
     terraform {
       backend "s3" {
         bucket       = "helios-tfstate-prod"
         key          = "helios/prod.tfstate"
         region       = "<region>"
         kms_key_id   = "<cmk-arn>"
         encrypt      = true
...
2. **First `terraform apply` to create ECR + secret scaffolding only is *not* required separately** — ECR repo and the empty third-party secret placeholders are created by the module on the first full apply (step B1). But because the first image must exist *before* the app service can pull, do the following ordering:
   - Run a **scoped first apply** targeting just ECR so the repo exists:
     ```bash
     terraform apply -target=module.helios.aws_ecr_repository.app
     ```
...
terraform apply
  └─> infra (VPC/SGs/ALB/Aurora/Redis/Meili/EFS/ECR/secrets/task defs)
       └─> migration run-task (null_resource → aws ecs run-task + waiter)
            • CREATE EXTENSION vector, pgcrypto   (Aurora master role)
            • yarn helios init  (schema + seed, FIRST bootstrap only)
...
4. **Post-deploy (one-time, manual).**
   - **Set storage_s3 to ambient.** The env preset only expresses `access_keys`, so flip the integration to `authMode: ambient` once per scope via the marketplace UI (this makes the storage driver use the AWS default chain = the ECS task role; no access keys in env). Future seam: an upstream `HELIOS_INTEGRATION_STORAGE_S3_AUTH_MODE` knob would make this fully env-automatable.
   - **Verify attachments → S3.** Upload an attachment through the app; confirm the object lands in the env's single bucket (`HELIOS_INTEGRATION_STORAGE_S3_BUCKET`) and the task role's least-privilege Get/Put/Delete/List is sufficient.
...
1. **Upstream enhancement (optional):** add `HELIOS_INTEGRATION_STORAGE_S3_AUTH_MODE=ambient` to `readS3EnvPreset` in `packages/storage-s3` so the env preset can express ambient directly — this removes the one manual marketplace step (§6.4) and makes the S3 path 100% Terraform-automatable. Backward-compatible PR.

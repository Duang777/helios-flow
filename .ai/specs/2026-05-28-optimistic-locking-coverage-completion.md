...
**Scope:** OSS
**Status:** Draft
**Tracking issue:** [#2191](https://github.com/helios/helios/issues/2191)
...
Complete optimistic-locking coverage across Helios edit/delete flows.
...
Helios has several write surfaces:
...
token. The `HELIOS_OPTIMISTIC_LOCK` environment contract from the related spec
remains unchanged.
...
from `@helios/shared/lib/crud/optimistic-lock-command` (#2055 Phase 16).
...
strictly additive (no expected token → no-op) and honors `HELIOS_OPTIMISTIC_LOCK`.
...
| Unified record-conflict bar (all forms) | **Done (#2055 resume)** | persistent error-styled bar in `AppShell` (`@helios/ui/backend/conflicts`); `CrudForm` + `useGuardedMutation` route 409s automatically, custom pages call `surfaceRecordConflict(err, t, opts)`. Replaces the transient flash/toast (`f2a23716c`) |
...
2. **Command-level document-aggregate** — `enforceCommandOptimisticLock`
   (`@helios/shared/lib/crud/optimistic-lock-command`, #2055 Phase 16)
...
`yarn helios auth sync-role-acls` precondition and no self-skip** on a fresh install or in
...
- #2055 QA round 3 (@alinadivante 2026-05-31) — closed the remaining UI gaps:
  ...
  - **Sales false-positive (defensive)**: the order detail `afterList`
    ...
    existing `HELIOS_OPTIMISTIC_LOCK_DEBUG=1` server flag logs exact expected-vs-current
...
- #2055 resume — **100% OSS optimistic-lock coverage**:
  ...
  - **Unified record-conflict bar** — the optimistic-lock 409 now surfaces as a
    ...
    transient toast. New `@helios/ui/backend/conflicts`
...
- #2055 Phase 4 (command classification) — partially implemented:
  - Added the generalist `enforceCommandOptimisticLock` primitive
    (`@helios/shared/lib/crud/optimistic-lock-command`) so Command-pattern

...
- Two per-provider workspace packages: `@helios/channel-gmail`, `@helios/channel-imap`.
...
- OAuth client infrastructure does not exist in OSS core (only in the enterprise SSO module). This spec ports the state-cookie pattern locally to the hub (`packages/core/src/modules/communication_channels/lib/oauth-state.ts`) — `@helios/core` MUST NOT import from `@helios/enterprise`.
...
Helios today has **outbound-only, Resend-only** transactional email and no per-user mailbox concept. The product needs each logged-in user to connect their own Gmail or IMAP+SMTP account so that:
...
- Inbound emails reach the user's unified Helios inbox alongside WhatsApp, Slack, and future channels — one place to triage all external communications.
...
```
┌──────────────────────────────────────────────────────────────┐
│  Hub: @helios/core/modules/communication_channels      │
...
| **OAuth state-cookie helper ported locally to the hub, not imported from enterprise** | `@helios/core` MUST NOT depend on `@helios/enterprise`. The pattern is small and self-contained. Phase 0 includes a `grep` verification step. |
...
- **A sales rep** wants to **connect their Gmail in Helios** so that **the customer conversations I have through Helios come from my own address and replies land in my inbox.**
...
packages/channel-gmail/                                # @helios/channel-gmail
  package.json
...
packages/channel-imap/                                 # @helios/channel-imap, parallel structure to channel-gmail
  src/modules/channel_imap/
    ...                                                 # imapflow + nodemailer; basic auth; UIDVALIDITY+UIDNEXT
```

### `apps/helios/src/modules.ts` entries

```ts
{ id: 'communication_channels', from: '@helios/core' },   // Hub (Phase 0)
{ id: 'channel_gmail',          from: '@helios/channel-gmail' },     // Phase 2
{ id: 'channel_imap',           from: '@helios/channel-imap' },      // Phase 1
```

Module IDs use underscores per the root `AGENTS.md` rule "package `@helios/<suffix>` ⇒ module id `<suffix>` with dashes converted to underscores". After enabling: `yarn helios configs cache structural --all-tenants` (root `AGENTS.md` mandate after module-graph changes).

### OSS Independence

`@helios/core` and the two provider packages MUST NOT import from `@helios/enterprise`. The OAuth state-cookie helper is **ported (re-implemented locally)** at `packages/core/src/modules/communication_channels/lib/oauth-state.ts` — its design is informed by `packages/enterprise/src/modules/sso/lib/state-cookie.ts` but the file is independent. Phase 0 acceptance includes:

```bash
grep -r "@helios/enterprise" \
...
The local helper reproduces: AES-256-GCM payload encryption, HKDF key derivation, encrypted payload `{ nonce, userId, providerKey, returnUrl, iat, exp }`, 5-minute TTL, signature/tamper detection. Unit tests cover encrypt/decrypt roundtrip, TTL expiry, signature tamper, userId mismatch rejection. Key source: `HELIOS_HUB_OAUTH_STATE_KEY` env var with HKDF fallback from `KMS_MASTER_KEY`.
...
```ts
import { sendAsUser } from '@helios/core/modules/communication_channels/lib/send-as-user'
...
After implementation: `yarn helios auth sync-role-acls` so existing tenants receive the new feature grant.
...
```ts
import type { EntityExtension } from '@helios/shared/modules/extensions'
...
HELIOS_HUB_GMAIL_OAUTH_CLIENT_ID
HELIOS_HUB_GMAIL_OAUTH_CLIENT_SECRET
HELIOS_HUB_GMAIL_REDIRECT_URI                    # e.g. https://app.example.com/api/communication_channels/oauth/gmail/callback

# Polling tuning (hub-side)
HELIOS_HUB_POLL_DEFAULT_INTERVAL_SECONDS=300     # default 5 min
HELIOS_HUB_POLL_BATCH_SIZE=100
HELIOS_HUB_POLL_SCHEDULER_TICK_SECONDS=60
HELIOS_HUB_POLL_ENUMERATION_CAP=500              # rows per tick

# Health log retention (hub-side, generic)
HELIOS_HUB_HEALTH_LOG_RETENTION_DAYS=90
...
| `apps/helios/src/modules.ts` | Adds 3 entries (`communication_channels` hub + 2 providers) | Required app change, documented |
...
After enabling: `yarn helios configs cache structural --all-tenants` (root `AGENTS.md` mandate).
...
7. New scheduler entry: `schedulers/poll-tick.ts` registering a 60s cron via `@helios/scheduler` per § Hub Deltas → Delta 6 (scheduler mechanism).
8. New OAuth state-cookie helper: `lib/oauth-state.ts` ported (not imported) from `packages/enterprise/src/modules/sso/lib/state-cookie.ts`. Phase 0 acceptance includes a `grep -r '@helios/enterprise'` returning empty against `packages/core/src/modules/communication_channels/` and the two provider packages once they ship.
...
11. Per-user ACL feature: add `communication_channels.connect_user_channel` to the hub's `acl.ts`; default-grant to all roles in `setup.ts`. Run `yarn helios auth sync-role-acls` in deploy runbook.
...
18. **Acceptance gates**:
    ...
    - `grep -r "@helios/enterprise" packages/core/src/modules/communication_channels/` returns empty
    - `yarn test` passes for the hub module
    - Six integration tests above pass against a real Postgres + Redis (no mocked DB)
    - `yarn helios configs cache structural --all-tenants` runs cleanly after the new ACL feature is added

### Phase 1 — IMAP provider (`@helios/channel-imap`)
...
8. Wire into `apps/helios/src/modules.ts`; run `yarn helios configs cache structural --all-tenants`.
9. **Acceptance**: User connects IMAP, sends, receives via polling within 5 min, threading correct, sees email in unified inbox.

### Phase 2 — Gmail provider (`@helios/channel-gmail`)
...
2. Deploy runbook: `HELIOS_HUB_*` env keys, `HELIOS_HUB_OAUTH_STATE_KEY` rotation, `yarn helios configs cache structural --all-tenants` after deploy, optional tenant OAuth app registration steps for Google Cloud Console.
...
| `apps/helios/src/modules.ts` | Modify | Enable `communication_channels` (if not enabled) + 2 providers |
...
- **Scenario**: Operator forgets `yarn helios configs cache structural --all-tenants` after enabling providers.
- **Severity**: Medium
...
- **User documentation:** added admin setup guides `apps/docs/docs/user-guide/communication-channels-gmail.mdx` (Google Cloud OAuth app → register in Helios → connect → optional Cloud Pub/Sub push with topic / publisher grant / service-account subscription / `HELIOS_GMAIL_PUBSUB_*` env + ngrok for local dev) and `…-imap.mdx` (IMAP/SMTP host/port/TLS fields, app passwords, common provider settings), wired into the docs sidebar under *Integrations & Payments → Email*; reframed the end-user `communication-channels.mdx` to scope this release to Gmail + IMAP. Docs build clean (`onBrokenLinks: throw`).
...
- **New end-user doc** at `apps/docs/docs/user-guide/communication-channels.mdx` — connect / reconnect / disconnect / multi-account / primary swap flows for Gmail and IMAP+SMTP. Troubleshooting section enumerates the five most common failure modes per provider (auth rejection, ECONNREFUSED, admin-consent gates, app-not-verified, etc.). Cross-linked with the inbox-ops user guide so users understand the per-user vs shared-mailbox split. Explicit "What Helios never does" section documenting the scope: inbox-only sync, no DKIM mining, no cross-user mailbox sharing.
- **New developer / admin doc** at `apps/docs/docs/framework/modules/communication-channels.mdx` — Hub architecture diagram + send/receive path diagrams; per-user channel column reference; OAuth setup walkthrough for Google Cloud Console; env-var reference (`HELIOS_HUB_OAUTH_STATE_KEY`, `HELIOS_HUB_OAUTH_STATE_TTL_SECONDS`, `HELIOS_HUB_POLL_DEFAULT_SECONDS`, `HELIOS_HUB_POLL_CONCURRENCY`, `HELIOS_HUB_OUTBOUND_RETRY_MAX`); deploy runbook covering initial deploy, routine ops (state-key rotation, OAuth secret rotation, force-poll), adding a new provider, security posture. Provider-package contract summary so future providers (Yahoo, ProtonMail Bridge, etc.) can be built from this doc plus the two shipping examples.
...
- **§ Hub Deltas → Delta 6** — pinned the scheduler mechanism to `@helios/scheduler` (the platform's canonical cron home) with an explicit `schedulers/poll-tick.ts` entry. Explicitly forbids `setInterval` in workers and ad-hoc BullMQ repeatables.
...
Complete rewrite of the email integration spec. The previous draft (`.ai/specs/2026-05-21-email-integration-foundation.md`) proposed a standalone `email` module with parallel entities, events, OAuth router, polling worker, and admin UI. That approach was rejected by the maintainer because Helios already has a finalized Communications Hub architecture (SPEC-045d) with an explicit `channel_email` slot, and a Universal Module Extension System (SPEC-041) for cross-module integration. This rewrite:

- Folds all email functionality under the `communication_channels` hub. Two workspace packages (`@helios/channel-gmail`, `@helios/channel-imap`) implement `ChannelAdapter` v2.
...
| 3b | Phase 0 — Polling worker + scheduler | Done | 2026-05-26 | `lib/credential-refresh.ts` + per-channel poll worker + `@helios/scheduler` cron entry registered in `setup.ts.seedDefaults`; access-control helpers `buildPerUserChannelFilter` / `assertCanAccessChannel` |
...
- [x] Wired into `apps/helios/src/modules.ts`

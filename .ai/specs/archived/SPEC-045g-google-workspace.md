...
| **Category** | `data_sync` |
| **Hub** | `data_sync` |
| **Package** | `@helios/sync-google-workspace` |
...
- **Admin** wants to connect a Google account so that Helios can read spreadsheets available to that account.
...
- **Admin** wants scheduled imports so that catalog changes in Google Sheets can flow into Helios without manual CSV uploads.
...
and enabled by the app from `apps/helios/src/modules.ts`.
...
│  │                                                                       │      │
│  │   Connect a Google account to allow Helios to read              │      │
...
- new package `@helios/sync-google-workspace`
...
5. Add the package to `apps/helios/src/modules.ts`.

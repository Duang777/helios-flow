...
Helios has no native promotions system. The POS module spec (SPEC-022) explicitly deferred promotions integration. Sales and catalog modules have no awareness of promotional discounts. Without this module:
...
import { PromotionExtensionRegistry } from '@helios/core/modules/promotions/lib/extension-registry'
...
import { promotionExtensionConfiguratorRegistry } from '@helios/core/modules/promotions/lib/extension-registry.client'
...
- **Mitigation**: Cache keys MUST include `tenantId` + `organizationId`. Cache implementation uses `@helios/cache` with tenant-scoped tag invalidation. Code review checklist includes cache key inspection.
...
| root AGENTS.md | Use `apiCall`/`apiCallOrThrow` — never raw `fetch` | Compliant | All backend page components use `apiCall` from `@helios/ui/backend/utils/apiCall` |
...
| packages/cache/AGENTS.md | Use `@helios/cache`; never raw Redis/SQLite | Compliant | `services/promotion-cache.ts` resolves cache via DI; uses tag-based invalidation |
...
| packages/ui/AGENTS.md | Use `LoadingMessage`/`ErrorMessage` | Compliant | All backend pages use `LoadingMessage`/`ErrorMessage` from `@helios/ui/backend/detail` |

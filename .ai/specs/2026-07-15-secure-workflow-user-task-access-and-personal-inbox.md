...
Helios already pauses a root workflow instance or parallel branch when a `USER_TASK` is entered. The current task page and API are therefore the right foundation; a new workflow step type is unnecessary.
...
Baseline reviewed: `helios/helios` `develop` at `39ab1d9e62950e84a5acfb10f2925a3ac41ec328` on 2026-07-22.
...
Concurrent work must be rechecked before implementation. As of the baseline above, [PR #4019](https://github.com/helios/helios/pull/4019) remains an open draft for visual-editor persistence, [PR #4085](https://github.com/helios/helios/pull/4085) remains open for user-task form normalization, and [PR #4291](https://github.com/helios/helios/pull/4291) remains open for role selection in the editor. None implements this access or navigation contract. Their merged replacements, if any, become the compatibility baseline; this specification does not modify their implementation branches.
...
Before canonical-only execution is enabled, `corepack yarn helios workflows audit-user-task-assignees` runs dry by default and scans the selected tenant/organization's active definition versions plus `PENDING`/`IN_PROGRESS` tasks. It classifies every direct value:
...
- New default employee grants require `yarn helios auth sync-role-acls` for existing tenants during implementation rollout.

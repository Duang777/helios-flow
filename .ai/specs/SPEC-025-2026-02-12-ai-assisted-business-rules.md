...
Business rules in Helios have a complex condition expression DSL (recursive AND/OR/NOT trees with 18 operators, field-to-field comparison, template values) and 10 action types with type-specific config. Building these manually through the ConditionBuilder and ActionBuilder UI is powerful but requires deep DSL knowledge.
...
```typescript
import type { JsonValue } from '@helios/shared/lib/types'

# SPEC-ENT-002: Helios Enterprise Health Endpoints (`live`, `ready`)
...
## Overview
Helios Enterprise needs first-class liveness/readiness endpoints for deployment health checks. Today, the app has an authenticated system-status API (`/api/configs/system-status`) intended for operators, not orchestrator probes.
...
## Problem Statement
- There is no stable, public endpoint for load balancers/orchestrators to determine if an Helios instance is alive.

...
`jose` is added as an explicit, version-pinned dependency of `@helios/enterprise`. The verifier:
...
- 1.1 Add pinned `jose` dep to `@helios/enterprise`. Verify: `yarn build:packages`.

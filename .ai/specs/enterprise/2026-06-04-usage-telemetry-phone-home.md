...
## TLDR
**Key Points:**
- On-premise / self-hosted Helios installs report **aggregate** usage counts to a central Helios instance ("phone home"). This enables transparent enterprise billing verification without Helios needing standing access to a customer's code or database.
- Two deliverables: (1) `@helios/telemetry-client` — a small **OSS** package in this monorepo that collects and sends aggregate metrics; (2) a **central telemetry admin module** shipped as a **separate commercial npm package in a separate repo** that ingests reports and lists every reporting installation.
- The client is **fail-safe and non-blocking by design**: it never runs on a request hot path, never scans user data, and hard-aborts any central call that errors or exceeds **100 ms** so it can never slow down Helios.
...
- Dual operating mode:
  - **Enterprise mode** — license key present (`HELIOS_LICENSE_KEY`): telemetry enabled by default, report carries the license key for billing verification.
  - **OSS-distribution mode** — no license key, but `HELIOS_ENABLE_TELEMETRY=true`: anonymous distribution observability, report carries no license key.
...
|----------|------|---------|-------|
| `@helios/telemetry-client` | this monorepo (`packages/telemetry-client`) | OSS | Collector + sender + daily worker. Present in OSS builds; dormant unless enabled. |
| `telemetry` module wiring | this monorepo (`apps/helios/src/modules.ts` enable) | OSS | Registers the worker + DI; no business logic of its own. |
| Central telemetry admin module | **separate commercial repo** (e.g. `helios/telemetry-central`) | Proprietary | Ingestion API + admin UI listing all installations. NOT in core, NOT in this monorepo. |

- The **client is OSS** so the OSS distribution can be observed too. It does not require `packages/enterprise`.
- The **central admin module is commercial** and lives in its own repo. This spec defines its ingestion contract and data model but its implementation ships in that repo. It is itself just an Helios instance running a dedicated `telemetry` module.
- No core (`packages/core`) module is added or modified for ingestion.

## Problem Statement
- Helios Enterprise is licensed by usage (seats / tenants / installs). Today there is no transparent, low-trust way to verify on-premise usage without asking customers for database or repository access — which customers reasonably refuse.
...
|------|---------|------------------------|---------|
| Enterprise | `HELIOS_LICENSE_KEY` set (and not killed) | Yes | Billing / seat verification |
| OSS-distribution | `HELIOS_ENABLE_TELEMETRY=true`, no license key | No (anonymous `installId` only) | Distribution observability |
| Disabled | neither set, or kill switch on | — | No outbound calls at all |

`env.example` ships with telemetry **off** by default (`HELIOS_ENABLE_TELEMETRY` unset/false, no license key).

### Enablement Resolution (precedence, first match wins)
1. `HELIOS_TELEMETRY_DISABLED=true` → **disabled** (hard kill switch, always wins).
2. `HELIOS_LICENSE_KEY` set → **enterprise mode** (enabled by default; the kill switch above is the only override).
3. `HELIOS_ENABLE_TELEMETRY=true` → **OSS-distribution mode**.
...
| License key supplied via `HELIOS_LICENSE_KEY` env | Simplest for on-prem ops; key is hashed/truncated in logs, sent over TLS only |
...
- As **Helios (licensor)**, I want aggregate seat/tenant/usage counts from on-prem installs so I can verify enterprise billing without accessing the customer's code or data.
...
└────────┼───────────────────────────────────────────────────────────────────────────┘
         ...
         ▼
┌─────────────────── Central Helios instance (separate commercial repo) ───────┐
│  telemetry module:                                                                  │
...
- A queue worker runs off the request path on the host's existing queue infrastructure (`@helios/queue`), so collection and the outbound call never touch user-facing latency.
...
1. **Total budget 100 ms.** A single `AbortController` aborts the fetch at `HELIOS_TELEMETRY_TIMEOUT_MS` (default `100`). DNS + connect + TLS + response must all fit; otherwise the attempt is abandoned.
...
export type TelemetryReport = {
  ...
  instance: {
    url: string | null              // HELIOS_TELEMETRY_INSTANCE_URL or derived APP_URL
...
- `POST {HELIOS_TELEMETRY_ENDPOINT}/api/telemetry/report`
- Auth: none required for OSS mode; enterprise mode carries `licenseKey` in body. TLS required (HTTPS endpoint).
- Headers: `Content-Type: application/json`, `User-Agent: helios-telemetry/<version>`
- Body: `TelemetryReport` (above)
- Timeout: hard-aborted at `HELIOS_TELEMETRY_TIMEOUT_MS` (default `100`).
...
|---------|---------|---------|
| `HELIOS_ENABLE_TELEMETRY` | unset (off) | Enables OSS-distribution mode when no license key is present |
| `HELIOS_LICENSE_KEY` | unset | Presence enables enterprise mode (telemetry on by default) and is sent for billing verification |
| `HELIOS_TELEMETRY_DISABLED` | unset (off) | Hard kill switch; when `true`, no outbound calls regardless of other flags |
| `HELIOS_TELEMETRY_ENDPOINT` | `https://telemetry.helios.com` | Central ingestion base URL |
| `HELIOS_TELEMETRY_TIMEOUT_MS` | `100` | Total outbound budget; fetch aborted past this |
| `HELIOS_TELEMETRY_INTERVAL_HOURS` | `24` | Reporting cadence (worker reschedule) |
| `HELIOS_TELEMETRY_INSTANCE_URL` | unset → derive from `APP_URL` | Reported `instance.url` |
| `HELIOS_TELEMETRY_IP_HINT` | unset | Optional explicit server IP hint; otherwise best-effort/none |
...
### Phase 1: Client package skeleton (`@helios/telemetry-client`)
...
1. Implement `TelemetrySender` with `AbortController` total budget (`HELIOS_TELEMETRY_TIMEOUT_MS`, default 100), TLS-only endpoint, fail-open on error/timeout/non-2xx.
...
1. Add `workers/telemetry-report.ts` (`metadata.queue = 'telemetry-report'`, idempotent, single-flight per day, reschedule with jitter per `HELIOS_TELEMETRY_INTERVAL_HOURS`).
2. DI registration; enable package in `apps/helios/src/modules.ts`. Run `yarn generate`.
3. Add all env vars to `apps/helios/.env.example` (telemetry off by default) with comments.
...
2. Docs in `apps/docs`: what is collected, why, how to disable (`HELIOS_TELEMETRY_DISABLED`), the 100 ms guarantee, and the dual OSS/enterprise modes. Transparency/disclosure page.
...
| `apps/helios/src/modules.ts` | Modify | Enable telemetry package |
| `apps/helios/.env.example` | Modify | Document env flags (off by default) |
...
- Enterprise mode (`HELIOS_LICENSE_KEY` set) → `mode: 'enterprise'`, `licenseKey` present.
- OSS mode (`HELIOS_ENABLE_TELEMETRY=true`, no key) → `mode: 'oss'`, `licenseKey: null`.
- Kill switch (`HELIOS_TELEMETRY_DISABLED=true`) → no outbound call attempted.
...
- **Mitigation**: documented disclosure page; `HELIOS_TELEMETRY_DISABLED` hard kill switch; startup debug log line stating mode; `env.example` defaults off for OSS.
...
- **Residual risk**: depends on operator using an HTTPS `HELIOS_TELEMETRY_ENDPOINT` — enforced by rejecting non-HTTPS endpoints in the sender.

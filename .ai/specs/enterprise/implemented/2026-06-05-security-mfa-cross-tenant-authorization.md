...
> **Issue:** [helios#2612](https://github.com/helios/helios/issues/2612) (comment 3 — enterprise variants)
...
- Add a helper (e.g. `assertActorOwnsTenantScope(ctx, requestedTenantId)` and `assertActorOwnsEnforcementScope(ctx, scope, scopeId)`) reusing `enforceTenantSelection` / `resolveIsSuperAdmin` from `@helios/core/modules/auth/lib/tenantAccess`.
...
- [x] Reused `enforceTenantSelection`/`resolveIsSuperAdmin` from `@helios/core/modules/auth/lib/tenantAccess` (no hand-rolled superadmin checks).
...
- [x] `yarn workspace @helios/enterprise build` + `typecheck` green (zero errors); full security suite **171 tests** green. (No `lint` script in the enterprise workspace; the TS build is the authoritative gate.)

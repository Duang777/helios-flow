...
- **`accept_mine`**:
  - Via mutation validation header: `x-helios-record-lock-resolution: accept_mine`
...
- **`merged`**:
  - Via mutation validation header: `x-helios-record-lock-resolution: merged`
...
**Multi-instance coordination**: Multiple widget instances on the same page are coordinated via a global owner map (`__heliosRecordLockWidgetOwners__`). Higher priority instances (formId-based = priority 2 > context-based = priority 1) claim ownership. Non-primary instances release locks and clear state. Ownership changes broadcast via `om:record-lock-owner-changed` custom event.
...
- `x-helios-record-lock-kind` — resource kind
- `x-helios-record-lock-resource-id` — record UUID
- `x-helios-record-lock-token` — lock ownership token
- `x-helios-record-lock-base-log-id` — latest action log ID for conflict detection
- `x-helios-record-lock-resolution` — `'normal'`, `'accept_mine'`, or `'merged'` (when armed)
- `x-helios-record-lock-conflict-id` — conflict UUID (when resolving)

Headers are scoped via `withScopedApiRequestHeaders()` from `@helios/ui/backend/utils/apiCall` and propagated through `apiCall` to server-side routes.
...
|--------|----------|--------|
| `x-helios-record-lock-kind` | yes | Resource kind string |
| `x-helios-record-lock-resource-id` | yes | Resource UUID |
| `x-helios-record-lock-token` | no | Lock ownership token |
| `x-helios-record-lock-base-log-id` | no | Action log UUID for conflict baseline |
| `x-helios-record-lock-resolution` | no | `'normal'` / `'accept_mine'` / `'merged'` |
| `x-helios-record-lock-conflict-id` | no | Conflict UUID being resolved |

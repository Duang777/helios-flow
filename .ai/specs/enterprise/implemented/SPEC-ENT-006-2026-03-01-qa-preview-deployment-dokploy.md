...
Helios has no automated QA preview environment. Developers must run the full stack locally or share a single staging server. This spec introduces named QA slot deployments spanning two workflows:
...
| Additional app secrets | — | As required by `apps/helios/.env.example` |
...
3. **Verify local build** — `docker build -f docker/preview/Dockerfile -t helios:preview .` — must complete without errors.
...
5. **Configure domain** for each slot (e.g. `qa1.helios.com`) in Dokploy → Domains. Requires a DNS A record pointing to the Dokploy server IP.
...
4. Wait ~10 minutes; confirm `https://qa1.helios.com/backend` is accessible.
...
- No database migrations in Helios's production database are involved.

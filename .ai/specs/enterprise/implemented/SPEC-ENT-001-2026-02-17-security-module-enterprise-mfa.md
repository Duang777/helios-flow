# ADR: Security Module for @helios/enterprise
...
This ADR defines a new **security** module inside the enterprise package that delivers these capabilities while respecting Helios's architectural principles: isomorphic module independence, widget injection for cross-module UI, feature-based ACL, event-driven audit, and foreign-key-only relationships between modules.
...
```typescript
// Example: in a hypothetical @helios/push-auth module's setup.ts

import type { ModuleSetupConfig } from '@helios/shared'
import type { MfaProviderInterface } from '@helios/enterprise/security'
...
- Where a mutation is logically reversible, command handlers must provide undo metadata/payload and follow the shared command/undo conventions from `@helios/shared/lib/commands`.
...
//   import { requireSudo } from '@helios/enterprise/security'
...
//   import { useSudoChallenge } from '@helios/enterprise/security/components'
...
import type { ComponentOverride } from '@helios/shared/modules/widgets/component-registry'
...
Existing dependencies already available and leveraged: `bcryptjs`, `@helios/shared` (encryption, auth, email via Resend), `zod`, `@mikro-orm/core`, `pg`, Redis client.
...
```env
# ── Security Module ──
HELIOS_SECURITY_TOTP_ISSUER=Helios          # TOTP issuer name shown in authenticator apps
HELIOS_SECURITY_TOTP_WINDOW=1                     # TOTP time-step tolerance (number of windows)
HELIOS_SECURITY_OTP_EXPIRY_SECONDS=600            # OTP email code validity period
HELIOS_SECURITY_OTP_MAX_ATTEMPTS=5                # Max OTP verification attempts per challenge
HELIOS_SECURITY_SUDO_DEFAULT_TTL=300              # Default sudo token TTL in seconds
HELIOS_SECURITY_SUDO_MAX_TTL=1800                 # Maximum configurable sudo TTL
HELIOS_SECURITY_WEBAUTHN_RP_NAME=Helios     # WebAuthn relying party name
HELIOS_SECURITY_WEBAUTHN_RP_ID=                   # WebAuthn RP ID (defaults to hostname)
HELIOS_SECURITY_RECOVERY_CODE_COUNT=10            # Number of recovery codes generated
HELIOS_SECURITY_MFA_EMERGENCY_BYPASS=false        # Emergency bypass for MFA (disaster recovery only)
```
...
- Custom MFA providers run in the same trust boundary as the module that registered them — this is consistent with Helios's module architecture but means third-party modules should be code-reviewed.

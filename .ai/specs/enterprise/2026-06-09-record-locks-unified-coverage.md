...
- **1:1 command coverage with OSS.** OSS optimistic locking covers not just `makeCrudRoute` entities but every command-pattern write via `enforceCommandOptimisticLock` (pre-implementation audit 2026-06-15 estimated 34 call lines across 21 files, incl. `checkout` and `webhooks`; **implemented:** ~22 files now route through the async seam and only 2 remain documented OSS-only exempts — the coverage guard is now **dynamic** and asserts a decision per call site, no hardcoded count). record_locks reaches them through an **additive async DI-aware seam** plus explicit call-site/wrapper migration, not by secretly changing the existing synchronous helper's behavior — Phase 6b.
...
2. **The Deal page shows presence but its writes are unguarded.** The deal route `/backend/customers/deals/<id>` **is** in the allowlist, so the global mount resolves `customers.deal` + id and presence/acquire **does** start. The gap is downstream: `DealForm` (`components/detail/DealForm.tsx`) wraps `CrudForm` with a custom `useDealMutationContext`/`runMutationWithContext` pipeline and passes **no** `injectionSpotId`, so the save-time `crud-form:customers.deal` widget never mounts. (**Implementation correction:** the detail-screen stage-change and Won/Lost closure do **not** use custom command endpoints — they ride `updateCrud('customers/deals')` → command `customers.deals.update` → the CRUD route, so once that resource is record-lock enabled they are auto-guarded by the Phase-0 CRUD decorator. The genuinely *custom* command endpoints are the **bulk** ones — `api/deals/bulk-update-stage|owner/route.ts`, addressed in Phase 2. Phase 1 still threads the lock header on the hooks and surfaces conflicts.) Presence is visible, yet without the header a concurrent save can still clobber.
...
The gap is the **command layer**, and it is bigger than a DI override. The OSS command guard is the standalone helper **`enforceCommandOptimisticLock`** (`packages/shared/src/lib/crud/optimistic-lock-command.ts`), and call sites invoked that helper directly across `packages/core`, `packages/checkout`, and `packages/webhooks` (pre-implementation audit 2026-06-15 estimated 34 lines across 21 files; **implemented:** ~22 files migrated to the async seam, 2 documented OSS-only exempts remain — see `optimistic-lock-command-coverage.test.ts`, which is dynamic and asserts a record_locks decision per call site rather than pinning a count). The companion `createCommandOptimisticLockGuardService({ resolveExpected })` factory existed but was **not registered in any `di.ts`** until record_locks' `di.ts` registered `commandOptimisticLockGuardService` (Phase 0).
...
**Enablement-switch parity.** OSS is gated by `HELIOS_OPTIMISTIC_LOCK` (env, default ON, allowlist by `resourceKind`); record_locks enrichment is gated by tenant settings via `isRecordLockingEnabledForResource(settings, resourceKind)`. These are **layered, not alternative**: the **floor** is governed solely by `HELIOS_OPTIMISTIC_LOCK` (so `=off` disables *all* locking, OSS and enterprise, identically — the single global kill switch); record_locks enrichment additionally requires the floor to be on **and** the resource enabled in settings. record_locks' guard MUST short-circuit to a pure floor pass-through when `HELIOS_OPTIMISTIC_LOCK=off`, so it can never re-enable locking the operator turned off. The **operation set reaching parity is update, delete, and custom/status-transition** (status changes, conversions) — not just update.
...
Mutation request (CRUD route OR command endpoint)
        │
        ▼
 HELIOS_OPTIMISTIC_LOCK = off ? ── yes ──▶ proceed (single global kill switch; OSS + enterprise both off)
        │ no
        ▼
...
- **Deal command endpoints** (stage change, closure) accept the existing `x-helios-record-lock-*` headers; no new event/command IDs are introduced.
...
- Deal custom command endpoints (stage-change, closure) — read `x-helios-record-lock-*` (and OSS `x-helios-ext-optimistic-lock-expected-updated-at`) headers and route through the command guard seam; on conflict return the same `409` shapes already defined (`record_lock_conflict` when record_locks active, else `optimistic_lock_conflict`).
...
3. **Enablement parity.** Gate the floor on `HELIOS_OPTIMISTIC_LOCK` (unchanged); make record_locks' guard short-circuit to a pure floor pass-through when `HELIOS_OPTIMISTIC_LOCK=off`, and require both env-on **and** `isRecordLockingEnabledForResource` for enrichment. One global kill switch; no asymmetry.
...
6. **Fail-safe regression tests (H1–H4):** (a) record_locks enabled + **widget unmounted** → two concurrent edits still 409 (proves server floor, H1/H4); (b) record_locks service **throws** → mutation degrades to OSS 409, not pass-through (H3); (c) `HELIOS_OPTIMISTIC_LOCK=off` → both layers off (parity); (d) tokenless API client → stale write still 409; (e) a migrated command call awaits the async seam before mutating, while an intentionally legacy direct helper remains OSS-only and is reported by the coverage guard.
...
**Step 2 — Per-site enable + decision.** Pre-implementation audit 2026-06-15 estimated 34 direct `enforceCommandOptimisticLock` call lines across 21 non-test files. **Implemented:** ~22 files now route through the async seam (`enforceCommandOptimisticLockWithGuards`) and only 2 remain documented OSS-only exempts; the coverage guard is **dynamic** (no hardcoded count — it asserts a decision per discovered call site). Sites already inside Phases 1–6 (sales, customers interactions, business_rules, dictionaries, workflows) migrated their wrappers/routes to the async seam. The rest belong to modules/packages the entity audit never lists and were silently missing from this spec — each needs a record_locks decision:
...
- **translations** — `components/TranslationManager.tsx` (a **cross-entity injected widget**, `crud-form:<entityId>:fields`) → raw `PUT/DELETE /api/translations/[entityType]/[entityId]`. The translation rows belong to the **host entity** being edited. **Implemented decision (revised, jury-flagged):** the host's EAV `entityType` (`module:entity`) cannot be cleanly mapped to a registered optimistic-lock reader key (those are derived from the host module's ORM entity name / events config — e.g. `CatalogProduct` → `catalog.product` — and do **not** equal `canonicalizeResourceTag('catalog:catalog_product')` = `catalog.catalog.product`), so there is no reliable server-side path to resolve the host's current version for an arbitrary `entityType`. The route therefore enforces against the **translation row's own `updated_at`** (resourceKind `translations.translation`) via `enforceCommandOptimisticLockWithGuards(container, …)` before the save/delete command runs, and `TranslationManager` sends the row version (`updatedAt` from the GET response). This closes the no-lock hole with **real server-side enforcement** and no cross-module coupling, mirroring the hand-written `auth.role_acl` route. Strictly additive (no header → no-op; no existing row → no-op on insert).
...
### Coverage Matrix (done-definition — IMPLEMENTED per-entity/per-site decisions)
> **Enablement is global by default.** `DEFAULT_RECORD_LOCK_SETTINGS.enabledResources = ['*']` (record_locks `lib/config.ts`), so every `resourceKind` is enrolled and the CRUD decorator runs the OSS `updated_at` floor first, then record_locks enrichment. Module phases therefore did **not** need per-resource enablement edits; "enabled" below means floor + record_locks for that `resourceKind`. The CRUD-layer decisions are pinned by `record-locks-coverage.test.ts`; command-helper sites by `optimistic-lock-command-coverage.test.ts`; raw UI header-helper sites by `optimistic-lock-ui-coverage-workspace.test.ts`.

**CRUD-layer audited entities** (the `optimistic-lock-editable-entities.test.ts` set):
| Module | Entity (audited) | Phase | resourceKind | record_locks decision |
|--------|------------------|-------|--------------|-----------------------|
| customers | CustomerEntity (person/company) | 1 | `customers.person` / `customers.company` | enabled (presence + unified guard) |
| customers | CustomerDeal | 1 | `customers.deal` | enabled (DealForm injectionSpotId + guard; detail stage/closure ride `customers.deals.update` CRUD route; bulk endpoints = custom, Phase 2) |
| customers | CustomerInteraction | 2 | `customers.interaction` | enabled (async command seam); status-transition endpoints exempt |
| customers | CustomerTag, CustomerLabel | 2 | `customers.tag` / `customers.label` | enabled (entity edit); assign/unassign junction exempt. **`customer_tags` has no `deleted_at` column** → the tags `makeCrudRoute` now sets `softDeleteField: null` so the auto-registered generic reader does not query a non-existent column (Phase-10 fix; previously a silent no-lock hole). |
| customers | CustomerPipeline, CustomerPipelineStage | 2 | `customers.pipeline` / `customers.pipeline_stage` | enabled (command guard); reorder exempt |
| sales | SalesOrder, SalesQuote | 3 | `sales.order` / `sales.quote` | enabled (parent-order aggregate guard via async seam) |
| sales | SalesChannel, SalesPaymentMethod, SalesShippingMethod | 3 | `sales.sales_channel` / `…payment_method` / `…shipping_method` | enabled (config CRUD route) |
| catalog | CatalogProduct, CatalogProductVariant, CatalogProductCategory | 4 | `catalog.product` / `catalog.variant` / **`catalog.category`** | enabled (presence + CRUD guard); `catalog.category` matches the existing versionHistory widget (not `…product_category`) |
| catalog | CatalogProductPrice, CatalogOffer, CatalogPriceKind, CatalogOptionSchemaTemplate | 4 | `catalog.product_price` / `catalog.offer` / `catalog.price_kind` / `catalog.option_schema_template` | enabled (CRUD guard; inherit product presence; PriceKindSettings dialog wired) |
| auth | User, Role | 5 | `auth.user` / `auth.role` | enabled; ACL routes versioned separately (Phase 6b) |
| directory | Organization, Tenant | 5 | `directory.organization` / `directory.tenant` | enabled |
| staff | StaffTeam, StaffTeamRole | 5 | `staff.team` / `staff.team_role` | enabled (team-member/leave-request also enabled; accept/reject = status txn) |
| resources | ResourcesResource, ResourcesResourceType | 5 | `resources.resource` / `resources.resource_type` | enabled |
| dictionaries | Dictionary, DictionaryEntry | 6 | `dictionaries.dictionary` / `dictionaries.entry` | enabled (async command seam); reorder/set-default exempt |
| currencies | Currency | 6 | `currencies.currency` | enabled (+ ExchangeRate `currencies.exchange_rate` enabled) |
| workflows | WorkflowDefinition | 6 | `workflows.definition` | enabled (form + visual editor presence) |
| feature_toggles | FeatureToggle | 6 | — | **exempt — OSS-floor-only.** Global non-tenant, superadmin-only entity; record_locks enrichment is tenant/org-scoped, so the global toggle keeps the OSS `updated_at` floor only (no presence). |
| business_rules | BusinessRule, RuleSet | 6 | `business_rules.rule` / **`business_rules.ruleSet`** | enabled (routes migrated to async command seam) |

**Command-helper + raw-helper sites outside the entity audit** (Phase 6b; see the command/UI-workspace coverage guards):
| Module | Site | resourceKind | record_locks decision |
|--------|------|--------------|-----------------------|
| entities | EAV records (`api/records.ts`) | `entities.record` | enabled (async command seam) |
| staff | job-histories (`commands/job-histories.ts`) | `staff.job_history` | **enabled** (async command seam — employment record edits) |
| feature_toggles | FeatureToggleOverride (`api/overrides`) | `feature_toggles.override` | **enabled** (async seam — per-tenant override values; spec Phase 6 wrongly marked exempt). **Phase-10 fixes:** `getOverrides` list serializer now returns `updatedAt`, and `FeatureToggleOverride.updated_at` gained an `onCreate` so the first edit after create is guarded (was `null` → lock unreachable). |
| customer_accounts | admin User, Role + domain mappings | `customer_accounts.*` | enabled (async command seam; domain mappings send the header) |
| inbox_ops | settings (`api/settings`) | `inbox_ops.settings` | **enabled** (async command seam) |
| data_sync | sync schedule (`lib/sync-schedule-service.ts`) | `data_sync.schedule` | **enabled** (async command seam; container threaded; `[id]` PUT additively gained a lock token) |
| auth | Role ACL, User ACL | `auth.role` / `auth.user` (ACL) | enabled (async command seam); **sidebar prefs = exempt** (per-user single-owner preference) |
| perspectives | saved views (`services/perspectiveService.ts`) | — | **exempt** (strictly per-owner personal views; pure em+cache lib service with no request container; OSS floor retained) |
| attachments | partitions | `attachments.partition` | **enabled** (409 → surface) |
| translations | translation rows (`TranslationManager`) | `translations.translation` | **enabled (server-side, row-version)** — route enforces the **translation row's own `updated_at`** via `enforceCommandOptimisticLockWithGuards` on PUT+DELETE; client sends the row version. Host-version inheritance was dropped because the EAV `entityType` has no reliable mapping to a registered reader key (jury-flagged deferral now closed). |
| planner | AvailabilityRuleSet, AvailabilityRule | `planner.*` | enabled (raw header-helper protected) |
| api_keys | ApiKey row delete/edit | `api_keys.api_key` | enabled (raw header-helper protected) |
| integrations | Integration state/credentials/bundle toggles | `integrations.*` | enabled; `buildOptimisticLockHeader(undefined)` bundle false-positive **fixed** |
| webhooks | Webhook endpoint (`packages/webhooks`) | `webhooks.endpoint` | enabled (async command seam + UI header sites) |
| checkout | CheckoutLink, CheckoutLinkTemplate (`packages/checkout`) | `checkout.link` / `checkout.link_template` | enabled for admin form edits (async command seam + `LinkTemplateForm`); list-row status toggles/quick deletes + public pay/transaction flows exempt (server-authoritative / idempotent) |
...
- **Fail-safe (Phase 0, the safety spine — MUST land first):**
  ...
  - **Enablement parity:** `HELIOS_OPTIMISTIC_LOCK=off` disables both layers; settings-enabled + env-off → no locking; env-on + settings-off → OSS floor only.
...
- **Residual risk**: With the widget gone the user sees the plain OSS conflict bar instead of the merge dialog (degraded UX), but the write is still blocked. `HELIOS_OPTIMISTIC_LOCK=off` is the only switch that disables protection, and it disables OSS too — no asymmetry.
...
- **OSS-only build unaffected.** record_locks lives in `@helios/enterprise`; with it absent, nothing registers the service and migrated async-seam calls run only the OSS compare.
...
- **Phases 0–7 landed** in the working tree (branch `feat/2187-record-locks-unified-coverage`; implementation ledger `.ai/runs/2026-06-09-record-locks-IMPL-ledger.md`). Phase 0 (floor-chaining CRUD decorator + async command seam + DI registration + AppShell context transport + `surfaceRecordConflict` deferral + fail-safe tests); Phase 1 CRM v2; Phases 2–6 per-module rollout; Phase 6b command/raw-helper sweep; Phase 7 cross-cutting delete sweep + coverage matrix + the `record-locks-coverage.test.ts` entity decision-map guard + TC-LOCK-OSS-026 realignment.
- **Phase 10 (docker integration QA) closed three product no-lock holes the unit guards missed** and made the coverage guard catch the class statically — see the 2026-06-18 changelog entry below. The `optimistic-lock-editable-entities.test.ts` guard now also asserts the generic reader can **resolve** (deleted_at OR `softDeleteField: null` OR command-layer/custom reader), not just that `updated_at` exists.
- **Done-definition met:** every audited editable entity has a record_locks decision (`record-locks-coverage.test.ts`); every command-helper site is migrated or documented OSS-only exempt (`optimistic-lock-command-coverage.test.ts`, dynamic); every raw UI header-helper site is covered/inline-exempt/allowlisted (`optimistic-lock-ui-coverage-workspace.test.ts`); the OSS floor guard (`optimistic-lock-editable-entities.test.ts`) still holds. All four guards pass.
- **Integration coverage:** per-phase Playwright specs (`TC-LOCK-CRM-*`, `TC-LOCK-CUST-*`, `TC-LOCK-SALES-*`, `TC-LOCK-CAT-*`, `TC-LOCK-{AUTH,DIR,STAFF,RES}-*`, `TC-LOCK-{DICT,CUR,WF,BR}-*`, plus the realigned `TC-LOCK-OSS-026`) are **written and typecheck**; they run in the **Phase-8** ephemeral batch alongside the Claude fresh-reviewer + cross-model jury.
- **Known partial — now CLOSED (Phase-8 jury fix, see changelog 2026-06-17 below):** `translations` previously only sent the host version client-side with **no server-side enforcement** on the polymorphic route (a real no-lock hole the jury flagged CRITICAL). It now enforces the **translation row's own `updated_at`** server-side on PUT+DELETE; the client sends the row version. No remaining no-lock holes.
- **Scope note — command-layer enrichment is floor-parity today (intentional, per S5/H5).** The enterprise `commandOptimisticLockGuardService` is registered and the async seam (`enforceCommandOptimisticLockWithGuards`) is wired at every migrated command site, but `recordLockService.resolveExpectedVersion` currently returns the client-supplied `updated_at` token — i.e. the command layer runs the **OSS `updated_at` floor** (the authoritative write-time gate, H5) and is fully covered by it, but does **not** yet add pessimistic-lock / action-log conflict *enrichment* at the command layer the way the CRUD layer does via `validateMutation`. This is **not** a regression (every command site is at least as protected as OSS; H6 holds) and is consistent with H5 (the pessimistic lock is advisory; the optimistic write-time check is authoritative). Richer command-layer (action-log–based) detection is a deliberate **future extension** — the DI seam exists precisely so it can be added without touching call sites. Flagged by the cross-model jury (Codex) and confirmed acceptable by the Claude fresh-reviewer.

## Changelog
### 2026-06-18 (Phase 10 docker-integration QA — three product no-lock holes + reader-resolution guard)
- **CRITICAL closed: `customers.tag` silent no-lock hole + the whole "missing `deleted_at`" CLASS.** `customer_tags` has no `deleted_at` column, but the tags `makeCrudRoute` (`packages/core/src/modules/customers/api/tags/route.ts`) did not set `softDeleteField: null`. The factory's auto-registered `createGenericOptimisticLockReader` therefore filtered on `deletedAt: null` against a non-existent column → `findOne` threw → the reader's catch returned `null` (fail-open) → the guard treated the row as "already gone" and passed the stale write (200 instead of 409). Fix: set `softDeleteField: null` on the tags route. **Audited the whole class** (every entity in `optimistic-lock-editable-entities.test.ts`): six audited entities lack `deleted_at` — `CustomerTag`, `CatalogProductPrice`, `DictionaryEntry`, `CustomerLabel`, `CustomerPipeline`, `CustomerPipelineStage`. Of these, only `CustomerTag` used a `makeCrudRoute` reader without disabling the soft-delete filter (the bug). `CatalogProductPrice` (`catalog/api/prices/route.ts`) and `DictionaryEntry` (`sales/api/adjustment-kinds/route.ts`, `sales/lib/makeStatusDictionaryRoute.ts`) already set `softDeleteField: null`. `CustomerLabel`/`CustomerPipeline`/`CustomerPipelineStage` do **not** use the generic `makeCrudRoute` reader — their locking is wired at the command layer (`enforceCommandOptimisticLockWithGuards` reading `updated_at` directly), so they never touch the soft-delete filter and are unaffected.
- **Generic reader hardened (loud, not silent).** `createGenericOptimisticLockReader` (`packages/shared/src/lib/crud/optimistic-lock.ts`) now `console.error`s with the `resourceKind` when `findOne` throws (a genuine not-found returns `null` without throwing, so the catch only fires on a real query failure / column misconfig). Control flow stays fail-open (returns `null`, no 500s) per the existing no-500 contract; the loud log makes a future missing-column misconfig visible at runtime.
- **Coverage guard strengthened (the durable fix).** `optimistic-lock-editable-entities.test.ts` gained a second describe block asserting that, for every audited editable entity, the OSS reader can **resolve**: the entity has a `deleted_at` column, OR every `makeCrudRoute` route using it as `orm.entity` sets `softDeleteField: null`, OR no `makeCrudRoute` route uses it (command-layer/custom reader). This statically FAILS for the `customer_tags` misconfig and PASSES after the route fix — so the class can never silently regress.
- **`feature_toggles` override lock made reachable.** `getOverrides` (`packages/core/src/modules/feature_toggles/lib/queries.ts`) omitted `updatedAt` from the serialized override shape, so no client could send the lock header; added `updatedAt` to the serializer (and to `overrideListResponseSchema`). `FeatureToggleOverride.updated_at` (`data/entities.ts`) had `onUpdate` but no `onCreate` → `null` on the first edit after create (lock unreachable); added an `onCreate`. The override detail/edit card already serialized `updatedAt` and sends the header (`FeatureToggleOverrideCard.tsx`).
- **`RoleAcl`/`UserAcl` first-edit guard.** `RoleAcl.updated_at` and `UserAcl.updated_at` (`packages/core/src/modules/auth/data/entities.ts`) had `onUpdate` only → `updatedAt` was `null` after create, leaving the first edit unguarded. Added `onCreate` to both (column already exists — no migration; SQL unchanged).

### 2026-06-17 (Phase 8 jury fixes — translations no-lock hole + workspace scan `.ts` gap)
- **CRITICAL closed: translations standalone route now enforces server-side.** The cross-model jury flagged that `TranslationManager` sent the host's optimistic-lock header on a standalone `PUT /api/translations/[entityType]/[entityId]`, but the route never read/enforced it — a stale standalone write succeeded without going through the host CrudForm (a real no-lock hole on a surface this spec claimed was protected). Investigated host-version enforcement via the OSS reader registry (`getAllOptimisticLockReaders()`) and rejected it: the registry keys are `canonicalizeResourceTag`'d ORM-entity-name / events-config strings (e.g. `CatalogProduct` → `catalog.product`), which do **not** equal `canonicalizeResourceTag(entityType)` (`catalog:catalog_product` → `catalog.catalog.product`), so there is no reliable mapping from an arbitrary EAV `entityType` to a registered reader. Chose the row-version fallback: the route loads the existing `entity_translations` row's `updated_at` and calls `enforceCommandOptimisticLockWithGuards(container, { resourceKind: 'translations.translation', current: row.updated_at, request })` before the save/delete command (mirrors the hand-written `auth.role_acl` route); `TranslationManager` now sends the translation row's own `updatedAt` (from the GET response) instead of the host version. Strictly additive (no header → no-op; no existing row → no-op on insert). Removed the now-unused `resolveHostUpdatedAt` helper + its tests; rewrote `translation-host-lock.test.ts` for the row-version contract; added `api/__tests__/optimistic-lock.test.ts` proving a stale write 409s with the structured conflict body. Files: `packages/core/src/modules/translations/api/[entityType]/[entityId]/route.ts`, `…/components/TranslationManager.tsx`, `…/lib/helpers.ts`.
- **Workspace raw-UI coverage scan now collects `.ts` too.** `optimistic-lock-ui-coverage-workspace.test.ts` only collected `.tsx`, so a `.ts` raw-UI adapter could regress to `buildOptimisticLockHeader(undefined)` or drop the header and the scan would still pass. Extended the collector to `.ts` (same mutation-detection + tokenless-is-not-covered classifier; existing assertions unchanged); added a `.ts` classifier regression fixture. Newly surfaced `.ts` mutating adapters are each covered (real header) or carry a documented decision.

### 2026-06-17 (Phase 7 — implementation finalization + done-definition)
- **Cross-cutting delete sweep.** Confirmed `makeCrudRoute`/`CrudForm` and command-seam deletes surface `record_locks.record.deleted` via the Phase-0 CRUD decorator / async seam. Wired the three genuine raw-delete (and edit) gaps the `.tsx`-only workspace scan missed in `.ts` adapters: `resources` and `staff` notes adapters and the `staff` address adapter now send the optimistic-lock header (the shared `Notes`/`Address` section already threads `updatedAt`; server routes are `makeCrudRoute` floor-covered). `messages` item/conversation deletes and `mfa` factor reset remain exempt (non-audited / status-transition / single-owner security actions).
- **TC-LOCK-OSS-026 realigned to the Phase-3 semantic shift.** Sales **payments/shipments** moved from ROW-level OSS locking to **PARENT-ORDER aggregate** locking (CRUD-layer readers in `sales/di.ts` resolve the parent order's `updated_at`; the UI sends `documentUpdatedAt`). The Playwright spec now advances the parent ORDER out-of-band and asserts a stale order header → 409 / fresh order header → 200, mirroring TC-LOCK-OSS-008. No other test asserted the old row semantics (`sectionConflict.test.ts` only tests generic version-read helpers).
- **record_locks entity decision-map guard.** Added `packages/core/src/__tests__/record-locks-coverage.test.ts`: for every entity audited by `optimistic-lock-editable-entities.test.ts` (parsed from that file so the two can't drift), assert an explicit record_locks decision (enabled with resourceKind, or exempt with a reason). 35 entities asserted. `feature_toggles:FeatureToggle` is the only documented exempt (global/non-tenant → OSS-floor-only).
- **Coverage Matrix made implementation-accurate.** Replaced the "decide" placeholders with the real per-entity/per-site decisions and the actual resourceKinds (`catalog.category`, `business_rules.ruleSet`, dot-notation kinds); `perspectives` and `auth` sidebar-prefs = exempt; `feature_toggles` global = OSS-floor-only; `feature_toggles` overrides, `staff` job-histories, `inbox_ops` settings, `data_sync` schedule = enabled; `translations` = host-version-inherited with the server-side deferral note.
- **Stale count removed.** The "34 direct call lines across 21 non-test files" pre-implementation estimate is now noted as historical; the coverage guard is **dynamic** (no hardcoded count) and the actual scan finds ~22 files on the async seam + 2 documented OSS-only exempts.
- **Deal statement corrected.** Detail-screen stage/closure ride the `customers.deals.update` CRUD route (auto-guarded), not custom command endpoints; the custom endpoints are the bulk ones (Phase 2).
...
- **Enablement-switch parity.** Defined the interaction of `HELIOS_OPTIMISTIC_LOCK` (env floor) and `isRecordLockingEnabledForResource` (settings enrichment): one global kill switch (`=off` disables both), enrichment requires env-on **and** settings-on; record_locks short-circuits to floor-only when env-off.

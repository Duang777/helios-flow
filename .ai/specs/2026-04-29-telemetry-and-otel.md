...
Add a new optional package **`@helios/telemetry`** that gives modules a vendor-neutral observability facade — structured logger, spans, counters, histograms — backed by a pluggable `Exporter` interface. Default exporter is **OpenTelemetry (OTLP)**, shipped as an `optionalDependency`; alternative exporters (`newrelic`, `console`, `noop`) plug in through the same seam.
...
Helios today has **no shared logger**. Modules use ad-hoc `console.log/warn/error` with `[module:feature]` prefixes (e.g. `console.warn('[customers.comments] failed to enrich…', ctx)`). There is no log level control, no structured fields, no trace correlation, and no way for a module author to participate in observability beyond writing to stdout.
...
Third-party module authors, who are increasingly the consumer of `@helios/core` (per the Backward Compatibility Contract in root `AGENTS.md`), have no observability seam at all today.
...
This makes distributed traces unusable for any flow that crosses a worker, a subscriber, or a webhook — i.e. most non-trivial Helios flows.
...
But the OTEL **SDK** is heavy (~3-5 MB installed, complex init, opinionated about resource detection) and not every Helios deployment wants it. We need the conceptual API to feel OTEL-shaped *without* dragging the SDK into installs that don't enable telemetry.
...
### S1 — New package: `@helios/telemetry`
...
- `apps/helios/instrumentation.ts` (Next.js standard hook),
...
```ts
import { reportError } from '@helios/telemetry'
...
### Public TypeScript surface (`@helios/telemetry`)
...
| Import paths | new package — STABLE from day 1; alias re-export from `@helios/shared/lib/telemetry` if package boundary is later moved | — |
...
- `initTelemetry()` callable from `apps/helios/instrumentation.ts`.
...
- **New package** under `packages/` per root `AGENTS.md` ("Where to Put Code" rules). Naming `@helios/telemetry` follows the convention.
...
- **Generated files**: package adds nothing under `apps/helios/.helios/generated/`.
...
- `apps/helios/instrumentation.ts` — calls `initTelemetry()`.

...
**Key Points:**
- `AiChatSessions` persists chat session metadata in `localStorage` under a global, unscoped key (`helios-ai-chat-sessions-v1`), causing stale sessions from a previous tenant/org to flash in the dock after a scope switch.
...
The AI dock's multi-tab session panel (`AiChatSessionsProvider`) was built before tenant/org switching became a first-class operation. Its session registry is stored under a single, app-global `localStorage` key (`helios-ai-chat-sessions-v1`). When a superadmin or multi-org user switches scope via the topbar, the provider re-mounts with the same stale registry — surfacing the previous scope's tab names, last-message timestamps, and (briefly) full transcripts before the server responds with `404`.

The open-source ecosystem (e.g., Linear, Notion) universally scopes client-side caches per workspace identity. This spec aligns Helios with that expectation and eliminates the secondary cross-tenant data leak.
...
Use `getCurrentOrganizationScope()` from the existing `@helios/shared/lib/frontend/organizationEvents` module to derive a scoped key at mount time. Subscribe to `subscribeOrganizationScopeChanged` inside the provider to reset state and re-fetch from the server when scope changes.

```
helios-ai-chat-sessions-v1                    ← legacy (abandoned, not migrated)
helios-ai-chat-sessions-v1:tenantA:org1       ← new scoped key (tenantId:organizationId)
helios-ai-chat-sessions-v1:no-tenant:no-org   ← fallback when scope not yet resolved
```
...
| `helios-ai-dock-v1` key NOT scoped | The dock state (`assistant`, `width`, `collapsed`) is UI-configuration only — it contains no conversation data and no cross-scope-sensitive content. Scoping it would disrupt "restore last docked assistant" after scope switch, which is desirable UX. |
...
AppShell
  └─ AiChatSessionsProvider
       • useState(() => readPersisted())    ← reads 'helios-ai-chat-sessions-v1' globally
...
topbar: emitOrganizationScopeChanged({ tenantId: 'T2', organizationId: 'O2' })
  → AiChatSessionsProvider.subscribeOrganizationScopeChanged handler
       setStorageKey('helios-ai-chat-sessions-v1:T2:O2')
       setState(readPersisted('helios-ai-chat-sessions-v1:T2:O2'))  ← empty or T2/O2's cached data
...
| Key | Fate |
|-----|------|
| `helios-ai-chat-sessions-v1` (legacy unscoped) | **Orphaned**. Not read, not written, not deleted. Fades naturally as browser storage is cleared. |
| `helios-ai-chat-sessions-v1:tenantId:orgId` (new) | Created on first load for each scope. Populated by server sync immediately. |
| `helios-ai-dock-v1` | **Unchanged**. No scoping needed (UI config only). |
| `helios-ai-chat:agent:conversationId` (per-session transcript cache in `useAiChat`) | **Unchanged**. `conversationId` is a UUID; cross-scope collision is impossible. These keys are orphaned naturally when `AiChatSessions` creates new sessions with new UUIDs after a scope change. |
...
- `yarn workspace @helios/ui test` must pass green after all phases.
...
- **Scenario**: User switches tenant, `helios-ai-chat-sessions-v1` (legacy) retains old data; a future code path inadvertently reads it.
- **Severity**: Low
...
- **Scenario**: `getCurrentOrganizationScope()` returns `{tenantId: null, orgId: null}` at mount; provider writes to `helios-ai-chat-sessions-v1:no-tenant:no-org`. Scope event fires shortly after; provider switches to correct scoped key. The null-scope bucket has empty state written into it.
- **Severity**: Low
...
- **Scenario**: Users who had sessions stored only in `helios-ai-chat-sessions-v1` (never synced to server) will not see them after the fix, because we do not migrate the legacy key.

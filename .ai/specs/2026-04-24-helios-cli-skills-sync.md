...
- **Related**:
  ...
  - [Develop Snapshot Release](implemented/2026-03-21-helios-develop-snapshot-release.md)
...
```bash
yarn helios skills sync
```

The command syncs `.ai/skills/` from the `helios/helios` `develop` branch, using the existing standalone skill source inside the monorepo. It is safe by default: locally modified skills are never overwritten silently. In the normal terminal flow, the user is asked what to do for each conflict.

There is no `yarn dev` automation in this proposal. Skill updates stay explicit and manual.

## Overview

The current standalone skill distribution model is operationally simple at scaffold time but wrong over the lifetime of the project. `create-helios-app` and `yarn helios agentic:init` copy skill files once, then those files drift forever unless someone manually updates them.

The previously explored plugin-based approach solves freshness, but at the cost of another distribution channel, another repository, and tool-specific behavior that mainly fits Claude Code. That increases complexity and does not help existing Helios installations, custom OM-adjacent projects, or teams using Codex and Cursor.
...
- `create-helios-app` and `agentic:init` still generate the initial files

What changes is the lifecycle: the official Helios CLI becomes the update mechanism for that folder.
...
Those files are not tied to package upgrades and are not refreshed by `yarn upgrade @helios/*`. The result is predictable:

- newer Helios conventions do not reach old apps
...
The problem is broader than freshly scaffolded standalone apps. Existing Helios installations and OM-adjacent repositories such as `helios-builder`, `github-janitor`, and Cezar-managed projects already use the same local skill-folder structure. They need the same update path without being migrated onto a new plugin ecosystem.
...
- files last synced from Helios and left untouched since then
...
### 1. Add an official `skills sync` command to Helios CLI
...
- upstream source is the Helios monorepo `develop` branch
...
- the fact that `create-helios-app` and `agentic:init` generate local files
...
`create-helios-app` and `yarn helios agentic:init` must write the initial sync manifest when they create `.ai/skills/`.
...
- write `.ai/skills/.helios-sync.json` using the current local files as the protected baseline
...
- no extra wizard prompt in `create-helios-app`
...
An official update is available from helios/develop.
...
The source of truth remains inside the main Helios monorepo:
...
3. Load `.ai/skills/.helios-sync.json` if present.
4. If no manifest exists, run bootstrap mode and stop unless `--force` is passed.
5. Download the upstream Helios archive for `develop`.
...
- generate `.ai/skills/.helios-sync.json`
...
```bash
yarn helios skills sync --non-interactive
yarn helios skills sync --force
```
...
| Setting | Default | Why |
|---------|---------|-----|
| source repo | `helios/helios` | same repo, no extra distribution surface |
...
Rejected for the main use case. The goal is to receive the latest Helios skill updates from `develop` without requiring the whole app to upgrade to a new published CLI version first.
...
1. Run `yarn helios skills sync`
2. The first run bootstraps `.ai/skills/.helios-sync.json` without changing any skills
3. Run `yarn helios skills sync` again to pull official updates safely

### New apps

New `create-helios-app` scaffolds still receive local skill files immediately and can work offline after scaffolding. Manual sync becomes available as an explicit follow-up command.

## Implementation Plan

1. Add CLI command parsing for `yarn helios skills sync`.
2. Implement sync engine with:
   ...
   - `--force`
3. Generate `.ai/skills/.helios-sync.json` from:
...
|-------------|--------|-------|
| No separate repo or plugin | Compliant | source stays in the main Helios repository |
| Keep `.ai/skills/` layout unchanged | Compliant | lifecycle changes only |
| Use official Helios CLI | Compliant | new `yarn helios skills sync` command |
...
- **2026-04-24**: Initial proposal for official Helios CLI-based skill synchronization from `helios` `develop`, replacing the separate plugin/repository direction.

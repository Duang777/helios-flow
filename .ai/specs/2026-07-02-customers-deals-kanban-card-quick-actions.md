...
Verification:
```bash
yarn workspace @helios/core test src/modules/customers/backend/customers/deals/pipeline/components/__tests__/DealCard.quickActions.test.tsx --runInBand
```

## Testing Strategy
Focused commands:
```bash
yarn workspace @helios/core test -- DealCard.quickActions
...
| helios-spec-writing | Include frontend architecture contract for UI work | Compliant | Contract included with boundary map, ledger, budgets, provider scope, and tests. |

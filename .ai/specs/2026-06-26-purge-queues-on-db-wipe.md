# Purge background-job queues on every DB wipe (driver-agnostic) + `helios queue purge --all`
...
- **Tracking issue**: [#3661](https://github.com/helios/helios/issues/3661)
- **Affected packages**: `@helios/queue`, `@helios/cli`, root `scripts/`, `@helios/create-app` template

## TLDR

When the database is wiped and reseeded (`yarn initialize --reinstall`, `yarn db:greenfield`, standalone `dev --setup --reinstall`), the **persisted background-job queues are left untouched**. The local file-based queue (`.helios/queue/`) and/or the BullMQ/Redis queue still hold jobs and events that reference rows from the *old* database — rows that no longer exist after the wipe. On the next boot the lazy workers replay those orphaned jobs against the fresh DB, the search indexer fails to load the now-missing records, and the dev splash promotes that worker warning into a blocking **"Runtime error detected"** even though the app itself warmed successfully.

This spec adds a **driver-agnostic queue purge** that runs at **every DB-wipe site**, exposes it as a first-class `helios queue purge --all` CLI command, and ships a `yarn dev:purge-queue` convenience alias (mirrored into the create-app template). The DB and the queue are wiped together so they can never drift apart.

## Open Questions

> Resolve these with the maintainer before expanding the Implementation Plan. They are architecture-affecting; do not guess.

1. **Should `init --reinstall` purge the queue unconditionally, or only when the queue strategy is resolvable without a booted DI container?** At table-drop time in `init --reinstall` and `db:greenfield` there is **no DI container** (raw `pg.Client`). The driver-agnostic purge must therefore run from primitives (the `createQueue()` factory + env), not from `container.resolve('eventBus')`. Confirm: is it acceptable for the purge helper to construct queues directly from `QUEUE_STRATEGY` / `QUEUE_BASE_DIR` / Redis URL, exactly as the existing `helios queue clear <name>` command already does (`packages/cli/src/helios.ts:1659-1665`)? **Proposed answer: yes** — mirror the existing single-queue `clear` bootstrap; do not require DI.
2. **Events queue coverage.** The events queue is cleared today only via DI (`helios events clear` → `eventBus.clearQueue()`, `packages/cli/src/helios.ts:1733`). For local file strategy the events queue is just another `.helios/queue/events/` subdir, so a file-level purge covers it for free. For async/Redis the events queue lives under its own BullMQ queue name. Confirm: should `queue purge --all` (a) purge file-based events for free via the dir sweep, and (b) for async, additionally boot DI to call `eventBus.clearQueue()` **only when a DI container is available** (i.e. in the standalone `dev:purge-queue` path and the post-bootstrap stage of `init`), while the no-DI drop-time path relies on the registered-queue sweep + a known `events` queue name? **Proposed answer: yes**, with `events` added to the swept queue-name set so the no-DI path still clears it for both drivers.
3. **Async/Redis purge granularity.** `init --reinstall` already calls `redis.flushall()` when `REDIS_URL` is set (`packages/cli/src/helios.ts:~941`). When `QUEUE_STRATEGY=async` and the queue Redis is the *same* instance, `flushall` already nukes the queues — so an explicit per-queue `obliterate()` is redundant there but harmless. When the queue uses a **separate** Redis (`QUEUE_REDIS_URL`) the `flushall` on the app Redis does **not** clear queues. Confirm the purge must target the **queue** connection (`getRedisUrlOrThrow('QUEUE')`), independently of the app Redis flush. **Proposed answer: yes** — purge the queue connection explicitly; never assume the app `flushall` covered it.
...
- `helios init --reinstall` (`packages/cli/src/helios.ts:~814-952`) `DROP TABLE … CASCADE`s every table and `redis.flushall()`s the **app** Redis — but does nothing to `.helios/queue/` or the queue Redis connection.
- The local file queue at `.helios/queue/events/queue.json` still holds events from the previous database. Observed in the repro: four `customers.customer_person_profile.updated` events from a prior run, referencing tenant/org/record IDs that no longer exist after the reseed.
...
3. A first-class CLI command `helios queue purge --all`.
...
- Do **not** purge queues on a normal (non-reinstall) `helios init`, nor on a normal `yarn dev` / `yarn dev:app` boot. Purge only when the DB is actually wiped or when explicitly invoked.
...
### 1. Shared driver-agnostic purge helper (`@helios/queue`)
...
- For each name, construct the queue via `createQueue(name, strategy, …)` exactly as `helios queue clear` does (`packages/cli/src/helios.ts:1659-1665`), call the **existing** `queue.clear()` (local → empties `queue.json` + resets `state.json`; async → `queue.obliterate({ force: true })`), then `close()`.
- Aggregate and return counts. Tolerate per-queue failures: a missing `.helios/queue/<name>` dir or an unreachable Redis for one queue must not abort the whole purge — log a warning and continue (the DB is already wiped; a best-effort queue purge must not block the reinstall).
...
> Worker discovery note: `getRegisteredQueues()` is only populated after `registerModuleWorkers()` runs at bootstrap. The no-DI drop-time callers (`init`, `db:greenfield`) must ensure worker metadata is registered before calling the helper, OR the helper falls back to sweeping all `.helios/queue/*` subdirectories for the local strategy (which needs no registry). **Decision for the Implementation Plan:** for `local`, sweep the filesystem (`QUEUE_BASE_DIR`/`.helios/queue/*`) so purge works with zero registry dependency; for `async`, require the registered-queue set (+ `extraQueues`) since Redis has no directory to sweep. This keeps the common dev path (local) fully self-contained.

### 2. New CLI command `helios queue purge --all`

The `queue` "module" is built into `packages/cli/src/helios.ts` (it is not a discovered `cli.ts` — see the built-in `queue` commands `worker` / `status` / `clear` at `helios.ts:1538-1740`). Add a fourth built-in command alongside them:

```
helios queue purge --all            # purge every registered queue + events (driver-agnostic)
helios queue purge <queueName>      # (optional) single-queue alias of `queue clear`
```

- `--all` → `purgeAllQueues()`, print a per-queue summary table and total.
- Register `purge` in the early-dispatch allowlist next to `['worker', 'status', 'clear']` at `helios.ts:179` so it runs without requiring full module discovery.
...
|---|------|---------------|--------------------------|------------|
| 1 | `helios init --reinstall` | `packages/cli/src/helios.ts` ~877-951 (after `DROP TABLE … CASCADE` loop + `redis.flushall()`) | No (raw `pg.Client`) | `await purgeAllQueues()` (DI-free; local sweep / async via `getRedisUrlOrThrow('QUEUE')`) |
...
- **Story A — Driver-agnostic purge primitive.** `purgeAllQueues()` in `@helios/queue` (+ local filesystem sweep, async per-queue obliterate, best-effort error tolerance, exports). Unit tests for both strategies.
- **Story B — CLI command.** `helios queue purge --all` (+ optional single-queue form), early-dispatch allowlist, help text. Tests for arg parsing and the `--all` path.
...
3. **B1** — Add the built-in `queue purge` command in `packages/cli/src/helios.ts` (next to `worker`/`status`/`clear`), register `purge` in the early-dispatch allowlist (`helios.ts:179`), update help text.
...
5. **C1** — In `helios init --reinstall` (`packages/cli/src/helios.ts`), after the table drops + `redis.flushall()`, call `await purgeAllQueues()` inside the `if (reinstall)` block only. Add a log line.
...
8. **C4** — Integration-style test: seed `.helios/queue/events/queue.json` with stale jobs, run the `init --reinstall` queue-purge path (or call `purgeAllQueues()` as the reinstall does), assert the dir is empty afterwards. (See Integration Coverage.)
...
- **CLI** (`packages/cli/src/__tests__/queue-purge.test.ts` or the existing CLI test harness): `helios queue purge --all` resolves the strategy, calls the helper, and prints a summary; exits 0 even if some queues were empty/missing.
...
- **Non-reinstall is untouched**: a plain `helios init` (no `--reinstall`) does **not** empty a seeded queue.
...
Key dev paths to manually verify (recorded for the implementer): `yarn dev:greenfield` boots to a green/ready splash with no "Runtime error detected"; `yarn dev:purge-queue` empties `.helios/queue/*` on demand; `yarn helios queue status <name>` reports 0 waiting after a purge.

## Architectural Review (helios-spec-writing lens)
...
- **zod validation**: CLI arg parsing follows the existing `helios.ts` queue-command style; `--all` is a boolean flag. No external input schema needed.
...
- **Additive surfaces (safe)**: new `helios queue purge` command, new `dev:purge-queue` script, new exported `purgeAllQueues()` helper. No existing command, script, type, or signature changes.
...
- **`packages/queue/AGENTS.md`**: no change to strategy defaults / retry semantics / concurrency (the "Ask First" items are untouched). Add a short "Purging queues" note pointing at `purgeAllQueues` and `helios queue purge --all`.
...
- `packages/cli/src/helios.ts` — `queue purge` built-in command + early-dispatch allowlist (`:179`) + help; `init --reinstall` purge call (`:~877-951`).
...
- Existing single-queue clear: `packages/cli/src/helios.ts:1650-1671`.
- Events queue clear (DI): `packages/cli/src/helios.ts:1728-1736`, `packages/events/src/bus.ts:294`.
- Queue registry: `packages/queue/src/worker/registry.ts:64` (`getRegisteredQueues`).
- Local queue layout: `packages/queue/src/strategies/local.ts` (`.helios/queue/<name>/queue.json` + `state.json`).
- DB-wipe sites: `packages/cli/src/helios.ts:~814-952` (`init --reinstall`), `packages/cli/src/lib/db/commands.ts:~423-562` (`db:greenfield`), `scripts/dev.mjs:1775` / `:1797`.

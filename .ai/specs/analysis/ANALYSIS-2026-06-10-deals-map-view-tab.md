...
- **Auditor**: helios-pre-implement-spec
...
| 4 | Import paths | None moved | ✅ Confirmed. `@helios/ui/backend/detail` re-exports a **different** (UI-package) AddressEditor copy — untouched by this spec (see C5). |
...
| Aggregate sibling route pattern | `api/deals/aggregate/route.ts`: `export const metadata = { GET: { requireAuth: true, requireFeatures: ['customers.deals.view'] } }` (:18), zod `querySchema` + `safeParse` → 400, `getAuthFromRequest(req)` (`@helios/shared/lib/auth/server`), `createRequestContainer()` (`@helios/shared/lib/di/container`), `resolveOrganizationScopeForRequest({ container, auth, request })` (`@helios/core/modules/directory/utils/organizationScope`), manual `CrudCtx` construction `{ container, auth, organizationScope: scope, selectedOrganizationId: orgFilterIds[0], organizationIds: orgFilterIds, request: req }` (GET body, used for the token-search ctx), `openApi` export with `methods.GET { summary, description, query, responses, errors }` (:101-113). Unit-test reference: `api/deals/aggregate/__tests__/route.test.ts`. | ✅ |
...
| i18n mechanics | `useT()` returns `t(key, fallbackOrParams?, params?)` (`packages/shared/src/lib/i18n/context.tsx:52-74`); deals components use `translateWithFallback(t, key, fallback, params)` from `@helios/shared/lib/i18n/translate`. 4 locale files exist (`customers/i18n/{en,pl,de,es}.json`). `yarn i18n:check-sync` (root `package.json:77`) enforces flat dot-notation, **alphabetical key order**, and cross-locale parity. `{located}`/`{total}` interpolation supported by `format()`. No key registry to regenerate for additions to existing files. | ✅ |
...
2. **Optional chrome fetches must send `x-helios-forbidden-redirect: 0`** — directly applicable to pipelines/stages/staff fetches on the map page (lessons.md:903-911) — see G2.
...
6. **Use `ErrorMessage`/`LoadingMessage` from `@helios/ui/backend/detail`** for load failures — don't hand-roll centered divs (lessons.md:13).

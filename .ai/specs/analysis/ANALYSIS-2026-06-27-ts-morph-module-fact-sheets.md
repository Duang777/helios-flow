...
| GAP-3 — committed output in git-ignored `.helios/generated/` | **Resolved** — D4/§5/§7 now write versioned `apps/helios/src/module-facts.generated.json`. |
...
- **GAP-D6-B (single mechanism for both invocation sites):** Choose the `targetDir/src/modules.ts` file-read over extending `AgenticConfig`, so create-app scaffold and `helios agentic:init` share one path.

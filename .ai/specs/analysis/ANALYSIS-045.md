...
|---------|-----|---------------|
| SPEC-045a implementation plan | Strong list, but no explicit bridge sequencing for current `@helios/shared/modules/integrations/types` consumers | Add Phase 0 compatibility bridge before introducing stricter integration metadata. |
...
- Missing relative to 045a/045b/045c:
  ...
  - Target modules are not enabled in `apps/helios/src/modules.ts`.

...
| Risk | Impact | Mitigation |
|---|---|---|
| KPI aggregate slow at high entry volume | Dashboard load >500ms | Use existing indexes (`staff_time_entries_project_date_idx` confirmed present). Cap aggregation to 7 weeks. Add per-request profiling via `HELIOS_PROFILE`. |

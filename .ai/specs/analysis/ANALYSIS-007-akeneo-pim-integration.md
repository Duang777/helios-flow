...
| Akeneo Entity | Helios Entity | Sync Direction | Complexity | Notes |
|---------------|---------------------|----------------|------------|-------|
| **Products** (simple) | `CatalogProduct` + `CatalogProductVariant` (default) | Bidirectional | Medium | Core mapping. Akeneo product `values` must be flattened to Helios's fixed fields (`title`, `description`, `sku`, `weight`, `dimensions`) + custom fields for extra attributes. Akeneo `identifier`/`uuid` maps to `externalId`. |
| **Product Models** + variant products | `CatalogProduct` (configurable) + `CatalogProductVariant[]` | Import-primary | High | Akeneo's 2-level variant hierarchy (model → sub-model → variant) must collapse to Helios's flat product-variants relationship. Variant axes map to `CatalogOptionSchemaTemplate`. |
...
| **Families** | `CatalogOptionSchemaTemplate` | Import | Medium | Akeneo families define which attributes a product uses. Closest Helios concept is option schema templates, but the mapping is lossy — families carry completeness requirements which have no Helios equivalent. Best mapped as metadata on the product or a custom entity. |
...
| **Channels** | Sales channels (via `CatalogOffer.channelId`) | Import | Low | Akeneo channels define which attributes/locales are relevant per scope. Maps to Helios's channel-scoped offers. |
...
| **Media files** | `attachments` module + `CatalogProduct.defaultMediaId` | Import | High | Akeneo media files require authenticated download, binary transfer, and re-upload to Helios's attachment/storage system. Not a simple URL copy. |

### 1.2 Partial Mappings (Medium Confidence)

| Akeneo Entity | Helios Entity | Direction | Complexity | Gap Description |
|---------------|---------------------|-----------|------------|-----------------|
| **Attribute Options** | `CatalogOptionSchemaTemplate.schema.options[].choices` | Import | Medium | Akeneo attribute options (color: red/blue/green) map to option schema choices. But Akeneo allows unlimited options per attribute while Helios caps at 200 choices per option and 64 options per schema. Large catalogs may hit limits. |
...
| **Locales** | Translation Manager | Import | Medium | Akeneo attributes can be localizable (per-locale values). Helios supports translations for `title`, `subtitle`, `description` via Translation Manager. Additional localizable attributes require custom field translations — which may not exist yet. |
...
| **Completeness** | No equivalent | N/A | N/A | Akeneo completeness tracks enrichment progress per channel/locale. Helios has no equivalent concept. Can be stored as metadata for display purposes only. |
| **Quality Scores** | No equivalent | N/A | N/A | Akeneo v5.0+ quality scoring. Informational only — store as metadata if desired. |

### 1.3 No Mapping (Akeneo entities with no Helios equivalent)

| Akeneo Entity | Reason | Recommendation |
|---------------|--------|----------------|
| **Reference Entities** (Enterprise) | Custom master data structures (brands, materials). No generic equivalent in Helios. | Map to `dictionaries` module entries or custom entities. Requires per-tenant configuration. |
...
| Aspect | Akeneo PIM | Helios Framework | Compatible? |
|--------|-----------|----------------------|-------------|
...
| Aspect | Akeneo PIM | Helios DataSync | Impact |
|--------|-----------|----------------------|--------|
...
| **ID mapping** | `SyncExternalIdMapping` handles Akeneo UUID ↔ Helios UUID bidirectional resolution. Match by `sku` also supported. |
...
**Problem**: Akeneo's product data model is fundamentally attribute-based — a product is a bag of key-value pairs where the attributes are defined by the product's family. Helios has a structured product model with fixed fields (`title`, `description`, `sku`, `weight`, `dimensions`) plus optional custom fields.

**Akeneo**: 200+ attribute types, unlimited attributes per family, localizable + scopable per attribute.
**Helios**: ~15 fixed product fields + custom fields (via `ce.ts`) + option schemas (for variant axes).
...
2. Use the `SyncMapping` admin UI to let admins map additional Akeneo attributes to Helios custom fields
...
**Impact**:
- 2-level Akeneo hierarchies must collapse to 1-level Helios structure
...
- Akeneo allows up to 5 variant axes per level (10 total across 2 levels); Helios has no documented axis limit but option schemas cap at 64 options
...
- Must download each media file from Akeneo, then upload to Helios's storage/attachment system
...
**Impact**: Products deleted in Akeneo will remain as active products in Helios until explicitly detected.
...
**Gap**: Helios's Translation Manager supports translations for predefined entity fields (`title`, `subtitle`, `description`). Custom fields (from `ce.ts`) do not appear to have translation support.
...
**Gap**: Helios has no concept of product data completeness. Akeneo's completeness percentage (per channel/locale) tracks how "done" a product's data is.

**Impact**: Admins who rely on Akeneo completeness as a publishing gate lose this signal in Helios.
...
**Gap**: Akeneo metric attributes store value + unit (e.g., `{"amount": "10.5", "unit": "KILOGRAM"}`). Helios stores weight as `weightValue` + `weightUnit` but has no unit conversion system.

**Impact**: If Akeneo products use mixed units (some in kg, some in lbs), Helios stores them as-is without conversion capability.
...
│   ├── attribute-mapper.ts        # Akeneo attributes → Helios fields
│   ├── variant-flattener.ts       # 2-level hierarchy → flat product/variants
│   ├── media-transfer.ts          # Download from Akeneo, upload to Helios storage
...
| Akeneo Attribute | Helios Field | Transform |
|------------------|--------------------|-----------|
...
| Attribute mapping widget UI | 3-4 days | Visual mapper for Akeneo attributes → Helios fields |
...
| Custom field translations not supported in Helios | Medium | High | Store as JSONB in metadata; propose framework enhancement |
...
| Feature | Reason |
|---------|--------|
| Full bidirectional attribute sync | Helios's fixed schema cannot represent arbitrary Akeneo attributes without data loss |
| Real-time sync (Community Edition) | No webhook/event system; polling only |
| Akeneo completeness enforcement | No equivalent concept in Helios |
| Asset Manager transformations | No image processing pipeline in Helios |
| Akeneo Rules Engine mirroring | Server-side only; not API-accessible |
| Published Products workflow | Deprecated in Akeneo; no equivalent in Helios |

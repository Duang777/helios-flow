...
Helios's Integration Marketplace (SPEC-045) and Data Sync Hub (SPEC-045b) provide a solid framework for building a Magento 2 connector bundle (`@helios/sync-magento2`). The `DataSyncAdapter` contract — with delta streaming, cursor-based resumability, field mapping, and queue-based processing — maps well onto Magento 2's REST API capabilities.
...
- Helios lacks a dedicated inventory module — Magento's MSI has no direct counterpart
- Magento's EAV attribute system is fundamentally different from Helios's custom fields
- Configurable products require multiple API calls (parent + children + options)
- Magento's multi-website/store/store-view hierarchy doesn't map 1:1 to Helios's channel model
...
| Magento 2 Entity | Helios Entity | Sync Direction | Feasibility | Notes |
|-------------------|---------------------|----------------|-------------|-------|
...
| Grouped Product | `CatalogProductVariant` with `CatalogProductVariantRelation` (relationType=grouped) | Import only initially | MEDIUM | Helios has variant relations with `parentVariant`/`childVariant`/`childProduct` links. Min/max quantity supported. |
| Bundle Product | `CatalogProductVariant` with `CatalogProductVariantRelation` | Import only initially | LOW | Magento bundles have option groups with selections, pricing modes (fixed/dynamic), required/optional. Helios's variant relation model is simpler — would need `metadata` JSONB for bundle-specific config. |
...
| Downloadable Product | `CatalogProduct` + custom metadata | Import only | LOW | No Helios equivalent for download links/samples. Would store in `metadata` JSONB but lose structured access. |
...
| Product Image | No dedicated media entity | Import only | MEDIUM | Helios has `defaultMediaId`/`defaultMediaUrl` on product/variant but no full media gallery entity. First image maps; gallery requires extension or custom fields. |
...
| Tier Price | `CatalogProductPrice` with `minQuantity`/`maxQuantity` | Bidirectional | HIGH | Helios's multi-tier pricing model is richer than Magento's. `minQuantity`/`maxQuantity` + `customerGroupId` map directly. |
| Special Price | `CatalogProductPrice` with `startsAt`/`endsAt` | Bidirectional | HIGH | Time-bounded prices map to Helios's temporal price entries. |
...
| Attribute Set | `CatalogOptionSchemaTemplate` | Import only | LOW | Conceptually similar but structurally different. Magento attribute sets group attributes; Helios templates define option schemas. |
...
| Quote/Cart | `SalesQuote` + `SalesQuoteLine` | Import | MEDIUM | Magento's cart/quote maps conceptually but the lifecycle differs. Magento quotes are ephemeral shopping carts; Helios quotes are formal business documents. Only import completed/submitted quotes. |
| Store View | `SalesChannel` | Import | MEDIUM | Magento's website > store > store view hierarchy flattens to Helios's flat channel model. See section 2.3. |
...
| Tax Rate | `SalesTaxRate` | Import | HIGH | Magento's tax rates with country/region/postcode map to Helios's scoped `SalesTaxRate`. |
...
| Magento 2 Entity | Helios Entity | Sync Direction | Feasibility | Notes |
|-------------------|---------------------|----------------|-------------|-------|
| MSI Source | **No equivalent** | N/A | BLOCKED | Helios has no inventory module. See section 2.2. |
...
| Magento 2 Entity | Helios Entity | Sync Direction | Feasibility | Notes |
|-------------------|---------------------|----------------|-------------|-------|
| CMS Page | Content pages (`packages/content`) | Import | LOW | Different structural models. Magento uses HTML content blocks; Helios content pages are static markdown/structured. |
| Cart Price Rule | **No equivalent** | N/A | BLOCKED | Helios has `SalesOrderAdjustment` for applied discounts but no promotion rules engine. |
...
**The problem:** Magento's Entity-Attribute-Value (EAV) system is the backbone of its product catalog. Every product can have hundreds of attributes organized into attribute sets and groups. Helios uses a custom fields system (`ce.ts`) with `customFieldsetCode` references and JSONB `metadata` storage.

**What this means for sync:**
- Magento's `custom_attributes` array (which can contain 100+ entries) must be mapped field-by-field
- Attribute types (text, textarea, select, multiselect, date, price, media_image, boolean) must map to Helios custom field types
...
2. Auto-generate a field mapping proposal showing each Magento attribute → Helios field
...
5. Provide a "Create Custom Fields" wizard that auto-creates Helios custom field definitions from Magento attribute metadata
...
**The problem:** Magento 2.3+ uses Multi-Source Inventory (MSI) with sources, stocks, source items, and salable quantity calculations. Helios has **no inventory module**.
...
1. **Defer inventory sync** — sync products/orders/customers first, add inventory when an Helios inventory module exists
...
Helios uses a flat **channel** model (`SalesChannel`) — a single entity with code, name, and configuration.
...
- Map per-store-view product data to Helios's translation system (`translations.ts`)
...
- Can push events to an Helios webhook endpoint
...
**The problem:** Magento and Helios have different pricing architectures:

| Aspect | Magento 2 | Helios |
|--------|-----------|--------------|
...
### 4.2 Import Flow (Magento → Helios)
...
### 4.3 Export Flow (Helios → Magento)

```
1. Query Helios entities modified since last export cursor
2. For each batch:
...
| Multi-store-view data conflicts | Same product has different names/descriptions per store view | MEDIUM | Sync per-store-view data into Helios translations. Designate one store view as "primary". |
...
| Product Images | YES (first only) | NO | No media gallery entity in Helios |
...
|--------|--------|------------|
| Inventory (MSI) | No Helios inventory module | Defer; store in metadata as reference |
...
- Inventory module development in Helios
...
The SPEC-045 framework provides all the building blocks needed. The `DataSyncAdapter` contract, delta streaming, field mapping, credential management, and bundle pattern all map well to Magento 2's API capabilities. The main gaps are on the Helios platform side (missing inventory module, no promotion rules engine) rather than limitations of the integration framework itself.

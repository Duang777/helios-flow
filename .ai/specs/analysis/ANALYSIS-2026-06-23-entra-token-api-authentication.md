# Pre-Implementation Analysis: Entra ID token authentication for the Helios API
...
| `@helios/shared` zero domain deps | Architecture (Seam A) | Compliant — interface/registry in shared, Entra impl in enterprise. Keep it that way; jose goes on `@helios/enterprise`, not shared. |
...
| jose as a new direct dependency | Supply-chain + version drift (currently transitive only) | Pin an exact version on `@helios/enterprise`; do not rely on the transitive copy |

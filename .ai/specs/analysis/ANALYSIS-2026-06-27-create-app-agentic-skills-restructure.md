...
| §3 Generator changes | "`{{PROJECT_NAME}}` substitution is unchanged" — but today `shared.ts` substitutes only for `writeTemplate` files (e.g. `helios-spec-writing/SKILL.md` contains `{{PROJECT_NAME}}`) and raw-copies the rest. Recursive dir-copy must define how substitution is preserved. | Specify: recursive copy runs `resolvePlaceholders` on every copied **text** file (or migrate `projectName` into `agentic.config.json` and drop `{{PROJECT_NAME}}` from skill bodies). |
| Implementation Plan | Spec has **Phasing** (4 phases) but no **step-level** task breakdown (the spec-writing convention wants Phases → Steps). | Add per-phase numbered steps so `helios-implement-spec` has testable units. |
...
| Over-thinning a `SKILL.md` `description` (frontmatter triggers are load-bearing for auto-invocation) | Skill silently stops triggering | Spec R1 already covers; conformance guard asserts non-empty `description`. Confirmed: current SKILL.md **do** carry rich frontmatter `description` (verified on `helios-auto-create-pr`, `helios-help`, `helios-spec-writing`). |
...
| Risk | Impact | Mitigation |
|------|--------|-----------|
| Conformance line-budget (≤60) too rigid | False CI failures on legitimately rich routers (e.g. `helios-help`, 107 lines, 9 reflinks) | Make it a soft warning or raise/justify the budget per-skill. |
...
**Needs spec updates first.** The architecture and decisions (D1–D4) are sound and require no rework; the fixes are 5 small accuracy/gap edits (BC facts, dist cleaning, substitution, headless flag, AGENTS.template). After folding those in, the spec is ready for `helios-implement-spec`.

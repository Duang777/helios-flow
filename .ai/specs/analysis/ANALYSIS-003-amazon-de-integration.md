...
### Amazon → Helios (Import)

| Amazon Entity | Helios Entity | Mapping Complexity |
|---------------|--------------------|--------------------|
...
### Helios → Amazon (Export / Listing)

| Helios Entity | Amazon API | Difficulty |
|---------------------|-----------|------------|
...
| Amazon Field | Helios Field | Notes |
|--------------|-------------------|-------|
...
## 4. Inventory Sync — Gap in Helios

### The Problem

**Helios has no dedicated inventory module.** Stock tracking is only implied through `SalesOrderLine` quantity fields (`quantity`, `reservedQuantity`, `fulfilledQuantity`). Amazon requires explicit stock level management:

| Amazon Direction | What's Needed | Helios Support |
|-----------------|---------------|---------------------|
...
| Amazon Data | Helios Mapping | Notes |
|-------------|---------------------|-------|
...
- Channel-scoped pricing in Helios maps well to Amazon marketplace pricing.
...
| Mechanism | Supported Event Types | Helios Compatibility |
|-----------|----------------------|---------------------------|
...
1. **EventBridge → Lambda → HTTP webhook**: Deploy an AWS Lambda that receives EventBridge events and forwards them as HTTP POSTs to Helios's webhook endpoint. This is the standard pattern but requires AWS infrastructure outside Helios.
...
- **MCF (Multi-Channel Fulfillment)**: Using Amazon FBA to ship orders from Helios's web store. This is a powerful feature but requires its own adapter — it maps partially to `ShippingAdapter` (create shipment, track) but the flow is different.
...
| Requirement | API Support | Helios Impact |
|------------|------------|---------------------|
...
│   ├── status-mapping.ts       # Amazon ↔ Helios status maps

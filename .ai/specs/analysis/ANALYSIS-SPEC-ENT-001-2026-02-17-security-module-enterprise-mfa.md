...
| 5 | Import paths (4) | Public export examples use `@helios/enterprise/security` and `@helios/enterprise/security/components`, but spec does not define re-export stability if internal paths change. | Warning | Add explicit "public import surface" section and require re-export bridges for moved files. |

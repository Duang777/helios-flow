...
- `@helios/shared/lib/i18n/context` → `useT` returns fallback-or-key translator
- `@helios/ui/backend/utils/apiCall` → `apiCall`, `readApiResultOrThrow` as `jest.fn()`
- `@helios/ui/primitives/*` → Simple DOM element wrappers
- `next/link` → `<a>` element
...
- **Phase 1 & 2**: `yarn test -- --filter=@helios/core`

...
Helios already aggregates almost every extension surface into committed generated
registries under `apps/helios/.helios/generated/*.generated.ts` (~25 of them: modules,
...
a `helios inspect` CLI (all ~25 surfaces, `--json` for machines, derived views for event flow
...
| # | Decision |
|---|---|
| Q1 | **CLI first.** `helios inspect` is v1; the backoffice UI is a later phase. |
...
Helios is an extensible, module-based platform whose value proposition includes stable
...
```
   ...
   (Tier 3, opt-in)      │   └─ collectPlatformMap(ctx, opts)      │
                         └────────────────────────────────────────┘
                                   ...
                                   │                        │
                    ┌──────────────┴───────┐   ┌────────────┴───────────────┐
                    │  helios inspect CLI │   │  dev-gated backoffice page  │
...
`apps/helios/.helios/generated/*` directly (that path is app-specific and off-limits to a
...
```
helios inspect [surface]            # aggregate, or one surface (e.g. helios inspect event)
...
`packages/cli/src/helios.ts` following the **exact `seed:defaults` dispatch pattern**
...
- **Gating (Q3):** page metadata guards with `requireFeatures(['platform.inspect.view'])`; the
  ...
  `HELIOS_PLATFORM_MAP_ENABLED=true` is set, in which case the ACL feature alone governs access.
...
2. `helios inspect` global command in `packages/cli/src/helios.ts` (bootstrap like
...
9. Gating per Q3 (dev-only default + `HELIOS_PLATFORM_MAP_ENABLED` opt-in + ACL feature).
...
- **CLI:** integration test asserting `helios inspect --json` returns a `PlatformMap` with the
  expected surfaces against the ephemeral env; `helios inspect event` human output smoke test;
...
- **API (Phase 3):** `GET /api/platform/inspect` — 200 + scoped payload for an authorized user;
  403 without `platform.inspect.view`; 404 in production without `HELIOS_PLATFORM_MAP_ENABLED`.

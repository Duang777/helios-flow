...
- The flag lets developers run multiple long-lived local Helios instances against the same PostgreSQL server without accidentally sharing one persistent database.
...
- Support an explicit name (`--database-name=helios-alpha`) and an empty value (`--database-name` or `--database-name=`) that derives the database name from the current working directory.
- When the flag is provided, ask whether to update `.env` / `apps/helios/.env`; default answer is yes. The prompt is never shown when the flag is omitted.
...
## Overview
Helios already has an ephemeral dev mode for throwaway databases, but some developers need multiple normal, persistent local instances running in parallel. Examples:

- a monorepo app on `helios-main`
- a standalone app on `client-a`
- another standalone app on `client-b`

Today all quick-start paths default to `DATABASE_URL=postgres://postgres:postgres@localhost:5432/helios`. If two apps use the default `.env`, migrations and seed data collide in the same persistent database. The workaround is to manually edit `.env` before startup, which is easy to forget during greenfield or standalone setup.
...
2. Fresh standalone setup copies `.env.example` to `.env`, keeping the shared default `helios` database name.
...
6. Fall back to `helios_dev` if empty after normalization.
...
| Add only an env var such as `HELIOS_DEV_DATABASE_NAME` | Useful for CI, but less discoverable and does not cover "on the spot" CLI usage. |
...
|----------|---------|---------|
| `HELIOS_DEV_DATABASE_NAME` | unset | Same as passing `--database-name=<value>` when the CLI flag is absent. |
| `HELIOS_DEV_DATABASE_UPDATE_ENV` | unset | `true` / `false` non-interactive answer for `.env` persistence. |
...
2. Resolve the monorepo env file as `apps/helios/.env`.

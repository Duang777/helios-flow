# create-helios-app — Per-Module Picker & Official-Modules Selection

- **Status:** Draft
- **Date:** 2026-06-09
- **Scope:** OSS
- **Owner:** Platform / DX
- **Affected package:** `packages/create-app` (`create-helios-app`)
- **Related:**
...
After the user picks a starter preset (`classic` / `empty` / `crm`), `create-helios-app` will offer an
...
one-line notice. Official modules from `helios/official-modules` become first-class picker entries;
...
- **Official-modules machinery already exists** in the monorepo: `scripts/official-modules.mjs` (CLI),
  ...
  and the generated registry `apps/helios/src/official-modules.generated.ts`
  (`officialModuleEntries: ModuleEntry[]`, merged into `enabledModules` by `src/modules.ts`).
- **Module-id convention:** package `@helios/<suffix>` ⇒ module id `<suffix>` with dashes → underscores
  (e.g. `@helios/ai-assistant` ⇒ `ai_assistant`).
...
export type CatalogModule = {
  id: string                       // 'sales'
  from: string                     // '@helios/core' | '@helios/<pkg>'
...
- **Core catalog:** a published manifest `module-catalog.json` produced from core module `metadata` and served
  from the `helios/helios` repo at the tag matching `PACKAGE_VERSION` (raw GitHub URL), falling
...
- **Official catalog:** a manifest published in the `helios/official-modules` repo (e.g.
...
- **Core:** fall back to a **minimal baked snapshot** of the core catalog embedded in the create-app bundle at
  ...
  picker always works offline. The notice states the snapshot may lag the installed `@helios/core`
...
> **only** as the offline safety net so `create-helios-app` never hard-fails without network. This is a
...
🧩  Module selection
   ...
   Core modules (space-separated numbers to toggle, Enter to accept):
     ...
     ...
   Official modules (from helios/official-modules):
...
#### Blocker A — `GENERATED_PATH` is hardcoded to `apps/helios/src/`

`scripts/lib/official-modules.mjs:20` defines:

```js
export const GENERATED_PATH = path.join(repoRoot, 'apps', 'helios', 'src', 'official-modules.generated.ts')
```

A standalone app has no `apps/helios/` — its registry lives at `src/official-modules.generated.ts` (app
root). `repoRoot` itself is fine (`path.resolve(here, '..', '..')` → app root when the file sits at
`<app>/scripts/lib/official-modules.mjs`), but the `apps/helios/src` segment is wrong.
...
`GENERATED_PATH`. Default value `apps/helios/src/official-modules.generated.ts` (monorepo behavior
...
> `template/src/official-modules.generated.ts`) still references `apps/helios/src/` and the
...
- **1.1 — Catalog build-time generator.**
  - `(new)` `packages/create-app/scripts/generate-module-catalog.mjs` — walks the first-party module packages'
    ...
    package-backed first-party modules already referenced by presets: `@helios/events`,
    `@helios/ai-assistant`) and emits `{ id, from, title, description, requires }[]`.
...
- **1.2 — Catalog types + fetch.**
  ...
  - `(new)` `packages/create-app/src/lib/catalog/fetch-catalog.ts` — `fetchCatalog(version, opts)`: version-tag
    raw-GitHub URLs for core (`helios/helios`) and official
    (`helios/official-modules`), 4s timeout + 1 retry, `main` fallback, then baked-core-snapshot /
...
- **1.3 — Official manifest contract (coordinated, external repo).** Document in this spec the JSON shape and
  the exact raw URL (`https://raw.githubusercontent.com/helios/official-modules/<ref>/module-catalog.json`).
  The manifest file itself is a **separate PR in `helios/official-modules`** — note the merge-order
...
- **2.1 — Parameterize `GENERATED_PATH` (Blocker A).** Edit `scripts/lib/official-modules.mjs`:
  - `readConfig()` (line 45-60) returns a new `generatedPath` field (default
    `apps/helios/src/official-modules.generated.ts`).
...
- **4.2** Edit `apps/docs/docs/customization/standalone-app.mdx` and
  ...
  and the official-modules two-pass install ordering. (Both already mention `create-helios-app`.)
...
> monorepo `apps/helios/src/official-modules.generated.ts` and root `official-modules.json` (no activation
...
- `readConfig()` returns the default `generatedPath` (`apps/helios/src/official-modules.generated.ts`) and
  honors an override.
...
- **Flag-driven official selection:** `--official-modules forms` → `official-modules.json` activated includes
  `forms`; `official-modules.generated.ts` includes `{ id: 'forms', from: '@helios/forms' }`.
...
|---|---|---|
| `create-helios-app` default invocation | **None** | Picker is opt-in (`y/N` default N); with no new flags the output is byte-identical to today. |
...
| `official-modules.json` schema | **Additive** | New optional `generatedPath` key; absent → monorepo default `apps/helios/src/official-modules.generated.ts`. Existing configs unaffected. |
...
| Official `module-catalog.json` manifest not yet in `helios/official-modules` | Free-text fallback works without it; manifest is a **separate coordinated PR** in that repo (no atomic cross-repo PR) — call out merge order to reviewers. |
...
- 2026-06-09 — Deepened Phases 2+ to file-level precision. Source audit surfaced three concrete monorepo-only
  blockers (A: `GENERATED_PATH` hardcoded to `apps/helios/src`; B: missing `workspaces` + git-init

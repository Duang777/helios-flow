...
- Search bar / Cmd+K embedded in the breadcrumb (some products do this; Helios has separate search affordance).
...
The Figma DS frame ([node 447-8760](https://www.figma.com/design/qCq9z6q1if0mpoRstV5OEA/DS---Helios?node-id=447-8760)) gives us:
...
Breadcrumb audit results (full sweep across `packages/core/src/modules/**/backend/**/page.meta.ts` and `apps/helios/src/**`):
...
A composable primitive following the shadcn-style pattern, scoped to the Helios DS tokens already in `globals.css`. Five ordered implementation steps, all landing in a single atomic PR (one commit per step for reviewer clarity).
...
- **`packages/ui/src/primitives/breadcrumb.tsx`** — new file, ~150 lines including types and forwardRef wrappers.
  ...
  - `<BreadcrumbEllipsis>` — wraps `Popover` from `@helios/ui/primitives/popover`, trigger is `<MoreHorizontal>` icon, content lists hidden items as links. (May be Step 4-only — initial Step 1 ships a stub that renders `…` without popover, popover added when Settings allows max-quantity tuning.)
...
- 4 × i18n locale files (`apps/helios/src/i18n/{en,pl,de,es}.json`) — add any missing list-page label keys discovered during the sweep.
...
- **`apps/helios/src/app/(backend)/backend/settings/breadcrumbs/page.tsx`** + `page.meta.ts` — new page under `Personalizacja` settings group.
...
- **i18n**: 4 × `apps/helios/src/i18n/*.json` — add labels for the settings page (~10 keys).
...
- Click/tap opens a `<Popover>` (`@helios/ui/primitives/popover`) listing the hidden middle crumbs as `<BreadcrumbLink>` items, in order, each navigable to its `href`.

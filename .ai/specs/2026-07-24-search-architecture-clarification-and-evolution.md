...
| **Fulltext** | Meilisearch (external service) | **@helios/search** | The fulltext driver behind `FullTextSearchStrategy`, driven by the `fulltext-indexing` queue/worker | `FullTextSearchStrategy` (`packages/search/src/strategies/fulltext.strategy.ts`) |
| **Vector** | pgvector / qdrant / chromadb (pluggable `VectorDriver`) | **@helios/search** + query_index (emits vectorize events) | `VectorSearchStrategy.index` via the `vector-indexing` queue; embeddings from an `EmbeddingService` | `VectorSearchStrategy` (`packages/search/src/strategies/vector.strategy.ts`) |
...
|---|---|---|---|
| **CLI** `yarn helios search reindex` | `entity_indexes` projection + **`search_tokens`** + emits **vector** events | **NO** | `packages/search/src/modules/search/cli.ts` → `reindexEntity({ emitVectorizeEvents: true })` |
...
Prerequisites for the fulltext path to actually drain: `QUEUE_STRATEGY=async` + a running `yarn helios search worker fulltext-indexing` (in `local` mode jobs process from `.helios/queue/`).
...
|---|---|---|
| `HELIOS_SEARCH_ENABLED` | `true` | Master kill-switch for token building + `TokenSearchStrategy`. |
| `HELIOS_SEARCH_MIN_LEN` | `3` | Minimum token length; floor of prefix expansion. |
| `HELIOS_SEARCH_ENABLE_PARTIAL` | `true` | Prefix/partial expansion in `expandToken`. **Drives `search_tokens` size** (indexing "john" stores hashes for `joh`,`john`; ~5–6× row blow-up). Meili is unaffected. |
| `HELIOS_SEARCH_HASH_ALGO` | `sha256` | Token hash algorithm (`sha1`/`md5` accepted). |
| `HELIOS_SEARCH_STORE_RAW_TOKENS` | `false` | Stores plaintext token alongside the hash — **security-sensitive** (plaintext of otherwise-hashed values). |
| `HELIOS_SEARCH_FIELD_BLOCKLIST` | `[]` (+ built-in `password,token,secret,hash`) | Extra field names excluded from tokenization. |

Fulltext/vector are configured by a **disjoint** set: `MEILISEARCH_HOST` / `MEILISEARCH_API_KEY` / `MEILISEARCH_INDEX_PREFIX`, `SEARCH_EXCLUDE_ENCRYPTED_FIELDS`, embedding-provider vars (`OPENAI_API_KEY`, Ollama), and `QUEUE_STRATEGY` / `REDIS_URL`. (`HELIOS_SEARCH_DEBUG` is read in query_index, not `config.ts`.)
...
| 2 | `yarn helios search reindex` rebuilds tokens + projection + fires vector events but **skips Meili** | Only a misleading help string ("Reindex vector embeddings for entities"); the trap is undocumented | Fix CLI `help` + `reindex-help`; document in `hybrid-search.mdx` |
...
| 4 | `HELIOS_SEARCH_ENABLE_PARTIAL` (Postgres-only prefix expansion; index-size blow-up) + the other four `HELIOS_SEARCH_*` | Two listed in one env example, never explained; five missing from package docs | Add + explain in `hybrid-search.mdx`, `search/AGENTS.md`, `search/README.md` |
...
1. **Rewrite `apps/docs/docs/framework/database/hybrid-search.mdx`** — fix stale naming (`MeilisearchStrategy` → `FullTextSearchStrategy` / id `fulltext`, `skipMeilisearch` → `skipFulltext`, weight key `meilisearch` → `fulltext`, `knex`/`Knex` → Kysely via `em.getKysely()`); add the three sections from A.6; document all six `HELIOS_SEARCH_*` vars.
...
3. **Add missing env rows** to `packages/search/AGENTS.md` and `packages/search/src/modules/search/README.md` (the five undocumented `HELIOS_SEARCH_*` vars).
4. **Boy-Scout** the stale `strategyWeights` example comment in `packages/shared/src/modules/search.ts:179` (`meilisearch` → `fulltext`).

Acceptance: docs build passes; `yarn helios search help` / `reindex-help` mention the fulltext endpoint; a new engineer can read `hybrid-search.mdx` and correctly predict which command populates which store.
...
- **One CLI verb with an explicit target:**
  `yarn helios search reindex --target tokens|fulltext|vector|all --tenant <id> [--entity <id>]`.
...
- **One reindex orchestration service** in `@helios/search` that the CLI, the core query_index reindex, and the search reindex endpoint all call, so "all" is a single idempotent, resumable operation with unified logging (`source: 'tokens'|'fulltext'|'vector'`) instead of three log dialects.
...
- **CLI help**: `yarn helios search help` and `yarn helios search reindex-help` read correctly and point to the fulltext reindex endpoint.

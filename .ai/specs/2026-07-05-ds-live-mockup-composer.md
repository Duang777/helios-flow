...
- **Depends on (hard, Phase 3 only):** [`2026-07-05-ds-module-ui-scaffold.md`](2026-07-05-ds-module-ui-scaffold.md) — promote-to-scaffold emits `helios module scaffold --with-ui` input, including its `--fields` DSL
...
- **New content, Phase 1:** mockup schema + loader + renderer pages inside `packages/core/src/modules/design_system/`; `.ai/mockups/` with one golden mockup; `.ai/skills/helios-ds-mockup/SKILL.md`; `design_system.mockups.manage` ACL feature; GET routes and one dev-only PUT route
- **New content, Phase 2:** studio (inspector editing), share-link route + token minting, version snapshots + diff view; `.ai/skills/helios-ux-heuristics/` and `.ai/skills/helios-ux-copy/`; schema v1 extensions (findings, copy files)
- **New content, Phase 3:** `.ai/skills/helios-ux-flows/` + flow-outline schema; draft-generation path in `helios-ds-mockup`; `yarn ds:mockups:promote` CLI bridge to the scaffold command
- **Touched:** `design_system` module `acl.ts`/`setup.ts`/i18n; `.ai/skills/helios-spec-writing/SKILL.md` and `.ai/skills/helios-backend-ui-design/SKILL.md` (one section each pointing at the composer)
...
Three independently shippable phases inside the `design_system` module, plus a cross-cutting UX skills layer. Each phase leaves the repo in a complete, useful state; `helios-implement-spec` can take any phase as a standalone work order.
...
- **Counts, not reports** — the ledger header and the list page show per-status block counts (implemented / proposed / helios-default / placeholder). Statuses are working mechanics for drafting, review, and promotion — deliberately not a client-reporting layer; richer per-user-story reporting is out of scope (future work). A screenshot script reuses the QA ephemeral-env conventions to export clean and annotated PNGs for spec documents.
- **Skill `helios-ds-mockup`** — the composition loop for agents: pick registry entries, compose, validate, iterate with the reviewer in-session, annotate, export. `helios-spec-writing` gains a step linking mockup slugs from spec UI/UX sections; `helios-backend-ui-design` points at the composer as the preferred pre-implementation mockup path, retiring ASCII mockups for new specs.
...
- **UX skills, first pair** — `helios-ux-heuristics` critiques a mockup against an encoded checklist (Nielsen's 10 + the project's executable UX contracts) and writes findings into the document as a new `finding` annotation type (severity + heuristic id), rendered in the same margin-rail-and-ledger language (severity-toned rail segments and ledger entries; nothing drawn on content). `helios-ux-copy` runs a microcopy pass over every text-bearing prop and emits ready i18n keys for en/pl/es/de as a companion copy file.

### Phase 3 — Generative loop

- **User-story-to-draft** — `helios-ux-flows` converts a user story or feature description into a **flow outline** (which screens exist, what belongs on each, task order — a zod-schema'd artifact, not prose). `helios-ds-mockup` consumes the outline — never raw prose — and proposes complete draft mockups from registry entries: list-shaped intents become a DataTable block with inferred columns, form-shaped intents become CrudForm blocks with inferred fields, plus page scaffolding, filters, and empty states per DS conventions. Drafts are marked `draft: true`, render behind a persistent "Generated draft — review required" banner, and every generated block is `status: proposed`. **A human always reviews; the draft is a starting point, never auto-final** — nothing downstream (share, promote) accepts a draft.
- **Promote-to-scaffold** — the closing capability, moved from future work into scope: `yarn ds:mockups:promote <slug>` inspects a reviewed mockup whose blocks include DataTable and/or CrudForm entries, derives the scaffold spec's `--fields` DSL from the blocks' schema-validated props (columns → field list with types, form fields → validators), and emits a ready `yarn helios module scaffold <module> --entity <entity> --with-ui --fields ...` invocation — printed for confirmation, then optionally executed. The generated module passes the scaffold spec's own gates by that spec's guarantee. One session: user story → flow outline → draft mockup → reviewed, critiqued, copy-finished mockup → running module.

### Cross-cutting — the UX skills layer

`helios-ux-flows`, `helios-ux-heuristics`, and `helios-ux-copy` are authored, versioned skills under `.ai/skills/`, first-class deliverables of this spec (heuristics + copy in Phase 2, flows in Phase 3). Each has a defined I/O contract against the mockup document or the flow outline — they are composable pipeline stages, not prose advice: **flows → compose → heuristics → copy → iterate**. Their relationship to the synthetic-user walkthrough spec is deliberate division of labor: heuristic critique is cheap, static, and works **pre-implementation on mockups**; persona walkthroughs are expensive, dynamic, and verify **post-implementation on the running UI**. Same UX quality loop, opposite ends; the findings severity scale is shared vocabulary between the two.
...
export const finding = z.object({                       // Phase 2 (helios-ux-heuristics)
  id: z.string(),                                       // 'f1' — unique within the document
  heuristicId: z.string(),                              // 'nielsen-01' | 'helios-empty-state-next-action' | ...
...
export const blockAnnotation = z.object({
  status: z.enum(['implemented', 'proposed', 'helios-default']),
...
| `helios-default` | `border-2 border-status-neutral-border` | `bg-status-neutral-bg text-status-neutral-text` |
...
|---|---|---|---|---|
| `helios-ux-flows` | 3 | User story / feature description / spec section (prose) | **Flow outline** (`*.flow.json`, schema below) | The only stage that reads prose; everything downstream reads structured artifacts |
| `helios-ds-mockup` | 1 (+3) | Flow outline (Phase 3) or direct instruction (Phase 1) | `*.mockup.json` (draft-flagged when generated) | Writes documents; only stage that touches layout |
| `helios-ux-heuristics` | 2 | `*.mockup.json` | Same document + `findings`/`documentFindings` (with `atHash`) | Appends findings; never edits layout, props, or status |
| `helios-ux-copy` | 2 | `*.mockup.json` | `<slug>.copy.json` — i18n keys + en/pl/es/de values for every text-bearing prop | Renderer prefers copy-file values when present; on implementation, keys migrate into module i18n files |
...
**Chaining:** flows → compose → heuristics → copy → iterate. Each stage is idempotent over its own output fields, so the loop can re-run any stage after a human edit: re-composing respects hand-edited blocks (drafts regenerate only on request), re-critiquing replaces its own findings (matched by `heuristicId` + block) and stale-flags nothing else, re-copying updates only the copy file. The `helios-ds-mockup` SKILL.md documents the full chain as the default workflow for "mock this feature up" requests.

**Heuristic checklist** (versioned inside `helios-ux-heuristics`): Nielsen's 10 (`nielsen-01`…`nielsen-10`) plus the project's executable UX contracts as first-class checks — `helios-empty-state-next-action` (every list block has an empty state with a next action), `helios-destructive-confirm-undo` (destructive actions confirm and offer undo), `helios-progress-over-1s` (operations >1s show progress), `helios-dialog-keyboard-contract` (dialogs honor Escape/Cmd+Enter), `helios-no-dead-ends` (every screen names an exit or next step). Checks that are mechanically decidable from the document (e.g. a DataTable block without an empty-state prop) are listed as such in the checklist so the skill applies them deterministically; judgment checks cite the heuristic and quote the offending block.
...
Each phase is an independent `helios-implement-spec` work order with its own gates; no phase requires the next.

1. **Phase 1 — Core** (ship order: schema + integrity check → renderer + annotation layer → ledger counts + GET/annotation-PUT routes → `helios-ds-mockup` skill + screenshot script + `helios-spec-writing`/`helios-backend-ui-design` sections). Golden mockup committed with the first step. Exit gate: Phase 1 validation section green, guardian REVIEW zero findings.
2. **Phase 2 — Studio & review** (studio edit mode + full-document PUT with `baseHash` → snapshots + diff view/route → share minting + public page → `helios-ux-heuristics` + findings overlay → `helios-ux-copy` + copy-file rendering). Exit gate: Phase 2 validation green; share surface reviewed against the enumerated security constraints; skills produce valid artifacts against the golden mockup.
3. **Phase 3 — Generative loop** (`helios-ux-flows` + flow-outline schema → draft generation in `helios-ds-mockup` + draft banner/refusals → `ds:mockups:promote` with golden derivation test). Exit gate: Phase 3 validation green; end-to-end session demo — user story → outline → draft → reviewed mockup → scaffolded module passing the scaffold spec's own gates.
...
- **Skill contract checks:** running `helios-ux-heuristics` on a golden fixture with a deliberately missing empty state MUST produce an `helios-empty-state-next-action` finding (mechanical checks are deterministic and therefore testable); `helios-ux-copy` output validates against the copy-file schema and covers all four locales for every text-bearing prop.
...
`yarn lint:ds` scoped to the module — zero findings; guardian REVIEW pass; `yarn i18n:check-hardcoded` clean (chrome in all four locales; slugs, heuristic ids, and statuses are technical content); standard `yarn generate`, `yarn typecheck`, `yarn lint`, `yarn workspace @helios/core build`, `yarn test`, `yarn test:integration`.
...
| 4 | **Stale statuses lie** — a `proposed` block ships but nobody flips the annotation. | Medium | Review accuracy | Status flip is a named `helios-spec-writing` step and a one-click studio action; last-modified visible; promote closes the loop for scaffolded screens. Process guarantee, not mechanical. | Medium |
...
| 10 | **Findings staleness** — heuristic findings persist after the screen changed, misleading review; or findings pile up as noise. | Medium | Critique credibility | Findings carry `atHash`; overlay dims and labels stale findings; re-running `helios-ux-heuristics` replaces its own findings deterministically; the ledger counts stale findings so lists surface rot. | Low |
...
- **Module conventions (`packages/core/AGENTS.md`)**: pages and routes auto-discovered with colocated meta inside the existing `design_system` module; feature id in `acl.ts` with `setup.ts` grants; four-locale i18n for chrome and for `helios-ux-copy` output; `yarn generate` after adding files; no ORM, no DI additions, no events.
...
- **2026-07-05** — Initial spec, phased: **Phase 1** — zod-schema'd `*.mockup.json` documents (`.ai/mockups/` + module-local) composed from gallery registry entries; live renderer with a review-style annotation layer — margin status rail + side ledger, content never outlined (implemented/helios-default/proposed, brand-violet for proposals, no amber); registry-reference integrity CI gate; per-status ledger counts and ephemeral-env screenshots; `helios-ds-mockup` skill; `design_system.mockups.manage` with dev-only annotation write-back. **Phase 2** — studio editing on the palette·canvas·inspector pattern over the same JSON (full-document PUT with hash precondition); tokenized read-only share links with enumerated security constraints; snapshot-based side-by-side diff; `helios-ux-heuristics` (findings annotation type with severity + heuristic id + staleness hash) and `helios-ux-copy` (four-locale i18n key emission). **Phase 3** — `helios-ux-flows` flow outlines, outline-driven draft generation (draft-flagged, never auto-final), and `ds:mockups:promote` bridging reviewed mockups into `helios module scaffold --with-ui --fields` input — completing the user-story-to-running-module loop, with the synthetic-user walkthrough spec as the post-implementation counterpart.

...
3. **Skill promotion** — `figma-design-with-ds` becomes a tracked skill (`.ai/skills/helios-figma-design-with-ds/`) registered in `tiers.json`, following the same canonical-source + symlink-install convention as `helios-ds-guardian`.
...
- **Figma → code handoff loses the import.** A designer or developer inspecting `Button` or `Alert` in the DS file sees Figma layer names, not `import { Button } from '@helios/ui/primitives/button'` and the real prop surface. Agents building screens from Figma re-derive imports from `.ai/ui-components.md` (5,600+ lines) every time; humans guess.
...
New `scripts/ds-tokens-export.mjs` (Node, zero new dependencies — matches the `scripts/*.mjs` house style). It parses **only** `apps/helios/src/app/globals.css` (single source of truth; the template copy is covered transitively by `template:sync`) and only the three token blocks:
...
|---|---|---|
| Button | `@helios/ui/primitives/button` | `variant`, `size` |
| Input | `@helios/ui/primitives/input` | type variants → dedicated components below |
...
| Select | `@helios/ui/primitives/select` (`Select`, `SelectTrigger`, `SelectValue`, `SelectContent`, `SelectItem`) | size, disabled |
...
1. **Rename to `helios-figma-design-with-ds`** — every tracked skill uses the `helios-` prefix; the unprefixed names in `.git/info/exclude` are exactly the local-only set. Frontmatter `name:` updated to match.
2. **Track it**: `git add .ai/skills/helios-figma-design-with-ds/` (tracking overrides the exclude; the maintainer additionally deletes the stale exclude line locally to avoid confusion).
...
```
globals.css (apps/helios)  ──template-sync (existing guard)──▶  globals.css (create-app template)
        │
...
{
  "source": "apps/helios/src/app/globals.css",
  "tokens": {
    "brand-lime": {
      "kind": "color",
...
- **No deprecations, no bridges.** Nothing existing is removed or renamed except the local-only skill directory (untracked → tracked under the `helios-` name; no history to preserve since it was never committed).

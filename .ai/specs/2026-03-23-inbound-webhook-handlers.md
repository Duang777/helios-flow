...
Helios's webhooks module (SPEC-057) covers **outbound** webhook delivery comprehensively. Phase 3 introduced a basic inbound receiver with `WebhookEndpointAdapter`, but the current design:
...
import type { WebhookSourceConfig } from '@helios/shared/modules/webhooks'
...
**Deliverables**:
1. `WebhookSourceConfig` and `WebhookHandlerMeta` types in `@helios/shared`

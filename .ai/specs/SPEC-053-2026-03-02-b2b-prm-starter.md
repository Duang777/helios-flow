...
| Example is a complete app in `helios/ready-apps` (per SPEC-068) | Keeps core clean; example modules live in app `src/modules/`; bootstrap via `create-helios-app --example b2b-prm` + `yarn initialize` |
...
### 4. Rollout
- An Engineer wants to scaffold a PRM app with `create-helios-app --example b2b-prm` and run `yarn initialize` to get a demo-ready system in minutes.
- A Maintainer wants to re-apply or upgrade starter baseline safely across tenants.

## Architecture
### Module Topology
```text
apps/helios/src/modules/
...
- Machine mode (`/import/external`): no session cookie; required headers:
  - `X-Helios-Import-Secret: <PARTNERSHIP_KPI_IMPORT_SECRET>`
  - `X-Helios-Request-Timestamp: <RFC3339 UTC>` (reject outside +/- 5 minutes)
...
The PRM example is distributed as a complete app in the `helios/ready-apps` repository. Bootstrap flow:

1. `npx create-helios-app my-prm --example b2b-prm` — fetches the B2B PRM example from `helios/ready-apps` via GitHub API tarball
...
To skip demo data: set `HELIOS_SEED_EXAMPLES=false` or equivalent starter config before running `yarn initialize`.
...
- Machine import routes enforce replay protection using `(Idempotency-Key, X-Helios-Request-Timestamp)` uniqueness windows.
...
- Starter seed profile config:
  - `HELIOS_PRM_SEED_PROFILE=demo_agency` (optional, controls which demo fixtures are seeded)
  - `HELIOS_SEED_EXAMPLES=false` (skip demo data during `yarn initialize`)
...
- Example E2E: `create-helios-app --example b2b-prm` -> `yarn initialize` -> verify demo-ready state.
...
## Changelog
### 2026-03-18
- Adopted `--example` pattern from `create-next-app`: bootstrap via `create-helios-app --example b2b-prm` (per SPEC-068).
...
### 2026-03-17
- Aligned bootstrap flow with Use-Case Starters Framework: replaced `helios init --starter` with `create-helios-app` + template repo + `yarn initialize` flow.

...
1. **Environment defaults** for deployment-wide bootstrap:
   - `NEXT_PUBLIC_HELIOS_DATE_FORMAT`
   - `NEXT_PUBLIC_HELIOS_DATE_TIME_FORMAT`
   - `NEXT_PUBLIC_HELIOS_TIME_FORMAT`
   - `HELIOS_DEFAULT_LOCALE`
   - `HELIOS_DEFAULT_TIMEZONE`
   - `HELIOS_FIRST_DAY_OF_WEEK`
...
- Add `NEXT_PUBLIC_HELIOS_DATE_FORMAT` and `NEXT_PUBLIC_HELIOS_DATE_TIME_FORMAT` public display-format aliases.
...
- **Client env leakage:** only public display preferences should use `NEXT_PUBLIC_` variables. Server-only tenant defaults stay behind `HELIOS_` variables and API/settings resolution.

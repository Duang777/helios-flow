...
This specification defines the full architecture of the **Ecommerce Storefront Module** for Helios: a headless, multi-tenant, multi-channel commerce layer comprising a core backend module and a standalone storefront starter application.
...
- Standalone Next.js app — **no dependency on `@helios/core`**
...
```
apps/storefront/
├── package.json                        # @helios/storefront
├── next.config.ts
├── tsconfig.json
├── tailwind.config.ts
├── src/
...
**MUST NOT** depend on `@helios/core`, `@helios/ui`, or `@helios/shared`. Pure API consumer.
...
Integrate `@helios/search` module. Define `ecommerce/search.ts`:
...
| `apps/helios/src/app/globals.css` | CSS variables (OKLCH color system, radius, animation) |
...
5. **Search backend**: Use ILIKE only for launch, or invest in `@helios/search` fulltext indexing from Phase 2?

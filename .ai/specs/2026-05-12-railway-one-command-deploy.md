# Railway One-Command Deployment from the Helios CLI
...
After running `create-helios-app` and `git init`, a user should be able to run a **single command** — `helios deploy railway` — and end up with a deployed, healthy Helios instance at a public Railway URL printed to stdout. No dashboard clicks, no copy-paste of template URLs, no manual Postgres/Redis provisioning.
...
3. Supports both source paths requested in issue #2414:
   ...
   The default `--source auto` mode prefers Git when available and falls back to local upload, preserving the original `create-helios-app` + `git init` one-command flow.
4. Ships a minimal, repo-owned Railway configuration inside the `create-helios-app` template (`railway.toml`, `railway.worker.toml`, `Dockerfile`, `scripts/railway-start.sh`, `scripts/railway-worker.sh`, and `src/app/api/healthz/route.ts`) so the build/run contract is committed to user code, not hidden in a Railway-side template we no longer control.
5. Persists Railway resource IDs (project, environments, services, database services, domains, source metadata) in `.helios/railway.json` inside the user's repo so re-runs are **idempotent** — the second invocation updates the existing project rather than creating a duplicate.
...
- Bootstrap-free `helios deploy railway` command with `auto|git|local` source modes, dry run, cleanup, resumable state, token handling, secret redaction, and protected variable computation.
...
### P1 — There is no one-command deploy from a fresh `create-helios-app` repo

Today a user who runs `npx create-helios-app my-shop && cd my-shop && git init` has no automated path from "I have an app on disk" to "I have a deployed app at a URL." Their options are:
...
There is no first-class Helios CLI flow that subsumes all of this.
...
A single new top-level command on the `helios` CLI (the existing `packages/cli` bin):

```
helios deploy railway [options]
```

Why `helios deploy railway` and not `create-helios-app --deploy railway`:

- `create-helios-app` is a one-shot scaffolder. Deployment is a recurring operation (deploy + redeploy + status). Coupling it to scaffolding would force users to re-scaffold to redeploy.
- The `helios` bin already runs inside scaffolded apps and has access to the app's `package.json`, `.env`, modules registry, and generated config. Deployment naturally belongs there.
- Future-proofing: `helios deploy <provider>` is the natural namespace for non-Railway providers (Fly, Render, Coolify, etc.) when those specs land.

Optional follow-up sugar (out of scope here): `create-helios-app --deploy railway` may pass through to `helios deploy railway` at the end of scaffolding once the deploy command is stable. Tracked, not built in this spec.

### Flags

| Flag | Type | Default | Purpose |
|------|------|---------|---------|
| `--project <name>` | string | derived from `package.json` `name` | Railway project name. If a `.helios/railway.json` exists, this flag is ignored unless `--force-rename` is also passed. |
...
| `--service <name>` | string | `helios-app` | Service name for the Next.js app. |
...
| `--cleanup` | boolean | false | Delete the Railway project recorded in `.helios/railway.json` (after explicit confirmation when interactive). |
...
| `--no-track` | boolean | false | Keep Railway state local-only by writing `.helios/railway.json.local` instead of committed `.helios/railway.json`. |
...
3. A file at `~/.config/helios/railway.json` with `{ "token": "<value>" }` (mode `0600`).
...
After a successful interactive prompt, the CLI offers to persist the token to `~/.config/helios/railway.json`. Default answer is **no**; persisting is opt-in. The file is created with mode `0600` and is **never** committed because it lives in the user's home directory, not the repo.
...
- `.helios/railway.json` (committed by default — see gitignore note below) records: `projectId`, `workspaceId`, per-environment IDs, app/worker service IDs, database service IDs, domain IDs, source metadata, last deployment IDs, and the CLI version that wrote them. Includes a `schemaVersion` so future CLI versions can migrate the file safely.
- `~/.config/helios/railway.json` (per-user, not committed) records only the auth token.

**Important — template `.gitignore` interaction.** `packages/create-app/template/gitignore` (and the legacy `apps/helios/.gitignore`) already contains a blanket `.helios/` rule that hides the entire directory from git. To make committing `.helios/railway.json` actually work, the scaffolded template MUST include an allowlist entry alongside the existing rule:
...
The implementation PR MUST update `packages/create-app/template/gitignore` accordingly. A bare `.helios/` ignore rule is not sufficient because Git will not re-include files inside an ignored directory. Without the allowlist pattern above, `.helios/railway.json` would be silently untracked and re-runs from a fresh clone would create a duplicate Railway project — exactly the failure mode this state file is meant to prevent.

Whether to commit `.helios/railway.json` at all is a real call. Recommended default: **commit it**. Reasons: it contains no secrets, only opaque Railway IDs; committing it lets the next teammate run `helios deploy railway` and update the *same* project rather than create a duplicate. Add an explicit `--no-track` CLI flag for users who prefer to keep it untracked (it tells the CLI to skip the allowlist line and write a `.helios/railway.json.local` instead). The spec implementer MUST add a section to the user docs explaining the tradeoff.

The state file stores one record per Railway environment because `helios deploy railway --env staging` should create a sibling environment, not overwrite production metadata. Shape:
...
1. Reads `.helios/railway.json` if present.
...
│   ├── service: helios-app          (Next.js app)
│   │   ├── domain: <random>.up.railway.app  (or --domain)
│   │   └── healthcheck: /api/healthz
│   └── service: helios-worker       (only when --worker)
└── (additional environments via --env staging, etc.)
```

Optional future services not built in this spec:

- `helios-cron` — a service running scheduled jobs via `node-cron` or Railway's native cron triggers.
...
| `HELIOS_AI_PROVIDER` | env-file | `openai` (matches repo default) | Honors `HELIOS_AI_PROVIDER` from the local env file. |
| `HELIOS_AI_MODEL` | env-file | `gpt-5-mini` (matches repo default) | Honors `HELIOS_AI_MODEL` from the local env file. |
...
| `AUTO_SPAWN_WORKERS` / `HELIOS_AUTO_SPAWN_WORKERS` | static | App: `false` when `--worker`, `true` when `--no-worker`; Worker: `false` | Avoid running duplicate queue workers inside the app service when a dedicated worker exists. |
...
- The Helios monorepo (and its scaffolded standalone apps) has non-trivial build steps: `yarn generate`, optional `yarn build:packages`, Next.js standalone output, and module discovery via the generated `.helios/` directory. Nixpacks' default Next.js detection misses the generate step.
...
The worker script runs the queue worker via the `helios` CLI. The current command is `yarn helios queue worker --all`, and `packages/queue/AGENTS.md` requires production workers to use `QUEUE_STRATEGY=async`. The app service should set worker auto-spawn off when a dedicated worker service exists.
...
- `scripts/railway-start.sh` (app service):
  ...
  2. `if [ ! -d ".helios/generated" ]; then yarn generate; fi` — re-run module generation if missing (e.g., first cold boot after a fresh image).
  3. Start the Next.js server. The repo's current `apps/helios/next.config.ts` does NOT set `output: 'standalone'` (verified at spec time), so the default start command is `exec yarn workspace @helios/app start` (which calls `next start`). The implementation PR SHOULD add `output: 'standalone'` to the scaffolded template's `next.config.ts` to shrink deploy images — when it does, the start line changes to `exec node apps/helios/.next/standalone/server.js`. Pick one and document the choice; do not assume standalone output exists.
...
The legacy doc at `apps/docs/docs/installation/railway.mdx` mentions a `/app/apps/helios/storage` volume for attachments. The implementation PR MUST decide whether to:
...
**No DB schema, no MikroORM entities, no migration files.** This feature is a CLI tool that talks to an external service; it does not persist any application-level data in the Helios database.

Two local-filesystem state files are introduced, documented in detail elsewhere in this spec:

- `.helios/railway.json` — per-repo Railway resource identifiers (`projectId`, `workspaceId`, environment records, service IDs, database service IDs, domain IDs, source metadata, last deployment IDs, `schemaVersion`, CLI version). See *Proposed Solution → Idempotency contract*. Committed by default via an allowlist line in `packages/create-app/template/gitignore`.
- `~/.config/helios/railway.json` — per-user Railway Account token (`{ "token": "..." }`). Mode `0600`. See *Security & secrets → Token storage*.
...
This spec exposes no new HTTP API routes inside the Helios app — the CLI is a pure outbound caller against Railway's public API.
...
The single command runs the following steps in order. State transitions are documented so an interrupted run can resume by re-reading `.helios/railway.json`.
...
5. Confirm a `package.json` exists at the cwd root and declares an Helios app (look for `@helios/*` dependencies).
...
**Persisted state:** none (token may be written to `~/.config/helios/railway.json` only after explicit consent).
...
If `.helios/railway.json` has a `projectId`, look it up (`query project($id)`). On 404 (project deleted by user out-of-band), prompt to recreate or exit.
...
**Persisted state:** `projectId` written to `.helios/railway.json` after success.
...
For `--worker`, repeat with a second service named `helios-worker`, same Dockerfile, different `startCommand`.
...
Log output is prefixed `[helios-app]` / `[helios-worker]` so a user with `--worker` can distinguish streams.
...
```
Helios deployed to Railway

  Project:     https://railway.com/project/<projectId>
  Environment: production
  App URL:     https://<appUrl>
  Health:      https://<appUrl>/api/healthz
  Worker:      helios-worker (running)   # only if --worker

  State recorded in .helios/railway.json
  Token cached in ~/.config/helios/railway.json   # only if user consented

Next steps:
  - Sign in at https://<appUrl>/backend
  - Set up DNS for your custom domain (if applicable)
  - Run `helios deploy railway --env staging` to deploy a second environment
```
...
- `~/.config/helios/railway.json` is created with mode `0600`. The CLI refuses to read it if the perms are wider (e.g., world-readable).
- The token is never written to repo files, never to `.helios/railway.json`, never to CLI logs even under `--verbose`.
- The token is redacted (`Bearer ****`) in any `--verbose` output.
- On Windows the equivalent path is `%APPDATA%\helios\railway.json`. NTFS permissions are not as tight as POSIX `0600`; document this gap and recommend users on Windows rotate their Account token if the machine is shared.

### Auto-generated secrets

`AUTH_SECRET`, `JWT_SECRET`, and `TENANT_DATA_ENCRYPTION_FALLBACK_KEY` are generated **on the user's machine** (using `crypto.randomBytes`) and uploaded to Railway. They are not echoed to stdout, not written to local files, and not persisted in `.helios/railway.json`. The only place they live is Railway's variable storage.
...
| `.helios/railway.json` in a public repo | Low — only opaque IDs, no secrets | Acceptable to commit by default. Document the `--no-track` opt-out. |
...
- **Local upload safety** — `--source local` preflight refuses archives that would include env files, private keys, `.git`, `node_modules`, local DB files, or `.helios/railway.json.local`.
- **Idempotency state machine** — given a synthetic `.helios/railway.json` at every possible partial state, assert the CLI takes the right next step.
...
1. Run `helios deploy railway --dry-run` on a freshly-scaffolded app. Confirm the planned operations match the spec.
2. Run `helios deploy railway` and verify a URL is printed.
...
4. Re-run `helios deploy railway`. Confirm no duplicate project is created.
5. Run `helios deploy railway --env staging` and confirm a second environment exists under the same project.
6. Run `helios deploy railway --cleanup` and confirm the project disappears from the Railway dashboard.
...
- **Task Router row** in root `AGENTS.md` mapping "Deploy a freshly-scaffolded Helios app to Railway with one CLI command" to this spec and the new doc page. (Added in *this* PR pointing at the spec only; updated in the implementation PR to also point at the doc.)
...
|----------------|-----------|-------|
| `helios deploy railway --dry-run` | `cli/deploy/railway/dry-run.spec.ts` | Snapshot of planned GraphQL ops. Mock Railway transport. |
| `helios deploy railway --source auto|git|local` | `cli/deploy/railway/source-strategy.spec.ts` | Unit tests for source-mode selection and failure messages. |
| `helios deploy railway` (happy path) | `cli/deploy/railway/full-deploy.integration.spec.ts` | Gated by `RAILWAY_INTEGRATION_TOKEN`. |
| `helios deploy railway` (resume after partial state) | `cli/deploy/railway/resume.spec.ts` | Synthetic `.helios/railway.json` fixtures. |
| `helios deploy railway --cleanup` | `cli/deploy/railway/cleanup.integration.spec.ts` | Gated. |
...
| Healthcheck endpoint | `apps/helios/__tests__/healthz.spec.ts` (or template equivalent) | Verifies DB/Redis ping logic and confirms the public response omits component-level infrastructure detail. |
...
| Local upload preflight | `cli/deploy/railway/local-upload-safety.spec.ts` | Unit tests that `.env*`, private keys, `.git`, `node_modules`, local DB files, and `.helios/railway.json.local` block `--source local` before Railway mutations. |
...
- New CLI command (`helios deploy railway`). No existing command is renamed or removed.
...
- New state file `.helios/railway.json`. Doesn't conflict with any existing path.
- New user-config file `~/.config/helios/railway.json`. Per-user, doesn't conflict.
...
- **Residual risk:** Railway may break the schema after we ship. We add a CLI-level error that surfaces the raw GraphQL error message and links to `https://docs.railway.com/reference/public-api` so users can self-diagnose without an Helios release.
...
### Risk: Token-leak via accidental commit of `~/.config/helios/railway.json`
...
- **Naming** — `helios deploy railway` aligns with future provider namespace `helios deploy <provider>`. ✓
...
- **Decision C — Track `.helios/railway.json` or not.** Resolved: commit-by-default, with `--no-track` writing `.helios/railway.json.local`.
- **Decision D — `railway.toml` vs. `railway.json`.** Resolved for v1: use `railway.toml`; Phase 0 accepted the required fields.
- **Decision E — Worker entry command.** Resolved for v1: use `yarn helios queue worker --all` with `QUEUE_STRATEGY=async`, unless implementation discovers a blocking runtime issue.

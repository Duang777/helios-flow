# Structured Logging Facade for `@helios/shared`

- **Status:** Implemented (2026-07-08, all 3 phases on `feat/structured-logging`)
- **Scope:** OSS (cross-cutting shared contract)
- **Issue:** [#3743](https://github.com/helios/helios/issues/3743)
- **Origin:** Follow-up to [#3738](https://github.com/helios/helios/issues/3738) (`HELIOS_WORKFLOW_TRIGGER_DEBUG` env flag added to make a silent wildcard subscriber observable)
- **Risk:** `risk-high` (new shared contract surface, eventually reaches broadly) · **Priority:** `priority-low`
- **Category:** `refactor`

## TLDR

Introduce a thin, swappable **logging facade** at `@helios/shared/lib/logger` exposing `createLogger(namespace)` → `{ debug, info, warn, error, child }`. It is backed by **pino** on the Node server (structured JSON) and a **console-backed transport** on the browser/edge (isomorphic-safe), selected at runtime so pino never leaks into client bundles. Global level is controlled by a single env var `HELIOS_LOG_LEVEL` (default `debug` in dev, `info` in prod). This issue delivers **the facade + unit tests + docs + one reference migration** (the `@helios/events` bus/bridge/worker) — **not** a big-bang rewrite of the ~2,300 existing `console.*` call sites. Everything is purely additive: no existing `console.*` call is removed by this spec.
...
- **No log levels.** `console.log` vs `console.warn` is the only granularity. There is no global "emit `debug` in dev, `info` in prod" switch. Teams work around this with per-feature env flags (`HELIOS_WORKFLOW_TRIGGER_DEBUG`, `HELIOS_EVENTS_SINGLE_DELIVERY` debug branches) — each reinvents level-gating locally.
...
**Goals**
- A stable, minimal facade export (`@helios/shared/lib/logger`) with `createLogger` + `child` + four levels.
- Isomorphic safety: no regression for existing browser/edge consumers of `@helios/shared`; pino stays server-only.
- Single global level knob (`HELIOS_LOG_LEVEL`).
- Unit tests, a docs page, and one real reference migration (`@helios/events`).
...
1. **Isomorphism.** pino is a Node library (uses `worker_threads`, `process.stdout`, `sonic-boom`). Importing it directly from `@helios/shared` — which is consumed by browser and edge bundles — would break those bundles. The facade lets us pick a console transport client-side and pino server-side behind one import.
...
@helios/shared/lib/logger/
  ...
  level.ts            # resolveLevel(): reads HELIOS_LOG_LEVEL + NODE_ENV, numeric ordering
...
Result: browser/edge consumers get a tiny console shim; Node server gets structured pino JSON. No consumer's existing import of `@helios/shared` regresses because the logger is a new, independently-resolved subpath.
...
|---|---|---|
| `HELIOS_LOG_LEVEL` set | `debug`\|`info`\|`warn`\|`error` (case-insensitive) | that level |
| `HELIOS_LOG_LEVEL` unset, `NODE_ENV=production` | — | `info` |
| `HELIOS_LOG_LEVEL` unset, otherwise (dev/test) | — | `debug` |
| `HELIOS_LOG_LEVEL` set to junk | — | fall back to the `NODE_ENV` default + one `warn` line |
...
## Reference Migration — `@helios/events`

`@helios/events` is the ideal reference target named by the issue ("the events/workflows subscribers that motivated it"): it is small (9 non-test `console.*` calls), already `dependsOn` `@helios/shared` (`packages/events/package.json`), and is exactly the subsystem where the motivating `HELIOS_WORKFLOW_TRIGGER_DEBUG` / `HELIOS_EVENTS_SINGLE_DELIVERY` debug flags live.
...
This demonstrates: namespace replacing the `[events]` prefix, structured fields replacing string interpolation, `child()` for context propagation, and `logger.debug(...)` gated by `HELIOS_LOG_LEVEL` replacing a bespoke debug env flag. It is behavior-preserving at `info` level (warn/error still print); it only *adds* queryable structure and dev-time `debug` visibility.
...
- Docs page `apps/docs/docs/framework/runtime/logging.mdx` (logging is a runtime cross-cutting concern, alongside `runtime/workers.mdx`, `runtime/request-lifecycle.mdx`) — usage, `child()`, `HELIOS_LOG_LEVEL`, isomorphic notes, the facade rationale, and migration guidance (Boy Scout rule, how to collapse a per-feature debug flag).
- Add advisory `console.*` checker script + `yarn logger:check-console`; document it as informational.
- Root `AGENTS.md` Task Router row: "Structured logging / replacing `console.*` → `@helios/shared/lib/logger`" pointing at the docs page and this spec. Add a short `packages/shared/AGENTS.md` import cheat-sheet entry.
...
- **Unit (`packages/events`)** — migrated bus/bridge/worker still emit on error paths; a `debug` line appears only when `HELIOS_LOG_LEVEL=debug`.
...
- **New shared export** `@helios/shared/lib/logger` — ADDITIVE to the import-path contract. Once shipped, the four-method `Logger` interface and `createLogger` signature are **STABLE**; changes follow the deprecation protocol (re-export/alias bridge ≥1 minor, `@deprecated` JSDoc, RELEASE_NOTES entry).
...
- **`HELIOS_LOG_LEVEL`** is a new, optional env var with safe defaults; unset behavior matches today's implicit "print everything in dev".
- **Browser/edge consumers** of `@helios/shared` are unaffected: the logger is an independently-resolved subpath and never statically imports pino.
...
| R3 | Level misconfiguration silences prod errors, or floods logs | Medium | Ops | `error`/`warn` always above default `info`; junk `HELIOS_LOG_LEVEL` falls back to NODE_ENV default + one warn; documented matrix | Low |
...
2. **Env var name — `HELIOS_LOG_LEVEL` ✅.** Confirmed.
...
| Isomorphic safety of `@helios/shared` | ✅ | Runtime transport selection; no eager pino; empty barrel + subpath export |
...
- `@helios/shared/lib/logger` with `createLogger` + `child` + levels, unit-tested → Phase 1.
- Isomorphic server/client transports, no browser-consumer regression → Phase 1 (transport selection + isomorphism test).
- `HELIOS_LOG_LEVEL` controls global level, documented → Phase 1 (`level.ts`) + Phase 3 (docs).
...
- 2026-07-02 — Initial draft (skeleton → full spec) for issue #3743. Library choice (pino), env var name (`HELIOS_LOG_LEVEL`), and advisory-lint timing captured as open decisions with recommendations.
- 2026-07-02 — Decisions resolved: **pino** behind the facade, **`HELIOS_LOG_LEVEL`** as the level knob, **advisory lint added now (non-blocking)**. Status → Ready for implementation.

...
The PR loop already answers "does it work?" — `helios-auto-verify-pr-ui` drives the changed surface with Playwright and posts screenshot evidence; module `__integration__` specs lock behavior in. Nothing in the loop answers **"can a user figure it out?"**:
...
2. **Existing automation is omniscient.** `helios-auto-verify-pr-ui` reads the diff, knows exactly which route to open and which button the author added, and drives it with discovered selectors. It verifies the *author's* mental model, so it is structurally incapable of noticing that no one else would ever find that button.
...
```
.ai/skills/helios-ux-walkthrough/
└── SKILL.md            # arguments, workflow, rules — same shape as helios-auto-verify-pr-ui
...
.ai/tmp/helios-ux-walkthrough/pr-{n}/run-{runId}/    # artifacts (gitignored tmp, mirrors auto-verify-pr-ui)
...
The skill **reuses verbatim** from `helios-auto-verify-pr-ui`: step-0 PR claim/lock with trap-guaranteed release, isolated worktree from the PR head, ephemeral env boot with `.ai/qa/ephemeral-env.json` reuse and started-by-this-run teardown discipline, default credentials from `helios init`, the `qa-evidence/pr-{n}` branch + raw-URL mechanism for inline screenshots (with the same private-repo/no-push fallback), and the "never fabricate results; if the env cannot boot, post the blocker and stop" rule. This spec defines only what is new: personas, the firewall, the navigator loop, metrics, and the report.
...
One PR comment, upserted by marker `<!-- helios-ux-walkthrough:report -->` (search existing comments for the marker; edit in place, never stack — the same sticky pattern as other house automation comments):

```markdown
<!-- helios-ux-walkthrough:report -->
...
- **Label:** a human adds `needs-ux-walkthrough` to a PR; the operator's periodic automation pass (the same loop that dispatches other `helios-auto-*` skills) picks it up and invokes the skill, then removes the label on completion. The label is a *request*, never a gate: no branch protection, no check run, no pipeline label is ever derived from it.
- **Manual:** `/helios-ux-walkthrough <prNumber|url> --persona <id> [options]`.
...
| `--keep-env` | no | Leave an env this run started running on exit (same semantics as `helios-auto-verify-pr-ui`). |
...
2. **Skill core** — `SKILL.md` with claim/worktree/env steps referenced from `helios-auto-verify-pr-ui`, the phase-1/phase-3 firewall structure, the navigator loop, artifacts layout, and the report template. Single-run mode first.
...
- **Environment integration.** Run with a pre-existing ephemeral env (`.ai/qa/ephemeral-env.json` present): assert reuse and no teardown; run without: assert boot and teardown. Artifacts land under `.ai/tmp/helios-ux-walkthrough/…`, never under any module's `__integration__/` (per `.ai/qa/AGENTS.md`, no executable specs are added anywhere by this skill).
...
| 7 | **Report spam / comment collisions** — repeated invocations stack comments or race another automation actor on the same PR. | Low | PR readability | Sticky marker-matched upsert (validated); step-0 claim lock shared with the other `helios-auto-*` skills serializes actors. | Low |
| 8 | **Screenshot data leakage** — walkthrough screenshots capture values that shouldn't be public on the evidence branch. | Low | Repo hygiene | Ephemeral env contains only `helios init` demo data; the inherited redact-or-omit rule from `helios-auto-verify-pr-ui` applies to every posted image. | Low |
...
- **QA conventions (`.ai/qa/AGENTS.md`):** no executable `.spec.ts` under `.ai/qa/tests`; no reliance on seeded data beyond `helios init` defaults; ephemeral env reused via `ephemeral-env.json`; artifacts in `.ai/tmp/`, mirroring `helios-auto-verify-pr-ui`.

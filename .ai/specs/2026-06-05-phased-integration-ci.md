...
**Issue:** [#2588](https://github.com/helios/helios/issues/2588)
...
|---|---|---|
| `HELIOS_INTEGRATION_PHASE` | `baseline`, `extended`, `full`, `standalone-sentinel`, `standalone-full` | Primary suite breadth selector |
| `HELIOS_INTEGRATION_GROUPS` | comma-separated group ids | Optional inclusion list for future targeted runs |
| `HELIOS_INTEGRATION_MODULES` | existing comma-separated module ids | Existing affected-module selector; unchanged |
| `HELIOS_INTEGRATION_FORCE_FULL` | `1` / unset | Escape hatch; CI sets it for release PRs and fail-closed paths |

Backward compatibility:

- If `HELIOS_INTEGRATION_PHASE` is unset, behave exactly as today.
...
3. Apply `HELIOS_INTEGRATION_MODULES` affected filtering as today, unless phase is `full`.
...
- [ ] Keep CI unset for `HELIOS_INTEGRATION_PHASE`, so behavior remains current.

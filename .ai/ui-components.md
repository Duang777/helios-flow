...
Detailed variant tables, size matrices, props, examples, and MUST rules for every `@helios/ui` primitive. Quick reference (one-line per component) is in [`packages/ui/AGENTS.md`](../packages/ui/AGENTS.md). Foundation rules (color tokens, typography, spacing, decision trees) live in [`.ai/ds-rules.md`](./ds-rules.md).
...
```typescript
import { LinkButton } from '@helios/ui/primitives/link-button'
```
...
```typescript
import { SocialButton, type SocialBrand } from '@helios/ui/primitives/social-button'
```
...
```typescript
import { FancyButton, type FancyButtonType } from '@helios/ui/primitives/fancy-button'
```
...
```tsx
<FancyButton intent="primary">Try Helios AI</FancyButton>
...
import { CheckboxField } from '@helios/ui/primitives/checkbox-field'
```
...
- One source of truth — `apps/helios/src/components/ui/checkbox.tsx` and `packages/create-app/template/src/components/ui/checkbox.tsx` re-export from `@helios/ui/primitives/checkbox`. Do NOT fork.
...
```tsx
import { FormField } from '@helios/ui/primitives/form-field'
import { Input } from '@helios/ui/primitives/input'
...
```typescript
import { EmailInput, type EmailInputProps } from '@helios/ui/primitives/email-input'
```
...
```typescript
import { SearchInput, type SearchInputProps } from '@helios/ui/primitives/search-input'
```
...
```typescript
import { PasswordInput, type PasswordInputProps } from '@helios/ui/primitives/password-input'
```
...
```typescript
import { WebsiteInput, type WebsiteInputProps } from '@helios/ui/primitives/website-input'
```
...
import {
  ...
  type AmountValue,
  type AmountCurrency,
} from '@helios/ui/primitives/amount-input'
```
...
```typescript
import { ButtonInput, type ButtonInputProps } from '@helios/ui/primitives/button-input'
```
...
import {
  ...
  type CardInputProps,
  type CardBrand,
} from '@helios/ui/primitives/card-input'
```
...
import {
  ...
  SelectItem,
  SelectSeparator,
} from '@helios/ui/primitives/select'
```
...
```tsx
import { FormField } from '@helios/ui/primitives/form-field'
import { Select, SelectTrigger, SelectValue, SelectContent, SelectItem } from '@helios/ui/primitives/select'
...
<Select value={brand} onValueChange={setBrand}>
  ...
  <SelectContent>
    <SelectItem value="om">Helios</SelectItem>
...
- For LARGE option lists with search, do NOT cram into `Select` — use [`ComboboxInput`](#comboboxinput) (single value) or [`LookupSelect`](#lookupselect) (rich card list) from `@helios/ui/backend/inputs/*` instead.
...
import { SwitchField } from '@helios/ui/primitives/switch-field'
```
...
```typescript
import { Radio, RadioGroup } from '@helios/ui/primitives/radio'
import { RadioField } from '@helios/ui/primitives/radio-field'
```
...
import {
  ...
  TooltipProvider,
  SimpleTooltip,
} from '@helios/ui/primitives/tooltip'
```
...
```typescript
import { Avatar, AvatarStack } from '@helios/ui/primitives/avatar'
```
...
```typescript
import { Kbd, KbdShortcut } from '@helios/ui/primitives/kbd'
```
...
```typescript
import { Tag, type TagMap } from '@helios/ui/primitives/tag'
```
...
```typescript
import { TagInput } from '@helios/ui/primitives/tag-input'
```

Two-row primitive (input on top, chips below) for collecting a flat list of free-form tags. Built on `Input` + `Tag shape="square"`. Use when the user types comma/separator-delimited values and the result is `string[]`. For value-from-suggestions (autocomplete with descriptions, async loaders), use [`TagsInput`](#tagsinput-backend) from `@helios/ui/backend/inputs/TagsInput` instead.
...
```typescript
import { CounterInput } from '@helios/ui/primitives/counter-input'
```
...
```typescript
import { DigitInput } from '@helios/ui/primitives/digit-input'
```
...
import {
  ...
  SelectSeparator,
  SelectValue,
} from '@helios/ui/primitives/compact-select'
```
...
```typescript
import { InlineInput } from '@helios/ui/primitives/inline-input'
```
...
- For high-level "click-to-edit with save / cancel buttons + validation" UX, use the `InlineTextEditor` from `@helios/ui/backend/detail/InlineEditors` instead. `InlineInput` is the **low-level** atom — consumers wire the save / cancel state machine themselves.
...
import {
  ...
  SelectSeparator,
  SelectValue,
} from '@helios/ui/primitives/inline-select'
```
...
- For high-level "click-to-edit with save / cancel + draft state" UX, use `InlineSelectEditor` from `@helios/ui/backend/detail/InlineEditors`. `InlineSelectTrigger` is the **low-level atom** — consumers wire their own state machine (the trigger is always live, no display-vs-edit boundary).
...
```typescript
import { DatePicker, type DatePickerProps, type DatePickerFooter } from '@helios/ui/primitives/date-picker'
```
...
- Do NOT import from `@helios/ui/backend/inputs/DatePicker` or `…/DateTimePicker` in new code — those are `@deprecated` shims that re-export this primitive (`<DateTimePicker>` is a thin wrapper that always sets `withTime`). New consumers import from `@helios/ui/primitives/date-picker` directly. See [Backend shims](#backend-shims-datepicker--datetimepicker--timepicker) below for the full migration table.
...
- Importing from `@helios/ui/backend/inputs/DatePicker` or `DateTimePicker` in new code → use `@helios/ui/primitives/date-picker` directly (shims are `@deprecated`).
...
} from '@helios/ui/primitives/date-range-picker'
import { defaultDateRangePresets } from '@helios/ui/primitives/date-picker-helpers'
import type { DateRange } from '@helios/ui/backend/date-range'
```
...
The range type comes from `@helios/ui/backend/date-range` (single source of truth — same shape feeds dashboards, analytics, and CSV exports):
...
- Range type is `{ start: Date; end: Date }` (from `@helios/ui/backend/date-range`), NOT `{ from, to }` — convert at the react-day-picker boundary if you ever need to interop directly.
...
- For dashboard-style "13 preset" UIs that share state with chart filters, pull the option list from `DATE_RANGE_OPTIONS` in `@helios/ui/backend/date-range` and map it to `DateRangePresetItem[]`. Do not duplicate preset logic in the consumer.
...
- Building your own preset list duplicating `DATE_RANGE_OPTIONS` → import `defaultDateRangePresets()` or pull `DATE_RANGE_OPTIONS` from `@helios/ui/backend/date-range` and map to `DateRangePresetItem[]`.
...
} from '@helios/ui/primitives/time-picker'
```
...
```typescript
import { EmptyState, type EmptyStateProps, emptyStateVariants } from '@helios/ui/primitives/empty-state'
```
...
|---|---|---|
| List has **no records yet** (no search/filter) | `ListEmptyState` | `@helios/ui/backend/filters/ListEmptyState` |
| List has **no results after a search** | `SearchEmptyResults` (auto-rendered by `DataTable`) | `@helios/ui/backend/filters/SearchEmptyResults` |
| List has **no results after filters** | `FilteredEmptyResults` (auto-rendered by `DataTable` via `filterAwareEmptyState`) | `@helios/ui/backend/filters/FilteredEmptyResults` |
| **Record not found** on a detail/edit page | `RecordNotFoundState` | `@helios/ui/backend/detail` |
| **No access** (forbidden) | `AccessDeniedMessage` | `@helios/ui/backend/detail` |
| A **section/tab** is empty but healthy | `TabEmptyState` | `@helios/ui/backend/detail` |
| Low-level building block for any of the above | `EmptyState` | `@helios/ui/primitives/empty-state` |
...
```typescript
import { RecordNotFoundState, type RecordNotFoundStateProps } from '@helios/ui/backend/detail'
```
...
```typescript
import { Skeleton, type SkeletonProps, type SkeletonShape } from '@helios/ui/primitives/skeleton'
```
...
- For ephemeral "save on action" feedback, prefer the global `flash()` helper from `@helios/ui/backend/FlashMessages` (it wraps `Alert` internally) over building your own toast queue.
...
} from '@helios/ui/primitives/notification'
// (NotificationProvider / NotificationStack / useNotification live in `notification-stack`)
import {
  NotificationProvider,
  NotificationStack,
  useNotification,
} from '@helios/ui/primitives/notification-stack'
```
...
| Persistent in-app notification panel (bell icon + popover) | `NotificationItem` / `NotificationPanel` from `@helios/ui/backend/notifications` |
...
```tsx
import { Avatar } from '@helios/ui/primitives/avatar'
import { LinkButton } from '@helios/ui/primitives/link-button'
...
} from '@helios/ui/primitives/accordion'
```
...
import {
  ...
  type LogListEntry,
  type LogListLevel,
} from '@helios/ui/backend/LogList'
```
...
} from '@helios/ui/primitives/rich-editor'
```
...
import {
  ...
  ScrollAreaThumb,
  ScrollAreaCorner,
} from '@helios/ui/primitives/scroll-area'
```
...
- No dedicated Figma node — DS Helios library did not ship a `ScrollArea` master component at the time this primitive was authored. Styling is inferred from DS scrollbar token decisions used elsewhere.
...
```typescript
import { ButtonGroup, buttonGroupVariants } from '@helios/ui/primitives/button-group'
```
...
- Figma node: DS Helios `Button Group [1.1]` (`componentKey: 3447dc22e79d714aded761678bcff3d8bd6221f0`). 3 sizes × 5 quantities × `Default | Hover | Active | Disabled` state per item. Item-level states (hover / active / disabled) come from the child `Button` itself — the wrapper does not own them.

---

## SegmentedControl

iOS-style segmented selector for **mutually-exclusive view state** per Figma `Switch / Chart / Cryptocurrency` (DS Helios `componentKey: 4fdcde6e834a674e7db86e3aa60d6b781377abb1`). Pill-shaped track with N items; exactly one selected at a time. Selecting a different item fires `onValueChange`.
...
```typescript
import { Slider } from '@helios/ui/primitives/slider'
```
...
- Anchored on Figma `Slider [1.1]` (DS Helios componentSet id `2617:1169`). 5-variant component set parameterized by `Percentage` (0% / 25% / 50% / 75% / 100%) and Boolean props for Label / Sublabel / Tooltip. The standalone "Level Slider" entry elsewhere in the file is an emoji icon, not this primitive.
...
1-to-N star / heart / dot rating widget. Two modes — read-only display (no `onChange`) and interactive input (`onChange` present). Anchored on Figma `Rating & Review [1.0]` (DS Helios componentSet `199969:1797`, key `544eab9fbc72c0038c0a28b7ff27a93ab8c3c01a`).

```typescript
import { Rating } from '@helios/ui/primitives/rating'
```
...
```typescript
import { StepIndicator, type StepIndicatorStep } from '@helios/ui/primitives/step-indicator'
```
...
- Anchored on Figma DS Helios component sets:
...
Swatch + hex color selection. Click trigger → popover with grid of recommended colors + optional hex input. Anchored on Figma `Color Picker [1.1]` (DS Helios componentKey `037353153a0ac1898322da4c20ceb88d2cb3d78a`).
...
import {
  ...
  normalizeHex,
} from '@helios/ui/primitives/color-picker'
```
...
Page navigation primitive per Figma `Pagination Group [1.1]` (DS Helios componentSet `199985:4135`). Layout: `[Page X of Y]  [⏮ ◀ pages ▶ ⏭]  [N / page]`.

```typescript
import { Pagination, buildPaginationItems } from '@helios/ui/primitives/pagination'
```
...
```tsx
import { buildPaginationItems } from '@helios/ui/primitives/pagination'
...
import {
  ...
  DrawerFooter,
  DrawerClose,
} from '@helios/ui/primitives/drawer'
```
...
- Figma source: DS Helios `Drawer` page (`486:7366`) — `Drawer Header [1.1]` (`3187:2897`) and `Drawer Footer [1.1]` (`4096:21416`) plus assembled examples (`167124:24738`, `167124:24794`, `167124:24859`, ...).
...
import {
  ...
  CommandMenuFooter,
} from '@helios/ui/primitives/command-menu'
```
...
- Figma source: DS Helios `Command Menu` page (`4152:24764`) — Search Input [1.1] (`4187:559`), Items [1.1] (`4171:15653`), Footer [1.1] (`4172:16590`).
...
} from '@helios/ui/primitives/activity-feed'
```
...
- Figma source: DS Helios `Activity Feed` page (`164611:26451`) — `Activity Feed [1.1]` (`166035:46833`, 5 entry variants), `Activity Feed File Items [1.1]` (`165967:4028`), `Activity Feed Comment Items [1.1]` (`166017:612`), `Activity Feed Task Status Items [1.1]` (`166035:47290`). Assembled example: `166707:8700` (audit-trail style).
...
} from '@helios/ui/primitives/notification-feed'
```
...
- Figma source: DS Helios `Notifications` page (`4096:21398`) — `Notifications Items [1.1]` (`4308:731`, 4 designs × 2 states), `Notifications Header [1.1]` (`4308:1004`), `Notifications Footer [1.1]` (`4308:5526`), `Notifications Tab Menu [1.1]` (`4349:46656`). Assembled examples: `166926:7047` (bell dropdown), `166926:7088`, `166926:7114`, `166926:7138`.
...
```typescript
import { Progress, CircularProgress } from '@helios/ui/primitives/progress'
```
...
- Figma source: DS Helios `Progress Bar` page (`450:17758`) — `Progress Bar [1.1]` (`450:17821`), `Progress Bar Label [1.1]` (`515:3758`), `Progress Bar Line [1.1]` (`450:17810`, 5 tone variants), `Circular Progress Bar [1.1]` (`466:4652`).
...
```typescript
import { ComboboxInput, type ComboboxInputProps, type ComboboxOption } from '@helios/ui/backend/inputs/ComboboxInput'
```
...
```typescript
import { TagsInput, type TagsInputProps, type TagsInputOption } from '@helios/ui/backend/inputs/TagsInput'
```
...
|---|---|---|
| Import | `@helios/ui/backend/inputs/TagsInput` | `@helios/ui/primitives/tag-input` |
...
```typescript
import { LookupSelect, type LookupSelectItem } from '@helios/ui/backend/inputs/LookupSelect'
```
...
```typescript
import { EventSelect, useAvailableEvents, type EventDefinition, type EventSelectProps } from '@helios/ui/backend/inputs/EventSelect'
```
...
```typescript
import { EventPatternInput, type EventPatternInputProps } from '@helios/ui/backend/inputs/EventPatternInput'
```
...
```typescript
import { PhoneNumberField, type PhoneNumberFieldProps, type PhoneDuplicateMatch } from '@helios/ui/backend/inputs/PhoneNumberField'
```
...
Validates and normalizes on blur via `validatePhoneNumber` from `@helios/shared/lib/phone`, with optional duplicate lookup (debounced 350 ms) that surfaces an existing contact link inline. Used in the customer-create flow, contact forms, and any onboarding step that captures a phone number.
...
```typescript
import { SwitchableMarkdownInput, type SwitchableMarkdownInputProps } from '@helios/ui/backend/inputs/SwitchableMarkdownInput'
```
...
```diff
- import { SwitchableMarkdownInput } from '@helios/ui/backend/inputs/SwitchableMarkdownInput'
- <SwitchableMarkdownInput value={md} onChange={setMd} isMarkdownEnabled={enabled} />
+ import { RichEditor } from '@helios/ui/primitives/rich-editor'
...
```typescript
import { TimeInput, type TimeInputProps } from '@helios/ui/backend/inputs/TimeInput'
```
...
|---|---|---|
| `@helios/ui/backend/inputs/DatePicker` | `@helios/ui/primitives/date-picker` (`DatePicker`) | Direct re-export. Default footer is now `'apply-cancel'` (Figma-aligned, applied globally 2026-05-09). Pass `footer="today-clear"` to opt back into the legacy Today/Clear footer. |
| `@helios/ui/backend/inputs/DateTimePicker` | `@helios/ui/primitives/date-picker` (`DatePicker` with `withTime`) | The shim is a thin wrapper: `<DateTimePicker {...props} />` ≡ `<DatePicker {...props} withTime />`. Props type is `Omit<DatePickerProps, 'withTime'>`. |
| `@helios/ui/backend/inputs/TimePicker` | `@helios/ui/primitives/time-picker` (`TimePicker`) | The shim wraps the primitive with a Figma-styled Clock-icon trigger, maps `minuteStep` → `intervalMinutes`, and adapts the legacy `showNowButton` / `showClearButton` flags onto `pinnedTopActions` / `legacyFooterActions`. `showClearButton` default flipped to `false` on 2026-05-11 — the primitive's Cancel already covers most "dismiss" intents. |

### Migration

```diff
- import { DatePicker } from '@helios/ui/backend/inputs/DatePicker'
+ import { DatePicker } from '@helios/ui/primitives/date-picker'

- import { DateTimePicker } from '@helios/ui/backend/inputs/DateTimePicker'
- <DateTimePicker value={value} onChange={setValue} />
+ import { DatePicker } from '@helios/ui/primitives/date-picker'
+ <DatePicker value={value} onChange={setValue} withTime />

- import { TimePicker } from '@helios/ui/backend/inputs/TimePicker'
+ import { TimePicker } from '@helios/ui/primitives/time-picker'
```
...
- `import { DatePicker } from '@helios/ui/backend/inputs/DatePicker'` in any new file → use the primitive path.
...
|---|---|---|---|
| `Card` (+ `CardHeader` / `CardTitle` / `CardDescription` / `CardAction` / `CardContent` / `CardFooter`) | `@helios/ui/primitives/card` | Generic surface container: `bg-card rounded-xl border shadow-sm`, `gap-6`, `px-6` sections | Use for standalone content cards; do NOT hand-roll `<div className="rounded-xl border bg-card">` |
| `Popover` (+ `PopoverTrigger` / `PopoverContent` / `PopoverAnchor` / `PopoverClose`) | `@helios/ui/primitives/popover` | Radix popover portal at `z-popover`, `bg-popover` surface, `min-w-[280px]` | Base for custom floating panels; prefer higher-level `Select`/`Tooltip`/`CompactSelect` when they fit |
| `Label` | `@helios/ui/primitives/label` | Radix label: `text-sm font-medium`, disabled propagation via `peer-disabled`/`group-data-[disabled]` | Every standalone input needs one (or use `FormField`, which renders it) |
| `DataLoader` | `@helios/ui/primitives/DataLoader` | `isLoading`-gated wrapper rendering a centered `Spinner` before children | For simple section-level loading; full pages prefer `LoadingMessage` |
| `Calendar` | `@helios/ui/primitives/calendar` | Internal engine for `DatePicker`/`DateRangePicker` (incl. month/year grid navigation) | INTERNAL — consume via `DatePicker`/`DateRangePicker`, do not embed directly |

...
|---|---|---|
| List records with sorting/paging/filters | `DataTable` — see `packages/ui/AGENTS.md` | `@helios/ui/backend/DataTable` |
| Build a create/edit form | `CrudForm` — see `packages/ui/AGENTS.md` | `@helios/ui/backend/crud` |
| Show a KPI number with trend | `KpiCard` | `@helios/ui/backend/charts` |
| Render bar / line / pie chart | `BarChart` / `LineChart` / `PieChart` | `@helios/ui/backend/charts` |
| Tiny inline trend line | `Sparkline` | `@helios/ui/backend/charts` |
| Compact "top N" ranking table | `TopNTable` | `@helios/ui/backend/charts` |
| Search box + filter overlay row | `FilterBar` | `@helios/ui/backend/FilterBar` |
| Standalone filter sheet | `FilterOverlay` | `@helios/ui/backend/FilterOverlay` |
| Composable AND/OR filter tree UI | `AdvancedFilterPanel` (+ `useAdvancedFilterTree`) | `@helios/ui/backend/filters/AdvancedFilterPanel` |
| Applied-filter chips row | `ActiveFilterChips` | `@helios/ui/backend/filters/ActiveFilterChips` |
| One-click filter presets | `QuickFilters` | `@helios/ui/backend/filters/QuickFilters` |
| Empty list (no records at all) | `ListEmptyState` | `@helios/ui/backend/filters/ListEmptyState` |
| Empty results due to filters / search | `FilteredEmptyResults` / `SearchEmptyResults` | `@helios/ui/backend/filters/FilteredEmptyResults` (DataTable renders these automatically) |
| Loading / error / not-found states | `LoadingMessage` / `ErrorMessage` / `RecordNotFoundState` | `@helios/ui/backend/detail` |
| Notes / activities / addresses / tags / attachments tab | `NotesSection` / `ActivitiesSection` / `AddressesSection` / `TagsSection` / `AttachmentsSection` | `@helios/ui/backend/detail` |
| Custom-field values panel | `CustomDataSection` | `@helios/ui/backend/detail` |
| Inline-editable field list on a detail page | `DetailFieldsSection` (or `InlineTextEditor` & friends) | `@helios/ui/backend/detail` |
| Empty tab inside a detail page | `TabEmptyState` | `@helios/ui/backend/detail` |
| Page wrapper + title row | `Page` / `PageHeader` / `PageBody` | `@helios/ui/backend/Page` |
| Section heading with count + action | `SectionHeader` / `CollapsibleSection` | `@helios/ui/backend/SectionHeader` |
| Page with left section nav (profile-style) | `SectionPage` / `SectionNav` | `@helios/ui/backend/section-page` |
| Settings area page | `SettingsPageWrapper` / `SettingsNavigation` | `@helios/ui/backend/settings` |
| Dashboard widgets | `registerDashboardWidgets` / `useWidgetData` (screen is framework-mounted) | `@helios/ui/backend/dashboard` |
| Toast after CRUD success/failure | `flash()` | `@helios/ui/backend/FlashMessages` |
| Confirmation before destructive action | `ConfirmDialog` / `useConfirmDialog` | `@helios/ui/backend/confirm-dialog` |
| Surface a 409 edit conflict | `surfaceRecordConflict` | `@helios/ui/backend/conflicts` |
| Undo bar after bulk/destructive op | `pushOperation` (banner is AppShell-mounted) | `@helios/ui/backend/operations/store` |
| Guided "next step" callout | `NextStepCallout` | `@helios/ui/backend/NextStepCallout` |
| Collapsible inline help box | `ContextHelp` | `@helios/ui/backend/ContextHelp` |
| Generic empty state | `EmptyState` | `@helios/ui/backend/EmptyState` |
| Notification inbox UI | `NotificationPanel` / `NotificationItem` / hooks | `@helios/ui/backend/notifications` |
| Calendar / availability view | `ScheduleView` | `@helios/ui/backend/schedule` |
| Compose / send a message about a record | `MessageComposer` / `SendObjectMessageDialog` | `@helios/ui/backend/messages` |
| Email thread timeline on a detail page | `EmailThreadsPanel` | `@helios/ui/backend/messages` |
| Form/detail page header & footer | `FormHeader` / `FormFooter` / `FormActionButtons` / `ActionsDropdown` | `@helios/ui/backend/forms` |
| Per-row "…" menu in tables | `RowActions` | `@helios/ui/backend/RowActions` |
| Truncate long cell text with tooltip | `TruncatedCell` (or `meta.truncate` on the column) | `@helios/ui/backend/TruncatedCell` |
| Boolean check/x icon, enum badge | `BooleanIcon` / `EnumBadge` | `@helios/ui/backend/ValueIcons` |
...
```typescript
import { KpiCard, Sparkline, BarChart, LineChart, PieChart, TopNTable, CHART_COLORS, getChartColor } from '@helios/ui/backend/charts'
```
...
```tsx
import { KpiCard, BarChart } from '@helios/ui/backend/charts'
...
```typescript
import { FilterBar } from '@helios/ui/backend/FilterBar'
import { FilterOverlay, type FilterDef, type FilterValues } from '@helios/ui/backend/FilterOverlay'
import { AdvancedFilterPanel } from '@helios/ui/backend/filters/AdvancedFilterPanel'
import { ActiveFilterChips } from '@helios/ui/backend/filters/ActiveFilterChips'
import { QuickFilters, type FilterPreset } from '@helios/ui/backend/filters/QuickFilters'
import { ListEmptyState } from '@helios/ui/backend/filters/ListEmptyState'
import { useAdvancedFilterTree } from '@helios/ui/backend/hooks/useAdvancedFilter'
```
...
|---|---|---|
| `Page`, `PageHeader`, `PageBody` | `@helios/ui/backend/Page` | Standard page wrapper (`space-y-6`), title row (`title`, `description?`, `actions?`), and content wrapper |
| `SectionHeader` | `@helios/ui/backend/SectionHeader` | Section heading: `title`, `count?` (muted badge), `action?` |
| `CollapsibleSection` | `@helios/ui/backend/SectionHeader` | Same header with chevron toggle; `defaultCollapsed?`, controlled `collapsed`/`onCollapsedChange`, `children` |
| `SectionPage`, `SectionNav` | `@helios/ui/backend/section-page` | Page with grouped left navigation (`sections: SectionNavGroup[]`, `activePath`, `userFeatures?: Set<string>`) — items support `labelKey`, `requireFeatures`, nesting |
| `SettingsPageWrapper`, `SettingsNavigation` | `@helios/ui/backend/settings` | Settings-area page shell: `sections: SectionNavGroup[]`, `requiredFeatures: string[]`; `SettingsNavigation` renders the settings card grid |
| `registerDashboardWidgets`, `useWidgetData`, `WidgetDataBatchProvider` | `@helios/ui/backend/dashboard` (+ `…/dashboard/widgetRegistry`, `…/dashboard/widgetData`) | Dashboard widget registry + batched data fetching. `DashboardScreen` itself is mounted by the framework — modules ship widgets, never the screen |

### Example

```tsx
import { Page, PageBody, PageHeader } from '@helios/ui/backend/Page'
import { useT } from '@helios/shared/lib/i18n/context'
...
| Export | Import | Purpose |
|---|---|---|
| `flash(message, type)` | `@helios/ui/backend/FlashMessages` | Toast after CRUD success/failure (`'success' \| 'error' \| 'warning' \| 'info'`) |
| `withFlash(url, message, type)`, `pushWithFlash(router, url, message)` | `@helios/ui/backend/utils/flash` | Flash that survives a navigation |
| `ConfirmDialog`, `useConfirmDialog` | `@helios/ui/backend/confirm-dialog` | Confirmation dialog: `onConfirm`, `title?`, `text?`, `variant?: 'default' \| 'destructive'`, `loading?`, optional `trigger` (declarative mode); the hook returns `{ confirm(options), dialogProps }` |
| `EmptyState` | `@helios/ui/backend/EmptyState` | Re-export of `@helios/ui/primitives/empty-state` for backend imports |
| `surfaceRecordConflict(error, t, options?)` | `@helios/ui/backend/conflicts` | Resolves a 409 to the shared conflict bar (`RecordConflictBanner`); returns `false` when the error is not a conflict so callers fall back to normal handling. `CrudForm`/`useGuardedMutation` call it automatically |
| `showRecordConflict`, `dismissRecordConflict`, `useRecordConflict` | `@helios/ui/backend/conflicts` | Lower-level conflict store access (rarely needed) |
| `pushOperation`, `useLastOperation`, `markUndoSuccess`, `dismissOperation`, `coalesceLastOperations` | `@helios/ui/backend/operations/store` | Feed the AppShell-mounted `LastOperationBanner` (undo bar) after destructive/bulk operations |
| `ProgressTopBar` | `@helios/ui/backend/progress/ProgressTopBar` | Sticky operation-progress bar — AppShell-mounted; drive it with `ProgressJob`s (see `packages/core/src/modules/progress/AGENTS.md`), never mount your own |
| `NextStepCallout` | `@helios/ui/backend/NextStepCallout` | Guided callout: `title`, `description?`, `steps?: { id, label, state }[]`, `actionLabel`, `onAction`, `status?` with tone/progress |
| `ContextHelp` | `@helios/ui/backend/ContextHelp` | Collapsible inline help box: `title`, `children`, `defaultOpen?`, `bulb?` |
...
```typescript
import { NotificationBell, NotificationPanel, NotificationItem, NotificationCountBadge, useNotifications, useNotificationEffect } from '@helios/ui/backend/notifications'
```
...
```typescript
import { ScheduleView, ScheduleToolbar, ScheduleGrid, ScheduleAgenda, type ScheduleItem, type ScheduleRange, type ScheduleViewMode } from '@helios/ui/backend/schedule'
```
...
```typescript
import { MessageComposer, SendObjectMessageDialog, MessageObjectRecordPicker, MessageObjectPreview, MessageObjectDetail, EmailThreadsPanel, MessagesIcon, useMessages } from '@helios/ui/backend/messages'
```
...
```typescript
import { FormHeader, FormFooter, FormActionButtons, ActionsDropdown, type ActionItem } from '@helios/ui/backend/forms'
```
...
| Export | Import | Purpose |
|---|---|---|
| `RowActions`, `RowActionItem` | `@helios/ui/backend/RowActions` | Per-row "…" menu: `items: { id?, label, onSelect?, href?, destructive? }[]` |
| `TruncatedCell` | `@helios/ui/backend/TruncatedCell` | Truncate + tooltip: `maxWidth?`, `tooltipContent?`, `disabled?`. DataTable applies it automatically from column `meta.truncate`/`meta.maxWidth` — import directly only outside DataTable |
| `BooleanIcon` | `@helios/ui/backend/ValueIcons` | Check/x icon for boolean cells (`value`, `trueLabel`/`falseLabel`) |
| `EnumBadge`, `EnumBadgeMap` | `@helios/ui/backend/ValueIcons` | Badge per enum value: `value`, `map: Record<string, { label, className?, icon? }>`, `fallback?` |
| `useSeverityPreset` | `@helios/ui/backend/ValueIcons` | Ready-made `EnumBadgeMap` for severity levels |

...
- **Version token = `updated_at`.** On load, an edit form captures the record's `updatedAt`.
  On Save/Delete it sends header `x-helios-ext-optimistic-lock-expected-updated-at: <iso>`
...
- **Default ON** (since Phase 14). Opt out with `HELIOS_OPTIMISTIC_LOCK=off` (or `false`/`0`/`no`/`disabled`/`none`).
...
- App booted from the **PR branch** (`feat/oss-optimistic-locking`), not a published-package standalone app.
  Reliable boot (per `qa-repro-report.md`): `yarn install` → `yarn build:packages` → `yarn turbo run generate` → `yarn build:packages` → `next dev -p 3100` in `apps/helios`. Or ephemeral: `yarn test:integration:ephemeral:start`.
- Admin login: `admin@acme.com` / `secret`.
- Sales specs need `yarn helios auth sync-role-acls` so the dev tenant's admin has sales features.
- **Default-ON** — nothing to enable. For the opt-out section, set `HELIOS_OPTIMISTIC_LOCK=off` and reboot.
...
| LOCK-M-GEN-02 | API probe: GET a record, PUT with a **stale** `x-helios-ext-optimistic-lock-expected-updated-at` | **409** with structured body |
...
| LOCK-M-NEG-02 | Set `HELIOS_OPTIMISTIC_LOCK=off`, reboot; re-run any conflict case above | **No 409 anywhere**; last-write-wins (feature disabled) |
| LOCK-M-NEG-03 | `HELIOS_OPTIMISTIC_LOCK` **unset** (default) | Feature **ON** — conflicts fire |
...
(run: `BASE_URL=<app> HELIOS_OPTIMISTIC_LOCK=all yarn test:integration -g "TC-LOCK-OSS"`).

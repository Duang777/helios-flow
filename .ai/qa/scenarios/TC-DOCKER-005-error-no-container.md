...
## Description
Verify that `docker-exec.mjs` prints a clear, actionable error message when no Helios app container is running, rather than hanging or producing a cryptic Docker error.

## Prerequisites
- No Helios containers running (`docker ps` shows no `helios-*` or compose app containers)
...
| 3 | Read error output | Message starts with `Error: No running Helios app container found.` |

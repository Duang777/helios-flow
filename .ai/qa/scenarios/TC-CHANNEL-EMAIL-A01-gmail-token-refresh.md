...
- An OAuth client ID/secret registered in Helios Integrations UI (`oauth_gmail` row in `integration_credentials`) for the tenant.
...
| 2 | Force the channel's `credentials.expiresAt` to a past timestamp (e.g. `now - 60s`). Update via DB query or by setting `HELIOS_PROFILE=communication_channels.*` and waiting >1 h for natural expiry. | `expiresAt` reflects the new past time. |

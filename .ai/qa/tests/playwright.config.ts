...
const STATIC_TEST_IGNORES = [
  ...
  // The create-app template ships specs that mirror apps/helios/example.
  ...
  // monorepo's apps/helios dev server duplicates the apps/helios suite
  // and produces cascading timeouts because the dev server is already
  // serving the apps/helios copies.
...
// Affected-only: when HELIOS_INTEGRATION_MODULES is set, restrict to those modules.
...
const affectedModules = process.env.HELIOS_INTEGRATION_MODULES
    ? new Set(
          process.env.HELIOS_INTEGRATION_MODULES.split(',')

...
- Reuse shared helpers from `@helios/core/helpers/integration/*`.
...
3. Use `/helios-integration-tests` against that URL
...
apps/helios/src/modules/<module>/__integration__/         # App-specific modules
...
Use shared helpers from `@helios/core/helpers/integration/*`. These are published in the npm package and available to both monorepo and standalone app developers.

> **Legacy path**: `@helios/core/modules/core/__integration__/helpers/*` still works in the monorepo via re-exports but is NOT available from npm. New code should use the `@helios/core/helpers/integration/*` path.

| Helper Import | Main Exports | Typical Use |
|------|-------|--------|
| `@helios/core/helpers/integration/auth` | `login`, `DEFAULT_CREDENTIALS` | UI authentication and role-based login (`admin`, `employee`, `superadmin`) |
| `@helios/core/helpers/integration/api` | `getAuthToken`, `apiRequest` | Authenticated API setup and raw API calls in integration tests |
| `@helios/core/helpers/integration/authUi` | `createUserViaUi` | Auth module UI flows for user creation/edit smoke coverage |
| `@helios/core/helpers/integration/catalogFixtures` | `createProductFixture`, `deleteCatalogProductIfExists` | Catalog fixture lifecycle for setup/cleanup |
| `@helios/core/helpers/integration/crmFixtures` | `createCompanyFixture`, `createPersonFixture`, `createDealFixture`, `deleteEntityIfExists`, `readJsonSafe` | Customers/CRM fixture creation and cleanup; `readJsonSafe` for parsing Playwright APIResponse body to JSON |
| `@helios/core/helpers/integration/salesFixtures` | `createSalesQuoteFixture`, `createSalesOrderFixture`, `createOrderLineFixture`, `deleteSalesEntityIfExists` | Sales API fixture lifecycle |
| `@helios/core/helpers/integration/salesUi` | `createSalesDocument`, `addCustomLine`, `updateLineQuantity`, `deleteLine`, `addAdjustment`, `addPayment`, `addShipment`, `readGrandTotalGross` | Sales document UI interactions and totals assertions |
| `@helios/core/helpers/integration/queue` | `drainIntegrationQueue` | Drains local queue jobs in the correct app context; standalone runs spawn from `HELIOS_TEST_APP_ROOT` so worker env/package resolution matches the app under test |
| `@helios/core/helpers/integration/authFixtures` | `createRoleFixture`, `deleteRoleIfExists`, `createUserFixture`, `deleteUserIfExists` | Role and user fixture lifecycle |
| `@helios/core/helpers/integration/generalFixtures` | `readJsonSafe`, `getTokenContext`, `expectId`, `deleteEntityByPathIfExists` | General-purpose test utilities |
| `@helios/core/helpers/integration/dictionariesFixtures` | `createDictionaryFixture` | Dictionary fixture creation |

Import pattern from module tests:

```ts
import { login } from '@helios/core/helpers/integration/auth';
import { apiRequest, getAuthToken } from '@helios/core/helpers/integration/api';
```
...
- This is required for standalone parity (`HELIOS_TEST_APP_ROOT`) because the Playwright process is the monorepo, while the job was created by the scaffolded app. The helper runs the drain from the target app root so env, generated files, package versions, and encryption keys match.

> **Barrel import** (subset): `@helios/core/testing/integration` re-exports the most common helpers as a single import for convenience.
...
`@helios/core/helpers/integration/crudFormPersistence`:
...
`HELIOS_INTEGRATION_CRUDFORM_EXTENSION_TESTS_DISABLED` (default **false** → the sweep runs). Set
truthy (`1`/`true`/`yes`/`on`) to skip every CrudForm-persistence spec wholesale without
deleting them — parsed via `parseBooleanWithDefault` from `@helios/shared/lib/boolean`.
...
# One module only:
HELIOS_INTEGRATION_MODULES=currencies npx playwright test --config .ai/qa/tests/playwright.config.ts CRUDFORM
# Disable the sweep (e.g. on a constrained CI lane):
HELIOS_INTEGRATION_CRUDFORM_EXTENSION_TESTS_DISABLED=1 yarn test:integration
```
...
| **From skill** | `/helios-integration-tests` | `.spec.ts` + optional scenario markdown |
...
### Option A: Use `/helios-integration-tests` Skill (Recommended)
...
- `apps/helios/src/modules/<module>/__integration__/TC-{CATEGORY}-{XXX}.spec.ts`
...
- Keep reusable helpers centralized in `packages/core/src/helpers/integration/` (importable as `@helios/core/helpers/integration/*`), and re-export from module-local helper files when needed
...
These accounts are created via `helios init` command:

...
**Tech Stack:** MikroORM v7, Postgres, Next.js (App Router), React, Awilix DI, Zod, Jest, Playwright, kysely (used by existing customers interactions route), `@helios/queue` (event subscribers), `@helios/shared/lib/encryption/find` (decryption helpers).
...
Run: `yarn workspace @helios/core typecheck 2>&1 | tail -20`
...
- Modify: `packages/core/src/modules/customers/migrations/.snapshot-helios.json`
...
```bash
git checkout -- packages/<other-module>/src/modules/<other-module>/migrations/.snapshot-helios.json
```
...
```bash
git add packages/core/src/modules/customers/migrations/Migration*_customers_email_integration.ts packages/core/src/modules/customers/migrations/.snapshot-helios.json
```
...
Run: `yarn workspace @helios/core typecheck 2>&1 | tail -10`
Expected: PASS.
...
Run: `yarn workspace @helios/core jest packages/core/src/modules/customers/lib/__tests__/findPeopleByAddresses.test.ts 2>&1 | tail -20`
...
Run: `yarn workspace @helios/core jest packages/core/src/modules/customers/lib/__tests__/findPeopleByAddresses.test.ts 2>&1 | tail -10`
...
Run: `yarn workspace @helios/core jest packages/core/src/modules/customers/lib/__tests__/visibilityFilter.test.ts 2>&1 | tail -10`
...
```ts
import { hasFeature } from '@helios/shared/security/features'
...
Run: `yarn workspace @helios/core jest packages/core/src/modules/customers/api/interactions 2>&1 | tail -15`
...
Run: `yarn workspace @helios/core jest packages/core/src/modules/customers/subscribers/__tests__/link-channel-message.test.ts 2>&1 | tail -10`
...
Run: `yarn workspace @helios/core jest packages/core/src/modules/customers/api/people/\[id\]/emails 2>&1 | tail -10`
...
```bash
yarn workspace @helios/core jest packages/core/src/modules/customers/subscribers/__tests__/link-channel-message.test.ts 2>&1 | tail -10
```
...
Run: `yarn workspace @helios/core jest packages/core/src/modules/customers/components/detail/__tests__/ComposeEmailDialog 2>&1 | tail -10`
Expected: FAIL.
...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@helios/ui/primitives/select'
...
- [ ] **Step 6: Run yarn helios configs cache structural to refresh the nav/structural cache**

Run: `yarn helios configs cache structural --all-tenants 2>&1 | tail -5`
...
```tsx
import { sanitizeChannelHtml } from '@helios/core/modules/communication_channels/lib/sanitize-channel-html'
...
Run: `yarn workspace @helios/core jest packages/core/src/modules/customers/api/interactions/\[id\]/visibility 2>&1 | tail -10`
Expected: PASS.
...
yarn workspace @helios/core typecheck
```
Expected: both PASS.
...
```ts
import type { ResponseEnricher } from '@helios/shared/lib/crud/response-enricher'
...
```bash
yarn workspace @helios/core jest packages/core/src/modules/customers/data/__tests__/enrichers.privateEmailCount
yarn workspace @helios/core typecheck
```
Expected: PASS.
...
Run: `yarn workspace @helios/docs dev 2>&1 | tail -5` (optional; depends on dev environment)
...
- [ ] **Step 8: yarn helios auth sync-role-acls on staging/local DB**

This ensures existing tenants get the two new features (`customers.email.compose` + `customers.email.view_private`):
```bash
yarn helios auth sync-role-acls 2>&1 | tail -5
```
Expected: PASS, two new features added to admin + employee roles in all tenants.

- [ ] **Step 9: yarn helios configs cache structural --all-tenants**

```bash
yarn helios configs cache structural --all-tenants 2>&1 | tail -5
```
...
4. **OSS independence**: the customers module must NOT import from `@helios/enterprise`. Run `grep -r "@helios/enterprise" packages/core/src/modules/customers/` after each phase and confirm no new matches.
...
8. **`yarn workspace @helios/core` vs `yarn workspace @helios/app`**: the customers module lives in `@helios/core`. Tests + typecheck run there. The Next.js app is `@helios/app` — only `yarn build:app` cares about it.

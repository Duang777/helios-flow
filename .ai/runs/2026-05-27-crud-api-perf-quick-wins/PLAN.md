...
Tracking issue: helios/helios#2044
...
| 1 | 1.2 | factory.ts: batch + fire-and-forget `logCrudAccess` + `HELIOS_CRUD_ACCESS_LOG_BLOCKING` | done | 68ca78035 |
...
- [x] Post-review fix: gate Phase 5 bootstrap once-guard behind `HELIOS_BOOTSTRAP_CACHE=1` (default OFF) so per-request bootstrap is restored unless explicitly enabled — cached `tenantEncryptionService`/event-bus close over the first request's `em.fork`/container, which manifests as a 500 on `/api/customers/people` under `next start` in the ephemeral integration runtime. — 85e0df6d3
...
Cut p50 latency for CRUD list/detail endpoints below 100 ms via five backward-compatible optimizations described in the spec. Measure the win with the existing `HELIOS_PROFILE=*` profiler and publish before/after numbers on the PR.
...
- New env flags: `HELIOS_CRUD_ACCESS_LOG_BLOCKING`, `HELIOS_CF_DEF_CACHE_TTL_MS`, `HELIOS_RBAC_DEFAULT_CACHE`, `HELIOS_ORG_SCOPE_CACHE_TTL_MS`
...
|---|---|---|
| Fire-and-forget access logs lose a row on crash | low | At-least-once already today; add `flushAccessLog()` helper + integration tests default to blocking via `HELIOS_CRUD_ACCESS_LOG_BLOCKING=1`. |
| CF def cache stale | medium | Tag-invalidate on CustomFieldDef CRUD side-effects; bounded TTL (5 min, override via `HELIOS_CF_DEF_CACHE_TTL_MS`). |
| RBAC memo desync mid-request | very low | Memo lifetime = single request via WeakMap on CrudCtx. |
| Org-scope cache staleness | low | 60s TTL with tag invalidation on `user_organizations`/`organizations` mutations; `HELIOS_ORG_SCOPE_CACHE_TTL_MS=0` disables. |
...
- Wrap the call in fire-and-forget by default; `await` only when `process.env.HELIOS_CRUD_ACCESS_LOG_BLOCKING === '1'`.
...
- Unit test: `logCrudAccess` defers when blocking flag is off and awaits when `HELIOS_CRUD_ACCESS_LOG_BLOCKING=1`.
...
- TTL from `HELIOS_CF_DEF_CACHE_TTL_MS` (default 300000); `0` disables.
...
- `auth/di.ts`: when no `cache` registration exists, register a process-scoped in-memory LRU `CacheStrategy` for RBAC. Opt-out via `HELIOS_RBAC_DEFAULT_CACHE=off`.
...
- TTL from `HELIOS_ORG_SCOPE_CACHE_TTL_MS` (default 60000); `0` disables.
...
**5.1** — `container.ts`:
- `globalThis.__heliosBootstrapped__` flag set after first `bootstrap(container)` call. Once set, only the request-scoped bindings (em, queryEngine, dataEngine, commandBus) are re-issued per request; the cache/event-bus/encryption side-effects are skipped.

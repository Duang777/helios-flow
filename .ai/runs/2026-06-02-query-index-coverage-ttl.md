...
Add tests proving fresh snapshots skip `refreshCoverageSnapshot`, stale snapshots trigger it, optimized mode serves stale data while scheduling async refresh, and run focused validation for `@helios/core`.
...
- `yarn workspace @helios/core test --runTestsByPath src/modules/query_index/__tests__/hybrid-engine.test.ts src/modules/query_index/__tests__/coverage-refresh.test.ts src/modules/query_index/__tests__/di.test.ts` — passed.
- `yarn workspace @helios/core build` — passed.
...
- `yarn workspace @helios/core test` — blocked by unrelated existing failure in `src/modules/api_keys/api/__tests__/keys.route.test.ts` (`em.transactional is not a function` in the test mock); rerunning that single test reproduces the same failure.
- Follow-up for CI: `yarn workspace @helios/core test --runTestsByPath src/modules/api_keys/api/__tests__/keys.route.test.ts` — passed after adding the missing transactional EM mock.

## PR

- Opened PR #2401 against `helios:develop`.

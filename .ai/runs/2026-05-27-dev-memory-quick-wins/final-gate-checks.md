...
| `yarn build:packages` / `yarn build:app` | Requires installed deps and built outputs | **Low** — no runtime code in `packages/*` or `apps/helios/src/*` is touched |
...
| `yarn generate` | Requires helios CLI built | **Low** — no module / entity / route / event additions |

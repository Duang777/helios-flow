...
- Add `scripts/profile-dev-rss.mjs` — a stand-alone Node script that snapshots the full `yarn dev` process tree's RSS over a fixed window (default 90s) and writes a JSON report to `.helios/dev-rss/<label>.json`. Reads PIDs via `ps` (linux/darwin) and computes per-process and total-tree RSS deltas.
...
- Add to root `package.json`:
  ...
  - `"dev:profile:report": "node ./scripts/profile-dev-rss.mjs --report"` — prints a Markdown comparison table of all `.helios/dev-rss/*.json` files.
- Document usage at the top of `scripts/profile-dev-rss.mjs`.

### Phase 2 — Opt-in heap cap

**Step 2.1 — `HELIOS_PACKAGE_WATCH_HEAP_MB` in `scripts/watch.mjs`**

- At the top of `scripts/watch.mjs`'s `watch()` function (before the esbuild context creation), read `process.env.HELIOS_PACKAGE_WATCH_HEAP_MB`.
- If set to a positive integer N and `process.env.HELIOS_PACKAGE_WATCH_HEAP_APPLIED !== '1'`:
  - Set `HELIOS_PACKAGE_WATCH_HEAP_APPLIED=1` in env to prevent re-exec loop.
...
- Log a single line on activation: `[watch] applying --max-old-space-size=${N} (HELIOS_PACKAGE_WATCH_HEAP_MB)`.
...
- New file `.ai/specs/2026-05-27-dev-mode-memory-quick-wins.md` with sections:
  ...
  - **Migration & Backward Compatibility** — no contract surface touched. `HELIOS_PACKAGE_WATCH_HEAP_MB` is additive and unset by default.
...
# (wait for "[profile] done → .helios/dev-rss/baseline-pre-2102.json")

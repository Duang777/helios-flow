...
When module files in a standalone Helios app are added, removed, or edited, `next dev --turbopack` keeps serving stale compiled chunks (or stale compile errors) until a full dev-server restart. Hook the existing **structural changes system** (`yarn helios configs cache structural`) so it also nudges Turbopack's filesystem watcher into re-evaluating the generated module barrels — without killing the dev server. Provide a `dev:reset` escape hatch for the rare cases where Turbopack's internal cache stays stuck.
...
- `apps/helios/scripts/dev-reset.mjs` + `package.json` — new `yarn dev:reset` script.
...
|---|---|
| `helios configs cache structural` | Wraps `runCachePurge` with `pattern: 'nav:*'`. Pure Redis-segment purge; no filesystem touch. |
| `runCachePurge` (Redis) | Deletes `nav:*` cache keys for all/specific tenants. |
| `helios generate` post-step | Already invokes `configs cache structural --all-tenants --quiet` after every successful generate (`packages/cli/src/helios.ts:520`). |
...
| Dev wrapper | `scripts/dev.mjs` spawns `helios generate watch` + `next dev`. Watcher subprocess re-runs generator on file change. No `.next` deletion logic. |
| Existing AGENTS.md guidance | Already tells agents to run `yarn helios configs cache structural --all-tenants` after structural changes — currently only purges nav cache. |
...
1. Add `packages/core/src/modules/configs/lib/touchGeneratedBarrels.ts` exporting `touchGeneratedBarrels(opts: { cwd?: string; quiet?: boolean })`.
   - Walks up from `cwd` (default `process.cwd()`) up to 4 levels looking for `.helios/generated/`.
...
2. Wire `runStructuralCachePurge` (configs/cli.ts) to call `touchGeneratedBarrels` after the Redis purge.
   ...
   - Log a single line in non-quiet mode: `🔁 [structural] touched N generated barrel(s) → /<path>/.helios/generated/`.
3. Don't change the existing `nav:*` Redis purge behavior.

### Phase 2 — Standalone escape hatch: `yarn dev:reset`

1. Create `apps/helios/scripts/dev-reset.mjs`:
...
2. Add `"dev:reset": "node scripts/dev-reset.mjs"` to `apps/helios/package.json`.
...
1. `packages/core/src/modules/configs/__tests__/touchGeneratedBarrels.test.ts`:
   - Creates `tmp/.helios/generated/foo.generated.ts` with known content + old mtime; runs helper; asserts content unchanged but mtime advanced.
   - Asserts no-throw when `.helios/generated/` is missing.
...
2. Update root `AGENTS.md` line `Agents MUST automatically run yarn helios configs cache structural --all-tenants ...` to mention Turbopack-cache invalidation alongside `nav:*` Redis purge.
...
6. `yarn test`
7. `yarn build:app`
8. Self-review against `helios-code-review` skill + `BACKWARD_COMPATIBILITY.md`.
...
- **Standalone vs monorepo path resolution** — Generated dir lives at `<app>/.helios/generated/` in both layouts; CLI is invoked from app cwd, so resolution is identical. Test covers cwd-walk fallback.
...
- [x] 1.3 Verify `yarn helios configs cache help` still passes — eb7ee11e5

### Phase 2: `dev:reset` escape hatch

- [x] 2.1 Add `apps/helios/scripts/dev-reset.mjs` + package.json entry — e14701495

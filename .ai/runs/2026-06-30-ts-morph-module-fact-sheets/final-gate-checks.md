...
| Check | Result |
|-------|--------|
| `yarn build:packages` | ✅ 21/21 tasks (cli, core [3429 entrypoints], create-helios-app [9 fact-sheets], all packages) |
| `yarn generate` | ✅ exit 0, "All generators completed"; **versioned `apps/helios/src/module-facts.generated.json` has ZERO drift** (generate-wired extractor matches the committed artifact — D4 validated). Structural cache purge skipped (no local Postgres) — tolerated/non-fatal. |
| `yarn i18n:check-sync` | ✅ all 4 locales in sync across 48 modules (no churn from this change) |
| `yarn workspace @helios/cli typecheck` | ✅ exit 0 |
| `yarn workspace @helios/cli test -- module-facts` | ✅ 29 tests (T1–T4) |
| `yarn workspace create-helios-app typecheck` | ✅ exit 0 |
| `yarn workspace create-helios-app test` | ✅ 68 tests (incl. T5 build smoke + T6 module-guides), 0 fail |
...
- Formal `helios-auto-review-pr` autofix pass — a self code-review + BC self-review were done instead (below); a formal review can run on the PR.

## Self code-review (helios-code-review + BACKWARD_COMPATIBILITY.md)

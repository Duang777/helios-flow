...
- Generated apps/helios/src/module-facts.generated.json for all 9 D5 modules; real registry-resolved API auth confirmed.
...
- Per maintainer decision (via `helios-help` → `auto-continue-pr`), the live run's process tree was stopped, draft PR #3715 was opened against helios:develop and claimed (`in-progress`), and this session took over from Step 1.7. PR #3685 (design-spec only) is superseded and to be closed once #3715 lands.
...
- Next: Phase 3 (build-system surgery) — Step 3.1 build.mjs extraction step. NOTE: extractor not yet exported from @helios/cli `exports`; resolve that first. Phase 3 verification needs full build runs (yarn build:packages + create-app build + yarn test:create-app for T5/T6).

## 2026-06-30T15:58:00Z — decision: @helios/cli as create-app build-time devDep (3.1)
- The cli `exports` wildcard already maps `@helios/cli/lib/generators/module-facts` → dist; build.mjs imports it. Declared cli as a create-app devDependency (build-time only, never shipped). Resolves via hoisting; lockfile synced (1 edge).

## 2026-06-30T16:02:00Z — decision: ts-morph as create-app RUNTIME dep (3.2, user-approved)
- shared.ts runs at `npx create-helios-app` time, where devDeps/@helios/cli are NOT installed. Per user choice, ts-morph added to create-app `dependencies` so the spec's literal "AST-parse enabledModules with ts-morph" works in the scaffolded context. Allowlist is materialized as the bundled fact-sheet set (enabled ∩ available), so no cli import is needed at runtime. Lockfile synced (1 edge).
...
- Deferred to PR CI: build:app + integration suites (N/A per spec §10 — no HTTP/UI) + a formal helios-auto-review-pr. ds-guardian N/A (no .tsx diff).

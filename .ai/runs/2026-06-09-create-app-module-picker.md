...
`create-helios-app` so that, after choosing a starter preset (classic / empty / crm), the user can
select **individual modules** to enable, with their `requires` dependencies auto-included, and can
optionally pull in **official modules** from `helios/official-modules` — reusing the existing
...
- https://github.com/helios/official-modules — the official-modules repo the scaffolder fetches
...
- Live remote fetch introduces a network dependency in the scaffolder hot path — spec must define
  timeout, caching, and offline fallback so `create-helios-app` never hard-fails without network.

...
- REMOVE full duplicates (now external): helios-auto-create-pr, helios-auto-continue-pr, helios-auto-create-pr-loop, helios-auto-continue-pr-loop, helios-auto-review-pr, helios-code-review, helios-integration-tests, helios-prepare-issue, helios-spec-writing. Plus helios-auto-fix-github → external helios-auto-fix-issue.
- KEEP repo-local OVERRIDE folders (fold STANDALONE.md standalone behavior into a slim override SKILL.md the external skill reads): helios-auto-create-pr, helios-auto-continue-pr, helios-auto-create-pr-loop, helios-auto-continue-pr-loop, helios-auto-review-pr. (These had STANDALONE.md overlays.) The autofix override folder becomes helios-auto-fix-issue (from helios-auto-fix-github's STANDALONE.md). helios-code-review/helios-integration-tests/helios-prepare-issue/helios-spec-writing had NO overlay → no override folder (config covers them).
- KEEP standalone-only local skills (never external): helios-module-scaffold, helios-trim-unused-modules, helios-system-extension, helios-eject-and-customize, helios-data-model-design, helios-troubleshooter.
- KEEP monorepo-local skills offline: helios-help, helios-implement-spec, helios-backend-ui-design, helios-integration-builder (has STANDALONE.md), helios-auto-upgrade-0.4.10-to-0.5.0.

## External install list for the standalone (only what it ships now):
helios-auto-create-pr, helios-auto-continue-pr, helios-auto-create-pr-loop, helios-auto-continue-pr-loop, helios-auto-review-pr, helios-code-review, helios-integration-tests, helios-prepare-issue, helios-spec-writing, helios-auto-fix-issue
...
- [x] 1.3 Add `agentic/shared/scripts/install-skills.sh` (offline-safe: local per-skill symlinks + OPTIONAL `npx skills add`/`update` of the explicit subset; `--no-external`/HELIOS_SKIP_EXTERNAL_SKILLS)
...
- [x] 2.1 Convert the 5 overlay skills (+ autofix) to slim repo-local OVERRIDE folders (SKILL.md derived from STANDALONE.md, external skill read in place); rename autofix override to helios-auto-fix-issue
- [x] 2.2 Delete the 9 duplicate full-copy folders + helios-auto-fix-github folder from agentic/shared/ai/skills/
...
- [x] Post-review fix: mirror the monorepo's QA runtime-state gitignore block (`.ai/qa/test-env.json`, `test-env.lock/`, `test-env-boot.log`, `artifacts_*/`, `ephemeral*`) into `template/gitignore` — required after base-branch commit 83c5408a0 (helios-prepare-test-env compile-once entrypoints) so scaffolded apps don't track skill-generated runtime state — 739006e01
- [x] Post-review fix: route the six helios-auto-* standalone overrides through the tracker abstraction — drop inlined raw `gh` commands (base-branch discovery, label opt-in guards, package.json script probing) in favor of the tracker descriptor's **default-branch** / `apply_label`/`label_exists` / **ensure-label-taxonomy** operations and the pipeline config (`baseBranch: "auto"`, `labels.enabled: false`, `validation.commands`); guard test now bans raw gh tracker commands in overrides. Verified: base skills and shipped tracker are already fully abstracted upstream — no helios/skills PR needed — 103f3fe04
</content>

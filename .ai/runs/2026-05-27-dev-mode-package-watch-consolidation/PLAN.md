...
2. `package.json` change: `watch:packages` becomes `node ./scripts/watch-packages.mjs`. Old behavior preserved as `watch:packages:legacy` (Turbo path) and honored by env var `HELIOS_WATCH_PACKAGES_MODE=legacy`.
...
- No change to the per-package `node watch.mjs` scripts under `packages/*/watch.mjs` — they still work when invoked directly (e.g. `yarn workspace @helios/shared watch`). Removing them would be a breaking workspace contract.
- No change to the `HELIOS_PACKAGE_WATCH_MODE=persistent` opt-in path inside `scripts/watch.mjs`. Users who want the legacy persistent esbuild context can still set the env var when running per-package.
...
- **Turbo escape hatch must keep working**: dev infra contributors used to invoking `turbo run watch` directly. Mitigation: `HELIOS_WATCH_PACKAGES_MODE=legacy` re-runs the original Turbo command.

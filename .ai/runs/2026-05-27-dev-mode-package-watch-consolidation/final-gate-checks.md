...
| `yarn build:app` | Same dependency requirement. The change does not touch `apps/helios/src/` — only `scripts/` and `package.json`. |
...
- **No backwards-compat regressions:** Per-package `packages/<pkg>/watch.mjs` scripts are untouched. `yarn workspace @helios/<pkg> watch` still works the same way (low-memory or persistent modes per `HELIOS_PACKAGE_WATCH_MODE`). The `HELIOS_WATCH_PACKAGES_MODE=legacy` env var routes through the prior Turbo path exactly as before for anyone who needs it.

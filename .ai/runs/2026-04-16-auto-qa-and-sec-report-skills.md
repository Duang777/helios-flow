...
# Execution Plan — `helios-auto-qa-scenarios` and `helios-auto-sec-report` skills
...
`helios-auto-create-pr` workflow for branch/worktree/commit/PR discipline and leave
resumable progress via `helios-auto-continue-pr` when the run cannot finish in one
pass.

## Scope

- `.ai/skills/helios-auto-qa-scenarios/SKILL.md` — QA-tester-facing report that
  groups merged PRs by practical testing routes (P0/P1/P2), tells QA where
  to click, what to verify, and what can go wrong.
- `.ai/skills/helios-auto-sec-report/SKILL.md` — Security-engineer-facing report
...
- Do not change the existing `helios-auto-create-pr` / `helios-auto-continue-pr` /
  `helios-auto-review-pr` skills. The new skills reuse them by reference.
...
<https://github.com/helios/helios/pull/1527/files> was consulted
...
- Draft `auto-qa-scenarios/SKILL.md` covering argument parsing (date / PR
  ...
  `helios-auto-create-pr` handoff, and the `helios-auto-continue-pr` resume contract.
...
- Skill descriptions must be precise enough that agents auto-select them
  ...
  "security audit", "OWASP", "helios-auto-sec-report").
...
- [x] 4.1 Add `helios-auto-sec-report-pr` single-unit skill covering PR / spec / branch inputs with paranoid deep vectors and "Next steps — go deeper"
- [x] 4.2 Add `references/deep-attack-vectors.md` paranoid checklist bundled with `helios-auto-sec-report-pr`
- [x] 4.3 Refactor `helios-auto-sec-report` to loop `helios-auto-sec-report-pr` in sub-unit mode and aggregate per-unit fragments
- [x] 4.4 Update README.md skills index: new row for `helios-auto-sec-report-pr`, updated row for driver `helios-auto-sec-report`, folder added to structure tree

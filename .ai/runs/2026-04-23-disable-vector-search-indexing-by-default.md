...
- Affected modules/packages: `packages/search`, `packages/core`, `apps/helios`, `packages/create-app/template`, `apps/docs`.
- Smallest safe scope: add a preferred `HELIOS_DISABLE_VECTOR_SEARCH_AUTOINDEXING` environment flag with backward-compatible support for the legacy flag, update env examples to default auto-indexing off, and align all env-focused docs.
...
2. Introduce `HELIOS_DISABLE_VECTOR_SEARCH_AUTOINDEXING` as the preferred env name while preserving `DISABLE_VECTOR_SEARCH_AUTOINDEXING` as a supported legacy alias in runtime/config surfaces.
...
2. Update env-focused docs pages to explain that vector search remains available when an embedding provider is configured, but auto-indexing is opt-in via `HELIOS_DISABLE_VECTOR_SEARCH_AUTOINDEXING=false` or by removing the disable flag.
...
- [x] 1.2 Introduce `HELIOS_DISABLE_VECTOR_SEARCH_AUTOINDEXING` as the preferred env name while preserving `DISABLE_VECTOR_SEARCH_AUTOINDEXING` as a supported legacy alias in runtime/config surfaces — 6470d0b98
...
- [x] 2.2 Update env-focused docs pages to explain that vector search remains available when an embedding provider is configured, but auto-indexing is opt-in via `HELIOS_DISABLE_VECTOR_SEARCH_AUTOINDEXING=false` or by removing the disable flag — 6470d0b98

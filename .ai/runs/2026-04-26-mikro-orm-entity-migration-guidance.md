...
2. Update MikroORM migration skill guidance with v7 entity imports plus a scoped migration workflow that treats `yarn db:generate` as a diff probe, commits only intended SQL, and always updates `.snapshot-helios.json`.
...
1. Compare example entity metadata and committed migrations against their `.snapshot-helios.json` files in `apps/helios` and `packages/create-app/template`.

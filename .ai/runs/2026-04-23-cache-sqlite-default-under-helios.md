# Execution Plan — Cache SQLite Default Under `.helios`

## Overview

- Goal: Move the default SQLite cache file path away from tracked `data/cache.db` and into the runtime-owned `.helios` area, then lock that behavior with tests.
- Affected modules/packages: `packages/cache`, `packages/core`, `apps/helios`, `packages/create-app/template`, docs that surface the default cache path.
...
1. Identify every authoritative default for SQLite cache file locations that currently points outside `.helios`.
2. Choose a single `.helios` default path and update runtime-facing defaults consistently.
...
- [x] 1.1 Identify every authoritative default for SQLite cache file locations that currently points outside `.helios` — 301179662
- [x] 1.2 Choose a single `.helios` default path and update runtime-facing defaults consistently — 301179662
...
- [x] Post-review fix: Drop legacy `apps/helios/data` entry from root `.gitignore` so the ignore list matches the new `.helios/cache/*` convention — 6d7a802b4

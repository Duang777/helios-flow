...
| Step | SHA | Subject |
|------|-----|---------|
| 0.1 | 064e832b4 | feat(ai-assistant): support HELIOS_AI_PROVIDER + HELIOS_AI_MODEL env defaults |
| 0.2 | 80ad3c567 | test(ai-assistant): cover HELIOS_AI_PROVIDER + HELIOS_AI_MODEL in model-factory |
| 0.3 | ad78f1aec | test(ai-assistant): smoke env_default source through model-factory integration |
| 0.4 | f602ddb97 | docs(env): document HELIOS_AI_PROVIDER and HELIOS_AI_MODEL |
| 0.5 | 267f0ae26 | docs(ai-assistant): document HELIOS_AI_PROVIDER + HELIOS_AI_MODEL |
| 0.6 | 54585d9b8 | docs(framework): document HELIOS_AI_PROVIDER + HELIOS_AI_MODEL in overview |
| 0.7 | 4d3b5bdc4 | feat(ai-assistant): honor HELIOS_AI_PROVIDER in routing route |
...
- **R1 (HIGH) — Phase 3 silent provider flip:** mitigation honored by *not* aliasing `OPENCODE_PROVIDER` to `HELIOS_AI_PROVIDER`. The model-factory error message names both vars distinctly.

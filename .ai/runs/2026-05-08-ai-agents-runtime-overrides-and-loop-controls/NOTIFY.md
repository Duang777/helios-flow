...
- Decision: open the PR with `Status: in-progress` and hand off the remaining 73 Steps to `helios-auto-continue-pr-loop`. This honors the auto-create-pr-loop "safety stop after ~20 consecutive Steps" guidance and the spec authors' explicit "1 PR per phase" plan, while still landing the prerequisite Phase 0 in a clean unit.

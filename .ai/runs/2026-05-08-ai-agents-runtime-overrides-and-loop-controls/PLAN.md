...
> Authoritative status table. `Status` is `todo` or `done`. The first row whose `Status` is not `done` is the resume point for `helios-auto-continue-pr`. Step ids are immutable once committed.

| Phase | Step | Title | Status | Commit |
|-------|------|-------|--------|--------|
| 1780-0 | 0.1 | Add `HELIOS_AI_PROVIDER` + `HELIOS_AI_MODEL` resolution to `model-factory.ts`; add `'env_default'` to `AiModelResolution.source` | done | 064e832b4 |
| 1780-0 | 0.2 | Update `model-factory.test.ts` with the 5 cases from spec 1780 §Phase 0 Tests | done | 80ad3c567 |
| 1780-0 | 0.3 | Update `model-factory.integration.test.ts` with end-to-end smoke for `HELIOS_AI_PROVIDER` | done | ad78f1aec |
| 1780-0 | 0.4 | Update `apps/helios/.env.example` and `packages/create-app/template/.env.example` | done | f602ddb97 |
...
| 1780-1 | 1.3 | Add `agentDefaultProvider` and `HELIOS_AI_<MODULE>_PROVIDER` env axis to `AiModelFactoryInput` (legacy `<MODULE>_AI_PROVIDER` honored as BC fallback) | done | 15d3248e7 |
...
- `apps/helios/.env.example`, `packages/create-app/template/.env.example`
...
- **Phase 3 risk (1780 R1 HIGH)** — call-site cleanup could silently flip a deployment's resolved provider; mitigated by explicitly *not* aliasing `OPENCODE_PROVIDER` to `HELIOS_AI_PROVIDER`.
...
- Per-checkpoint validation per `helios-auto-create-pr-loop` (typecheck + scoped unit tests + i18n when strings change + generate when module structure changes).
- Final gate: full validation gate + `yarn test:integration` + `yarn test:create-app:integration` + `helios-ds-guardian` pass.

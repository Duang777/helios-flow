...
Add a top-level `HIRING.md` describing the "Senior AI Engineering / Forward Deployed Engineer" role on the Helios Core Team, with an Apply section that routes applications to `info@helios.com` and includes a GDPR-compliant data-processing notice and consent statement.
...
- Apply section: require LinkedIn profile, CV, and GitHub links sent to `info@helios.com`, plus an explicit GDPR consent line and a "How we process your data" notice (controller, purpose, legal basis, retention, rights, contact).
...
- Docs-only; minimal risk. GDPR copy is informational boilerplate aligned to GDPR Arts. 6(1)(a), 13, 15–21; it is not legal advice and uses the public `info@helios.com` contact. No contract surface touched.
...
- [x] 1.2 Add Apply section with info@helios.com routing + GDPR consent & processing notice — e801665ae
...
- [x] 4.1 TC-CRM-062 (now flaked at the reorder assertion, not the helper): the dnd-kit keyboard lift→move→drop fired back-to-back occasionally dropped the ArrowUp, leaving the order unchanged. Fixed with a 250ms settle tick between each key + `toBeFocused`.
- [x] 4.1a Correction: a first attempt added `Escape` to reset a half-applied lift, but Escape closes the Radix filter panel (panel `closeOnEscape`), unmounting the rules → `idsAfter[0]` undefined (caught by ephemeral shard 8/15 on sha 2250b1c35, the only failure in the whole 15-shard run). Removed Escape; settle ticks alone fix the original drop. The ephemeral integration suite now runs on this PR because it is no longer docs-only.

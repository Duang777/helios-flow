# Execution plan: spec-tracking-issue support in `helios-followup-issue-from-pr`

Tracking plan for an `helios-auto-create-pr` run.

## Goal

Extend the `helios-followup-issue-from-pr` skill so that, in addition to turning a review
...
- Edit only `.ai/skills/helios-followup-issue-from-pr/SKILL.md` (docs/process automation).
- No code changes. No new dependencies. No contract surfaces touched.

### Non-goals

- Not changing `helios-prepare-issue`, `helios-spec-writing`, or `helios-implement-spec`.
...
- Tracking-issue convention (from `helios-prepare-issue`): title `Implement: <feature>`,
  body has `## Spec` (spec path + spec PR) and `## How to implement` (run
  `/helios-implement-spec <path>` after the spec PR merges); labels `feature`

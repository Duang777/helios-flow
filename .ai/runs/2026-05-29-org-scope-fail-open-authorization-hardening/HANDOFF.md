...
- Branch: `fix/org-scope-fail-open-authorization-hardening` (fork `adeptofvoltron`, cross-repo PR → `helios/helios:develop`).
...
## Still blocked — require WRITE access to `helios/helios` (this account is `READ`)
...
6. **`helios-auto-review-pr` autofix pass**: needs review-verdict rights (and a non-author reviewer — the author cannot approve their own PR). Self code-review + BC self-review found no actionable findings.

## Documented skip (environment, not code)
7. **`yarn test:create-app:integration`**: blocked by a pre-existing `helios-verdaccio` container-name conflict on this host; justified skip (the only added shared export is the additive internal predicate `isOrganizationAccessAllowed`, not used by the create-app template).

## Node version (load-bearing)
Helios requires **Node 24.x**. The default shell here is Node 22 — `yarn generate` / `build:app` / the ephemeral harness fail the runtime gate under Node 22. Activate Node 24 (`nvm use 24`) + `yarn install` before any gate/integration command.

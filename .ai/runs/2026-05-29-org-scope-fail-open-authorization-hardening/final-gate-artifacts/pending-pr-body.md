...
- `@helios/shared` + `@helios/core`: build + typecheck clean.
...
- Standalone `yarn test:create-app:integration` was environment-blocked (pre-existing `helios-verdaccio` container name conflict, not a code issue) — justified skip; the only added shared export is an additive internal predicate not used by the create-app template.

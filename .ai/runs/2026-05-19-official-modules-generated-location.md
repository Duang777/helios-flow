...
Document — in code, docs, and a new ADR-style spec — the rationale for keeping `apps/helios/src/official-modules.generated.ts` in `src/` rather than moving it into a `generated/` folder or inlining it into `apps/helios/src/modules.ts`. No file moves. No runtime behaviour changes.
...
- `scripts/lib/official-modules.mjs` — extend the generated-file banner to point at the spec/docs, so the next developer who opens the file sees "why am I here, not in `.helios/generated/`?" immediately.
- Re-run the generator so the committed `apps/helios/src/official-modules.generated.ts` banner picks up the new wording.
...
| B. Move into `apps/helios/.helios/generated/` | **Rejected** | `.helios` is gitignored AND wiped by `scripts/clean-generated.sh`. Activation state would be lost across clones / on every `yarn clean-generated`. Destroys the single source of truth. |
| C. Move into `apps/helios/src/generated/` | **Rejected** | `.gitignore` line 62 (`/src/generated/`) makes the file untracked. Same source-of-truth loss as B. |
| D. Inline contents into `apps/helios/src/modules.ts` | **Rejected** | `modules.ts` is hand-curated (env gates, enterprise toggles, comments). `yarn official-modules add/remove` would have to do AST edits on user-curated code — fragile, conflict-prone, and weakens diff review (auto-gen interleaved with hand-curated logic). |
...
- 2.2 Re-run the generator (manually invoke `writeGenerated` via `yarn official-modules` no-op path, or run the postinstall) so the committed `apps/helios/src/official-modules.generated.ts` banner picks up the new wording. Commit the regenerated file alongside the script change.
...
- 3.2 Run the full gate per `helios-auto-create-pr` step 7 — `yarn build:packages`, `yarn generate`, `yarn build:packages`, `yarn i18n:check-sync`, `yarn i18n:check-usage`, `yarn typecheck`, `yarn test`, `yarn build:app`. If `yarn install` is impractical in this environment, document the blocker and ship docs-only.
...
- 4.3 Run `helios-auto-review-pr` against the PR in autofix mode; address actionable findings as new commits.
...
- [x] 2.2 Re-run generator and commit refreshed apps/helios/src/official-modules.generated.ts — captured in autosave 04b60fee4
...
- [x] 4.1 Open PR against develop — https://github.com/helios/helios/pull/1983

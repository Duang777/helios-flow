...
- Do not change `CACHE_STRATEGY=sqlite` in `apps/helios/.env.example`.
...
Run focused package validation, self-review backward compatibility, commit, push to the fork, and open a PR against `helios:develop`.
...
- `yarn workspace @helios/cache test` — passed.
- `yarn workspace @helios/cache build` — passed.
...
- `yarn workspace @helios/cache typecheck` — blocked by package-local script not resolving root `typescript`; root `tsc` command above covered the package.
...
- Opened PR #2400 against `helios:develop`.

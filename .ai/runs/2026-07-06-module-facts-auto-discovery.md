...
- `apps/helios/src/module-facts.generated.json` — regenerated snapshot.
- `RELEASE_NOTES.md` — allowlist deprecation + widened artifact note.

## Non-goals

- Parsing compiled `.js` (`node_modules/@helios/*/dist/modules`) — deferred (spec §11).
...
- [x] 1.4 Regenerate `apps/helios/src/module-facts.generated.json` snapshot via `yarn generate` (9 → 47 modules; enterprise env-gated off by default, app-local excluded) — 31ef54051
...
- [x] 5.1 Remove committed `apps/helios/src/module-facts.generated.json` + `generateModuleFacts` wrapper (`module-facts-generate.ts`), its `helios.ts` `yarn generate` call site, and the `generators/index.ts` re-export — 512f98ceb
...
- [x] 5.3 Rewrite `module-facts.discovery.test.ts` onto `discoverPackageModuleSources`; trim `generateModuleFacts` mocks from `helios.test.ts` — 512f98ceb
...
- [x] 6.2 Drop the now-dead `apps/helios/src/module-facts.generated.json` rule from `.gitignore` (file removed in Phase 5, `yarn generate` no longer emits it) — 29b1b162e

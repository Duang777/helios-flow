...
Run the `helios-auto-update-changelog` handoff to prepend a drafted `0.5.0` release
...
- **Scope.** `CHANGELOG.md` only. No other files. No spec updates. No
  ...
  `helios-auto-update-changelog` skill rules).
- **Non-goals.**
  - Bumping `package.json` version to `0.5.0` (separate PR).
  - Rewording entries beyond what `helios-auto-update-changelog` already produced.
...
- **2.3** Run `helios-auto-review-pr` in autofix mode to catch any review-surface
...
- [x] 2.3 Run `helios-auto-review-pr` autofix pass — 268351ee3 (replaced 11 GitHub-truncated titles with full summaries)

...
Make the dev-mode watch-scope memory optimization discoverable and pleasant: document `HELIOS_WATCH_SCOPE` in `.env.example`, point README/docs at it near `yarn dev`, and display the active watch mode with a friendly emoji in the dev log.

## Overview

The consolidated package watcher already supports narrowing which packages it tracks via `HELIOS_WATCH_SCOPE` (`all` | `auto-optimized` | `popular` | `env`), which is the main lever for reducing dev-mode memory usage. The behavior is fully implemented (`scripts/watch-scope.mjs`, `scripts/watch-packages.mjs`, `scripts/dev.mjs`) and documented in `apps/docs/docs/appendix/troubleshooting.mdx`, but:

- It is missing from `apps/helios/.env.example` and the create-app template `.env.example`, so users never discover it.
...
- `apps/helios/.env.example` — add a documented `HELIOS_WATCH_SCOPE` block (+ related `HELIOS_WATCH_*` vars).
- `packages/create-app/template/.env.example` — mirror the same block (kept in sync with helios).
...
- [x] 2.1 Add documented `HELIOS_WATCH_SCOPE` block to `apps/helios/.env.example` and template `.env.example` — 8080bad6d

...
|---------|--------|-------|
| `yarn workspace @helios/ui test -- AttachmentMetadataDialog` | PASS | Regression test covers bounded assignment row classes and accessible remove action. |
| `yarn workspace @helios/ui build` | PASS | UI package builds with the `IconButton` import and layout changes. |
...
| `yarn test` | FAIL | Fails in `packages/create-app/src/lib/template-api-dispatcher-require-roles.test.ts` because the template API dispatcher is not byte-identical to the monorepo dispatcher. This branch does not touch `packages/create-app`, `apps/helios`, `packages/core`, or `packages/shared`. |

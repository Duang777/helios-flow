...
- `yarn test` fails in `packages/create-app/src/lib/template-api-dispatcher-require-roles.test.ts` because the template API dispatcher is no longer byte-identical to the monorepo dispatcher. This branch has no changes under `packages/create-app`, `apps/helios`, `packages/core`, or `packages/shared`.
...
- Full final gate per `helios-auto-create-pr-loop`:
  - `yarn build:packages`
  - `yarn generate`
  - `yarn build:packages`
...
- DS/code/BC review:
  - `helios-ds-guardian` review against `origin/develop..HEAD`
  - `.ai/skills/helios-code-review/SKILL.md`

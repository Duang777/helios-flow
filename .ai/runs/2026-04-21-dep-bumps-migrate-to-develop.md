...
([#1621 — major group](https://github.com/helios/helios/pull/1621),
[#1620 — minor/patch group](https://github.com/helios/helios/pull/1620))
...
Open the PR against `develop`, apply labels, run `helios-auto-review-pr`, post the
summary comment.
...
- [x] 5.4 Resume via `helios-auto-continue-pr` — revert BC-breaking majors, adapt to remaining majors
- [x] 5.5 Post comprehensive summary comment

## Phase 6 (resume): Skip BC-breaking majors, adapt to remaining majors

- [x] 6.1 Revert `@mikro-orm/*` `^7.0.11 → ^6.6.10` in root + `apps/helios` — a94f97e94
...
- [x] 9.4 Verify the fix locally: `yarn workspaces focus helios-docs` + `yarn build` in `apps/docs` succeeds, plus full `docker build -f apps/docs/Dockerfile .` reproduces the green CI build — 91b4e78cb
...
- 2026-04-21 — Phases 1–3 complete; Phase 4 gate failed at typecheck/test/build:app with
  ...
  `Status: in-progress`.
- 2026-04-21 — Resumed via `helios-auto-continue-pr`. Per user directive ("skip the mikro-orm

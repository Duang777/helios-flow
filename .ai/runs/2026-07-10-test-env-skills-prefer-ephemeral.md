...
**Scope:**
- Monorepo repo-local skill overrides: `.ai/skills/helios-prepare-test-env/SKILL.md`,
  `.ai/skills/helios-integration-tests/SKILL.md`.
- Standalone override shipped by create-app:
  `packages/create-app/agentic/shared/ai/skills/helios-prepare-test-env/SKILL.md`.
- Standalone template scripts: re-add the cross-platform helios-CLI aliases
  `test:integration:ephemeral:start[:verbose]` to
  `packages/create-app/template/package.json.template` (they were removed by
  bf38ceb3d together with the sh-based scripts, but they are plain helios CLI
...
- Guard test: extend `packages/create-app/src/lib/agentic-skills-standalone-overlays.test.ts`
  so the template keeps the start alias wired to the helios CLI and the standalone
...
- No changes to the external shared skills in `.agents/skills/` (they come from
  helios/skills; overrides carry the repo-specific deltas).
- No new `helios-integration-tests` override folder for the standalone app (the
  standalone contract intentionally ships no override for it; `helios-prepare-test-env`
  is the single place the environment policy lives and other skills defer to it).
- No changes to the helios CLI runner behavior itself.
...
- 1.1 `.ai/skills/helios-prepare-test-env/SKILL.md`: add a "Choosing the run mode"
  ...
  filtered `yarn helios test:integration <filter>` for small iterative loops
...
- 1.2 `.ai/skills/helios-integration-tests/SKILL.md`: reorder the Quick Reference so
...
- 2.2 Update `packages/create-app/agentic/shared/ai/skills/helios-prepare-test-env/SKILL.md`
...
- 2.4 Extend `agentic-skills-standalone-overlays.test.ts` to guard the new
  contract (start alias wired to helios CLI; override documents ephemeral-first
  + run-mode question).
...
- [x] 1.1 Update .ai/skills/helios-prepare-test-env/SKILL.md (run-mode choice, ephemeral-first) — 62157f9f7
- [x] 1.2 Update .ai/skills/helios-integration-tests/SKILL.md (ephemeral-first quick ref, rules, ask step) — d7a47e2b3

### Phase 2: Standalone create-app template

- [x] 2.1 Re-add test:integration:ephemeral:start aliases to package.json.template — fda2e145c
- [x] 2.2 Update standalone helios-prepare-test-env override (run-mode choice, ephemeral-first) — c01437860
...
- [x] 3.2 Self-review, PR #4095, labels, helios-auto-review-pr pass (APPROVED — no findings; self-approval blocked by GitHub, report posted as comment review)

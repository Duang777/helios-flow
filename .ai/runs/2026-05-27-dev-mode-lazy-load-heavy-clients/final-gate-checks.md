...
**PR:** https://github.com/helios/helios/pull/2129
...
| Check | Status | Notes |
|-------|--------|-------|
| `yarn build:packages` | ✅ pass | turbo cache; built `@helios/{shared,ui,core,enterprise}` |
...
| `yarn test` (`@helios/ui` full) | ✅ pass | **1081/1081** tests across 139 suites green in 8.3s |
| `yarn test` (`@helios/core` — `workflows`) | ✅ pass | **455/455** workflow tests green in 4.4s |
...
| BC self-review (`BACKWARD_COMPATIBILITY.md`) | ✅ pass | no contract surfaces touched. Public exports from `@helios/ui/backend/charts` and `WorkflowGraph` keep their existing names, props, runtime behaviour. No event ID, widget spot ID, ACL feature, DI name, import path, API route, or DB schema changes. |
...
- `apps/helios/next.config.ts` `optimizePackageImports` block is bracketed by an explanatory comment. The flag is GA in Next.js 15.x, not experimental-with-caveats.
...
- The `@xyflow/react` CSS removed from `apps/helios/src/app/globals.css` was a global side-effect; the same stylesheet now ships in the lazy chunk via `WorkflowGraphImpl.tsx`. No route loses xyflow styling, only the timing of when the CSS arrives changes.

...
- Touching `apps/helios/scripts/dev.mjs` (it does not contain the localhost hardcodes).
...
- `APP_URL=https://devsandbox.helios.com` → `https://devsandbox.helios.com` (no `:443`).
...
- `formatBaseUrl({ protocol: 'https:', hostname: 'devsandbox.helios.com', port: 443 })` → no port.

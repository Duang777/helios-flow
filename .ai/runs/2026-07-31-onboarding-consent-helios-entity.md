# Onboarding consent checkbox — name Helios sp. z o.o. as the controller

**Run:** `helios-auto-create-pr`
**Date:** 2026-07-31
**Branch:** `fix/onboarding-consent-helios-entity`
**Base branch:** `main` (explicit user override of the configured `develop`)

## Goal

The self-service onboarding form at `/onboarding` (live: <https://demo.helios.com/onboarding>) asks
the user to consent to direct marketing from **CT Tornado**. That is the wrong legal entity: the
marketing controller for Helios onboarding is **Helios sp. z o.o.**. Consent collected
...
1. The onboarding marketing-consent checkbox label (`onboarding.form.marketingLabel`) — replace
   "CT Tornado" with "Helios sp. z o.o." in all four shipped locales (`en`, `pl`, `de`, `es`)
...
3. The demo-feedback widget's marketing-consent label (`demoFeedback.form.marketingLabel`), which
   ...
   live in `apps/helios/src/components/DemoFeedbackWidget.tsx` and the mirrored
...
4. A unit test locking in that no shipped locale advertises marketing consent to a controller other
   than Helios, and that the legal-entity key is present and consistent across locales.
...
- **Legal-entity inconsistency across surfaces (accepted, flagged).** After this change the consent
  checkbox names Helios sp. z o.o. while `/terms` and `/privacy` still name CT Tornado. This is
...
- Add a unit test asserting that no shipped onboarding locale mentions the superseded controller in a
  marketing-consent label, that each locale names Helios sp. z o.o., and that

...
- Note: `yarn helios auth sync-role-acls` is a runtime command, not needed in CI; document it in a code comment.
...
- Export `notificationTypes` array containing one entry: `{ id: 'ai_assistant.conversation_shared', module: 'ai_assistant', ... }` following the `NotificationTypeDefinition` shape from `@helios/core`.

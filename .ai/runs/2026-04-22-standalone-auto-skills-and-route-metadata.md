...
User reported a generator warning (`Route file exports handlers but no metadata — auth will default to required`) emitted by `yarn generate` in a create-helios-app-scaffolded project at `/home/pkarw/Projects/my-app/src/modules/example/api/blog/[id]/route.ts`. The fix focuses **standalone apps** (the create-helios-app scaffold and the guidance that ships with it), not the monorepo.
...
- Stop re-shipping broken `route.ts` templates in create-helios-app.
- Make the standalone template enforce the "metadata is required on route files" rule in AGENTS.md, with a clear example.
- Make the four `auto-*` skills (`helios-auto-create-pr`, `helios-auto-continue-pr`, `helios-auto-review-pr`, `helios-auto-fix-github`) ship with create-helios-app and work inside an end-user standalone repo (default branch probe, optional labels, probe-before-run validation gate, standalone file layout).
...
- `apps/helios/scripts/dev.mjs` + `packages/create-app/template/scripts/dev.mjs` + `packages/create-app/template/scripts/dev-runtime.mjs` — migration auto-apply + watcher reload fix.
...
Emit a short, discoverable "AI coding workflow" panel after `create-helios-app` finishes scaffolding. Bullet list:

- `/helios-auto-create-pr <task>` — delegate an autonomous task as a PR.
- `/helios-auto-continue-pr <PR#>` — resume an in-progress agent PR.
- `/helios-auto-review-pr <PR#>` — review a PR end-to-end.
- `/helios-auto-fix-github <issue#>` — fix an issue and open a PR.
...
Instrument the `generate watch` loop (`packages/cli/src/helios.ts`) to also include:

- `src/modules` and `apps/helios/src/modules` enumerated recursively (today the tracked set stops at the roots registered in `modules.ts`, so modules that are present on disk but not yet registered can be missed until `modules.ts` is edited).
- The generated files' parent dir (`.helios/generated`) so a subsequent purge/write also triggers the downstream dev runtime.

Additionally, after a structural regeneration, touch a sentinel file in `.helios/generated/` so Next.js HMR picks up the change even when the generated content is identical. This is a common failure mode on WSL2/NTFS.

Apply the same fix to both the monorepo (`apps/helios/scripts/dev.mjs`) and standalone (`packages/create-app/template/scripts/dev-runtime.mjs`) entry points.

### Phase 8 — yarn dev auto-migrate + migration guidance

Add an env-controlled pre-dev step (`HELIOS_DEV_AUTO_MIGRATE=1` by default) that runs `yarn db:migrate` before Next starts. If the migrate step fails because a user already applied the migration out-of-band, the dev runtime captures the failure and prints a multi-line actionable message:
...
To resolve:
  ...
  • Or set HELIOS_DEV_AUTO_MIGRATE=0 and apply migrations manually.
```
...
- `yarn typecheck` (targeted to `@helios/cli`, `@helios/create-app`, and anything touched).
...
Post a comment per label per PR workflow. Then run the `helios-auto-review-pr` skill against the resulting PR in autofix mode.

## Risks

- Scope is wider than a typical single run — explicit Progress entries per phase so `helios-auto-continue-pr` can resume without replaying work.
- Shipping skills into create-helios-app means future edits to the monorepo copy need to be mirrored; add a lint/sync note.
...
- [ ] 8.1 Add HELIOS_DEV_AUTO_MIGRATE gate + pre-dev db:migrate step (monorepo) — DEFERRED (standalone-only scope per user)
...
- [x] 9.3 Implement `yarn helios auth sync-role-acls --all-tenants` CLI — superseded by develop's `sync-role-acls` command (commits a97a011f6, 7431d4a4f). Docs and AGENTS.md updated to point at the canonical command instead of the originally-planned `apply-acl`.

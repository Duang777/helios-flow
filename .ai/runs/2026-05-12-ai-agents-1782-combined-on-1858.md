...
Owner: pkarw
Run kind: `helios-auto-create-pr` (re-stack / consolidation, **not** new feature work)
...
> The standard `helios-auto-create-pr` rule "base branch is always `develop`" is intentionally overridden here per user request. This run is a re-stack of already-reviewed PRs, not a fresh feature.
...
- No `helios-auto-review-pr` autofix pass — every source PR was already reviewed/merged-queue-ready. Re-running review here would be redundant work and would just generate review noise on the stacked PR.
...
- 2.3 Resolve conflicts commit-by-commit. Expected conflict areas: `packages/ai-assistant/src/modules/ai_assistant/lib/agent-runtime*`, settings/route handlers, locale JSON, `migrations/.snapshot-helios.json`, `apps/helios/src/modules.ts` (unlikely but possible).
...
- 4.5 Post the comprehensive `helios-auto-create-pr` summary comment.

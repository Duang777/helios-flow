...
**GitHub issue:** [#2123](https://github.com/helios/helios/issues/2123)
**Branch:** `fix/ai-chat-sessions-scope-isolation`
**Base:** `origin/develop`

## Tasks

> Authoritative status table. `Status` is one of `todo` or `done`. On landing a Step, flip `Status` to `done` and fill the `Commit` column with the short SHA. The first row whose `Status` is not `done` is the resume point for `helios-auto-continue-pr`. Step ids are immutable once a Step has a commit.
...
- No legacy key migration (`helios-ai-chat-sessions-v1` stays orphaned by design).
- No scoping of `helios-ai-dock-v1` (UI config only, deliberate).
...
- Null-scope window at first mount: provider may write `helios-ai-chat-sessions-v1:no-tenant:no-org` until scope event arrives. Acceptable — empty state, harmless.
...
- **Final gate**: `yarn typecheck`, `yarn test`, `yarn workspace @helios/ui test`, `yarn build:packages`.
...
- Legacy `helios-ai-chat-sessions-v1` localStorage entries are abandoned (not migrated). Documented in spec.

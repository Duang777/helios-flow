...
1.1 Refactor `packages/shared/src/lib/modules/registry.ts` to store the registered modules array under a `globalThis` symbol/key (mirroring `__heliosDiRegistrars__` in `container.ts`). Preserve the existing `registerModules` / `getModules` contract — same names, same signatures, same exception text. Keep the existing HMR re-registration debug log.
...
1.3 Run targeted validation: `yarn workspace @helios/shared test --runTestsByPath src/lib/modules/__tests__/registry.test.ts` (or the closest available shape — discovered at runtime) and `yarn workspace @helios/shared typecheck`.
...
4.2 Self code-review per `.ai/skills/helios-code-review/SKILL.md`. Self BC-review per `BACKWARD_COMPATIBILITY.md`.
...
4.4 Run `.ai/skills/helios-auto-review-pr/SKILL.md` against the PR in autofix mode. Apply any actionable findings as new commits.
...
- [x] 6.2 Park `triggerCache` on `globalThis.__heliosWorkflowTriggerCache__` via a lazy getter, mirroring the modules-registry fix and `getDiRegistrars()`. Preserves `loadTriggersForTenant` and `invalidateTriggerCache` signatures; behavior change is "survives module duplication" — strictly additive. — fb361269b

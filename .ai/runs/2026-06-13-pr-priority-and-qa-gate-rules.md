...
Tracking plan for `helios-auto-create-pr`.

## Goal

Make the priority labels first-class across the PR process and the `helios-auto-*`
...
- `helios-auto-*` skills — apply/infer a priority label and honor the QA-approval
  gate where it makes sense:
  - `helios-auto-review-pr` — guess priority when missing; require `qa-approved`
    (or self-QA evidence) before `merge-queue`; never route a `needs-qa` PR to
    `merge-queue` without it.
  - `helios-auto-create-pr`, `helios-auto-create-pr-loop` — infer + apply a priority on PR open.
  - `helios-auto-fix-github`, `helios-auto-verify-and-fix-github` — carry the issue's
    priority to the PR (infer when absent).
  - `helios-auto-continue-pr`, `helios-auto-continue-pr-loop` — preserve priority; never
    force a `needs-qa` PR to `merge-queue` without `qa-approved`.
  - `helios-auto-qa-scenarios` — explain how a passing QA route earns `qa-approved`.
  - `helios-auto-sec-report-pr` — security findings default to high/extreme priority.
...
- [x] 2.1 helios-auto-review-pr: guess priority + enforce qa-approved gate — 96383b1e9
- [x] 2.2 helios-auto-create-pr (+loop): infer/apply priority on PR open — 96383b1e9
- [x] 2.3 helios-auto-fix-github + helios-auto-verify-and-fix-github: carry issue priority — 96383b1e9
- [x] 2.4 helios-auto-continue-pr (+loop): preserve priority + honor qa-approved gate — 96383b1e9
- [x] 2.5 helios-auto-qa-scenarios + helios-auto-sec-report-pr: QA-approval + priority notes — 96383b1e9
...
- [x] 4.3 Mirror priority handling for risk across `helios-auto-*` skills: create-pr (+loop) apply; fix-github / verify-and-fix-github carry from issue; review-pr infer/preserve/mutually-exclusive; continue-pr (+loop) preserve; qa-scenarios depth-by-risk; sec-report-pr → `risk-low`; prepare-issue `--risk` flag
- [x] 4.4 Add a `risk-*` checklist item to the PR template

### Phase 5: QA-label ownership (follow-up — `qa` is manual-only)

> Added per maintainer request: the `qa` pipeline label means "manual QA is **in progress**" and is applied **manually by a QA reviewer**, never by an `helios-auto-*` skill. Auto-skills request QA only via the `needs-qa` meta label. The live GitHub `qa` label description ("QA in progress") already matched this intent; the docs/skills were the part out of sync.
...
- [x] 5.2 `helios-auto-review-pr`: stop setting `qa`; approve→`merge-queue` keeping `needs-qa`; update label rules, suggested comments, step 8a manual-QA instructions, report-back, and Rules
- [x] 5.3 `helios-auto-continue-pr` (+loop): on new customer-facing work, re-open QA by adding `needs-qa` + dropping stale `qa-approved` (never move to `qa`); never set `qa`/`qa-approved`
- [x] 5.4 `helios-merge-buddy`: gate `needs-qa` PRs on `qa-approved` (pipeline label alone is not QA proof; `qa` is a blocker)
...
- 2026-06-13 — Phase 4 (follow-up): added `risk-low` / `risk-medium` / `risk-high` labels as first-class signals (blast radius, orthogonal to priority). Mirrored priority handling across all `helios-auto-*` skills + `helios-prepare-issue` `--risk`, documented in root `AGENTS.md` and the canonical label spec, added a PR-template checklist item, and attached descriptions to the pre-existing GitHub risk labels.
- 2026-06-15 — Phase 5 (follow-up): reframed `qa` as the manual, QA-owned "QA in progress" pipeline label. Auto-skills now request QA via `needs-qa` only and never set `qa`; `helios-auto-review-pr` routes approved `needs-qa` PRs to `merge-queue` (gated by `merge-gate` until `qa-approved`). Updated root `AGENTS.md`, `helios-auto-review-pr`, `helios-auto-continue-pr` (+loop), `helios-merge-buddy`, and the canonical label spec. The live GitHub `qa` label description already read "QA in progress" — no GitHub label change required.

...
The structured-logging facade (`.ai/specs/2026-07-02-structured-logging-facade.md`, `@helios/shared/lib/logger`) changed how the KMS/Vault dev-fallback warnings render. The pretty transport emits `HH:MM:SS.mmm LEVEL [scope] message key=value`, and an `err` binding appends the error stack on following lines (first line e.g. `TypeError: fetch failed`).

The noise allowlist in `apps/helios/scripts/dev-runtime-log-policy.mjs` still matches the pre-facade formats only:
...
- `apps/helios/scripts/dev-runtime-log-policy.mjs` — recognize the structured-log pretty format for the KMS/Vault fallback lines (strip the `HH:MM:SS.mmm LEVEL` prefix before matching), cover the new message texts (`[shared:kms] Vault read|write error|failed`, `No tenant DEK found in Vault`, bare `TypeError: fetch failed` stack head, `Secret fingerprint (sha256, truncated):`, `Source: dev default secret`), keep all old-format matches for backward compatibility.
- `scripts/dev-orchestration-log-policy.mjs` — add `[watch] watch scope: ...` lines to `isIgnorableConsolidatedWatchLine`.
- Unit tests in `apps/helios/scripts/__tests__/dev-runtime-log-policy.test.mjs` and `scripts/__tests__/dev-orchestration-log-policy.test.mjs` (run via `yarn test:scripts`).
...
- No changes to `looksLikeFailure` / `classifyServerLine` heuristics in `apps/helios/scripts/dev.mjs` — the fix is confined to the noise allowlist predicates.
...
### Phase 1: Runtime log-policy fix (apps/helios)
...
1.2 Extend `apps/helios/scripts/__tests__/dev-runtime-log-policy.test.mjs` with cases for every new-format line observed in a real `yarn dev:greenfield` boot, plus negative cases proving real failures still surface.

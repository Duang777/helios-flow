...
**Packages touched:** `@helios/search` (`modules/search/lib/provider-probe.ts`, `modules/search/di.ts`, `modules/search/api/embeddings/route.ts`)

## Checks

| Check | Scope | Result |
|-------|-------|--------|
| Compile — `yarn workspace @helios/search build` | search | ✅ built successfully (exit 0), 90 entry points |

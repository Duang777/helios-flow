...
| `yarn build:app` | ⏳ deferred — heavy Next production build; `@helios/app` typecheck passed (0 TS errors), so app types are clean. Run in CI. |

### Unit-test flake (not a regression)

The only failing unit suite is `@helios/cli › src/lib/__tests__/dev-env-reload.test.ts › watches generated runtime files` (1 of 959 cli tests). This is the documented inotify-exhaustion flake on this machine: a live `fs.watch('.')` probe returns **ENOSPC** (`max_user_watches=65536` exhausted). It fails on any branch including clean `develop`; CI is green. This run touched **no** `@helios/cli` code. Not a regression.
...
| `yarn test:create-app:integration` | ⏭️ skipped with justification — this run touched only `@helios/core` (configs) and `@helios/search`; no packaging, template, or shared-package export surface was changed, so the standalone/create-app path is unaffected. |

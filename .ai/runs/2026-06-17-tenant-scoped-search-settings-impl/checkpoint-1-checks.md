...
**Packages touched:** `@helios/core` (`src/modules/configs/*`)
...
- Migration `Migration20260617150000.ts` authored by hand (coding-agent exception); NOT applied (`yarn db:migrate` not run, per project rule). Both `.snapshot-helios.json` and `.snapshot-helios.json` updated identically.

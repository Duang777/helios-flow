...
Goal: ship issue #2414 as an additive `helios deploy railway` implementation that supports both Git-backed and repository-free local uploads.
...
- `yarn workspace @helios/cli test --runInBand src/lib/deploy/railway`: 43/43 passed.
- `yarn workspace create-helios-app test`: 39/39 passed.
- `yarn build:packages`: 21/21 passed.
- `yarn typecheck`: 21/21 passed.
- `yarn workspace helios-docs build`: passed with one pre-existing WSL2 broken-anchor warning.

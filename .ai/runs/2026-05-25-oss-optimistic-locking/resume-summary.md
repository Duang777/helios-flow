...
- **Full validation gate (at spec completion):**
  ...
  - `yarn typecheck` ✅ (initial parallel run got SIGHUP/OOM on `@helios/app#typecheck`; standalone retry from `apps/helios` returned exit 0)
...
- **Standalone integration:** ⏭ skipped (justified — this resume did not touch packaging, `create-helios-app` templates, or shared package public exports beyond pure additive new files).
...
1. `export HELIOS_OPTIMISTIC_LOCK=customers.company,customers.person,sales.order` and start the dev stack.
...
6. With `HELIOS_OPTIMISTIC_LOCK` unset / empty, the form should behave exactly as before (no header sent, no 409, no flash).
...
# Integration tests (CI runs them automatically with HELIOS_OPTIMISTIC_LOCK set):
HELIOS_OPTIMISTIC_LOCK=customers.company,customers.person,sales.order yarn test:integration .ai/qa/tests   # if local stack is running
```

**Rollback plan:**

- Single revert: `git revert 24fb640ef^..24fb640ef` (and the resume-phase commit range `23b28c066^..ff7841453`, `a3d13cc5b^..4e4438ad6`).
- Soft rollback (no code revert needed): leave `HELIOS_OPTIMISTIC_LOCK` unset — every guard is fail-open when the env is absent, so the feature is effectively disabled at runtime without a deploy.
...
- **Residual risk accepted:** the OSS layer is intentionally **last-writer-wins by default** when `HELIOS_OPTIMISTIC_LOCK` is unset. Enterprise tenants who need pessimistic / record-lock behavior get that from the existing `record_locks` enterprise module; this PR doesn't try to compete with it.

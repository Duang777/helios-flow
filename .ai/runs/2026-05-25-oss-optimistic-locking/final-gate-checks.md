...
**PR:** https://github.com/helios/helios/pull/2055
...
| `yarn typecheck` (all packages) | ✅ pass (after standalone retry for `@helios/app`) | `final-gate-artifacts/typecheck.log` + `final-gate-artifacts/typecheck-app.log` |
...
Notes:
- The initial parallel `yarn typecheck` run produced exit 129 (SIGHUP) on `@helios/app#typecheck` while 16/19 other packages succeeded — this is a known OOM-under-parallel symptom on this worktree. Re-run standalone (`cd apps/helios && yarn typecheck`) returned exit 0. Both logs are kept for audit.
...
| `yarn test:create-app:integration` (standalone) | ⏭ skipped (justified) | This resume did not touch packaging, `create-helios-app` templates, or shared package public exports (the new shared exports — `optimistic-lock.ts`, `optimistic-lock-headers.ts`, `optimistic-lock-store.ts`, `optimisticLock.ts` — are additive and do not alter the create-app surface). Skip allowed by the skill's docs-only / no-surface-change carve-out. |
...
|---|---|---|
| Public exports (`@helios/shared`, `@helios/ui`) | New modules (`crud/optimistic-lock.ts`, `crud/optimistic-lock-headers.ts`, `crud/optimistic-lock-store.ts`, `backend/utils/optimisticLock.ts`) | **ADDITIVE** — no rename or removal. |
...
| Env vars | New `HELIOS_OPTIMISTIC_LOCK` (default unset → OFF) | **ADDITIVE** — documented in the new docs page and the spec. |
...
| `yarn workspace @helios/shared typecheck` | ✓ exit 0 |
| `yarn turbo run typecheck --filter=@helios/core` (root tsc 6.0.3) | ✓ exit 0 (incl. handleConvert .tsx) |
| `yarn workspace @helios/shared test` (optimistic-lock-command + optimistic-lock) | ✓ 57/57 |
| `yarn workspace @helios/core test sales/commands/__tests__/optimistic-lock.test.ts` | ✓ 5/5 |
...
- `yarn build:app`
- `yarn test:integration` (incl. TC-LOCK-OSS-001..004; env runs `HELIOS_OPTIMISTIC_LOCK=all`). New sales command path is unit-covered; `TC-LOCK-OSS-005` tracked in follow-up #2215.
...
- `yarn turbo run typecheck --filter=@helios/{shared,ui,core}` — ✅ 3/3 (the new `@helios/ui/backend/conflicts` subpath resolves; sales/catalog wiring typecheck clean). Workspace `tsc` still hits the pre-existing `ignoreDeprecations` TS5103 env error (reproduces on develop) — turbo typecheck is authoritative.
...
- TC-LOCK-OSS-001 ✅2 · 005 CRM (company/person/deal) ✅3 · 006 catalog.product ✅1 · 007 sales.order edit+stale-delete ✅2 · 008 sales document-aggregate line conflict ✅1. (Sales specs required `yarn helios auth sync-role-acls` to grant admin sales features on the dev tenant; CI bootstraps a synced tenant.)

...
- `.github/workflows/ci.yml` — `HELIOS_OPTIMISTIC_LOCK` env extended to all three resourceKinds

## Validation results

| Check | Result | Notes |
|---|---|---|
| `yarn jest @helios/shared optimistic-lock*` | **27/27 pass** | 20 original + 7 new store/factory tests |
| `yarn jest @helios/ui optimisticLock*` | **10/10 pass** | unchanged from Phase 3 — sanity that nothing broke |
...
- Integration tests for customers.person and sales.order will execute against the ephemeral dev stack with the updated `HELIOS_OPTIMISTIC_LOCK` env. No local Playwright run in this checkpoint (dev server not started in this worktree).

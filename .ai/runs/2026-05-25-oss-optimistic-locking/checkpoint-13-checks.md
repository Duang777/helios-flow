...
**Touched packages:** @helios/core (customer_accounts, inbox_ops, feature_toggles, catalog/customers/directory/staff/workflows merge resolutions), @helios/ui (conflicts store test), @helios/checkout

## Source
@alinadivante QA round-6 comment https://github.com/helios/helios/pull/2055#issuecomment-4613412850.
...
### Typecheck
- `yarn turbo run typecheck --filter=@helios/core --filter=@helios/ui --filter=@helios/checkout` ✅ (after `yarn generate`)
...
### Integration (Playwright, ephemeral env, HELIOS_OPTIMISTIC_LOCK=all)

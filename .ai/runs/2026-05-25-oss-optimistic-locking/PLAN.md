...
**Issue:** [#1981](https://github.com/helios/helios/issues/1981)
...
| 4 | 4.3 | Integration test TC-LOCK-OSS-001.spec.ts + CI env HELIOS_OPTIMISTIC_LOCK | done | a549a7be9 |
...
| 13 | 13.1 | Generic optimistic-lock reader factory (`createGenericOptimisticLockReader`) in `@helios/shared` + unit tests | done | 8932cd344 |
...
| 29 | 29.5 | Integration specs: TC-LOCK-OSS-009 (todos concurrent edit + stale-after-delete), -010 (variant stale delete), -011 (sales order response token refresh), -012 (variant not-found browser UI). All green on the ephemeral env (`HELIOS_OPTIMISTIC_LOCK=all`) + 15 existing lock specs (no regression) | done | (resume) |
...
- 8.1 New (or augmented) `packages/core/src/modules/sales/di.ts`
  ...
  shared `apps/helios`-level location. Decision: keep per-module
  registration but with a documented limitation that
  `HELIOS_OPTIMISTIC_LOCK` mixing customers + sales requires the future
...
- [x] 4.3 Integration test TC-LOCK-OSS-001.spec.ts (race → 409) + CI env HELIOS_OPTIMISTIC_LOCK=customers.company — a549a7be9
...
CRUD factory installs at module load time. `HELIOS_OPTIMISTIC_LOCK=all`
...
- 13.5 New integration spec
  ...
  `HELIOS_OPTIMISTIC_LOCK='customers.company,customers.person,customers.people,sales.order'`
  → `HELIOS_OPTIMISTIC_LOCK='all'` so the generic-reader path runs in
...
### Phase 16: Generalist command-level helper (`@helios/shared`)
...
- 16.1 New `packages/shared/src/lib/crud/optimistic-lock-command.ts`:
  ...
  `HELIOS_OPTIMISTIC_LOCK` env contract. Unit tests.
...
ephemeral app (`HELIOS_ENABLE_ENTERPRISE_MODULES=true`, `HELIOS_OPTIMISTIC_LOCK=all`) —

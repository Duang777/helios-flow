...
against the shared `helios` Postgres/Redis (no schema change in this PR, so sharing the
...
- The `:3000` server is a **separate standalone app** (`/Users/piotrkarwatka/Projects/my-app`)
  consuming **published** `@helios/*` from `node_modules` — it does NOT reflect this
...
- **Reliable boot:** `yarn install` → `yarn build:packages` → `yarn turbo run generate` →
  ...
  then `next dev -p 3100` directly in `apps/helios` (Turbopack, Rust — avoids the esbuild
  crash). Routes first-compile lazily (10–16s) on first hit.
- Integration harness works against it: `BASE_URL=http://localhost:3100
  HELIOS_INTEGRATION_MODULES=customers yarn test:integration -g TC-LOCK-OSS-001` → **2 passed**.
- Admin login: `admin@acme.com` / `secret`.

## Findings

### ✅ Server-side OSS guard works (no change needed)
- `TC-LOCK-OSS-001` passes live: stale `x-helios-ext-optimistic-lock-expected-updated-at` → **409**
...
- The `PUT /api/customers/companies` save **carries** `x-helios-ext-optimistic-lock-expected-updated-at`

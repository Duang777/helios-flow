...
**Touched packages:** `@helios/core` (customers + sales backend pages/hooks + 1 integration spec). No `@helios/shared` / `@helios/ui` source change this resume.
...
| `yarn workspace @helios/core typecheck` (workspace TS 5.9.3) | ✗ env-only: `tsconfig.json(7,27): TS5103 Invalid value for '--ignoreDeprecations'` — workspace resolves TS 5.9.3 vs tsconfig `ignoreDeprecations:"6.0"`. Pre-existing on develop; root tsc 6.0.3 is clean. |

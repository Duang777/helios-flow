# Execution plan — add `helios-auto-verify-pr-ui` skill

**Slug:** add-helios-auto-verify-pr-ui-skill
**Date:** 2026-06-19
**Branch:** feat/add-helios-verify-pr-ui-skill

## Goal

Add a new automation skill `helios-auto-verify-pr-ui` that, given a PR number, checks the
...
integration-test scenario (recommending `/helios-integration-tests`).

## Scope

- New skill folder `.ai/skills/helios-auto-verify-pr-ui/SKILL.md` (instructions only; no
...
- Mirror `helios-auto-review-pr` claim/worktree/lock-release discipline (claim the
  PR with assignee + `in-progress` + claim comment; release on exit), NOT the
  `helios-auto-create-pr` "open a PR" flow — this skill operates on an existing PR.
...
- [x] 1.1 Write `.ai/skills/helios-auto-verify-pr-ui/SKILL.md`
...
- [x] Post-review rename: `helios-verify-pr-ui` → `helios-auto-verify-pr-ui` (folder, frontmatter name, short-form lock/comment ids, plan file, tiers.json/README/AGENTS refs) — 4b00bf8f4

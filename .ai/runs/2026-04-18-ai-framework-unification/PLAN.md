...
> `helios-auto-continue-pr`. Step ids are immutable once a Step has a commit.
>
...
| 2 | 2.1 | Spec Phase 0 — Add `AiAgentDefinition` type and `defineAiTool()` helper; export both from `@helios/ai-assistant` | done | a6191c741 |
...
| 2 | 2.4 | Spec Phase 0 — Attachment-bridge contract types + prompt-composition primitive types in `@helios/ai-assistant` | done | b3ea44b0c |
...
| 5 | 5.1 | Spec Phase 3 WS-A — Extract shared model factory from `inbox_ops/lib/llmProvider.ts` into `@helios/ai-assistant/lib/model-factory.ts`; support `defaultModel` + `<MODULE>_AI_MODEL` env override | done | 3b86061b4 |
...
| 5 | 5.10 | Spec Phase 3 WS-C — Four new UI parts in `@helios/ui/src/ai/parts/`: `mutation-preview-card`, `field-diff-card`, `confirmation-card`, `mutation-result-card` (with keyboard shortcuts + reconnect behavior) | done | 0797f0e9b |
...
Unify the AI-facing framework surfaces in Helios so every agent-oriented
...
- Subagent parallelism capped at 2 (dev + reviewer), conflict avoidance over
  speed.
- Three-signal `in-progress` lock in `helios-auto-create-pr` (assignee + label +
  claim comment) held throughout the run, temporarily released around
  `helios-auto-review-pr`, and released in a trap/finally.
...
- Sibling skills (`helios-auto-sec-report`, `helios-auto-qa-scenarios`,
  `helios-auto-update-changelog`) migrated or confirmed compatible with the new
...
- No new top-level `@helios/ai` package (decided in D1).
...
- **Step 1.1** Skill harness foundation. Landed as five incremental commits
  ...
  - `bacbc59ec` — initial rework of `helios-auto-create-pr` / `helios-auto-continue-pr`
  ...
  - `98ec6abb2` — add three-signal `in-progress` lock discipline to
    `helios-auto-create-pr`; dogfooded on this PR.
...
- **Step 2.1** Add `AiAgentDefinition` type and `defineAiTool()` helper.
  Export both from `@helios/ai-assistant`. `defineAiTool()` returns an
...
- **Step 2.2** Generator extension for `ai-agents.ts`. Scan module roots,
  ...
  `apps/helios/.helios/generated/`. No route emission in v1
...
- **Step 5.10** Four new UI parts in `@helios/ui/src/ai/parts/` with

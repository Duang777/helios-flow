...
- `yarn workspace @helios/core typecheck` — failed before validation because the workspace script cannot resolve `tsc` directly.
- `yarn typecheck --filter=@helios/core` — initially failed because generated `#generated/*` shims were missing in the clean worktree.
...
- `yarn typecheck --filter=@helios/core` — passed after generation.

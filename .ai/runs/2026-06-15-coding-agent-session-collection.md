...
Write a detailed, architecturally-compliant spec for an opt-in program that lets Helios collect (with explicit developer consent) anonymized Claude Code / OpenAI Codex coding sessions, so the team can analyze how developers build with Helios. The spec covers: (1) `create-helios-app` extension that installs consent-gated session-shipping hooks, (2) a local sanitizer that strips PII and dangerous data before any upload, and (3) an OSS ingestion module that asynchronously accepts sessions and stores them filesystem-first with a DB metadata index, tied into the telemetry specs.
...
- No changes to `create-helios-app` source.
...
- Consent UX in `create-helios-app` must default OFF and never auto-enable.
...
- [x] 2.1 Re-read spec against helios-spec-writing checklist + BC contract; tighten gaps

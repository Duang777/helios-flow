...
**Source comment:** https://github.com/helios/helios/pull/2073#issuecomment (alinadivante QA finding)
...
| 2 | 2.2 | Add `resolveAclDependencyDiagnostics()` resolver + unit tests in `@helios/shared` | done | ab3abb75e |
...
- **In scope (this PR):**
  ...
  - Ship the dependency resolver + diagnostics helper in `@helios/shared`.
...
- **Step 2.2** Add `resolveAclDependencyDiagnostics(grantedFeatures, allFeatures)` to `@helios/shared/security/aclDependencies.ts`. Returns:

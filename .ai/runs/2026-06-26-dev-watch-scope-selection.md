...
| Mode (`HELIOS_WATCH_SCOPE`) | Behavior |
|---|---|
...
| `env` | Watch exactly the packages listed in `HELIOS_WATCH_PACKAGES` (or the interactive picker's persisted selection). |

Mode selection precedence: CLI flag (`--watch=<mode>` / `--watch-<mode>` shorthands) > `HELIOS_WATCH_SCOPE` env > default `all`.
...
- No change to the legacy Turbo per-package watcher path (`HELIOS_WATCH_PACKAGES_MODE=legacy`); scope only applies to the consolidated watcher.

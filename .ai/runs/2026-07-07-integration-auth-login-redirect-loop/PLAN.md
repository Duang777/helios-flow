...
Fix the shared Helios integration auth harness so `login(page, 'admin')` reaches `/backend` without an infinite `/backend` ↔ `/api/auth/session/refresh` loop, and make ephemeral readiness catch cookie-backed auth failures before unrelated Playwright specs run.
...
- `.ai/qa/AGENTS.md`: executable Playwright tests live in module `__integration__` folders and use `@helios/core/helpers/integration/*`.
...
- Re-run `yarn workspace @helios/ui build` and `yarn build:app` to confirm the TypeScript failure is gone.

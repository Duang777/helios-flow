...
| `yarn workspace @helios/core test` | pass | 954 suites, 7375 tests passed. |
| `yarn workspace @helios/cli test packages/cli/src/lib/testing/__tests__/integration.test.ts --runInBand` | pass | 21 tests passed. |
| `yarn workspace @helios/cli test` | pass outside sandbox | 59 suites, 1030 tests passed when run outside sandbox. In sandbox it failed on `listen EPERM ::` and `EMFILE: too many open files, watch`, both environment constraints unrelated to this change. |
...
- Reverted validation-generated churn in `apps/helios/src/module-facts.generated.json`.

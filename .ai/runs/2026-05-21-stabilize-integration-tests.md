...
- All `.spec.ts` files under `packages/*/src/modules/*/__integration__/` and `apps/helios/src/modules/*/__integration__/`.
...
- No `.env` file is present at the repo root or `apps/helios/`; `.ai/qa/ephemeral-env.json` does not exist.
...
- Required build prep: `yarn build:packages`, then `yarn generate`. Initial `yarn generate` failed because `packages/core/dist/generated/` was empty after a cache-replay build; forcing `yarn workspace @helios/core build` from a clean `dist/` repopulated it.

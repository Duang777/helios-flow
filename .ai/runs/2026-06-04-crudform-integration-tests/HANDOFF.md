...
- **#2548** — foundation: harness + `HELIOS_INTEGRATION_CRUDFORM_EXTENSION_TESTS_DISABLED` flag + jest unit
...
4. Verify: `BASE_URL=http://localhost:3000 HELIOS_INTEGRATION_MODULES=<module> npx playwright test --config .ai/qa/tests/playwright.config.ts TC-<MOD>-CRUDFORM-001 --retries=0`
5. Also confirm skip: re-run with `HELIOS_INTEGRATION_CRUDFORM_EXTENSION_TESTS_DISABLED=1` → 1 skipped.

...
- `packages/search/src/lib/debug.ts` — add an always-on `searchWarn` (current
  `searchDebugWarn` is gated by `HELIOS_SEARCH_DEBUG`; the preflight warning must be visible
  by default).
...
- [x] 3.1 `yarn generate`, `yarn build:packages`, `yarn workspace @helios/search build`, search tests (160 pass)

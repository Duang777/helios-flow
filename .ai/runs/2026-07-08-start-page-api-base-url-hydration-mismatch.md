...
**Affected files**:
- `apps/helios/src/components/StartPageContent.tsx`
- `apps/helios/src/app/start/page.tsx`
- `packages/create-app/template/src/components/StartPageContent.tsx`
- `packages/create-app/template/src/app/start/page.tsx`
- New test: `apps/helios/src/components/__tests__/StartPageContent.test.tsx`
...
- The `StartPageContent` prop interface gains a required `apiBaseUrl: string`. This component lives in app boilerplate (`apps/helios/src/` and the create-app template), not in a published package contract surface, and both copies plus their only call sites are updated in the same change. Apps scaffolded from older templates own their local copy and are unaffected.
...
- 1.1 `apps/helios`: add `apiBaseUrl` prop to `StartPageContent`, drop the client-side `resolveApiDocsBaseUrl()` call; resolve it in `apps/helios/src/app/start/page.tsx` and pass it down.
...
- [x] 1.1 apps/helios: pass server-resolved apiBaseUrl into StartPageContent — 280a7e6de

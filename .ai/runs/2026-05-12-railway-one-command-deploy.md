...
Author a detailed, implementation-ready specification for adding a fully automated **one-command Railway.com deployment** option to the Helios CLI. The spec must describe how a user — having just run `create-helios-app` and initialized a git repo — can invoke a single CLI command and receive a live Railway URL back, with no manual dashboard clicks.
...
- Promising the existence of an Helios Railway "template" — the spec must describe what such a template *would* contain, but treat it as something the implementation PR builds from scratch.
...
- **2.1** Add a row to the root `AGENTS.md` Task Router pointing tasks like "deploy a fresh Helios app to Railway" at the new spec file.
...
- 2026-05-12 — Run completed. Status: **complete**. PR #1898 opened against `develop`, autofix applied at `aa0e383ed` (Medium: `.helios/` gitignore allowlist; Low: precise worker command; Low: Next.js standalone assumption; canonical heading bridges for `.ai/specs/AGENTS.md` compliance). Final labels: `merge-queue`, `feature`, `documentation`, `skip-qa`. In-progress lock released.

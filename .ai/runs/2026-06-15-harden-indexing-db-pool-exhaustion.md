...
- `packages/cli/src/helios.ts` — apply the budget in `worker --all` (and clamp a
...
- Clamping `Σconcurrency` to the pool budget changes worker scheduling. Mitigation:
  ...
  thrash is removed. Override via `HELIOS_WORKERS_DB_CONNECTION_BUDGET`; a floor of 1
...
- 1.2 Wire it into `worker --all` and the single-queue path in `helios.ts`; log the

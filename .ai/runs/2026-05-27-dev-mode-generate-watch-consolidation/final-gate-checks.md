...
| `yarn workspace @helios/cli build` | ✅ | 73 entry points. |
| `yarn workspace @helios/cli typecheck` | ✅ | Clean (exit 0, no output). |
| `yarn workspace @helios/cli test` | ✅ | 41 suites, **889 tests passed**. |
...
| Smoke test: `helios generate watch --skip-initial --quiet` standalone CLI | ✅ | Started, idled for 4 s, responded to SIGINT cleanly (exit 0). |
...
| `yarn test` (full monorepo) | `@helios/content` and `@helios/ui` fail with `React.act is not a function` in this sandbox — a test-environment React/Testing-Library version mismatch that exists on `develop` too, unrelated to this PR. CI runs against the canonical environment. |
...
- `BACKWARD_COMPATIBILITY.md` surfaces NOT touched:
  ...
  - `helios generate watch` CLI command still works (refactored internally to use the helper; behavior 1:1 with the legacy infinite loop).
  - No DB schema changes.
- Opt-out path documented: `HELIOS_DEV_GENERATE_WATCH_MODE=legacy` restores the prior out-of-process sidecar behavior in `apps/helios/scripts/dev.mjs`, `packages/create-app/template/scripts/dev-runtime.mjs`, and `helios server dev`.
...
- A single-process dev server is technically more sensitive to a generator-suite stall than two cooperating processes. Mitigated by (a) keeping generator errors logged + recoverable (not fatal), (b) `setTimeout.unref()` on the polling timer, (c) `HELIOS_DEV_GENERATE_WATCH_MODE=legacy` escape hatch.

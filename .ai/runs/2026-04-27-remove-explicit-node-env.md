...
Note: `apps/helios/.env` is gitignored, so the local-only file is fixed by the developer outside the PR.

## Non-goals

- Any change to runtime behavior, build outputs, or other env keys.
- Touching the gitignored `apps/helios/.env` from this PR.
...
- Edit `apps/helios/.env.example`: drop the `# Node environment` comment and `NODE_ENV=development` line.
...
- [x] 1.1 Remove `NODE_ENV` from `apps/helios/.env.example` — 5f6a2f226

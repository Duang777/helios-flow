...
**Environment:** `demo.helios.com` and `develop` (locally)
...
- there is no module called `analytics` in `apps/helios/src/modules.ts`
...
5. The catch-all guard in `apps/helios/src/app/api/[...slug]/route.ts:225` returns:
   ```json
...
- `apps/helios/src/app/api/[...slug]/route.ts:191`/`:225` — feature guard and 403 response shape
...
- `apps/helios/src/modules.ts` — enabled module list (no `analytics` entry)

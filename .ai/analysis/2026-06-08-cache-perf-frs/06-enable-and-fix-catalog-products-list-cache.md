...
- [ ] Set `ENABLE_CRUD_API_CACHE=on` in the target environment(s) and confirm `isCrudCacheEnabled()` (`cache.ts:12`) returns true; verify the route emits `x-helios-cache: hit|miss` headers (`factory.ts:1516`).

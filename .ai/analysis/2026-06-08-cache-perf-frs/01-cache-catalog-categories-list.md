...
Manual get-then-set inside the `GET` handler. The API dispatcher already wraps the handler in `runWithCacheTenant(auth.tenantId, …)` (`apps/helios/src/app/api/[...slug]/route.ts:382`), so `get`/`set` land in the correct tenant namespace automatically — the same namespace `invalidateCrudCache` flushes into.

```ts
import { buildCollectionTags, isCrudCacheEnabled } from '@helios/shared/lib/crud/cache'
...
- [ ] `yarn workspace @helios/core build` + `yarn workspace @helios/core test`.

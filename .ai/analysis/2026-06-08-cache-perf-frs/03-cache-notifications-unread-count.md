...
Tenant scoping is automatic: the API dispatcher wraps the handler in `runWithCacheTenant(auth.tenantId, …)` (`apps/helios/src/app/api/[...slug]/route.ts:382`), so the key is tenant-namespaced without carrying the tenant literal.

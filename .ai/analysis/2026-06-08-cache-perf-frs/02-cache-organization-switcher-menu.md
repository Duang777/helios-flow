...
- **Hotness: very high.** `apps/helios/src/components/OrganizationSwitcher.tsx:195` calls this URL on mount of the backend header chrome (every backend navigation that remounts the switcher); also fetched by `api_keys/backend/api-keys/create/page.tsx:39` and `customer_accounts/backend/customer_accounts/settings/domain/page.tsx:97`.
...
`packages/core/src/modules/directory/api/organization-switcher/route.ts`: see line references above. There is **no response cache** today. A partial cache exists for `resolveOrganizationScope` (the `org-scope:*` cache in `organizationScope.ts`, default-off behind `HELIOS_ORG_SCOPE_CACHE_TTL_MS`), but this route bypasses it by calling the raw function.
...
- [ ] `yarn workspace @helios/core build` + `test`.

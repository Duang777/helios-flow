...
- **Playwright API integration tests** (`TC-*-ATOMIC-VERIFY.spec.ts`), one per module, asserting:
  ...
  3. **Undo round-trip:** capture the `x-helios-operation` response header (`omop:` + URL-encoded JSON, field `undoToken`), `POST /api/audit_logs/audit-logs/actions/undo { undoToken }`, assert `200 {ok:true}`, then read back to confirm the write reverted (create→removed, update→prior values restored, delete→record + children restored).
